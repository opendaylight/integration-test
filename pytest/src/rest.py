...
def check_status_code(
    ...
    final_allowed_list = allowed_status_codes_list
...
def put_to_uri(
    ...
    if http_timeout == "":
        ...
        response = session.put(
            url=url, data=data, headers=headers, timeout=int(http_timeout)
        )
    ...
    text_normalized = norm_json.normalize_json_text(
        text=response.text, jmes_path=jmes_path
    )
    return (text_normalized, status_code)
...
def post_to_uri(
    ...
    normalize_json=False,
    jmes_path="",
    additional_allowed_status_codes=NO_STATUS_CODES,
...
def get_as_xml_from_uri(
    url, session=default_session, http_timeout="", log_response=True
):
    """Specify XML headers and return get_from_uri response text"""
    response_text, status_code = get_from_uri(
        url=url,
        accept=ACCEPT_XML,
        session=session,
        normalize_json=False,
...
def post_as_xml_to_uri(url, data, sesion=default_session, http_timeout=""):
    ...
    response_text, status_code = post_to_uri(
        url=url,
        data=data,
        accept=ACCEPT_XML,
...
def put_as_xml_to_uri(url, data, session=default_session, http_timeout=""):
    ...
    response_text, status_code = put_to_uri(
        url=url,
        data=data,
        accept=ACCEPT_XML,
        content_type=HEADERS_XML,
...
def resolve_text_from_template_file(
    folder, file_name, mapping={}, percent_encode=False, ODL_STREAM="Sulfur-SR1"
):
...
def resolve_text_from_template_folder(
    folder,
    name_prefix="",
    base_name="data",
    extension="json",
    ...
    if not iterations:
        return resolve_text_from_template_file(
            folder=folder,
...
def verify_response_templated(
    ...
    if normalize_json:
        return normalize_jsons_and_compare(
            expected_raw=expected_text, actual_raw=response
        )
    else:
        return expected_text == response


def verify_response_as_xml_templated(
    response,
    folder,
    base_name="response",
    mapping={},
    ...
    return verify_response_templated(
        response=response,
        folder=folder,
        base_name=base_name,
        extension="xml",
...
def verify_response_as_json_templated(
    response,
    folder,
    base_name="response",
    mapping={},
...
def get_as_xml_templated(
    ...
    response_text, status_code = get_templated(
        folder=folder,
        mapping=mapping,
        accept=ACCEPT_XML,
        session=session,
...
def put_as_xml_templated(
    ...
    response_text, status_code = put_templated(
        folder=folder,
        base_name="data",
        extension="xml",
        accept=ACCEPT_XML,
...
def post_as_xml_templated(
    ...
    response_text, status_code = post_templated(
        folder=folder,
        base_name="data",
        extension="xml",
        accept=ACCEPT_XML,

...
Create_Limiting_Stability_Safe_Stateful_Validator_From_Value_To_Overcome
    ...
    ${validator} =    ScalarClosures.Create_Closure_From_Keyword_And_Arguments    WaitUtils.Limiting_Stability_Safe_Stateful_Validator_As_Keyword    state_holder    data_holder    valid_minimum=${valid_minimum}
    [Return]    ${validator}
...
WaitUtils__Is_Deadline_Reachable
    ...
    # TODO: Sensible default for deadline?
    ${date_now} =    DateTime.Get_Current_Date
    ...
    BuiltIn.Run_Keyword_If    ${time_minimal} >= ${time_deadline}    BuiltIn.Fail    Not possible to succeed within deadline. ${message}
...
Stateless_Assert_Closure_Has_To_Succeed_Consecutively_By_Deadline
    ...
    [Documentation]    Pass only if ${assertor} passes ${count} times in a row with ${period_in_seconds} between attempts; less standart arguments.
...
Propagate_Fail_If_Message_Starts_With_Prefix
    ...
    [Documentation]    Helper keyword to distinguish escalable failures by their prefix. If it is escalable, Fail without changing the message.
...
Wait_For_Getter_And_Safe_Stateful_Validator_Consecutive_Success
    ...
    \    # Are we out of time?
    \    Propagate_Fail_If_Message_Starts_With_Prefix    ${result}    Not possible to succeed within deadline.

...
*** Keywords ***
Docker Ovs Start
    [Arguments]    ${nodes}    ${guests}    ${odl_ip}    ${log_file}=myFile2.log
    [Documentation]    Run the docker-ovs.sh script with specific input arguments. Run ./docker-ovs.sh --help for more info.
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/dovs/;sudo ./dovs.py spawn --nodes=${nodes} --guests=${guests} \ --odl=${odl_ip} > >(tee ${log_file}) 2> >(tee ${log_file})    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}

...
Docker Ovs Start
    ...
    Run Keyword If    '${nets}'=='0'    ${result}    SSHLibrary.Execute Command    cd sfc-docker/dovs/;sudo ./dovs.py -d spawn --nodes=${nodes} --guests=${guests} \ --odl=${odl_ip} > >(tee ${log_file}) 2> >(tee ${log_file})    return_stderr=True    return_stdout=True
    ...    return_rc=True
    Run Keyword If    '${nets}'!='0'    ${result}    SSHLibrary.Execute Command    cd sfc-docker/dovs/;sudo ./dovs.py spawn -d --nodes=${nodes} --guests=${guests} \ --nets=${nets} --odl=${odl_ip} > >(tee ${log_file}) 2> >(tee ${log_file})    return_stderr=True    return_stdout=True
    ...    return_rc=True
    log    ${result}

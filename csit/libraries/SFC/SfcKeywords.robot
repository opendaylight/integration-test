...
Get Rendered Service Path Name
    ...
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${sfp_state_list} =    Collections.Get_From_Dictionary    ${resp.json()}    key=sfp-rendered-service-path
...
Create Sfp And Wait For Rsp Creation
    [Arguments]    ${sfp_file_name} ${created_rsps}= Create List    None
    ...
    Run Keyword If    "${created_rsps_length}" > "0"    Wait Until Keyword Succeeds    60s    2s    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}    ${created_rsps}
...
Delete Sfp And Wait For Rsps Deletion
    ...
    Remove All Elements At URI And Verify    ${SERVICE_FUNCTION_PATHS_URI}/service-function-path/${sfp_name}

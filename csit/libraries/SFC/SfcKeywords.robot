...
Get Rendered Service Path Name
    ...
    ${sfp_state_dict} =    Collections.Get_From_List    ${resp.json()}[service-function-path-state]    0
    # TODO this log statement is temporary
    BuiltIn.Log    ${sfp_state_dict}
    ${sfp_state_list} =    Collections.Get_From_Dictionary    ${sfp_state_dict}    sfp-rendered-service-path
    # TODO this log statement is temporary
...
Create Sfp And Wait For Rsp Creation
    ...
    ${created_sfps_length} =    BuiltIn.Get Length    ${created_sfps}
    Run Keyword If    "${created_sfps_length}" > "0"    Wait Until Keyword Succeeds    60s    2s    Check For Elements At URI    ${SERVICE_FUNCTION_PATHS_STATE_URI}
    ...    ${created_sfps}

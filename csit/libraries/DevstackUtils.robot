...
Devstack Suite Setup
    ...
    Write Commands Until Prompt    sed -i 's/echo "Running devstack"/echo "Running devstack"\nsudo ovs-vsctl add-br br-int/' /opt/stack/new/devstack-gate/devstack-vm-gate.sh
    ${output}=    Write Commands Until Prompt    cat /opt/stack/new/devstack-gate/devstack-vm-gate.sh
    LOG    ${output}
        ${output}=    Write Commands Until Prompt    cat /opt/stack/new/devstack/stack.sh
    LOG    ${output}
    Write Commands Until Prompt    export DEVSTACK_LOCAL_CONFIG+="ODL_NETVIRT_DEBUG_LOGS=True;ODL_RELEASE=${odl_version_to_install};PHYSICAL_NETWORK=dr-external;ODL_NETVIRT_KARAF_FEATURE=odl-base-all,odl-restconf-all,odl-aaa-authn,odl-dlux-core,odl-mdsal-apidocs,odl-adsal-northbound,odl-nsf-all,odl-ovsdb-northbound"

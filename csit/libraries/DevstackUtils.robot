...
Run Tempest Tests
    ...
    Write Commands Until Prompt    sudo testr list-tests | egrep ${tempest_regex} | egrep -v ${tempest_exclusion_regex} > tests_to_exclude.txt
    ${tests_to_exclude}=    Write Commands Until Prompt    sudo cat tests_to_exclude.txt
    Log    ${tests_to_exclude}
    # From Ocata and moving forward, we can replace 'ostestr' with 'tempest run'
    ${results}=    Write Commands Until Prompt    ostestr --regex ${tempest_regex} --blacklist_file tests_to_exclude.txt    timeout=${timeout}
    Log    ${results}
    # Save stdout to file

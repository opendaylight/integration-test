...
Run Tempest Tests
    ...
    ${results}=    Run Keyword If    "${openstack_branch}" != "stable/mitaka" and "${skip}" != "yes"    BuiltIn.Evaluate    Write Commands Until Prompt    sudo -E ${tempest_directory}/run_tempest.sh -C ${tempest_conf} -N ${tempest_regex} -- --load-list tests_to_execute.txt    timeout=${timeout}
    ...    ELSE    BuiltIn.Evaluate    "SKIP - it being a bug that was fixed in Newton, but not Mitaka v1 so skipping to avoid the noise of a failure that will never be resolved"
    Log    ${results}
    # Save stdout to file

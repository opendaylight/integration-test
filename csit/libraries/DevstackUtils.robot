...
Devstack Suite Setup
    ...
    ${OS_CONTROL_NODE_CXN}=     Run Keyword If     0 < ${NUM_OS_SYSTEM}       Get Ssh Connection     ${OS_CONTROL_NODE_IP}     control_1
    Run Keyword If     0 < ${NUM_OS_SYSTEM}       Set Suite Variable    ${OS_CONTROL_NODE_CXN}
    ${OS_COMPUTE_1_CXN}=     Run Keyword If     1 < ${NUM_OS_SYSTEM}       Get Ssh Connection     ${OS_COMPUTE_1_IP}     compute_1
    Run Keyword If     1 < ${NUM_OS_SYSTEM}       Set Suite Variable    ${OS_COMPUTE_1_CXN}
    ${OS_COMPUTE_2_CXN}=     Run Keyword If     2 < ${NUM_OS_SYSTEM}       Get Ssh Connection     ${OS_COMPUTE_2_IP}     compute_2
...
Get Ssh Connection
    [Arguments]    ${os_ip}     ${os_alias}
    ${conn_id}=    SSHLibrary.Open Connection    ${os_ip}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=1 hour     alias=${os_alias}
    [Return]    ${conn_id}

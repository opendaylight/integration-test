...
Run Tempest Tests Python
    ...
    SSHLibrary.Set Client Configuration    prompt=pdb.set_trace()
    ${results}=    Write Commands Until Prompt    python -m testtools.run ${tempest_regex}    timeout=${timeout}
    SSHLibrary.Set Client Configuration    prompt=${previous_prompt}
    Log    ${results}
    # Save stdout to file
...
Run Tempest Tests Tempest Run
    ...
    # From Ocata and moving forward, we can replace 'ostestr' with 'tempest run'
    Modify Config In File On Existing SSH Connection    ${tempest_conf}    set    DEFAULT    pause_teardown    True
    ${results}=    Write Commands Until Prompt    tempest run ${tempest_regex}    timeout=${timeout}
    Log    ${results}
    # Save stdout to file
    Create File    tempest_output_${tempest_regex}.log    data=${results}
    # output tempest generated log file which may have different debug levels than what stdout would show

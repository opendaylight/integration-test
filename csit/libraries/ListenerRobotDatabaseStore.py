...
class ListenerRobotDatabaseStore:
    ...
    def insert_into_table(self, status, startTime, elapsedTime, critical, testName, jobName):
        '''
            Insert the required data into robot_results table
        '''
        try:
            self.cursor.execute("INSERT INTO scripts_Robot_Results(job_name,status,elapsed_time,start_time,critical,test_name)" +
                                "VALUES" + "(%s,%s,%s,%s,%s,%s)", (jobName, status, elapsedTime, startTime, critical, testName))
            self.conn.commit()
    ...
    def end_test(self, name, attrs):
        ...
        try:
            self.insert_into_table(
                status, startTime, elapsedTime, critical, testName, jobName)
        except self.conn.DatabaseError, error:
            if self.conn:
                self.conn.rollback()
            print 'Error %s' % error

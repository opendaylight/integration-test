...
Create_Default_Session
    [Arguments]    ${url}=http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    ${headers}=${EMPTY}    ${auth}=${AUTH}    ${timeout}=1    ${max_retries}=0
    ...
    RequestsLibrary.Create_Session    alias=default    url=${url}    headers=${headers}    auth=${auth}    timeout=${timeout}    max_retries=${max_retries}

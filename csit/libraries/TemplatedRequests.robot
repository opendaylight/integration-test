...
...               automatically (not by provided mapping) with integers starting with ${iter_start}.
...
...
...               so that negative tests do not need to parse failure message.
...
...
Put_Templated
    ...
    ${uri} =    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=location    extension=uri    mapping=${mapping}
    ${data} =    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=${base_name}    extension=${extension}    mapping=${mapping}    endline=${endline}    iterations=${iterations}    iter_start=${iter_start}
...
Verify_Response_Templated
    ...
    ${expected_text} =    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=${base_name}    extension=${extension}    mapping=${mapping}    endline=${endline}    iterations=${iterations}    iter_start=${iter_start}
...
Resolve_Text_From_Template_Folder
    [Arguments]    ${folder}    ${name_prefix}=${EMPTY}    ${base_name}=data    ${extension}=json    ${mapping}={}    ${iterations}=${EMPTY}    ${iter_start}=1
    ...    ${endline}=${\n}

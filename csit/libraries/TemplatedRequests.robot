...
Get_As_Json_Templated
    ...
    ...    ${http_timeout}=${EMPTY}    ${log_response}=True    ${j_offset}=0
    ...
    ...    http_timeout=${http_timeout}    log_response=${log_response}    j_offset=${j_offset}
    ...
    ...    iterations=${iterations}    iter_start=${iter_start}    j_offset=${j_offset}
    [Return]    ${response_text}

Get_As_Xml_Templated
    [Arguments]    ${folder}    ${mapping}={}    ${session}=default    ${verify}=False    ${iterations}=${EMPTY}    ${iter_start}=1
    ...    ${http_timeout}=${EMPTY}    ${j_offset}=0
    ...
    ...    http_timeout=${http_timeout}    j_offset=${j_offset}
...
Put_As_Json_Templated
    ...
    ...    http_timeout=${http_timeout}    j_offset=${j_offset}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=response    mapping=${mapping}    j_offset=${j_offset}
    [Return]    ${response_text}

Put_As_Xml_Templated
    [Arguments]    ${folder}    ${mapping}={}    ${session}=default    ${verify}=False    ${iterations}=${EMPTY}    ${iter_start}=1
    ...    ${http_timeout}=${EMPTY}    j_offset=${j_offset}
    ...
    ...    http_timeout=${http_timeout}    ${j_offset}=0
...
Post_As_Json_Templated
    [Arguments]    ${folder}    ${mapping}={}    ${session}=default    ${verify}=False    ${iterations}=${EMPTY}    ${iter_start}=1
    ...    ${additional_allowed_status_codes}=${NO_STATUS_CODES}    ${explicit_status_codes}=${NO_STATUS_CODES}    ${http_timeout}=${EMPTY}    ${j_offset}=0
    ...
    ...    mapping=${mapping}    session=${session}    normalize_json=True    endline=${\n}    iterations=${iterations}    iter_start=${iter_start}
    ...    additional_allowed_status_codes=${additional_allowed_status_codes}    explicit_status_codes=${explicit_status_codes}    http_timeout=${http_timeout}    j_offset=${j_offset}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=response    mapping=${mapping}    j_offset=${j_offset}
    [Return]    ${response_text}
...
Post_As_Xml_Templated
    ...
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Xml_Templated    response=${response_text}    folder=${folder}    base_name=response    mapping=${mapping}    j_offset=${j_offset}
    [Return]    ${response_text}
...
Verify_Response_As_Json_Templated
    [Arguments]    ${response}    ${folder}    ${base_name}=response    ${mapping}={}    ${iterations}=${EMPTY}    ${iter_start}=1    ${j_offset}=0
...
Verify_Response_As_Xml_Templated
    ...
    ...    endline=${EMPTY}    iterations=${iterations}    iter_start=${iter_start}    j_offset=${j_offset}
...
Put_Templated
    [Arguments]    ${folder}    ${base_name}    ${extension}    ${content_type}    ${accept}    ${mapping}={}    ${j_offset}=0
...
Post_Templated
    ...
    ...    ${explicit_status_codes}=${NO_STATUS_CODES}    ${http_timeout}=${EMPTY}    ${j_offset}=0
    ...
    ...    endline=${endline}    iterations=${iterations}    iter_start=${iter_start}    j_offset=${j_offset}
    ${jmes_expression} =    Resolve_Jmes_Path    ${folder}
...
Verify_Response_Templated
    ...
    ...    ${endline}=${\n}    ${iterations}=${EMPTY}    ${iter_start}=1    ${j_offset}=0
...
Resolve_Text_From_Template_Folder
    ...
    ...    Template variable ${j} is ${i} moved by ${j_offset} ( j = i + j_offset ) to create non uniform data for validating UPDATE operations.
    ...
    FOR    ${iteration}    IN RANGE    ${iter_start}    ${iterations}+${iter_start}
        ...
        ${item} =    BuiltIn.Evaluate    string.Template('''${item_template}''').substitute({"i":"${iteration}", "j":${iteration}+${j_offset}})    modules=string

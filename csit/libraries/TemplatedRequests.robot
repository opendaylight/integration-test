...
Get_As_Json_Templated
    [Arguments]    ${folder}    ${mapping}={}    ${session}=default    ${verify}=False    ${iterations}=
    ...
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=data    extension=json    mapping=${mapping}    iteration=${iterations}
...
Get_As_Xml_Templated
    ...
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=data    extension=xml    mapping=${mapping}    iteration=${iterations}
    [Return]    ${response_text}
...
Put_As_Json_Templated
    ...
    ${response_text} =    Put_Templated    folder=${folder}    base_name=data    extension=json    accept=${ACCEPT_JSON}    content_type=${HEADERS_YANG_JSON}    mapping=${mapping}    session=${session}    normalize_json=True    iterations=${iterations}    endline=${\n}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=response    extension=json    mapping=${mapping}    iteration=${iterations}
    [Return]    ${response_text}
...
Put_As_Xml_Templated
    ...
    ${response_text} =    Put_Templated    folder=${folder}    base_name=data    extension=xml    accept=${ACCEPT_XML}    content_type=${HEADERS_XML}    mapping=${mapping}    session=${session}    normalize_json=False    iterations=${iterations}    endline=${\n}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Xml_Templated    response=${response_text}    folder=${folder}    base_name=response    extension=xml    mapping=${mapping}    iteration=${iterations}
    [Return]    ${response_text}
...
Post_As_Json_Templated
    ...
    ...    Optionally, verification against response.json is called.
    ${response_text} =    Post_Templated    folder=${folder}    base_name=data    extension=json    accept=${ACCEPT_JSON}    content_type=${HEADERS_YANG_JSON}    mapping=${mapping}    session=${session}    normalize_json=True    iterations=${iterations}    endline=${\n}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Json_Templated    response=${response_text}    folder=${folder}    base_name=response    extension=json    mapping=${mapping}    iteration=${iterations}
    [Return]    ${response_text}
...
Post_As_Xml_Templated
    ...
    ...    Optionally, verification against response.json is called.
    # In case of iterations, we use endlines in data to send, as it should not matter and it is more readable.
    ${response_text} =    Post_Templated    folder=${folder}    base_name=data    extension=xml    accept=${ACCEPT_XML}    content_type=${HEADERS_XML}    mapping=${mapping}    session=${session}    normalize_json=False    iterations=${iterations}    endline=${\n}
    BuiltIn.Run_Keyword_If    ${verify}    Verify_Response_As_Xml_Templated    response=${response_text}    folder=${folder}    base_name=response    extension=xml    mapping=${mapping}    iteration=${iterations}
    [Return]    ${response_text}
...
Verify_Response_As_Json_Templated
    [Arguments]    ${response}    ${folder}    ${base_name}=response    ${mapping}={}    iterations=
    ...
    Verify_Response_Templated    response=${response}    folder=${folder}    base_name=${base_name}    extension=json    mapping=${mapping}    normalize_json=True    iterations=${iterations}    ${endline}=${\n}
...
Verify_Response_As_Xml_Templated
    ...
    Verify_Response_Templated    response=${response}    folder=${folder}    base_name=${base_name}    extension=xml    mapping=${mapping}    normalize_json=False    iterations=${iterations}    endline=
...
Put_Templated
    [Arguments]    ${folder}    ${base_name}    ${extension}    ${content_type}    ${accept}    ${mapping}={}    ${session}=default    ${normalize_json}=False    ${iterations}=    ${endline}=${\n}
    [Documentation]    Resolve URI and data from folder, call Put_To_Uri, return response text.
    ${uri} =    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=location    extension=uri    mapping=${mapping}
    ${data} =    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=${base_name}    extension=${extension}    mapping=${mapping}    iterations=${iterations}    endline=${endline}
...
Verify_Response_Templated
    [Arguments]    ${response}    ${folder}    ${base_name}    ${extension}    ${mapping}={}    normalize_json=False    ${iterations}=    ${endline}=${\n}
    ...
    ${expected_text}=    Resolve_Text_From_Template_Folder    folder=${folder}    base_name=${base_name}    extension=${extension}    mapping=${mapping}    iterations=${iterations}    endline=${endline}
...
Resolve_Text_From_Template_Folder
    [Arguments]    ${folder}    ${base_name}=data    ${extension}=json    ${mapping}={}    ${iterations}=    ${endline}=${\n}
    ...
    \    BuiltIn.Run_Keyword_If    ${iteration} > 0    Collections.Append_To_List    ${items}    ${separator}
    ...
    ${final_text} =    BuiltIn.Catenate    SEPARATOR=    ${prolog}    ${endline}    ${items}    ${endline}    ${epilog}
    [Return]    ${final_text}

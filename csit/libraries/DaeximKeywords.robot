...
Cleanup The Export Files
    ...
    ${host_index}    Builtin.Convert To Integer    ${host_index}
    ${output1}    ClusterManagement.Run Bash Command On Member    sudo rm -rf ${WORKSPACE}/${BUNDLEFOLDER}/daexim;clear    ${host_index}
...
Verify Netconf Mount
    ...
    ${session} =    Resolve_Http_Session_For_Member    member_index=${host_index}
    ${resp} =    Builtin.Wait Until Keyword Succeeds    120 seconds    5 seconds    TemplatedRequests.Get_As_Json_From_Uri    uri=${NTCF_TPLG_OPR_URL}${endpoint}    session=${session}
    ${output1}    Builtin.Set Variable    ${resp}
    ${output}    Builtin.evaluate    json.loads('''${output1}''')    json
    Builtin.Log    ${output}
    ...
    Builtin.Should Be Equal    ${status}    ${NTCF_OPR_STATUS}

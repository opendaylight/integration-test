...
Configure ODL Clustering
    ...
    Switch Connection     ${os_node_cxn}  
    ${output}    ${rc}=    Execute Command     sudo /opt/opendaylight/bin/configure_cluster.sh ${index} ${OS_CONTROL_1_IP},${OS_CONTROL_2_IP},${OS_CONTROL_3_IP}     return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}
    ${output}    ${rc}=    Execute Command     sed -ie 's/JAVA_MAX_MEM="2048m"/JAVA_MAX_MEM="4096m"/g' /opt/opendaylight/bin/setenv    return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}

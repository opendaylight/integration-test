...
Configure ODL Clustering
    ...
    Switch Connection    ${os_node_cxn}
    ${cmd}=       Set Variable If    4 > ${NUM_CONTROL_NODES}     sudo /opt/opendaylight/bin/configure_cluster.sh ${index} ${OS_CONTROL_1_IP},${OS_CONTROL_2_IP},${OS_CONTROL_3_IP}       sudo /opt/opendaylight/bin/configure_cluster.sh ${index} ${OS_CONTROL_1_IP},${OS_CONTROL_2_IP},${OS_CONTROL_3_IP},${OS_CONTROL_4_IP},${OS_CONTROL_5_IP}
    ${output}    ${rc}=     Run Keyword If    4 > ${NUM_CONTROL_NODES}      Execute Command    ${cmd}    return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}

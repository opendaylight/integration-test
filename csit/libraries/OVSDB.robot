...
Get Tunnel Id And Packet Count
    ...
    ${base_cmd} =    BuiltIn.Set Variable    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=${table_id}
    SSHLibrary.Switch Connection    ${conn_id}
    ${output} =    Utils.Write Commands Until Expected Prompt    ${base_cmd}    ${DEFAULT_LINUX_PROMPT_STRICT}
    ${full_cmd} =    BuiltIn.Run Keyword If    "${table_id}" == "${ELAN_DMACTABLE}"    BuiltIn.Catenate    ${base_cmd} | grep ${mac} | grep set_field:${tun_id}
    ${full_cmd} =    BuiltIn.Run Keyword If    "${table_id}" == "${L3_TABLE}"    BuiltIn.Catenate    ${base_cmd} | grep ${mac} | grep set_field:${tun_id}
    ${full_cmd} =    BuiltIn.Run Keyword If    "${table_id}" == "${INTERNAL_TUNNEL_TABLE}"    BuiltIn.Catenate    ${base_cmd} | grep tun_id=${tun_id}
    ...
    ${output} =    BuiltIn.Run Keyword If    "${table_id}" == "${ELAN_DMACTABLE}"    String.Get Regexp Matches    ${output}    n_packets=([0-9]+),.*set_field:(${tun_id})->tun_id    1
    ...    2
    ${output} =    BuiltIn.Run Keyword If    "${table_id}" == "${L3_TABLE}"    String.Get Regexp Matches    ${output}    n_packets=([0-9]+),.*set_field:(${tun_id})->tun_id    1
    ...    2
    ${output} =    BuiltIn.Run Keyword If    "${table_id}" == "${INTERNAL_TUNNEL_TABLE}"    String.Get Regexp Matches    ${output}    n_packets=([0-9]+),.*tun_id=(${tun_id})    1
    ...    2
    ${packet_count} =    BuiltIn.Set Variable    @{output}[0]
    ${tunnel_id} =    BuiltIn.Set Variable    @{output}[1]

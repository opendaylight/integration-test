...
Add Bridge To Ovsdb Node
    [Arguments]    ${node_ip}    ${bridge}    ${datapath_id}    ${port}=${OVSDB_NODE_PORT}
    ...
    ${uri} =    BuiltIn.Set Variable    ${SOUTHBOUND_CONFIG_API}${node_ip}:${port}%2Fbridge%2F${bridge}
    BuiltIn.Log    URI is ${uri}
    BuiltIn.Log    data: ${body}
    ${resp} =    RequestsLibrary.Put Request    session    ${uri}    data=${body}
    OVSDB.Log Request   ${resp.content}
...
Add Bridge To Ovsdb Node With NodeId
    ...
    ${body} =    String.Replace String    ${body}    61644    ${port}
    ${body} =    String.Replace String    ${body}    0000000000000001    ${datapath_id}
    ${uri} =    BuiltIn.Set Variable    ${SOUTHBOUND_CONFIG_API}${node_id}%2Fbridge%2F${bridge}
    BuiltIn.Log    URI is ${uri}
    BuiltIn.Log    data: ${body}
    ${resp} =    RequestsLibrary.Put Request    session    ${uri}    data=${body}
    OVSDB.Log Request   ${resp.content}
...
Delete Bridge From Ovsdb Node
    [Arguments]    ${node_ip}    ${bridge}    ${port}=${OVSDB_NODE_PORT}
...
Add Vxlan To Bridge
    [Arguments]    ${node_ip}    ${bridge}    ${vxlan_port}    ${remote_ip}    ${custom_port}=create_port.json    ${port}=${OVSDB_NODE_PORT}
...
Get OVSDB UUID
    ...
    OVSDB.Log Request   ${resp.content}
    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    200
    ${resp_json} =    RequestsLibrary.To Json    ${resp_content}

...
Connect To Ovsdb Node
    ...
    BuiltIn.Log    URI is ${uri}
    BuiltIn.Log    data: ${body}
    ${resp} =    RequestsLibrary.PUT On Session    session    ${uri}    data=${body}
    OVSDB.Log Request    ${resp.text}
    BuiltIn.Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
...
Disconnect From Ovsdb Node
    ...
    ${resp} =    RequestsLibrary.DELETE On Session    session    ${SOUTHBOUND_CONFIG_API}${node_ip}:${port}
...
Delete Bridge From Ovsdb Node
    ...
    ${resp} =    RequestsLibrary.DELETE On Session    session    ${SOUTHBOUND_CONFIG_API}${node_id}%2Fbridge%2F${bridge}
    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    200
...
Add Termination Point
    ...
    ${node_id_} =    BuiltIn.Evaluate    """${node_id}""".replace("/","%2F")
    ${uri} =    BuiltIn.Set Variable    ${SOUTHBOUND_CONFIG_API}${node_id_}%2Fbridge%2F${bridge}
    ${resp} =    RequestsLibrary.PUT On Session    session    ${uri}/termination-point/${tp_name}/    data=${body}
    BuiltIn.Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
...
Get OVSDB UUID
    ...
    ${resp} =    RequestsLibrary.GET On Session    ${controller_http_session}    ${OPERATIONAL_TOPO_API}/topology/ovsdb:1
    OVSDB.Log Request    ${resp.text}
    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    200
...
Config and Operational Topology Should Be Empty
    ...
    ${config_resp}    RequestsLibrary.GET On Session    session    ${CONFIG_TOPO_API}
    ${operational_resp}    RequestsLibrary.GET On Session    session    ${OPERATIONAL_TOPO_API}
...
Create Qos Linked Queue
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:HOST1    data=${body}
    OVSDB.Log Request    ${resp.text}
    BuiltIn.Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
...
Suite Teardown
    ...
    FOR    ${uri}    IN    @{uris}
        RequestsLibrary.DELETE On Session    session    ${uri}
    END
    ${resp} =    RequestsLibrary.GET On Session    session    ${CONFIG_TOPO_API}
    OVSDB.Log Config And Operational Topology

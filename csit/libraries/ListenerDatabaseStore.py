"""
ListenerDatabaseStore library for OpenDaylight project robot system
test framework.
...
class ListenerDatabaseStore:
    ...
    conn = None
    cursor = None
    test_col_schema_dict = dict()
    test_row_data_dict = dict()
            ...
            Args:
                :param dict_col_schema: dictionary containing column schema
                    example "{'TestRunId': 'VARCHAR(70) PRIMARY KEY',
                              ...
                              'RestPerSec': 'INT'}"
        """
        for column_name, column_def in dict_col_schema.iteritems():
            self.test_col_schema_dict[column_name] = column_def
    ...
    def store_data(self, test_name, data):
        """Store data in dict for later use in database.
            ...
            Returns:
                :returns string: result - TRUE OR FALSE based on success
                creating table
            """
        time_date = datetime.datetime.strftime(datetime.datetime.now(),
                                               '%Y-%m-%d-%H:%M:%S')
        test_run = test_name + "_" + time_date
        test_data = {test_run: test_run + ", " + data}
        self.test_row_data_dict[test_name] = test_data
    ...
    def check_table_exits(self, table_name, database_name):
        """Check that a table exists.
            Args:
                ...
                :param database_name: A string that describes the database to
                use
                    example 'PERF'
        ...
        """Execute a table cmd.
            Args:
                ...
                :param data: A string that describes the data to use
                    example table col attributes 'Id INTEGER PRIMARY KEY,
                    Name VARCHAR(20), RestPerSec INT'
    ...
    def create_table(self, table_name, table_col_attributes):
        """Create a table.
            Args:
                ...
                :param table_col_attributes: A string that describes the
                table columns to use
                    example 'Id INTEGER PRIMARY KEY, Name VARCHAR(20),
                    RestPerSec INT'
        ...
        result = self.execute_table_cmd(sql_cmd, table_name,
                                        table_col_attributes)
    ...
    def insert_into_table(self, table_name, row_data):
        ...
        result = self.execute_table_cmd(sql_cmd, table_name + " VALUES",
                                        row_data)
    ...
    def end_test(self, name,  attributes):
        """Store data in PostgreSQL database using Listener Interface method
           when "status = pass". It executes at the conclusion of a test.
            ...
            Args:
                ...
                :param attributes: dictionary with the following keys:
                    id: Test id in format like s1-s2-t2, where the beginning is
                        the parent suite id and the last part shows test index
                        in that suite. New in RF 2.8.5.
                    longname: Test name including parent suites.
                    doc: Test documentation.
                    tags: Test tags as a list of strings.
                    critical: yes or no depending is test considered critical
                    or not.
                    template: The name of the template used for the test.
                    An empty string if the test is not templated.
                    ...
                    message: Status message. Normally an error message or
                    an empty string.
        """
        if attributes['status'] == 'PASS':
            try:
                print "Storing %s Test Data" % name
                for test_name, test_run in self.test_row_data_dict.iteritems():
                    if not self.check_table_exits(test_name,
                                                  self.database_name):
                        self.create_table(test_name,
                                          self.test_col_schema_dict[test_name])
                    for test_run_sig, data in test_run.iteritems():

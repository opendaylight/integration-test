...
class DockerVm(object):
    ...
    def create_docker_container(self,
        ...
        method_name = "DockerVM.create_ovs_docker_container"
        logger.info("Starting method %s" % method_name)
        logger.info("Creating OVS Docker Container with IP: %s Name: %s " %
                    (docker_ip, docker_name))

        args_dict = dict(command="tail -f /dev/null",
                         hostname=docker_name,
                         name=docker_name,
                         detach=True)
        for arg, value in passed_args_dict.iteritems():
            args_dict[arg] = value
    ...
    def add_ovs_switch(self,
        ...
        """
        method_name = "DockerVM.add_switch"
        logger.info("Starting method %s" % method_name)
    ...
    def setup_test_switches(self,
        ...
        process = Popen(["ip",  "a"], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        ip_output, err = process.communicate()
        if "br-docker-ovs" not in ip_output:
            call(["brctl", "addbr", "br-docker-ovs"])
            call(["sudo", "ip", "addr", "add",
                  "172.0.100.1/16", "dev", "br-docker-ovs"])
            call(["sudo", "ip", "link", "set",
        ...
        for switch in range(0, int(switches)):
            ...
            if int(switches) > 252 and int(
                    self.return_processed_ip(
                        ipaddress_start,
                        3, switch+2).split(".")[3]) > 253:
                subnet_iteration += 1
                ipaddress = self.return_processed_ip(
                    ipaddress_start,
                    2, subnet_iteration)
                ipaddress = self.return_processed_ip(ipaddress, 3, switch-251)
            else:
                ipaddress = self.return_processed_ip(
                    ipaddress_start,
                    3, switch+2)
            ...
            call(["/usr/local/bin/pipework", "--wait", "-i", "br-docker-ovs"])
            call(["/usr/local/bin/pipework",
                  "br-docker-ovs",
                  "%s-ovs" % ipaddress,
                  "%s/16@%s" % (ipaddress, external_route)])

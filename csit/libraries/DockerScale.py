...
Authors: Marcus Williams - Email: marcus.williams@intel.com
         IRC: mgkwill - Intel Inc.
Updated: 2016-01-10

*Copyright (c) 2015-2016 Intel Corp. and others.  All rights reserved.
*
...
class DockerScale(object):
    ...
    def setup_test_switches(self,
        ...
        """
        self.dockervm.setup_test_switches(switches,
                                          ipaddress_start,
                                          controller)
...
class DockerVm(object):
    ...
    def create_docker_container(self,
        ...
        default_args_dict = dict(command="tail -f /dev/null",
                                 ...
                                 name=docker_name,
                                 detach=True)
        args_dict = Docker.DockerClientWrapper.docker_process_args(
            passed_args_dict,
            default_args_dict,
            method_name)
    ...
    def add_ovs_switch(self,
                       ...
                       docker_repo='socketplane/openvswitch',
                       ovs_version='2.3.1'):
    ...
    def add_controller(self,
                       ...
                       odl_repo='mgkwill/odl',
                       odl_version='0.3.0-debian'):
    ...
    def setup_test_switches(self,
        ...
        ip_output, error_output = DockerVm \
            .run_command_on_remote(self.ipaddress, "ip a")
        logger.info("Output of remote 'ip a' command: %s" % ip_output)

        if "br-docker-ovs" not in ip_output:
            logger.info("Setting up br-docker-ovs")
            DockerVm.run_command_on_remote(self.ipaddress,
                                           "brctl addbr "
                                           "br-docker-ovs")
            DockerVm.run_command_on_remote(self.ipaddress,
                                           "sudo ip addr "
    ...
    def scalability_docker_suite_teardown(self):
        ...
        docker_ps_output, error_output = DockerVm \
            .run_command_on_remote(self.ipaddress, "docker ps -a -q")
        logger.info("'docker ps -a -q' command output: %s Error %s" % (
            docker_ps_output,
            error_output))

        if docker_ps_output:
            for docker_instance in docker_ps_output.split():
                docker_stop_output, error_output = DockerVm \
                    .run_command_on_remote(self.ipaddress,
                                           "docker stop %s"
                                           % docker_instance)
                logger.info("'docker stop' command output: %s Error %s"
                            % (docker_stop_output, error_output))
                docker_rm_output, error_output = DockerVm \
                    .run_command_on_remote(self.ipaddress,
                                           "docker rm %s"
                                           % docker_instance)
                logger.info("'docker rm' command output: %s Error %s"
                            % (docker_rm_output, error_output))

        ip_link_set_output, error_output = DockerVm \
            .run_command_on_remote(self.ipaddress,
                                   "ip link set dev br-docker-ovs down")
        logger.info("'ip link set dev br-docker-ovs down' command output:"
                    " %s Error %s"
                    % (ip_link_set_output, error_output))
        brctl_delbr_output, error_output = DockerVm \
            .run_command_on_remote(self.ipaddress,
                                   "brctl delbr br-docker-ovs")
        logger.info("'brctl delbr br-docker-ovs' command output: %s"
                    % brctl_delbr_output)
    ...
    @staticmethod
    def run_command_on_remote(ipaddress, cmd):

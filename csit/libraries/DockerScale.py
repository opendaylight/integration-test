...
class DockerVm(object):
    ...
    def __init__(self, ipaddress, port, pickle_location="docker.pickle"):
        # Debug
        cat_docker_output, error_output = DockerVm \
            .run_commmand_on_remote(ipaddress,
                                    "cat /etc/sysconfig/docker")
        logger.info("'cat /etc/sysconfig/docker' command output: %s Error %s"
                    % (cat_docker_output, error_output))
        echo_docker_vars_output, error_output = DockerVm \
            .run_commmand_on_remote(ipaddress,
                                    "echo $DOCKER_OPTS; "
                                    "echo $DOCKER_HOST; "
                                    "echo $DOCKER; "
                                    "echo $DOCKER*")
        logger.info("'echo $DOCKER*' command output: %s Error %s"
                    % (echo_docker_vars_output, error_output))
        # End Debug
    ...
    def scalability_docker_suite_teardown(self):
        ...
        docker_ps_output, error_output = DockerVm \
            .run_commmand_on_remote(self.ipaddress, "docker ps -a -q")
        ...
        if docker_ps_output:
            for docker_instance in docker_ps_output.split():
                docker_stop_output, error_output = DockerVm \
                    .run_commmand_on_remote(self.ipaddress,
                                            "docker stop %s"
                                            % docker_instance)

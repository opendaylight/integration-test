...
PC_Setup
    ...
    WaitUtils.WU_Setup
    ${getter} =    ScalarClosures.Create_Closure_From_Keyword_And_Arguments    Get_Ipv4_Topology_Count
    BuuiltIn.Set_Suite_Variable    ${PrefixCounting__getter}    ${getter}

Get_Ipv4_Topology
    [Arguments]
    [Documentation]    GET the example-ipv4-topology data, check status is 200, return the topology data.
    ${response} =    Requests.Get_Request    operational    network-topology:network-topology/topology/example-ipv4-topology
...
Check_Ipv4_Topology_Count
    ...
    BuiltIn.Shoulr_Be_Equal_As_Strings    ${actual_count}    ${expected_count}
...
Wait_For_Ipv4_Topology_Prefixes_To_Become_Stable
    ...
    ${result} =    WaitUtils.Wait_For_Getter_Error_Or_Safe_Stateful_Validator_Consecutive_Success    timeout=${timeout}    period=${period}    count=${repetitions}    getter=${PrefixCounting__getter}    safe_validator=${validator}    initial_state=${count_to_overcome}
    [Return]    ${result}

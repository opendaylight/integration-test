...
...
...
...               Currently, all keywords count prefixes only in ${topology_id}.
...               Prefix is identified by simplistic regular expression on JSON data.
...
...               This resource assumes that RequestsLibrary has open a connection named ${connection_id}
...
*** Keywords ***
PC_Setup
    [Arguments]    ${connection_id}=operational    ${topology_id}=example-ipv4-topology
    [Documentation]    Call dependency setups and construct suite variables.
    WaitUtils.WU_Setup    # includes ScalarClosures.SC_Setup

Get_Ipv4_Topology
    [Arguments]    ${connection_id}=operational    ${topology_id}=example-ipv4-topology
    [Documentation]    GET the ${topology_id} data, check status is 200, return the topology data.
    ...
    ...    Contrary to Utils.Get_Data_From_URI, this does not Log the (potentially huge) content.
    ${response} =    RequestsLibrary.Get_Request    ${connection_id}    network-topology:network-topology/topology/${topology_id}
    Run_Keyword_If    ${response.status_code} != 200    Fail    Get on ${topology_id} returned status code ${response.status_code} with message: ${response.text}
    [Return]    ${response.text}

Get_Ipv4_Topology_Count
    [Arguments]    ${connection_id}=operational    ${topology_id}=example-ipv4-topology
    [Documentation]    Get topology. If not fail, return number of prefixes in the topology.
    ${topology} =    Get_Ipv4_Topology    connection_id=${connection_id}    topology_id=${topology_id}
...
Check_Ipv4_Topology_Count
    [Arguments]    ${expected_count}    ${connection_id}=operational    ${topology_id}=example-ipv4-topology
    ...
    ${actual_count} =    ScalarClosures.Run_Keyword_And_Collect_Garbage    Get_Ipv4_Topology_Count    connection_id=${connection_id}    topology_id=${topology_id}
...
Check_Ipv4_Topology_Is_Empty
    ...
    Check_Ipv4_Topology_Count    0    connection_id=${connection_id}    topology_id=${topology_id}

Wait_For_Ipv4_Topology_Prefixes_To_Become_Stable
    [Arguments]    ${timeout}=60s    ${period}=5s    ${repetitions}=1    ${excluded_count}=-1    ${connection_id}=operational    ${topology_id}=example-ipv4-topology
    ...
    ${getter} =    ScalarClosures.Closure_From_Keyword_And_Arguments    Get_Ipv4_Topology_Count    connection_id=${connection_id}    topology_id=${topology_id}

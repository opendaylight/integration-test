...
Get_Ipv4_Topology_Count_With_Shards_Check
    ...
    [Documentation]    Get topology and check shards stability. Given If not fail, return number of prefixes in the topology.
    ${details_actual}=    ShardStability.Shards_Stability_Get_Details    ${shards_list}
    ShardStability.Shards_Stability_Compare_Same    ${details_actual}    stored_details=${details_exp}
    ${topology} =    Get_Ipv4_Topology    session=${session}    topology=${topology}
    # Triple quotes are precaution against formatted output.
    ${prefix_count} =    Builtin.Evaluate    len(re.findall('"prefix":"', '''${topology}'''))    modules=re
    [Return]    ${prefix_count}
...
Wait_For_Ipv4_Topology_Prefixes_To_Become_Stable
    ...
    ELSE    ScalarClosures.Closure_From_Keyword_And_Arguments    ScalarClosures.Closure_From_Keyword_And_Arguments    Get_Ipv4_Topology_Count_With_Shards_Check    ${shards_list}    ${shards_details}    session=${session}
    ...    topology=${topology}

...
Create OVSVTEP Command
    ...
    ${cmd}=    Catenate    SEPARATOR=    ${cmd}    ${SPACE}    --log-file=/var/log/openvswitch/ovs-vtep    ${num}    .log
    ${cmd}=    Catenate    SEPARATOR=    ${cmd}    ${SPACE}    --pidfile=/var/run/openvswitch/ovs-vtep    ${num}    .pid
...
Port Start Hwvtep
    ...
    \    Create Namespace Port    ${hwvtep_conn_id}    ${ns_name}    ${ovs_port_name}    ${ns_tap_name}    ${vlan}    ${hwvtep_bridge}

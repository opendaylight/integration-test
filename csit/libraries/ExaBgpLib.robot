...
Start_ExaBgp_And_Verify_Connected
    ...
    ${BGP_RIB_NAME}=    BuiltIn.Set_Variable_If    ${RIB_NAME}    example-bgp-rib    ${RIB_NAME}
    FOR    ${idx}    IN RANGE    ${connection_retries}
        ...
        ...    Verify_ExaBgps_Connection    ${session}    ${exabgp_ip}    connected=${True}    ${BGP_RIB_NAME}
...
Verify_ExaBgps_Connection
    [Arguments]    ${session}    ${exabgp_ip}=${TOOLS_SYSTEM_IP}    ${connected}=${True}    ${BGP_RIB_NAME}
    [Documentation]    Checks peer presence in operational datastore
    ${exp_status_code}=    BuiltIn.Set_Variable_If    ${connected}    ${200}    ${404}
    ${PEER_CHECK_URL}=    BuiltIn.Catenate    ${PREFIX}/${BGP_RIB_NAME}/${SUFFIX}

...
Find Unknown Device
    ...
    ...    1. Sends a RestConf request to fetch for the operational node inventory of the device
    ...    2. Finds the unknown device's IP address and returns if not found  
    ${resp}=    RequestsLibrary.Get    session    ${OPERATIONAL_NODES_API}
    Should Be Equal As Strings    ${resp.status_code}    200
    Log    ${resp.content}
    Return From Keyword If    '${device_IP}' not in '${resp.content}'    ${device_IP}

Find Device Identification Information
    [Documentation]    Extract DIDM identification information
    ...    This keyword performs the following:
    ...    1. Sends a RestConf request to fetch for the operational nodes inventory of the device

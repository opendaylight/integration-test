...
def applyFilter(jsonArg, filteringLine):
    ...
    logging.info('applyFilter:starting. input json=\n%s', jsonArg)
    asJson = json.loads(jsonArg)
    logging.info('applyFilter: applying filter! jsonPath filter=[%s]', filteringLine)

    res = jsonpath(asJson, filteringLine, result_type='PATH')
    ...
    patchedJson = jsonpatch.apply_patch(asJson,asJsonPatch);
...
def prefilter(jsonArg, initialPreFilter):
    ...
    for filteringLine in lines:
        jsonArg = applyFilter(jsonArg, filteringLine)
...
def prefilterJsonFilesThenCompare(args):
    ...
    logging.info('prefilterJsonFilesThenCompare: initial json after patching: %s', jsonInitialFiltered)

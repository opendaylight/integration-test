...
Start SuiteVtnMaTest
    ...
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       ClusterManagement Setup
    Create Session    session    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS_YANG_JSON}
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       MininetKeywords.Start Mininet Multiple Controllers    ${TOOLS_SYSTEM_IP}    ${ClusterManagement__member_index_list}     options=${start_cluster}
    Run Keyword If    ${NUM_ODL_SYSTEM}==1       MininetKeywords.Start Mininet Single Controller    ${TOOLS_SYSTEM_IP}    options=${start_cluster}


Start SuiteVtnMaVlanTest
    [Documentation]    Start VTN Manager Test Suite
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       ClusterManagement Setup
    Create Session    session    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS_YANG_JSON}
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       MininetKeywords.Start Mininet Multiple Controllers    ${TOOLS_SYSTEM_IP}    options=${start_cluster}     custom=${CURDIR}/MininetTopo/${vlan_topo_file} 
    Run Keyword If    ${NUM_ODL_SYSTEM}==1       MininetKeywords.Start Mininet Single Controller    ${TOOLS_SYSTEM_IP}    options=${start_cluster}     custom=${CURDIR}/MininetTopo/${vlan_topo_file} 
...
Stop SuiteVtnMaTest
    [Documentation]    Stop VTN Manager Test Suite
    Run Keyword If    ${NUM_ODL_SYSTEM}>1        Utils.Clean Mininet System    ${TOOLS_SYSTEM_IP}
    Delete All Sessions
...
Start vlan_topo
    ...
    Run Keyword If    '${OF}' == 'OF13'    Start vlan_topo_OF13      ${NUM_ODL_SYSTEM}
    ...    ELSE IF    '${OF}' == 'OF10'    Start vlan_topo_OF10      ${NUM_ODL_SYSTEM}

...
Get Cluster Details
    ...
    ${resp_entity_owners}    RequestsLibrary.Get Request    session    restconf/operational/entity-owners:entity-owners
    Log    ${resp_entity_owners.content}
    ${resp_cluster_jolokia}    RequestsLibrary.Get Request    session    jolokia/read/akka:type=Cluster
    Log    ${resp_cluster_jolokia.content}
    ${resp_operational_topology}    RequestsLibrary.Get Request    session    restconf/operational/network-topology:network-topology
    Log    ${resp_operational_topology.content}
    ${resp_config_topology}    RequestsLibrary.Get Request    session    restconf/config/network-topology:network-topology
    Log    ${resp_config_topology.content}

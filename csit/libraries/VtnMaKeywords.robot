...
Start SuiteVtnMaVlanTest
    ...
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       ClusterManagement Setup
    Create Session    session    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS_YANG_JSON}
    Run Keyword If    ${NUM_ODL_SYSTEM}>1       MininetKeywords.Start Mininet Multiple Controllers    ${TOOLS_SYSTEM_IP}    ${ClusterManagement__member_index_list}     options=${start_cluster}     custom=${vlan_topo_file} 
...
Collect Debug Info
    ...
    ${owners_pretty}=     To Json    ${resp_owners.content}      pretty_print=True
    Log      ${owners_pretty}

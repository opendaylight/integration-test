...
Add a flowfilter
    [Arguments]    ${vtn_name}    ${vBridge_name}    ${interface_name}    ${flowfilter_data}
    [Documentation]    Create a flowfilter for a vtn
    ${resp}=    RequestsLibrary.POST    session    restconf/operations/vtn-flow-filter:set-flow-filter   data=${flowfilter_data}
    Should Be Equal As Strings    ${resp.status_code}    200

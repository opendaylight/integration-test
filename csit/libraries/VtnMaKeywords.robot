...
Add a pathmap
    ...
    ${resp}=    RequestsLibrary.Post    session    restconf/operations/vtn-path-map:set-path-map    data=${pathmap_data}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Add a pathpolicy
    ...
    ${resp}=    RequestsLibrary.Post    session    restconf/operations/vtn-path-policy:set-path-policy    data=${pathpolicy_data}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete a pathpolicy
    ...
    ${resp}=    RequestsLibrary.Post    session    restconf/operations/vtn-path-policy:remove-path-policy    data={"input": {"id": "${path_id}"}}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Get flow
    ...
    ${resp}=    RequestsLibrary.Get    session    restconf/operational/vtn-flow-impl:vtn-flows/vtn-flow-table/${vtn_name}
    Should Be Equal As Strings    ${resp.status_code}    200

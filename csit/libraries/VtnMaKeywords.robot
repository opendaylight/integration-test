...
${OVSDB_CONFIG_DIR}    ${CURDIR}/${CREATE_SCF_TOPOLOGY_FILE_PATH}

${scf_topo}    sudo mn --controller=remote,ip=${CONTROLLER} --custom topo_handson.py --topo scftopo --switch ovsk,protocols=OpenFlow13
${flowfilterdropdata}    {"index":10,"condition":"cond_1","filterType":{"drop":{}}}
${flowfilterRedirectAnydata}    {"index":10,"condition":"cond_Any","filterType":{"redirect":{"destination": {"bridge": "Bridge1","interface": "IF2"},"output": true}}}
${flowfilterRedirectdata}    {"index":10,"condition":"cond_1","filterType":{"redirect":{"destination": {"terminal": "vt_srvc1_1","interface": "IF"},"output": true}}}
...
Configure service nodes
    ...
    ${mininet_conn_id}    Start Mininet    ${MININET}    ${scf_topo}    ${OVSDB_CONFIG_DIR}
...
Start Mininet Session
    [Documentation]    Starts Service Chain Functioning custom topology.
    Start Mininet    ${MININET}    ${scf_topo}    ${OVSDB_CONFIG_DIR}
...
Add a flowcondition for service chain function
    [Arguments]    ${cond_name}    ${flowcond_data}
    [Documentation]    Create a flowcondition for a interface of a vbridge
    ${json_data}=   json.dumps    ${flowcond_data}
    ${resp}=    RequestsLibrary.Put    session    ${REST_CONTEXT}/flowconditions/${cond_name}    data=${json_data}    headers=${HEADERS}

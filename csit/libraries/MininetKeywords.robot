...
Start Mininet Multiple Controllers
    ...
    ${output}=    Execute Command    sudo ps -elf | egrep 'usr/local/bin/mn' | egrep python | awk '{print "sudo kill -9",$4}' | sh
    Log    ${output}
    ${primary}=    Get From List    ${controller_index_list}    0
    ${cmd}=    Set Variable    sudo mn ${options}
    Log    Starting mininet to ${ODL_SYSTEM_${primary}_IP}
    Write    ${cmd}
    Read Until    mininet>
...
Send Mininet Command Multiple Sessions
    ...
    [Documentation]    Sends Command ${cmd} to Mininet sessions in ${mininet_conn_list} and returns read buffer responses.
    ${output}=    Create List

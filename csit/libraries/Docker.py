...
class DockerClientWrapper(object):
    ...
    def __init__(self, ipaddress=None, port=None):
        ...
        if ipaddress and port:
            base_url = "tcp://%s:%s" % (ipaddress, port)
            self.docker_client = DockerClientWrapper.docker_get_client(
                dict(base_url=base_url))
            # debugging
            logger.info("Connected and printing remote images: ###")
            logger.info(self.docker_client.images())
            logger.info("Finished printing remote images #########")
        else:
            self.docker_client = DockerClientWrapper.docker_get_client()

    def docker_create(self, docker_image_name, passed_args_dict=None):
        ...
        self.docker_client.pull(docker_image_name, stream=True)

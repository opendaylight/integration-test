...
Get_Change_Count
    ...
    ${stream}=    CompareStream.Set_Variable_If_At_Least_Boron    ${True}    ${False}
    ${count} =    BuiltIn.Run Keyword If    "${stream}" == "${False}"    BuiltIn.Evaluate    ${response.text}["data-change-counter"]["count"]

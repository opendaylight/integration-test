...
Get_Change_Count
    [Documentation]    GET data change request, assert status 200, return the value. Use resource CompareStream.
    ...
    ${count1} =    BuiltIn.Evaluate    ${response.text}["data-change-counter"]["count"]
    ${count2} =    BuiltIn.Evaluate    ${response.text}["data-change-counter"]["counter"][0]["count"]
    ${count} =    CompareStream.Set_Variable_If_At_Most_Beryllium    ${count1}    ${count2}
    [Return]    ${count}

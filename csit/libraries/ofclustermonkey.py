...
class ClusterData(object):
    ...
    def init(cls, *cluster_nodes):
        ...
        """
        cls.cluster_nodes = dict(cluster_nodes)
        cls.reload_data()
    ...
    def check_member(cls, switch_name, controller_ip, callback=_blank_check_callback):
        member = cls.cluster_nodes[controller_ip]
        switch_id = cls._get_entity_switch_id(switch_name)
    ...
    def _make_chaos(self):
        ...
        if self._controller is not None:
            ...
            def member_not_in_candidates_or_owner(member, candidates, owner):
                ...
                election_check = master_member != owner if ClusterData.cluster_nodes[controller_ip] == master_member else True
...
def start_monkey(monkey_count, interval, run_time, logfile, loglevel):
    global _stop
    ...
    OVSData.init()
    ClusterData.init(*_cluster_data)

...
from these distrubtions and that during these recovery phases it's behaving as
inteded.
...
connection to controller from ovsctl bridge list, and disconnects it by chaning port
...
def start_monkey(monkey_count, interval, run_time, logfile, dumpfile, loglevel, cluster_nodes):
    ...
    formatter = logging.Formatter('[%(asctime)s] %(levelname)7s %(name)22s %(threadName)10s: %(message)s',
                                  datefmt='%Y/%m/%d %H:%M:%S')

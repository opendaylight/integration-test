...
Assert L2-Flood-Domain
    ...
    ${parent}    Assert Forwarding Domain
    ...    ${l2_flood_domain_to_check}
    ...    ${l2_flood_domain_name}
    ...    return_parent=TRUE
    RETURN    ${parent}
...
Assert L2-Bridge-Domain
    ...
    ${parent}    Assert Forwarding Domain
    ...    ${l2_bridge_domain_to_check}
    ...    ${l2_bridge_domain_name}
    ...    return_parent=TRUE
    RETURN    ${parent}
...
Check Endpoint
    ...
    FOR    ${l3}    IN    @{ep_l3-addresses}
        ...
        LOG    ${l3['l3-context']}
        LOG    ${l3['ip-address']}
        IF    "${l3['l3-context']}" == "${l3-context}" and "${l3['ip-address']}" == "${ip_address}"
            CONTINUE
        END
        Fail
    END
...
Check Group References
    ...
    FOR    ${epg}    IN    @{endpoint_groups}
        IF    "${epg}" == "${epg_to_look_for}"            CONTINUE
        IF    "${epg}" == "${NETWORK_CLIENT_GROUP}"            CONTINUE
        Fail
    END
...
Check Endpoint Group Name and Selector
    ...
    FOR    ${selector}    IN    @{named_selectors}
        IF    "${selector['contract'][0]}" == "${contract_id}"    RETURN
    END
    Fail

...
Ping from Docker
    ...
    ${output}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} ping ${dest_address} -c ${count} && echo success    return_stdout=True    return_stderr=True    return_rc=True
...
Start Endless Ping from Docker
    ...
    ${output}     ${err}        ${rc}    SSHLibrary.Execute Command    docker exec -d ${docker_name} ping ${dest_address}    return_stdout=True    return_stderr=True    return_rc=True
...
Stop HTTP Service on Docker
    ...
    ${stdout}    SSHLibrary.Execute Command    docker exec ${docker_name} ps aux | grep 'SimpleHTTPServer ${service_port}' | awk '{print $2}'    return_stdout=True    return_stderr=True    return_rc=True
    Should Not Be Empty    ${stdout}
    ${stdout}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} kill ${stdout}    return_stdout=True    return_stderr=True    return_rc=True
...
Start Endless Curl from Docker
    ...
    ${output}    SSHLibrary.Execute Command    docker exec ${docker_name} ls | grep curl_running    return_stdout=True    return_stderr=False    return_rc=False
    Should Be Empty    ${output}
    ${output}    ${err}    ${rc}  SSHLibrary.Execute Command    docker exec ${docker_name} touch curl_running && echo success    return_stdout=True    return_stderr=True    return_rc=True
    Should Contain    ${output}    success
...
Stop Endless Curl from Docker
    ...
    ${output}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} rm curl_running && echo success    return_stdout=True    return_stderr=True    return_rc=True
    Should Contain    ${output}    success
...
Execute Curl
    ...
    ${output}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} curl ${dest_address}:${service_port} -m 15 && echo success    return_stdout=True    return_stderr=True    return_rc=True
    Should Contain    ${output}    success

...
Start Endless Ping from Docker
    ...
    SSHLibrary.Write    docker exec ${docker_name} ping ${dest_address} & >/dev/null 2>&1
...
Curl from Docker
    ...
    ${output}    ${stderr}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} ls | grep curl_running

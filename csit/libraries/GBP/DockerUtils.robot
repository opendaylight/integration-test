...
Start HTTP Service on Docker
    ...
    ...    docker exec ${docker_name} ps aux | grep 'SimpleHTTPServer ${service_port}'
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
...
Stop HTTP Service on Docker
    ...
    ${stdout}    SSHLibrary.Execute Command
    ...    docker exec ${docker_name} kill ${stdout}
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
...
Curl from Docker
    ...
    ...    docker exec ${docker_name} ls | grep curl_running
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
...
Start Endless Curl from Docker
    ...
    Should Be Empty    ${output}
    ${output}    SSHLibrary.Execute Command
    ...    docker exec ${docker_name} touch curl_running && echo success
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
    Should Contain    ${output}    success
...
Stop Endless Curl from Docker
    ...
    ${output}    SSHLibrary.Execute Command
    ...    docker exec ${docker_name} rm curl_running && echo success
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
    Should Contain    ${output}    success
...
Stop Endless Ping from Docker to Address
    ...
    [Arguments]    ${docker_name}    ${dest_address}
    ${output}    SSHLibrary.Execute Command
    ...    docker exec ${docker_name} ps aux | grep 'ping ${dest_address}' | grep -v grep | awk '{print $2}'
    ...    return_stdout=True
    ...    return_stderr=False
    ...    return_rc=False
    Should Not Be Empty    ${output}
...
Test Port On Docker
    ...
    ...    docker exec ${docker_name} nc -z -w 5 127.0.0.1 ${service_port} && echo 'opened'
...
Execute Curl
    ...
    IF    ${endless} == "TRUE"
        ...
        ...    docker exec -d ${docker_name} /bin/sh -c "while [ -f curl_running ]; do curl ${dest_address}:${service_port} -m 1 && sleep ${sleep}; done"
        RETURN
    END
    ${output}    SSHLibrary.Execute Command
    ...    docker exec ${docker_name} curl ${dest_address}:${service_port} -m 5 >/dev/null 2>&1 && echo success
    Should Contain    ${output}    success

...
Start Endless Ping from Docker
    ...
    ${output}     ${err}        ${rc}    SSHLibrary.Execute Command    docker exec -d ${docker_name} ping ${dest_address}
...
Start HTTP Service on Docker
    ...
    ${output}    ${err}    ${rc}    SSHLibrary.Write    docker exec ${docker_name} python -m SimpleHTTPServer ${service_port} &
...
Stop HTTP Service on Docker
    ...
    ${stdout}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} kill ${stdout}    return_stdout=True    return_stderr=False    return_rc=False
...
Execute Curl
    ...
    ${output}    ${err}    ${rc}    SSHLibrary.Execute Command    docker exec ${docker_name} curl ${dest_address}:${service_port} -m 5 && echo success
    Should Contain    ${output}    success

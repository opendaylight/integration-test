...
Inspect Classifier Inbound
    ...
    [Documentation]    Inspect flows on SFF
    @{matches}    Create List
    @{actions}    Create List
    Append In Port Check    ${matches}    ${in_port}
    Append Ether-Type Check    ${matches}    ${eth_type}
...
#    ${output}    Wait Until Keyword Succeeds  2 min  300ms    Dump Flows
    Log  ${output}
...
Dump Flows
    ${out}    SSHLibrary.Execute Command    sudo ovs-dpctl dump-flows
    Should Not Be Empty    ${out}
    [Return]    ${out}

Manager and Switch Are Connected
...
Wait For Flows On Switch
    ...
    Wait Until Keyword Succeeds  20s  3s  Count Flows On Switch  ${switch_name}

...
def initiate_write_transactions_on_nodes(host_list, id_prefix, duration, rate, chained_flag=True):
    """Invoke publish notification rpcs and verify the response.
  ...
  <chained-transactions>$CHAINED_FLAG</chained-transactions>
        ...
        logging.info('write-transactions rpc indoked with details: {}'.format(data)
        try:
            resp = requests.post(url=url, headers={'Content-Type': 'application/xml'},
                                 data=data, auth=('admin', 'admin'), timeout=int(duration)+60)
        except Exception as exc:
    ...
    for i,host in enumerate(host_list):
        ...
        t = threading.Thread(target=_write_transactions, args=(resqueue, url, '{}{}'.format(id_prefix,i), duration, rate, chained_flag))
        t.daemon = True
        t.start()
        lthreads.append(t)

    _globals.update({'threads': lthreads, 'hosts': host_list, 'result_queue': resqueue})

"""
Python invocation of several rpcs defined in module module odl-mdsal-lowlevel-control.yang.
"""
import Queue
import requests
import string
...
def publish_notifications(host, grprefix, duration, rate, nrpairs=1):
    """Invoke publish notification rpc and verify the response
  ...
  <notifications-per-second>$RATE</notifications-per-second>
        ...
        try:
            resp = requests.post(url=url, headers={'Content-Type': 'application/xml'},
                                 data=data, auth=('admin', 'admin'), timeout=duration+60)
        except Exception as exc:
            resp = exc
        print 'Response for{}: {}'.format(grid, resp)
        rqueue.put(resp)

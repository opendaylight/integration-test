...
Invoke_Restperfclient
    ...
    ${status}    ${message}=    Run_Keyword_And_Ignore_Error    Restperfclient__Invoke_With_Timeout    ${keyword_timeout}    ${command}
    BuiltIn.Run_Keyword_If    '${status}' == 'FAIL'    BuiltIn.Run_Keyword_And_Ignore_Error    RestPerfClient__Kill
    Set_Known_Bug_Id    5413
    BuiltIn.Run_Keyword_If    '${status}' == 'FAIL'    Fail    ${message}
    Set_Unknown_Bug_Id

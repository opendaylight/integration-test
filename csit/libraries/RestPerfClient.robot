...
Setup_Restperfclient
    ...
    ${filename}=    Run Keyword If    ${IS_KARAF_APPL} == 'False'    NexusKeywords.Deploy_Test_Tool    netconf    netconf-testtool    rest-perf-client    build_version=${NETCONF_TESTTOOL_VERSION}    build_location=org/opendaylight/netconf
    ...    ELSE    NexusKeywords.Deploy_Test_Tool    netconf    netconf-testtool    rest-perf-client
    Run keyword if    '${SCALE_TESTS}' == 'TRUE'    Set Suite Variable    ${JAVA_HEAP_MEMORY}    -Xmx16G
    ...    ELSE    Set Suite Variable    ${JAVA_HEAP_MEMORY}    -Xmx1G
    ${prefix}=    NexusKeywords.Compose_Full_Java_Command    ${JAVA_HEAP_MEMORY} -jar ${filename}

...
import sys
sys.path.insert(0, '/tmp/odl-lispflowmapping-yang-files') # Bindings must present in /tmp/odl-lispflowmapping-yang-files 
...
def get_lisp_address_obj(eid_string, vni=None):
    """Returns a lisp address object based its string representation"""
    print "Ashish:lisp-address", eid_string
...
def get_lisp_address_json(eid_string, vni=None):
    ...
    pbj_dump = pbJ.dumps(get_lisp_address_obj(eid_string, vni), filter=True, mode="default")
    out_dump = {"eid": json.loads(pbj_dump)}
    return clean_json(json.dumps(out_dump))

def get_locator_record_obj(rloc, weights='1/1/255/0', flags=001, loc_id="ISP1"):
    rpc_input = input()
    ...
    lrecord_obj[loc_id].rloc = get_lisp_address_obj(rloc)
...
def get_locator_record_json(rloc, weights='1/1/255/0', flags=001, loc_id="ISP1"):
    # TODO: flags = 101
    # TODO: ietf mode is throwing some errors
    pbj_dump = pbJ.dumps(get_locator_record_obj(rloc, weights, flags, loc_id), filter=True, mode="default")
    out_dump = {"LocatorRecord": json.loads(pbj_dump)}
    return clean_json(json.dumps(out_dump))

def get_mapping_record_obj(eid, locators, ttl=1440, authoritative=True, action='NoAction'):
    rpc_input = input()
...
def get_mapping_record_json(eid, locators, ttl=1440, authoritative=True, action='NoAction'):
    #print "Ashish:mapping", eid, locators
    pbj_dump = pbJ.dumps(get_mapping_record_obj(eid, locators, ttl, authoritative, action), filter=True, mode="ietf")
    out_dump = {"mapping-record": json.loads(pbj_dump)}
    print "Ashish:", clean_json(json.dumps(out_dump))
    return clean_json(json.dumps(out_dump))


def get_mapping_authkey_obj(key_string="password", key_type=1):
...
def get_mapping_authkey_json(key_string="password", key_type=1):
    pbj_dump = pbJ.dumps(get_mapping_authkey_obj(key_string, key_type), filter=True, mode="default")
    out_dump = {"mapping-authkey": json.loads(pbj_dump)}
    return clean_json(json.dumps(out_dump))

def rpc_add_key_input_obj(eid, key_string="password", key_type=1):
    from LISPFlowMappingYANGBindings.odl_mappingservice_rpc.add_key.input import input as add_key_input
    rpc_input = add_key_input()
    rpc_input.eid = get_lisp_address_obj(eid)
    rpc_input.mapping_authkey.key_string = key_string
    rpc_input.mapping_authkey.key_type = key_type
    return rpc_input


def rpc_add_key_input_json(eid, key_string="password", key_type=1):
    pbj_dump = pbJ.dumps(rpc_add_key_input_obj(eid, key_string, key_type), filter=True, mode="default")
    out_dump = {"input": json.loads(pbj_dump)}
    return clean_json(json.dumps(out_dump))

print json.dumps(get_mapping_authkey_json(), indent=4, sort_keys=True)
print json.dumps(rpc_add_key_input_json("ipv4:192.0.2.1/32"), indent=4, sort_keys=True)
print json.dumps(get_lisp_address_json("ipv4:192.0.2.1/32"), indent=4, sort_keys=True)
print json.dumps(get_mapping_record_json(get_lisp_address_obj("ipv4:192.0.2.1/32"), [get_locator_record_obj("ipv4:10.10.10.10")]), indent=4, sort_keys=True)
print json.dumps(get_locator_record_json("ipv4:192.0.2.1/32", loc_id="ISP2"), indent=4, sort_keys=True)
print json.dumps(wrap_input(merge(get_lisp_address_json("ipv4:192.0.2.1/32"), get_mapping_authkey_json())), indent=4, sort_keys=True)
...
def get_lisp_address_json(eid_string, vni=None):
    ...
    return pbJ.dumps(get_lisp_address_obj(eid_string, vni), filter=True, mode="ietf")


def get_locator_record_obj(rloc, weights='1/1/255/0', flags=101):
    rpc_input = input()
...
def get_locator_record_json(rloc, weights='1/1/255/0', flags=101):
    return pbJ.dumps(get_locator_record_obj(rloc, weights, flags), filter=True, mode="ietf")
...
def get_mapping_record_json(eid, locators, ttl=1440, authoritative=True, action='NoAction'):
    return pbJ.dumps(get_mapping_record_obj(eid, locators, ttl=1440, authoritative=True, action='NoAction'), filter=True, mode="ietf")

print get_locator_record_json("ipv4:10.10.10.10")
print get_mapping_record_json(get_lisp_address_obj("ipv4:10.10.10.10"), [get_locator_record_obj("ipv4:10.10.10.10"), get_locator_record_obj("ipv4:10.10.10.10")])

...
Get OpenFlow Entity Owner Status For One Device
    ...
    [Documentation]    Check Entity Owner Status and identify owner and candidates for the device ${device}. Request is sent to first member available.
    ${owner}    ${candidate_list}    Wait Until Keyword Succeeds    10s    1s    ClusterManagement.Verify_Owner_And_Candidates_For_Device    device_name=${device}
    ...    device_type=openflow    member_index=@{controller_index_list}[0]    member_index_list=${controller_index_list}

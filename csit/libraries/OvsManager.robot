...
Get OVS Flows
    ...
    ${flow_output} =    Utils.Run Command On Remote System    ${ovs_ip}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${bridge} |sed 's/cookie=\\S\\+\\s\\+duration=\\S\\+\\s\\+\\(.*\\)n_packets=\\S\\+\\s\\+n_bytes=\\S\\+\\s\\(.*\\)/\\1 \\2/g'|grep -v idle_timeout

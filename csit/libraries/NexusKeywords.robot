...
Install_And_Start_Java_Artifact
    [Arguments]    ${component}    ${artifact}    ${suffix}=executable    ${options}=${EMPTY}    ${java_options}=${EMPTY}    ${openjdk}=${JDKVERSION}
    ...
    ${actual_java_options} =    BuiltIn.Set_Variable_If    """${java_options}""" != ""    ${java_options}    ${defualt_java_options}
    ${filename}=    Deploy_Test_Tool    ${component}    ${artifact}

...
Deploy_Artifact
    ...
    ${is_staged} =    BuiltIn.Set_Variable_If    "opendaylight.release" in '${urlbase}' or "opendaylight.snapshot" in '${urlbase}'    "FALSE"    "TRUE"
    # if we have a staged artifact we need to use the urlbase given to us in the job params
    ${urlbase} =    BuiltIn.Set_Variable_If    '${component}' in @{RELEASE_INTEGRATED_COMPONENTS} and '${is_staged}' == "FALSE"    ${NEXUS_RELEASE_BASE_URL}    ${urlbase}

...
Configure_Devices_In_Netconf
    ...
    : FOR    ${iteration}    IN RANGE    ${first_device_port}    ${max_index}+1
...
Check_Topology_Completely_Gone
    ...
    ${uri}=    Builtin.Set_Variable    ${CONFIG_API}/network-topology:network-topology/topology/topology-netconf/
    ${status}    ${response}=    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Get_As_Xml_From_Uri    ${uri}    session=${session}
    BuiltIn.Should_Be_Equal_As_Strings    ${status}    FAIL
    BuiltIn.Should_Contain    ${response}    404
...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device
    [Arguments]    ${operation}    ${deadline_Date}    ${first_netconf_device_port}=${FIRST_TESTTOOL_PORT}    ${device_name_prefix}=${DEVICE_NAME_BASE}
...
Perform_Operation_On_Each_Device
    [Arguments]    ${operation}    ${count}=${NetconfKeywords__testtool_device_count}    ${timeout}=30m    ${first_netconf_device_port}=${FIRST_TESTTOOL_PORT}    ${device_name_prefix}=${DEVICE_NAME_BASE}
    ${current_Date}=    DateTime.Get_Current_Date
    ${deadline_Date}=    DateTime.Add_Time_To_Date    ${current_Date}    ${timeout}
    BuiltIn.Set_Suite_Variable    ${current_port}    ${first_netconf_device_port}

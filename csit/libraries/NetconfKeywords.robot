...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device
    ...
    BuiltIn.Run_Keyword_If    ${ENABLE_GLOBAL_TIMEOUT}    NetconfKeywords__Check_Global_Timeout    ${deadline_Date}
...
Perform_Operation_On_Each_Device
    [Arguments]    ${operation}    ${count}=none    ${timeout}=30m    ${skipmemorycheck}=False
    ${count}=    BuiltIn.Set_Variable_If    '${count}'=='none'    ${NetconfKeywords__testtool_device_count}    ${count}
    ${current_Date}=    DateTime.Get_Current_Date

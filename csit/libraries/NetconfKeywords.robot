...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device
    [Arguments]    ${operation}    ${deadline_Date}    ${BASE_NETCONF_DEVICE_PORT}=17830
...
Perform_Operation_On_Each_Device
    [Arguments]    ${operation}    ${count}=${NetconfKeywords__testtool_device_count}    ${timeout}=30m    ${BASE_NETCONF_DEVICE_PORT}=17830
    ${current_Date}=    DateTime.Get_Current_Date
    ...
    BuiltIn.Repeat_Keyword    ${count} times    NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device    ${operation}    ${deadline_Date}    ${BASE_NETCONF_DEVICE_PORT}

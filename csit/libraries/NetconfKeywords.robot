...
Configure_Devices_In_Netconf
    [Arguments]    ${device_name_prefix}    ${device_type}=default    ${first_device_port}=${FIRST_TESTTOOL_PORT}    ${device_address}=${TOOLS_SYSTEM_IP}    ${device_user}=admin    ${device_password}=topsecret
    ...
    ${mapping}=    BuiltIn.Create_dictionary    DEVICE_IP=${device_address}    DEVICE_NAME_PREFIX=${device_name_prefix}    DEVICE_PORT=${first_device_port}    DEVICE_USER=${device_user}    DEVICE_PASSWORD=${device_password}
    ...    SCHEMA_DIRECTORY=${schema_directory}
    ...
    : FOR    ${iteration}    IN RANGE    0    ${number_of_devices}
...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device
    [Arguments]    ${operation}    ${deadline_Date}    ${base_netconf_device_port}=17830    ${device_name_prefix}=${DEVICE_NAME_BASE}
...
Perform_Operation_On_Each_Device
    [Arguments]    ${operation}    ${count}=${NetconfKeywords__testtool_device_count}    ${timeout}=30m    ${base_netconf_device_port}=17830    ${device_name_prefix}=${DEVICE_NAME_BASE}
    ${current_Date}=    DateTime.Get_Current_Date
    ...
    BuiltIn.Repeat_Keyword    ${count} times    NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device    ${operation}    ${deadline_Date}    ${base_netconf_device_port}    ${device_name_prefix}

...
NetconfKeywords__Check_Testtool_Is_Up
    [Arguments]    ${last-port}
...
Install_And_Start_Testtool
    ...
    ${last-port}=    BuiltIn.Evaluate    ${first_testtool_port}+${device-count}-1
    BuiltIn.Wait_Until_Keyword_Succeeds    ${timeout}s    1s    NetconfKeywords__Check_Testtool_Is_Up    ${last-port}

...
Delete_Whole_Netconf_Topology
    [Arguments]    ${session}=default    ${location}=location    ${topology_location}=None
    [Documentation]    Tell Netconf to deconfigure all devices
    TemplatedRequests.Delete_Templated    ${DIRECTORY_WITH_DEVICE_TEMPLATES}/${topology_location}   session=${session}    location=${location}

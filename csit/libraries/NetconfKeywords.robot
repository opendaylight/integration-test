...
Configure_Devices_In_Netconf
    [Arguments]    ${device_name_prefix}    ${device_type}=default    ${device_port}=${FIRST_TESTTOOL_PORT}    ${device_address}=${TOOLS_SYSTEM_IP}    ${device_user}=admin    ${device_password}=topsecret
    ...    ${session}=default    ${schema_directory}=/tmp/schema    ${http_timeout}=${EMPTY}    ${iterations}=${EMPTY}    ${iter_start}=0
    [Documentation]    Tell Netconf about the specified devices so it can add it into its configuration.
    ${mapping}=    BuiltIn.Create_dictionary    DEVICE_IP=${device_address}    DEVICE_NAME=${device_name_prefix}    DEVICE_PORT=${device_port}    DEVICE_USER=${device_user}    DEVICE_PASSWORD=${device_password}
    ...    SCHEMA_DIRECTORY=${schema_directory}
    TemplatedRequests.Put_As_Xml_Templated    folder=${DIRECTORY_WITH_DEVICE_TEMPLATES}${/}${device_type}    mapping=${mapping}    session=${session}    iterations=${iterations}    iter_start=${device_port}    http_timeout=${http_timeout}
...
Delete_Whole_Netconf_Topology
    [Arguments]    ${device_type}    ${session}=default    ${location}=location
    [Documentation]    Tell Netconf to deconfigure all devices
    TemplatedRequests.Delete_Templated    ${DIRECTORY_WITH_DEVICE_TEMPLATES}${/}${device_type}    session=${session}    location=${location}
...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device
    [Arguments]    ${operation}    ${deadline_Date}    ${BASE_NETCONF_DEVICE_PORT}=17830    ${DEVICE_NAME_BASE}=netconf-scaling-device
...
Perform_Operation_On_Each_Device
    [Arguments]    ${operation}    ${count}=${NetconfKeywords__testtool_device_count}    ${timeout}=30m    ${BASE_NETCONF_DEVICE_PORT}=17830    ${DEVICE_NAME_BASE}=netconf-scaling-device
    ${current_Date}=    DateTime.Get_Current_Date
    ...
    BuiltIn.Repeat_Keyword    ${count} times    NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device    ${operation}    ${deadline_Date}    ${BASE_NETCONF_DEVICE_PORT}    ${DEVICE_NAME_BASE}

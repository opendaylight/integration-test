...
Check_Topology_Completely_Gone
    ...
    ${status}    ${response}=    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Get_As_Xml_From_Uri    ${uri}    session=${session}
    BuiltIn.Should_Be_Equal_As_Strings    ${status}    PASS
    BuiltIn.Log    ${response}
    BuiltIn.Should_Contain    ${response}    data-missing
    TemplatedRequests.Check_Status_Code    ${response}    explicit_status_codes=404

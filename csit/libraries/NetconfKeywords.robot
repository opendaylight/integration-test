...
Delete_Whole_Netconf_Topology
    [Arguments]    ${session}=default    ${location}=location    ${devices_type}=None
    [Documentation]    Tell Netconf to deconfigure all devices
    TemplatedRequests.Delete_Templated    ${DIRECTORY_WITH_DEVICE_TEMPLATES}/${devices_type}   session=${session}    location=${location}

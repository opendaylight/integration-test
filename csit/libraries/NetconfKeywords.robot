...
Check_Device_Up_And_Running
    ...
    # Will fail if ${device-number} == ${EMPTY}
...
NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device_Using_New_Name
    [Documentation]    Private function of Perform_Operation_On_Each_Device except with using the new name generated by netconf-testtool.
    [Arguments]    ${operation}    ${deadline_Date}
    NetconfKeywords__Check_Netconf_Test_Timeout_Not_Expired    ${deadline_Date}
    ${number}=    BuiltIn.Evaluate    ${current_port}-${BASE_NETCONF_DEVICE_PORT}+1
    ${device_name}=    BuiltIn.Set_Variable    ${current_port}-sim-device
    ${operation_start}=    DateTime.Get_Current_Date
    BuiltIn.Run_Keyword    ${operation}    ${device_name}
    ${operation_end}=    DateTime.Get_Current_Date
    ${operation_delta}=    DateTime.Subtract_Date_From_Date    ${operation_end}    ${operation_start}
    ${operation_delta}=    DateTime.Convert_Time    ${operation_delta}    verbose
    Log    Operation '${operation}' took ${operation_delta} to complete on device '${device_name}'
    ${next}=    BuiltIn.Evaluate    ${current_port}+1
    BuiltIn.Set_Suite_Variable    ${current_port}    ${next}

Perform_Operation_On_Each_Device_Using_New_Name
    [Documentation]    Perform a keyword operation on each device until timeout using new names generated by netconf-testtool. Default timeout is 30 minutes.
    [Arguments]    ${operation}    ${count}=${NetconfKeywords__testtool_device_count}    ${timeout}=30m
    ${start_date}=    DateTime.Get_Current_Date
    ${deadline_Date}=    DateTime.Add_Time_To_Date    ${start_date}    ${timeout}
    BuiltIn.Set_Suite_Variable    ${current_port}    ${BASE_NETCONF_DEVICE_PORT}
    BuiltIn.Repeat_Keyword    ${count} times    NetconfKeywords__Perform_Operation_With_Checking_On_Next_Device_Using_New_Name    ${operation}    ${deadline_Date}
    ${end_date}=    DateTime.Get_Current_Date
    ${delta_date}=    DateTime.Subtract_Date_From_Date    ${end_date}    ${start_date}
    ${delta_date}=    DateTime.Convert_Time    ${delta_date}    verbose
    Log    The entire operation, for all devices, took ${delta_date}
...
Check_If_Data_Present_On_Device
    ...
    ...    and optionally check if ${data} equals ${expected_data}, where ${data} is the output of TemplatedRequests.Get_As_Xml_From_Uri
    ...
    ${device_number}=    BuiltIn.Evaluate   int('${device_name}'.split("-")[0]) - ${BASE_NETCONF_DEVICE_PORT}

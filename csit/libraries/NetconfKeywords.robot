...
NetconfKeywords__Deploy_Custom_RPC
    ...
    SSHKeywords.Copy_File_To_Tools_System  127.0.0.1  ${rpc_config}  /tmp
    BuiltIn.Log To Console    STEP4
    # Construct a command line argument pointing to custom rpc
    OperatingSystem.File Should Exist    /tmp/customaction.xml
    ${out}=    OperatingSystem.Run    cat /tmp/customaction.xml
    BuiltIn.Log To Console    WAS#2

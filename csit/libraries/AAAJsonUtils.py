...
def get_id_by_name(args):
    ...
    if ncount > 0:
        for i in range(ncount):
            ...
            name_record = jsonpath_rw.jsonpath(jsonobj, bkey1)
            # find corresponding node info, for that name
            node_record = jsonpath_rw.jsonpath(jsonobj, bkey2)
            ...
            # find records with same name
            altname_record = jsonpath_rw.jsonpath(jsonobj, bkey3)
            # find corresponding record node info, for that name
            altnode_record = jsonpath_rw.jsonpath(jsonobj, bkey4)
            try:

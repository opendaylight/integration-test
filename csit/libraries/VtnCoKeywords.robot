...
Start SuiteVtnCo
    ...
    SSHLibrary.Execute Command    wget "${boron_nexus_path}/maven-metadata.xml"
    SSHLibrary.Execute Command    sudo cp maven-metadata.xml vtncoo_metadata.xml
    SSHLibrary.Put_File    vtncoo_metadata.xml    ${WORKSPACE}
    ${time_stamp}=    XML.Get Element Text    ${WORKSPACE}/vtncoo_metadata.xml    xpath=.//snapshot/timestamp
    ${build_number}=    XML.Get Element Text    ${WORKSPACE}/vtncoo_metadata.xml    xpath=.//snapshot/buildNumber
    SSHLibrary.Execute Command    wget '${boron_nexus_path}/distribution.vtn-coordinator-6.3.0-${time_stamp}-${build_number}-bin.tar.bz2'
    SSHLibrary.Execute Command    tar -C/ -jxvf distribution.vtn-coordinator*-bin.tar.bz2
    #SSHLibrary.Execute Command    tar -C/ -jxvf /tmp/*vtn-coordinator*-bin.tar.bz2

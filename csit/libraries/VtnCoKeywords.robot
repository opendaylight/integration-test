...
Download VtnCo Distribution
    ...
    CompareStream.Run_Keyword_If_At_Least_Boron    SSHLibrary.Get_file    maven-metadata.xml
    ${snapshot_version}=    XML.Get Element Text    maven-metadata.xml    xpath=.//snapshotVersions/snapshotVersion[1]/value
    CompareStream.Run_Keyword_If_Equals    boron    SSHLibrary.Execute Command    wget '${nexus_path}/${boron_version}/${vtn_dist}-${snapshot_version}-bin.tar.bz2'
    CompareStream.Run_Keyword_If_Equals    carbon    SSHLibrary.Execute Command    wget '${nexus_path}/${carbon_version}/${vtn_dist}-${snapshot_version}-bin.tar.bz2'

...
Get Bindings
    ...
    ${resp}    CompareStream.Run_Keyword_If_At_Least_Boron    Post Request    ${session}    ${REST_CONTEXT}:get-node-bindings    data=${DATA}
    ...    headers=${HEADERS_XML}
    ...    ELSE    Get Request    ${session}    /restconf/operational/network-topology:network-topology/topology/sxp/node/${node}/master-database/    headers=${HEADERS_XML}
    Should be Equal As Strings    ${resp.status_code}    200
    [Return]    ${resp.content}
...
Clean Bindings
    ...
    \    CompareStream.Run_Keyword_If_At_Least_Boron    Clean Binding    ${binding['sgt']}    ${binding['ip-prefix']}    ${node}
    \    ...    ${session}    ${domain}
...
Clean Binding
    ...
    \    CompareStream.Run_Keyword_If_At_Least_Boron    Delete Binding    ${sgt}    ${prefix}    ${node}
    \    ...    ${domain}    ${session}
...
Setup SXP Environment
    ...
    \    CompareStream.Run_Keyword_If_At_Least_Boron    Wait Until Keyword Succeeds    20    1    Check Node Started
    \    ...    ${ip}

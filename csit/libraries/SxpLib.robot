...
Get Bindings
    ...
    ${resp}    CompareStream.Run_Keyword_If_At_Least_Else    boron    TemplatedRequests.Post_To_Uri    ${REST_CONTEXT}:get-node-bindings    data=${DATA}     accept=${ACCEPT_JSON}    content_type=${HEADERS_XML}    session=${session}
    ...    ELSE    TemplatedRequests.Get_As_Json_From_Uri    /restconf/operational/network-topology:network-topology/topology/sxp/node/${node}/master-database/    session=${session}
    [Return]    ${resp}
...
#    \    ...    ${domain}
#    \    CompareStream.Run_Keyword_If_At_Most_Beryllium    Clean Binding    ${binding}    ${binding['binding']}    ${node}    ${session}
#    \    ...    ${domain}
    \    CompareStream.Run_Keyword_If_At_Least_Boron    Clean Binding Default    ${binding}    ${node}    ${session}
    \    ...    ${domain}
    \    CompareStream.Run_Keyword_If_At_Most_Beryllium    Clean Binding At Most Be    ${binding}    ${node}    ${session}
    \    ...    ${domain}

Clean Binding Default
    [Arguments]    ${binding}    ${node}    ${session}

...
Clean Bindings
    ...
    ${resp}    Get Bindings    ${node}    ${session}    ${domain}    all
...
Bindings Should Not Contain
    [Arguments]    ${sgt}    ${prefix}    ${domain}=global    ${origin}=all
    [Documentation]    Retrieves bindings and verifies they do not contain given binding
    ${resp}    Get Bindings    127.0.0.1    session    ${domain}    ${origin}

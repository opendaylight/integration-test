...
Setup SXP Environment
    ...
    \    Run Keyword If    '${ODL_STREAM}' == 'boron'    Wait Until Keyword Succeeds    30    1    Check Node Started
    \    ...    ${ip}
...
Check Node Started
    ...
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}    Run    netstat -tan | grep LISTEN | grep ${node}:${port}
    LOG    ${resp}
    ${rc}    Run And Return Rc    netstat -tan | grep LISTEN | grep ${node}:${port}

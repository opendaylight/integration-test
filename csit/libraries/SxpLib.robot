...
Prepare SSH Keys On Karaf
    [Arguments]    ${system}=${ODL_SYSTEM_IP}    ${user}=${ODL_SYSTEM_USER}    ${passwd}=${ODL_SYSTEM_PASSWORD}    ${prompt}=${ODL_SYSTEM_PROMPT}
    [Documentation]    Create session to Controller
    ${stdout}    Run Command On Remote System    ${system}    ${KARAF_HOME}/bin/client echo READY    ${user}    ${passwd}    prompt=${prompt}

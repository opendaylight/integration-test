...
Setup SXP Session
    [Arguments]    ${session}=session    ${address}=${ODL_SYSTEM_IP}
    [Documentation]    Create session to Controller
    Verify Feature Is Installed    odl-sxp-controller
    Create Session    ${session}    url=http://${address}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS_XML}
...
Check Node Started
    [Arguments]    ${node}    ${port}=64999    ${system}=${ODL_SYSTEM_IP}    ${session}=session

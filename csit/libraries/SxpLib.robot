...
Check RPC response
    [Arguments]    ${resp}
    [Documentation]    Check if RPC request was success
    Should be Equal As Strings    ${resp.status_code}    200
    ${check}    Check Response    ${resp.content}
...
Get Bindings
    [Arguments]    ${node}=127.0.0.1    ${session}=session    ${domain}=global    ${range}=all
    [Documentation]    Gets all binding via RPC from Master DB of node
    ${DATA}    Get Bindings From Node Xml    ${node}    ${range}    ${domain}
...
Add Domain Filter
    [Arguments]    ${name}    ${domains}    ${entries}    ${node}=127.0.0.1    ${session}=session
    [Documentation]    Add Domain Filter via RPC from Node
    ${DATA}    Add Domain Filter Xml    ${name}    ${domains}    ${entries}    ${node}
...
Delete Domain Filter
    [Arguments]    ${name}    ${node}=127.0.0.1    ${session}=session
    [Documentation]    Delete Filter via RPC from Node
    ${DATA}    Delete Filter Xml    ${name}    ${node}
...
Check Node Started
    ...
    Should Be Equal As Strings    ${resp.status_code}    200
    ${output}    Run    netstat -ln
    LOG    ${output}
    ${rc}    ${output}    Run And Return Rc and Output    netstat -tln | grep ${node}:${port}

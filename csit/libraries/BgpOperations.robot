...
Start Quagga Processes On ODL
    ...
    ${conn_id}=    Get System Connection ID    ${odl_ip}
    Log    ${conn_id}
...
Start Quagga Processes On DCGW
    ...
    ${dcgw_conn_id}=    Get System Connection ID    ${dcgw_ip}
...
Create Quagga Telnet Session
    ...
    ${conn_id}=    Get System Connection ID    ${ip}
    Switch Connection    ${conn_id}
...
Add Address Family On DCGW
    [Arguments]    ${dcgw_ip}    ${loopback_ip}    ${as_id}    ${rd}    ${user}=zebra    ${password}=zebra

...
Play_To_Odl_Non_Removal_Template
    [Arguments]    ${path_to_library}    ${ip}    ${custom_lib_name}    ${totest}    ${dir}    ${ipv}=ipv4
    ${announce_hex}=    OperatingSystem.Get_File    ${dir}/${totest}/announce_${totest}.hex
    BuiltIn.Import Library    ${path_to_library}    ${ip}    WITH NAME    ${custom_lib_name}
    ${custom_lib_name}.play_clean
    ${custom_lib_name}.play_send    ${announce_hex}
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${dir}/${totest}/rib    mapping=${LOC_RIB}    session=${CONFIG_SESSION}
    ...    verify=True
...
Play_To_Odl_Routes_Removal_Template
    ...
    BuiltIn.Import Library    ${path_to_library}    ${ip}    WITH NAME    ${custom_lib_name}
    ${custom_lib_name}.play_clean
    ${custom_lib_name}.play_send    ${withdraw_hex}

Get_Update_Message
    [Arguments]    ${path_to_library}    ${ip}    ${custom_lib_name}
    [Documentation]    Returns hex update message.
    BuiltIn.Import Library    ${path_to_library}    ${ip}    WITH NAME    ${custom_lib_name}
    ${update} =    ${custom_lib_name}.play_get

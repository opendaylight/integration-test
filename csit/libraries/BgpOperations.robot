...
Check_Example_Bgp_Rib_Content
    [Arguments]    ${substr}    ${error_message}=${JSONKEYSTR} not found, but expected.    ${session}
...
Check_Example_Bgp_Rib_Does_Not_Contain
    [Arguments]    ${substr}    ${error_message}=${JSONKEYSTR} found, but not expected.    ${session}
...
Check_Example_IPv4_Topology_Content
    [Arguments]    ${string_to_check}=${EMPTY}    ${session}
...
Check_Example_IPv4_Topology_Does_Not_Contain
    [Arguments]    ${string_to_check}    ${session}

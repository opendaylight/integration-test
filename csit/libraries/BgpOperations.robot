...
Start Quagga Processes On ODL
    ...
    Write Commands Until Expected Prompt    sudo /opt/quagga/etc/init.d/zrpcd start -N ${odl_ip}    ${DEFAULT_LINUX_PROMPT_STRICT}
...
Configure BGP On DCGW
    [Arguments]    ${dcgw_ip}    ${as_id}    ${router_id}
    [Documentation]    Configure BGP on the dcgw
...
Configure BGP Neighbor On DCGW
    [Arguments]    ${dcgw_ip}    ${as_id}    ${neighbor_ip}    ${nbr_as_id}    @{addr_families} 
    [Documentation]    Configure BGP neighbor on the dcgw
    Create Quagga Telnet Session    ${dcgw_ip}    bgpd    sdncbgpc
    Execute Command On Quagga Telnet Session    configure terminal
    Execute Command On Quagga Telnet Session    router bgp ${as_id}
    Execute Command On Quagga Telnet Session    neighbor ${neighbor_ip} send-remote-as ${nbr_as_id}
...
Get BGP Neighbor Status On DCGW
...
Is ODL Fib Contains Routes From DCGW

...
Abort_Execution
    ...
    # TODO: If there were excess newlines, there may be more prompt lines waiting. Should we read them?
    BuiltIn.Log    ${text}
    [Return]    ${text}
...
Wait_Without_Idle
    ...
    [Documentation]    Wait until prompt, while sending newlines to avoid idling. Return the text before prompt.
    ${text} =   Wait_Until_Keyword_Succeeds    ${timeout}    ${refresh}    Wait_Iteration
    [Return]    ${text}

Flush_Read
    [Documentation]    Read, Log and return excess data (probably just multiple prompts).
    ${data} =    SSHLibrary.Read
...
Invoke_With_Timeout
    ...
    [Return]   ${text}
    [Teardown]    RemoteBash__Teardown_For_Invoke    ${KEYWORD_STATUS}

RemoteBash__Teardown_For_Invoke
    [Arguments]    ${status}
    # Status of Invoke_With_Timeout. ${KEYWORD_STATUS} here would give status of RemoteBash__Teardown_For_Invoke which has not failed yet.
    [Documentation]    If the timeout was exceeded, abort the tool. Unconditionally flush read.
    BuiltIn.Run_Keyword_If    """${status}""" != "PASS"    Abort_Execution
    [Teardown]    Flush_Read

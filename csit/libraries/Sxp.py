...
def get_bindings_from_node_xml(ip, bindings_type):
    ...
    templ = Template('''<input>
  <requested-node xmlns="urn:opendaylight:sxp:controller">$ip</requested-node>
  <bindings-type xmlns="urn:opendaylight:sxp:controller">$bindings_type</bindings-type>
</input>''')
    data = templ.substitute({'ip': ip, 'bindings_type': bindings_type})
    return data

...
def parse_prefix_groups(input, source_):
    ...
    output = []
    for list in bindings.values():
        for source in list:
            if source['binding-source'] == source_:
                for prefixgroup in source['prefix-group']:
                    output.append(prefixgroup)
    return output
...
def find_binding_with_peer_sequence(input, sgt, prefix, source_, action, peer_source_, peer_seq):
            ...
            for binding in prefixgroup['binding']:
                if binding['ip-prefix'] == prefix and binding['action'] == action:
                    for peer in binding['peer-sequence']['peer']:
                        if peer['seq'] == int(peer_seq) and peer['node-id'] == peer_source_:
                            correct_sequence = True
                    for peer_source in binding['sources']['source']:
                        if peer_source == peer_source_:
                            found_source = True
...
def delete_connections_xml(ip, port, ip_):
    templ = Template('''<input>
   <requested-node xmlns="urn:opendaylight:sxp:controller">$ip_</requested-node>
   <peer-address xmlns="urn:opendaylight:sxp:controller">$ip</peer-address>
   <tcp-port xmlns="urn:opendaylight:sxp:controller">$port</tcp-port>
</input>''')
    data = templ.substitute({'ip': ip, 'port': port, 'ip_': ip_})
    return data

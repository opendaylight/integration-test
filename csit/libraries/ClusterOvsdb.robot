...
Configure Exit Netvirt Connection
    ...
    ${show_list}=    Utils.Run Command On Mininet    ${OS_CONTROL_NODE_IP}    sudo ovs-vsctl show
    Log    ${show_list}
    OVSDB.Clean OVSDB Test Environment    ${OS_COMPUTE_1_IP}
    ${show_list}=    Utils.Run Command On Mininet    ${OS_COMPUTE_1_IP}    sudo ovs-vsctl show
    Log    ${show_list}
    OVSDB.Clean OVSDB Test Environment    ${OS_COMPUTE_2_IP}
    ${show_list}=    Utils.Run Command On Mininet    ${OS_COMPUTE_2_IP}    sudo ovs-vsctl show
    Log    ${show_list}
    ${dictionary1}=    Create Dictionary    ovsdb:1=1
    ${dictionary2}=    Create Dictionary    netvirt:1=1
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${OPERATIONAL_TOPO_API}    dictionary=${dictionary1}    member_index_list=${controller_index_list}
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${OPERATIONAL_TOPO_API}    dictionary=${dictionary2}    member_index_list=${controller_index_list}

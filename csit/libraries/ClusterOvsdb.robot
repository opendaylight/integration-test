...
Delete Internal Bridge Manually And Verify
    ...
    ${show_list_compute_1}=    Utils.Run Command On Mininet    ${OS_COMPUTE_1_IP}    sudo ovs-vsctl show
    ${show_list_compute_2}=    Utils.Run Command On Mininet    ${OS_COMPUTE_2_IP}    sudo ovs-vsctl show
    Log    ${show_list}
    Log    ${show_list_compute_1}
    Log    ${show_list_compute_2}
    Log    ${controller_index_list}
    Wait Until Keyword Succeeds    5s    1s    ClusterKeywords.Check Item Occurrence At URI In Cluster    ${controller_index_list}    ${dictionary}    ${OPERATIONAL_TOPO_API}

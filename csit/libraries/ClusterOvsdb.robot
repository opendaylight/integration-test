...
Create Bridge And Verify
    ...
    Log    ${body}
    ${dictionary}=    Create Dictionary    br01=1
...
Check Expected And Received Body
    ...
    ${expected_splitted}=    Split String    ${expected_body}    :
    Log    ${expected_splitted}
    ${received_splitted}=    Split String    ${received_body}    :
    Log    ${received_splitted}
    ${size}=    Get Length    ${expected_splitted}
    : FOR    ${index}    IN RANGE    ${size}
    \    ${expected_element}=    Get From List    ${expected_splitted}    ${index}
    \    Should Contain    ${received_splitted}    ${expected_element}

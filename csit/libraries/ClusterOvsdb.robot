...
Put And Check At URI In Cluster Ovsdb
    ...
    \    Check Expected And Received Body    ${body}    ${data}
...
Check Expected And Received Body
    ...
    Log    ${body}
    Log    ${data}
    ${bodysplitted}=    Split String    ${body}    :
    ${datasplitted}=    Split String    ${data}    :
    ${size}=    Get Length    ${bodysplitted}
    Log    ${size}
    : FOR    ${index}    IN RANGE    ${size}
    \    ${expected_element}=    Get From List    ${bodysplitted}    ${index}
    \    Should Contain    ${datasplitted}    ${expected_element}

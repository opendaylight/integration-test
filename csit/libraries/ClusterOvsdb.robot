...
Delete Managers in OVS instance
    ...
    [Documentation]   Delete the OVSDB node.
...
Create Bridge Before OVSDB Node Reconnect
    [Arguments]    ${controller_index}    ${controller_index_list}=${EMPTY}
    [Documentation]    Create bridge ${BRIDGE} in controller ${controller_index} and verify it gets created in all instances in ${controller_index_list}.
    ${body}=    OperatingSystem.Get File    ${CURDIR}/../variables/ovsdb/create_bridge_3node.json
    ${body}    Replace String    ${body}    ovsdb://127.0.0.1:61644    ovsdb://uuid/${ovsdb_uuid}
...
Verify Operational Topology after Deletion of OVSDB Node
    [Arguments]    ${ovs_system_ip}=${TOOLS_SYSTEM_IP}    ${controller_index_list}=${EMPTY}
    [Documentation]   Verify bridge ${BRIDGE} is not updated in operational topology after the OVSDB node is deleted.
    ${dictionary_operational}=    Create Dictionary    br01=0
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${OPERATIONAL_TOPO_API}    dictionary=${dictionary_operational}    member_index_list=${controller_index_list}

Verify Configuration Topology after Deletion of OVSDB Node
    [Arguments]    ${ovs_system_ip}=${TOOLS_SYSTEM_IP}    ${controller_index_list}=${EMPTY}
    [Documentation]   Verify bridge ${BRIDGE} is not updated in configuration topology after the OVSDB node is deleted.
    ${dictionary_config}=    Create Dictionary    br01=0
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${CONFIG_TOPO_API}    dictionary=${dictionary_config}    member_index_list=${controller_index_list}

Verify Operational Topology After Node Reconnect
    [Arguments]    ${ovs_system_ip}=${TOOLS_SYSTEM_IP}    ${controller_index_list}=${EMPTY}
    [Documentation]   Verify bridge ${BRIDGE} is updated in operational topology after the OVSDB node is reconnect.
    ${dictionary_operational}=    Create Dictionary    br01=6
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${OPERATIONAL_TOPO_API}    dictionary=${dictionary_operational}    member_index_list=${controller_index_list}

Verify Configuration Topology After Node Reconnect
    [Arguments]    ${ovs_system_ip}=${TOOLS_SYSTEM_IP}    ${controller_index_list}=${EMPTY}
    [Documentation]   Verify bridge ${BRIDGE} is updated in configuration topology after the OVSDB node is reconnect.
    ${dictionary_config}=    Create Dictionary    br01=2
    Wait Until Keyword Succeeds    5s    1s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${CONFIG_TOPO_API}    dictionary=${dictionary_config}    member_index_list=${controller_index_list}

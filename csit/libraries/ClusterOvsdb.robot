...
Take Ovsdb Device Link Down and Verify
    ...
    Log    {$CONTROLLER}
    Log    ${CONTROLLER1}
    ${ouput}=    Send Mininet Command    ${mininet_conn_id}    sudo mn --controller=remote,ip=${CONTROLLER} --topo=linear,2 --switch ovsk,protocols=OpenFlow13
    Log    ${output}
    ${ouput1}=    Send Mininet Command    ${mininet_conn_id}    link s1 h1 down
    Log    ${output1}

...
Configure_Next_Hops_On_Router
    ...
    @{next_hop_list_2} =    MultiPathOperations.Generate_Next_Hops    ${static_ip_2}    ${mask}    @{vm_list_2}
    ${final_route} =    BuiltIn.Catenate    @{next_hop_list_1}    @{next_hop_list_2}
    OpenStackOperations.Update Router    ${router_name}    ${final_route}
    OpenStackOperations.Show Router    ${router_name}

Verify_Multipath_Bucket_From_Flows_In_Compute_Node

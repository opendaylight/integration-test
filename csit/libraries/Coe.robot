...
Verify Config Files
    ...
    Utils.Verify File Exists On Remote System    ${K8s_MASTER_IP}    ${CONFIG_FILE}
    Utils.Verify File Exists On Remote System    ${K8s_MINION1_IP}    ${CONFIG_FILE}
    Utils.Verify File Exists On Remote System    ${K8s_MINION2_IP}    ${CONFIG_FILE}
    Utils.Verify File Exists On Remote System    ${K8s_MASTER_IP}    ${CNI_BINARY_FILE}
    Utils.Verify File Exists On Remote System    ${K8s_MINION1_IP}    ${CNI_BINARY_FILE}
    Utils.Verify File Exists On Remote System    ${K8s_MINION2_IP}    ${CNI_BINARY_FILE}
...
Verify Node Status
    [Documentation]    Waits till the keyword Check Node Status succeeds implying that all nodes are ready
...
Check Node Status
    ...
    : FOR    ${node}    IN    @{cluster}
    \    BuiltIn.Should Match Regexp    ${node}    ${NODE_STATUS}
...
Check Pod Status
    ...
    : FOR    ${pod}    IN    @{cluster}
    \    BuiltIn.Should Match Regexp    ${pod}    ${POD_STATUS}
    SSHLibrary.Close Connection
...
Check If Pods Are Terminated
    ...
    ${status} =    Utils.Run Command On Remote System    ${K8s_MASTER_IP}    kubectl get pods -o wide    ${DEFAULT_USER}    \    ${EMPTY}
...
Dump Config File
    ...
    Utils.Run Command On Remote System And Log    ${K8s_MASTER_IP}    cat ${CONFIG_FILE}
    Utils.Run Command On Remote System And Log    ${K8s_MINION1_IP}    cat ${CONFIG_FILE}
    Utils.Run Command On Remote System And Log    ${K8s_MINION2_IP}    cat ${CONFIG_FILE}
...
Stop Suite
    ...
    Coe.Kube_reset    ${K8s_MINION1_IP}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    Coe.Kube_reset    ${K8s_MINION2_IP}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}

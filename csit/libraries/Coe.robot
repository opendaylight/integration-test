...
Collect Pod Names and Ping
    ...
    ${lines} =    Utils.Run Command On Remote System    ${K8s_MASTER_IP}    kubectl get pods -o wide
    @{pods}=    String.Get Regexp Matches    ${lines}    ^\\w+-\\w+
...
Ping Pods
    ...
    SSHLibrary.Open Connection    ${K8s_MASTER_IP}
    SSHKeywords.Flexible_SSH_Login    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    @{pods}=    String.Get Regexp Matches    ${lines}    \\d+.\\d+.\\d+.\\d+
    : FOR    ${pod ip}    IN    @{pods}
    \    ${ping} =    Utils.Write Commands Until Expected Prompt    ${K8s_MASTER_IP}    kubectl exec -it ${pod_name} -- ping -c 3 ${pod_ip}
    \    BuiltIn.log    ${ping}

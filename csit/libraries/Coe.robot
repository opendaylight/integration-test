...
Modifying templates in playbook
    [Documentation]    Parameters that have to be assigned during run time are given values by overwriting the string in template file in this keyword.
...
Log Statements
    ...
    [Documentation]    Log statements provides clarity about the two pods piniging each other.
...
Ping Pods
    ...
    [Documentation]    This keyword checks if ping between two pods is successful.
    ${i} =    Set Variable    0

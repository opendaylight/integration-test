...
Start_Suite
    ...
    ${d} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    sudo apt-get install curl
    log    ${d}
    ${b} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    go version
    log    ${b}
    ${c} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    curl https://glide.sh/get | sh
    log    ${c}
    ${e} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_2_IP}    curl https://glide.sh/get | sh
    log    ${e}
    ${f} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_3_IP}    curl https://glide.sh/get | sh
    log    ${f}
    ${a} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    glide --version
    log    ${a}

...
Conn_id
    ...
    Log    ${TOOLS_SYSTEM_IP}
    Log    ${TOOLS_SYSTEM_1_IP}
    Log    ${TOOLS_SYSTEM_2_IP}
    Log    ${TOOLS_SYSTEM_3_IP}

Verify_Config_Files
    [Documentation]    Checks if the configuration files are present in all nodes
    Utils.Verify File Exists On Remote System    ${TOOLS_SYSTEM_IP}    ${CONFIG_FILES_PATH}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}
    Utils.Verify File Exists On Remote System    ${TOOLS_SYSTEM_2_IP}    ${CONFIG_FILES_PATH}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}
    Utils.Verify File Exists On Remote System    ${TOOLS_SYSTEM_3_IP}    ${CONFIG_FILES_PATH}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}
    Utils.Verify File Exists On Remote System    ${TOOLS_SYSTEM_IP}    ${CNI_BINARY_PATH}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}

Check_Watcher
    [Documentation]    Checks if watcher is running in the background
    ${lines} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    ps -ef | grep watcher    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
...
Verify_Nodes_Status
    ...
    SSHLibrary.Open Connection    ${TOOLS_SYSTEM_IP}
    SSHKeywords.Flexible_SSH_Login    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
...
Create_Label
    ...
    ${nodes} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl get nodes    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    ...
    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl label nodes ${minion_1} disktype=ssd    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl label nodes ${minion_2} disktype=ssl    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl get nodes --show-labels    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
...
Delete_Pods
    ...
    ${lines} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl get pods -o wide    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    @{lines} =    String.Split To Lines    ${lines}    1
    : FOR    ${status}    IN    @{lines}
    \    ${pod_name} =    BuiltIn.Should Match Regexp    ${status}    ^\\w+
    \    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl delete pods ${pod_name}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
...
Delete_Status
    ...
    ${status} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kubectl get pods -o wide    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${EMPTY}
...
Stop_Suite
    ...
    Coe.Stop_Watcher
    Coe.Kube_reset    ${TOOLS_SYSTEM_IP}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
...
Stop_Watcher
    ...
    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    kill -9 ${pid}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}

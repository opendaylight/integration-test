...
Create Vteps
    ...
    ...    ${option_of_tunnel}=default false
    ...
    ...    ${gateway-ip}    ${subnet}    option_of_tunnel=${option_of_tunnel}
...
Set Json
    [Arguments]    ${Dpn_id_1}    ${Dpn_id_2}    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${vlan}    ${gateway-ip}
    ...    ${subnet}    ${option_of_tunnel}=default false
    ...
    ${body}    replace string    ${body}    "option-of-tunnel": "false"    "option-of-tunnel": "${option_of_tunnel}"
    Log    ${body}
...
Ovs Verification For 2 Dpn
    ...
    Log    ${check}
    Run Keyword If    '${remote-1}'=='flow'    Should Contain    ${check}    local_ip="${local}"    remote_ip=flow    ${tunnel}
    ...    ${tunnel-type}

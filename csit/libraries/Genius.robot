...
Check Table0 Entry For 2 Dpn
    [Arguments]    ${connection_id}    ${Bridgename}    ${port_num}
    ...
    Log    ${check}
    ${items}    Get Length    ${port_num}
    :FOR    ${i}    INRANGE    ${items}
    \    Should Contain    ${check}    in_port=${port_num[${i}]}
...
Get Port Number
    ...
    \    log    ${port}
    \    ${check1}    Run Command On Remote System And Log    ${TOOLS_SYSTEM_${i+1}_IP}    sudo ovs-ofctl -OOpenFlow13 dump-flows BR${i}
    \    Should Contain    ${check1}    in_port=${port[${i}]}
    \    Comment    Set Global Variable    ${port}

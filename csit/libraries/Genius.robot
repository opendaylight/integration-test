...
Set Json
    ...
    Comment    ${body} =    BuiltIn.Run Keyword If    &{Stream_dict}[${ODL_STREAM}] <= &{Stream_dict}[neon]    OperatingSystem.Get File    ${genius_config_dir}/Itm_creation_no_vlan_below_sodium.json
    ...    ELSE    OperatingSystem.Get File    ${genius_config_dir}/Itm_creation_no_vlan_sodium_and_above.json
    Comment    ${body} =    BuiltIn.Run Keyword If    &{Stream_dict}[${ODL_STREAM}] <= &{Stream_dict}[neon]    String.Replace String    ${body}    1.1.1.1
    ...    ${subnet}
    ...    ELSE    BuiltIn.Set Variable    ${body}
    Comment    ${body} =    BuiltIn.Run Keyword If    &{Stream_dict}[${ODL_STREAM}] <= &{Stream_dict}[neon]    String.Replace String    ${body}    "vlan-id": 0
    ...    "vlan-id": ${vlan}
    ...    ELSE    BuiltIn.Set Variable    ${body}
    Comment    ${body} =    BuiltIn.Run Keyword If    &{Stream_dict}[${ODL_STREAM}] <= &{Stream_dict}[neon]    String.Replace String    ${body}    "gateway-ip": "0.0.0.0"
    ...    "gateway-ip": "${gateway_ip}"
    ...    ELSE    BuiltIn.Set Variable    ${body}
    ${body} =    CompareStream.Run_Keyword_If_Less_Than_Sodium    OperatingSystem.Get File    ${genius_config_dir}/Itm_creation_no_vlan_below_sodium.json
    ...    ELSE    OperatingSystem.Get File    ${genius_config_dir}/Itm_creation_no_vlan_sodium_and_above.json
    ${body} =    CompareStream.Run_Keyword_If_Less_Than_Sodium    String.Replace String    ${body}    1.1.1.1    ${subnet}
    ...    ELSE    Set Variable    ${body}
    ${body} =    CompareStream.Run_Keyword_If_Less_Than_Sodium    String.Replace String    ${body}    "vlan-id": 0    "vlan-id": ${vlan}
    ...    ELSE    Set Variable    ${body}
    ${body} =    CompareStream.Run_Keyword_If_Less_Than_Sodium    String.Replace String    ${body}    "gateway-ip": "0.0.0.0"    "gateway-ip": "${gateway_ip}"
    ...    ELSE    Set Variable    ${body}
    log    ${body}
    Comment    ${body} =    CompareStream.Run_Keyword_If_At_Least_Sodium    OperatingSystem.Get File    ${genius_config_dir}/Itm_creation_no_vlan_sodium_and_above.json
    Comment    ${body} =    CompareStream.Run_Keyword_If_At_Least_Sodium    BuiltIn.Set Variable    ${body}
    : FOR    ${tool_system_index}    IN RANGE    ${NUM_TOOLS_SYSTEM}

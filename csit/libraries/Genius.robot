...
Check Table0 Entry For 2 Dpn
    ...
    \    Should Contain    ${check}    in_port=${port_number}
...
Get Port Number
    : FOR    ${i}    INRANGE    ${NUM_TOOLS_SYSTEM}
    \    ${check}    Run Command On Remote System And Log    ${conn_id_list[${i}]}    sudo ovs-ofctl -O OpenFlow13 show ${BR[${i}]}
    \    ${lines}    Get Lines Matching Regexp    ${check}    tun[\\w\\d]+

...
Stop Suite
    ...
    : FOR    ${tool_system_index}    INRANGE    ${NUM_TOOLS_SYSTEM}
...
ITM Direct Tunnels Stop Suite
    ...
    : FOR    ${controller_index}    INRANGE    ${NUM_ODL_SYSTEM}
    \    Utils.Run Command On Remote System And Log    ${ODL_SYSTEM_${controller_index+1}_IP}    sed -i -- 's/<itm-direct-tunnels>true/<itm-direct-tunnels>true/g' ${GENIUS_IFM_CONFIG_FLAG}
    Genius Suite Teardown
...
Check Table0 Entry In a Dpn
    ...
    : FOR    ${port_index}    INRANGE    ${num_ports}
...
Verify Tunnel Status
    ...
    : FOR    ${each_tunnel}    INRANGE    ${number_of_tunnels}
...
Get Tunnel
    ...
    ${resp} =    Run Keyword If    '${config_api_type}' == '${EMPTY}'    RequestsLibrary.Get Request    session    ${CONFIG_API}/itm-state:tunnel-list/internal-tunnel/${src}/${dst}/${type}/
    ...
    Run Keyword If    '${config_api_type}' == '${EMPTY}'    BuiltIn.Should Contain    ${resp.content}    ${src}
    ...
    ${ovs bridge output}    ${bridge} =    Run Keyword Unless    '${config_api_type}' == '${EMPTY}'    BuiltIn.Should Match Regexp    ${resp.content}    "tunnel-name":"(tun[\\w\\d]+)"
    [Return]    ${tunnel}
...
Verify Data From Dpn endpoint config API
...
Get tunnel between DPNs
    [Arguments]    ${tunnel_type}    ${config_api_type}
    [Documentation]    This keyword will Get All the Tunnels available on DPN's
    : FOR    ${tool_system_index}    INRANGE    ${NUM_TOOLS_SYSTEM} -1
    ...
    BuiltIn.Set Suite Variable    ${dpn_id_index}
...
Update Dpn id list and get tunnels
    ...
    \    Get tunnel between DPNs    ${tunnel_type}    ${config_api_type}

...
Start Suite
    ...
    Comment    : FOR    ${i}    IN RANGE    ${NUM_TOOLS_SYSTEM}
    Comment    \    Collections.Append To List    ${BRIDGE_LIST}    BR${i}
    Comment    BuiltIn.Log    ${BRIDGE_LIST}
    Comment    BuiltIn.Set Global Variable    ${BRIDGE_LIST}
    ...
    @{data}    Collections.Combine Lists    ${DPN_ID_LIST}    ${TOOLS_SYSTEM_LIST}
...
Build Dpn List
    ...
    : FOR    ${tools_ip}    IN    @{TOOLS_SYSTEM_LIST}
    \    ${Bridgename1}    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-vsctl show | grep Bridge | awk '{print $2}'
...
Genius Test Teardown
    ...
    : FOR    ${i}    INRANGE    ${NUM_TOOLS_SYSTEM}
    \    OVSDB.Get DumpFlows And Ovsconfig    ${CONN_ID_LIST[${i}]}    ${BRIDGE}
...
Check Tunnel Delete On OVS
    ...
    : FOR    ${tools_ip}    IN    ${TOOLS_SYSTEM_LIST}
    \    ${return} =    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-vsctl show
...
Check Table0 Entry In a Dpn
    ...
    Comment    SSHLibrary.Switch Connection    ${connection_id}
    Comment    ${check}    SSHLibrary.Execute Command    sudo ovs-ofctl -OOpenFlow13 dump-flows ${Bridgename}
    Comment    BuiltIn.Log    ${check}
    ${items}    BuiltIn.Get Length    ${port_numbers}
    : FOR    ${i}    INRANGE    ${items}
...
Verify Tunnel Status
    ...
    ${no. of tunnels}    BuiltIn.Get Length    ${tunnel_names}
    : FOR    ${each_item}    INRANGE    ${no. of tunnels}
...
Ovs Verification For Each Dpn
    ...
    ${no. of tools_ip}    BuiltIn.Get Length    @{updated_tools_ip_list}
    : FOR    ${num}    INRANGE    ${no. of tools_ip}
...
Verify Deleted Tunnels on OVS
    [Arguments]    ${tunnel-list}    ${resp_data}

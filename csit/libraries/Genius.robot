...
Get Tunnel
    ...
    ${ret}    Run Keyword If    '${return}'=='True'    Get Tunnel Interface Name    ${json["internal-tunnel"][0]}    tunnel-interface-names
    [Return]    ${ret}
...
SRM Start Suite
    ...
    Wait Until Keyword Succeeds    60s    10s    Verify Tunnels are UP
...
Verify Tunnels are UP
    ${Result}=    Issue Command On Karaf Console    ${TEP_SHOW}
    ${Line}    Get Lines Containing String    ${Result}    TZA
    ${NoOfNodes}    Get Line Count    ${Line}
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    ${lines}    Get Lines Containing String    ${output}    VXLAN
    ${noofCreatedTunnels}    Get Line Count    ${lines}
    ${noofExpectedTunnels}    Set Variable    ${NoOfNodes*${NoOfNodes-1}}
    Should Be Equal As Strings    ${noofCreatedTunnels}    ${noofExpectedTunnels}

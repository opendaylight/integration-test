...
Check Tunnel Delete On OVS
    ...
    : FOR    ${i}    INRANGE    ${NUM_TOOLS_SYSTEM}
    \    ${return} =    Run Command On Remote System And Log    ${TOOLS_SYSTEM_${i+1}_IP}    sudo ovs-vsctl show
    \    Genius.verify Deleted Tunnels on OVS    ${tunnel-list}    ${return}
    Comment    Switch Connection    ${connection-id}
    Comment    ${return} =    Execute Command    sudo ovs-vsctl show
    Comment    log    ${return}
    Comment    Should Not Contain    ${return}    ${tunnel}
    [Return]    ${return}
...
Check Table0 Entry For 2 Dpn
    ...
    Log    ${connection_id}
    ${check}    Execute Command    sudo ovs-ofctl -OOpenFlow13 dump-flows ${Bridgename}
    Log    ${check}
    ${items}    Get Length    ${port}
    : FOR    ${i}    INRANGE    ${items}
    \    Should Contain    ${check}    in_port=${port[${i}]}
...
Verify Tunnel Status
    ...
    ${items}    Get Length    @{tunnel_names}
    :FOR    ${i}    INRANGE    ${items}
    \    ${tep_output} =    String.Get Lines Containing String    ${tep_result}    ${tunnel_names[${i}]}
...
SRM Start Suite
    ...
    Comment    ${tunnel} =    BuiltIn.Wait Until Keyword Succeeds    40    20    Genius.Get Tunnel    ${Dpn_id_List[0]}
    ...    ${Dpn_id_List[1]}    odl-interface:tunnel-type-vxlan
...
Get Port Number
    : FOR    ${i}    INRANGE    ${NUM_TOOLS_SYSTEM}
    \    ${check}    Run Command On Remote System And Log    ${TOOLS_SYSTEM_${i+1}_IP}    sudo ovs-ofctl -O OpenFlow13 show BR${i}
    \    log    ${check}
    \    ${port}    Get Regexp Matches    ${check}    (\\d+).tun.*    1
    \    log    ${port}
    \    Comment    ${check1}    Run Command On Remote System And Log    ${TOOLS_SYSTEM_${i+1}_IP}    sudo ovs-ofctl -OOpenFlow13 dump-flows BR${i}
    \    Comment    Should Contain    ${check1}    in_port=${port[${i}]}

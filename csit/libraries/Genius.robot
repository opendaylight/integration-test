...
Stop Suite
    ...
    \    SSHLibrary.Execute Command    sudo ovs-vsctl del-br ${INTEGRATION_BRIDGE}
...
>>>>>>> da8e1a1... Enhance Genius-CSIT to 3 Switches
...
Genius Test Teardown
    ...
    : FOR    ${i}    INRANGE    ${NUM_TOOLS_SYSTEM}
    \    OVSDB.Get DumpFlows And Ovsconfig    @{TOOLS_SYSTEM_ALL_CONN_IDS}[${i}]    ${INTEGRATION_BRIDGE}
...
Set Bridge Configuration
    ...
    \    SSHLibrary.Execute Command    sudo ovs-vsctl add-br ${INTEGRATION_BRIDGE}
    \    SSHLibrary.Execute Command    sudo ovs-vsctl set bridge ${INTEGRATION_BRIDGE} protocols=OpenFlow13
    \    SSHLibrary.Execute Command    sudo ovs-vsctl set-controller ${INTEGRATION_BRIDGE} tcp:${ODL_SYSTEM_IP}:6653
    \    SSHLibrary.Execute Command    sudo ifconfig ${INTEGRATION_BRIDGE} up
    \    SSHLibrary.Execute Command    sudo ovs-vsctl add-port ${INTEGRATION_BRIDGE} tap${i}ed70586-6c -- set Interface tap${i}ed70586-6c type=tap
...
Verify Table0 Entry After fetching Port Number
    ...
    : FOR    ${tools_ip}    IN    @{TOOLS_SYSTEM_ALL_IPS}
    \    ${check} =    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-ofctl -O OpenFlow13 show ${INTEGRATION_BRIDGE}
    \    ${port_numbers} =    String.Get Regexp Matches    ${check}    (\\d+).tun.*    1
    \    Genius.Check Table0 Entry In a Dpn    ${tools_ip}    ${INTEGRATION_BRIDGE}    ${port_numbers}

...
Start Suite
    ...
    Comment    BuiltIn.Set Suite Variable    ${BRIDGE_LIST}
    ...
    BuiltIn.Log    ${check}
    @{DPN_ID_LIST} =    Genius.Build Dpn List
...
Create Vteps
    ...
    : FOR    ${tools_ip}    IN    @{TOOLS_SYSTEM_ALL_IPS}
    \    ${body} =    Genius.Set Json    ${vlan}    ${gateway-ip}    ${subnet}    ${TOOLS_SYSTEM_ALL_IPS}
...
Ovs Interface Verification
    [Arguments]    @{TOOLS_SYSTEM_ALL_IPS}
...
Ovs Verification For Each Dpn
    ...
    @{updated_tools_ip_list} =    BuiltIn.Create List    @{TOOLS_SYSTEM_ALL_IPS}

...
Verify Tunnel Status As Up
    ...
    ${no_of_tunnels_temp1} =    CompareStream.Run_Keyword_If_At_Most_Aluminium    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    ${no_of_tunnels_temp2} =    CompareStream.Run_Keyword_If_At_Least_Silicon    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW_OF_PORTS}
    ${lines_of_state_up_temp1} =    CompareStream.Run_Keyword_If_At_Most_Aluminium    String.Get Lines Containing String    ${no_of_tunnels_temp1}    ${STATE_UP}
    ${lines_of_state_up_temp2} =    CompareStream.Run_Keyword_If_At_Least_Silicon    String.Get Lines Containing String    ${no_of_tunnels_temp2}    Up
    ${actual_tunnel_count_temp1} =    CompareStream.Run_Keyword_If_At_Most_Aluminium    String.Get Line Count    ${lines_of_state_up_temp1}
    ${actual_tunnel_count_temp2} =    CompareStream.Run_Keyword_If_At_Least_Silicon    String.Get Line Count    ${lines_of_state_up_temp2}
    ${expected_tunnel_count} =    BuiltIn.Evaluate    ${no_of_switches}*(${no_of_switches}-1)
    CompareStream.Run_Keyword_If_At_Most_Aluminium    BuiltIn.Should Be Equal As Strings    ${actual_tunnel_count_temp1}    ${expected_tunnel_count}
    CompareStream.Run_Keyword_If_At_Least_Silicon    BuiltIn.Should Be Equal As Strings    ${actual_tunnel_count_temp2}    ${no_of_switches}

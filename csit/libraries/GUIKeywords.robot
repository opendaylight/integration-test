...
Library           Process
Library           Common.py
Library           Selenium2Library    timeout=10    implicit_wait=15    run_on_failure=Log Source
...
Open Virtual Display
    ...
    Start Process    Xvfb    :${XVFB PORT}    -ac    -screen    0    1280x1024x16    alias=xvfb
...
Verify Submenu Presence
    [Arguments]    ${submenu}
    Wait Until Page Contains Element    ${submenu}
...
Verify Elements of DLUX Home Page
    ...
    Verify Submenu Presence     ${Topology_SUBMENU}
...
Launch Or Open DLUX
    ${status}=    BuiltIn.Run Keyword And Return Status    Launch DLUX
    BuiltIn.Run Keyword If    "${status}"=="False"    Open DLUX Login Page    ${LOGIN URL}
    Verify Elements Of DLUX Login Page

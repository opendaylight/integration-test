...
Start Suite
    ...
    sleep    10
    Verify Feature Openflow is Installed    ${feature_name_openflow} 
    sleep    5 
    Verify Feature AAA is Installed    ${feature_name_aaa}  
...
Verify Feature Openflow is Installed
    [Arguments]    ${feature_name_openflow}    ${controller}=${ODL_SYSTEM_IP}    ${karaf_port}=${KARAF_SHELL_PORT}
    [Documentation]    Will Succeed if the given ${feature_name_openflow} is found in the output of "feature:list -i"
    ${output}=    Issue Command On Karaf Console    feature:list -i | grep ${feature_name_openflow}    ${controller}    ${karaf_port}
    Log    ${output}
    Should Contain    ${output}    ${feature_name_openflow}
    [Return]    ${output}

Verify Feature AAA is Installed
    [Arguments]    ${feature_name_aaa}    ${controller}=${ODL_SYSTEM_IP}    ${karaf_port}=${KARAF_SHELL_PORT}
    [Documentation]    Will Succeed if the given ${feature_name_aaa} is found in the output of "feature:list -i"
    ${output}=    Issue Command On Karaf Console    feature:list -i | grep ${feature_name_aaa}    ${controller}    ${karaf_port}
    Log    ${output}
    Should Contain    ${output}    ${feature_name_aaa}
    [Return]    ${output}

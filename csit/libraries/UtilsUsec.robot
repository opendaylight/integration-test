...
*** Keywords ***
Start Suite
    [Arguments]    ${system}=${SYSTEM_IP}    ${user}=${TOOLS_SYSTEM_USER}    ${password}=${TOOLS_SYSTEM_PASSWORD}    ${prompt}=${DEFAULT_LINUX_PROMPT}    ${timeout}=30s   
...
Flexible SSH Login
    ...
    BuiltIn.Run Keyword And Return    SSHLibrary.Login With Public Key    ${user}    ${HOME}/.ssh/${SSH_KEY}    ${KEYFILE_PASS}    delay=${delay}

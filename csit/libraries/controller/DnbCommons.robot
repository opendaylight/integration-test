...
Dom_Notification_Broker_Test_Templ
    ...
    BuiltIn.Log
    ...    Overall requested rate: ${total_notification_rate}, test duration: ${test_duration_in_seconds} seconds.
    WaitUtils.WU_Setup
    ...
    FOR    ${index}    IN RANGE    1    ${count}+1
        MdsalLowlevel.Start_Publish_Notifications
        ...    ${DNB_TESTED_MEMBER_INDEX}
        ...    ${DNB_PUBLISHER_LISTENER_PREFIX}${index}
        ...    ${test_duration_in_seconds}
        ...    ${DNB_PUBLISHER_SUBSCRIBER_PAIR_RATE}
    END
    ...
    ...    data_holder
    ${validation_timeout_in_seconds} =    BuiltIn.Evaluate
    ...    ${test_duration_in_seconds}+${DNB_CHECK_TOLERANCE_IN_SECONDS}
    WaitUtils.Wait_For_Getter_Failure_Or_Stateless_Validator_Pass
    ...    timeout=${validation_timeout_in_seconds}s
    ...    period=${DNB_CHECK_PERIOD_IN_SECONDS}s
    ...    getter=${getter}
    ...    stateless_validator=${validator}
    ...
    FOR    ${index}    IN RANGE    1    ${count}+1
        ${all_not}    ${id_not}    ${err_not}    ${local_number} =    MdsalLowlevel.Unsubscribe_Ynl
        ...    ${DNB_TESTED_MEMBER_INDEX}
        ...    ${DNB_PUBLISHER_LISTENER_PREFIX}${index}
...
Get_Notifications_Active_Status
    ...
    FOR    ${index}    IN RANGE    1    ${nr_pairs}+1
        ${active}    ${publ_count}    ${last_error} =    MdsalLowlevel.Check_Publish_Notifications
        ...    ${node_to_ask}
        ...    ${DNB_PUBLISHER_LISTENER_PREFIX}${index}

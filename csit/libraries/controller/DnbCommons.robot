...
Dom_Notification_Broker_Test_Templ
    ...
    #BuiltIn.Wait_Until_Keyword_Succeeds    ${test_duration_in_seconds}    15s    Check_Notifications_Finished    ${DNB_TESTED_MEMBER_INDEX}    ${count}
    ${getter} =    ScalarClosures.Closure_From_Keyword_And_Arguments    Get_Notifications_Active_Status    ${DNB_TESTED_MEMBER_INDEX}    ${count}
    ${validator} =     ScalarClosures.Closure_From_Keyword_And_Arguments    Check_Notifications_Active_Status    state_holder    data_holder
    WaitUtils.Wait_For_Getter_Error_Or_Safe_Stateful_Validator_Consecutive_Success    timeout=${test_duration_in_seconds}    period=15s    getter=${getter}    safe_validator=${validator}
...
Check_Notifications_Active_Status
    [Arguments]    ${ignored_state}    ${active_list}

...
Explicit_Leader_Movement_Test_Templ
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${ODL_SYSTEM_${idx_trans}_IP}    ${ID_PREFIX}    ${TEST_DURATION}   ${TRANSACTION_RATE}    chained_flag=${False}
...
Clean_Leader_Shutdown_Test_Templ
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${ODL_SYSTEM_${producer_idx}_IP}    ${ID_PREFIX}    ${TEST_DURATION}   ${TRANSACTION_RATE}    chained_flag=${False}
...
Leader_Isolation_Test_Templ
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${all_ip_list}   ${ID_PREFIX}   ${producing_transactions_time}    ${TRANSACTION_RATE}    chained_flag=${SIMPLE_TX}
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    ClusterManagement.Get_Leader_And_Followers_For_Shard    ${SHARD_NAME}    ${SHARD_TYPE}
...
Leader_Isolation_Heal_Default
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${restart_producer_node}   ${ID_PREFIX}   ${time_to_finish}    ${TRANSACTION_RATE}    chained_flag=${SIMPLE_TX}
    ${resp_list} =    MdsalLowlevelPy.Wait_For_Write_Transactions
    : FOR    ${resp}   IN    @{resp_list}
    \    TemplatedRequests.Check_Status_Code    ${resp}

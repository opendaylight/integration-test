...
${ID_PREFIX}                                        prefix-
# different-prefix- has been used before, but currently is neither needed nor supported
${ID_PREFIX2}
...                                                 prefix-
${SIMPLE_TX}                                        ${False}
${CHAINED_TX}                                       ${True}
...
Explicit_Leader_Movement_Test_Templ
    ...
    ${idx_from}    ${idx_to}    ${idx_trans} =    Get_Node_Indexes_For_The_ELM_Test
    ...    ${leader_from}
    ...    ${leader_to}
    ...    ${shard_name}
    ...    ${shard_type}
    KarafKeywords.Log_Message_To_Controller_Karaf
    ...    Starting leader movement from node${idx_from} to node${idx_to}, transaction producer at node${idx_trans}.
    ...
    ${new_leader}    ${new_followers} =    BuiltIn.Wait_Until_Keyword_Succeeds
    ...    60s
    ...    5s
...
Get_Node_Indexes_For_The_ELM_Test
    ...
    ${idx_trans} =    BuiltIn.Set_Variable_If
    ...    "${leader_from}" == "remote" and "${leader_to}" == "remote"
    ...    ${follower_list}[1]
    ...    "${leader_from}" == "local"
    ...    ${leader}
    ...    "${leader_to}" == "local"
    ...    ${follower_list}[0]
    RETURN    ${idx_from}    ${idx_to}    ${idx_trans}
...
Clean_Leader_Shutdown_Test_Templ
    ...
    ${producer_idx}    ${actual_leader}    ${follower_list} =    Get_Node_Indexes_For_Clean_Leader_Shutdown_Test
    ...    ${leader_location}
    ...    ${shard_name}
    ...    ${shard_type}
...
Clean_Leader_Shutdown_PrefBasedShard_Test_Templ
    ...
    BuiltIn.Sleep    ${SLEEP_AFTER_TRANSACTIONS_INIT}
    BuiltIn.Comment    Bug 8794 workaround: Use remove-shard-replica until shutdown starts behaving properly.
    ClusterAdmin.Remove_Prefix_Shard_Replica
    ...    ${actual_leader}
    ...    ${shard_name}
    ...    member-${actual_leader}
    ...    ${shard_type}
    BuiltIn.Wait_Until_Keyword_Succeeds
    ...    60s
    ...    5s
...
Get_Node_Indexes_For_Clean_Leader_Shutdown_Test
    ...
    RETURN    ${producer_idx}    ${leader}    ${follower_list}
...
Leader_Isolation_Test_Templ
    ...
    ${producing_transactions_time} =    BuiltIn.Set_Variable_If
    ...    ${heal_timeout}<${REQUEST_TIMEOUT}
    ...    ${${REQUEST_TIMEOUT}+60}
    ...    ${2*${REQUEST_TIMEOUT}}
    ...
    IF    ${heal_timeout} < ${REQUEST_TIMEOUT}
        Leader_Isolation_Heal_Within_Rt
    ELSE
        Module_Leader_Isolation_Heal_Default    ${leader}    ${time_to_finish}
    END
...
Leader_Isolation_PrefBasedShard_Test_Templ
    ...
    IF    ${heal_timeout} < ${REQUEST_TIMEOUT}
        Leader_Isolation_Heal_Within_Rt
    ELSE
        Prefix_Leader_Isolation_Heal_Default    ${leader}    ${time_to_finish}
    END
    [Teardown]    BuiltIn.Run_Keyword_If    ${li_isolated}    BuiltIn.Run_Keywords    ClusterManagement.Rejoin_Member_From_List_Or_All    ${leader}
    ...    AND    BuiltIn.Wait_Until_Keyword_Succeeds    70s    10s    ShardStability.Shards_Stability_Get_Details    ${DEFAULT_SHARD_LIST}
...
Module_Leader_Isolation_Heal_Default
    ...
    ${resp} =    MdsalLowlevelPy.Get_Next_Transactions_Response
    BuiltIn.Log    ${resp}
    BuiltIn.Should_Not_Be_Equal
    ...    ${NONE}
    ...    ${resp}
    ...    Write-transaction should have returned error from isolated leader node.
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes
    ...    ${restart_producer_node_ip_as_list}
    ...    ${restart_producer_node_idx_as_list}
    ...    ${ID_PREFIX2}
    ...    ${time_to_finish}
    ...    ${TRANSACTION_RATE_1K}
    ...    chained_flag=${CHAINED_TX}
...
Prefix_Leader_Isolation_Heal_Default
    ...
    ...    ${NONE}
    ...    ${resp}
    ...    Produce-transaction should have returned error from isolated leader node.
    # TODO: check on response status code
    ${restart_producer_node_idx_as_list} =    BuiltIn.Create_List    ${isolated_node}
    ${restart_producer_node_ip} =    ClusterManagement.Resolve_IP_Address_For_Member    ${isolated_node}
    ${restart_producer_node_ip_as_list} =    BuiltIn.Create_List    ${restart_producer_node_ip}
...
Client_Isolation_Test_Templ
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes
    ...    ${client_node_ip_as_list}
    ...    ${client_node_idx_as_list}
    ...    ${ID_PREFIX}
    ...    ${TRANSACTION_PRODUCTION_TIME_2X_REQ_TIMEOUT}
    ...    ${TRANSACTION_RATE_1K}
    ...    chained_flag=${trans_chain_flag}
    BuiltIn.Sleep    ${SLEEP_AFTER_TRANSACTIONS_INIT}
...
Client_Isolation_PrefBasedShard_Test_Templ
    ...
    ...    ${TRANSACTION_PRODUCTION_TIME_2X_REQ_TIMEOUT}
    ...    ${TRANSACTION_RATE_1K}
    ...    isolated_transactions_flag=${isolated_transactions_flag}
    BuiltIn.Sleep    ${SLEEP_AFTER_TRANSACTIONS_INIT}
    KarafKeywords.Log_Message_To_Controller_Karaf    Isolating node ${client_node_dst}
    ClusterManagement.Isolate_Member_From_List_Or_All    ${client_node_dst}
    ${rpc_timeout} =    Get_Seconds_To_Time    ${abort_date}
...
Listener_Stability_Test_Templ
    ...
    ${all_indices} =    ClusterManagement.List_All_Indices
    ${idx_from}    ${idx_to}    ${idx_listen} =    Get_Node_Indexes_For_The_ELM_Test
    ...    ${leader_from}
    ...    ${leader_to}
    ...    ${shard_name}
    ...    ${shard_type}
...
Verify_Shard_Replica_Not_Present
    ...
    ...    /jolokia/read/org.opendaylight.controller:Category=Shards,name=member-${member_index}-shard-${shard_name}-${shard_type},type=${type_class}
...
Change_Use_Tell_Based_Protocol
    ...
    ClusterManagement.Check_Bash_Command_On_List_Or_All
    ...    sed -ie "s/^#use-tell-based-protocol=false/use-tell-based-protocol=false/g" ${DATASTORE_CFG}
    BuiltIn.Run_Keyword_And_Return_If
    ...    "${status}" == "True"
    ...    ClusterManagement.Check_Bash_Command_On_List_Or_All
    ...    sed -ie "s/^use-tell-based-protocol=false/use-tell-based-protocol=true/g" ${DATASTORE_CFG}
    BuiltIn.Run_Keyword_And_Return_If
    ...    "${status}" == "False"
    ...    ClusterManagement.Check_Bash_Command_On_List_Or_All
    ...    sed -ie "s/^use-tell-based-protocol=true/use-tell-based-protocol=false/g" ${DATASTORE_CFG}

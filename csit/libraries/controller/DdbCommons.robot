...
Clean_Leader_Shutdown_Test_Templ
    ...
    ClusterAdmin.Remove_Shard_Replica    ${actual_leader}    shard_name=${shard_name}    member_name=member-${actual_leader}-shard-${shard_name}-${shard_type}    data-store-type=${shard_type}
    ${removed} =    BuiltIn.Set_Variable    ${True}
    ${resp_list} =    MdsalLowlevelPy.Wait_For_Write_Transactions
    TemplatedRequests.Check_Status_Code    @{resp_list}[0]
    [Teardown]    MdsalLowlevel.Add_Shard_Replica    ${actual_leader}    shard_name=${shard_name}    data-store-type=${shard_type}
...
Client_Isolation_Test_Templ
    ...
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${client_node_ip_as_list}   ${ID_PREFIX}   ${DURATION}    ${TRANSACTION_RATE_1K}    chained_flag=${trans_chain_flag}
...
Listener_Isolation_Test_Templ
    ...
    ${all_ip_list} =    ClusterManagement.Resolve_IP_Address_For_Members    ${all_indices}
    MdsalLowlevelPy.Start_Write_Transactions_On_Nodes    ${all_ip_list}   ${ID_PREFIX}   ${DURATION_10S}    ${TRANS_PER_SEC}    chained_flag=${SIMPLE_TX}

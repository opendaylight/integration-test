...
Clean_Leader_Shutdown_Test_Templ
    ...
    ClusterAdmin.Remove_Shard_Replica    ${actual_leader}    ${shard_name}    member-${actual_leader}-shard-${shard_name}-${shard_type}    ${shard_type}
    ${removed} =    BuiltIn.Set_Variable    ${True}
    ${resp_list} =    MdsalLowlevelPy.Wait_For_Write_Transactions
    TemplatedRequests.Check_Status_Code    @{resp_list}[0]
    [Teardown]    MdsalLowlevel.Add_Shard_Replica    ${actual_leader}    ${shard_name}    ${shard_type}

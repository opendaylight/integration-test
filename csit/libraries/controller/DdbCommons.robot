...
Clean_Leader_Shutdown_Test_Templ
    ...
    ${producer_idx}    ${actual_leader} =    Get_Node_Indexes_For_Leader_Shutdown_Test    ${leader_location}
    MdsalLowlevelPy.Initiate_Write_Transactions_On_Nodes    ${ODL_SYSTEM_${producer_idx}_IP}    ${ID_PREFIX}    ${TEST_DURATION}   ${TRANSACTION_RATE}    chained_flag=${False}
    MdsalLowlevel.Remove_Shard_Replica    ${actual_leader}    shard_name=${shard_name}    shard_type=${shard_type}
    ${removed} =    BuiltIn.Set_Variable    ${True}
    ${resp_list} =    MdsalLowlevelPy.Wait_For_Write_Transactions
    TemplatedRequests.Check_Status_Code    @{resp_list}[0]
    [Teardown]    MdsalLowlevel.Add_Shard_Replica    ${actual_leader}    shard_name=${shard_name}    shard_type=${shard_type}

...
Setup Device Session
    ...
    RequestsLibrary.Create Session    ${DEVICE_SESSION}    url=http://${TOOLS_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}    timeout=${DEFAULT_TIMEOUT_HTTP}    max_retries=0
...
Clean SXP Cluster
    ...
    SxpLib.Delete Node    ${CLUSTER_NODE_ID}    session=ClusterManagement__session_${controller_index}
...
Check Cluster is Connected
    ...
    [Documentation]    Get SXP connections from cluster and verify that they contain a connection to the device
    ${resp} =    SxpLib.Get Connections    node=${node}    session=${session}
    SxpLib.Should Contain Connection    ${resp}    ${DEVICE_NODE_ID}    ${port}    ${mode}    ${version}

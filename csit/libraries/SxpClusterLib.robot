...
Setup SXP Cluster
    ...
    : FOR    ${i}    IN RANGE    ${NUM_ODL_SYSTEM}
    \    SxpLib.Add Node    ${ODL_SYSTEM_${i+1}_IP}    session=ClusterManagement__session_${i+1}
    \    BuiltIn.Wait Until Keyword Succeeds    1m    1x    Check Node started    ${ODL_SYSTEM_${i+1}_IP}    session=ClusterManagement__session_${i+1}
    \    SxpLib.Add Connection    version4    ${cluster_mode}    ${DEVICE_NODE_ID}    64999    ${ODL_SYSTEM_${i+1}_IP}    session=ClusterManagement__session_${i+1}
...
Clean SXP Cluster
    ...
    : FOR    ${i}    IN RANGE    ${NUM_ODL_SYSTEM}
    \    SxpLib.Delete Node    ${ODL_SYSTEM_${i+1}_IP}    session=${CONTROLLER_SESSION}

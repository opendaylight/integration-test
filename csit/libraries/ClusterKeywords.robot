...
Post And Check At URI In Cluster
    [Arguments]    ${controller_index_list}    ${controller_index}    ${uri}    ${body}
    [Documentation]    Send a POST with the supplied ${uri} and ${body} (json string) to a ${controller_index}
    ...    and check the data is replicated in all instances in ${controller_index_list}.
...
Create Data And Check At URI In Cluster
    ...
    Log    ${body}
    ${resp}    Run Keyword If    "${create_method}" == "POST"    RequestsLibrary.Post Request    controller${controller_index}    ${uri}    data=${body}    ${HEADERS}
    ...    ELSE    RequestsLibrary.Put Request    controller${controller_index}    ${uri}    data=${body}    ${HEADERS}
    Log    ${resp.content}

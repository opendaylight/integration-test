...
Namespace Dhclient Verify1
    [Arguments]    ${ns_name}    ${ns_tap}    ${ns_port_ip}
    [Documentation]    Keyword to run dhclient for the tap port ${ns_tap} and verify if it has got assigned with ${ns_port_ip}.
    ${output}=    Exec Command    ${hwvtep_conn_id}    ${NETNS_EXEC} ${ns_name} dhclient ${ns_tap}
    Log    ${output}

Namespace Dhclient Verify2a
    [Arguments]    ${ns_name}    ${ns_tap}    ${ns_port_ip}
    [Documentation]    Keyword to run dhclient for the tap port ${ns_tap} and verify if it has got assigned with ${ns_port_ip}.
    ${output}=    Exec Command    ${hwvtep_conn_id}    ${NETNS_EXEC} ${ns_name} dhclient ${ns_tap} > /tmp/ping_out.txt
    Log    ${output}

Namespace Dhclient Verify2b
    [Arguments]    ${ns_name}    ${ns_tap}    ${ns_port_ip}
    [Documentation]    Keyword to run dhclient for the tap port ${ns_tap} and verify if it has got assigned with ${ns_port_ip}.
    ${output}=    Exec Command    ${hwvtep_conn_id}    cat /tmp/ping_out.txt
    Log    ${output}

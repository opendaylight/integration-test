...
Attach Port To Hwvtep Namespace
    ...
    [Documentation]    Configuring mac and ip address to the port attached to hwvtep node depending on use cases mentioned below:
    ...
...
Exec Command
    ...
    Set Client Configuration    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
...
Verify Entries in Hwvtep DB Dump
    ...
    [Documentation]    Opendaylight controller configures entries in hwvtep database on l2gateway connection creation. This keyword validates these entries.
...
Verify Remote Macs in Hwvtep DB
    ...
    [Documentation]    Keyword checks if the given mac entries are present in ucast_mac_remote table in hwvtep database.
...
Enable Controller Dhcp
    ...
    Wait Until Keyword Succeeds    60s    30s    Check System Status

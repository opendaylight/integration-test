...
Add Ovs Bridge Manager Controller And Verify
    ...
    Wait Until Keyword Succeeds    60s    2s    Verify String In Command Output    ${ovs_conn_id}    Manager "tcp:${ODL_IP}:${OVSDBPORT}"    ${OVS_SHOW}
    Wait Until Keyword Succeeds    60s    2s    Verify String In Command Output    ${ovs_conn_id}    Controller "tcp:${ODL_IP}:${ODL_OF_PORT}"    ${OVS_SHOW}
...
Namespace Dhclient Verify
    ...
    Wait Until Keyword Succeeds    30s    2s    Verify String In Command Output    ${hwvtep_conn_id}    ${ns_port_ip}    ${NETNS_EXEC} ${ns_name} ${IFCONF}

Verify String In Command Output
    [Arguments]    ${conn_id}    ${string}    ${command}
...
Verify Ping In Namespace
    ...
    ${output}=    Exec Command    ${ovs_conn_id}    sudo ovs-ofctl dump-flows br-int -O Openflow13 | grep table=51
    Log    ${output}
...
Get L2gw Debug Info
    ...
    Exec Command    ${ovs_conn_id}    sudo ovs-ofctl dump-flows br-int -O Openflow13

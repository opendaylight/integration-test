...
${KARAF_PATH}     /root/distribution-karaf-0.4.0-SNAPSHOT
...
Get Stats XML
    ...
    ${sid}=    RequestsLibrary.Create_Session    session    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    headers=${HEADER_TSDR}    auth=${AUTH}
...
Verify Log
    [Arguments]    ${pattern}
    [Documentation]    Check if a paritcular pattern is available in log:display
...
Severity Iterator For TSDR
    [Arguments]    ${key}    ${facility_value}    ${iterator_value}    ${syslogs}    ${MESSAGE}
...
Customized tsdr list Query
    [Arguments]    ${query}
    [Documentation]    Issues tsdr:list with grep values and returns output
    ${output}=    Issue Command On Karaf Console    ${query}    timeout=120
    [Return]    ${output}
...
Get OID_Value
    ...
    [Documentation]    Get OID values for MIB from REC file

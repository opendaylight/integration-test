...
# many files
@{YANG_MODEL_PATHS}    /home/jenkins/src/main/yang/experimental/ietf-extracted-YANG-modules
...
Get_Yang_Model_Validator_Path_Option
    [Documentation]    Return the path option for yang-model-validator from ${YANG_MODEL_PATHS}.
    ${separator} =    CompareStream.Set_Variable_If_At_Most_Sulfur    :    ${SPACE}
    ${path_option} =    Evaluate    ${separator}.join(${YANG_MODEL_PATHS})
    ${path_option} =    Catenate    SEPARATOR=${SPACE}    -p    ${path_option}
    [Return]    ${path_option}

...
Find Max Ovsdb Switches
    ...
    \    ...    Check No Ovsdb Nodes Topology    ${switches}
    \    Exit For Loop If    '${status}' == 'FAIL'
    \    ${status}    ${result}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    ${switches*2}    10s
...
Setup Docker Test Suite
    ...
    Run Keyword And Ignore Error    Put File On Remote System   ${MININET}    ${WORKSPACE}/pipework   /usr/local/pipework     prompt=$
    Run Keyword And Ignore Error    Run Command On Mininet    ${CHMOD_PIPEWORK_CMD}     prompt=$
    Run Keyword And Ignore Error    Run Command On Mininet    ${SETUP_DOCKER}       prompt=$

...
Verify_Packet_Count
    ...
    Comment    ${OVS_FLOW}    Run Command On Remote System    ${COMPUTE_IP}    ${DUMP_FLOWS}
    ${OVS_GROUP_STAT}    Run Command On Remote System    ${COMPUTE_IP}    ${DUMP_GROUP_STATS}
    Comment    ${MATCH}    ${TOTAL_PACKET_COUNT}    Should Match Regexp    ${OVS_FLOW}    ${TABLE_21}.*n_packets=(\\d+).*nw_dst=${STATIC_IP}.*${GROUP_ID}
    ...
    Comment    ${TOTAL_PACKET_COUNT}    ConvertToInteger    ${MULTI_PATH_GROUP_PACKET_COUNT}
...
Get_Table21_Packet_Count
    ...
    ${OVS_FLOW}    Run Command On Remote System    ${COMPUTE_IP}    ${DUMP_FLOWS}
    ${MATCH}    ${PACKET_COUNT}    Should Match Regexp    ${OVS_FLOW}    ${TABLE_21}.*n_packets=(\\d+).*nw_dst=${STATIC_IP}.*
    Log    ${PACKET_COUNT}
    [Return]    ${PACKET_COUNT}
...
Configure_Next_Hop_On_Router
    ...
    ...    ${MASK}=32
    [Documentation]    Keyword for updating Next Hop Routes
...
Configure_IP_On_Sub_Interface
    ...
    Wait Until keyword succeeds    100s    20s    Execute Command on VM Instance    ${NETWORK_NAME}    ${VM_IP}    sudo ifconfig ${INTERFACE}:${SUB_INTERFACE_NUMBER} ${IP} netmask ${MASK} up
...
Verify_Ping_To_Sub_Interface
    ...
    Should Not Contain    ${PING_RESP}    ${PING_EXP}
...
Verify_VM_Mac
    ...
    Comment    ${OVS_FLOW}    Run Command On Remote System    ${COMPUTE_IP}    ${DUMP_FLOWS}
    Comment    ${MATCH}    ${GROUP_ID}    Should Match Regexp    ${OVS_FLOW}    table=21.*nw_dst=${STATIC_IP}.*group:(\\d+)
    ${OVS_GROUP}    Run Command On Remote System    ${COMPUTE_IP}    ${DUMP_GROUPS}

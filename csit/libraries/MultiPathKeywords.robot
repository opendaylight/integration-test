...
Tep_Port_Operations
    [Arguments]    ${OPERATION}    ${COMPUTE_IP}
    [Documentation]    Keyword to add/delete tep port
    ${FIRST_TWO_OCTETS}    ${THIRD_OCTET}    ${LAST_OCTET}=    Split String From Right    ${COMPUTE_IP}    .    2
    ${SUBNET_1}=    Set Variable    ${FIRST_TWO_OCTETS}.0.0/16
    ${COMPUTE_NODE_1_ID}    Get DPID    ${COMPUTE_IP}
    ${NODE_ADAPTER}=    Get Ethernet Adapter    ${COMPUTE_IP}
    Issue Command On Karaf Console    tep:${OPERATION} ${COMPUTE_NODE_1_ID} ${NODE_ADAPTER} 0 ${COMPUTE_IP} ${SUBNET_1} null TZA

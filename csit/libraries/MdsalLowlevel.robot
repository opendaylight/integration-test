...
Rpc_Become_Id_Ints_Leader
    [Arguments]    ${session}
    [Documentation]    Invoke become-id-ints-leader rpc.
    ${resp} =    RequestsLibrary.Post_Request    ${session}    ${URL_PREFIX}:become-id-ints-leader
    BuiltIn.Should_Be_Equal_As_Integers    ${resp.status_code}    200

Rpc_Become_Default_Leader
    [Arguments]    ${session}
    [Documentation]    Invoke become-default-leader rpc.
    ${resp} =    RequestsLibrary.Post_Request    ${session}    ${URL_PREFIX}:become-default-leader
    BuiltIn.Should_Be_Equal_As_Integers    ${resp.status_code}    200

...
*** Keywords ***
Get_Constant
    [Arguments]    ${member_index}    ${use_explicit_status_codes}=${NO_STATUS_CODES}
    [Documentation]    Invoke get-constant rpc on the requested member and return the registered constant.
    ${session} =    Resolve_Http_Session_For_Member    member_index=${member_index}
    ${text} =    TemplatedRequests.Post_As_Xml_Templated    ${GET_CONSTANT_DIR}    session=${session}    use_explicit_status_codes=${use_explicit_status_codes}

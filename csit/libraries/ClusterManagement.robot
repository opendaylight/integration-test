...
Get_Owner_And_Candidates_For_Type_And_Id
    [Arguments]    ${type}    ${id}    ${member_index}    ${require_candidate_list_size}=${EMPTY}
    ...
    ...    Candidates are all members that register to own a device, so the list of candiates includes the owner.
    ...    If ${require_candidate_list_size} is not empty, check whether the actual number of candidates matches.
    ...
    \    ${candidate} =    BuiltIn.Convert_To_Integer    ${candidate}
    \    Collections.Append_To_List    ${candidate_list}    ${candidate}
    ${candidate_list_size} =    BuiltIn.Get_Size    ${candidate_list}
    BuiltIn.Run_Keyword_If    "${require_candidate_list_size}"    BuiltIn.Should_Be_Equal    ${require_candidate_list_size}    ${candidate_list_size}    Cnadidate list size does not match: ${require_candidate_list_size} is not ${require_candidate_list_size}
    [Return]    ${owner}    ${candidate_list}

...
Get_Owner_And_Successors_For_Device
    ...
    Collections.Sort_List    ${successor_list}
...
Get_Owner_And_Candidates_For_Type_And_Id
    [Arguments]    ${type}    ${id}    ${member_index}    ${require_candidate_list_length}=${EMPTY}
    ...
    ...    Candidates are all members that register to own a device, so the list of candiates includes the owner.
    ...    If ${require_candidate_list_length} is not empty, check whether the actual number of candidates matches.
    ...
    \    ${candidate} =    BuiltIn.Convert_To_Integer    ${candidate}
    \    Collections.Append_To_List    ${candidate_list}    ${candidate}
    ${candidate_list_length} =    BuiltIn.Get_Length    ${candidate_list}
    BuiltIn.Run_Keyword_If    "${require_candidate_list_length}"    BuiltIn.Should_Be_Equal    ${require_candidate_list_length}    ${candidate_list_length}    Cnadidate list size does not match: ${require_candidate_list_length} is not ${require_candidate_list_length}
    [Return]    ${owner}    ${candidate_list}
...
Install_Feature_On_List_Or_All
    ...
    : FOR    ${index}    IN    @{index_list}
    \    ${status}    ${text} =    BuiltIn.Run_Keyword_And_Ignore_Error    Install_Feature_On_Member    feature_name=${feature_name}    member_index=${index}    timeout=${timeout}
    \    BuiltIn.Log    ${text}
...
With_Ssh_To_List_Or_All_Run_Keyword
    ...
    ...    Beware that in order to avoid "got positional argument after named arguments", first two arguments should not be named in call.

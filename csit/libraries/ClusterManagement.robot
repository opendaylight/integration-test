...
Dump_Local_Shards_For_Each_Member
    ...
    \    ${session} =    Resolve_Http_Session_For_Member    member_index=${member_index}
    \    ${conf_shard_list} =    Wait Until Keyword Succeeds    60    2    TemplatedRequests.Get_As_Json_From_Uri    uri=${JOLOKIA_CONFIG_LOCAL_SHARDS_URI}    session=${session}
    \    Log    ${conf_shard_list}
    \    ${oper_shard_list} =    Wait Until Keyword Succeeds    60    2    TemplatedRequests.Get_As_Json_From_Uri    uri=${JOLOKIA_OPER_LOCAL_SHARDS_URI}    session=${session}
    \    Log    ${oper_shard_list}

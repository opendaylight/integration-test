...
Run_Karaf_Command_On_Member
    ...
    ${member_ip} =    Collections.Get_From_Dictionary    dictionary=${ClusterManagement__index_to_ip_mapping}    key=${member_index}
    ${output} =    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    ${command}    controller=${member_ip}
    [Return]    ${output}
...
Install_Feature_On_Member
    ...
    ${status}    ${output} =    Run_Karaf_Command_On_Member    command=feature:install ${feature_name}    member_index=${member_index}    timeout=${timeout}
...
Clean_Directories_On_List_Or_All
    ...
    KarafKeywords.Run_Keyword_Preserve_Connection    With_Ssh_To_List_Or_All_Run_Keyword    member_index_list=${member_index_list}    ClusterManagement__Clean_Directories    ${path_list}    ${karaf_home}

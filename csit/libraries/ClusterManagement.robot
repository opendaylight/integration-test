...
Get_Leaders_And_Followers_For_Shard
    [Arguments]    ${shard_type}=operational    ${shard_name}=default    ${validate}=True    ${member_index_list}=${EMPTY}
    ...
    ...    If \${validate}, Fail on unknown raft state or incorrect number of leaders.
    ...
    Return_From_Keyword_If    not ${validate}    ${leader_list}    ${follower_list}

...
Start_Members_From_List_Or_All
    [Arguments]    ${member_index_list}=${EMPTY}    ${wait_for_sync}=True    ${timeout}=300s    ${karaf_home}=${EMPTY}    ${export_java_home}=${EMPTY}    ${gc_log_dir}=${EMPTY}    ${verify_restconf}=True
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    ${timeout}    10s    Verify_Members_Are_Ready    ${member_index_list}    ${wait_for_sync}    ${verify_restconf}    ${check_system_status}    @{service_list}
...
Verify_Members_Are_Ready
    ...
    BuiltIn.Run_Keyword_If    ${wait_for_sync}    Check_Cluster_Is_In_Sync    ${member_index_list}
    BuiltIn.Run_Keyword_If    ${verify_restconf}    Verify_Restconf_Is_Available    ${member_index_list}
    BuiltIn.Run_Keyword_If    ${check_system_status}    CompareStream.Run_Keyword_If_At_Least_Oxygen    ClusterManagement.Check Status Of Services Is OPERATIONAL    @{service_list}

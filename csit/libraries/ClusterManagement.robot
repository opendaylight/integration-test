...
Get_As_Json_From_Member_At_Uri
    [Arguments]    ${member_index}    ${uri}    ${headers}=${HEADERS_YANG_JSON}
    [Documentation]    Translate member index to session alias, return Get_As_Json_From_Uri response.
    ...
    ${response_text} =    TemplatedRequests.Get_As_Json_From_Uri    uri=${uri}    session=${session}
...
Get_Raft_State_Of_Shard_At_Member
    ...
    ${data_text} =    Get_As_Json_From_Member_At_Uri    member_index=${index}    uri=${uri}
...
Get_Sync_Status_Of_Member
    ...
    ${conf_text} =    Get_As_Json_From_Member_At_Uri    member_index=${member_index}    uri=${JOLOKIA_CONF_SHARD_MANAGER_URI}
    ...
    ${oper_text} =    Get_As_Json_From_Member_At_Uri    member_index=${member_index}    uri=${JOLOKIA_OPER_SHARD_MANAGER_URI}
...
Clean_Journals_And_Snapshots_On_List_Or_All
    ...
    \    #    Log    ${body}
    \    #    ${resp}    RequestsLibrary.Put Request    controller${controller_index}    ${uri}    ${body}
    \    ...    # ${HEADERS_YANG_JSON}
    \    #    Log    ${resp.content}
    ...
    \    #    : FOR    ${i}    IN    @{controller_index_list}
    \    #    ${data}    Wait Until Keyword Succeeds    5s    1s    Get Data From URI
    \    ...    # controller${i}
    \    #    ...    ${uri}
    \    #    Log    ${data}
    ...
    \    #    : FOR    ${i}    IN    @{controller_index_list}
    \    #    Wait Until Keyword Succeeds    5s    1s    No Content From URI    controller${i}
    \    ...    # ${uri}
    \    #
    \    #Get Controller List
    ...
    \    #    ${leader}    GetLeader    ${shard_name}    ${3}    ${3}
    \    ...    # ${1}    # ${RESTCONFPORT}
    \    #    ...    @{searchlist}
    ...
    \    #    ${followers}    GetFollowers    ${shard_name}    ${3}    ${3}
    \    ...    # ${1}    # ${RESTCONFPORT}
    #    ...    @{searchlist}
    #    Log    ${followers}

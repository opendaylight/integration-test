...
Run_Karaf_Command_On_List_Or_All
    [Arguments]    ${command}    ${timeout}=10s    ${member_index_list}=${EMPTY}
    ...
    \    KarafKeywords.Issue_Command_On_Karaf_Console    ${command}    ${member_ip}    timeout=${timeout}
...
Install_Feature_On_List_Or_All
    [Arguments]    ${feature_name}    ${timeout}=60s    ${member_index_list}=${EMPTY}
    [Documentation]    Attempt installation on each member from list (or all). Then look for failures.
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List    given_list=${member_index_list}
    ${status_list} =    BuiltIn.Create_List
    : FOR    ${index}    IN    @{index_list}
    \    ${status}    ${text} =    BuiltIn.Run_Keyword_And_Ignore_Error    Install_Feature_On_Member    feature_name=${feature_name}    timeout=${timeout}    member_index=${index}
    \    BuiltIn.Log    ${text}
...
Install_Feature_On_Member
    [Arguments]    ${feature_name}    ${timeout}=60s    ${member_index}
    [Documentation]    Run feature:install karaf command, fail if installation was not successful. Return output.
    ${status}    ${output} =    Run_Karaf_Command_On_Member    command=feature:install ${feature_name}    timeout=${timeout}    member_index=${member_index}
...
With_Ssh_To_List_Or_All_Run_Keyword
    [Arguments]    ${member_index_list}    @{keyword_cells}
    ...
    \    BuiltIn.Run_Keyword    @{keyword_cells}
...
Clean_Directories_On_List_Or_All
    ...
    With_Ssh_To_List_Or_All_Run_Keyword    member_index_list=${member_index_list}    ClusterManagement__Clean_Directories    ${path_list}    ${karaf_home}

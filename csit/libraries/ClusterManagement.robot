*** Settings ***
Library           ${CURDIR}/RequestsLibrary
Library           ${CURDIR}/Collections
...
ClusterManagement_Setup
    ...
    ${status}    ${possibly_int_of_members} =    BuiltIn.Convert_To_Integer    ${NUM_ODL_SYSTEM}
...
Get_Data_From_Uri_At_Index
    ...
    ${response} =    Utils.Get_Data_From_Uri    session=${session}    uri=${uri}
    [Return]    ${response}
...
Get_Raft_State_Of_Shard_At_Member
    ...
    ${data_text} =    Get_Data_From_Uri_At_Index    uri=${uri}    index=${index}
...
Kill_Members_From_List_Or_All
    ...
    \    Utils.Run_Command_On_Member    command=${command}    member_index=${index}
...
Start_Members_From_List_Or_All
    ...
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List    given_list=${member_index_list}
    ${command} =    BuiltIn.Set_Variable    ${WORKSPACE}/${BUNDLEFOLDER}/bin/start
    : FOR    ${index}    IN    @{index_list}
    \    Run_Command_On_Member    command=${command}    member_index=${index}
    BuiltIn.Return_From_Keyword_If    not ${wait_for_sync}
    BuiltIn.Fail    Sync waiting is not supported yet.

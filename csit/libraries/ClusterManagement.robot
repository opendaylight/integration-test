...
Isolate_Member_From_List_Or_All
    ...
    FOR    ${index}    IN    @{index_list}
        ...
        ${command} =    BuiltIn.Set_Variable    sudo /sbin/iptables -I OUTPUT -p ${protocol} --source ${source} ${sport} --destination ${destination} ${dport} -j DROP
...
Rejoin_Member_From_List_Or_All
    [Arguments]    ${rejoin_member_index}    ${member_index_list}=${EMPTY}    ${protocol}=all    ${sport}=${EMPTY}    ${dport}=${EMPTY}    ${timeout}=60s
    ...
    ${sport} =    BuiltIn.Set_Variable_If    '${sport}' != '${EMPTY}'    --dport ${sport}    ${EMPTY}
    ${dport} =    BuiltIn.Set_Variable_If    '${dport}' != '${EMPTY}'    --dport ${dport}    ${EMPTY}
    FOR    ${index}    IN    @{index_list}
        ${destination} =    Collections.Get_From_Dictionary    ${ClusterManagement__index_to_ip_mapping}    ${index}
        ${command} =    BuiltIn.Set_Variable    sudo /sbin/iptables -D OUTPUT -p ${protocol} --source ${source} ${sport} --destination ${destination} ${dport} -j DROP

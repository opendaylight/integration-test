...
Post_As_Json_And_Check_Member_List_Or_All
    [Arguments]    ${uri}    ${data}    ${member_index}    ${member_index_list}=${EMPTY}
    [Documentation]    Send a POST with the supplied uri ${uri} and body ${data} to member ${member_index}.
    ...    Then check data is replicated data in all or some members defined in ${member_index_list}.
    ${response_text} =    Post_As_Json_To_Member    uri=${uri}    data=${data}    member_index=${member_index}
    Wait Until Keyword Succeeds    5s    1s    Check_Json_Member_List_Or_All    uri=${uri}    expected_data=${data}    member_index_list=${member_index_list}
    [Return]    ${response_text}
...
Delete_From_Member
    ...
    [Documentation]    Send a DELETE with the supplied uri ${uri} to member ${member_index}.
    ${session} =    Resolve_Http_Session_For_Member    member_index=${member_index}
...
Get_From_Member
    [Arguments]    ${uri}    ${member_index}
    [Documentation]    Send a GET with the supplied uri ${uri} to member ${member_index}.
    ${session} =    Resolve_Http_Session_For_Member    member_index=${member_index}
    ${response_text} =    TemplatedRequests.Get_From_Uri    uri=${uri}    accept=${ACCEPT_EMPTY}    session=${session}
    [Return]    ${response_text}
...
Verify_Owner_And_Candidates_For_Device
    [Arguments]    ${device_name}    ${device_type}    ${member_index}    ${member_index_list}=${EMPTY}
    [Documentation]    Returns the owner and candidates for the device ${device_name} of type ${device_type}. Request is sent to member ${member_index}.
    ...    Extra check is done to verify candidates are resolved. This KW is useful when combined with WUKS.
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List    given_list=${member_index_list}
    ...
    ${expected_candidate_list} =    BuiltIn.Create_List    @{index_list}
    Collections.Remove_Values_From_List    ${expected_candidate_list}    ${owner}
    Collections.Lists_Should_Be_Equal    ${expected_candidate_list}    ${candidate_list}    Candidates list ${candidate_list} is not ${index_list}
...
Get_Owner_And_Candidates_For_Device
    ...
    ${clear_data} =    BuiltIn.Run_Keyword_If    '${device_style}' == 'openflow'    Extract_OpenFlow_Device_Data    ${data}
    ...    ELSE IF    '${device_style}' == 'ovsdb'    Extract_Ovsdb_Device_Data    ${data_object}
    ...
    Collections.Remove_Values_From_List    ${candidate_list}    ${owner}
    [Return]    ${owner}    ${candidate_list}

...
*** Keywords ***
Extract SSH Host RSA public key from device
    [Documentation]    Executes the docker command and parses the ssh host rsa key of the device(Netopeer).
    ${output}    SSHLibrary.Execute_Command    docker run odlcallhome/netopeer cat /etc/ssh/ssh_host_rsa_key.pub
    @{words}    Split String    @{output}[0]
    Set Suite Variable    ${ssh_host_rsa_public_key}    @{words}[1]
    Return From Keyword    ${ssh_host_rsa_public_key}

Check Device status
    [Arguments]    ${status}    ${id}=netopeer
    [Documentation]    Checks the operational device status.
    ${key}    Extract SSH Host RSA public key from device
    ${expectedValues}    Create Dictionary    "unique-id":"${id}"=1    "ssh-host-key":"${key}"=1    "device-status":"${status}"=1
    Run Keyword If    '${status}'=='FAILED_NOT_ALLOWED' or '${status}'=='FAILED_AUTH_FAILURE'    Remove From Dictionary    ${expectedValues}    "unique-id":"${id}"

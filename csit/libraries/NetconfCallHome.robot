...
Generate certificates for TLS configuration
    ...
    ${stdout}    SSHLibrary.Execute Command    openssl req -x509 -new -nodes -key ./certs/ca.key -sha256 -days 365 -subj "/C=US/ST=CA/L=Netopeer/O=netopeerCA/CN=netopeerCA" -out ./certs/ca.pem
    ...
    ${stdout}    SSHLibrary.Execute Command    openssl x509 -req -in ./certs/server.csr -CA ./certs/ca.pem -CAkey ./certs/ca.key -CAcreateserial -out ./certs/server.crt -days 365 -sha256
    ...
    ${stdout}    SSHLibrary.Execute Command    openssl x509 -req -in ./certs/client.csr -CA ./certs/ca.pem -CAkey ./certs/ca.key -CAcreateserial -out ./certs/client.crt -days 1024 -sha256
...
Register keys and certificates in ODL controller
    ...
    ${base64-client-key}    SSHLibrary.Execute_Command    openssl enc -base64 -A -in configuration-files/certs/client.key
    ...
    ${client-key}    SSHLibrary.Execute_Command    sed -u '1d; $d' configuration-files/certs/client.key | sed -z 's/\n/\\n/g'
    ${certificate-chain}    SSHLibrary.Execute_Command    sed -u '1d; $d' configuration-files/certs/client.crt | sed -z 's/\n/\\n/g'
    ...
    ${ca-certificate}    SSHLibrary.Execute_Command    sed -u '1d; $d' configuration-files/certs/ca.pem | sed -z 's/\n/\\n/g'
    ${device-certificate}    SSHLibrary.Execute_Command    sed -u '1d; $d' configuration-files/certs/server.crt | sed -z 's/\n/\\n/g'

...
Get Neutron Networks
    ...
    [Setup]    NeutronNB.NeutronNB Setup
    ${test_out}    Utils.Write Commands Until Expected Prompt    cmd=command -v neutron    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${test_out}
    ${test_out}    Utils.Write Commands Until Expected Prompt    cmd=help    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${test_out}
    ${tools_system_shell}    SSHLibrary.Open Connection    ${TOOLS_SYSTEM_IP}    prompt=${TOOLS_SYSTEM_PROMPT}
    Switch Connection    ${tools_system_shell}
    ${test_out}    Utils.Write Commands Until Expected Prompt    cmd=command -v neutron    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${test_out}
    ${test_out}    Utils.Write Commands Until Expected Prompt    cmd=help    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${test_out}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=neutron net-list    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${output}
    [Return]    ${output}

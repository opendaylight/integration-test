...
Get Neutron Networks
    ...
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=pip freeze
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=sh pip freeze
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=sh command -v neutron    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${output}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=sh pip freeze    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${output}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=sh whoami    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${output}

...
Get Neutron Networks
    ...
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=sh ls -R ${WORKSPACE}    prompt=${DEFAULT_LINUX_PROMPT}
    Log    ${output}
    ...
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=ls -R
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=ls -R ${WORKSPACE}
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=find . -name activate
    SSHKeywords.Virtual_Env_Run_Cmd_At_Cwd    cmd=find ${WORKSPACE} -name activate
    [Return]    ${output}

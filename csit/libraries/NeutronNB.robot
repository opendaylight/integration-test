...
Get Neutron Networks
    ...
    ${con}    SSHLibrary.Get Connection
    ${cons}    SSHLibrary.Get Connections
    ${output}    Utils.Run Command On Remote System    system=${neutron_client_ip}    cmd=ls -R
    ${tools_sys_con_id}    SSHLibrary.Open Connection    ${neutron_client_ip}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    Switch Connection    ${tools_sys_con_id}
    ${cons}    SSHLibrary.Get Connections
    ${ls}    SSHLibrary.List Directory    path=${WORKSPACE}
    ${ls}    SSHLibrary.List Directory    path=.
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=ls -R    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=ls -R ${WORKSPACE}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=find . -name activate    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${output}    Utils.Write Commands Until Expected Prompt    cmd=find ${WORKSPACE} -name activate    prompt=${DEFAULT_LINUX_PROMPT_STRICT}

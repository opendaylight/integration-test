...
Add_Several_People
    ...
    FOR    ${i}    IN RANGE    ${iter_start}    ${iter_start}+${iterations}
        ${person_id} =    localhost/people/person_id_${i}
        ${gender} =    gender_${i}
        ${age} =    ${i}
        ${address} =    address_${i}
        ${constact} =    contact_${i}
        KarafKeywords.Execute_Controller_Karaf_Command_On_Background    test-app:add-person ${person_id} ${gender} ${age} ${address} ${constact}    member_index=${member_index}
    END
...
Buy_Single_Car
    ...
    ${person_ref} =    /people:people/people:person[people:id='localhost/people/person_id_${iteration}']
    ${car_id} =    localhost/cars/car_id_${iteration}
    ${person_id} =    localhost/people/person_id_${iteration}
    KarafKeywords.Execute_Controller_Karaf_Command_On_Background    test-app:buy-car ${person_ref} ${car_id} ${person_id}    member_index=${member_index}

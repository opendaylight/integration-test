...
def _wt_request_sender(
    ...
    while loop:
        ...
        try:
            ...
            logging.warning(f"Timeout: {req.url}")
            counter[99] += 1
            if counter[99] > 10:
                logging.error("Too many timeouts")
                break
            continue
        else:
            if rsp.status_code not in [200, 201, 204]:
                logging.warning(f"URL: {rsp.url}")
                logging.warning(f"Status code: {rsp.status_code}")
                logging.warning(f"Response text: {rsp.text}")

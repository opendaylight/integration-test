...
...               The wrapped keywords can contain both named and positional arguments,
...               but \${timeout} and \${refresh} are consumed by the outmost BuiltIn.Wait_Until_Keyword_Succeeds
...               which means the inner loops are not possible (without resorting to ScalarClosures).

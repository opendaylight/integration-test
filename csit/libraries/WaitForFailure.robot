...
Invert_Failure
    [Arguments]    @{cell_sequence}
    [Documentation]    Result of Keyword execution is either return value or failure message.
    ...    This keyword calls the argument keyword and fails with its return value converted to string message,
    ...    or returns its failure message string.
    ${status}    ${output} =    BuiltIn.Run_Keyword_And_Ignore_Error    @{cell_sequence}
    BuiltIn.Run_Keyword_And_Return_If    "${status}" == "PASS"    BuiltIn.Fail    ${output}
    ...    ELSE    BuiltIn.Return_From_Keyword    ${output}

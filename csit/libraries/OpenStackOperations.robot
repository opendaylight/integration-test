...
Create Glance Image
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    ${output}=    Write Commands Until Prompt    glance --os-image-api-version 1 image-create --name ${image_name} --min-disk ${disk_size} --min-ram ${ram} --location ${image_url} --file \ centos-6.5-20140117.0.x86_64.qcow2 --is-public True --disk-format=qcow2 --progress    30s
    Log    ${output}

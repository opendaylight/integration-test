...
Get DumpFlows And Ovsconfig
    ...
    Write Commands Until Expected Prompt    sudo ovs-dpctl show    ]>
...
Get OvsDebugInfo
    ...
    Run Keyword If    0 < ${NUM_ODL_SYSTEM}   Collect OdlVmTop     ${ODL_SYSTEM_1_IP}
    Run Keyword If    1 < ${NUM_ODL_SYSTEM}   Collect OdlVmTop     ${ODL_SYSTEM_2_IP}
    Run Keyword If    2 < ${NUM_ODL_SYSTEM}   Collect OdlVmTop     ${ODL_SYSTEM_3_IP}
...
Collect OdlVmTop
    [Arguments]    ${odl_ip_address}
    ${vtn_conn_id}=    SSHLibrary.Open Connection    ${odl_ip_address}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=30s
    Set Suite Variable    ${vtn_conn_id}
    SSHLibrary.Login_With_Public_Key    ${ODL_SYSTEM_USER}    ${USER_HOME}/.ssh/${SSH_KEY}    any
    Write     top -n 1 -b 
    ${result}    Read Until    ${DEFAULT_LINUX_PROMPT}
    Log      ${result}
    Write     exit
    Close Connection

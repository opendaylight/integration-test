...
Ssh Vm Instance
    [Arguments]    ${net_id}    ${src_ip}    ${dst_ip_list}    ${l2_or_l3}=l2    ${other_dst_ip_list}=${NONE}    ${user}=cirros    ${password}=cubswin:)    ${key_file}=test.pem
    [Documentation]    Login to the vm instance using ssh in the network.
    ${output}=   Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ssh -i ${key_file} ${user}@${src_ip}    (yes/no)?
    Log    ${output}

...
Get Vm Instance Ip
    ...
    : FOR    ${index}    IN    @{vm_indices}
    \    ${source}=    Write Commands Until Prompt    nova console-log ${index} | grep "inet addr"    40s
    \    Log    ${source}
    \    ${inet_list}=    Split String    ${source}    ${SPACE}
    \    ${inet_addr}=    Get from List    ${inet_list}    11
    \    ${inet_addr_list}=    Split String    ${inet_addr}    :
    \    ${vm_ip}=    Get from List    ${inet_addr_list}    1
    \    Log    ${vm_ip}

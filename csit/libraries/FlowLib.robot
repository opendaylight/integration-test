...
Check Switches In Topology
    ...
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${count}=    Get Count    ${resp.text}    "node-id":"openflow%3A
...
Check Number Of Links
    ...
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${count}=    Get Count    ${resp.text}    "link-id":"openflow%3A
...
Remove Default Flows
    ...
    ${resp}    RequestsLibrary.Post Request    session    restconf/operations/${RFC8040_SAL_FLOW_API}:remove-flow    data=${flow.xml}    headers=${headers}
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=    RequestsLibrary.Get Request    session    ${RFC8040_OPERATIONAL_NODES_API}
    Log    ${resp.text}
...
Add Flow Via RPC
    ...
    Add Element    ${req}    <node>/inv:nodes/inv:node[inv:id="openflow%3A${node_id}"]</node>
    ...
    ${resp}=    RequestsLibrary.Post Request    session    /restconf/operations/${RFC8040_SAL_FLOW_API}:add-flow    data=${strxml}
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Update Flow Via RPC
    ...
    Add Element    ${xml}    <node>/inv:nodes/inv:node[inv:id="openflow%3A${node_id}"]</node>
    ${nodeelm}=    Get Element    ${xml}    node
    Set Element Attribute    ${nodeelm}    xmlns:inv    urn:opendaylight:inventory
    Log Element    ${xml}
    ${strxml}=    Element To String    ${xml}
    ${resp}=    RequestsLibrary.Post Request    session    /restconf/operations/${RFC8040_SAL_FLOW_API}:update-flow    data=${strxml}
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete Flow Via RPC
    ...
    Log Element    ${req}
    ${strxml}=    Element To String    ${req}
    ${resp}=    RequestsLibrary.Post Request    session    /restconf/operations/${RFC8040_SAL_FLOW_API}:remove-flow    data=${strxml}
    Log    ${resp.text}
    Should Be Equal As Strings    ${resp.status_code}    200

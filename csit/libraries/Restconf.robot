...
Generate URI
    ...
    ...    ELSE IF    "${datastore_flag}"=="config"    Set Variable    ${OPERATIONAL_API}/${identifier}
    ...    ELSE    Set Variable    ${OPERATIONS_API}
...
Generate RFC8040 URI
    [Arguments]    ${identifier}    ${datastore_flag}=config    ${rpc}=False    @{node_value_list}
    ${node_value_path} =    Set Variable    ${EMPTY}
    FOR    ${nv}    IN    @{node_value_list}
        ${node_value_path} =    Set Variable    ${node_value_path}/${nv}
    END
    ${uri} =    Run Keyword If    "${datastore_flag}" == "config"    Set Variable    rests/data/${identifier}${node_value_path}?content=config
    ...    ELSE IF    "${datastore_flag}"=="config"    Set Variable    rests/data/${identifier}${node_value_path}?content=nonconfig

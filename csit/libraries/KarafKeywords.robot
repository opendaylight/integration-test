...
Fail If Exceptions Found During Test
    [Arguments]    ${test_name}    ${log_file}=${KARAF_LOG}    ${fail}=False    ${wlfile}=/tmp/whitelist.exceptions.json
    ...
    \    ${exlist} =    ODLTools.Karaf Exceptions    ${log_file}    /tmp/odl${i}_exceptions.txt    ${wlfile}    testname=${test_name}

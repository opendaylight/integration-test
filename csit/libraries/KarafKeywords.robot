...
Restore Current SSH Connection
    [Arguments]    ${index}
    # There is a bug in SSHLibrary: Invoking "Switch Connection" and passing
    # None as the "index_or_alias" argument to it has exactly the same effect
    # as invoking "Close Connection". The problem is that when you try to
    # determine whether there is an active connection (so you can return to
    # it later) and there was no active connection, you will get None as a
    # result. Thus to properly restore the active SSH connection state we
    # need to check for None and open and then close a dummy connection
    # instead of invoking "Switch Connection". Unfortunately this will fail
    # if run on Jython and there is no SSH server running on localhost, port
    # 22 but there is nothing easy that can be done about it.

...
Start Karaf
    ...
    # TODO: prepare this for cluster environment and multiple controllers
    Run Command On Remote System    ${ODL_SYSTEM_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${USER_HOME}/${BUNDLEFOLDER}/bin/start
    Run Keyword And Return Status    Wait Until Keyword Succeeds    240s    60s    Wait For Karaf Log    Karaf started in
...
Stop Karaf
    ...
    Run Command On Remote System    ${ODL_SYSTEM_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${USER_HOME}/${BUNDLEFOLDER}/bin/stop
...
Karaf Status Not Running
    ...
    ${status} =    Run Command On Remote System    ${ODL_SYSTEM_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${USER_HOME}/${BUNDLEFOLDER}/bin/status

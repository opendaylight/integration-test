...
Get and Verify Exceptions
    [Arguments]    ${lines}
    @{exlist} =    verify exceptions    ${lines}
    Collections.Log List    ${exlist}
    [Return]    ${exlist}

Get Karaf Log Lines From Test Start
    [Arguments]    ${ip}    ${test_name}    ${user}=${ODL_SYSTEM_USER}    ${password}=${ODL_SYSTEM_PASSWORD}    ${prompt}=${ODL_SYSTEM_PROMPT}    ${log_file}=${KARAF_LOG}    ${cmd}="echo hi"
    ...
    Collections.Log List    ${log_lines}
    [Return]    ${log_lines}
...
Fail If Exceptions Found During Test
    ...
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file} | grep "| WARN \ |\\|| ERROR |"
    \    ${output15} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file} | grep "| WARN \ |\|| ERROR |"
    \    ${output14} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${warn_error_grep} =    BuiltIn.Set Variable    "| WARN \ |\\|| ERROR |"
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file} | grep ${warn_error_grep}
    \    ${output13} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${warn_error_grep} =    BuiltIn.Set Variable    "| WARN \ |\|| ERROR |"
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file} | grep ${warn_error_grep}
    \    ${output12} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file} | grep "WARN\|ERROR"
    \    ${output11} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${cmd} =    Set Variable    sed '1,/ROBOT MESSAGE: Starting test ${test_name}/d' ${log_file}
    \    ${output10} =    Get Karaf Log Lines From Test Start    ${ODL_SYSTEM_${i}_IP}    ${test_name}    cmd=${cmd}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output10} != ['']    BuiltIn.Set Variable    ${output10}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output11} != ['']    BuiltIn.Set Variable    ${output11}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output12} != ['']    BuiltIn.Set Variable    ${output12}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output13} != ['']    BuiltIn.Set Variable    ${output13}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output14} != ['']    BuiltIn.Set Variable    ${output14}
    \    ${output0} =    BuiltIn.Run Keyword If    ${output15} != ['']    BuiltIn.Set Variable    ${output15}
    \    @{exlist} =    Get and Verify Exceptions    ${output0}
    \    Builtin.Log    ${exlist}
    \    ${exlength} =    BuiltIn.Get Length    ${exlist}
    \    BuiltIn.Run Keyword If    ${exlength} != 0    BuiltIn.Fail    Exceptions Found: ${\n}${exlist}${\n}

...
Stabilize Mininet System
    ...
    [Documentation]    Check till stale state (such as TIME_WAIT) is over
    Wait Until Keyword Succeeds    ${mininet_timeout}    1s    Wait Till Stale State Over In Mininet    ${switch_connection_state}

Clean And Stabilize Mininet System
    [Arguments]    ${switch_connection_state}
    [Documentation]    Clean Mininet system and check till stale state (such as TIME_WAIT) is over

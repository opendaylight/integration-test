...
FIXME__REFACTOR_THE_CODE_BELOW_AND_EXTRACT_TO_ITS_OWN_MODULE
    #
    # Refactoring of the code below to satisfy "best Robot coding practices"
    ...
    # to look good can wait as it is "nice to have" priority.
    #
    ...
    # to reduce the "Utils resource bloat" problem. Unfortunately it is
    # nearly impossible to do that now because some sort of "Robot automatic
    # review system" needs to be created first. The reason is that Robot is
    # not exactly helpful when it comes to reporting where an undefined
    # keyword was used. The first problem is that it only says that an
    # undefined keyword was used but it fails to reveal where, and second
    # problem is that the failure can get pretty hidden if the offending code
    # happens to run under "Run Keyword And Ignore Error" or similar (in that
    # case one needs to dig the error report out of the logs which slows the
    # edit-run-debug cycles down considerably).

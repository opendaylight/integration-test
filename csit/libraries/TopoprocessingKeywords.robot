...
Send Basic Request
    ...
    ${resp}    Get Request    session    ${OPERATIONAL_API}/ietf-network:network/topo:1
    Should Be Equal As Strings    ${resp.status_code}    200
    Log    ${resp.content}
    [Return]    ${resp}
...
Insert Filter
    [Arguments]    ${request_template}    ${model}    ${filter_template}    ${target_field}
...
Insert Filter With ID
    [Arguments]    ${request_template}    ${model}    ${filter_template}    ${target_field}    ${filter_id}
    [Documentation]    Add filter to filtration with specified id
    ${request_template}    Insert Filter    ${request_template}    ${model}    ${filter_template}    ${target_field}

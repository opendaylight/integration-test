...
Check Number of Termination Points in Node
    [Arguments]    ${model}    ${node}    ${tp_count}    ${supp_tp_count}
    [Documentation]    Checks number of supporting termination points in. Model should be 'network-topology-model', 'opendaylight-inventory-model' or 'i2rs-model'.
    Check Supported Model    ${model}
    Should Contain X Times    ${node}    <termination-point    ${tp_count}
    Run Keyword If    '${model}' == 'network-topology-model' or '${model}' == 'i2rs-model'    Should Contain X Times    ${node}    <tp-ref>    ${supp_tp_count}
    Run Keyword If    '${model}' == 'opendaylight-inventory-model'    Should Contain X Times    ${node}    <inventory-node-connector-ref    ${supp_tp_count}
...
Check Aggregated Node in Topology
    ...
    Check Number of Termination Points in Node    ${model}    ${aggregated_node}    ${tp_count}    ${tp_count}

...
class VpnUtils(Openstack):
    ...
    def __init__(self, odlip, logger = None):
        ...
        self.sid = res.create_session(self.vpnsession, baseurl1,  headers = ost.HEADERS, auth = ost.AUTH)
    ...
    def get_interfaceConfigData(self):
        ...
        print resp.content
        self.logmessage("All vlan interfaces config datastore ",0,resp.content)
        return resp
    ...
    def get_interfaceOperState(self):
        ...
        resp = res.get(self.vpnsession, odlurl)
        print resp.content
        self.logmessage("All vlan interfaces operational state ",0,resp.content)
        return resp
    ...
    def create_l3vpn(self, name,RD,importRT,exportRT):
        ...
        try:
            resp = res.postjson(self.vpnsession, odlurl, data = neutron_l3vpn)
    ...
    def getOVS_DpnIds(self, brname):
        ...
        self.logmessage("Response Code is",0,resp.status_code)
        try:
            ret = json.loads(resp.content)["bridge-ref-info"]["bridge-ref-entry"]
        except Exception as e:
            self.logmessage(" Exception is",0,e)
            return dpnId
        foundbr = 0
    ...
    def getL3vpn(self, vpnname):
        ...
        #odlurl = "/restconf/operations/neutronvpn:getL3VPN"
        try:
            resp = res.postjson(self.vpnsession, odlurl, data = get_l3vpn)
        except Exception as e:
    ...
    def deleteL3vpn(self, vpnname):
        ...
        self.logmessage("JASON structure of neutron_network",0,get_l3vpn)
        #odlurl = "/restconf/operations/neutronvpn:deleteL3VPN"
    ...
    def modify_l3vpn(self, name,RD,importRT,exportRT):
        ...
        #odlurl = self.odlneutron + "neutronvpn/createL3VPN"
        try:
            resp = res.putjson(self.vpnsession, odlurl, data = neutron_l3vpn)
        except Exception as e:
        ...
        if resp.status_code == 200:
            ...
            #delete self.l3vpnnamelist[name]
            self.l3vpnnamelist[name] = l3vpnid
            return l3vpnid
        else:
    ...
    def create_tunnel(self, **kwargs):
        """It creates gre tunnel .parameters are as shown below.
           netname --->  Name of  the network
           nettype --->  Network type, if you don't pass this, it takes default value as local
           i.e create_network("mynetwork")"""
        retval = 0
        ...
        #odlurl = self.odlneutron + "neutronvpn/createL3VPN"
        try:
            resp = res.postjson(self.vpnsession, odlurl, data = gre_tunnel)
        except Exception as e:
    ...
    def ping_from_node(self,node,ip,count):
        #PinfCMDN13ToN14 = 'ip netns exec n13 ping -c 4 10.0.1.14'
        cmd = "ip netns exec "+str(node)+" ping -c "+str(count)+" "+ip
        print cmd
        f = os.popen(cmd)
        output = f.read()
        print "Output of ping command :\n" + str(output)

...
class VpnUtils(object):
    ...
    def __init__(self, odlip, logger=None):
        ...
        self.ODLIP = odlip
        self.ODL_DATASTORE_CONFIG = "NO"
        self.RESTCONFPORT = '8181'
        self.PORT = '8080'
        self.ODLNEUTRON_DATA = "/restconf/config/neutron:neutron/"
        self.ODLNEUTRON = "/controller/nb/v2/neutron/"
        self.HEADERS = {'Content-Type': 'application/json'}
        self.AUTH = [u'admin', u'admin']
        if self.ODL_DATASTORE_CONFIG == "YES":
        ...
        if self.datastoreconfig == "YES":
            baseurl = "http://" + self.ODLIP + ":" + self.RESTCONFPORT
        else:
            baseurl = "http://" + self.ODLIP + ":" + self.PORT
        print baseurl
        if self.datastoreconfig == "YES":
            self.odlneutron = self.ODLNEUTRON_DATA
        else:
            self.odlneutron = self.ODLNEUTRON
        self.sid = res.create_session(self.odlsession,
                                      baseurl,
                                      headers=self.HEADERS,
                                      auth=self.AUTH)
        baseurl1 = "http://" + self.ODLIP + ":" + self.RESTCONFPORT
        self.datastore = res.create_session(self.odlsession1,
                                            baseurl1,
                                            headers=self.HEADERS,
                                            auth=self.AUTH)
        self.tntId = '6c53df3a-3456-11e5-a151-feff819cdc9f'
        self.l3vpnId = "4ae8cd92-48ca-49b5-94e1-b2921a26"
        self.networkjson = {}
        self.suname = "admin"
        self.minninetpasswd = "admin"
        self.ofctlcmd = "sudo ovs-ofctl"
        self.nindex = 0
        self.ipdigit = 1
        self.l3vpnurl = "/restconf/operations/neutronvpn"
        self.l3vpnnamelist = {}

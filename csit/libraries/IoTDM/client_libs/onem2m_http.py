...
http_headers = OneM2MEncodeDecodeData("HTTPHeaders")                \
    ...
    (http_header_content_location, http_header_content_location)    \
    (http_header_content_length, http_header_content_length)        \
    (OneM2M.short_from, http_header_origin)                         \
    (OneM2M.short_request_identifier, http_header_ri)               \
    (OneM2M.short_group_request_identifier, http_header_gid)        \
    (OneM2M.short_originating_timestamp, http_header_ot)            \
    (OneM2M.short_response_status_code, http_header_rsc)
...
class OneM2MHttpJsonEncoderRx(IoTDataEncoder):
    ...
    def encode(self, onem2m_primitive):
        ...
        if not http_result_code in proto_params:

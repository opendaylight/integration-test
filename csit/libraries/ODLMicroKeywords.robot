...
Run ODL Micro
    ...
    Run Keyword If    '${SCALE_TESTS}' == 'TRUE'    BuiltIn.Set_Suite_Variable    ${JAVA_HEAP_MEMORY}    -Xms12G -Xmx12G
    ...    ELSE    BuiltIn.Set_Suite_Variable    ${JAVA_HEAP_MEMORY}    -Xms128M -Xmx2048m
    ${command} =    NexusKeywords.Compose_Full_Java_Command    ${JAVA_HEAP_MEMORY} -XX:+UnlockDiagnosticVMOptions -XX:+HeapDumpOnOutOfMemoryError -XX:+DisableExplicitGC -Dcom.sun.management.jmxremote -Dfile.encoding=UTF-8 -Djava.security.egd=file:/dev/./urandom -cp "etc/initial/:lib/*" org.opendaylight.netconf.micro.NetconfMain > /tmp/odlmicro.log 2>&1
    BuiltIn.Log    ${command}

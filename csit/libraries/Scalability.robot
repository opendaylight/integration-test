...
Find Max Ovsdb Sb Switches
    [Arguments]    ${start}    ${stop}    ${step}
    [Documentation]    Find max ovsdb southbound switches from ${start} until ${stop} stepping by ${step}
    ${max-switches}    Set Variable    ${0}
    ${start}    Convert to Integer    ${start}
    ${stop}    Convert to Integer    ${stop}
    ${step}    Convert to Integer    ${step}
    ${NUM_TOOLS_SYSTEM}     Convert to Integer    ${NUM_TOOLS_SYSTEM}
    Install a Feature    odl-ovsdb-southbound-impl
    Install a Feature    odl-ovsdb-southbound-impl-rest
    : FOR    ${switches}    IN RANGE    ${start}    ${stop+1}    ${step}
    \    ${step_counter}    Set Variable If   ${step}==${switches}    ${25}    ${step_counter}
    \    ${timeout}    Set Variable If    ${switches}>100   ${step_counter}   ${switches/4}
    \    ${status}    ${result}    Run Keyword And Ignore Error    Start Mininet Linear    ${switches}
    \    Exit For Loop If    '${status}' == 'FAIL'
    ...
    \    Exit For Loop If    '${status}' == 'FAIL'
    \    ${step_counter}    Evaluate    ${step_counter}+${20}
    \    ${max-switches}    Convert To String    ${switches}
    [Return]    ${max-switches}
...
Find Max Ovsdb Netvirt Switches
    ...
    ${step}    Convert to Integer    ${step}
    ${NUM_TOOLS_SYSTEM}     Convert to Integer    ${NUM_TOOLS_SYSTEM}
    Install a Feature    odl-ovsdb-openstack
    : FOR    ${switches}    IN RANGE    ${start}    ${stop+1}    ${step}
    \    ${step_counter}    Set Variable If   ${step}==${switches}    ${25}    ${step_counter}
    \    ${timeout}    Set Variable If    ${switches}>100   ${step_counter}    ${switches/4}
    \    ${status}    ${result}    Run Keyword And Ignore Error    Start Mininet Linear    ${switches}
    ...
    \    Exit For Loop If    '${status}' == 'FAIL'
    \    ${status}    ${result}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    ${timeout*2}    10s
    \    ...    Check Netvirt Br    ${ovs_switch_uuid_list}
    \    Exit For Loop If    '${status}' == 'FAIL'
    \    ${status}    ${result}    Run Keyword And Ignore Error    Stop Mininet Simulation
    \    Exit For Loop If    '${status}' == 'FAIL'
...
Start Mininet Linear
    [Arguments]    ${switches}    ${timeout}=${switches*2}
    ...
    \   Read Until    mininet>
    \   Sleep    6
    \   Write    s1 ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
    \   Read Until    mininet>
    \   Get Ovsdb Mininet Node Names    ${num_ovs_per_mininet}
    Set Suite Variable    ${tools_conn_ids_list}
...
Stop Mininet Simulation
    ...
    Stop Multiple Suite  ${tools_conn_ids_list}

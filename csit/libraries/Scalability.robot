...
Find Max Switches
    ...
    \    ${status}    ${result}    Run Keyword And Ignore Error    Verify Controller Is Not Dead    ${ODL_SYSTEM_1_IP}
    \    Exit For Loop If    '${status}' == 'FAIL'
...
Find Max Links
    ...
    \    ${status}    ${result}    Run Keyword And Ignore Error    Verify Controller Is Not Dead    ${ODL_SYSTEM_1_IP}
    \    Exit For Loop If    '${status}' == 'FAIL'
    \    ${status}    ${result}    Run Keyword And Ignore Error    Verify Controller Has No Null Pointer Exceptions    ${ODL_SYSTEM_1_IP}
    \    Exit For Loop If    '${status}' == 'FAIL'
...
Start Mininet With One Switch And ${hosts} hosts
    ...
    Write    sudo mn --controller=remote,ip=${ODL_SYSTEM_1_IP} --topo linear,1,${hosts} --switch ovsk,protocols=OpenFlow13
    Read Until    mininet>
...
Start Mininet Linear
    ...
    \   Set Suite Variable  ${mininet_conn_ids_list}
    \   Login With Public Key    ${MININET_USER}    ${USER_HOME}/.ssh/${SSH_KEY}    any
    \   Write    sudo mn --controller=remote,ip=${ODL_SYSTEM_1_IP} --topo linear,${num_ovs_per_mininet} --switch ovsk,protocols=OpenFlow13
    \   Read Until    mininet>
    \   Sleep    6
...
Start Mininet With Custom Topology
    ...
    \   Write    sudo mn --controller=remote,ip=${ODL_SYSTEM_1_IP} --custom switch.py --topo demotopo --switch ovsk,protocols=OpenFlow13
    \   Read Until    mininet>
...
Stop Mininet Simulation
    ...
    Stop Suite  ${mininet_conn_list}=${mininet_conn_ids_list}

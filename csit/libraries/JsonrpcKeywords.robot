...
Run Read Service Python Script on Controller Vm
    ...
    ${stdout}    SSHLibrary.Write    nohup python ${WORKSPACE}/${BUNDLEFOLDER}/odl-jsonrpc-test-read tcp://0.0.0.0:${DEFAULT_PORT} 'config' ${DEFAULT_ENDPOINT} '{"ietf-interfaces:interfaces":{}}' '{"interface":[{"name": "test","type": "other"}]}' &
    Log    ${stdout}
...
Mount Read Service Endpoint
    ...
    ${ctrl_ip}    Builtin.Set Variable    127.0.0.1

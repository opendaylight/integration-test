...
Run Read Service Python Script on Controller Vm
    ...
    Log    ${cmd}
    Virtual_Env_Set_Path    /tmp/jsonrpc_venv
    Virtual_Env_Create    True
    Virtual_Env_Install_Package    pyzmq
    Virtual_Env_Install_Package    zmq
    Virtual_Env_Activate_On_Current_Session    True
    ${stdout}    SSHLibrary.Write    echo | rm -rf nohup.out
    ${stdout}    SSHLibrary.Write    echo | ${cmd} &

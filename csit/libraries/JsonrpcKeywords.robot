...
Run Read Service Python Script
    [Documentation]    This keyword installs pip,zmq,pyzmq and starts the read service on jsonrpc
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    sudo yum -y install python-pip    return_stdout=True    return_stderr=True
    ...    return_rc=True
    Log    ${stdout}

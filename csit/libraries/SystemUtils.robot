...
Rsync Directory
    ...
    Switch Connection     ${os_node_cxn}
    Write Commands Until Expected Prompt      sudo rsync -av ${src_dir} ${dst_node_ip}:${dst_dir}          d:         30s
    Write Commands Until Expected Prompt      ${OS_PASSWORD}         >     30s
...
Generic HAProxy Entry
    ...
    Append To File      ${os_node_cxn}     /etc/haproxy/haproxy.cfg     ' 'option tcpka
    Append To File      ${os_node_cxn}     /etc/haproxy/haproxy.cfg     ' 'option httpchk
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'option tcplog
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_1_IP}:${port_to_listen} check inter 2000 rise 2 fall 5
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_2_IP}:${port_to_listen} check inter 2000 rise 2 fall 5
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_3_IP}:${port_to_listen} check inter 2000 rise 2 fall 5
...
Keystone Manage Setup
    ...
    Log    ${output}
    Should Not Be True    ${rc}
    ${output}    ${rc}=    Execute Command    sudo keystone-manage credential_setup --keystone-user ${keystone_user} --keystone-group ${keystone_password}     return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}

...
Chown File
    ...
    Switch Connection     ${os_node_cxn}
    ${output}    ${rc}=    Execute Command    sudo chown -R ${user_group}:${user_group} ${file_or_path}    return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}
    [Return]    ${output}
...
Rsync Directory
    ...
    Switch Connection     ${os_node_cxn}
    Write Commands Until Expected Prompt      sudo rsync -e "ssh -o StrictHostKeyChecking=no" -av ${src_dir} ${dst_node_ip}:${dst_dir}          d:         30s
...
Generic HAProxy Entry
    ...
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_1_IP}:${port_to_listen} check inter 1s
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_2_IP}:${port_to_listen} check inter 1s
    Append To File      ${os_node_cxn}    /etc/haproxy/haproxy.cfg      ' 'server controller1 ${OS_CONTROL_3_IP}:${port_to_listen} check inter 1s

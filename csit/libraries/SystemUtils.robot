...
Create Database for Mysql
    ...
    Switch Connection     ${os_node_cxn}
    ${output}    ${rc}=    Execute Command    sudo mysql -u${mysql_user} -p${mysql_pass} -e CREATE DATABASE ${db_name} CHARACTER SET utf8;    return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}
    [Return]    ${output}
...
Grant Privileges To Mysql Database
    ...
    ${output}=     Write Commands Until Expected Prompt      GRANT ALL PRIVILEGES ON ${db_name} TO '${db_user}'@'${host_name}' identified by '${host_user}'; exit;      Bye       30s
    Log    ${output}
    [Return]    ${output}
...
Copy RabbitMQ Cookie File
    [Arguments]    ${os_node_cxn}    ${rabbit_user}       ${dst_hostname}
    ...
    Log    ${output}
    Should Not Be True    ${rc}
    ${output}    ${rc}=    Execute Command    rabbitmqctl join_cluster --ram ${rabbit_user}@${dst_hostname}     return_rc=True    return_stdout=True
    Log    ${output}
    Should Not Be True    ${rc}

...
Run Rest Command For Connecting Networks
    ...
    Create Session    session1    http://${session_ip}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    ...
    ${resp}=    RequestsLibrary.Post Request    session1    /restconf/operations/federation-plugin-rpc:update-federated-networks    data=${data}
...
Run Rest Command For Disconnecting Networks
    ...
    Create Session    session2    http://${odl_ip}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    ...
    ${resp}=    RequestsLibrary.Post Request    session2    /restconf/operations/federation-plugin-rpc:update-federated-networks    data=${data}
    Should Be Equal As Strings    ${resp.status_code}    200

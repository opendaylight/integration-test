...
*** Variables ***
${OS_CONTROL_NODE_IP1}   10.0.97.139
${OS_CONTROL_NODE_IP2}    10.0.77.132
${ODL_SYSTEM_IP1}    10.0.97.134
${ODL_SYSTEM_IP2}    10.0.97.100
...
Put Federation Config Data Site A
    ...
    ${response}=    RequestsLibrary.Put Request    session    /restconf/config/federation-service-config:federation-config-data    data={"federation-config-data": {"site-ip": "${ODL_SYSTEM_IP1}","mqUserPwd": "guest","startService": true,"controlQueueName": "CONTROL_QUEUE_${ODL_SYSTEM_IP1}","mqPortNumber": 5672,"mqBrokerIp": "${ODL_SYSTEM_IP1}","mqUser": "guest"}}
    Should Be Equal As Strings    ${response.status_code}    200

Put Federation Config Data Site B
    [Documentation]    Putting federation config data by put rest command on site b.
    Create Session    session    http://${ODL_SYSTEM_IP2}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    ${response}=    RequestsLibrary.Put Request    session    /restconf/config/federation-service-config:federation-config-data    data={"federation-config-data": {"site-ip": "${ODL_SYSTEM_IP2}","mqUserPwd": "guest","startService": true,"controlQueueName": "CONTROL_QUEUE_${ODL_SYSTEM_IP2}","mqPortNumber": 5672,"mqBrokerIp": "${ODL_SYSTEM_IP2}","mqUser": "guest"}}
    Should Be Equal As Strings    ${response.status_code}    200
...
Run Rest Command For Connecting Networks
    [Arguments]    ${netSiteA}    ${netSiteB}    ${subnetNetSiteA}    ${subnetNetSiteB}    ${subnet_range}    ${session_ip}    ${odl_ip}
...
Connect Two Networks
    ...
    Run Rest Command For Connecting Networks    ${netSiteA}    ${netSiteB}    ${subnetNetSiteA}    ${subnetNetSiteB}    ${subnet_range}    ${ODL_SYSTEM_IP1}    ${ODL_SYSTEM_IP2}
    Run Rest Command For Connecting Networks    ${netSiteB}    ${netSiteA}    ${subnetNetSiteB}    ${subnetNetSiteA}    ${subnet_range}    ${ODL_SYSTEM_IP2}    ${ODL_SYSTEM_IP1}

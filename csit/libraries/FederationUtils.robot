...
Run Rest Command For Connecting Networks
    ...
    Create Session    session    http://${session_ip}:${RESTCONFPORT}    auth=[u'admin', u'admin']    headers=${HEADERS}
    ${data}=    Set Variable    {"input":{"federated-networks-in": [{"self-subnet-id": "${subnetNetSiteA}", "self-net-id": "${netSiteA}", "site-network": [{"site-tenant-id": "7dddfebf-0c32-4d82-9b62-ffbd02d57870", "id": "bc2EaF0B-eBAf-B7b7-92fa-BAA1e83BDEcE", "site-ip": "${odl_ip}", "site-net-id": "${netSiteB}", "site-subnet-id": "${subnetNetSiteB}"}], "subnet-ip": "${subnet_range}", "self-tenant-id": "155d9562-325c-4b3e-bd59-cc551510aa03"}]}, "federated-acls-in":[]}
    Log    ${data}
...
Run Rest Command For Disconnecting Networks
    ...
    Create Session    session    http://${odl_ip}:${RESTCONFPORT}    auth=[u'admin', u'admin']    headers=${HEADERS}

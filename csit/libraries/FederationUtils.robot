...
Config Federation Sites
    ...
    : FOR    ${i}    IN RANGE    1    3
    \    ${odl_ip}=    Catenate    SEPARATOR=    HA_PROXY_IP_    ${i}
    \    Create Session    session    http://${${odl_ip}}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    \    ${data}=    Set Variable    {"federation-service-config:federation-sites-config": {"federation-service-config:federation-site-config": [{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${HA_PROXY_IP_1}","federation-service-config:mqUserPwd": "guest","federation-service-config:controlQueueName": "CONTROL_QUEUE_${HA_PROXY_IP_1}","federation-service-config:mqUser": "guest","federation-service-config:broker-ip": "${HA_PROXY_IP_1}"},{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${HA_PROXY_IP_2}","federation-service-config:mqUserPwd": "guest","federation-service-config:controlQueueName":"CONTROL_QUEUE_${HA_PROXY_IP_2}","federation-service-config:mqUser": "guest","federation-service-config:broker-ip": "${HA_PROXY_IP_2}"}]}}
...
Connect Two Networks
    ...
    Run Rest Command For Connecting Networks    ${netSiteA}    ${netSiteB}    ${subnetNetSiteA}    ${subnetNetSiteB}    ${subnet_range}    ${HA_PROXY_IP_1}
    ...    ${HA_PROXY_IP_2}
    Run Rest Command For Connecting Networks    ${netSiteB}    ${netSiteA}    ${subnetNetSiteB}    ${subnetNetSiteA}    ${subnet_range}    ${HA_PROXY_IP_2}
    ...    ${HA_PROXY_IP_1}
...
Disconnect Two Networks
    ...
    ${netSiteB}=    Get Network Id    ${siteB_index}    ${net_name}
    ${subnetNetSiteB}=    Get Network Subnet Id    ${siteB_index}    ${subnet_name}
    Run Rest Command For Disconnecting Networks    ${netSiteA}    ${subnetNetSiteA}    ${subnet_range}    ${HA_PROXY_IP_1}
    Run Rest Command For Disconnecting Networks    ${netSiteB}    ${subnetNetSiteB}    ${subnet_range}    ${HA_PROXY_IP_2}

...
Put Federation Config Data On Specific Site
    [Arguments]    ${odl_ip}
    [Documentation]    Putting federation config data by put rest command on a site.
...
Config Federation Sites
    ...
    \    Create Session    session    http://${${odl_ip}}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    \    ${data}=    Set Variable    {"federation-service-config:federation-sites-config": {"federation-service-config:federation-site-config": [{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${ODL_SYSTEM_1_IP}","federation-service-config:mqUserPwd": "guest","federation-service-config:controlQueueName": "CONTROL_QUEUE_${ODL_SYSTEM_1_IP}","federation-service-config:mqUser": "guest","federation-service-config:broker-ip": "${ODL_SYSTEM_1_IP}"},{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${ODL_SYSTEM_2_IP}","federation-service-config:mqUserPwd": "guest","federation-service-config:controlQueueName":"CONTROL_QUEUE_${ODL_SYSTEM_2_IP}","federation-service-config:mqUser": "guest","federation-service-config:broker-ip": "${ODL_SYSTEM_2_IP}"}]}}
...
Run Rest Command For Connecting Networks
    ...
    Log    ${data}
    ${resp}=    RequestsLibrary.Post Request    session    /restconf/operational/federation-plugin-rpc:update-federated-networks    data=${data}    headers=${HEADERS}
...
Connect Two Networks
    ...
    Run Rest Command For Connecting Networks    ${netSiteA}    ${netSiteB}    ${subnetNetSiteA}    ${subnetNetSiteB}    ${subnet_range}    ${ODL_SYSTEM_2_IP}
    ...    ${ODL_SYSTEM_1_IP}
    Run Rest Command For Connecting Networks    ${netSiteB}    ${netSiteA}    ${subnetNetSiteB}    ${subnetNetSiteA}    ${subnet_range}    ${ODL_SYSTEM_1_IP}
    ...    ${ODL_SYSTEM_2_IP}

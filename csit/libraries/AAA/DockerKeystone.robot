...
Get Keystone Token
    ...
    ${token_received}    Get From Dictionary    ${resp.headers}    x-subject-token
    Set Global Variable    ${token}    ${token_received}
...
Run Docker Keystone
    ...
    ${output}    SSHLibrary.Execute Command    ./start_keystone.sh    return_stdout=True    return_stderr=True    return_rc=True
    sleep    15
    ${output}    SSHLibrary.Execute Command    docker exec -t keystone bash -c "hostname -f;cat openrc;source openrc;openstack user create --password cscuser CSC_user;openstack user set --project admin CSC_user;openstack role add --project admin --user CSC_user admin;openstack role add --domain default --user CSC_user admin;openstack user list"    return_stdout=True    return_stderr=True    return_rc=True
    ${output}    SSHLibrary.Execute Command    docker exec -t keystone bash -c "source openrc;openstack user create --password cscusernoadmin CSC_user_no_admin;openstack role add --domain default --user CSC_user_no_admin user;openstack user list"    return_stdout=True    return_stderr=True    return_rc=True
    [Return]    ${output}

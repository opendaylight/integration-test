...
then do manipulation in the Robot file."""
...
def get_variables(mininet_ip):
    ...
    # Comments a analyze from high level, to low level, then code builds from low level back to high level.
 ...
 "network-topology-pcep:path-computation-client": {
  ...
  "stateful-tlv": {
   "odl-pcep-ietf-stateful:stateful": {
    ...
    # Discussion amount delegated and instantiated implies that $LSPS is either a single delegated LSP

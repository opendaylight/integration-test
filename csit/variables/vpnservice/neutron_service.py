...
FLOWS_DPN1 = " sudo ovs-ofctl dump-flows BR1 -O OpenFlow13"
FLOWS_DPN2 = " sudo ovs-ofctl dump-flows BR2 -O OpenFlow13"
...
sudo /sbin/ip netns exec ns1 /sbin/ip link set dev tap-port-1 up
sudo /sbin/ip link set dev tap79ad0001-19 up
sudo /sbin/ip netns exec ns1 /sbin/ifconfig tap-port-1 20.1.1.2 netmask 255.255.255.0
sudo /sbin/ip netns exec ns1 /sbin/ifconfig tap-port-1 hw ether 00:16:3E:19:F7:8B
sudo /sbin/ip netns exec ns1 /sbin/ip link set dev lo up
sudo /sbin/ip netns exec ns1 /sbin/ip route add default via 20.1.1.1
...
sudo /sbin/ip netns exec ns2 /sbin/ip link set dev tap-port-2 up
sudo /sbin/ip link set dev tap79ad0002-19 up
sudo /sbin/ip netns exec ns2 /sbin/ifconfig tap-port-2 20.1.1.3 netmask 255.255.255.0
sudo /sbin/ip netns exec ns2 /sbin/ifconfig tap-port-2 hw ether 00:16:3E:BB:9B:0F
sudo /sbin/ip netns exec ns2 /sbin/ip link set dev lo up
sudo /sbin/ip netns exec ns2 /sbin/ip route add default via 20.1.1.1
...
sudo /sbin/ip netns exec ns3 /sbin/ip link set dev tap-port-3 up
sudo /sbin/ip link set dev tap79ad0003-19 up
sudo /sbin/ip netns exec ns3 /sbin/ifconfig tap-port-3 30.1.1.4 netmask 255.255.255.0
sudo /sbin/ip netns exec ns3 /sbin/ifconfig tap-port-3 hw ether 00:16:3E:92:F5:F8
sudo /sbin/ip netns exec ns3 /sbin/ip link set dev lo up
sudo /sbin/ip netns exec ns3 /sbin/ip route add default via 30.1.1.1
...
sudo /sbin/ip netns exec ns4 /sbin/ip link set dev tap-port-4 up
sudo /sbin/ip link set dev tap79ad0004-19 up
sudo /sbin/ip netns exec ns4 /sbin/ifconfig tap-port-4 30.1.1.5 netmask 255.255.255.0
sudo /sbin/ip netns exec ns4 /sbin/ifconfig tap-port-4 hw ether 00:16:3E:19:57:58
sudo /sbin/ip netns exec ns4 /sbin/ip link set dev lo up
sudo /sbin/ip netns exec ns4 /sbin/ip route add default via 30.1.1.1

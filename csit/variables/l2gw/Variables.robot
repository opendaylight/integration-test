*** Variables ***
#TEMPORARY Variables for Local Run
${USER_HOME}     /home/mininet
${OS_CONTROL_NODE_IP}    192.168.122.186
${OVS_IP}        192.168.122.181
${HWVTEP_IP}     192.168.122.62
${ODL_IP}        192.168.122.171
${OS_USER}       stack
...
${GET_ALL_PORTS_URL}    neutron:neutron/ports
...
${SLEEP1S}        sleep 1
${START_OVSDB_SERVER}    sudo ovsdb-server --pidfile --detach --log-file --remote punix:/usr/var/run/openvswitch/db.sock --remote=db:hardware_vtep,Global,managers /etc/openvswitch/ovs.db /etc/openvswitch/vtep.db
${START_OVSVTEP}    sudo /usr/share/openvswitch/scripts/ovs-vtep --log-file=/var/log/openvswitch/ovs-vtep.log --pidfile=/usr/var/run/openvswitch/ovs-vtep.pid --detach ${HWVTEP_BRIDGE}
...
${NETSTAT_OVSDB_REGEX}    ${ODL_IP}:${OVSDBPORT}\\s+ESTABLISHED\\s
${NETSTAT_OF_REGEX}    ${ODL_IP}:${ODL_OF_PORT}\\s+ESTABLISHED\\s

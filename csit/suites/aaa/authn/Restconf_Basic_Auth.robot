...
IDM Endpoints Only Available To admin Role
    ...
    ${resp} =    RequestsLibrary.GET On Session    httpbin    ${USERS_REST_URL}    expected_status=401
...
Auth Should Fail
    ...
    Create Session    httpbin    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${auth}    headers=${HEADERS}
    ${resp} =    RequestsLibrary.GET On Session    httpbin    ${url}    expected_status=401
...
Auth Should Pass
    ...
    @{auth} =    Create List    ${user}    ${password}
    Create Session    httpbin    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${auth}    headers=${HEADERS}
    ${resp} =    RequestsLibrary.GET On Session    httpbin    ${url}    expected_status=200

...
TLS on Restconf without Server Cert
    ...
    Request Method    GET
    Run Keyword And Expect Error    error: (7, 'Failed to connect to * port 8443: Connection refused')    Perform
    Log Response
...
Log Certificates in Keystore
    ...
    ${output}    Run Command On Remote System    ${ODL_SYSTEM_IP}    keytool -list -storepass 123456 -keystore ${KEYSTORE_PATH}
    log    ${output}

Clean Up Certificates In Server
    [Documentation]    Cleans keystore content (only for private keys and trusted certificates)
    Log Certificates in Keystore
    Run Command On Remote System    ${ODL_SYSTEM_IP}    keytool -list -keystore ${KEYSTORE_PATH} -storepass 123456|egrep -e "(trustedCertEntry|PrivateKeyEntry)"|cut -d"," -f1|xargs -I[] keytool -delete -alias [] -keystore ${KEYSTORE_PATH} -storepass 123456
    Log Certificates in Keystore
...
Generate Server Self-Signed Certificate
    ...
    Run Command On Remote System    ${ODL_SYSTEM_IP}    keytool -importkeystore -deststorepass 123456 -destkeypass myPass -destkeystore ${KEYSTORE_PATH} -srckeystore server.p12 -srcstoretype PKCS12 -srcstorepass myPass -alias odl
    Log Certificates in Keystore
    Restart Jetty
...
Generate Client Self-Signed Certificate
    ...
    Run Command On Remote System    ${ODL_SYSTEM_IP}    keytool -import -trustcacerts -file clientcert.pem -keystore ${KEYSTORE_PATH} -storepass 123456 -noprompt
    Log Certificates in Keystore
    Restart Jetty

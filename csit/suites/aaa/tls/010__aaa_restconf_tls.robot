...
TLS on Restconf without Server Cert
    ...
    Request Method    GET
    Run Keyword And Expect Error    error: (7, 'Failed *${RESTCONFPORT_TLS}* Connection refused')    Perform
    Log Response
...
TLS on Restconf with Server & Client Certs (Self-signed)
    ...
    Add Header    "Content-Type:application/json"
    Add Header    Authorization:Basic YWRtaW46YWRtaW4=
    Client Certificate File    ${USER_HOME}/clientcert.pem
    Private Key File    ${USER_HOME}/clientkey.pem
    Request Method    GET
    Perform
    Log Response
    Response Status Should Contain    200

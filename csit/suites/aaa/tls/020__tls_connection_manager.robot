...
Disable TLS in AAA
    [Documentation]    Remove TLS configuration in custom.properties
    Run Command On Remote System    ${ODL_SYSTEM_IP}    sudo sed -i 's/<use-config>true<\\/use-config>/<use-config>false<\\/use-config>/' ${AAA_TLS_CONFIG}
...
Set Valid Certificates
    ...
    Copy File To Remote System    ${ODL_SYSTEM_IP}    certs/ctl.jks   ${KEYSTORE_PATH}
    Copy File To Remote System    ${ODL_SYSTEM_IP}    certs/truststore.jks    ${TRUSTSTORE_PATH}
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    certs/sc-privkey.pem    /tmp/.
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    certs/sc-cert.pem    /tmp/.
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    certs/ca-chain.cert.pem    /tmp/.

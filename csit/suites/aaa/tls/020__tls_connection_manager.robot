...
Connecting an OVS instance to the controller
    ...
    Wait Until Keyword Succeeds    10s    1s    Verify OVS Reports Connected
    Run Command On Remote System    ${TOOLS_SYSTEM_IP}    sudo ovs-vsctl del-manager
...
Set Valid Certificates
    ...
    Copy File To Remote System    ${ODL_SYSTEM_IP}    ${WORKSPACE}/test/csit/suites/aaa/tls/certs/ctl.jks    ${KEYSTORE_PATH}
    Copy File To Remote System    ${ODL_SYSTEM_IP}    ${WORKSPACE}/test/csit/suites/aaa/tls/certs/truststore.jks    ${TRUSTSTORE_PATH}
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    ${WORKSPACE}/test/csit/suites/aaa/tls/certs/sc-privkey.pem    /tmp/.
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    ${WORKSPACE}/test/csit/suites/aaa/tls/certs/sc-cert.pem    /tmp/.
    Copy File To Remote System    ${TOOLS_SYSTEM_IP}    ${WORKSPACE}/test/csit/suites/aaa/tls/certs/ca-chain.cert.pem    /tmp/.

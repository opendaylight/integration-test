...
DIDM Suite Teardown
    ...
    Uninstall a Feature    odl-didm-ovs-all
    Verify Feature Is Not Installed    odl-didm-ovs-all
    Uninstall a Feature    odl-didm-ovs-impl
    Verify Feature Is Not Installed    odl-didm-ovs-impl
    Uninstall a Feature    odl-openflowplugin-all-li
    Verify Feature Is Not Installed    odl-openflowplugin-all-li

Start Device
    [Arguments]    ${device}    ${cmd}    ${user}    ${password}    ${prompt}    ${prompt_timeout}=30s
    [Documentation]    Login to the device and start it
    Log    Start the device
    ${cleanup_cmd}=    Set Variable    sudo mn -c
    Run Command On Remote System    ${device}    ${cleanup_cmd}    ${user}    ${password}    ${prompt}
    Run Command On Remote System    ${device}    ${cmd}    ${user}    ${password}    ${prompt}
...
Setup DIDM Environment
    ...
    Log    Start device
    ${device_start_cmd}=    Set Variable    sudo mn --controller=remote,ip=${CONTROLLER} --topo linear,1,2 --switch ovsk,protocols=OpenFlow13
    Start Device    ${DEVICE_IP}    ${device_start_cmd}    ${DEVICE_USER}    ${DEVICE_PASSWORD}    ${TOOLS_SYSTEM_PROMPT}

...
Create Setup And Verify Instance Connectivity
    [Documentation]    Assumption is that netvirt features are not installed to begin with, so once those features are up we will
    ...    Create 2 VXLAN networks, subnets with 2 VMs each and a router. Ping all 4 VMs.

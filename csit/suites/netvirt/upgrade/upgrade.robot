...
${SECURITY_GROUP}    up_sg
${JOURNAL}    ${KARAF_HOME}/journal
${SNAPSHOTS}    ${KARAF_HOME}/snapshots
${ADD_IPTABLE_RULE}    -A
${DELETE_IPTABLE_RULE}    -D
@{NETWORKS}    up_net_1    up_net_2
@{SUBNETS}    up_sub_1    up_sub_2
@{NET_1_VMS}    up_net_1_vm_1    up_net_1_vm_2
@{NET_2_VMS}    up_net_2_vm_1    up_net_2_vm_2
...
Wipe cache
    ...
    Delete Directory    ${OS_CONTROL_NODE_IP}    ${JOURNAL}    ${SNAPSHOTS}
    Run Keyword If    1 < ${NUM_OS_SYSTEM}    Delete Directory    ${OS_COMPUTE_1_IP}    ${JOURNAL}    ${SNAPSHOTS}
    Run Keyword If    2 < ${NUM_OS_SYSTEM}    Delete Directory    ${OS_COMPUTE_2_IP}    ${JOURNAL}    ${SNAPSHOTS}

Block ports
    [Documentation]    Block OVS and OVSDB ports on ODL node for all OpenStack nodes.
    Iptables Modifications    ${ADD_IPTABLE_RULE}    ${OS_CONTROL_NODE_IP}
    Run Keyword If    1 < ${NUM_OS_SYSTEM}    Iptables Modifications    ${ADD_IPTABLE_RULE}    ${OS_COMPUTE_1_IP}
    Run Keyword If    2 < ${NUM_OS_SYSTEM}    Iptables Modifications    ${ADD_IPTABLE_RULE}    ${OS_COMPUTE_2_IP}
...
Unblock ports
    ...
    Iptables Modifications    ${DELETE_IPTABLE_RULE}    ${OS_CONTROL_NODE_IP}
    sleep    10
    Run Keyword If    1 < ${NUM_OS_SYSTEM}    Iptables Modifications    ${DELETE_IPTABLE_RULE}    ${OS_COMPUTE_1_IP}
    sleep    10
    Run Keyword If    2 < ${NUM_OS_SYSTEM}    Iptables Modifications    ${DELETE_IPTABLE_RULE}    ${OS_COMPUTE_2_IP}
...
Create resources and check connectivity
    ...
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_1    @{NET1_VM_IPS}[0]
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_1    @{NET1_VM_IPS}[1]
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_1    @{NET2_VM_IPS}[0]
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_2    @{NET2_VM_IPS}[0]
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_2    @{NET2_VM_IPS}[1]
    OpenStackOperations.Ping Vm From DHCP Namespace    up_net_2    @{NET1_VM_IPS}[0]
...
Get flows from br-int
    ...
    ${br_int_show}=    BuiltIn.Write Commands Until Expected Prompt    sudo ovs-ofctl show br-int -OOpenFlow13    ${DEFAULT_LINUX_PROMPT_STRICT}
    OperatingSystem.Append To File    ${log_file}    ${br_int_show}
    ${flows}=    BuiltIn.Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13    ${DEFAULT_LINUX_PROMPT_STRICT}
...
Delete Directory
    [Arguments]    ${node_ip}    ${dir1}    ${dir2}
    [Documentation]    Delete the specified directory {dir} from node {node_ip}
    Utils.Run Command On Remote System And Log    ${node_ip}    OperatingSystem.Remove Directory    ${dir1}
    Utils.Run Command On Remote System And Log    ${node_ip}    OperatingSystem.Remove Directory    ${dir2}
...
Iptable Modifications
    ...
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    ${action} INPUT -p tcp --destination-port 6640 -s ${node_ip} -j DROP
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    ${action} INPUT -p tcp --destination-port 6653 -s ${node_ip} -j DROP
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    ${action} INPUT -p tcp --destination-port 6633 -s ${node_ip} -j DROP

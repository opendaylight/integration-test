...
Set OVS Manager And Controller
    ...
    : FOR    ${node}    IN    @{OS_ALL_IPS}
    \    ${mgr}=    Run Command On Remote System    ${node}    ${OVS_SET_MGR}:${ODL_SYSTEM_IP}:${OVSDBPORT}
    \    ${con}=    Run Command On Remote System    ${node}    ${OVS_SET_CTRLR} ${BRINT} tcp:${ODL_SYSTEM_IP}:${ODL_OF_PORT_6653}
    \    Log    ${mgr}
    \    Log    ${con}

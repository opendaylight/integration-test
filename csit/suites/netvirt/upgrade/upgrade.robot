...
Disconnect OVS
    ...
    : FOR    ${node}    IN    @{OS_ALL_IPS}
    \    Run Command On Remote System    ${node}    sudo ovs-vsctl del-manager
    \    Run Command On Remote System    ${node}    sudo /usr/share/openvswitch/scripts/ovs-ctl stop
    \    Run Command On Remote System    ${node}    sudo rm -rf /etc/openvswitch/conf.db
...
Start ODL
    ...
    BuiltIn.Sleep    100
    ${ClusterManagement__has_setup_run}    BuiltIn.Set Suite Variable    False
...
Set OVS Manager And Controller
    ...
    : FOR    ${node}    IN    @{OS_ALL_IPS}
    \    Run Command On Remote System    ${node}    sudo /usr/share/openvswitch/scripts/ovs-ctl start

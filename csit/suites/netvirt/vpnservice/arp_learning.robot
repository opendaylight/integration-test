...
Verify Setup
    ...
    # Verify ping between VMs on the same network. For this, we ssh to the VM using the dhcp-namespace on the
...
Verify GARP Requests
    ...
    # Verify that for VM1_net1, FIB Entry exists in the ODL Datastore with its nexthop pointing to Compute-1 hostIP
    ...
    # Create an alias interface (eth0:1) on VM1_net0, configure it with an extra_route_ip, trigger 5 GARPs from
    # the VM for the extra_route_ip and ensure that ODL learns the extra_route_ip info.
    ...
    # Query the odl-fib:fibEntries config store info and ensure that extra_route_ip configured on VM1_net0 is present in

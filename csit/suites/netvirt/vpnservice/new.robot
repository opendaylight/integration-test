...
Configure Extra IPv4/IPv6 Addresss On Interface For Subnet Routing
    ...
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPV6}[0]    sudo ip -6 addr add @{EXTRA_NW_IPV6}[0]/64 dev eth0; ip -6 a
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPV6}[1]    sudo ip -6 addr add @{EXTRA_NW_IPV6}[1]/64 dev eth0; ip -6 a
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    @{NET_2_VM_IPV6}[0]    sudo ip -6 addr add @{EXTRA_NW_IPV6}[3]/64 dev eth0; ip -6 a
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    @{NET_2_VM_IPV6}[1]    sudo ip -6 addr add @{EXTRA_NW_IPV6}[4]/64 dev eth0; ip -6 a

...
Create Neutron Subnets
    ${net1_additional_args}=    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${vpn6_net_1_ADDR_POOL}
    ${net2_additional_args}=    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${vpn6_net_2_ADDR_POOL}
...
Add Multiple Extra Routes And Check Datapath Before L3VPN Creation
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    ${vm_instances}
...
Delete And Recreate Extra Route
    ...
    [Teardown]    BuiltIn.Run Keywords    OpenStackOperations.Update Router    @{ROUTERS}[0]    ${RT_CLEAR}
    ...    AND    OpenStackOperations.Show Router    @{ROUTERS}[0]    -D
    ...    AND    OpenStackOperations.Get Test Teardown Debugs

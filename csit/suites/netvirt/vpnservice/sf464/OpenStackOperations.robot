...
Source Password
    ...
    Run Keyword If    '${source_pwd}' == 'yes' or '${force}' == 'yes'    OpenStackOperations.Write Commands Until Prompt    cd ${DEVSTACK_DEPLOY_PATH}; source openrc
...
Get Tenant ID From Security Group
    ...
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron security-group-show default | grep "| tenant_id" | awk '{print $4}'
    Log    ${output}
    [Return]    ${output}
...
Create Network
    ...
    ${output}=    OpenStackOperations.Write Commands Until Prompt    ${command}    30s
    Log    ${output}
    [Return]    ${output}
...
List Networks
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron net-list    30s
...
List Subnets
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron subnet-list    30s
    SSHLibrary.Close Connection
    Log    ${output}
    [Return]    ${output}
...
Delete Network
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v net-delete ${network_name}    30s
    SSHLibrary.Close Connection
    Log    ${output}
...
Create SubNet
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v subnet-create ${network_name} ${range_ip} --name ${subnet} ${additional_args}    30s
    SSHLibrary.Close Connection
    Log    ${output}
...
Create Port
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v port-create ${network_name} --name ${port_name} --security-group ${sg}    30s
    SSHLibrary.Close Connection
    Log    ${output}
...
Delete Port
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v port-delete ${port_name}    30s
    SSHLibrary.Close Connection
    Log    ${output}
...
List Ports
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron port-list    30s
    SSHLibrary.Close Connection
    Log    ${output}
    [Return]    ${output}
...
Create And Associate Floating IPs
    ...
    : FOR    ${vm}    IN    @{vm_list}
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova floating-ip-create ${external_net} | grep "${external_net}"    30s
    \    Log    ${output}
    \    @{output_words}    Split String    ${output}
    \    Append To List    ${ip_list}    @{output_words}[3]
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova floating-ip-associate ${vm} @{output_words}[3]    30s
    \    Log    ${output}
    [Return]    ${ip_list}

Verify Gateway Ips
    [Documentation]    Verifies the Gateway Ips with dump flow.
    ${output}=    OpenStackOperations.Write Commands Until Prompt    sudo ovs-ofctl -O OpenFlow13 dump-flows br-int
    Log    ${output}
...
Delete SubNet
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v subnet-delete ${subnet}
    SSHLibrary.Close Connection
    Log    ${output}
...
Delete Vm Instance
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova force-delete ${vm_name}    40s
    SSHLibrary.Close Connection
...
Get Net Id
    ...
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron net-list | grep "${network_name} " | awk '{print $2}'    30s
    Log    ${output}
...
Get Subnet Id
    ...
    [Documentation]    Retrieve the net id for the given network name to create specific vm instance
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron subnet-list | grep "${subnet_name} " | awk '{print $2}'    30s
    Log    ${output}
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
...
Get Port Id
    ...
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron port-list | grep "${port_name} " | awk '{print $2}'    30s
    Log    ${output}
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
...
Get Router Id
    ...
    [Documentation]    Retrieve the net id for the given network name to create specific vm instance
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron router-list | grep "${router1} " | awk '{print $2}'    30s
    Log    ${output}
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
...
Get VM Instance
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output} =    OpenStackOperations.Write Commands Until Prompt    nova show ${vm_name} | grep 'instance_name' | awk '{ print $4}'    30s
    SSHLibrary.Close Connection
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
    ${instance_name}=    Get from List    ${splitted_output}    0
...
Get VM IP
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output} =    OpenStackOperations.Write Commands Until Prompt    nova show ${vm_name} | grep "network" | awk '{print $5,$6}'    30s
    SSHLibrary.Close Connection
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
...
Get BgpVpn Id
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron bgpvpn-show ${vpnName} | grep " id" | awk '{print $4}'    30s
    SSHLibrary.Close Connection
    Log    ${output}
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
...
Create Vm Instances
    ...
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova boot --image ${image} --flavor ${flavor} --nic net-id=${net_id} ${VmElement} --security-groups ${sg}    30s
    \    Log    ${output}

Create Vm Instance With Port On Compute Node
    [Arguments]    ${port_name}    ${vm_instance_name}    ${compute_node}    ${image}=cirros-0.3.4-x86_64-uec    ${flavor}=m1.nano    ${sg}=default    ${additional_args}=${EMPTY}
    [Documentation]    Create One VM instance using given ${port_name} and for given ${compute_node}
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${port_id}=    OpenStackOperations.Get Port Id    ${port_name}    ${devstack_conn_id}
    ${hostname_compute_node}=    Run Command On Remote System    ${compute_node}    hostname
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova boot --image ${image} --flavor ${flavor} --nic port-id=${port_id} ${additional_args} ${vm_instance_name} --security-groups ${sg} --availability-zone nova:${hostname_compute_node}    30s
    Log    ${output}
    Wait Until Keyword Succeeds    180s    10s    Verify VM Is ACTIVE    ${vm_instance_name}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova show ${vm_instance_name} | grep "instance_name" | awk '{print $4}'    30s
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
    ${instance_id}=    Get from List    ${splitted_output}    0
    Log    ${instance_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova show ${vm_instance_name} | grep "network" | awk '{print $5}'    30s
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
    ${VmIp}=    Get from List    ${splitted_output}    0
    Log    ${VmIp}
    [Return]    ${instance_id}    ${VmIp}
...
Verify VM Is ACTIVE
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova show ${vm_name} | grep OS-EXT-STS:vm_state    30s
    Log    ${output}
...
Verify VMs Received DHCP Lease
    ...
    ${ip_list}    Create List
    : FOR    ${vm}    IN    @{vm_list}
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova console-log ${vm} | grep -i "obtained"    30s
    \    ${dhcp_ip_line}=    OpenStackOperations.Write Commands Until Prompt    nova console-log ${vm} | grep "^nameserver"    30s
    \    Log    ${output}
    \    @{output_words}    Split String    ${output}
...
View Vm Console
    ...
    : FOR    ${VmElement}    IN    @{vm_instance_names}
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova show ${VmElement}
    \    Log    ${output}
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova console-log ${VmElement}
    \    Log    ${output}
...
Ping Vm From DHCP Namespace
    ...
    ${net_id}=    Get Net Id    ${net_name}    ${devstack_conn_id}
    Log    ${net_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    sudo ip netns exec qdhcp-${net_id} ping -c 3 ${vm_ip}    20s
    Log    ${output}
    SSHLibrary.Close Connection
...
Ping Vm From Control Node
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    ping -c 3 ${vm_floating_ip}    20s
    Log    ${output}
    SSHLibrary.Close Connection
    Should Contain    ${output}    64 bytes
...
Curl Metadata Server
    ...
    OpenStackOperations.Write Commands Until Prompt    exit
    Should Contain    ${output}    200

Close Vm Instance
    [Documentation]    Exit the vm instance.
    ${output}=    OpenStackOperations.Write Commands Until Prompt    exit
    Log    ${output}
...
Exit From Vm Console
    ...
    Run Keyword If    ${rcode}    OpenStackOperations.Write Commands Until Prompt    exit
    Get OvsDebugInfo
...
Create Router
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v router-create ${router_name}    30s
    SSHLibrary.Close Connection
...
Add Router Interface
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v router-interface-add ${router_name} ${interface_name}
    SSHLibrary.Close Connection
...
Add Router Gateway
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v router-gateway-set ${router_name} ${network_name}
    SSHLibrary.Close Connection
...
Remove Interface
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v router-interface-delete ${router_name} ${interface_name}
    SSHLibrary.Close Connection
...
Update Router
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output} =    OpenStackOperations.Write Commands Until Prompt    neutron router-update ${router_name} ${cmd}    30s
    SSHLibrary.Close Connection
...
Show Router
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output} =    OpenStackOperations.Write Commands Until Prompt    neutron router-show ${router_name} ${options}    30s
    Log    ${output}
    ${splitted_output}=    Split String    ${output}    ${EMPTY}
    ${net_id}=    Get from List    ${splitted_output}    0
    Log    ${net_id}
...
Delete Router
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v router-delete ${router_name}    60s
    SSHLibrary.Close Connection
...
Get ControlNode Connection
    ...
    Utils.Flexible SSH Login    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    30s
...
Show Debugs
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    sudo ip netns list
    Log    ${output}
    : FOR    ${index}    IN    @{vm_indices}
    \    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova show ${index}    30s
    \    Log    ${output}
    SSHLibrary.Close Connection
    List Networks
    List Subnets
...
Create Security Group
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    nova secgroup-create ${sg_name} ${desc}    40s
    SSHLibrary.Close Connection
...
Create Security Rule
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron security-group-rule-create --direction ${direction} --protocol ${protocol} --port-range-min ${min_port} --port-range-max ${max_port} --remote-ip-prefix ${remote_ip} ${sg_name}
    SSHLibrary.Close Connection
...
Neutron Security Group Show
    ...
    Log    ${cmd}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    ${cmd}    30s
    Log    ${output}
    SSHLibrary.Close Connection
    [Return]    ${output}
...
Create Bgpvpn
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v bgpvpn-create --name ${vpnname} ${additional_args}    30s
    SSHLibrary.Close Connection
    Log    ${output}
...
Bgpvpn Net Associate
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron bgpvpn-net-assoc-create --network ${network_name} ${vpnname}
    SSHLibrary.Close Connection
    Log    ${output}
...
Bgpvpn Router Associate
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron bgpvpn-router-assoc-create --router ${router_name} ${vpnname}
    SSHLibrary.Close Connection
    Log    ${output}
...
Delete Bgpvpn
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v bgpvpn-delete --name ${vpnname}    30s
    SSHLibrary.Close Connection
    Log    ${output}
    Should Contain    ${output}    Deleted bgpvpn
#    Should Contain    ${output}    Created a new bgpvpn
...
Bgpvpn Net DisAssociate
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron bgpvpn-net-assoc-delete ${AssociationID} ${vpnname}
    SSHLibrary.Close Connection
    Log    ${output}
...
Bgpvpn Router DisAssociate
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron bgpvpn-router-assoc-delete ${AssociationID} ${vpnname}
    SSHLibrary.Close Connection
    Log    ${output}
...
Bgpvpn Update
    ...
    ${devstack_conn_id}=    Get ControlNode Connection
    SSHLibrary.Switch Connection    ${devstack_conn_id}
    ${output}=    OpenStackOperations.Write Commands Until Prompt    neutron -v bgpvpn-update ${vpnname} ${additional_args}    30s
    SSHLibrary.Close Connection
    Log    ${output}

Execute Command on server
    [Arguments]    ${cmd}    ${vm_id}=${empty}    ${vm_instance}=${empty}    ${compute_ip}=${empty}    ${user}=cirros    ${password}=cubswin:)
    [Documentation]    Login to the vm instance using console, executes a command inside the VM and returns the otuput.
    ${vm_instance}=    Run keyword if    not "${vm_instance}"     Get VM Instance    ${vm_id}
    ...    ELSE    Set Variable    ${vm_instance}
    ${compute_ip}=    Run keyword if    not "${compute_ip}"     Get Compute IP for Server Instance    ${vm_instance}
    ...    ELSE    Set Variable    ${compute_ip}
    ${current_ssh_connection}=    SSHLibrary.Get Connection
    ${conn_id}=    SSHLibrary.Open Connection    ${compute_ip}    prompt=#    timeout=${DEFAULT_TIMEOUT}
    Flexible SSH Login    root    admin123
    ${output} =    Write Commands Until Expected Prompt    virsh console ${vm_instance} --force    ]
    ${output} =    Write Commands Until Expected Prompt    ${empty}    ogin:
    ${output} =    Write Commands Until Expected Prompt    ${user}    assword:
    ${output} =    Write Commands Until Expected Prompt    ${password}    ${DEFAULT_LINUX_PROMPT}
    ${output} =    Write Commands Until Expected Prompt    ${cmd}    ${DEFAULT_LINUX_PROMPT}

    [Teardown]    Run keywords    SSHLibrary.Write    exit
    ...    AND    SSHLibrary.Close connection
    #[Return]    ${output.splitlines()[:-1]}
    [Return]    ${output}

Verify Telnet From Server
    [Arguments]    ${IP}    ${vm_id}=${empty}    ${vm_instance}=${empty}    ${compute_ip}=${empty}    ${user}=cirros    ${password}=cubswin:)    ${uname}=redback    ${pword}=bete123
    [Documentation]    Login to the vm instance using console, and login to given IP by telnet from inside the VM and returns the response
    ${vm_instance}=    Run keyword if    not "${vm_instance}"     Get VM Instance    ${vm_id}
    ...    ELSE    Set Variable    ${vm_instance}
    ${compute_ip}=    Run keyword if    not "${compute_ip}"     Get Compute IP for Server Instance    ${vm_instance}
    ...    ELSE    Set Variable    ${compute_ip}
    ...
    ${output} =    Write Commands Until Expected Prompt    ${empty}    ogin:
    ${output} =    Write Commands Until Expected Prompt    ${user}    assword:
    ${output} =    Write Commands Until Expected Prompt    ${password}    ${DOLLER_PROMPT}
    ${Result}    ${output}    Run Keyword And Ignore Error    Write Commands Until Expected Prompt    telnet ${IP}    ogin:
    ${output}    Run Keyword If    '${Result}'=='PASS'    Write Commands Until Expected Prompt    ${uname}    assword:    ELSE    Run Keyword And Return    Set Variable    1
    ${output} =    Write Commands Until Expected Prompt    ${pword}    ${HASH_PROMPT}
    ${resp}    Run Keyword And Ignore Error    Should Match Regexp    ${output}    local
    ${output} =    Write Commands Until Expected Prompt    exit    ${DOLLER_PROMPT}
...
Neutron Security Group Rule Delete
    ...
    Log    ${cmd}
    ${output}=    DevstackUtils.Write Commands Until Prompt    ${cmd}    30s
    Log    ${output}

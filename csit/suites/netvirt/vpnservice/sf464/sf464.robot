*** Settings ***
Documentation     Test suite for ECMP base and bfd  with two dc-gw failover detection 
...
*** Testcases ***
TC_01 Verify Traffic from DC-GW is successfully sent to both CSSs in dual stack dual dc-gw 
    [Documentation]    Verify Traffic from DC-GW is successfully sent on both CSSs hosting ECMP VMs
    ...
    BfdOperations.Create BFD Configuration On ODL   $localTx   $localRx   $multiplier 
    BfdOperations.AddNeighbor To BFD Configuration On ODL    neighborAddr=${DCGW_SYSTEM_IP} $remoteRx   $remoteTx
    BfdOperations.Verify BFD Neighbor UP Status On DCGW    ${DCGW_SYSTEM_IP}    ${ODL_SYSTEM_IP}

    Log    Verify Traffic from DC-GW is successfully when the other CSS ECMP candidate VMs are deleted and also one dcgw down.
    Log    Verify the ECMP flow in all Compute Nodes
    ${GROUP_ID_1}    Verify flows in Compute Node    ${OS_COMPUTE_1_IP}    ${BUCKET_COUNTS[2]}    ${BUCKET_COUNTS[2]}    ${ALLOWED_IP[0]}
    ${GROUP_ID_2}    Verify flows in Compute Node    ${OS_COMPUTE_2_IP}    ${BUCKET_COUNTS[2]}    ${BUCKET_COUNTS[2]}    ${ALLOWED_IP[0]}
    Should be Equal As Strings    ${GROUP_ID_1}    ${GROUP_ID_2}

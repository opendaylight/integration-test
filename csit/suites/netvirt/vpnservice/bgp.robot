...
Verify BGP Neighbor Status
    ...
    BgpOperations.Verify BGP Neighbor UP Status On DCGW    ${DCGW_SYSTEM_IP}    ${ODL_SYSTEM_IP}
    #    BuiltIn.Log    ${output}
...
Verify Route Addition to DCGW
    [Documentation]    Verify route Addition to DCGW
    ${fib_values} =    BuiltIn.Create List    ${LOOPBACK_IP}    @{VM_IPS}
    BuiltIn.Wait Until Keyword Succeeds    60s    15s    Verify Routes On Quagga    ${DCGW_SYSTEM_IP}    ${DCGW_RD}    ${fib_values}

Verify Route Addition from DCGW to FIB
    [Documentation]    Verify Route Addition from DCGW to FIB
    ${fib_values} =    BuiltIn.Create List    ${LOOPBACK_IP}    @{VM_IPS}
    BuiltIn.Wait Until Keyword Succeeds    60s    15s    Utils.Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${DCGW_RD}/    ${fib_values}
// add flow check here

Verify External Route deletion in ODL
    [Documentation]    Verify External Route deletion in ODL
    BgpOperations.Delete Loopback Interface On DCGW    ${DCGW_SYSTEM_IP}    lo   
    BuiltIn.Log to console    Get FIB show output from KARAF CLI of ODL
    ${fib_values} =    Issue_Command_On_Karaf_Console    ${FIB_SHOW}    ${controller}
    BuiltIn.Wait Until Keyword Succeeds    60s    15s        BuiltIn.Should Not Contain    ${fib_values}    ${ LOOPBACK_IP }
    // add flow check here


Verify Route Deletion in DC-GW
    [Documentation]    Verify Route Deletion in DC-GW
    ${router_id}=    OpenStackOperations.Get Router Id    ${ROUTER}    ${devstack_conn_id}
    VpnOperations.Dissociate VPN to Router    routerid=${router_id}    vpnid=@{VPN_INSTANCE_IDS}[0]
    ${output} =    Execute Show Command On quagga    ${dcgw_ip}    show ip bgp vrf ${DCGW_RD}
    BuiltIn.Should Not Contain    ${output}    ${VM_IPS}

...
Create Neutron Subnets
    ...
    OpenStackOperations.Create SubNet    network_name=@{NETWORKS}[0]    subnet=@{SUBNETS4}[0]    range_ip=@{SUBNETS4_CIDR}[0]
    ${net1_additional_args} =    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac
    OpenStackOperations.Create SubNet    network_name=@{NETWORKS}[0]    subnet=@{SUBNETS6}[0]    range_ip=@{SUBNETS6_CIDR}[0]    additional_args=${net1_additional_args}
    OpenStackOperations.Create SubNet    network_name=@{NETWORKS}[1]    subnet=@{SUBNETS4}[1]    range_ip=@{SUBNETS4_CIDR}[1]
    ${net2_additional_args} =    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac
    OpenStackOperations.Create SubNet    network_name=@{NETWORKS}[1]    subnet=@{SUBNETS6}[1]    range_ip=@{SUBNETS6_CIDR}[1]    additional_args=${net2_additional_args}
...
Create Router
    ...
    BuiltIn.Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${ROUTER_URL}    ${ROUTER}

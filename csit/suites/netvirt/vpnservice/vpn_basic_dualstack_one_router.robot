...
Suite Teardown    VpnOperations.Basic Vpnservice Suite Cleanup    ${vpn_instance_ids}=@{VPN_INSTANCE_ID}
...
Delete And Recreate Extra Route
    ...
    OpenStackOperations.Update Router    ${ROUTER}    ${RT_CLEAR}
    [Teardown]    OpenStackOperations.Update Router    ${ROUTER}    ${RT_CLEAR}
    ...    AND    OpenStackOperations.Get Test Teardown Debugs

Create L3VPN
    ${net_id} =    OpenStackOperations.Get Net Id    @{NETWORKS}[0]
    ${tenant_id} =    OpenStackOperations.Get Tenant ID From Network    ${net_id}
    VpnOperations.VPN Create L3VPN    ${VPN_INSTANCE_ID[0]}    ${VPN_NAME[0]}    @{RDS}[0]    @{RDS}[0]    @{RDS}[0]    ${tenant_id}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
...
Associate L3VPN To Routers
    ...
    VpnOperations.Associate VPN to Router    ${router_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
...
Dissociate L3VPN From Routers
    ...
    ${router_id} =    OpenStackOperations.Get Router Id    ${ROUTER}
    VpnOperations.Dissociate VPN to Router    ${router_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
...
Associate L3VPN To Networks
    ...
    VpnOperations.Associate L3VPN To Network    ${network1_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
    BuiltIn.Should Contain    ${resp}    ${network1_id}
    VpnOperations.Associate L3VPN To Network    ${network2_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
...
Dissociate L3VPN From Networks
    ...
    ${network1_id} =    OpenStackOperations.Get Net Id    @{NETWORKS}[0]
    ${network2_id} =    OpenStackOperations.Get Net Id    @{NETWORKS}[1]
    VpnOperations.Dissociate L3VPN From Networks    ${network1_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
    BuiltIn.Should Not Contain    ${resp}    ${network1_id}
    VpnOperations.Dissociate L3VPN From Networks    ${network2_id}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VpnOperations.VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
    BuiltIn.Should Not Contain    ${resp}    ${network2_id}

Delete L3VPN
    [Documentation]    Delete L3VPN.
    VpnOperations.VPN Delete L3VPN    ${VPN_INSTANCE_ID[0]}
...
Create Multiple L3VPN
    ...
    ${tenant_id} =    OpenStackOperations.Get Tenant ID From Network    ${net_id}
    VpnOperations.VPN Create L3VPN    ${VPN_INSTANCE_ID[0]}    ${VPN_NAME[0]}    ${RDS[0]}    ${RDS[0]}    ${RDS[0]}    ${tenant_id}
    VpnOperations.VPN Create L3VPN    ${VPN_INSTANCE_ID[1]}    ${VPN_NAME[1]}    ${RDS[1]}    ${RDS[1]}    ${RDS[1]}    ${tenant_id}
    VpnOperations.VPN Create L3VPN    ${VPN_INSTANCE_ID[2]}    ${VPN_NAME[2]}    ${RDS[2]}    ${RDS[2]}    ${RDS[2]}    ${tenant_id}
    ${resp} =    VPN Get L3VPN    ${VPN_INSTANCE_ID[0]}
    BuiltIn.Should Contain    ${resp}    ${VPN_INSTANCE_ID[0]}
    ${resp} =    VPN Get L3VPN    ${VPN_INSTANCE_ID[1]}
    BuiltIn.Should Contain    ${resp}    ${VPN_INSTANCE_ID[1]}
    ${resp} =    VPN Get L3VPN    ${VPN_INSTANCE_ID[2]}

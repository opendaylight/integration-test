...
Create Nova VMs
    ...
    ${NET_1_VM_IPV6}=    Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=${NET_1_VM_INSTANCES}    network=@{NETWORKS}[0]    subnet=${prefix_net10}
    ${NET_2_VM_IPV6}=    Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=${NET_2_VM_INSTANCES}    network=@{NETWORKS}[1]    subnet=${prefix_net20}
...
Verify GWMAC Flow Entry On Flow Table
    ...
    BuiltIn.Log    ${arpResponder_table}

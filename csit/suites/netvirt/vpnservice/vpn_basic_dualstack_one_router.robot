...
Add Multiple Extra Routes And Check Datapath Before L3VPN Creation
    ...
    ${vm_instances} =    BuiltIn.Create List    @{EXTRA_NW_SUBNET_IPV4}    @{EXTRA_NW_SUBNET_IPV6}
    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    ${vm_instances}
...
Delete And Recreate Extra Route
    ...
    OpenStackOperations.Show Router    ${ROUTER}    -D
    BuiltIn.Log    Add and test an extra route IPv4
    ${CONFIG_EXTRA_ROUTE_IPV4}=    BuiltIn.Catenate    sudo ip addr add @{EXTRA_NW_IPV4}[0]/24 dev eth0
    ...
    BuiltIn.Should Contain    ${output}    64 bytes
    BuiltIn.Log    Add and test an extra route IPv6
    ${CONFIG_EXTRA_ROUTE_IPV6} =    BuiltIn.Catenate    sudo ip -6 addr add @{EXTRA_NW_IPV6}[0]/64 dev eth0
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${NET_1_VM_IPV6[0]}    ${CONFIG_EXTRA_ROUTE_IPV6}

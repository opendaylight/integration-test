...
Create Neutron Ports
    ...
    ${output} =    OpenStackOperations.Show Port    @{PORTS}[0]
...
Add Multiple Extra Routes And Check Datapath Before L3VPN Creation
    ...
    ${ext_rt1} =    BuiltIn.Set Variable    destination=40.1.1.0/24,gateway=@{NET_1_VM_IPS}[0]
    ${ext_rt2} =    BuiltIn.Set Variable    destination=50.1.1.0/24,gateway=@{NET_1_VM_IPS}[0]
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    ${ext_rt2}
...
Delete Router And Router Interfaces With L3VPN
    ...
    BuiltIn.Wait Until Keyword Succeeds    3s    1s    Check For Elements Not At URI    ${ROUTER_URL}    ${router_list}
    ${resp} =    VpnOperations.VPN Get L3VPN    vpnid=@{VPN_INSTANCE_ID}[0]
    BuiltIn.Should Not Contain    ${resp}    ${router_id}

...
@{FILES_PATH}     /home/stack/odl/karaf-0.8.2-SNAPSHOT/etc/opendaylight/datastore/initial/config/netvirt-vpnmanager-config.xml    /home/stack/odl/karaf-0.8.2-SNAPSHOT/etc/opendaylight/datastore/initial/config/netvirt-natservice-config.xml    /home/stack/odl/karaf-0.8.2-SNAPSHOT/etc/opendaylight/datastore/initial/config/netvirt-elanmanager-config.xml
...
VPN Create L3VPNs
    ...
    : FOR    ${index}    IN RANGE    0    2
    \    ${output} =    VPN Get L3VPN ID    @{DCGW_RD_IRT_ERT}[${index}]    @{VPN_ID}[${index}]
...
Create Nova VMs
    ...
    #\    Create Vm Instance With Port On Compute Node    @{PORT_LIST}[${index}]    @{VM_LIST}[${index}]    ${OS_CMP1_HOSTNAME}    sg=${SECURITY_GROUP}
    ...
    @{VM_LIST}    ${DHCP_IP} =    OpenStackOperations.Get VM IPs    @{VM_LIST}
...
Restart Karaf
    ...
    ${status} =    Utils.Run Command On Remote System    ${ODL_SYSTEM_IP}    ./odl/karaf-0.8.2-SNAPSHOT/bin/status
    Log    ${status}
    ${stop_msg} =    Utils.Run Command On Remote System    ${ODL_SYSTEM_IP}    ./odl/karaf-0.8.2-SNAPSHOT/bin/stop
    Log    ${stop_msg}
    Sleep    60s
    ${start_msg} =    Utils.Run Command On Remote System    ${ODL_SYSTEM_IP}    ./odl/karaf-0.8.2-SNAPSHOT/bin/start
    Log    ${start_msg}
...
VPN Get L3VPN ID
    [Arguments]    ${vrf_id}    ${vpn_instnt_id}
    ...
    @{list_any_matches} =    String.Get_Regexp_Matches    ${resp.content}    \"vpn-instance-name\":\"${vpn_instnt_id}\",.*"vrf-id":"${vrf_id}",\"vpn-id\":(\\d+)    1
...
Create Dictionary For DPN ID And Compute IP Mapping For 2 DPNS
    ...
    ${COMPUTE_1_DPNID} =    Get DPID    ${OS_COMPUTE_1_IP}
    BuiltIn.Set Suite Variable    ${COMPUTE_1_DPNID}
    ${COMPUTE_2_DPNID} =    Get DPID    ${OS_COMPUTE_2_IP}
    BuiltIn.Set Suite Variable    ${COMPUTE_2_DPNID}
    ${DPN_TO_COMPUTE_IP} =    BuiltIn.Create Dictionary
    Collections.Set To Dictionary    ${DPN_TO_COMPUTE_IP}    ${COMPUTE_1_DPNID}    ${OS_COMPUTE_1_IP}
    Collections.Set To Dictionary    ${DPN_TO_COMPUTE_IP}    ${COMPUTE_2_DPNID}    ${OS_COMPUTE_2_IP}
...
Get SNAT NAPT Switch DPIN ID
    ...
    ${output} =    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    curl -v -u admin:admin GET http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}${CONFIG_API}/odl-nat:napt-switches/router-to-napt-switch/${router_id}
...
Get Compute IP From DPIN ID
    ...
    ${dpnid} =    BuiltIn.Convert To Integer    ${dpnid}

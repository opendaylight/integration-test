...
Verify the subnet route punt path rate limiting is off, by bring up the destination IP host and by deleting the destination IP host brought in previous case
    ...
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    Verify Dump Flows For Specific Table     ${snat_napt_switch_ip}    @{OF_PUNT_TABLES}[${index}]   True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=${HARD_TIMEOUT_180}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}
    ${pkt_count_before}    Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE}    |grep "nw_dst=@{EXTRA_NW_IP}[1] actions=drop"
    Log    ${pkt_count_before}
    Should Be True    ${pkt_count_before} == 4
    Create Vm Instance With Port On Compute Node    @{EXTRA_PORTS}[1]    @{EXTRA_VMS}[1]    ${OS_CMP1_HOSTNAME}    sg=${SECURITY_GROUP}
    Sleep    30s
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    ${pkt_count_after}    Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE}    |grep "nw_dst=@{EXTRA_NW_IP}[1] actions=drop"
    Log    ${pkt_count_after}
    Should Be True    ${pkt_count_after} == 4
    Delete Vm Instance     @{EXTRA_VMS}[1]
    Sleep    30s
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    ${pkt_count}    Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE}    |grep "nw_dst=@{EXTRA_NW_IP}[1] actions=drop"
    Log    ${pkt_count}
    Should Be True    ${pkt_count} == 9
    [Teardown]    Set Default TimeOut In Xml
...
Verify the ARP request punt path for same destination from different source
    ...
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    True     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=@{EXTRA_NW_IP}[0]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}    True     ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_tpa=@{VM_IPS}[0]
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[1]    sudo arping -c 2 @{EXTRA_NW_IP}[0]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_tpa=@{EXTRA_NW_IP}[0]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}    True     ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_tpa=@{VM_IPS}[1]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ELAN_BASETABLE}    True     ${EMPTY}    arp,reg4=0x1/0xffff
...
Verify the ARP punt path for ARP Request from the VM should be punted to CSC and the reply from the subnet gateway shouldn’t be punted
    ...
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=11.1.1.1
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_spa=11.1.1.1,arp_tpa=@{VM_IPS}[0]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    arp_spa=11.1.1.1,arp_tpa=@{VM_IPS}[0]
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   False     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=11.1.1.1
    Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ELAN_BASETABLE}   True     ${EMPTY}    arp,reg4=0x101/0xffff
...
Verify the SNAT punt path from same source IP to different destination
    ...
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_dst=@{EXTRA_NW_IP}[2]    tp_dst=${TELNET_PORT} actions=drop
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_src=@{VM_IPS}[5]    actions=set_field:${EXT_SUBNETS_FIXED_IP}->ip_src
    ${pkt_count_before}    Get Packet Count From Table    ${napt_switch_ip}    ${INTEGRATION_BRIDGE}    ${SNAT_PUNT_TABLE}    |grep nw_src=@{VM_IPS}[5],tp_src=
    Log    ${pkt_count_before}
    Should Be True    ${pkt_count_before} > 0
    #Get packet count before
    #Get packet count after
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[2]    @{VM_IPS}[5]    telnet @{EXTRA_NW_IP}[3] &
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_dst=@{EXTRA_NW_IP}[3]    tp_dst=${TELNET_PORT} actions=drop
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_src=@{VM_IPS}[5]    actions=set_field:${EXT_SUBNETS_FIXED_IP}->ip_src
...
Verify the punt path with traffic for same destination from different source IP
    ...
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_dst=@{EXTRA_NW_IP}[2]    tp_dst=${TELNET_PORT} actions=drop
    Verify Dump Flows For Specific Table    ${napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_src=@{VM_IPS}[4]    actions=set_field:${EXT_SUBNETS_FIXED_IP}->ip_src
    ${pkt_count_before}    Get Packet Count From Table    ${napt_switch_ip}    ${INTEGRATION_BRIDGE}    ${SNAT_PUNT_TABLE}    |grep nw_src=@{VM_IPS}[5],tp_src=
    Log    ${pkt_count_before}
    Should Be True    ${pkt_count_before} > 0
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[2]    @{VM_IPS}[5]    telnet @{EXTRA_NW_IP}[2] &

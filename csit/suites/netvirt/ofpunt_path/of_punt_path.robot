...
Verify learnt flow for SNAT in table=46 for UDP
    ...
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${compute_ip}    ${INTEGRATION_BRIDGE}    table=${SNAT_PUNT_TABLE}    |grep "udp,metadata=${vpn_id_1}/0xfffffe actions=CONTROLLER:65535,learn(table=${SNAT_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}"
    ...
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${compute_ip}    ${INTEGRATION_BRIDGE}    table=${SNAT_PUNT_TABLE}    |grep "udp,metadata=${vpn_id_1}/0xfffffe actions=CONTROLLER:65535,learn(table=${SNAT_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}"
    #BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    #Verify Controller Flows In SNAT Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${NET_3_VM_IP_0}    ${EXT_SUBNETS_FIXED_IP}
    OVSDB.Verify Dump Flows For Specific Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    True    ${EMPTY}    actions=set_field:55.1.1.100->ip_src
    ${controller_packet_count} =    OvsManager.Get Packet Count From Table    ${compute_ip}    ${INTEGRATION_BRIDGE}    table=${SNAT_PUNT_TABLE}    |grep "udp,metadata=${vpn_id_1}/0xfffffe,nw_src=33.1.1.12"
...
VPN Create L3VPNs
    ...
    ${vpn_id_1} =    VPN Get L3VPN ID    @{VPN_ID}[0]
    ${vpn_id_2} =    VPN Get L3VPN ID    @{VPN_ID}[1]
...
VPN Get L3VPN ID
    ...
    @{list_any_matches} =    String.Get_Regexp_Matches    ${resp.content}    \"vpn-instance-name\":\"${vrf_id}\",.*"vrf-id":"${vrf_id}",\"vpn-id\":(\\d+)    1

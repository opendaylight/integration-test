...
Verify learnt flow for Subnet route in table=22
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    #${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE},    |grep "ip actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${L3_DEFAULT_HARD_TIMEOUT}"
    ...
    BuiltIn.Run Keyword And Ignore Error    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    BuiltIn.Run Keyword And Ignore Error    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP2_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    BuiltIn.Run Keyword And Ignore Error    OVSDB.Verify Dump Flows For Specific Table    ${OS_CNTL_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    ...
    ${count_after_traffic}    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE},    |grep "ip actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${L3_DEFAULT_HARD_TIMEOUT}"

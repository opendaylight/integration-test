...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_spa=@{VM_IPS}[3],arp_tpa=@{EXTRA_NW_IP}[1]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[1],arp_tpa=@{VM_IPS}[3]
    #${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp_spa=@{VM_IPS}[3],arp_tpa=@{EXTRA_NW_IP}[1]"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 1
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ...
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_spa=@{VM_IPS}[3].*arp_tpa=@{EXTRA_NW_IP}[1].*actions=load:0x1"
    ...
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
...
Verify learnt flow in table=195 and table=196 for ARP reply
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_tpa=@{EXTRA_NW_IP}[0]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_tpa=@{VM_IPS}[1]
    #${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp_spa=@{VM_IPS}[1],arp_tpa=@{EXTRA_NW_IP}[0]"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 1
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ...
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_spa=@{VM_IPS}[1].*arp_tpa=@{EXTRA_NW_IP}[0].*actions=load:0x1"
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    #${count_after_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp}

...
Suite Setup       Create Common Suite Setup
Suite Teardown    OpenStackOperations.OpenStack Cleanup All
...
@{VALID_TIMEOUT}    20    30    100    1000    10000
...
Set punt timeout to combination of valid ranges and verfiy flows
    ...
    : FOR    ${index}    IN RANGE    0    3
    \    Change Hard Timeout Value In XML File    @{FILES_PATH}[${index}]    @{DEFAULT_HARD_TIMEOUT}[${index}]    @{VALID_TIMEOUT}[0]
    \    Verify Punt Values In XML File    @{FILES_PATH}[${index}]    @{VALID_TIMEOUT}[0]
    ${count} =    BuiltIn.Get length    ${VALID_TIMEOUT}
    : FOR    ${index}    IN RANGE    1    ${count}
    \    Change Hard Timeout Value In XML File    @{FILES_PATH}[0]    @{VALID_TIMEOUT}[${index - 1}]    @{VALID_TIMEOUT}[${index}]
    \    Verify Punt Values In XML File    @{FILES_PATH}[0]    @{VALID_TIMEOUT}[${index}]
    \    Change Hard Timeout Value In XML File    @{FILES_PATH}[1]    @{VALID_TIMEOUT}[${index - 1}]    @{VALID_TIMEOUT}[${index}]
    \    Verify Punt Values In XML File    @{FILES_PATH}[1]    @{VALID_TIMEOUT}[${index}]
    \    Change Hard Timeout Value In XML File    @{FILES_PATH}[2]    @{VALID_TIMEOUT}[${index - 1}]    @{VALID_TIMEOUT}[${index}]
    \    Verify Punt Values In XML File    @{FILES_PATH}[2]    @{VALID_TIMEOUT}[${index}]
    \    Restart Karaf Using Karaf Shell File
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    \    ${snat_napt_switch_ip} =    Get NAPT Switch IP From DPID    @{ROUTERS}[1]
    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}
    \    ...    True    ${EMPTY}    learn(table=${L3_PUNT_TABLE},hard_timeout=@{VALID_TIMEOUT}[${index}]
    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE}
    \    ...    True    ${EMPTY}    learn(table=${ARP_PUNT_TABLE},hard_timeout=@{VALID_TIMEOUT}[${index}]
    ...
    \    ...    True    ${EMPTY}    learn(table=${SNAT_PUNT_TABLE},hard_timeout=@{VALID_TIMEOUT}[${index}]
    Set Default TimeOut In Xml    @{VALID_TIMEOUT}[4]

*** Keywords ***
Create Common Suite Setup
    [Documentation]    Create common setup related SF441 openflow punt path

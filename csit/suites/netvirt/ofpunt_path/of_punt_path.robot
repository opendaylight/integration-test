...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=22.1.1.101,arp_op=1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_spa=22.1.1.101,arp_op=1
    ...
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 3
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_tpa=22.1.1.101,arp_op=1.*actions=load:0x1"
...
Verify learnt flow in table=195 and table=196 for ARP reply
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=11.1.1.101,arp_op=2
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_spa=11.1.1.101,arp_op=2
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp actions=CONTROLLER:65535,learn(table=${ARP_PUNT_TABLE_1},hard_timeout=${HARD_TIMEOUT_180}"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 2
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_tpa=11.1.1.101,arp_op=2.*actions=load:0x1"
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp}
...
Verify resync subnet, SNAT and ARP route flow table after disconnect and reconnecting CSC-CSS control path
    ...
    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=@{VM_IPS}[1],arp_op=1
    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_op=1
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_tpa=@{VM_IPS}[1],arp_op=1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   False    ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_op=1
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   False    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[1],arp_op=1
...
Verify resync subnet, SNAT and ARP flow table after CSS restart
    ...
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo arping -c 2 @{VM_IPS}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=@{VM_IPS}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_op=1
...
Verify the ARP request punt path for same destination from different source
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    True     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}    True     ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_op=1
...
Verify the ARP punt path for ARP Request from the VM should be punted to CSC and the reply from the subnet gateway shouldn’t be punted
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=11.1.1.1,arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_tpa=@{VM_IPS}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    arp_spa=11.1.1.1,arp_op=1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   False     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_op=1

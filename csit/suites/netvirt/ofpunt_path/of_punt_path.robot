...
Verify resync subnet, SNAT and ARP route flow table after disconnect and reconnecting CSC-CSS control path
    ...
    ${switch_ip} =     Get Compute IP From DPIN ID    @{ROUTERS}[1]
    Log    ${switch_ip}
    Restart Karaf
    Sleep    120s
    ${snat_napt_switch_ip} =     Get Compute IP From DPIN ID    @{ROUTERS}[1]
    BuiltIn.Set Suite Variable    ${snat_napt_switch_ip}
    BuiltIn.Run Keyword And Ignore Error    OVSDB.Check OVS OpenFlow Connections    ${snat_napt_switch_ip}    3
    #BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${snat_napt_switch_ip}    3
    : FOR    ${index}    IN RANGE    0    3
    ...
    ${switch_ip1} =     Get Compute IP From DPIN ID    @{ROUTERS}[1]
    Log    ${switch_ip1}
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    ...
    OVSDB.Verify Dump Flows For Specific Table   ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=@{VM_IPS}[1]
    OVSDB.Verify Dump Flows For Specific Table   ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_tpa=@{VM_IPS}[0]
    OVSDB.Delete OVS Manager    ${snat_napt_switch_ip}
    OVSDB.Delete OVS Controller    ${snat_napt_switch_ip}
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=@{VM_IPS}[1]
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_2}   False    ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_tpa=@{VM_IPS}[0]
    ...
    BuiltIn.Run Keyword And Ignore Error     OVSDB.Check OVS OpenFlow Connections    ${snat_napt_switch_ip}    2
    #BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${snat_napt_switch_ip}    2
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_spa=@{VM_IPS}[0],arp_tpa=@{EXTRA_NW_IP}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_2}   False    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[1],arp_tpa=@{VM_IPS}[0]
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   True     ${EMPTY}    learn(table=${L3_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    learn(table=${ARP_PUNT_TABLE_1},hard_timeout=${HARD_TIMEOUT_180}

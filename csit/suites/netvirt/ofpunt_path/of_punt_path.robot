...
Verify default hard timeout in XML file in ODL Controller and default flow in OVS for Subnet Route, SNAT and ARP 
    ...
    Verify Punt Values In XML File    ${SUBNET_FILE}    ${L3_DEFAULT_HARD_TIMEOUT}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${ARP_FILE}    ${ARP_DEFAULT_HARD_TIMEOUT}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${ARP_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${SNAT_FILE}    ${SNAT_DEFAULT_HARD_TIMEOUT}
    Verify Default Flows In Punt Table for SNAT    @{ROUTERS}[1]    ${SNAT_PUNT_TABLE}    ${SNAT_DEFAULT_HARD_TIMEOUT}
...
Verify No default flow for Subnet Route, SNAT and ARP after hard timeout is set to zero in XML file in ODL Controller
    ...
    Change Hard Timeout Value In XML File    ${SUBNET_FILE}    ${L3_DEFAULT_HARD_TIMEOUT}    ${HARD_TIMEOUT_VALUE_ZERO}
    Verify Punt Values In XML File    ${SUBNET_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}
    Change Hard Timeout Value In XML File    ${ARP_FILE}    ${ARP_DEFAULT_HARD_TIMEOUT}    ${HARD_TIMEOUT_VALUE_ZERO}
    Verify Punt Values In XML File    ${ARP_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}
    Change Hard Timeout Value In XML File    ${SNAT_FILE}    ${SNAT_DEFAULT_HARD_TIMEOUT}    ${HARD_TIMEOUT_VALUE_ZERO}
    Verify Punt Values In XML File    ${SNAT_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}  
    Restart Karaf
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify No Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    Change Hard Timeout Value In XML File    ${SUBNET_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}    ${L3_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${SUBNET_FILE}    ${L3_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify No Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${ARP_DEFAULT_HARD_TIMEOUT}
    Change Hard Timeout Value In XML File    ${ARP_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}    ${ARP_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${ARP_FILE}    ${ARP_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify No Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${SNAT_PUNT_TABLE}    ${SNAT_DEFAULT_HARD_TIMEOUT}
    Change Hard Timeout Value In XML File    ${SNAT_FILE}    ${HARD_TIMEOUT_VALUE_ZERO}    ${SNAT_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${SNAT_FILE}    ${SNAT_DEFAULT_HARD_TIMEOUT}
    Restart Karaf
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${ARP_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table for SNAT    @{ROUTERS}[1]    ${SNAT_PUNT_TABLE}    ${SNAT_DEFAULT_HARD_TIMEOUT}
...
Verify default flow for Subnet Route and ARP after changing hard timeout to different valus in XML file in ODL Controller
    ...
    Change Hard Timeout Value In XML File    ${SNAT_FILE}    ${SNAT_DEFAULT_HARD_TIMEOUT}    @{HARD_TIMEOUT_VALUES}[0]
    Verify Punt Values In XML File    ${SNAT_FILE}    @{HARD_TIMEOUT_VALUES}[0] 
    Change Hard Timeout Value In XML File    ${SUBNET_FILE}    ${L3_DEFAULT_HARD_TIMEOUT}    @{HARD_TIMEOUT_VALUES}[0]
    Verify Punt Values In XML File    ${SUBNET_FILE}    @{HARD_TIMEOUT_VALUES}[0]
    Change Hard Timeout Value In XML File    ${ARP_FILE}    ${ARP_DEFAULT_HARD_TIMEOUT}    @{HARD_TIMEOUT_VALUES}[0]
    Verify Punt Values In XML File    ${ARP_FILE}    @{HARD_TIMEOUT_VALUES}[0] 
    Restart Karaf
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    @{HARD_TIMEOUT_VALUES}[0]
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    @{HARD_TIMEOUT_VALUES}[0]
    BuiltIn.Wait Until Keyword Succeeds    180s    5s    Verify Default Flows In Punt Table for SNAT    @{ROUTERS}[1]    ${SNAT_PUNT_TABLE}    @{HARD_TIMEOUT_VALUES}[0]
    : FOR    ${index}    IN RANGE    1    ${cnt}
    \    Change Hard Timeout Value In XML File    ${SUBNET_FILE}    @{HARD_TIMEOUT_VALUES}[${index - 1}]   @{HARD_TIMEOUT_VALUES}[${index}]
    \    Verify Punt Values In XML File    ${SUBNET_FILE}    @{HARD_TIMEOUT_VALUES}[${index}]  
    \    Change Hard Timeout Value In XML File    ${ARP_FILE}    @{HARD_TIMEOUT_VALUES}[${index - 1}]   @{HARD_TIMEOUT_VALUES}[${index}]
    \    Verify Punt Values In XML File    ${ARP_FILE}    @{HARD_TIMEOUT_VALUES}[${index}] 
    \    Change Hard Timeout Value In XML File    ${SNAT_FILE}    @{HARD_TIMEOUT_VALUES}[${index - 1}]   @{HARD_TIMEOUT_VALUES}[${index}]
    \    Verify Punt Values In XML File    ${SNAT_FILE}    @{HARD_TIMEOUT_VALUES}[${index}]
    \    Restart Karaf
    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    @{HARD_TIMEOUT_VALUES}[${index}]
    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    @{HARD_TIMEOUT_VALUES}[${index}]
    \    BuiltIn.Wait Until Keyword Succeeds    180s    5s    Verify Default Flows In Punt Table for SNAT    @{ROUTERS}[1]    ${SNAT_PUNT_TABLE}    @{HARD_TIMEOUT_VALUES}[${index}]
    Change Hard Timeout Value In XML File    ${SUBNET_FILE}    @{HARD_TIMEOUT_VALUES}[4]    ${L3_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${SUBNET_FILE}    ${L3_DEFAULT_HARD_TIMEOUT}
    Change Hard Timeout Value In XML File    ${ARP_FILE}    @{HARD_TIMEOUT_VALUES}[4]    ${ARP_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${ARP_FILE}    ${ARP_DEFAULT_HARD_TIMEOUT}  
    Change Hard Timeout Value In XML File    ${SNAT_FILE}    @{HARD_TIMEOUT_VALUES}[4]    ${SNAT_DEFAULT_HARD_TIMEOUT}
    Verify Punt Values In XML File    ${SNAT_FILE}    ${SNAT_DEFAULT_HARD_TIMEOUT}  
    Restart Karaf
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    120s    5s    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${ARP_DEFAULT_HARD_TIMEOUT}
    BuiltIn.Wait Until Keyword Succeeds    180s    5s    Verify Default Flows In Punt Table for SNAT    @{ROUTERS}[1]    ${SNAT_PUNT_TABLE}    ${SNAT_DEFAULT_HARD_TIMEOUT}

Verify learnt flow for Subnet route in table=22
    [Documentation]    To verify the learnt flow in ovs for Subnet Route, after traffic is generated from a Vm to unknow IP in the different subnet.
    ${NET_2_VM_IP_0} =    Get VM Ip Addresses    @{NETWORKS}[1]    @{NET_2_VMS}[0]
    BuiltIn.Set Suite Variable    ${NET_2_VM_IP_0}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    ${NET_2_VM_IP_0}    ping @{EXTRA_NW_IP}[0] -c 4 -w 4
    Verify Learnt Flows In Subnet Punt Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}    @{EXTRA_NW_IP}[0]
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for Subnet    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}    @{EXTRA_NW_IP}[0]
    BuiltIn.Should be true    ${learnt_packet_count} > 1
    ${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}

Verify learnt flow for SNAT in table=46 for UDP
    [Documentation]    To verify the learnt flow in ovs for SNAT, after traffic is generated from a Vm to unknow IP in the external subnet..
    Create Dictionary For DPN ID And Compute IP Mapping For 2 DPNS
    ${NET_3_VM_IP_0} =    Get VM Ip Addresses    @{NETWORKS}[2]    @{NET_3_VMS}[0]
    BuiltIn.Set Suite Variable    ${NET_3_VM_IP_0}
    ${compute_ip}    Get Compute IP From DPIN ID    @{ROUTERS}[1]
    Verify Default Flows In Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${UDP}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[2]    ${NET_3_VM_IP_0}    traceroute @{EXTRA_NW_IP}[2] -w 1 -q 1 -m 4
    Verify Learnt Flows In SNAT Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_3_VM_IP_0}    @{EXTRA_NW_IP}[2]
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${UDP}    ${NET_3_VM_IP_0}    @{EXTRA_NW_IP}[2]
    BuiltIn.Should be true    ${learnt_packet_count} == 0
    ${count_after_traffic}    Get Packet Count from Default Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${UDP}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    Verify Controller Flows In SNAT Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${NET_3_VM_IP_0}    ${EXT_SUBNETS_FIXED_IP}
    ${controller_packet_count}    Get Packet Count from Controller Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${UDP}    ${NET_3_VM_IP_0}    ${EXT_SUBNETS_FIXED_IP}
    BuiltIn.Should be true    ${controller_packet_count} > 1

Verify learnt flow for SNAT in table=46 for TCP
    [Documentation]    To verify the learnt flow in ovs for SNAT, after traffic is generated from a Vm to unknow IP in the external subnet..
    Create Dictionary For DPN ID And Compute IP Mapping For 2 DPNS
    ${NET_3_VM_IP_0} =    Get VM Ip Addresses    @{NETWORKS}[2]    @{NET_3_VMS}[0]
    BuiltIn.Set Suite Variable    ${NET_3_VM_IP_0}
    ${compute_ip}    Get Compute IP From DPIN ID    @{ROUTERS}[1]
    Verify Default Flows In Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${TCP}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[2]    ${NET_3_VM_IP_0}    wget -qc http://@{EXTRA_NW_IP}[2]/ &
    Verify Learnt Flows In SNAT Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_3_VM_IP_0}    @{EXTRA_NW_IP}[2]
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${TCP}    ${NET_3_VM_IP_0}    @{EXTRA_NW_IP}[2]
    BuiltIn.Should be true    ${learnt_packet_count} == 0
    ${count_after_traffic}    Get Packet Count from Default Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${TCP}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    Verify Controller Flows In SNAT Punt Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${NET_3_VM_IP_0}    ${EXT_SUBNETS_FIXED_IP}
    ${controller_packet_count}    Get Packet Count from Controller Flow for SNAT    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${TCP}    ${NET_3_VM_IP_0}    ${EXT_SUBNETS_FIXED_IP}
    BuiltIn.Should be true    ${controller_packet_count} > 1
...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    ${NET_2_VM_IP_1} =    Get VM Ip Addresses    @{NETWORKS}[1]    @{NET_2_VMS}[1]
    BuiltIn.Set Suite Variable    ${NET_2_VM_IP_1}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    ${NET_2_VM_IP_1}    sudo arping -c 5 -I eth0 @{EXTRA_NW_IP}[1] &
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_2_VM_IP_1}    @{EXTRA_NW_IP}[1]
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_2}    ${L3_DEFAULT_HARD_TIMEOUT}    @{EXTRA_NW_IP}[1]    ${NET_2_VM_IP_1}
    ${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for ARP    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${NET_2_VM_IP_1}    @{EXTRA_NW_IP}[1]
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    ${count_after_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
...
Verify learnt flow in table=195 and table=196 for ARP reply
    [Documentation]    To verify the learnt flow in ovs for ARP request, after traffic is generated from a Vm to unknow IP destination using arping..
    ${NET_1_VM_IP_1} =    Get VM Ip Addresses    @{NETWORKS}[0]    @{NET_1_VMS}[1]
    BuiltIn.Set Suite Variable    ${NET_1_VM_IP_1}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    ${NET_1_VM_IP_1}    sudo arping -A -c 5 -I eth0 -s ${NET_1_VM_IP_1} @{EXTRA_NW_IP}[0] &
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_1_VM_IP_1}    @{EXTRA_NW_IP}[0]
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_2}    ${L3_DEFAULT_HARD_TIMEOUT}    @{EXTRA_NW_IP}[0]    ${NET_1_VM_IP_1}
    ${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for ARP    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${NET_1_VM_IP_1}    @{EXTRA_NW_IP}[0]
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    ${count_after_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp} 
...
Verify learnt flow in table=195 and table=196 for GARP
    [Documentation]    To verify the learnt flow in ovs for ARP request, after traffic is generated from a Vm to unknow IP destination using arping..
    ${NET_1_VM_IP_0} =    Get VM Ip Addresses    @{NETWORKS}[0]    @{NET_1_VMS}[0]
    BuiltIn.Set Suite Variable    ${NET_1_VM_IP_0}
    Verify Default Flows In Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}
    ${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}
    ${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_1_IP}    ${ELAN_BASETABLE}    ${GARP_REG}
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    ${NET_1_VM_IP_0}    sudo arping -U -c 5 -I eth0 ${NET_1_VM_IP_0} &
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_1_VM_IP_0}    ${NET_1_VM_IP_0}
    Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_2}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_1_VM_IP_0}    ${NET_1_VM_IP_0}
    ${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic + 1}
    ${learnt_packet_count}    Get Packet Count from Learnt Flow for ARP    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${NET_1_VM_IP_0}    ${NET_1_VM_IP_0}
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    ${count_after_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_1_IP}    ${ELAN_BASETABLE}    ${GARP_REG}
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp} 
...
Verify the subnet route punt path rate limiting is off, by bring up the destination IP host and by deleting the destination IP host brought in previous case
    ...
    Sleep    30s
    OpenStackOperations.Get VM IPs    @{EXTRA_VMS}[1]
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    Sleep    20s
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
...
Verify the ARP request punt path for same destination from different source
    ...
    : FOR    ${index}    IN RANGE    0    3
    \    Change Hard Timeout Value In XML File    @{FILES_PATH}[${index}]    @{DEFAULT_HARD_TIMEOUT}[${index}]    ${HARD_TIMEOUT_10}
    \    Verify Punt Values In XML File    @{FILES_PATH}[${index}]    ${HARD_TIMEOUT_10}
    Restart Karaf
    Sleep    120s
    ${snat_napt_switch_ip} =     Get Compute IP From DPIN ID    @{ROUTERS}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    : FOR    ${index}    IN RANGE    0    3
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    Verify Dump Flows For Specific Table     ${snat_napt_switch_ip}    @{OF_PUNT_TABLES}[${index}]   True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=${HARD_TIMEOUT_10}
    ...
    [Teardown]    Set Default TimeOut In Xml     ${HARD_TIMEOUT_10}
...
Set Default TimeOut In Xml
    [Arguments]    ${hard_timeout}=${HARD_TIMEOUT_180}

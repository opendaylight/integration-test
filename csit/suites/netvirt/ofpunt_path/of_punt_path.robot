...
Verify No default flow for subnet route, SNAT and ARP after hard timeout is set to zero in XML file in ODL Controller
    ...
    [Teardown]    Set Default TimeOut In Xml    ${HARD_TIMEOUT_VALUE_ZERO}
...
Create Neutron Ports
    ...
    ${NETWORKS_ALL} =    BuiltIn.Combine Lists    ${NETWORKS}    ${ext_net}
    : FOR    ${index}    IN RANGE    0    3
...
Get SNAT NAPT Switch DPID
    ...
    ${output} =    Utils.Run Command On Remote System    ${ODL_SYSTEM_1_IP}    curl -v -u ${ODL_RESTCONF_USER}:${ODL_RESTCONF_PASSWORD} GET http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}${CONFIG_API}/odl-nat:napt-switches/router-to-napt-switch/${router_id}

...
#    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    @{OF_PUNT_TABLES}[${index}]   True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=@{DEFAULT_HARD_TIMEOUT}[${index}]
#    
...
#    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
#    \    ${snat_napt_switch_ip} =     Get Compute IP From DPIN ID    @{ROUTERS}[1]
#    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${L3_PUNT_TABLE}   True    ${EMPTY}    learn(table=${L3_PUNT_TABLE},hard_timeout=@{HARD_TIMEOUT_VALUES}[${index}]
#    \    BuiltIn.Wait Until Keyword Succeeds    120s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}   True    ${EMPTY}    learn(table=${ARP_PUNT_TABLE_1},hard_timeout=@{HARD_TIMEOUT_VALUES}[${index}]
#    \    BuiltIn.Wait Until Keyword Succeeds    180s    5s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   True    ${EMPTY}    learn(table=${SNAT_PUNT_TABLE},hard_timeout=@{HARD_TIMEOUT_VALUES}[${index}]
#    : FOR    ${index}    IN RANGE    0    3
...
Verify learnt flow for Subnet route in table=22
    ...
    Sleep    120s
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE},    |grep "ip actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}"
    ...
    ${count_after_traffic}    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${L3_PUNT_TABLE},    |grep "ip actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}"
...
Verify learnt flow for SNAT in table=46 for UDP
    ...
    #OVSDB.Verify Dump Flows For Specific Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}
    OVSDB.Verify Dump Flows For Specific Table    ${compute_ip}    ${SNAT_PUNT_TABLE}    ${HARD_TIMEOUT_180}
    ...
    #${compute_ip}    ${SNAT_PUNT_TABLE}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_3_VM_IP_0}    @{EXTRA_NW_IP}[2]
...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    #OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${SNAT_PUNT_TABLE}    ${HARD_TIMEOUT_180}
    #${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp actions=CONTROLLER:65535,learn(table=${ARP_PUNT_TABLE_1},hard_timeout=${HARD_TIMEOUT_180}"
    #${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    ${count_before_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    @{VM_IPS}[3]    sudo arping -c 5 -I eth0 @{EXTRA_NW_IP}[1] &
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1.*actions=load:0x1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_tpa=@{VM_IPS}[3],arp_op=1.*actions=load:0x1
    #${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1.*actions=load:0x1"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 1
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ...
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1.*actions=load:0x1"
    ...
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
...
Verify learnt flow in table=195 and table=196 for ARP reply
    ...
    #${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    ${count_before_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[1]    sudo arping -A -c 5 -I eth0 -s @{VM_IPS}[1] @{EXTRA_NW_IP}[0] &
    #Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}    ${NET_2_VM_IP_1}    @{EXTRA_NW_IP}[0]
    #Verify Learnt Flows In ARP Punt Table    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_2}    ${L3_DEFAULT_HARD_TIMEOUT}    @{EXTRA_NW_IP}[0]    ${NET_2_VM_IP_1}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1.*actions=load:0x1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True    ${EMPTY}    arp_tpa=@{VM_IPS}[1],arp_op=1.*actions=load:0x1
    #${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1.*actions=load:0x1"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 1
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    #${learnt_packet_count}    Get Packet Count from Learnt Flow for ARP    ${OS_COMPUTE_2_IP}    ${ARP_PUNT_TABLE_1}    ${NET_2_VM_IP_1}    @{EXTRA_NW_IP}[0]
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1.*actions=load:0x1"
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    #${count_after_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_2_IP}    ${ELAN_BASETABLE}    ${ARP_REG}
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp}
...
Verify learnt flow in table=195 and table=196 for GARP
    ...
    #OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}    ${L3_DEFAULT_HARD_TIMEOUT}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    ${HARD_TIMEOUT_180}
    #${count_before_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "ip actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=${HARD_TIMEOUT_180}"
    #${count_before_traffic_arp}    Get Packet Count From Default Flow For ARP Or GARP    ${OS_COMPUTE_1_IP}    ${ELAN_BASETABLE}    ${GARP_REG}
    ${count_before_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${GARP_REG}.0xffff"
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   True     ${EMPTY}    table=${ARP_PUNT_TABLE_1}.*n_packets=[\0-9+].*arp_tpa=@{VM_IPS}[0],arp_op=1.*actions=load:0x1
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   True     ${EMPTY}    table=${ARP_PUNT_TABLE_2}.*n_packets=[\0-9+].*arp_tpa=@{VM_IPS}[0],arp_op=1.*actions=load:0x1
    #${count_after_traffic}    Get Packet Count from Default Flow    ${OS_COMPUTE_1_IP}    ${ARP_PUNT_TABLE_1}
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp_tpa=@{VM_IPS}[0],arp_op=1.*actions=load:0x1"
    ${count_before_traffic} =    BuiltIn.Evaluate     ${count_before_traffic} + 1
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}

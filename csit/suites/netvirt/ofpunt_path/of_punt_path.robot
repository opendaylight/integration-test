...
Verify resync subnet, SNAT and ARP route flow table after disconnect and reconnecting CSC-CSS control path
    ...
    Restart Karaf
    Sleep    120s
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CNTL_IP}    3
    : FOR    ${index}    IN RANGE    0    3
    ...
    OVSDB.Delete OVS Manager    ${OS_CNTL_IP}
    OVSDB.Delete OVS Controller    ${OS_CNTL_IP}
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    ...
    OVSDB.Set Controller In OVS Bridge    ${OS_CNTL_IP}    ${INTEGRATION_BRIDGE}    tcp:${ODL_SYSTEM_IP}:6653
    OVSDB.Set OVS Manager    ${OS_CNTL_IP}    ${ODL_SYSTEM_IP}
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CONTROL_NODE_IP}    2
...
Common Setup
    ...
    ${snat_napt_switch_ip_1} =     Get Compute IP From DPIN ID    @{ROUTERS}[0]
    BuiltIn.Set Suite Variable    ${snat_napt_switch_ip_1}

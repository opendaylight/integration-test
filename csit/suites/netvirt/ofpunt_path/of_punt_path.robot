...
Verify No default flow for subnet route, SNAT and ARP after hard timeout is set to zero in XML file in ODL Controller
    ...
    [Teardown]    Run Keywords    OpenStackOperations.Get Test Teardown Debugs    AND    Set Default TimeOut In Xml    ${0}
...
Verify default flow for subnet route and ARP after changing hard timeout to different values in XML file in ODL Controller
    ...
    ${cnt} =    BuiltIn.Get length    ${HARD_TIMEOUT_VALUES}
    ...
    [Teardown]    Run Keywords    OpenStackOperations.Get Test Teardown Debugs    AND    Set Default TimeOut In Xml    @{HARD_TIMEOUT_VALUES}[4]
...
Create Neutron Routers
    ...
    \    Collections.Append To List    @{ROUTER_ID}    ${router_id}
...
VPN Create L3VPNs
    ...
    VpnOperations.Associate VPN to Router    routerid=@{ROUTER_ID}[0]    vpnid=@{VPN_ID}[0]
...
Create Neutron Ports
    ...
    ${address_pair}    BuiltIn.Set Variable    --allowed-address ip-address=0.0.0.0

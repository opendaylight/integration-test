...
Verify resync subnet route, SNAT and ARP route flow tables after disconnect and reconnecting OVS control path
    ...
    ...    False    ${EMPTY}    tp_dst=${TCP_PORT} actions=drop
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE}
    ...    False    ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_LEARN_TABLE}
    ...    False    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[1],arp_op=1

...
Verify resync subnet, SNAT and ARP route flow table after disconnect and reconnecting CSC-CSS control path
    ...
    ...    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    ...
    BuiltIn.Wait Until Keyword Succeeds    190s    40s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}
    ...    ${SNAT_PUNT_TABLE}    False     ${EMPTY}    tp_dst=${TCP_PORT} actions=drop
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}
    ...    ${ARP_PUNT_TABLE_1}   False     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}
    ...    ${ARP_PUNT_TABLE_2}   False    ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[1],arp_op=1

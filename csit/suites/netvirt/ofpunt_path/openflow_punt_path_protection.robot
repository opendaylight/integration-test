...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE}    |grep "table=${ARP_PUNT_TABLE}.*n_packets=[\0-9+].*arp_tpa=22.1.1.101,arp_op=1.*actions=load:0x1"
...
Verify learnt flow in table=195 and table=196 for ARP reply
    ...
    BuiltIn.Should be true    ${count_after_traffic} == ${count_before_traffic}
    ${learnt_packet_count} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE}    |grep "table=${ARP_PUNT_TABLE}.*n_packets=[\0-9+].*arp_tpa=11.1.1.101,arp_op=2.*actions=load:0x1"
    BuiltIn.Should be true    ${learnt_packet_count} > 0
    ${count_after_traffic_arp} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ELAN_BASETABLE}    |grep "table=${ELAN_BASETABLE}.*n_packets=[\0-9+].*reg4=${ARP_REG}.0xffff"
    BuiltIn.Should be true    ${count_after_traffic_arp} > ${count_before_traffic_arp}
...
Verify resync subnet, SNAT and ARP route flow table after ODL and cluster restart
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    : FOR    ${index}    IN RANGE    0    3
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    @{OF_PUNT_TABLES}[${index}]    True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=@{VALID_TIMEOUTS}[0]
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
...
Verify resync subnet, SNAT and ARP flow table after CSS restart
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    : FOR    ${index}    IN RANGE    0    3
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    @{OF_PUNT_TABLES}[${index}]   True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=@{VALID_TIMEOUTS}[0]
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    ...
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo arping -c 2 @{VM_IPS}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_PUNT_TABLE}   True     ${EMPTY}    arp_tpa=@{VM_IPS}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table   ${OS_CMP1_IP}    ${ARP_LEARN_TABLE}   True     ${EMPTY}    arp_spa=@{VM_IPS}[1],arp_op=1
...
Verify the subnet route punt path rate limiting is off, by bring up the destination IP host and by deleting the destination IP host brought in previous case
    ...
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${L3_PUNT_TABLE}   True     ${EMPTY}    actions=CONTROLLER:65535,learn(table=${L3_PUNT_TABLE},hard_timeout=@{VALID_TIMEOUTS}[0]
...
Verify the ARP request punt path for same destination from different source
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE}    True     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_LEARN_TABLE}    True     ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_op=1
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[1]    sudo arping -c 2 @{EXTRA_NW_IP}[0]
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP2_IP}    ${ARP_PUNT_TABLE}    True     ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP2_IP}    ${ARP_LEARN_TABLE}    True     ${EMPTY}    arp_spa=@{EXTRA_NW_IP}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP2_IP}    ${ELAN_BASETABLE}    True     ${EMPTY}    arp,reg4=0x1/0xffff
...
Verify the ARP punt path for ARP Request from the VM should be punted to CSC and the reply from the subnet gateway shouldn’t be punted
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE}   True     ${EMPTY}    arp_tpa=11.1.1.1,arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE}   False     ${EMPTY}    arp_tpa=@{VM_IPS}[0],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_LEARN_TABLE}   True     ${EMPTY}    arp_spa=11.1.1.1,arp_op=1
...
Verify the SNAT punt path from same source IP to different destination
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_dst=@{EXTRA_NW_IP}[2]    tp_dst=${TELNET_PORT} actions=drop
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_dst=@{EXTRA_NW_IP}[3]    tp_dst=${TELNET_PORT} actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   True     |grep nw_src=@{VM_IPS}[5]    actions=set_field:${EXT_SUBNETS_FIXED_IP}->ip_src

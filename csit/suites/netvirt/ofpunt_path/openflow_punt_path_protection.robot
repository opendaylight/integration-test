...
Verify learnt flow in table=195 and table=196 for ARP request
    ...
    Run Command On Remote System And Log    ${OS_CMP1_IP}    sudo ovs-ofctl dump-flows br-int -O Openflow13
    Run Command On Remote System And Log    ${OS_CMP2_IP}    sudo ovs-ofctl dump-flows br-int -O Openflow13
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${SNAT_PUNT_TABLE}    @{VALID_TIMEOUTS}[0]
    Sleep    25s
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp actions=CONTROLLER:65535,learn(table=${ARP_PUNT_TABLE_1},hard_timeout=@{VALID_TIMEOUTS}[0]"
    ...
    ${count_after_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp actions=CONTROLLER:65535,learn(table=${ARP_PUNT_TABLE_1},hard_timeout=@{VALID_TIMEOUTS}[0]"
...
Verify learnt flow in table=195 and table=196 for GARP
    [Documentation]    To verify the learnt flow in ovs for ARP request, after traffic is generated from a Vm to unknow IP destination using arping..
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    @{VALID_TIMEOUTS}[0]
    Sleep    25s
    ${count_before_traffic} =    OvsManager.Get Packet Count From Table    ${OS_CMP1_IP}    ${INTEGRATION_BRIDGE}    table=${ARP_PUNT_TABLE_1}    |grep "arp actions=CONTROLLER:65535,learn(table=${ARP_PUNT_TABLE_1},hard_timeout=@{VALID_TIMEOUTS}[0]"
...
Verify resync subnet, SNAT and ARP route flow table after disconnect and reconnecting CSC-CSS control path
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Check OVS OpenFlow Connections    ${OS_CMP1_IP}    2
    : FOR    ${index}    IN RANGE    0    3
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}
    \    ...    @{OF_PUNT_TABLES}[${index}]   True    ${EMPTY}    learn(table=@{OF_PUNT_TABLES}[${index}],hard_timeout=@{VALID_TIMEOUTS}[0]
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{VM_IPS}[0]    sudo ping -c 5 @{EXTRA_NW_IP}[1]
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}
    ...    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=11.1.1.255 actions=drop
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}
    ...    ${L3_PUNT_TABLE}   False     ${EMPTY}    nw_dst=@{EXTRA_NW_IP}[1] actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   False     ${EMPTY}
    ...    nw_dst=11.1.1.255 actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}    ${SNAT_PUNT_TABLE}   False     ${EMPTY}
    ...    tp_dst=${TCP_PORT} actions=drop
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    False    ${EMPTY}
    ...    arp_tpa=@{VM_IPS}[1],arp_op=1
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}    False    ${EMPTY}
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${snat_napt_switch_ip}
    ...    ${SNAT_PUNT_TABLE}    False    ${EMPTY}    tp_dst=${UDP_PORT} actions=drop
    ...
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}   False
    ...    ${EMPTY}    arp_tpa=@{EXTRA_NW_IP}[1],arp_op=1
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_2}   False
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_CMP1_IP}    ${ARP_PUNT_TABLE_1}    True    ${EMPTY}    learn(table=${ARP_PUNT_TABLE_1},hard_timeout=@{VALID_TIMEOUTS}[0]

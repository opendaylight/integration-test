...
${VPN_NAME}       vpn_1
${DCGW_RD}        1:1
${DEFAULT_TIMEOUT}    140s
${AS_ID}          100
${TELNET_REGEX}    telnet: can't connect to remote host
...
Verify Update Router With Single External IP While Router Is Hosting Single Subnet
    ...
    ${output} =    Verify Telnet Status    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ${LOOPBACK_IP}    ${TELNET_REGEX}
...
Verify Telnet Status
    [Arguments]    ${net}    ${vm_ip}    ${ip}    ${telnet_regx}
    [Documentation]    Telnet from given vm to destined ip and check the status
    ${output} =    BuiltIn.Wait Until Keyword Succeeds    240s    10s    OpenStackOperations.Execute Command on VM Instance    ${net}    ${vm_ip}
    ...    telnet ${ip}

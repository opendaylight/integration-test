...
${ENABLE}         "enable_snat": true
${DISABLE}        "enable_snat": false
...
Verify successful creation of external network with router external set to TRUE
    OpenStackOperations.Add Router Gateway    @{ROUTER}[0]    @{EXTERNAL_NETWORKS}[0]    --enable-snat
    ${output} =    OpenStackOperations.Show Router    @{ROUTER}[0]
    BuiltIn.Should Contain    ${output}    ${ENABLE}
...
Verify successful update of router with external_gateway_info, disable SNAT and enable SNAT
    OpenStackOperations.Add Router Gateway    @{ROUTER}[0]    @{EXTERNAL_NETWORKS}[0]    --disable-snat
    ${output} =    OpenStackOperations.Show Router    @{ROUTER}[0]
    BuiltIn.Should Contain    ${output}    ${DISABLE}
    OpenStackOperations.Add Router Gateway    @{ROUTER}[0]    @{EXTERNAL_NETWORKS}[0]    --enable-snat
    ${output} =    OpenStackOperations.Show Router    @{ROUTER}[0]
    BuiltIn.Should Contain    ${output}    ${ENABLE}

Verify successful deletion of external network
    OpenStackOperations.Remove Gateway    @{ROUTER}[0]
    ${output} =    OpenStackOperations.Show Router    @{ROUTER}[0]
    BuiltIn.Should Not Contain    ${output}    ${ENABLE}
...
Verify update router with single external IP while router is hosting single subnet
    ...
    ${output} =    OpenStackOperations.Show Router    @{ROUTER}[0]
    BuiltIn.Should Contain    ${output}    ${ENABLE}
    ${ext_ip} =    Get External Ip From Router    @{ROUTER}[0]
...
Create Neutron Networks
    ...
    OpenStackOperations.Create Network    @{EXTERNAL_NETWORKS}[0]    --router:external=@{BOOL_VALUES}[0] --provider:network_type ${NETWORK_TYPE}
...
Get External Ip From Router
    ...
    ${ip} =    DevstackUtils.Write Commands Until Prompt    openstack router show ${router_name} | grep ip_address | awk '{print $12}'    30s

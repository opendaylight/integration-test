...
ACL Service Recovery CLI
    ...
    ${resp}=      RequestsLibrary.Delete Request    session    ${CONFIG_NODES_API}/node/openflow:${node_id}/flow-node-inventory:table/${table_id}
    OpenStackOperations.Get DumpFlows And Ovsconfig    ${OS_CMP1_CONN_ID}
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
...
ACL Interface Recovery CLI
    ...
    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=240 | grep ${port_mac}    ${DEFAULT_LINUX_PROMPT_STRICT}
    ${output}=    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=240 | wc -l    ${DEFAULT_LINUX_PROMPT_STRICT}
    @{list}=    Split String    ${output}
    ${count_before}    Set Variable    ${list[0]}
    ${node_id}    Genius.Get Dpn Ids    ${OS_CMP1_CONN_ID}
    Write Commands Until Expected Prompt    sudo ovs-ofctl del-flows br-int -OOpenflow13 "table=240,dl_dst=${port_mac}"    ${DEFAULT_LINUX_PROMPT_STRICT}
    OpenStackOperations.Get DumpFlows And Ovsconfig    ${OS_CMP1_CONN_ID}
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    ${output}=    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=240    ${DEFAULT_LINUX_PROMPT_STRICT}
    ...
    OpenStackOperations.Get DumpFlows And Ovsconfig    ${OS_CMP1_CONN_ID}
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=240    ${DEFAULT_LINUX_PROMPT_STRICT}
    ${output}=    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=240 | wc -l    ${DEFAULT_LINUX_PROMPT_STRICT}
    @{list}=    Split String    ${output}
    ${count_after}    Set Variable    ${list[0]}
    Should Be Equal As Numbers    ${count_before}    ${count_after}

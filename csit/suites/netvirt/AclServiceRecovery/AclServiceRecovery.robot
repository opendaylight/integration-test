...
Test Teardown     Run Keywords    OpenStackOperations.Get DumpFlows And Ovsconfig    ${OS_CMP1_CONN_ID}
...
ACL Service Recovery CLI
    ...
    ${count_before}=    Get Dump Flows Count    ${table_ids[0]}
    ...
    ${count_after}=    Get Dump Flows Count    ${table_ids[0]}
...
ACL Instance Recovery CLI
    ...
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    ${count_before}=    Get Dump Flows Count    ${table_ids[1]}
    ${node_id}    OVSDB.Get DPID    ${OS_COMPUTE_1_IP}
    ...
    Should Contain    ${output}    RPC call to recover was successful
    ${count_after}=    Get Dump Flows Count    ${table_ids[1]}
    Should Be Equal As Numbers    ${count_before}    ${count_after}
...
ACL Interface Recovery CLI
    ...
    ${output} =    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=${table_ids[2]} | grep ${port_mac} | wc -l    ${DEFAULT_LINUX_PROMPT_STRICT}
    @{list} =    Split String    ${output}
...
Get Dump Flows Count
    [Arguments]    ${acl_sr_table_id}
    ${output} =    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=${acl_sr_table_id} | wc -l    ${DEFAULT_LINUX_PROMPT_STRICT}
    @{list} =    Split String    ${output}

...
Verify Datapath After OVS Restart
    ...
    Restart OVSDB    ${OS_CMP1_IP}
    Restart OVSDB    ${OS_CMP2_IP}
...
Verify Datapath After Recreate VM Instance
    ...
    ${SRCMAC_CN1} =    Create List    ${VM_MACAddr_ELAN1[0]}
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Removed For ELAN Service    ${OS_CMP1_IP}    ${SRCMAC_CN1}
    Remove RSA Key From KnowHosts    ${VM_IP_ELAN1[0]}
    Log    ReCreate VM and verify flow updated
    Create Vm Instance With Port On Compute Node    ${ELAN1_PORT_LIST[0]}    ${VM_INSTANCES_ELAN1[0]}    ${OS_CMP1_IP}
    ...
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Present For ELAN Service    ${OS_CMP1_IP}    ${SRCMAC_CN1}    ${VM_MACAddr_ELAN1}
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_ELAN1[0]}    ping -c 3 ${VM_IP_ELAN1[1]}
    Should Contain    ${output}    ${PING_PASS}
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_ELAN1[1]}    ping -c 3 ${VM_IP_ELAN1[0]}
    Should Contain    ${output}    ${PING_PASS}
...
Delete All ELAN1 VM And Verify Flow Table Updated
    ...
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Removed For ELAN Service    ${OS_CMP1_IP}    ${VM_MACAddr_ELAN1}
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Removed For ELAN Service    ${OS_CMP2_IP}    ${VM_MACAddr_ELAN1}
...
Verify Datapath for Multiple ELAN with Multiple DPN
    ...
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Present For ELAN Service    ${OS_CMP1_IP}    ${SRCMAC_CN1}    ${MAC_LIST}
    Wait Until Keyword Succeeds    30s    10s    Verify Flows Are Present For ELAN Service    ${OS_CMP2_IP}    ${SRCMAC_CN2}    ${MAC_LIST}
    ...
    ${cn1_conn_id} =    Start Packet Capture on Node    ${OS_CMP1_IP}    file_Name=${filename_prefix}_CN1
    ${cn2_conn_id} =    Start Packet Capture on Node    ${OS_CMP2_IP}    file_Name=${filename_prefix}_CN2
    ${os_conn_id} =    BuiltIn.Run Keyword If    '${OS_CNTL_IP}' != '${OS_CMP1_IP}'    Start Packet Capture on Node    node_ip=${OS_CNTL_IP}    file_Name=${filename_prefix}_OS
    ...
    ...    AND    BuiltIn.Run Keyword If    '${OS_CNTL_IP}' != '${OS_CMP1_IP}'    Stop Packet Capture on Node    ${os_conn_id}

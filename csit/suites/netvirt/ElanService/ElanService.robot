...
Verify Datapath After Recreate VM Instance
    ...
    ${VM_IP_ELAN1}    Wait Until Keyword Succeeds    60s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_ELAN1}
    Log    ${VM_IP_ELAN1}
...
Verify Datapath for Multiple ELAN with Multiple DPN
    ...
    ${VM_IP_ELAN2}    ${DHCP_IP2}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_ELAN2}
    Log    ${VM_IP_ELAN2}
...
SingleElan SuiteSetup
    ...
    ${VM_IP_ELAN1}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_ELAN1}
    Log    ${VM_IP_ELAN1}
    Set Suite Variable    ${VM_IP_ELAN1}
...
MultipleElan Testsuit Setup
    ...
    \    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}
    ${VM_IP_ELAN2}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_ELAN2}
    Log    ${VM_IP_ELAN2}
    Set Suite Variable    ${VM_IP_ELAN2}
    ${VM_IP_ELAN3}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_ELAN3}
    Log    ${VM_IP_ELAN3}

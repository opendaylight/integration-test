...
Check Datapath Traffic Across Networks With L3VPN
    ...
    ${output}=    Wait Until Keyword Succeeds    60s    10s    VpnOperations.Get Fib Entries    session
    BuiltIn.Log    ${output}
    ${output} =    Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 20 @{NET_1_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 20 @{NET_2_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 20 @{NET_2_VM_IPS}[0]
    BuiltIn.Should Contain    ${output}    64 bytes

...
@{EXTRA_NW_IP}    71.1.1.2    72.1.1.2    73.1.1.2    
@{EXTRA_NW_SUBNET}    71.1.1.0/24    72.1.1.0/24    73.1.1.0/24    
...
${CREATE_RD_1}    ["2200:2","2300:2","2400:2"]
...
Create Nova VMs
    ...
    \    @{NET_1_VM_IPS}[${index}]    ${NET_${index}_DHCP_IP} =    OpenStackOperations.Get VM IPs    @{NET_${index}_VMS}
    \    BuiltIn.Set Suite Variable   @{NET_${index}_VM_IPS} 
    \    BuiltIn.Should Not Contain    @{NET_${index}_VM_IPS}    None
    \    BuiltIn.Should Not Contain    ${NET_${index}_DHCP_IP}    None
...
Verification of route update after VM port removed and re added to VPN
    ...
    Delete Port    @{PORTS}[0]
    ${resp}=    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements Not At URI    ${FIB_ENTRY_URL}    @{NET_1_VM_IPS}[0]
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    @{NET_1_VM_IPS}[0]  
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_1_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
...
Verification of route update after reconfiguring vpn by adding new ports
    ...
    ${VM_IP_NET1_New}    ${NET_1_DHCP_IP}    ${console_log}    Get VM IP    true    ${VM_NAME_NEW}
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    ${VM_IP_NET1_New}
...
Verification of route download with 5 vpns in SE & qbgp with 1-1 export import route target
    ...
    ${fib_values} =    BuiltIn.Create List    @{SUBNET_CIDRS}    @{LOOPBACK_IPS}    
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Utils.Check For Elements At URI    ${FIB_ENTRY_URL}    ${fib_values}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_1_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_2_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[0]
    BuiltIn.Should Contain    ${output}    0 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    0 bytes
    [Teardown]    Pretest Cleanup

Verification of route download with 5 vpns in SE & qbgp with 1-many export import route target
    [Documentation]    Verification of route download with 5 vpns in SE & qbgp with 1-many export import route target
    VpnOperations.VPN Create L3VPN    vpnid=@{VPN_INSTANCE_IDS}[0]    name=@{VPN_NAMES}[0]    rd=@{RDS}[0]}    exportrt=${CREATE_RD_1}    importrt=@{RDS}[0]}    tenantid=${tenant_id}
    : FOR    ${index}    IN RANGE    1    ${NUM_OF_L3VPN}
    \    VpnOperations.VPN Create L3VPN    vpnid=@{VPN_INSTANCE_IDS}[${index}]    name=@{VPN_NAMES}[${index}]    rd=@{RD}[${index}]    exportrt=@{RDS}[${index}]    importrt=${RT_LIST_${index}}    tenantid=${tenant_id}
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_L3VPN}
    \    BgpOperations.Configure BGP And Add Neighbor On DCGW    ${DCGW_SYSTEM_IP}    ${AS_ID}    ${DCGW_SYSTEM_IP}    ${ODL_SYSTEM_IP}    @{VPN_NAMES}[${index}]
    \    ...    @{DCGW_RD}[${index}]    @{LOOPBACK_IPS}[${index}]
    \    BgpOperations.Add Loopback Interface On DCGW    ${DCGW_SYSTEM_IP}    @{LOOPBACK_NAMES}[${index}]    @{LOOPBACK_IPS}[${index}]
...
Verification of route download with 5 vpns in SE & qbgp with many-1 export import route target
    [Documentation]    Verification of route download with 5 vpns in SE & qbgp with many-1 export import route target
    VpnOperations.VPN Create L3VPN    vpnid=@{VPN_INSTANCE_IDS}[0]    name=@{VPN_NAMES}[0]    rd=@{RD}[0]    exportrt=@{RD}[0]}    importrt=${CREATE_RD_1}    tenantid=${tenant_id}
    : FOR    ${index}    IN RANGE    1    ${NUM_OF_L3VPN}
    \    VpnOperations.VPN Create L3VPN    vpnid=@{VPN_INSTANCE_ID}[${index}]    name=@{VPN_NAMES}[${index}]    rd=@{RD}[${index}]    exportrt=${RT_LIST_${index}}    importrt=@{RD}[${index}]    tenantid=${tenant_id}
    : FOR    ${index}    IN RANGE    0   ${NUM_OF_L3VPN}
    \    BgpOperations.Configure BGP And Add Neighbor On DCGW    ${DCGW_SYSTEM_IP}    ${AS_ID}    ${DCGW_SYSTEM_IP}    ${ODL_SYSTEM_IP}    @{VPN_NAMES}[${index}]
    \    ...    @{DCGW_RD}[${index}]    @{LOOPBACK_IPS}[${index}]
    \    BgpOperations.Add Loopback Interface On DCGW    ${DCGW_SYSTEM_IP}    @{LOOPBACK_NAMES}[${index}]    @{LOOPBACK_IPS}[${index}]
...
Verification of route download with 5 vpns in SE & qbgp with many-many export import route target
    [Documentation]    Verification of route download with 5 vpns in SE & qbgp with many-1 export import route target
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_L3VPN}
    \    VpnOperations.VPN Create L3VPN    vpnid=@{VPN_INSTANCE_IDS}[${index}]    name=@{VPN_NAME}[${index}]    rd=@{RD}[${index}]    exportrt=${CREATE_RD_1}    importrt=${CREATE_RD_1}    tenantid=${tenant_id}
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_L3VPN}
    \    BgpOperations.Configure BGP And Add Neighbor On DCGW    ${DCGW_SYSTEM_IP}    ${AS_ID}    ${DCGW_SYSTEM_IP}    ${ODL_SYSTEM_IP}    @{VPN_NAMES}[${index}]
    \    ...    ${DCGW_RD[${index}]}    ${LOOPBACK_IP[${index}]}
    \    BgpOperations.Add Loopback Interface On DCGW    ${DCGW_SYSTEM_IP}    @{LOOPBACK_NAMES}[${index}]    @{LOOPBACK_IPS}[${index}]
    ${Req_no_of_networks} =    Evaluate    2
    Associate Multiple L3VPN To Networks    ${Req_no_of_networks}
    ${fib_values} =    BuiltIn.Create List    @{SUBNET_CIDRS}    @{LOOPBACK_IPS}
...
Pretest Cleanup
    ...
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_L3VPN}
    \    BgpOperations.Delete Loopback Interface On DCGW    ${DCGW_SYSTEM_IP}    @{LOOPBACK_NAMES}[${index}]    @{LOOPBACK_IPS}[${index}]

...
Start Suite
    ...
    KarafKeywords.Issue_Command_On_Karaf_Console    configure-bgp --as-num ${AS_ID} --router-id ${ODL_SYSTEM_IP} -op start-bgp-server
    ${output} =    KarafKeywords.Issue_Command_On_Karaf_Console    ${BGP_CACHE}
    Log    ${output}
    : FOR    ${dcgw}    IN    @{DCGW_IP_LIST}
...
Add Routes On DCGW
    ...
    BgpOperations.Execute Command On Quagga Telnet Session    configure terminal
    BgpOperations.Execute Command On Quagga Telnet Session    router bgp ${AS_ID}
    BgpOperations.Execute Command On Quagga Telnet Session    address-family ${VPNV4_ADDR_FAMILY} unicast

...
TC2 Verify CSC supports REST API/CLI for max path configuration
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify L3VPN On ODL    @{VPN_INSTANCE_ID}
    ...
    : FOR    ${idx}    IN RANGE    0    ${NUM_OF_DCGW}
    \    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[${idx}]
...
TC5 Verify CSC supports dynamic configuration changes for max path value
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[1]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    2
    Configure Maxpath    @{MAX_PATH_LIST}[0]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[0]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    30    5    Verify FIB Entry    @{NETWORK_IP}[0]    3
...
TC7 Verify ECMP path advertised by QBGP to CSC in case of distinct VRF
    ...
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[0]    vpnid=@{VPN_INSTANCE_ID}[0]    rd=@{L3VPN_RD}[0]    exportrt=@{L3VPN_EXPORT_RT}[0]    importrt=@{L3VPN_IMPORT_RT}[0]
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[1]    vpnid=@{VPN_INSTANCE_ID}[1]    rd=@{L3VPN_RD}[1]    exportrt=@{L3VPN_EXPORT_RT}[1]    importrt=@{L3VPN_IMPORT_RT}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify L3VPN On ODL    @{VPN_INSTANCE_ID}[0]    @{VPN_INSTANCE_ID}[1]
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Multipath
    BuiltIn.Wait Until Keyword Succeeds    10    2    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[1]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[1]
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    2
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[1]    2
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[1]    3
    ...
    BuiltIn.Wait Until Keyword Succeeds    30    5    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}    flag=False    start=2
    BuiltIn.Wait Until Keyword Succeeds    30    5    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[1]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[1]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    2
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[1]    2
...
TC8 Verify ECMP path advertised by QBGP to CSC when routes shared across VPNs
    ...
    \    BuiltIn.Wait Until Keyword Succeeds    10    2    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[${idx}]
    BuiltIn.Log    "Verifying maxpath configuration"
    : FOR    ${idx}    IN RANGE    0    3
    \    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[${idx}]
    BuiltIn.Log    "Adding set of Routes on Dcgateway"
    ...
    BuiltIn.Wait Until Keyword Succeeds    60    10    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    2
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    6
    ...
    BuiltIn.Log    "Configuring maxpath=8"
    BuiltIn.Wait Until Keyword Succeeds    10    2    Configure Maxpath    @{MAX_PATH_LIST}[1]    @{MULTIPATH_RD}[0]
    BuiltIn.Log    "Verifying maxpath configuration"
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Maxpath    @{MAX_PATH_LIST}[1]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10    2    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    1
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    2
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    5
    BuiltIn.Log    "Verify the routes with correct next hop"
    Verify Route Entry With Nexthop    @{RD}[0]    @{NETWORK_IP}[0]    ${NUM_OF_DCGW}
    Verify Route Entry With Nexthop    @{RD}[1]    @{NETWORK_IP}[0]    1
    Verify Route Entry With Nexthop    @{RD}[2]    @{NETWORK_IP}[0]    3    start=1
...
TC9 Verify BGP routes advertised to CSC when QBGP receives only unequal cost routes
    ...
    BuiltIn.Wait Until Keyword Succeeds    30    5    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify FIB Entry    @{NETWORK_IP}[0]    1
    BuiltIn.Log    "Verify the routes with correct next hop"
    ...
    Check BGP Session On ODL    ${NUM_OF_DCGW}
    Check BGP Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    30    2    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    3
    BuiltIn.Wait Until Keyword Succeeds    30    2    Verify FIB Entry    @{NETWORK_IP}[0]    2

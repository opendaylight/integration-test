...
*** Variables ***
@{DCGW_RD}        11:1    22:2    33:3    44:4
@{DCGW_IRT}       11:1    22:2    33:3
@{DCGW_ERT}       11:1    22:2    33:3
@{DCGW_IP_LIST}    ${TOOLS_SYSTEM_1_IP}    ${TOOLS_SYSTEM_2_IP}    ${TOOLS_SYSTEM_3_IP}
@{LABEL}          51    52    53
@{L3VPN_RD}       ["11:1"]    ["22:2"]    ["33:3"]
@{L3VPN_IRT}      ["11:1"]    ["22:2"]    ["33:3"]
@{L3VPN_ERT}      ["11:1"]    ["22:2"]    ["33:3"]
...
Verify ODL supports REST API/CLI for max path configuration with multiple VPNs
    ...
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[${idx}]
...
Verify that max path default is set to 8 and max path configurable is 64 on ODL
    ...
    Enable Or Disable Multipath On ODL
    Verify Multipath
    Configure Maxpath    @{MAX_PATH_LIST}[4]    @{MULTIPATH_RD}[0]
...
Verify ODL supports dynamic configuration changes for max path value
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{MULTIPATH_RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry On ODL    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    ...
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry On ODL    @{MULTIPATH_RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    ...
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry On ODL    @{MULTIPATH_RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry On ODL    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[1]
    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{MULTIPATH_RD}[0]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry On ODL    @{MULTIPATH_RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Verify FIB Entry On ODL    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
...
Start Suite
    ...
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    BgpOperations.Configure BGP And Add Neighbor On DCGW    ${DCGW_IP_LIST[${idx}]}    ${AS_ID}    ${DCGW_IP_LIST[${idx}]}    ${ODL_SYSTEM_IP}    @{VPN_NAME}[0]
    \    ...    @{DCGW_RD}[0]    @{NETWORK_IP}[0]
...
Check BGP Session On ODL
    ...
    : FOR    ${index}    IN RANGE    ${START_VALUE}    ${dcgw_count}
...
Check BGP Session On DCGW
    ...
    : FOR    ${index}    IN RANGE    ${START_VALUE}    ${dcgw_count}
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    BgpOperations.Verify BGP Neighbor Status On Quagga    ${DCGW_IP_LIST[${index}]}    ${ODL_SYSTEM_IP}

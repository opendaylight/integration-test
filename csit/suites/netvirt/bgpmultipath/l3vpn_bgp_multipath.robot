...
Verify ODL supports dynamic configuration changes for max path value
    ...
    \    ...    @{NUM_OF_ROUTES}[2]
    \    BuiltIn.Wait Until Keyword Succeeds    120s    20s    Verify FIB Entry On ODL    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[${index}]
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
...
Verify ECMP path advertised by QBGP to ODL in case of distinct VRF
    [Documentation]    Verify ECMP path advertised by QBGP to ODL in case of distinct VRF
...
Verify ECMP path advertised by QBGP to ODL when routes shared across VPNs
    [Documentation]    Verify ECMP path advertised by QBGP to ODL when routes shared across VPNs
...
Verify BGP routes advertised to ODL when QBGP receives only unequal cost routes
    [Documentation]    Verify BGP routes advertised to ODL when QBGP receives only unequal cost routes
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[0]    vpnid=@{VPN_ID}[0]    rd=@{L3VPN_RD_IRT_ERT}[0]    exportrt=@{L3VPN_RD_IRT_ERT}[0]    importrt=@{L3VPN_RD_IRT_ERT}[0]
    VpnOperations.Verify L3VPN On ODL    @{VPN_ID}[0]
    VpnOperations.Associate VPN to Router    routerid=@{router_id_list}[0]    vpnid=@{VPN_ID}[0]
    : FOR    ${dcgw}    IN    @{DCGW_IP_LIST}
...
Verify ECMP routes on ODL when QBGP receives combination of equal and unequal cost routes
    [Documentation]    Verify ECMP routes on ODL when QBGP receives combination of equal and unequal cost routes
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[0]    vpnid=@{VPN_ID}[0]    rd=@{L3VPN_RD_IRT_ERT}[0]    exportrt=@{L3VPN_RD_IRT_ERT}[0]    importrt=@{L3VPN_RD_IRT_ERT}[0]
    VpnOperations.Verify L3VPN On ODL    @{VPN_ID}[0]
    VpnOperations.Associate VPN to Router    routerid=@{router_id_list}[0]    vpnid=@{VPN_ID}[0]
    : FOR    ${dcgw}    IN    @{DCGW_IP_LIST}
...
Verify Route Entry With Nexthop
    ...
    [Documentation]    Verification of route entry with correct nexthop (DC gateways) in FIB
...
Verify Leaking Route Across VPNs
    ...
    [Documentation]    Verify multiple VPNs routes should not mix with each other
    ...    only matching VPN route should learn to particular VPNs

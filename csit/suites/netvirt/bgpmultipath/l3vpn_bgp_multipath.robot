...
Verify ODL supports REST API/CLI for max path configuration with multiple VPNs
    ...
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{L3VPN_RD_IRT_ERT}[${idx}]
    \    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{L3VPN_RD_IRT_ERT}[${idx}]
...
Verify max-path configuration value should not be supported invalid values(0/-ve)
    ...
    \    Configure Maxpath    ${invalid_maxpath}    @{L3VPN_RD_IRT_ERT}[0]
    \    Verify Maxpath    ${invalid_maxpath}    @{L3VPN_RD_IRT_ERT}[0]
...
Verify that max path default is set to 8 and max path configurable is 64 on ODL
    ...
    Configure Maxpath    @{MAX_PATH_LIST}[4]    @{L3VPN_RD_IRT_ERT}[0]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Maxpath    @{MAX_PATH_LIST}[4]    @{L3VPN_RD_IRT_ERT}[0]
...
Verify ODL supports dynamic configuration changes for max path value
    ...
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Create L3VPN on Dcgateway    @{DCGW_IP_LIST}[${idx}]    @{VPN_NAME}[0]    @{DCGW_RD_IRT_ERT}[0]    @{DCGW_RD_IRT_ERT}[0]    @{DCGW_RD_IRT_ERT}[0]
    Verify L3VPN On DCGW    ${NUM_OF_DCGW}    @{VPN_NAME}[0]    @{DCGW_RD_IRT_ERT}[0]    @{DCGW_RD_IRT_ERT}[0]    @{DCGW_RD_IRT_ERT}[0]
    Configure Maxpath    @{MAX_PATH_LIST}[2]    @{L3VPN_RD_IRT_ERT}[0]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Maxpath    @{MAX_PATH_LIST}[2]    @{L3VPN_RD_IRT_ERT}[0]
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Add Routes    @{DCGW_IP_LIST}[${idx}]    @{DCGW_RD_IRT_ERT}[0]    @{NETWORK_IP}[0]    @{LABEL}[${idx}]
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{L3VPN_RD_IRT_ERT}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry On ODL    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    : FOR    ${index}    IN RANGE    0    3
    \    Configure Maxpath    @{MAX_PATH_LIST}[${index}]    @{L3VPN_RD_IRT_ERT}[0]
    \    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Maxpath    @{MAX_PATH_LIST}[0]    @{L3VPN_RD_IRT_ERT}[0]
...
Test Cleanup
    ...
    : FOR    ${l3vpn_rd}    IN    @{L3VPN_RD_IRT_ERT}

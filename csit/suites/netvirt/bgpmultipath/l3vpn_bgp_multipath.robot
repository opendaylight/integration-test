...
Test Cleanup
    ...
    : FOR    ${dcgw}    IN    @{DCGW_IP_LIST}
    \    BuiltIn.Run Keyword And Ignore Error    BgpOperations.Delete L3VPN on ODL    ${dcgw}    ${AS_ID}    ${VPN_NAME}
    Configure Multipath On ODL    ${ENABLE}
...
Configure Multipath On ODL
    ...
    Run Keyword If    '${setting}' == 'enable'    KarafKeywords.Issue_Command_On_Karaf_Console    odl:multipath -f ${VPNV4_ADDR_FAMILY} ${setting}
    ...    ELSE    KarafKeywords.Issue_Command_On_Karaf_Console    odl:multipath -f ${VPNV4_ADDR_FAMILY} ${setting}

Verify Multipath
    [Arguments]    ${setting}
    [Documentation]    verify multipath on ODL
    ${output} =    KarafKeywords.Issue_Command_On_Karaf_Console    ${BGP_CACHE}
    Run Keyword If    '${setting}' == 'enable'    BuiltIn.Should Contain    ${output}    ${VPNV4_ADDR_FAMILY}

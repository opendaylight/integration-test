...
${MULTIPATH_ENABLE}    odl:multipath -f vpnv4 enable
${MULTIPATH_DISABLE}    odl:multipath -f vpnv4 disable
${NUM_OF_DCGW}    3
${NUM_OF_L3VPN}    3
${NUM_OF_NETWORKS}    3
${NUM_OF_SUBNETS}    3
...
*** Test Cases ***
Verify CSC supports REST API/CLI for multipath configuration (enable/disable multipath)
...
Verify that max path default is set to 8 and max path configurable is 64 on CSC
...
Verify CSC supports dynamic configuration changes for max path value
    [Documentation]    Verify CSC supports dynamic configuration changes for max path value
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[0]    vpnid=@{VPN_INSTANCE_ID}[0]    rd=@{L3VPN_RD}[0]    exportrt=@{L3VPN_EXPORT_RT}[0]    importrt=@{L3VPN_IMPORT_RT}[0]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify L3VPN On ODL    @{VPN_INSTANCE_ID}[0]
    VpnOperations.Associate VPN to Router    routerid=${router_id_1}    vpnid=@{VPN_INSTANCE_ID}[0]
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Create L3VPN on Dcgateway    @{DCGW_IP_LIST}[${idx}]    @{VPN_NAME}[0]    @{DCGW_RD}[0]    @{DCGW_IMPORT_RT}[0]    @{DCGW_EXPORT_RT}[0]
    Verify L3VPN On DCGW    ${NUM_OF_DCGW}    @{VPN_NAME}[0]    @{DCGW_RD}[0]    @{DCGW_IMPORT_RT}[0]    @{DCGW_EXPORT_RT}[0]
    Check BGP Session On DCGW    ${NUM_OF_DCGW}
...
Verify ECMP path advertised by QBGP to CSC in case of distinct VRF
    ...
    VpnOperations.Associate VPN to Router    routerid=${router_id_1}    vpnid=@{VPN_INSTANCE_ID}[0]
    VpnOperations.Associate VPN to Router    routerid=${router_id_2}    vpnid=@{VPN_INSTANCE_ID}[1]
...
Verify ECMP path advertised by QBGP to CSC when routes shared across VPNs
    ...
    VpnOperations.Associate VPN to Router    routerid=${router_id_1}    vpnid=@{VPN_INSTANCE_ID}[0]
    VpnOperations.Associate VPN to Router    routerid=${router_id_2}    vpnid=@{VPN_INSTANCE_ID}[1]
    VpnOperations.Associate VPN to Router    routerid=${router_id_3}    vpnid=@{VPN_INSTANCE_ID}[2]
...
Verify route resynchronization of DC-GW with QBGP after cluster reboot
    ...
    #TODO Log    Restarting ODL Cluster
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[5]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[1]
...
Verify equal cost route resynchronization of DC-GW with QBGP after ODL reboot/restart
...
Verify route resynchronization of DC-GW with QBGP, when one or more DC-GW fails/rebooted
...
Verify the scenario when CSC receives back to back route advertisement and withdrawal for the same prefix and single VRF
...
Verify the scenario when CSC receives back to back route advertisement and withdrawal for the same prefix and multi VRF after Qbpg restart
...
Start Suite
    ...
    BuiltIn.Wait Until Keyword Succeeds    10    2    Verify BGP Neighbor On ODL    ${NUM_OF_DCGW}
    : FOR    ${idx}    IN RANGE    ${START_VALUE}    ${NUM_OF_DCGW}
    \    Configure BGP And Add Neighbor On DCGW    ${DCGW_IP_LIST[${idx}]}    ${ODL_MIP_IP}    ${AS_ID}
    Create Neutron Networks    ${NUM_OF_NETWORKS}
    Create Neutron Subnets    ${NUM_OF_SUBNETS}
    Create Neutron Routers    ${NUM_OF_ROUTERS}
...
Configure Maxpath
    ...
    ${maxpath_command}    Catenate    multipath -r    ${rd} -f vpnv4 -n ${maxpath} setmaxpath
    ${output} =    KarafKeywords.Issue_Command_On_Karaf_Console    ${maxpath_command}
    Run Keyword If    0 < ${maxpath} < 65    BuiltIn.Should Not Contain    ${output}    ${ERROR}
    ...    ELSE    BuiltIn.Should Contain    ${output}    ${INVALID_INPUT}
...
Delete BGP Neighbor On ODL
    ...
    \    ${del_bgp_nbr} =    BuiltIn.Set Variable    configure-bgp -op delete-neighbor --ip ${DCGW_IP_LIST[${index}]} --as-num ${AS_ID} --use-source-ip ${ODL_MIP_IP}
...
Configure BGP Neighbor On ODL
    ...
    \    ${add_bgp_nbr} =    BuiltIn.Set Variable    configure-bgp -op add-neighbor --ip ${DCGW_IP_LIST[${index}]} --as-num ${AS_ID} --use-source-ip ${ODL_MIP_IP}
...
Delete L3VPN
    ...
    \    BuiltIn.Run Keyword And Ignore Error    VpnOperations.VPN Delete L3VPN    vpnid=${VPN_INSTANCE_ID[${idx}]}
...
Check BGP Session On DCGW
    ...
    : FOR    ${index}    IN RANGE    ${start}    ${dcgw_count}
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify BGP Neighbor Status On Quagga    ${DCGW_IP_LIST[${index}]}    ${ODL_MIP_IP}
...
Verify VPN Config
    ...
    BuiltIn.Should Contain    ${output}    ${vpn_id}
    BuiltIn.Should Contain    ${output}    ${rd}
...
Verify Route Entry With Nexthop
    ...
    \    BuiltIn.Should Match Regexp    ${output}    \\s*${prefix}\\s*${DCGW_IP_LIST[${index}]}    msg="FIB table does not contain ${DCGW_IP_LIST[${index}]} for ${prefix} prefix "
...
Verify Best Route In RIB
    ...
    [Documentation]    Verification of best routes in Routing table at ODL
    BuiltIn.Should Match Regexp    ${output_str}    \\s*${best}${prefix}\\s*${nexthop}

Verify Equal Route In RIB
    [Arguments]    ${output_str}    ${rd}    ${prefix}    ${nexthop}    ${equal}=\\*=i
    [Documentation]    Verification of best routes in Routing table at ODL
    BuiltIn.Should Match Regexp    ${output_str}    \\s*${equal}${prefix}\\s*${nexthop}
...
Check Routes on DC_GW
    ...
    \    BuiltIn.Should Contain    ${output}    ${LOOPBACK_IP[${index}]}

Create Neutron Networks
    [Arguments]    ${network_count}
    [Documentation]    Create required number of networks
    : FOR    ${index}    IN RANGE    0    ${network_count}
    \    OpenStackOperations.Create Network    @{NETWORKS}[${index}]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Utils.Check For Elements At URI    ${NETWORK_URL}    ${NETWORKS}

Create Neutron Subnets
    [Arguments]    ${subnet_count}
...
Create Neutron Routers
    [Arguments]    ${router_count}
    [Documentation]    Create required number of routers
    : FOR    ${index}    IN RANGE    0    ${router_count}
    \    OpenStackOperations.Create Router    @{ROUTERS}[${index}]
    ${router_id_1} =    OpenStackOperations.Get Router Id    @{ROUTERS}[0]
    ${router_id_2} =    OpenStackOperations.Get Router Id    @{ROUTERS}[1]
    ${router_id_3} =    OpenStackOperations.Get Router Id    @{ROUTERS}[2]
    BuiltIn.Set Suite Variable    ${router_id_1}
    BuiltIn.Set Suite Variable    ${router_id_2}
    BuiltIn.Set Suite Variable    ${router_id_3}

Add Interfaces To Routers
    [Documentation]    Add Multiple Interfaces to Router and Verify
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_SUBNETS}

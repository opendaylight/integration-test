...
TC11_Verify route resynchronization of DC-GW with QBGP after cluster reboot
    ...
    Log    Configure different local preference across DCGW
    : FOR    ${idx}    IN RANGE    0    2
    \    Configure BGP Preference    @{DCGW_IP_LIST}[${idx}]    @{PREF_LIST_110}[${idx}]
    ...
    Check BGP Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[6]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    BuiltIn.Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[5]
    Verify Route Entry With Nexthop    @{RD}[0]    @{NETWORK_IP}[0]    start=0    end=3
    Verify Route Entry With Nexthop    @{RD}[1]    @{NETWORK_IP}[0]    start=1    end=2
    Verify Route Entry With Nexthop    @{RD}[2]    @{NETWORK_IP}[0]    start=2    end=3
    Log    "Restarting Cluster node"
    #Restart_ODL_Cluster
    #Delete Routes    ${DCGW_IP_LIST[2]}    ${DCGW_RD[2]}    ${NETWORK_IP[0]}    ${LABEL[2]}
    #Sleep    10s
    #Wait Until Keyword Succeeds    10    2    Verify Routing Entry    ${RD[0]}    ${NETWORK_IP[0]}    2
    #Wait Until Keyword Succeeds    10    2    Verify Routing Entry    ${RD[1]}    ${NETWORK_IP[0]}    4
    #Wait Until Keyword Succeeds    10    2    Verify Routing Entry    ${RD[2]}    ${NETWORK_IP[0]}    3
    #Wait Until Keyword Succeeds    10    2    Verify FIB Entry    ${NETWORK_IP[0]}    7
    #Log    "Verify the routes with correct next hop"
    #Verify Route Entry With Nexthop    ${RD[2]}    ${NETWORK_IP[0]}    7    start=6
...
TC16_Verify the scenario when CSC receives back to back route advertisement and withdrawal for the same prefix and multi VRF after Qbpg restart
    ...
    Add Routes    @{DCGW_IP_LIST}[2]    @{DCGW_RD}[2]    @{NETWORK_IP}[0]    @{LABEL}[2]
    Wait Until Keyword Succeeds    60s    10s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[1]
    Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[6]
    Verify Route Entry With Nexthop    @{RD}[0]    @{NETWORK_IP}[0]    start=0    end=1
    ...
    Delete Routes    @{DCGW_IP_LIST}[2]    @{DCGW_RD}[2]    @{NETWORK_IP}[0]    @{LABEL}[2]
    Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[0]
    Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[1]
    Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    Add Routes    @{DCGW_IP_LIST}[0]    @{DCGW_RD}[0]    @{NETWORK_IP}[0]    @{LABEL}[0]
    Add Routes    @{DCGW_IP_LIST}[1]    @{DCGW_RD}[1]    @{NETWORK_IP}[0]    @{LABEL}[1]
    Add Routes    @{DCGW_IP_LIST}[2]    @{DCGW_RD}[2]    @{NETWORK_IP}[0]    @{LABEL}[2]
    Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[0]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[1]
    Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry    @{RD}[1]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[2]
    Wait Until Keyword Succeeds    10s    2s    Verify Routing Entry    @{RD}[2]    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[3]
    Wait Until Keyword Succeeds    10s    2s    Verify FIB Entry    @{NETWORK_IP}[0]    @{NUM_OF_ROUTES}[6]
...
Pre Cleanup Configuration Check on Odl Quagga
    ...
    Write Commands Until Expected Prompt    show running-config    ${DEFAULT_LINUX_PROMPT_STRICT}
    SSHLibrary.Close Connection
...
Delete BGP Neighbor On ODL
    ...
    \    Issue_Command_On_Karaf_Console    ${output}
    \    Issue_Command_On_Karaf_Console    ${BGP_CACHE}
...
Configure BGP Neighbor On ODL
    ...
    \    Issue_Command_On_Karaf_Console    ${add_nbr}

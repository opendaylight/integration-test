...
Verify ECMP path advertised by QBGP to CSC in case of distinct VRF
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Check BGP VPNv4 Nbr On ODL    ${NUM_OF_DCGW}
    : FOR    ${idx}    IN RANGE    0    2
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{DCGW_RD_IRT_ERT}[0]    @{NETWORK_IP}[${idx}]    @{NUM_OF_ROUTES}[0]
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{DCGW_RD_IRT_ERT}[1]    @{NETWORK_IP}[${idx}]    @{NUM_OF_ROUTES}[1]
    ...
    : FOR    ${idx}    IN RANGE    0    2
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{DCGW_RD_IRT_ERT}[0]    @{NETWORK_IP}[${idx}]    @{NUM_OF_ROUTES}[0]
    \    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Routing Entry On ODL    @{DCGW_RD_IRT_ERT}[1]    @{NETWORK_IP}[${idx}]    @{NUM_OF_ROUTES}[0]
    \    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Verify FIB Entry On ODL    @{NETWORK_IP}[${idx}]    @{NUM_OF_ROUTES}[1]
...
Verify ECMP path advertised by QBGP to CSC when routes shared across VPNs
    ...
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[1]    vpnid=@{VPN_ID}[1]    rd=@{L3VPN_RD_IRT_ERT}[1]    exportrt=@{L3VPN_RD_IRT_ERT}[0]    importrt=@{L3VPN_IMPORT_RT}[0]
    VpnOperations.VPN Create L3VPN    name=@{VPN_NAME}[2]    vpnid=@{VPN_ID}[2]    rd=@{L3VPN_RD_IRT_ERT}[2]    exportrt=@{L3VPN_RD_IRT_ERT}[0]    importrt=${L3VPN_IMPORT_RT_123}
...
Verify Route Entry With Nexthop
    ...
    \    BuiltIn.Should Match Regexp    ${output}    \\s*${prefix}\\s*${DCGW_IP_LIST[${index}]}

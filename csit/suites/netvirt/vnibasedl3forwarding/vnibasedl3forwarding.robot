...
VNI Based L3 Forwarding
    ...
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI_NET_0_PORTS}[0]    @{VNI_NET_1_PORTS}[0]    @{VNI_NETWORKS}[0]
...
VNI Based L3 Forwarding With BGPVPN Network Association
    ...
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI_NET_2_PORTS}[0]    @{VNI_NET_3_PORTS}[0]    @{VNI_NETWORKS}[2]
...
VNI Based L3 Forwarding With BGPVPN With Irt Ert
    ...
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI_NET_4_PORTS}[0]    @{VNI_NET_5_PORTS}[0]    @{VNI_NETWORKS}[4]
...
Verify Vni Segmentation Id and Tunnel Id
    ...
    ${egress_tun_id}    ${before_count_egress_port1} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP1_CONN_ID}    ${L3_FIB_TABLE}    direction=${EGRESS}    tun_id=${segmentation_id2}
    ...    dst_mac=${port_mac2}
    Should Be Equal As Numbers    ${segmentation_id2}    ${egress_tun_id}
    ${egress_tun_id}    ${before_count_egress_port2} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP2_CONN_ID}    ${L3_FIB_TABLE}    direction=${EGRESS}    tun_id=${segmentation_id1}
    ...    dst_mac=${port_mac1}
    ...
    ${tun_id}    ${after_count_egress_port1} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP1_CONN_ID}    ${L3_FIB_TABLE}    direction=${EGRESS}    tun_id=${segmentation_id2}
    ...    dst_mac=${port_mac2}
    ...
    ${tun_id}    ${after_count_egress_port2} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP2_CONN_ID}    ${L3_FIB_TABLE}    direction=${EGRESS}    tun_id=${segmentation_id1}
    ...    dst_mac=${port_mac1}

...
${br_name}    br-int
${cmd}    sudo ovs-ofctl -O OpenFlow13 dump-flows br-int | grep table=210
...
Verify the ACL table flow entries by deleting VM's
    ...
    ${abc}    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${cmd}     
...
Create Setup
    ...
    OpenStackOperations.Neutron Security Group Rule Create    @{SECURITY_GROUP}[1]    direction=egress    protocol=icmp    port_range_max=65535    port_range_min=1    remote_ip_prefix=0.0.0.0/0

...
Verify Datapath for Single ELAN with Multiple DPN
    ...
    Create Vm Instance With Port On Compute Node    ${ELAN1_PORT_LIST[1]}    ${VM_INSTANCES_ELAN1[1]}    ${OS_COMPUTE_2_IP}
    #Verify VM active
    : FOR    ${VM}    IN    @{VM_INSTANCES_ELAN1}
    \    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}
    # Get IP address
    ${VM_IP_ELAN1}    ${DHCP_IP1}    Wait Until Keyword Succeeds    30s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_NET10}
    Log    ${VM_IP_ELAN1}
...
Verify Datapath for Multiple ELAN with Multiple DPN
    ...
    Create Port    ${NETWORKS[1]}    ${ELAN2_PORT_LIST[0]}
    Create Port    ${NETWORKS[1]}    ${ELAN2_PORT_LIST[1]}
    Create Vm Instance With Port On Compute Node    ${ELAN2_PORT_LIST[0]}    ${VM_INSTANCES_NET20[0]}    ${OS_COMPUTE_1_IP}
    Create Vm Instance With Port On Compute Node    ${ELAN2_PORT_LIST[1]}    ${VM_INSTANCES_NET30[1]}    ${OS_COMPUTE_2_IP}
    #Verify VM active
    ${VM_list} = Create List    @{VM_INSTANCES_ELAN2}     @{VM_INSTANCES_ELAN3}
    : FOR    ${VM}    IN    @{VM_list}
    \    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}

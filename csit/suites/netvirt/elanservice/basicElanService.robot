...
Verify Datapath After Recreate VM Instance 
     ...
     Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM_INSTANCES_ELAN1[0]}
     Sleep     1212121212121212121212120s
...
Verify Datapath for Multiple ELAN with Multiple DPN
    ...
    Should Contain    ${output}    ${PING_PASS}
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[2]    ${VM_IP_ELAN2[0]}    ping -c 3 ${VM_IP_ELAN3[0]}
    Should Contain    ${output}    ${PING_FAIL}	
...
MultipleElan Testsuit Setup 
    ...
    Log To Console     Get MACAddr for ELAN2 and ELAN3 PORT_LIST
...
Reboot Nova VM
    ...
    Log    ${output}
    Wait Until Keyword Succeeds    35s    10s    Verify VM Is ACTIVE    ${VM}
    Close Connection

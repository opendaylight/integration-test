...
Suite Teardown    SingleElan SuitTeardown
...
Verify Datapath for Single ELAN with Multiple DPN
    ...
    Should Contain    ${output}    ${PING_PASS}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}      @{VM_MACAddr_ELAN1}    ${ELAN_SMACTABLE}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}      @{VM_MACAddr_ELAn1}    ${ELAN_SMACTABLE}
...
SingleElan SuitSetup
    ...
    Create Vm Instance With Port On Compute Node    ${ELAN1_PORT_LIST[1]}    ${VM_INSTANCES_ELAN1[1]}    ${OS_COMPUTE_1_IP}     sg=sg-elanservice

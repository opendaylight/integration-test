...
Verify that table Miss entry for GWMAC table 19 points to table 17 dispatcher table
    ...
    BuiltIn.Should Contain    ${flow_output}    priority=0
    BuiltIn.Should Contain    ${flow_output}    actions=resubmit(,17)
...
Verify the pipeline flow from dispatcher table 17 (L3VPN) to table 19
    ...
    BuiltIn.Should Contain    ${flow_output}    in_port=${port_num_1}
    BuiltIn.Should Contain    ${flow_output}    goto_table:${DISPATCHER_TABLE}
    ...
    BuiltIn.Should Contain    ${flow_output}    ${vpn_id}
    BuiltIn.Should Contain    ${flow_output}    goto_table:${GWMAC_TABLE}
    ...
    BuiltIn.Should Contain    ${flow_output}    set_field:${gw_mac_addr}
    BuiltIn.Should Contain    ${flow_output}    resubmit(,220)
...
Verify that Multiple GWMAC entries in GWMAC table points to FIB table 21 (L3VPN pipeline)
    ...
    ${flow_output} =    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows} |grep table=${GWMAC_TABLE}
    BuiltIn.Should Contain    ${flow_output}    dl_dst=${gw_mac_addr_1}
    BuiltIn.Should Contain    ${flow_output}    actions=goto_table:${L3_TABLE}
...
Verify ARP eth_type entries and actions for ARP request and ARP response are populated on GWMAC table
    ...
    BuiltIn.Should Contain    ${flow_output}    arp_op=${ARP_REQUEST_OPERATIONAL_CODE}
    BuiltIn.Should Contain    ${flow_output}    ${RESUBMIT_VALUE}
...
Verify GWMAC entires are populated with Neutron Router MAC address per network in GWMAC table
    ...
    ${output1} =    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep table=${GWMAC_TABLE}
    Log    ${output1}
    ${output2}=    Issue_Command_On_Karaf_Console    fib-show
    Log    ${output2}
    ${output3}=    Issue_Command_On_Karaf_Console    l3vpn-config-show
    Log    ${output3}
    VpnOperations.Dissociate VPN to Router    routerid=${router_id}    vpnid=${VPN_INSTANCE_ID}
    Sleep    60s
    ${output4}=    Issue_Command_On_Karaf_Console    fib-show
    Log    ${output4}
    ${output5}=    Issue_Command_On_Karaf_Console    l3vpn-config-show
    Log    ${output5}
    ${gw_mac_addr_2} =    OpenStackOperations.Get Port Mac Address From Ip    ${DEFAULT_GATEWAY_IPS[1]}
    ${output} =    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows}
    BuiltIn.Log    ${output}
    ${flow_output} =    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep table=${GWMAC_TABLE}    prompt_timeout=30s
    BuiltIn.Log    ${flow_output}
    BuiltIn.Should Contain    ${flow_output}    dl_dst=${gw_mac_addr_2}
    BuiltIn.Should Contain    ${flow_output}    actions=goto_table:${L3_TABLE}

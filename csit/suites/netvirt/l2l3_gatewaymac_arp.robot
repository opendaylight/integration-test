...
Verify GWMAC entires are populated with Neutron Router MAC address per network in GWMAC table
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    15s    Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${GWMAC_TABLE}    dl_dst=${gw_mac_addr}    actions=goto_table:${L3_TABLE} 
...
Verify Dump Flows For Specific Table
    [Arguments]    ${compute_ip}    ${table_num}    @{matching_paras}    ${flag}=True

...
Verify that Multiple GWMAC entries in GWMAC table points to FIB table 21 (L3VPN pipeline)
    ...
    ${pkt_count_before_ping} =    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${BRIDGE_INTERFACE}    table=${GWMAC_TABLE} |grep dl_dst=${gw_mac_addr_1}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 8 @{NET_2_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping} =    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${BRIDGE_INTERFACE}    table=${GWMAC_TABLE} |grep dl_dst=${gw_mac_addr_1}
    ${pkt_diff} =    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    BuiltIn.Should Be True    ${pkt_diff} > ${PING_COUNT_VALUE}
    ${pkt_count_before_ping} =    OvsManager.Get Packet Count From Table     ${OS_COMPUTE_2_IP}    ${BRIDGE_INTERFACE}    table=${GWMAC_TABLE} |grep dl_dst=${gw_mac_addr_1}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{NET_2_VM_IPS}[0]    ping -c 8 @{NET_1_VM_IPS}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping} =    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_2_IP}    ${BRIDGE_INTERFACE}    table=${GWMAC_TABLE} |grep dl_dst=${gw_mac_addr_1}
    ${pkt_diff} =    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    BuiltIn.Should Be True    ${pkt_diff} > ${PING_COUNT_VALUE}

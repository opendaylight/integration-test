...
Verify ICMP traffic in same network by configuring ICMP ACL rules with remote security group in custom SG
    [Documentation]    Verify ICMP traffic in same network by configuring ICMP ACL rules with remote security group in custom SG
    Log    >>>>>Delete VMs and brings the VMs back with SG2>>>>>>
    Delete Vm Instance    ${DPN1_VM_NAMES[0]}
    Delete Port    ${DPN1_PORT_LIST[0]}
    Delete Vm Instance    ${DPN1_VM_NAMES[1]}
    Delete Port    ${DPN1_PORT_LIST[1]}
    Delete Vm Instance    ${DPN2_VM_NAMES[0]}
    Delete Port    ${DPN2_PORT_LIST[0]}
    Create Port    ${REQ_NETWORKS[0]}    ${DPN1_PORT_LIST[0]}    ${SECURITY_GROUP[1]}
    Create Port    ${REQ_NETWORKS[0]}    ${DPN1_PORT_LIST[1]}    ${SECURITY_GROUP[1]}
    Create Port    ${REQ_NETWORKS[1]}    ${DPN2_PORT_LIST[0]}    ${SECURITY_GROUP[1]}
    Create Vm Instance With Port On Compute Node    ${DPN1_PORT_LIST[0]}    ${DPN1_VM_NAMES[0]}    ${OS_CNTL_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[1]}
     Create Vm Instance With Port On Compute Node    ${DPN1_PORT_LIST[1]}    ${DPN1_VM_NAMES[1]}    ${OS_CNTL_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[1]}
    Create Vm Instance With Port On Compute Node    ${DPN2_PORT_LIST[0]}    ${DPN2_VM_NAMES[0]}    ${OS_CMP2_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[1]}
    sleep    240s
    Wait Until Keyword Succeeds    60s    10s    Verify VM Is ACTIVE    ${DPN1_VM_NAMES[0]}
    Wait Until Keyword Succeeds    60s    10s    Verify VM Is ACTIVE    ${DPN1_VM_NAMES[1]}
    Wait Until Keyword Succeeds    60s    10s    Verify VM Is ACTIVE    ${DPN2_VM_NAMES[0]}
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
...
Verify TCP traffic in same network by configuring TCP ACL rules with remote security group in custom SG
    [Documentation]    Verify TCP traffic in same network by configuring TCP ACL rules with remote security group in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
...
Verify UDP traffic in same network by configuring UDP ACL rules with remote security group in custom SG
    [Documentation]    Verify UDP traffic in same network by configuring UDP ACL rules with remote security group in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
...
Verify TCP traffic in different network by configuring TCP ACL rules with remote security group in custom SG
    [Documentation]    Verify TCP traffic in different network by configuring TCP ACL rules with remote security group in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
...
Verify UDP traffic in different network by configuring UDP ACL rules with remote security group in custom SG
    [Documentation]    Verify UDP traffic in different network by configuring UDP ACL rules with remote security group in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
    ...
    ${vm2_metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${VM2_Port}
    ${vm7_metadata}    Get Metadata    ${OS_CMP2_CONN_ID}    ${VM7_Port}
    Log    >>>>>>Test UDP traffic>>>>>>
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[1]}    ${VM1_IP}    ${VM7_IP}    12345    Hello

Verify ICMP traffic in different network by configuring ICMP ACL rules with remote ip prefix in custom SG
    [Documentation]    Verify ICMP traffic in different network by configuring ICMP ACL rules with remote ip prefix in custom SG
    Log    >>>>>Delete VMs and brings the VMs back with SG2>>>>>>
    Update Port    ${DPN1_PORT_LIST[0]}    --no-security-group
    Update Port    ${DPN1_PORT_LIST[0]}    --security-group ${SECURITY_GROUP[2]}
    Update Port    ${DPN1_PORT_LIST[1]}    --no-security-group
    Update Port    ${DPN1_PORT_LIST[1]}    --security-group ${SECURITY_GROUP[2]}
    Update Port    ${DPN2_PORT_LIST[0]}    --no-security-group
    Update Port    ${DPN2_PORT_LIST[0]}    --security-group ${SECURITY_GROUP[2]}
    Sleep    60s
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ...
    @{validation}    Create List    ${vm1_metadata}    goto_table:210
    ${pckt_before_ping_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}   @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_before_ping_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}   @{validation}
    Log    >>>>>>Ping from VM1 in DPN1 to VM7 in DPN2 with diff subnet>>>>>>
    ${output} =    OpenStackOperations.Execute Command on VM Instance    ${REQ_NETWORKS[0]}    ${VM1_IP}    ping -c 3 ${VM7_IP}
    BuiltIn.Should Contain    ${output}    64 bytes
    Sleep    5s
    Log    >>>>>Data path validation>>>>>>
    ...
    ${pckt_after_ping_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}    @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_after_ping_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}    @{validation}
    ${pkt_cnt_diff_egress}    Evaluate    ${pckt_after_ping_egress}-${pckt_before_ping_egress}
    Should Be True    ${pkt_cnt_diff_egress}==0
    ${pkt_cnt_diff_ingress}    Evaluate    ${pckt_after_ping_ingress}-${pckt_before_ping_ingress}
    Should Be True    ${pkt_cnt_diff_ingress}==0

Verify TCP traffic in different network by configuring TCP ACL rules with remote ip prefix in custom SG
    [Documentation]    Verify TCP traffic in different network by configuring TCP ACL rules with remote ip prefix in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
    ...
    ${pckt_before_tcp_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}    @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_before_tcp_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}   @{validation}
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[1]}    ${VM1_IP}    ${VM7_IP}    12345    Hello    tcp
    Sleep    5s
    Log    >>>>>Data path validation after TCP traffic>>>>>>
    @{validation}    Create List    ${vm1_metadata}    goto_table:210
    ${pckt_after_tcp_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}    @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_after_tcp_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}   @{validation}
    ${pkt_cnt_diff_egress}    Evaluate    ${pckt_after_tcp_egress}-${pckt_before_tcp_egress}
    Should Be True    ${pkt_cnt_diff_egress}==0
    ${pkt_cnt_diff_ingress}    Evaluate    ${pckt_after_tcp_ingress}-${pckt_before_tcp_ingress}
    Should Be True    ${pkt_cnt_diff_ingress}==0

Verify UDP traffic in different network by configuring UDP ACL rules with remote ip prefix in custom SG
    [Documentation]    Verify UDP traffic in different network by configuring UDP ACL rules with remote ip prefix in custom SG
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM2_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${DPN1_VM_NAMES[1]}
    ${VM7_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${DPN2_VM_NAMES[0]}
    ${VM1_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[0]}
    ...
    ${pckt_before_udp_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}    @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_before_udp_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}   @{validation}
    Log    >>>>>>Test UDP traffic between VM1 in DPN1 to VM7 in DPN2 with diff subnet>>>>>>
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[1]}    ${VM1_IP}    ${VM7_IP}    12345    Hello
    Log    >>>>>Data path validation after UDP traffic>>>>>>
    sleep    5s
    ...
    ${pckt_after_udp_egress}    Data Path Validation Egress Dispatcher Table    ${br_name}    ${OS_CMP1_CONN_ID}    ${egress_table_list[0]}    @{validation}
    @{validation}    Create List    ${vm7_metadata}    output:
    ${pckt_after_udp_ingress}    Data Path Validation Ingress Dispatcher Table    ${br_name}    ${OS_CMP2_CONN_ID}    ${igress_table_list[0]}   @{validation}
    ${pkt_cnt_diff_egress}    Evaluate    ${pckt_after_udp_egress}-${pckt_before_udp_egress}
    Should Be True    ${pkt_cnt_diff_egress}==0
    ${pkt_cnt_diff_ingress}    Evaluate    ${pckt_after_udp_ingress}-${pckt_before_udp_ingress}
    Should Be True    ${pkt_cnt_diff_ingress}==0

Create port with Allowed address pair and Security Group
    [Documentation]    Create port with Allowed address pair and Security Group
    Log    >>>>>Code to get the sg id out of SG1>>>>>
    ${sgid}    Get Security Group Id    ${SECURITY_GROUP[0]}    ${OS_CMP1_CONN_ID}
    ${Allowed_Address_Pair_Args}    Set Variable    --allowed-address ip-address=${AAP_PARAMS[0]},mac-address=${AAP_PARAMS[1]}
    Create Port    ${REQ_NETWORKS[0]}    ${DPN1_PORT_LIST[2]}    ${SECURITY_GROUP[0]}    additional_args=${Allowed_Address_Pair_Args}
    Create Vm Instance With Port On Compute Node    ${DPN1_PORT_LIST[2]}    ${AAP_VM}    ${OS_CNTL_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]}
    ${Port_sg_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Contain    ${Port_sg_output}    ${sgid}
    ${Port_aap_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Contain    ${Port_aap_output}    ${AAP_PARAMS[0]}    ${AAP_PARAMS[1]}
    ${VM5_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${AAP_VM}
    ${VM5_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[2]}
    Check In Port    ${VM5_Port}   ${OS_CMP1_CONN_ID}
    ${vm5_metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${VM5_Port}
    Log    >>>>>Table validation for AAP>>>>>>
    @{validation_list}    create list    ${AAP_PARAMS[0]}    ${AAP_PARAMS[1]}    ${egress_table_list[2]}
    Table Check    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]} | grep ${vm5_metadata} | grep ${AAP_PARAMS[0]}    ${validation_list}

Update port with Allowed address pair and Security Group
    [Documentation]    Update port with Allowed address pair and Security Group
    Log    >>>>>Code to get the sg id out of SG2>>>>>
    ${sgid}    Get Security Group Id    ${SECURITY_GROUP[1]}    ${OS_CMP1_CONN_ID}
    Update Port    ${DPN1_PORT_LIST[2]}    additional_args=--security-group=SG2
    ${Port_sg_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Contain    ${Port_sg_output}    ${sgid}
    Update Port    ${DPN1_PORT_LIST[2]}    additional_args=--allowed-address ip-address=${AAP_PARAMS[2]}
    ${Port_aap_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Contain    ${Port_aap_output}    ${AAP_PARAMS[2]}
    ${VM5_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[2]}
    ${vm5_metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${VM5_Port}
    Log    >>>>>Table validation for AAP>>>>>>
    @{validation_list}    create list    ${AAP_PARAMS[2]}    ${egress_table_list[2]}
    Table Check    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]} | grep ${vm5_metadata} | grep ${AAP_PARAMS[2]}   ${validation_list}
    @{validation_list}    create list    ${AAP_PARAMS[0]}    ${AAP_PARAMS[1]}    ${egress_table_list[2]}
    Table Check    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]} | grep ${vm5_metadata} | grep ${AAP_PARAMS[0]}    ${validation_list}

Update port with allowed address pair removed
    [Documentation]    Update port with allowed address pair removed
    Update Port    ${DPN1_PORT_LIST[2]}    additional_args=--no-allowed-address
    ${Port_aap_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Not Contain    ${Port_aap_output}    ${AAP_PARAMS[2]}
    ${VM5_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[2]}
    ${vm5_metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${VM5_Port}
    Log    >>>>>Table validation for AAP>>>>>>
    @{validation_list}    create list    ${AAP_PARAMS[2]}    ${AAP_PARAMS[0]}    ${AAP_PARAMS[1]}
    Table Check With Negative Scenario    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]} | grep ${vm5_metadata}    ${validation_list}

Delete port with Allowed address pair
    [Documentation]    Delete port with Allowed address pair
    Update Port    ${DPN1_PORT_LIST[2]}    additional_args=--allowed-address ip-address=${AAP_PARAMS[2]}
    ${Port_aap_output}    Show Port    ${DPN1_PORT_LIST[2]}
    BuiltIn.Should Contain    ${Port_aap_output}    ${AAP_PARAMS[2]}
    ${VM5_Port}    In Port VM    ${OS_CMP1_CONN_ID}    ${br_name}    ${DPN1_PORT_LIST[2]}
    ${vm5_metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${VM5_Port}
    @{validation_list}    create list    ${AAP_PARAMS[2]}    ${egress_table_list[2]}
    Table Check    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]} | grep ${vm5_metadata}    ${validation_list}
    Delete Vm Instance    ${AAP_VM}
    @{validation_list}    create list    ${AAP_PARAMS[2]}
    Table Check With Negative Scenario    ${OS_CMP1_CONN_ID}    ${br_name}    ${egress_table_list[1]}    ${validation_list}

Create port with invalid Allowed address pair
    [Documentation]    Create port with invalid Allowed address pair
    ${Allowed_Address_Pair_Args}    Set Variable    --allowed-address ip-address=50.50.50.x0
    ${status}    ${value}    Run Keyword And Ignore Error    Create Port    ${REQ_NETWORKS[1]}    ${DPN2_PORT_LIST[2]}    ${SECURITY_GROUP[0]}     additional_args=${Allowed_Address_Pair_Args}
    Log    ${status}
    Log    ${value}

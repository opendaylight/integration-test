...
Verify TCP traffic in same subnet and different subnet across DPNs with default security group
    ...
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[0]}    ${VM_IP_NET1[0]}    ${VM_IP_NET1[1]}    12345    Hello    tcp
    Sleep    5s
    ...
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[1]}    ${VM_IP_NET1[0]}    ${VM_IP_NET2[0]}    12345    Hello    tcp
...
Verify UDP traffic in same subnet and different subnet across DPNs with default security group
    ...
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[0]}    ${VM_IP_NET1[0]}    ${VM_IP_NET1[1]}    12345    Hello
    ...
    Verify TCP/UDP Traffic Using Netcat    ${REQ_NETWORKS[0]}    ${REQ_NETWORKS[1]}    ${VM_IP_NET1[0]}    ${VM_IP_NET2[0]}    12345    Hello
...
Check Vm Instances Have Ip Address
    @{VM_IP_NET1}    ${NET1_DHCP_IP} =    Get VM IPs    @{DPN1_VM_NAMES}
    @{VM_IP_NET2}    ${NET2_DHCP_IP} =    Get VM IPs    @{DPN2_VM_NAMES}
...
Create Nova VMs
    ...
    Create Vm Instance With Port On Compute Node    ${DPN1_PORT_LIST[0]}    ${DPN1_VM_NAMES[0]}    ${OS_CNTL_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]}
    Create Vm Instance With Port On Compute Node    ${DPN1_PORT_LIST[1]}    ${DPN1_VM_NAMES[1]}    ${OS_CNTL_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]}
...
Get Metadata
    ...
    Switch Connection    ${conn_id}
    ${grep_metadata}    Write Commands Until Expected Prompt    sudo ovs-ofctl dump-flows -O Openflow13 ${br_name}| grep table=0 | grep in_port=${port} | awk '{print$7}'    $    30s
...
Verify TCP/UDP Traffic Using Netcat
    ...
    ${Server_Command}    Set Variable    ${proto_arg} -s ${vm1_ip} -l -p ${port_no} -v
    ${Client_Command}    Set Variable    ${proto_arg} ${vm1_ip} ${port_no} -v
     ...
     Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    ${net_name2}    ${vm2_ip}    echo Opendaylight|nc ${vm1_ip} 12345 &

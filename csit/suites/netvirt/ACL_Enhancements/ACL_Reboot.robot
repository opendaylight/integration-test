...
Verify ICMP traffic with between two VMs with Security Group after restarting the OVSDB in Multi-DPN 
    ...
    ${abc}     Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[0]
    ...
    ${def}     Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    ping -c 3 @{NET_2_VM_IPS}[0]
    Should Contain    ${def}    0% packet loss
    ${ijk}    OpenStack CLI    sudo ovs-ofctl -O OpenFlow13 dump-flows br-int | grep table=210
    Should Contain    ${ijk}    nw_src=@{NET_1_VM_IPS}[0]
...
Verify TCP/UDP traffic with between two VMs with Security Group after restarting the OVSDB in Multi-DPN
    ...
    Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    nc -l -p 12345 > abc &
    Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    @{NET_2_VM_IPS}[0]    echo Opendaylight|nc @{NET_1_VM_IPS}[0] 12345 &
    ${abc}    Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    cat abc
    ...
    Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET_1_VM_IPS}[0]    nc -l -p 12346 > xyz &
    Wait Until Keyword Succeeds    240 sec    60 sec    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[1]    @{NET_2_VM_IPS}[0]    echo Opendaylight|nc @{NET_1_VM_IPS}[0] 12346 &

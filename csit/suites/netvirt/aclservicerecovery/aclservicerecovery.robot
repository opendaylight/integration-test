...
ACL Service Recovery CLI
    ...
    ${count_before} =    Get Dump Flows Count    ${OS_CMP1_CONN_ID}    ${INGRESS_ACL_REMOTE_ACL_TABLE}
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify Dump Flows Count    ${count_before}    ${INGRESS_ACL_REMOTE_ACL_TABLE}

ACL Instance Recovery CLI
    [Documentation]    This test case covers ACL instance recovery.
    ${count_before} =    Get Dump Flows Count    ${OS_CMP1_CONN_ID}    ${EGRESS_LEARN_ACL_FILTER_TABLE}
    ${node_id} =    OVSDB.Get DPID    ${OS_COMPUTE_1_IP}
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify Dump Flows Count    ${count_before}    ${EGRESS_LEARN_ACL_FILTER_TABLE}
...
ACL Interface Recovery CLI
    ...
    ${count_before} =    Get Dump Flows Count    ${OS_CMP1_CONN_ID}    ${EGRESS_ACL_TABLE}    port_mac=${port_mac}
    ${node_id}    OVSDB.Get DPID    ${OS_COMPUTE_1_IP}
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify Dump Flows Count    ${count_before}    ${EGRESS_ACL_TABLE}    port_mac=${port_mac}
...
Verify Dump Flows Count
    ...
    [Documentation]    Verify dump flows count should be same as before and after for a table id.
    BuiltIn.Run Keyword If    ${Kwargs}    BuiltIn.Log    ${Kwargs}
    ${port_mac} =    BuiltIn.Run Keyword If    ${Kwargs}    Collections.Pop From Dictionary    ${Kwargs}    port_mac    default=${None}
...
Start Suite
    ...
    SetupUtils.Setup_Logging_For_Debug_Purposes_On_List_Or_All    ${TEST_LOG_LEVEL}    @{TEST_LOG_COMPONENTS}[0]
    SetupUtils.Setup_Logging_For_Debug_Purposes_On_List_Or_All    ${TEST_LOG_LEVEL}    @{TEST_LOG_COMPONENTS}[1]
    SetupUtils.Setup_Logging_For_Debug_Purposes_On_List_Or_All    ${TEST_LOG_LEVEL}    @{TEST_LOG_COMPONENTS}[2]

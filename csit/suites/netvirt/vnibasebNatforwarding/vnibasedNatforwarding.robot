...
Test Netcat Operations Between Vm Instances
    ...
    ${nc_output} =    OpenStackOperations.Execute Command on VM Instance    ${net_name}    ${server_ip}    ( ( echo "${server_data}" | sudo timeout 60 nc -l ${additional_args} ${port} ) & )
    BuiltIn.Log    ${output}
    ${nc_output} =    OpenStackOperations.Execute Command on VM Instance    ${net_name}    ${client_ip}    sudo echo "${client_data}" | nc -v -w 5 ${additional_args} ${dest_ip} ${port}
    BuiltIn.Log    ${output}

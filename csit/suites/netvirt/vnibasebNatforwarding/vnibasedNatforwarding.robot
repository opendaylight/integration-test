...
Verify Nat Based Vni Segmentation Id and Tunnel Id
    ...
    ${egress_tun_id}    ${before_count_egress_port1} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP1_CONN_ID}    ${L3_TABLE}    tun_id=${external_router_vni_value}
    ...    mac=@{VM_FLOATING_IPS}[1]
    BuiltIn.Should Be Equal As Numbers    ${external_router_vni_value}    ${egress_tun_id}
    ${egress_tun_id}    ${before_count_egress_port2} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP2_CONN_ID}    ${L3_TABLE}    tun_id=${external_router_vni_value}
    ...    mac=@{VM_FLOATING_IPS}[0]
    BuiltIn.Should Be Equal As Numbers    ${external_router_vni_value}    ${egress_tun_id}
...
Verify Snat Based Vni Segmentation Id and Tunnel Id
    ...
    ${tunnel_id} =    BuiltIn.Set Variable    @{output}[0]
    ${tunnel_id} =    BuiltIn.Convert To Integer    ${tunnel_id}    16    
...
Get Vni Tunnel Id And Packet Count After Traffic
    ...
    ${egress_tun_id}    ${after_count_egress_port2} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP2_CONN_ID}    ${L3_TABLE}    tun_id=${external_router_vni_value}
    ...    mac=@{VM_FLOATING_IPS}[0]
    ${egress_tun_id}    ${after_count_egress_port1} =    OVSDB.Get Tunnel Id And Packet Count    ${OS_CMP1_CONN_ID}    ${L3_TABLE}    tun_id=${external_router_vni_value}
    ...    mac=@{VM_FLOATING_IPS}[1]
    BuiltIn.Should Be Equal As Numbers    ${external_router_vni_value}    ${egress_tun_id}

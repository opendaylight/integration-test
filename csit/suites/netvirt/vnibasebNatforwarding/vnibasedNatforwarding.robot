...
Vni Based NAT Forwarding SNAT To DNAT
    ...
    BuiltIn.Run Keyword If    "${dpn_id1}" == "${napt_switch_id}"    Verify Snat Based Vni Segmentation Id and Tunnel Id    ${OS_CNTL_CONN_ID}    ${OS_CMP1_CONN_ID}    ${OS_CMP1_HOSTNAME}    @{VM_FLOATING_IPS}[1]
    BuiltIn.Run Keyword If    "${dpn_id2}" == "${napt_switch_id}"    Verify Snat Based Vni Segmentation Id and Tunnel Id    ${OS_CMP1_CONN_ID}    ${OS_CMP2_CONN_ID}    ${OS_CMP2_HOSTNAME}    @{VM_FLOATING_IPS}[0]
    BuiltIn.Run Keyword If    "${dpn_id3}" == "${napt_switch_id}"    Verify Snat Based Vni Segmentation Id and Tunnel Id    ${OS_CMP2_CONN_ID}    ${OS_CMP1_CONN_ID}    ${OS_CMP1_HOSTNAME}    @{VM_FLOATING_IPS}[1]
...
Verify Snat Based Vni Segmentation Id and Tunnel Id
    [Arguments]    ${napt_switch_id}    ${non_napt_switch_id}    ${host_name}    ${floating_ip}
    [Documentation]    verify VNI id for L3 Unicast frames exchanged with NAT Configured.
    ...
    ${router_vni_value_hex} =    BuiltIn.Convert To Hex    prefix=0x    lowercase=yes    ${router_vni_value}
    ...
    ${base_cmd} =    BuiltIn.Set Variable    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=${INTERNAL_TUNNEL_TABLE} | grep ${router_vni_value_hex}
    ...
    ${base_cmd} =    BuiltIn.Set Variable    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 | grep table=${PSNAT_TABLE} | grep ${router_vni_value_hex}
    ...
    ${tunnel_id} =    BuiltIn.Convert To Integer    ${output}    16
    BuiltIn.Should Be Equal As Numbers    ${router_vni_value}    ${tunnel_id}
    OpenStackOperations.Test Netcat Operations From Vm Instance    ${VNI_NAT_NETWORKS}    @{NET_VM_IPS}[2]    ${floating_ip}

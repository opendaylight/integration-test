...
To Verify ELAN Service Recovery
    ...
    ${flow_id} =    FlowLib.Get OpenFlow Id    ${DPNID_1}    ${ELAN_SMAC_TABLE}    @{PORT_MAC_ADDR}[0]
    FlowLib.Delete Flow Via Restconf    ${DPNID_1}    ${ELAN_SMAC_TABLE}    ${flow_id}
    #${flow_id2} =    FlowLib.Get OpenFlow Id    ${DPNID_2}    ${ELAN_SMAC_TABLE}    @{PORT_MAC_ADDR}[2]
    #FlowLib.Delete Flow Via Restconf    ${DPNID_2}    ${ELAN_SMAC_TABLE}    ${flow_id2}
    ${table_id} =   BuiltIn.Set Variable    ${ELAN_SMAC_TABLE}
    BuiltIn.Set Suite Variable    ${table_id}
    ${switch_idx} = BuiltIn.Set Variable    ${DPNID_1}
    BuiltIn.Set Suite Variable    ${switch_idx}
    BuiltIn.Set Suite Variable    ${flow_id}
    ${flow_id} =    FlowLib.Get OpenFlow Id    ${DPNID_2}    ${ELAN_SMAC_TABLE}    @{PORT_MAC_ADDR}[2]
    FlowLib.Delete Flow Via Restconf    ${DPNID_2}    ${ELAN_SMAC_TABLE}    ${flow_id}
    FlowLib.Check Config Flow    ${False}    @{PORT_MAC_ADDR}[0]
    ${switch_idx} = BuiltIn.Set Variable    ${DPNID_2}
    BuiltIn.Set Suite Variable    ${switch_idx}
    BuiltIn.Set Suite Variable    ${flow_id}
    FlowLib.Check Config Flow    ${False}    @{PORT_MAC_ADDR}[2]  
    #FlowLib.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    False    @{PORT_MAC_ADDR}[0]
    #FlowLib.Verify Flows on ODL    ${DPNID_2}    ${ELAN_SMAC_TABLE}    False    @{PORT_MAC_ADDR}[2]
    ...
    KarafKeywords.Issue Command On Karaf Console    ${SHOW-SRM-STATUS}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_2_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[2]    actions=goto_table:${ELAN_DMACTABLE}
    FlowLib.Check Config Flow    ${True}    @{PORT_MAC_ADDR}[2]
    ${flow_id} =    FlowLib.Get OpenFlow Id    ${DPNID_1}    ${ELAN_SMAC_TABLE}    @{PORT_MAC_ADDR}[0]
    ${switch_idx} = BuiltIn.Set Variable    ${DPNID_1}
    BuiltIn.Set Suite Variable    ${switch_idx}
    BuiltIn.Set Suite Variable    ${flow_id}
    FlowLib.Check Config Flow    ${True}    @{PORT_MAC_ADDR}[0]
    #FlowLib.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    True    @{PORT_MAC_ADDR}[0]
    #FlowLib.Verify Flows on ODL    ${DPNID_2}    ${ELAN_SMAC_TABLE}    True    @{PORT_MAC_ADDR}[2]
...
To Verify Elan Interface recovery
    ...
    FlowLib.Check Config Flow    ${True}    @{PORT_MAC_ADDR}[0]
    #BuiltIn.Wait Until Keyword Succeeds    10s    5s    FlowLib.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    True    @{PORT_MAC_ADDR}[0]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}

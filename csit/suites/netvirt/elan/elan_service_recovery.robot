...
${SRM-STATUS-COMMAND}    srm:recover service elan
${INTERFACE-STATUS-COMMAND}    srm:recover instance elan-interface
...
To Verify ELAN Service Recovery
    ...
    KarafKeywords.Issue Command On Karaf Console    ${SRM-STATUS-COMMAND}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_2_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[2]    actions=goto_table:${ELAN_DMACTABLE}
...
To Verify Elan Interface recovery
    ...
    BuiltIn.Wait Until Keyword Succeeds    10s    5s    FlowLib.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    True
    ...    @{PORT_MAC_ADDR}[0]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}
...
Recover Elan Flows
    ...
    ${recover_msg} =    KarafKeywords.Issue Command On Karaf Console    ${INTERFACE-STATUS-COMMAND} ${interface}

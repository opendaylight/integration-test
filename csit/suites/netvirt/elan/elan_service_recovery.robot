...
To Verify ELAN Service Recovery
    ...
    OVSDB.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    False    @{PORT_MAC_ADDR}[0]
    OVSDB.Verify Flows on ODL    ${DPNID_2}    ${ELAN_SMAC_TABLE}    False    @{PORT_MAC_ADDR}[2]
    ...
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_2_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[2]    actions=goto_table:${ELAN_DMACTABLE}
    OVSDB.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    True    @{PORT_MAC_ADDR}[0]
    OVSDB.Verify Flows on ODL    ${DPNID_2}    ${ELAN_SMAC_TABLE}    True    @{PORT_MAC_ADDR}[2]
...
To Verify Elan Interface recovery
    ...
    BuiltIn.Wait Until Keyword Succeeds    10s    5s    OVSDB.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    True
    ...    @{PORT_MAC_ADDR}[0]
    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=@{PORT_MAC_ADDR}[0]    actions=goto_table:${ELAN_DMACTABLE}

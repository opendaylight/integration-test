...
To Verify ELAN Service Recovery
    ...
    ${mac_elements} =    Collections.Convert To List    @{PORT_MAC_ADDR}[0]
    Utils.Check For Elements At URI    ${CONFIG_NODES_API}/node/openflow:${DPNID_1}/table/${ELAN_SMAC_TABLE}    ${mac_elements}
    ${mac_elements} =    Collections.Convert To List    @{PORT_MAC_ADDR}[2]
...
To Verify Elan Interface recovery
    ...
    Recover Elan Flows    ${elan_interface_id}    @{PORT_MAC_ADDR}[0]    #BuiltIn.Wait Until Keyword Succeeds    10s    5s    FlowLib.Verify Flows Not Present In Config
    ...    # ${DPNID_1}    ${ELAN_SMAC_TABLE}    True
    ${mac_elements} =    Collections.Convert To List    @{PORT_MAC_ADDR}[0]

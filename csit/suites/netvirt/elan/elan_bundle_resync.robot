...
Verify the Bundle based reconciliation with multiple restarts (OVS1)followed by resync check
    ...
    : FOR    ${index}    IN RANGE    0    3
    \    OVSDB.Restart OVSDB    ${OS_CMP1_IP}
    \    BuiltIn.Wait Until Keyword Succeeds    15s    5s    Verify Elan Flows    ${SMAC_LIST_DPN1}    
    \    ...    ${dmac_list}    ${OS_CMP1_IP}
    \    BuiltIn.Wait Until Keyword Succeeds    15s    5s    Verify Elan Flows    ${SMAC_LIST_DPN2}    
    \    ...    ${dmac_list}    ${OS_CMP2_IP}
    \    Check Karaf Message
    Verify Elan Traffic

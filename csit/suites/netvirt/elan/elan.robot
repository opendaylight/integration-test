...
Create Single Elan
    ...
    Builtin.BuiltIn.Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${PORT_URL}    ${NET_1_PORTS}
    ...
    @{NET_1_MACS} =    BuiltIn.BuiltIn.Wait Until Keyword Succeeds    30s    10s    OpenStackOperations.Get Ports MacAddr    ${NET_1_PORTS}
...
Verify Datapath for Multiple ELAN with Multiple DPN
    ...
    BuiltIn.Should Contain    ${output}    ${PING_PASS}
    [Teardown]    Run Keywords    OpenStackOperations.Get Test Teardown Debugs
...
MultipleElan Testsuite Setup
    ...
    Builtin.BuiltIn.Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${PORT_URL}    ${NET_3_PORTS}
    ...
    @{VM_MACAddr_elan_net_2}    BuiltIn.BuiltIn.Wait Until Keyword Succeeds    30s    10s    OpenStackOperations.Get Ports MacAddr    ${NET_2_PORTS}
    Builtin.Set Suite Variable    @{VM_MACAddr_elan_net_2}
    @{VM_MACAddr_elan_net_3}    BuiltIn.BuiltIn.Wait Until Keyword Succeeds    30s    10s    OpenStackOperations.Get Ports MacAddr    ${NET_3_PORTS}
...
Verify Flows Are Removed For ELAN Service
    ...
    Builtin.Log    ${dmac_output}
    : FOR    ${dmac}    IN    @{smacs}
    \    ${resp} =    Builtin.BuiltIn.Should Not Contain    ${dmac_output}    ${dmac}

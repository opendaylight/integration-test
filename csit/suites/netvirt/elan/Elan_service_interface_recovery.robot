...
To Verify ELAN Service Recovery
    ...
    BuiltIn.Wait Until Keyword Succeeds    10s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    False    ${EMPTY}    dl_src=${PORT_MAC_ADDR[0]}
    BuiltIn.Wait Until Keyword Succeeds    10s    5s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_2_IP}    ${ELAN_SMAC_TABLE}    False    ${EMPTY}    dl_src=${PORT_MAC_ADDR[2]}
...
To Verify Elan Interface recovery 
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    15s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    True    ${EMPTY}    dl_src=${PORT_MAC_ADDR[0]}    actions=goto_table:${ELAN_DMACTABLE}
    OVSDB.Delete Flows From ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    ${PORT_MAC_ADDR[0]}
    OVSDB.Verify Flows on ODL    ${DPNID_1}    ${ELAN_SMAC_TABLE}    ${PORT_MAC_ADDR[0]}    False
    BuiltIn.Wait Until Keyword Succeeds    60s    15s    OVSDB.Verify Dump Flows For Specific Table    ${OS_COMPUTE_1_IP}    ${ELAN_SMAC_TABLE}    False    ${EMPTY}    dl_src=${PORT_MAC_ADDR[0]}

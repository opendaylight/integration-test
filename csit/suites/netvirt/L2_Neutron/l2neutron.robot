...
Create BGP and Add Neighbor On DCGW
    [Arguments]    ${BGP_AS_ID}    ${ROUTER_ID}    ${NEIGHBOR_IP}    ${conn_id}=${dcgw_conn_id}
    [Documentation]    Execute cmd on DCGW and returns the ouput.
    DCGW Connect Telnet Session    ${conn_id}    bgpd    sdncbgpc
    Execute Command On DCGW Telnet Session    configure terminal
    Execute Command On DCGW Telnet Session    router bgp ${BGP_AS_ID}
    ...
    Execute Command On DCGW Telnet Session    neighbor ${NEIGHBOR_IP} remote-as ${BGP_AS_ID}
...
Create BGP Config On DCGW
    ...
    Configure BGP And Add Neighbor On DCGW    ${TOOLS_SYSTEM_IP}    ${BGP_AS_ID}    ${TOOLS_SYSTEM_IP}    ${HA_PROXY_IP}    ${VPN_NAME[0]}    ${DCGW_RD}
    ...    ${LOOPBACK_IP}
    ...
    Log    ${output}
    ${output1} =    Execute Show Command On Quagga    ${TOOLS_SYSTEM_IP}    show ip bgp vrf ${DCGW_RD}
    Log    ${output1}

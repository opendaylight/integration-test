...
@{ELAN1_PORT_LIST}    PORT1    PORT2
@{ELAN2_PORT_LIST}    PORT3    PORT4
...
TC01 Broadcast/Unicast testing with L3VPN service
    ...
    ${ELAN_Packets_1}    ${VPN_Packets_1}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN1[1]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_2}    ${VPN_Packets_2}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN1[1]}
    Should Be True    ${ELAN_Packets_2} > ${ELAN_Packets_1}
    Should Be True    ${VPN_Packets_1} == ${VPN_Packets_2}
    ${ELAN_Packets_3}    ${VPN_Packets_3}    Get PacketCount from Flow Table    ${OS_COMPUTE_2_IP}    ${VM_IP_NET20[1]}    ${VM_MACAddr_ELAN2[1]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET20[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_4}    ${VPN_Packets_4}    Get PacketCount from Flow Table    ${OS_COMPUTE_2_IP}    ${VM_IP_NET20[1]}    ${VM_MACAddr_ELAN2[1]}
...
TC02 Multicast/Unicast testing with L3VPN service
    ...
    ${RX_Packets_CPN1_2}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[1]}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_1} < ${RX_Packets_CPN1_2}
    ${ELAN_Packets_1}    ${VPN_Packets_1}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[0]}    ${VM_MACAddr_ELAN1[1]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_2}    ${VPN_Packets_2}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN1[1]}
    Should Be True    ${ELAN_Packets_2} > ${ELAN_Packets_1}

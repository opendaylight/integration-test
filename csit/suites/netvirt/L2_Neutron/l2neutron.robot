...
TC01 Broadcast/Unicast testing with L3VPN service
    ...
    ${ELAN_Packets_1}    ${VPN_Packets_1}    ${ARP_Packets_1}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET20[0]}    ${VM_MACAddr_ELAN1[1]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET20[0]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_2}    ${VPN_Packets_2}    ${ARP_Packets_2}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET20[0]}    ${VM_MACAddr_ELAN1[1]}
    Should Be True    ${ELAN_Packets_2} > ${ELAN_Packets_1}
    Should Be True    ${VPN_Packets_1} == ${VPN_Packets_2}
    ${ELAN_Packets_3}    ${VPN_Packets_3}    ${ARP_Packets_3}    Get PacketCount from Flow Table    ${OS_COMPUTE_2_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_4}    ${VPN_Packets_4}    ${ARP_Packets_4}    Get PacketCount from Flow Table    ${OS_COMPUTE_2_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
...
Get PacketCount from Flow Table
    ...
    ${ARP_RESPONSE_REGEX} =    Set Variable    table=${ARP_RESPONSE_TABLE}, n_packets=\\d+,\\s.*,arp_tpa=${dest_ip},arp_op=1 actions=.*,set_field:${dest_mac}->eth_src
    ...
    Log    ${n_packets_L3VPN}
    ${flowEntry} =    Get Regexp Matches    ${flowOutput}    ${ARP_RESPONSE_REGEX}
    Log    ${flowEntry}
    ${match} =    Get Regexp Matches    ${flowEntry[0]}    ${PACKET_COUNT_REGEX}
    Log    ${match}
    ${n_packets} =    Split String    ${match[0]}    separator==
    ${n_packets_ARP}=    Get from List    ${n_packets}    1
    ${n_packets_ARP} =    Convert To Integer    ${n_packets_ARP}
    Log    ${n_packets_ARP}
    [Return]    ${n_packets_ELAN}    ${n_packets_L3VPN}    ${n_packets_ARP}

...
TC01 Broadcast/Unicast testing with L3VPN service
    ...
    ${vm_instances} =    Create List    @{VM_IP_DPN1}    @{VM_IP_DPN2}
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${VM_IP_DPN1}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_2_IP}    ${VM_IP_DPN2}
    Log    L3 Datapath test across the networks using router
    ${dst_ip_list} =    Create List    ${VM_IP_DPN1[1]}    @{VM_IP_DPN2}
    Log    ${dst_ip_list}
    Test Operations From Vm Instance    ${NETWORKS[0]}    ${VM_IP_DPN1[1]}    ${dst_ip_list}
    ${dst_ip_list} =    Create List    ${VM_IP_DPN2[1]}    @{VM_IP_DPN1}
    Log    ${dst_ip_list}
    Test Operations From Vm Instance    ${NETWORKS[1]}    ${VM_IP_DPN2[0]}    ${dst_ip_list}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_DPN1[0]}    ping -c 3 ${BROADCAST_IP[0]}
    Should Contain    ${output}    64 bytes
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_DPN1[0]}    ping -c 3 ${VM_IP_DPN2[1]}
    Should Contain    ${output}    64 bytes

...
TC01 Broadcast/Unicast testing with L3VPN service
    ...
    ${ELAN_Packets_1}    ${VPN_Packets_1}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_2}    ${VPN_Packets_2}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
    Should Be True    ${ELAN_Packets_2} > ${ELAN_Packets_1}
    Should Be True    ${VPN_Packets_1} == ${VPN_Packets_2}
    ${ELAN_Packets_3}    ${VPN_Packets_3}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_4}    ${VPN_Packets_4}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
...
TC02 Multicast/Unicast testing with L3VPN service
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN Service    ${OS_COMPUTE_1_IP}    ${SRCMAC_CPN1}    ${VM_MACAddr_ELAN1}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN Service    ${OS_COMPUTE_2_IP}    ${SRCMAC_CPN2}    ${VM_MACAddr_ELAN2}
    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[1]}    sudo ifconfig ${vm_int} multicast
    Execute Command on VM Instance    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    sudo ifconfig ${vm_int} multicast
    Execute Command on VM Instance    ${NETWORKS}[1]    ${VM_IP_NET10[0]}    sudo ifconfig ${vm_int} -multicast
    Execute Command on VM Instance    ${NETWORKS}[1]    ${VM_IP_NET20[1]}    sudo ifconfig ${vm_int} -multicast
    ${RX_Packets_CPN1_1}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[1]}    ${VM_INT}
    ${RX_Packets_CPN2_1}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[1]}    ${VM_INT}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${MULTICAST_IP[0]}
    ${RX_Packets_CPN1_2}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[1]}    ${VM_INT}
    ${RX_Packets_CPN2_2}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[1]}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_2} > ${RX_Packets_CPN1_1}
    Should Be True    ${RX_Packets_CPN2_2} == ${RX_Packets_CPN2_1}
    ${ELAN_Packets_1}    ${VPN_Packets_1}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${ELAN_Packets_2}    ${VPN_Packets_2}    Get PacketCount from Flow Table    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10[1]}    ${VM_MACAddr_ELAN2[0]}

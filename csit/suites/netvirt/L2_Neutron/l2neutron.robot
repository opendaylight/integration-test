...
TC01 Broadcast/Unicast testing with L3VPN service
    ...
    ${RX_Packets_CPN1_1}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ${VM_INT}
    ${RX_Packets_CPN2_1}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    ${VM_INT}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${BROADCAST_IP[0]}
    ${RX_Packets_CPN1_2}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ${VM_INT}
    ${RX_Packets_CPN2_2}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_1} < ${RX_Packets_CPN1_2}
    Should Be True    ${RX_Packets_CPN2_1} < ${RX_Packets_CPN1_2}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${RX_Packets_CPN1_3}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ${VM_INT}
    ${RX_Packets_CPN2_3}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_2} < ${RX_Packets_CPN1_3}
    Should Be True    ${RX_Packets_CPN2_2} == ${RX_Packets_CPN1_3}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET20[1]}
    Should Contain    ${output}    64 bytes
    ${RX_Packets_CPN1_4}    Get RXPacketCount from Ifconfig    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ${VM_INT}
    ${RX_Packets_CPN2_4}    Get RXPacketCount from Ifconfig    ${NETWORKS}[1]    ${VM_IP_NET20[0]}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_3} == ${RX_Packets_CPN1_4}
    Should Be True    ${RX_Packets_CPN2_3} < ${RX_Packets_CPN1_4}

TC02 Multicast/Unicast testing with L3VPN service
    [Documentation]    Generate multicast&Unicast traffic from VM1 & Verify
    Log    Verification of FIB Entries and Flow
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${VM_IP_NET10}
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN Service    ${OS_COMPUTE_1_IP}    ${SRCMAC_CPN1}    ${VM_MACAddr_ELAN1}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For ELAN Service    ${OS_COMPUTE_2_IP}    ${SRCMAC_CPN2}    ${VM_MACAddr_ELAN2}
    ${RX_Packets_CPN1_1}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_1_IP}    ${VM_INT}
    ${RX_Packets_CPN2_1}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_2_IP}    ${VM_INT}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${MULTICAST_IP[0]}
    ${RX_Packets_CPN1_2}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_1_IP}    ${VM_INT}
    ${RX_Packets_CPN2_2}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_2_IP}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_1} < ${RX_Packets_CPN1_2}
    Should Be True    ${RX_Packets_CPN2_1} < ${RX_Packets_CPN1_2}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${RX_Packets_CPN1_3}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_1_IP}    ${VM_INT}
    ${RX_Packets_CPN2_3}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_2_IP}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_2} < ${RX_Packets_CPN1_3}
    Should Be True    ${RX_Packets_CPN2_2} == ${RX_Packets_CPN1_3}
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[0]}    ping -c 3 ${VM_IP_NET20[1]}
    Should Contain    ${output}    64 bytes
    ${RX_Packets_CPN1_4}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_1_IP}    ${VM_INT}
    ${RX_Packets_CPN2_4}    Get RXPacketCount from Ifconfig    ${OS_COMPUTE_2_IP}    ${VM_INT}
    Should Be True    ${RX_Packets_CPN1_3} == ${RX_Packets_CPN1_4}
    Should Be True    ${RX_Packets_CPN2_3} < ${RX_Packets_CPN1_4}
...
Get PacketCount from Flow Table
    ...
    ${{ARP_RESPONSE_REGEX} =    Set Variable    table=${ARP_RESPONSE_TABLE}, n_packets=\\d+,\\s.*,nw_dst=${dest_ip}

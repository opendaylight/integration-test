...
@{VPN_INSTANCE_ID}    4ae8cd92-48ca-49b5-94e1-b2921a261110    4ae8cd92-48ca-49b5-94e1-b2921a261111    4ae8cd92-48ca-49b5-94e1-b2921a261112
@{VPN_NAME}       vpn1
...
Create Neutron Networks
    [Documentation]    Create three networks
...
Create Neutron Subnets
    [Documentation]    Create three subnets. One subnets per network
...
Create Neutron Ports
    [Documentation]    Create six ports under previously created subnets
...
Create Nova VMs
    ...
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[2]}    ${VM_INSTANCES_DPN1[1]}    ${OS_COMPUTE_2_IP}    sg=sg-vpnservice
    ...
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[5]}    ${VM_INSTANCES_DPN2[2]}    ${OS_COMPUTE_2_IP}    sg=sg-vpnservice
...
Check ELAN Datapath Traffic Within The Networks
    ...
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[1]}    ping -c 3 ${VM_IP_NET10[2]}
    Should Contain    ${output}    64 bytes
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[0]    ${VM_IP_NET10[2]}    ping -c 3 ${VM_IP_NET10[0]}
    Should Contain    ${output}    64 bytes
    ...
    Should Contain    ${output}    64 bytes
    ${output} =    Execute Command on VM Instance    ${NETWORKS}[1]    ${VM_IP_NET20[2]}    ping -c 3 ${VM_IP_NET20[0]}
    Should Contain    ${output}    64 bytes

...
Verify creation of host route via openstack subnet update option
    ...
    Verify Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{NON_NEUTRON_DESTINATION}[0]\\s+@{SUBNET_GW_IP}[0]
...
Verify removal of host route
    ...
    Verify No Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{NON_NEUTRON_DESTINATION}[0]\\s+@{SUBNET_GW_IP}[0]
...
Verify creation of host route via openstack subnet set option with VM port as next hop IP
    ...
    Verify Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{SUBNET_CIDR}[2]\\s+@{GATEWAY_VM_IPS}[0]
...
Verify creation of host route via openstack subnet set option with VM port as next hop IP with change in destination prefix
    ...
    Verify Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{SUBNET_CIDR}[1]\\s+@{GATEWAY_VM_IPS}[0]
...
Verify creation of host route via openstack subnet set option with change in next hop IP
    ...
    Verify Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{SUBNET_CIDR}[1]\\s+@{GATEWAY_VM_IPS}[1]

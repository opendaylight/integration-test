...
Verify host route in VM with multi destination prefix to next hop IP VM.
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    Verify Hostroutes In VM    @{NETWORKS}[0]    ${NETWORK_1_VM_IPS}    @{EXTRA_SUBNETS_CIDR}[0]\\s+@{GATEWAY_VM_IPS}[0]    @{EXTRA_SUBNETS_CIDR}[1]\\s+@{GATEWAY_VM_IPS}[0]    @{EXTRA_SUBNETS_CIDR}[2]\\s+@{GATEWAY_VM_IPS}[0]    @{EXTRA_SUBNETS_CIDR}[3]\\s+@{GATEWAY_VM_IPS}[0]    @{EXTRA_SUBNETS_CIDR}[4]\\s+@{GATEWAY_VM_IPS}[0]    @{EXTRA_SUBNETS_CIDR}[0]\\s+@{GATEWAY_VM_IPS}[5]

...
TC00 Verify Setup
    ...
    ${output}=    Write Commands Until Prompt    glance image-list    30s
    Log    ${output}
    ${output}=    Write Commands Until Prompt    openstack flavor list    30s
    Close Connection
    Log    ${output}
    [Return]    ${output}
...
#    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ping -c 5 @{EXTRA_NW_IP}[0]

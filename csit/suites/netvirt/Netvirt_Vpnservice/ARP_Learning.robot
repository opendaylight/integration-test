...
TC01 Verify GARP
    ...
    Wait Until Keyword Succeeds    5s    1s    Get Learnt IP    ${FIB_ENTRY_2}    session
    Log    Checking MAC-IP table in Config DS via REST
    #    ${output}=    Get Fib Entries    session
    #    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_3}\\/32".*"${OS_COMPUTE_2_IP}\\"
    #    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_1}\\/32".*"${OS_COMPUTE_1_IP}\\"
    #    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_2}\\/32".*"${OS_COMPUTE_2_IP}\\"
    Verify Ping To Sub Interface On Same ELAN    ${FIB_ENTRY_2}

TC02 Verify GARP requests onto Different ELAN Domains
    [Documentation]    Verify GARP requests onto Different ELAN Domains
    ...
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    ${FIB_ENTRIES_URL}
    Verify Ping On Same Networks
    ...
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_1}\\/32".*"${OS_COMPUTE_1_IP}\\"
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_2}\\/32".*"${OS_COMPUTE_2_IP}\\"
    Verify Ping To Sub Interface On ALL ELAN    ${FIB_ENTRY_2}

TC03 Verify MIP migration
...
Verify Ping To Sub Interface On Same ELAN
...
Verify Ping To Sub Interface On ALL ELAN
    [Arguments]    ${sub_interface_ip}
    [Documentation]    Verify ping to the sub-interface
    ${output}=    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ping -c 3 ${sub_interface_ip}
    Should Contain    ${output}    ${PING_REGEXP}
...
Delete Setup
    ...
    \    Remove Interface    ${ROUTERS}    ${INTERFACE}

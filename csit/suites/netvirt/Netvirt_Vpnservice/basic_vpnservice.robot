...
Check L3_Datapath Traffic Across Networks With Router
    ...
    Wait Until Keyword Succeeds    60s    10s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${router_id}/    ${vm_instances}
...
Verify L3VPN Datapath With Router Association
    ...
    Log     ${RD}
    Wait Until Keyword Succeeds    180s    30s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${RD}/    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_2_IP}    ${vm_instances}
...
Dissociate L3VPN From Routers
    ...
    Log    Verify FIB Table entry
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    180s    30s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${router_id}/    ${vm_instances}
...
Verify L3VPN datapath with Networks Association
    ...
    Wait Until Keyword Succeeds    300s    50s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${vm_instances}
    Wait Until Keyword Succeeds    300s    50s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_2_IP}    ${vm_instances}
    Get OvsDebugInfo

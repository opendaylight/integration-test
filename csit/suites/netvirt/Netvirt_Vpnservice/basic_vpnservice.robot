...
Verify Datapath After Migrate VM Instance
    ...
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo ssh-keygen -t rsa -N "" -y 
    Log    ${output}
    ...
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo ssh-keygen -t rsa -N "" -y 
    Log    ${output}

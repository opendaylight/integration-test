...
Verify Datapath After Reboot Nova VM Instance
    ...
    ${VM_IP_NET20}    ${DHCP_IP2}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_NET20}
    Log    ${VM_IP_NET20}
    Should Not Contain    ${VM_IP_NET20}    None
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
...
Verify Datapath After Recreate VM Instance
    ...
    ${VM_IP_NET10}    ${DHCP_IP1}    Wait Until Keyword Succeeds    60s    10s    Collect VM IP Addresses    true    @{VM_INSTANCES_NET10}
    Log    ${VM_IP_NET10}
    Set Suite Variable    ${VM_IP_NET10}

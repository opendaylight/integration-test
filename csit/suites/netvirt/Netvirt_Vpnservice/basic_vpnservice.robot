...
#${SSH_COPY_CMD}    sudo cat ~/.ssh/id_rsa.pub | ssh jenkins@${OS_COMPUTE_1_IP} "mkdir -p ~/.ssh && cat >>    ~/.ssh/authorized_keys"
${SSH_COPY_CMD}    sudo cat ~/.ssh/id_rsa.pub | ssh jenkins@${OS_COMPUTE_1_IP} "sudo cat >>    ~/.ssh/authorized_keys"
${SSH_KEYGEN}     sudo ssh-keygen -f /home/jenkins/.ssh/id_rsa -N "" -y
...
Verify Datapath After Migrate VM Instance
    ...
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo virsh list
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${SSH_KEYGEN}
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${SSH_COPY_CMD}
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo virsh list
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    ${SSH_KEYGEN}
    Log    ${output}
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    ${SSH_COPY_CMD}
    Log    ${output}

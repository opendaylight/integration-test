...
Verify Ping From VMInstance To DCGW
    ...
    ${output1} =    Run Keyword And Ignore Error    Get Quagga configuration On ODL    ${ODL_SYSTEM_IP}    ${DCGW_RD}
    Log    ${output1}
    ${fib_values} =    Create List    ${LOOPBACK_IP}    @{VM_IP_NET10}    @{VM_IP_NET20}
    ${RD} =    Strip String    ${CREATE_RD[0]}    characters="[]
    Log    ${RD}
    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    60s    5s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${RD}/    ${fib_values}
    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    60s    5s    Verify Routes On Quagga    ${DCGW_SYSTEM_IP}    ${DCGW_RD}    ${fib_values}
    Log    L3 Datapath test across the networks using router

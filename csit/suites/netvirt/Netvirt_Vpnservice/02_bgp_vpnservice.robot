...
Add Additional Neighbor to ODL
    ...
    Configure BGP And Add Neighbor On DCGW    ${DCGW_SYSTEM_2_IP}    ${AS_ID}    ${DCGW_SYSTEM_2_IP}    ${ODL_SYSTEM_IP}    ${VPN_NAME[0]}    ${DCGW_RD}
    ...    ${LOOPBACK_2_IP}
...
Delete one BGP Neighbor Config From ODL
    ...
    Should Contain     ${output}     ${DCGW_SYSTEM_1_IP} 
    ${dcgw2_routes} =    Create List    ${LOOPBACK_2_IP}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements Not At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${DCGW_RD}/    {dcgw_routes}
    ${dcgw1_routes} =    Create List    ${LOOPBACK_1_IP}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${DCGW_RD}/    {dcgw1_routes}
...
Delete BGP Config On ODL
    ...
    Log    ${output}
    Should Not Contain    ${output}    ${DCGW_SYSTEM_1_IP}
    #${dcgw_routes} =    Create List    ${LOOPBACK_1_IP}    ${LOOPBACK_2_IP}
    #Wait Until Keyword Succeeds    60s    10s    Check For Elements Not At URI    ${CONFIG_API}/odl-fib:fibEntries/vrfTables/${DCGW_RD}/    {dcgw_routes}
    #Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Removed For L3VPN    ${OS_COMPUTE_1_IP}    ${dcgw_routes}
    #Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Removed For L3VPN    ${OS_COMPUTE_2_IP}    ${dcgw_routes}

...
@{SUBNET_CIDR}    101.1.1.0/8    102.1.1.0/16    103.1.1.0/32    104.1.1.0/32
...
Delete External Tunnel Endpoint
    ...
    ${output} =    Delete External Tunnel Endpoint Configuration    destIp=${DCGW_SYSTEM_IP}
...
ODL Suite Setup
    ...
    ${VM_IPS}    ${DHCP_IPS}    Wait Until Keyword Succeeds    30s    10s    Collect VM IP Addresses    true    ${VM_NAMES}
    Log    ${VM_IPS}
    ...
    ${resp} =    VPN Get L3VPN    vpnid=${VPN_INSTANCE_IDS[${idx}]}
    Log    ${resp}

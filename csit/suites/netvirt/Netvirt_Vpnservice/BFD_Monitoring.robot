...
#    #    Log    ${bridge_up}
#    #    Sleep    5
#    #    #${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-int up
#    #    #Log    ${bridge_up}
#    #    #Sleep    5
#    #    #${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-int up
#    #    #Log    ${bridge_up}
...
#    #    Log    ${bridge_up}
#    #    Sleep    5
#    #    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-int up
#    #    Log    ${int_down}
#    #    Sleep    5
#    #    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-int up
#    #    Log    ${bridge_up}
...
#
TC09
    [Documentation]     Disconnect controller
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
    Disconnect From Ovsdb Node     ${OS_COMPUTE_1_IP}
    Sleep     10
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
    Connect To Ovsdb Node     ${OS_COMPUTE_1_IP}
    Sleep     10
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
...
TC08
    [Documentation]    Verify that the Access port reflect the proper status with BFD Monitoring SET to ON
#    ${ifconfig_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo ovs-vsctl show
#    @{keys}=    Create List
#    ${matches1} =    Get Lines Containing String    ${matches}    tun
#    Log    ${matches1}
#    @{lines}    Split To Lines    ${matches1}
#    : FOR    ${line}    IN    @{lines}
#    \    ${str} =    Remove String    ${line}    Port
#    \    ${str}=    Strip String    ${str}
#    \    ${str} =    Remove String    ${str}    "
#    \    Log    ${str}
#    \    Append To List    ${keys}    ${str}
#    Log     ${keys}
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
    Disconnect Switch From Controller And Verify Disconnected     ${OS_COMPUTE_1_IP}     ${ODL_SYSTEM_IP}     ${True}
    Sleep     10
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-int up
#    Log    ${bridge_up}
#    Sleep    5
...
#    #Sleep    4
    ...
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
    Reconnect Switch To Controller And Verify Connected      ${OS_COMPUTE_1_IP}     ${ODL_SYSTEM_1_IP}
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-int up
#    Log    ${int_down}
#    Sleep    20
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-int up
#    Log    ${bridge_up}
#    Sleep    20
#    #${int_down}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    #Log    ${int_down}
#    #Sleep    20
#    #${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    #Log    ${bridge_up}
#    #Sleep    20
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig
#    Log    ${int_down}
...
TC10
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}     netstat -nap | grep 6633
    Log    ${bridge_up}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}     netstat -nap | grep 6653
    Log    ${bridge_up}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}     ps -ef | grep odl
    Log    ${bridge_up}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}     netstat -nap|grep 6653|awk -F "ESTABLISHED" '{print $2}'|awk -F "/ovs" '{print $1}'
    Log    ${bridge_up}
    ${str}=    Strip String    ${bridge_up}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}     kill -9 str

...
Create Setup
    ...
    Set Suite Variable    ${VM_IP_NET2}
    Set Suite Variable    ${VM_IP_NET1}
    ${output}=    Execute Command on VM Instance    @{NETWORKS}[0]    30.1.1.3    ping -c 3 30.1.1.4
    Should Contain    ${output}    ${PING_REGEXP}
    ${output}=    Execute Command on VM Instance    @{NETWORKS}[1]    40.1.1.3    ping -c 3 40.1.1.4
    Should Contain    ${output}    ${PING_REGEXP}

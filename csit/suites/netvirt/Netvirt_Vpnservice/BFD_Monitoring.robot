...
TC04
    ...
    Log    ${resp.content}
    ${matches} =    Get Lines Containing String    ${resp.content}    interface-name
    ${matches}=    Fetch From Right    ${matches}    :
    ${ifconfig_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig ${matches}
...
Verify Flows Are Present
    ...
    ${resp}=    Should Match regexp    ${flow_output}    goto_table:50

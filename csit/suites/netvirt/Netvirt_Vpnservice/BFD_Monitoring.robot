...
TC04
    ...
    ${ifconfig_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo route add -net 11.1.1.60 netmask 255.255.255.255 gw ${gtw}
    ${ifconfig_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo route del -net 11.1.1.60 netmask 255.255.255.255 gw ${gtw}
   ${ifconfig_output}=    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    sudo /usr/sbin/ifconfig ${dpid}:tunnel_port:0
    ${resp}    RequestsLibrary.Get Request    session     /restconf/operational/ietf-interfaces:interfaces-state/
    Log     ${resp.content}
...
Verify Flows Are Present
    ...
    ${resp}=    Should Match regexp    ${flow_output}    resubmit(,50)

...
TC00 Verify if tunnels are present
     ...
     Run Keyword If    Should Be Empty    ${output}    ITM Create Tunnel    tunnel-type=vxlan    vlan-id=0    ip-address1="${OS_COMPUTE_1_IP}"    dpn-id1=${node_1_dpid}    portname1="${node_1_adapter}"    ip-address2="${OS_COMPUTE_2_IP}"
...
TC01 Verify that the default tunnel type is set to BFD
    ...
    Sould Not Contain     ${output}    ${STATE_DISABLE}
...
Verify Flows Are Present
    ...
    @{table50_output}=    Split To Lines    ${table50_output}    0    -2

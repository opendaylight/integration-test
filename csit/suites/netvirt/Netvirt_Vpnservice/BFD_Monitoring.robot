...
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig
#    Log    ${bridge_up}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 ${OS_COMPUTE_1_IP}
#    Log    ${bridge_up}
#    Sleep     10
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}    
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig
#    Log    ${bridge_up}
#    Log    Chanding IP address
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 0.0.0.0
#    Log    ${bridge_up}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
...
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-int down
#    Log    ${bridge_up}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_1_IP}    br-int
...
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 down
#    Log    ${bridge_up}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_1_IP}    br-int
...
TC12 Veify port goes to UNKNOWN
    [Documentation]    Verify that access ports go to UNKNOWN state
    Wait Until Keyword Succeeds    30s    5s    Verify Tunnel Status as UP
    ${port}=    SW_GET_ALL_PORT    ${OS_COMPUTE_1_IP}    br-int
    Log    ${port}
    ${matches} =    Get Lines Containing String    ${port}    tap
    : For    ${line}    IN    ${matches}
    \    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig ${line} up
    \    Log    ${bridge_up}
    Sleep    5
    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_1_IP}    br-int
    Log    ${all_ports_status}
...
TC10
    ...
    ${command}    Set Variable    nova reboot @{VM_INSTANCES_NET1}[0]

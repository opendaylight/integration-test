...
TC03 Verify that the monitoring interval value boundaries with Monitoring Enabled
    ...
    Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Log    ${resp.content}
    Should Not Contain    ${resp.content}    ${TMI_31000}
    Should Contain    ${resp.content}    ${TMI_30000}
...
TC05 Verify that the tunnel monitoring protocol can be configured to LLDP
...
*** Keywords ***
Start Suite
    [Documentation]    Run before the suit execution
...
Create Setup
    ...
    #\    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}
    ${VM_IP_NET1}    ${DHCP_IP1}    ${VM_IP_NET2}    ${DHCP_IP2}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_NET1}    @{VM_INSTANCES_NET2}
    Log    ${VM_IP_NET1}
    ...
    #${count}=    Get Count    ${output}    tunnel_port
    #Log    ${count}
    #Should Not Contain    ${count}    0
    Should Match Regexp    ${ODL_STREAM}    boron|carbon
    #Fatal Error    Tunnel Autoconfig feature failed

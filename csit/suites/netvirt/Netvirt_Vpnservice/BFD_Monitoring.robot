...
TC01
    ...
    #    Log    Bridge Up
    #    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ifconfig br-ext up
    #    Log    ${bridge_up}
    #    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    ifconfig br-ext up
    #    Log    ${bridge_up}
...
TC02
    ...
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/tunnel_monitoring    session=session
    ...
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/monitor_interval    mapping={"integer":"2000"}    session=session
...
TC03
    ...
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/monitor_interval    mapping={"integer":"1000"}    session=session
    ${resp}    RequestsLibrary.Get Request    session    ${MONITOR_INTERVAL_URL}
    Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Log    ${resp.content}
    Should Contain    ${resp.content}    ${TMI_100}
    Log    Verifying the tunnel monitoring interval to 3000ms
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/monitor_interval    mapping={"integer":"3000"}    session=session
    ${resp}    RequestsLibrary.Get Request    session    ${MONITOR_INTERVAL_URL}
    Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Log    ${resp.content}
    Should Contain    ${resp.content}    ${TMI_3000}

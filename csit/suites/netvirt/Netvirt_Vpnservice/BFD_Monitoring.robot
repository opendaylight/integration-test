...
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 0.0.0.0 netmask 255.255.0.0 up
#    Log    ${bridge_up}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
...
#    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_2_IP}    br-int
#    Log    ${all_ports_status}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 ${OS_COMPUTE_1_IP} netmask 255.255.0.0 up
#    Log    ${bridge_up}
#    Sleep    10 
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
...
TC11
    ...
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfi
    Log    ${bridge_up}
    Sleep    5
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}

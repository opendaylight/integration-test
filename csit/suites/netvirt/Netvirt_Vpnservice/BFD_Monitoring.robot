...
TC07
    ...
    Sleep     5
    Wait Until Keyword Succeeds    30s    5s    Verify Tunnel Status as UP
    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 down
    Log    ${bridge_up}
    ...
    Log    ${output}
    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 up 
    Log    ${bridge_up}
    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig
    Log    ${bridge_up}
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}

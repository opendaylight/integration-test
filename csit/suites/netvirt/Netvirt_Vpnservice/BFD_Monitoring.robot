...
TC01
    ...
    Log    ${ifconfig_output}
    ${ifconfig_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig tunnel_port
    Log    ${ifconfig_output}
...
TC03
    ...
    Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Log    ${resp.content}
    Wait Until Keyword Succeeds    30s    5s    Should Contain    ${resp.content}    ${TMI_30000}
    Log    Verifying the tunnel monitoring interval to 50ms
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/monitor_interval    mapping={"int":"50"}    session=session
    ${resp}    RequestsLibrary.Get Request    session    ${MONITOR_INTERVAL_URL}
    Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Log    ${resp.content}
    Wait Until Keyword Succeeds    30s    5s    Should Contain    ${resp.content}    ${TMI_50}

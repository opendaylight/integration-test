...
#    @{keys}=    Create List
#    ${matches} =    Get Lines Containing String    ${ifconfig_output}    Port
#    Log    ${matches}
...
#    \    ${stripped}=    Strip String    ${str}
#    \    ${str} =    Remove String    ${line}    "
#    \    Log    ${str}
#    \    Append To List    ${keys}    ${str}
#    Log     ${keys}
    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig eth0 down 
    Sleep    10 
    Log    Verifying default tunnel status and protocol
    #Wait Until Keyword Succeeds    30s    5s    Verify Tunnel Status as UP
    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
    Log    ${output}
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    Log    ${bridge_up}
#    Sleep    5
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    Log    ${bridge_up}
...
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 down
#    Log    ${int_down}
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig
#    Log    ${int_down}
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 down
#    Log    ${int_down}
#    Sleep    4
#    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_1_IP}    br-physnet1
#    Log    ${all_ports_status}
#    ${all_ports_status} =    SW_GET_ALL_PORT_STATUS    ${OS_COMPUTE_2_IP}    br-physnet1
#    Log    ${all_ports_status}
#    Sleep    5
#    Get OvsDebugInfo
#    ${output}=    Issue Command On Karaf Console    ${TEP_SHOW_STATE}
#    Log    ${output}
#    ${int_down}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    Log    ${int_down}
#    Sleep    20
#    ${bridge_up}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo /usr/sbin/ifconfig br-physnet1 up
#    Log    ${bridge_up}

...
@{SUBNET_CIDR}    2001:db8:0:2::/64    2001:db8:0:3::/64
...
${IP_REGEX}       (2001:([0-9A-Fa-f]{0,4}:){1,6}([0-9A-Fa-f]{1,4}))
...
Create Neutron Subnets
    ...
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS[0]}    ${SUBNET_CIDR[0]}    ${net1_additional_args}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS[1]}    ${SUBNET_CIDR[1]}    ${net2_additional_args}
...
Add Ssh Allow Rule
    ...
    ...    remote_ip_prefix=::/0
    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=ingress    ethertype=IPv6    protocol=icmp    remote_ip_prefix=::/0
    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=egress    ethertype=IPv6    protocol=icmp    remote_ip_prefix=::/0
...
Create Nova VMs
    ...
    ${prefix_net10}=    Replace String    ${SUBNET_CIDR[0]}    ::/64    (:[a-f0-9]{,4}){,4}
    Log    ${prefix_net10}
    ${status}    ${message}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    180s    30s    Collect VM IPv6 SLAAC Addresses
    ...    true    ${prefix_net10}    @{VM_INSTANCES_NET10}
    ${prefix_net20}=    Replace String    ${SUBNET_CIDR[1]}    ::/64    (:[a-f0-9]{,4}){,4}
    Log    ${prefix_net20}
    ${status}    ${message}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    180s    30s    Collect VM IPv6 SLAAC Addresses
...
Check L3_Datapath Traffic Across Networks With Router
    ...
    Log    ${dst_ip_list}
    Test Operations From Vm Instance with IPv6 GUA    ${NETWORKS[0]}    ${VM_IP_NET10[1]}    ${dst_ip_list}
    ${dst_ip_list} =    Create List    ${VM_IP_NET20[1]}    @{VM_IP_NET10}
    Log    ${dst_ip_list}
    Test Operations From Vm Instance with IPv6 GUA    ${NETWORKS[1]}    ${VM_IP_NET20[0]}    ${dst_ip_list}
...
Add Multiple Extra Routes And Check Datapath Before L3VPN Creation
    ...
    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET10[0]}    ${CONFIG_EXTRA_ROUTE_IP1}
    ...
    ${EXT_RT1} =    Set Variable    destination=${EXTRA_NW_SUBNET[0]},nexthop=${VM_IP_NET10[0]}
    ${EXT_RT2} =    Set Variable    destination=${EXTRA_NW_SUBNET[1]},nexthop=${VM_IP_NET10[0]}
...
Verify L3VPN Datapath With Router Association
    ...
    ${dst_ip_list} =    Create List    @{VM_IP_NET10}[1]    @{VM_IP_NET20}
    Log    ${dst_ip_list}
    Test Operations From Vm Instance with IPv6 GUA    ${NETWORKS[0]}    @{VM_IP_NET10}[0]    ${dst_ip_list}
...
Wait For Routes To Propogate
    ...
    Switch Connection    ${devstack_conn_id}
    ${net_id} =    Get Net Id    @{NETWORKS}[0]    ${devstack_conn_id}
    ${output} =    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ip -6 route    ]>

...
@{SUBNETS1_RT2}    SUBNET1_IPV4_2RT    SUBNET1_IPV6_2RT
@{SUBNETS2_RT2}    SUBNET2_IPV4_2RT    SUBNET2_IPV6_2RT
...
@{ROUTERS}        ROUTER_1_2RT    ROUTER_2_2RT
...
Create Neutron Subnets
    ...
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS1_RT2[0]}    ${SUBNETS1_CIDR[0]}
    ${net1_additional_args}=    Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${NET1_IPV6_ADDR_POOL}
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS2_RT2[0]}    ${SUBNETS2_CIDR[0]}    ${net1_additional_args}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS1_RT2[1]}    ${SUBNETS1_CIDR[1]}
    ${net2_additional_args}=    Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${NET2_IPV6_ADDR_POOL}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS2_RT2[1]}    ${SUBNETS2_CIDR[1]}    ${net2_additional_args}
    ${SUB_LIST}    List Subnets
    Log    ${SUB_LIST}
    Should Contain    ${SUB_LIST}    ${SUBNETS1_RT2[0]}
    Should Contain    ${SUB_LIST}    ${SUBNETS1_RT2[1]}
    Should Contain    ${SUB_LIST}    ${SUBNETS2_RT2[0]}
    Should Contain    ${SUB_LIST}    ${SUBNETS2_RT2[1]}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${SUBNETWORK_URL}    ${SUBNETS1_RT2}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${SUBNETWORK_URL}    ${SUBNETS2_RT2}
    Update SubNet    ${SUBNETS1_RT2[0]}    additional_args=--description ${UPDATE_SUBNET}
    ${output} =    Show SubNet    ${SUBNETS1_RT2[0]}

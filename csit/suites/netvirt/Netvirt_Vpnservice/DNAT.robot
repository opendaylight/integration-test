...
${NETWORK_URL}    /restconf/config/neutron:neutron/networks/
${SUBNETWORK_URL}    /restconf/config/neutron:neutron/subnets/
...
TC01 Create external network and update router with enable SNAT
    ...
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${NETWORK_URL}    ${EXT_NETWORK}
    ...
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${SUBNETWORK_URL}    ${EXT_SUBNET}
    ...
    Set Suite Variable     ${ext_net_id}
    ${ext_subnet_id} =    Get Subnet Id    ${EXT_SUBNET}    ${devstack_conn_id}
    Set Suite Variable     ${ext_subnet_id}

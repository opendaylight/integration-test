...
TC03 Verify floating IP provision and reachability from external network via neutron router through l3vpn
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    ${output}=    Write Commands Until Prompt    ping -c 5 12.12.12.200
    Log    ${output}
    ${output}=    Write Commands Until Prompt    ping -c 5 ${VM_IP_NET1[0]}
    Log    ${output}
    ${output}=    Write Commands Until Prompt    ping -c 5 ${floating_ip_add}
    Log    ${output}
    ${dpn1_flows}    ${dpn2_flows}    Dump Flows
    ${devstack_conn_id} =    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    ${output}=    Write Commands Until Prompt    ping -c 5 
    Log    ${output}
    ${dpn1_flows}    ${dpn2_flows}    Dump Flows
...
#    Log    ${resp.content}
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    ${output}=    Write Commands Until Prompt    ping -c 5 ${floating_ip_add2}
    Log    ${output}
    ${dpn1_flows}    ${dpn2_flows}    Dump Flows
...
TC11 Verify_update_router_with_single_external_IP_while_router_is_hosting_single_subnet
    ...
    Log    ${resp}
    ${output}=    Execute Command on VM Instance    ${NETWORKS[0]}    ${VM_IP_NET1[0]}    ping -c 5 ${OS_CONTROL_NODE_IP}
    Log    ${output}
    ${dpn1_flows}    ${dpn2_flows}    Dump Flows
...
TC13 Verify successful update of router with disable SNAT and then with enable SNAT with external_gateway_info disable SNAT
    [Documentation]    Verify successful update of router with disable SNAT and then with enable SNAT with external_gateway_info disable SNAT
    Log     Disable SNAT
    Add Router Gateway    ${ROUTERS[0]}    ${EXT_NETWORK}    ${Disable_SNAT}
    ...
    ${output}=    Execute Command on VM Instance    ${NETWORKS[0]}    ${VM_IP_NET2[0]}    ping -c 5 ${OS_CONTROL_NODE_IP}
    Log    ${output}
...
TC13 Verify_update_router_with_scenario_where_externalIPs_are_more_than_subnet_associated_to_router
    [Documetation]    Verify_update_router_with_scenario_where_externalIPs_are_more_than_subnet_associated_to_router

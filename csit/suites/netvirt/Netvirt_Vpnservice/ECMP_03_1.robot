...
*** Variables ***
@{Port_List}    Create List    Network1_Port1    Network1_Port2    Network1_Port3    Network1_Port4    Network1_Port5    Network1_Port6    Network1_Port7    Network1_Port8
@{VmList}    Create List    NOVA_VM11    NOVA_VM12    NOVA_VM13    NOVA_VM21    NOVA_VM22    NOVA_VM23    NOVA_VM31    NOVA_VM32
...
#    Verify Static Ip Configured In VM    ${VM_IPs[4]}    Network1    ${StaticIp}
    ...
    Run Keyword If    ${Compute1PacketCountAfterPing}==${Compute1PacketCountBeforePing}+${NoOfPingPackets}    Run Keywords    Log    Packets forwarded via Compute 1    AND    Verify Packet Count    ${OS_COMPUTE_1_IP}    ${StaticIp}
    ...    ELSE IF    ${Compute2PacketCountAfterPing}==${Compute2PacketCountBeforePing}+${NoOfPingPackets}    Run Keywords    Log    Packets forwarded via Compute 2    AND    Verify Packet Count    ${OS_COMPUTE_2_IP}    ${StaticIp}
...
Delete Setup
    ...
    Append To List    ${VmList}    NOVA_VM24    NOVA_VM25    NOVA_VM31
    ...
    \    Run Keyword And Ignore Error    Delete Port    ${name}
...
#    \    ${VmIp}    Get VM IP    ${VmName}
    \    Append To List    ${NextHopList}    destination=${ip}/${mask},nexthop=${VmIp}
    [Return]    @{NextHopList}
...
Configure Next Hop on Router
    ...
    ${routes1}    catenate    --routes type=dict list=true    @{NextHopList_1}
...
Configure Ip on Sub Interface
    ...
    Wait Until keyword succeeds    100s    20s    Run Keyword    Execute Command on VM Instance    Network1    ${VmIp}    sudo ifconfig eth0:0 ${StaticIp} netmask 255.255.255.0 up
#    Wait Until keyword succeeds    100s    20s    Run Keyword    Execute command on server    sudo ifconfig eth0:0 ${ip}/${mask} up    ${VmName}
...
Verify Ping to Sub Interface
   ...
   #(\\d+)\\% packet loss
    ${resp}    Run Keyword If    ${PacketCount}<=${20}    Evaluate    ${resp}+0    ELSE    Evaluate    ${resp}+1

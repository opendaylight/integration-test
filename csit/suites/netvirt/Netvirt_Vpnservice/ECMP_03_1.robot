...
TC01 Verify Distribution of traffic with weighted buckets-3 VM on CSS1 , 2 VM on CSS2
    ...
    ${RouterUpdateCmd}    Set Variable    --route destination=${StaticIp}/32,gateway=${VM_IPs[0]} --route destination=${StaticIp}/32,gateway=${VM_IPs[1]} 
#--route destination=${StaticIp}/32,gateway=${VM_IPs[2]} --route destination=${StaticIp}/32,gateway=${VM_IPs[3]} --route destination=${StaticIp}/32,gateway=${VM_IPs[4]}
...
#    Log    Verify the traffic originated from VM getting splitted between nexthop-VMs
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[0]}    sudo arp -an
    Log    ${arp_output}
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[1]}    sudo arp -an
    Log    ${arp_output}
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[2]}    sudo arp -an
    Log    ${arp_output}
    ${pingresp_0}    Execute Command on VM Instance    Network1    ${VM_IPs[0]}    ping -c 15 ${VM_IPs[1]}
    ${pingresp_1}    Execute Command on VM Instance    Network1    ${VM_IPs[1]}    ping -c 15 ${VM_IPs[2]}
    ${pingresp_2}    Execute Command on VM Instance    Network1    ${VM_IPs[2]}    ping -c 15 ${VM_IPs[0]}
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[0]}    sudo arp -an
    Log    ${arp_output}
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[1]}    sudo arp -an
    Log    ${arp_output}
    ${arp_output}=    Execute Command on VM Instance    Network1    ${VM_IPs[2]}    sudo arp -an
    Log    ${arp_output}
    ${pingresp}    Execute Command on VM Instance    Network1    ${VM_IPs[2]}    ping ${StaticIp} -c 15
...
Create Setup
    ...
    Create Port    Network1    Network1_Port1    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port2    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port3    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port4    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port5    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port6    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}
    Create Port    Network1    Network1_Port7    sg=${SECURITY_GROUP}    allowed_address_pairs=${StaticIp}

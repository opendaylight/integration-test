...
Check ELAN Datapath Traffic Within The Networks
    ...
    ${status}    ${output} =    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    3x    10s    Execute Command on VM Instance
    ...    @{NETWORKS}[0]    ${VM_IP_NET10[0]}    ping6 -c 3 ${VM_IP_NET10[1]}
    Should Contain    ${output}    64 bytes
    ${status}    ${output} =    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    3x    10s    Execute Command on VM Instance
    ...    @{NETWORKS}[1]    ${VM_IP_NET20[0]}    ping6 -c 3 ${VM_IP_NET20[1]}
    Should Contain    ${output}    64 bytes

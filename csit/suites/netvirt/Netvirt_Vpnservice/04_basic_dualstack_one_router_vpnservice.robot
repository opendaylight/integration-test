...
=======
@{SUBNETS4}       SUBNET1_IPV4    SUBNET2_IPV4
@{SUBNETS6}       SUBNET1_IPV6    SUBNET2_IPV6
@{SUBNETS4_CIDR}    10.1.1.0/24    20.1.1.0/24
@{SUBNETS6_CIDR}    2001:db8:0:2::/64    2001:db8:0:3::/64
>>>>>>> 67b03ef17b5f... CSIT: add dualstack test for one-router solution
...
=======
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS4[0]}    ${SUBNETS4_CIDR[0]}
    ${net1_additional_args}=    Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${NET1_IPV6_ADDR_POOL}
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS6[0]}    ${SUBNETS6_CIDR[0]}    ${net1_additional_args}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS4[1]}    ${SUBNETS4_CIDR[1]}
    ${net2_additional_args}=    Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${NET2_IPV6_ADDR_POOL}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS6[1]}    ${SUBNETS6_CIDR[1]}    ${net2_additional_args}
    ${SUB_LIST}    List Subnets
    Log    ${SUB_LIST}
    Should Contain    ${SUB_LIST}    ${SUBNETS4[0]}
    Should Contain    ${SUB_LIST}    ${SUBNETS4[1]}
    Should Contain    ${SUB_LIST}    ${SUBNETS6[0]}
    Should Contain    ${SUB_LIST}    ${SUBNETS6[1]}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${SUBNETWORK_URL}    ${SUBNETS4}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${SUBNETWORK_URL}    ${SUBNETS6}
    Update SubNet    ${SUBNETS4[0]}    additional_args=--description ${UPDATE_SUBNET}
    ${output} =    Show SubNet    ${SUBNETS4[0]}
>>>>>>> 67b03ef17b5f... CSIT: add dualstack test for one-router solution
...
=======
    ...
    ${VM_IPV6_NET10}=    Collect VM IPv6 SLAAC Addresses    false    ${VM_INSTANCES_NET10}    ${NETWORKS[0]}    ${SUBNETS6_CIDR[0]}
    ${VM_IPV6_NET20}=    Collect VM IPv6 SLAAC Addresses    false    ${VM_INSTANCES_NET20}    ${NETWORKS[1]}    ${SUBNETS6_CIDR[1]}
>>>>>>> 67b03ef17b5f... CSIT: add dualstack test for one-router solution
    Log    ${VM_IPV6_NET10}
    Log    ${VM_IPV6_NET20}

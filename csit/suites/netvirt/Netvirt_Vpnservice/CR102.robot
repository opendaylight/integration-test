...
TC01
    ...
    Log    ${resp}    #Create And Verify Networks Subnets And Ports    #    [Documentation]    UC_8 Verify the creation of two networks, two subnets and four ports using Neutron CLI    #
    ...    # Log    Create two networks    #    Create Network    ${NETWORKS[0]}    #
    ...    # Create Network    ${NETWORKS[1]}    #    Create Network    ${NETWORKS[2]}    #
    ...    # ${NET_LIST}    List Networks    #    Log    ${NET_LIST}    #
    ...    # Should Contain    ${NET_LIST}    ${NETWORKS[0]}    #    Should Contain    ${NET_LIST}
    ...    # ${NETWORKS[1]}    #    Should Contain    ${NET_LIST}    ${NETWORKS[2]}    #
    ...    # Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    /restconf/config/neutron:neutron/networks/    ${NETWORKS}
    ...    #    Log    Create two subnets for previously created networks    #    Create SubNet    ${NETWORKS[0]}
    ...    # ${SUBNETS[0]}    ${SUBNET_CIDR[0]}    #    Create SubNet    ${NETWORKS[1]}    ${SUBNETS[1]}
    ...    # ${SUBNET_CIDR[1]}    #    Create SubNet    ${NETWORKS[2]}    ${SUBNETS[2]}    ${SUBNET_CIDR[2]}
    ...    #    ${SUB_LIST}    List Subnets    #    Log    ${SUB_LIST}
    ...    #    Should Contain    ${SUB_LIST}    ${SUBNETS[0]}    #    Should Contain
    ...    # ${SUB_LIST}    ${SUBNETS[1]}    #    Should Contain    ${SUB_LIST}    ${SUBNETS[2]}
    ...    #    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    /restconf/config/neutron:neutron/subnets/
    ...    # ${SUBNETS}    #    Log    Create four ports under previously created subnets    #    Create Port
    ...    # ${NETWORKS[0]}    ${PORT_LIST[0]}    sg=sg-vpnservice    #    Create Port    ${NETWORKS[0]}
    ...    # ${PORT_LIST[1]}    sg=sg-vpnservice    #    Create Port    ${NETWORKS[1]}    ${PORT_LIST[2]}
    ...    # sg=sg-vpnservice    #    Create Port    ${NETWORKS[1]}    ${PORT_LIST[3]}    sg=sg-vpnservice
    ...    #    Create Port    ${NETWORKS[2]}    ${PORT_LIST[4]}    sg=sg-vpnservice    #
    ...    # Create Port    ${NETWORKS[2]}    ${PORT_LIST[5]}    sg=sg-vpnservice    #    Wait Until Keyword Succeeds
    ...    # 3s    1s    Check For Elements At URI    /restconf/config/neutron:neutron/ports/    ${PORT_LIST}    #
    ...    # Log    Create VM Instances    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[0]}    ${VM_INSTANCES_NET10[0]}
    ...    # ${OS_COMPUTE_1_IP}    sg=sg-vpnservice    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[1]}    ${VM_INSTANCES_NET10[1]}
    ...    # ${OS_COMPUTE_2_IP}    sg=sg-vpnservice    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[2]}    ${VM_INSTANCES_NET20[0]}
    ...    # ${OS_COMPUTE_1_IP}    sg=sg-vpnservice    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[3]}    ${VM_INSTANCES_NET20[1]}
    ...    # ${OS_COMPUTE_2_IP}    sg=sg-vpnservice    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[4]}    ${VM_INSTANCES_NET30[0]}
    ...    # ${OS_COMPUTE_1_IP}    sg=sg-vpnservice    #    Create Vm Instance With Port On Compute Node    ${PORT_LIST[5]}    ${VM_INSTANCES_NET30[1]}
    ...    # ${OS_COMPUTE_2_IP}    sg=sg-vpnservice    #    : FOR    ${VM}    IN
    ...    # @{VM_INSTANCES_NET10}    #    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE
    ...    # ${VM}    #    : FOR    ${VM}    IN    @{VM_INSTANCES_NET20}
    ...    #    Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}
    ...    #    : FOR    ${VM}    IN    @{VM_INSTANCES_NET30}    #
    ...    # Wait Until Keyword Succeeds    25s    5s    Verify VM Is ACTIVE    ${VM}    #
    ...    # Log    Check for routes    #    Wait Until Keyword Succeeds    30s    10s
    ...    # Wait For Routes To Propogate    #    ${VM_IP_NET10}    ${DHCP_IP1}    Wait Until Keyword Succeeds    180s
    ...    # 10s    Verify VMs Received DHCP Lease    # @{VM_INSTANCES_NET10}    #    Log    ${VM_IP_NET10}
    ...    #    Set Suite Variable    ${VM_IP_NET10}    #    ${VM_IP_NET20}    ${DHCP_IP2}
    ...    # Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    # @{VM_INSTANCES_NET20}    #
    ...    # Log    ${VM_IP_NET20}    #    Set Suite Variable    ${VM_IP_NET20}    #
    ...    # ${VM_IP_NET30}    ${DHCP_IP3}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease
    ...    # @{VM_INSTANCES_NET30}    #    Log    ${VM_IP_NET30}    #    Set Suite Variable
    ...    # ${VM_IP_NET30}    #    Log    Create Router    #    Create Router
    ...    # ${ROUTERS[0]}    #    ${router_list} =    Create List    ${ROUTERS[0]}    #
    ...    # Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${NEUTRON_ROUTERS_API}    ${router_list}
    ...    #    Log    Add Interfaces to router    #    Add Router Interface    ${ROUTERS[0]}
    ...    # ${SUBNETS[1]}    #    Add Router Interface    ${ROUTERS[0]}    ${SUBNETS[2]}    #
    ...    # Log    Verification of FIB Entries and Flow    #    ${vm_instances} =    Create List    ${VM_IP_NET20[0]}
    ...    # ${VM_IP_NET20[1]}    ${VM_IP_NET30[0]}    ${VM_IP_NET30[1]}    #    Wait Until Keyword Succeeds    30s
    ...    # 5s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}    #    Log
    ...    # Create a L3VPN and associate network1 and Router    #    ${devstack_conn_id} =    Get ControlNode Connection    #    Switch Connection
    ...    # ${devstack_conn_id}    #    ${net_id} =    Get Net Id    @{NETWORKS}[0]    ${devstack_conn_id}
    ...    #    ${tenant_id} =    Get Tenant ID From Network    ${net_id}    #    VPN Create L3VPN
    ...    # vpnid=${VPN_INSTANCE_ID[0]}    name=${VPN_NAME[0]}    rd=${CREATE_RD}    exportrt=${CREATE_EXPORT_RT}    importrt=${CREATE_IMPORT_RT}    # tenantid=${tenant_id}
    ...    #    ${resp}=    VPN Get L3VPN    vpnid=${VPN_INSTANCE_ID[0]}    #    Should Contain
    ...    # ${resp}    ${VPN_INSTANCE_ID[0]}    #    Associate L3VPN To Network    networkid=${net_id}    vpnid=${VPN_INSTANCE_ID[0]}
    ...    #    ${resp}=    VPN Get L3VPN    vpnid=${VPN_INSTANCE_ID[0]}    #    Should Contain
    ...    # ${resp}    ${net_id}    #    ${router_id}=    Get Router Id    ${ROUTERS[0]}
    ...    # ${devstack_conn_id}    #    Associate VPN to Router    routerid=${router_id}    vpnid=${VPN_INSTANCE_ID[0]}    #
    ...    # ${resp}=    VPN Get L3VPN    vpnid=${VPN_INSTANCE_ID[0]}    #    Should Contain    ${resp}
    ...    # ${router_id}    #    Log    Verification of FIB Entries and Flow    #    ${vm_instances} =
    ...    # Create List    ${VM_IP_NET10[0]}    ${VM_IP_NET10[1]}    ${VM_IP_NET20[0]}    ${VM_IP_NET20[1]}    # ${VM_IP_NET30[0]}
    #    ...    ${VM_IP_NET30[1]}

...
TC01
    ...
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    REGEX_CR_1 = "destPrefix\""+"\:\""+"10.1.1.3"+"\/32\"\,\"origin\"\:\"[a-z]\"\,\"label\"\:\\d+\,\"nextHopAddressList\"\:"+"\[\""+${OS_COMPUTE_1_IP}+"\"\]"
    Log    ${resp}
    ...
    #    ${VM_IP_NET10}    ${DHCP_IP1}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease
    ...    # @{VM_INSTANCES_NET10}
    #    Log    ${VM_IP_NET10}
    #    Set Suite Variable    ${VM_IP_NET10}
    #    ${VM_IP_NET20}    ${DHCP_IP2}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease
    ...    # @{VM_INSTANCES_NET20}
    #    Log    ${VM_IP_NET20}
    ...
    #    VPN Create L3VPN    vpnid=${VPN_INSTANCE_ID[0]}    name=${VPN_NAME[0]}    rd=${CREATE_RD}    exportrt=${CREATE_EXPORT_RT}    importrt=${CREATE_IMPORT_RT}
    ...    # tenantid=${tenant_id}
    ...
    #    Log    Verification of FIB Entries and Flow
    #    ${vm_instances} =    Create List    ${VM_IP_NET10[0]}    ${VM_IP_NET10[1]}    ${VM_IP_NET20[0]}    ${VM_IP_NET20[1]}
    ...    # ${VM_IP_NET30[0]}
    #    ...    ${VM_IP_NET30[1]}
    #    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}

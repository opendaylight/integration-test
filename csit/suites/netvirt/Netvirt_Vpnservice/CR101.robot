...
@{SUBNET_CIDR}    20.1.1.0/24    20.2.1.0/24    20.3.1.0/24    20.4.1.0/24     20.5.1.0/24
@{PORT_LIST}      PORT1    PORT2    PORT3    PORT4    PORT5    PORT6    PORT7    PORT8   PORT9    PORT10
@{VM_INSTANCES_NET1}    VM1    VM2
@{VM_INSTANCES_NET2}    VM3    VM4
@{VM_INSTANCES_NET3}    VM5    VM6
@{VM_INSTANCES_NET4}    VM7    VM8
@{VM_INSTANCES_NET5}    VM9    VM10
...
TC01
    ...
    Log    Test the Setup
    ${output}=    Issue Command On Karaf Console    display:tep-show-config
    Log To Console    ${output}
    Should Contain    ${TUNNEL_MONITOR_ON}
...
#    ${rx_packet1_before} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET10[1]}    ifconfig eth0
#    Log    ${rx_packet1_before}
#    Log    Checking the RX Packets Count on VM1 before ARP Broadcast
#    ${rx_packet0_before} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET10[0]}    ifconfig eth0
...
#    ${rx_packet1_after} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET10[1]}    ifconfig eth0
#    Log    ${rx_packet1_after}
#    Log    Checking the RX Packets Count on VM1 before ARP Broadcast
#    ${rx_packet0_after} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET10[0]}    ifconfig eth0
...
Create Setup
    ...
    Create SubNet    ${NETWORKS[2]}    ${SUBNETS[3]}    ${SUBNET_CIDR[3]}
    Create SubNet    ${NETWORKS[2]}    ${SUBNETS[4]}    ${SUBNET_CIDR[4]}
    ...
    Create Port    ${NETWORKS[3]}    ${PORT_LIST[6]}    sg=sg-vpnservice
    Create Port    ${NETWORKS[3]}    ${PORT_LIST[7]}    sg=sg-vpnservice
    Create Port    ${NETWORKS[4]}    ${PORT_LIST[8]}    sg=sg-vpnservice
    ...
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[6]}    ${VM_INSTANCES_NET4[0]}    ${OS_COMPUTE_1_IP}    sg=sg-vpnservice
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[7]}    ${VM_INSTANCES_NET4[1]}    ${OS_COMPUTE_2_IP}    sg=sg-vpnservice
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[8]}    ${VM_INSTANCES_NET5[0]}    ${OS_COMPUTE_1_IP}    sg=sg-vpnservice
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[9]}    ${VM_INSTANCES_NET5[1]}    ${OS_COMPUTE_2_IP}    sg=sg-vpnservice
#    @{VM_INSTANCES}    Create List    @{VM_INSTANCES_NET1}     @{VM_INSTANCES_NET2}     @{VM_INSTANCES_NET3}      @{VM_INSTANCES_NET4}     @{VM_INSTANCES_NET5}
...
#    Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate
    ...
    ${VM_IP_NET4}    ${DHCP_IP4}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_NET4}
    Log    ${VM_IP_NET4}
    Set Suite Variable    ${VM_IP_NET4}
    ${VM_IP_NET5}    ${DHCP_IP5}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_NET5}
    Log    ${VM_IP_NET5}
    Set Suite Variable    ${VM_IP_NET5}
#    Log    Create Router

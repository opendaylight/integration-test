...
Create BGP Config On DCGW
    ...
    Log    ${output} 
    ${output} =    Execute Command On DCGW    show bgp neighbors
    Log    ${output}
    ${output1} =    Execute Command On DCGW    show ip bgp vrf ${DCGW_RD}
    Log    ${output1}
...
Add BGP Configuration On DCGW
    ...
    ${output} =    Read
    ${output} =    Write    neighbor ${ODL_SYSTEM_IP} send-remote-as ${AS_ID}
    ${output} =    Read
    ${output} =    Write    vrf ${VPN_NAME[0]}
    ${output} =    Read
    ${output} =    Write    rd ${DCGW_RD}
    ${output} =    Read
    ${output} =    Write    rt import ${DCGW_RD}
    ${output} =    Read
    ${output} =    Write    rt export ${DCGW_RD}
    ${output} =    Read
    ...
    ${output} =    Read
    ${output} =    Write    network ${LOOPBACK_IP}/32 rd ${DCGW_RD} tag ${AS_ID}
    ${output} =    Read
    ...
    ${output} =    Read
    Log    ${output}
    ${output} =    Write    show bgp neighbors
    ${output} =    Read
    Log    ${output}
    ${output} =    Write    exit
    ${output} =    Read
...
Add Loopback Interface On DCGW
    [Arguments]    ${user}=zebra    ${password}=zebra
    [Documentation]    Execute cmd on DCGW and returns the ouput.
    Switch Connection    ${dcgw_conn_id}
    ${output} =    Write    telnet localhost ${user}
    Log    ${output}
    ...
    ${output} =    Read
    ${output} =    Write    ip address ${LOOPBACK_IP}/32
    ${output} =    Read
    ${output} =    Write    write terminal
    ${output} =    Read
    Log    ${output}

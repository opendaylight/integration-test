...
...               AND    DCGW Suite Setup     AND     Start Processes on ODL      AND     Start Processes on DCGW  
...
Add BGP Configuration On DCGW
    ...
    ${output} =    Write Commands Until Expected Prompt      telnet localhost ${user}       ]>
    Log    ${output}
    ...
    ${output} =    Read
    ${output} =    Write    neighbor ${LOOPBACK_IP} remote-as ${AS_ID}
    ${output} =    Read
    ${output} =    Write    neighbor ${LOOPBACK_IP} update-source lo
    ${output} =    Read
    ${output} =    Write    network ${DCGW_SYSTEM_IP}/32
    ${output} =    Read
    ${output} =    Write    vrf ${VPN_NAME[0]}
    ${output} =    Read
    ${output} =    Write    rd ${CREATE_RD[0]}
    ${output} =    Read
    ${output} =    Write    rt irt ${CREATE_IMPORT_RT[0]}
    ${output} =    Read
    ${output} =    Write    rt ert ${CREATE_EXPORT_RT[0]}
    ${output} =    Read
    ...
    ${output} =    Read
    ${output} =    Write    network ${LOOPBACK_IP}/32
    ${output} =    Read
...
Start Processes on ODL
    ...
    ${output} =    Write     /opt/quagga/etc/init.d/zrpcd start
    ${output} =    Read 
    ${output} =    Write     cd /opt/quagga/sbin/ 
    Log    ${output}
    ${output} =    Read
    Log    ${output}
    ...
    ${output} =    Read
    Log    ${output}
    ${output} =    Write    sudo ps -ef | grep bgpd
    ${output} =    Read
    Log    ${output}
...
Start Processes on DCGW 
    ...
    ${output} =    Read
    Log    ${output}
    ${output} =    Write Commands Until Expected Prompt    sudo ./bgpd     ]>
    Sleep    2
    ${output} =    Read
    Log    ${output}
    ${output} =    Write Commands Until Expected Prompt    sudo ./zebra     ]>
    Sleep    2
    ${output} =    Read
    Log    ${output}

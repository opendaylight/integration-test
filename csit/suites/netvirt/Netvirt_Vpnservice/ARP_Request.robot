...
TC09 Restart Controller
    ...
    ${output}=    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    ps -ef | grep karaf | grep -v grep | awk '{sudo print \"kill -9 \" $1}'
    Log    ${output}
    ${output}=    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    ps -ef | grep karaf | grep -v grep | awk '{sudo print \"kill -9 \" $1}' | sh
    Log    ${output}

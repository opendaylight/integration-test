...
${FIB_ENTRY_MIP_REGEX_DPN1}    destPrefix\\":\\"${FIB_ENTRY_2}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_2_IP}\\"\\],\\"origin\\":\\"[a-z]
...
TC01
    ...
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    ${FIB_ENTRY_VM11_REGEX}
    Log    ${resp}
    ${resp}=    Should Match Regexp    ${resp.content}    ${FIB_ENTRY_VM21_REGEX}
    Log    ${resp}
    ...
    ${resp}    RequestsLibrary.Get Request    session    ${FIB_ENTRIES_URL} 
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    ${FIB_ENTRY_MIP_REGEX_DPN2}
    Log    ${resp}
...
TC02
    ...
    ${resp}    RequestsLibrary.Get Request    session    ${FIB_ENTRIES_URL}
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    ${FIB_ENTRY_MIP_REGEX_DPN1}
    Log    ${resp}
    Log    Checking MAC-IP table in Config DS via REST
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/neutronvpn:neutron-vpn-portip-port-data/
    Log    ${resp.content}

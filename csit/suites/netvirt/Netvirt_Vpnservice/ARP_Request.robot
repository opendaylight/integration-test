...
TC01
    ...
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${VM_IP_NET1[1]}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_2_IP}\\"\\],\\"origin\\":\\"[a-z] 
    Log    ${resp}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${VM_IP_NET1[0]}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_1_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
...
TC02
    ...
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ${UNNCONFIG_EXTRA_ROUTE_IP1}
    Sleep    10
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ifconfig
    ...
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ping 10.1.1.110 -c 5
    Should Contain    ${output}    ${PING_REGEXP}
     Log    Validate the FIB entries in Controller
    ${resp}    RequestsLibrary.Get Request    session    ${FIB_ENTRIES_URL}
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${FIB_ENTRY_2}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_1_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
    Log    Checking MAC-IP table in Config DS via REST
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/neutronvpn:neutron-vpn-portip-port-data/
    Log    ${resp.content}

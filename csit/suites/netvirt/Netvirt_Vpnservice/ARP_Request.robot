...
TC09 Restart Controller
    ...
    Log    ${output}
    ${output}=    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    ps -ef | grep karaf | grep -v grep | awk '{sudo print "kill -9 " $2}' | sh
    Log    ${output}
...
TC08 Same ELAN
    ...
    ...    AND    Associate VPN to Router    routerid=${router_id}    vpnid=${VPN_INSTANCE_ID[0]}

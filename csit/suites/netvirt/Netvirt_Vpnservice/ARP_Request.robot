...
TC01 Verify GARP
    ...
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[1]}\/32".*"${OS_COMPUTE_2_IP}\\"
#    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[1]}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_2_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
     ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[0]}\/32".*"${OS_COMPUTE_1_IP}\\"
...
#    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ping 50.1.1.110 -c 5
#    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ping 50.1.1.110 -c 5
#    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET2[0]}    ping 50.1.1.110 -c 5
    Log    Checking MAC-IP table in Config DS via REST
    ${output}=    Get Fib Entries    session
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[1]}\\/32".*"${OS_COMPUTE_2_IP}\\"
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[0]}\\/32".*"${OS_COMPUTE_1_IP}\\"
...
TC05 Validate multiple mip migration
    ...
    ${output}=    Get Fib Entries    session
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[1]}\\/32".*"${OS_COMPUTE_2_IP}\\"
    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[0]}\\/32".*"${OS_COMPUTE_2_IP}\\"
...
#    ${output}    Get Fib Entries    session
#    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${FIB_ENTRY_4}\\/32".*"${OS_COMPUTE_1_IP}\\"
#    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET2[0]}\\/32".*"${OS_COMPUTE_1_IP}\\"
#    ${resp}=    Should Match Regexp    ${output}    destPrefix\\":\\"${VM_IP_NET1[0]}\\/32".*"${OS_COMPUTE_1_IP}\\"

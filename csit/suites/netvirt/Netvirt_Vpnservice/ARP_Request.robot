...
TC06 Same DPN MIP Migration
    ...
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${CONFIG_EXTRA_ROUTE_IP1}
    Sleep    5
    ${rx_packet2_before}=    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ifconfig eth0:1
    Log    ${rx_packet2_before}
    Log    Generate ARP from VM2
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${RPING_MIP_IP_2}
    Should Contain    ${output}    Received 0 reply
    Log    Verify Ping
    ${output} =    Execute Command on VM Instance    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ping 60.1.1.110 -c 5
    Should Contain    ${output}    ${PING_REGEXP}

...
TC01
    ...
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${VM_IP_NET10[1]}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_2_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${VM_IP_NET10[0]}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_1_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
    ...
    Log    ${resp.content}
    ${resp}=    Should Match Regexp    ${resp.content}    destPrefix\\":\\"${FIB_ENTRY_2}\/32\\",\\"label\\":\\d+,\\"nextHopAddressList\\":\\[\\"${OS_COMPUTE_2_IP}\\"\\],\\"origin\\":\\"[a-z]
    Log    ${resp}
...
Create Setup
    ...
    Create Port    ${NETWORKS[2]}    ${PORT_LIS4[3]}    sg=sg-vpnservice
    Create Port    ${NETWORKS[2]}    ${PORT_LIS5[3]}    sg=sg-vpnservice

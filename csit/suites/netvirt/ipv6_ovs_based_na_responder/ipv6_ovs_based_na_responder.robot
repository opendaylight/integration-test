...
Verify NA Responder flows of IPv6 with Single VNIC per server
    ...
    BuiltIn.Wait Until Keyword Succeeds    240s    10s    Ipv6 Ping Verification
    Verify NA Responder flows
...
Verify IPV6 NA responder flows
    [Arguments]    ${compute_node}    ${network}    ${port}    ${per_port_flow_count}    ${ipv6_address_count}
    ...
    BuiltIn.Should Contain X Times    ${flow_output}    icmp_type=135    ${per_port_flow_count}
    ...
    BuiltIn.Should Contain X Times    ${flow_output}    table=${ARP_RESPONSE_TABLE}    ${per_port_flow_count}
    ...
    ${flow_output} =    Utils.Run Command On Remote System    ${compute_node}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${INTEGRATION_BRIDGE} | grep ${IPV6_TABLE} | grep icmp_type=135 | grep metadata=${elan_tag} | grep ipv6_src=::
    BuiltIn.Should Contain X Times    ${flow_output}    ipv6_src=::    ${ipv6_address_count}
...
Verify NA Responder flows
    [Documentation]    Verify ipv6 NA responder flows accross VM's
    OVSDB.Get Flow Entries On Node    ${OS_CMP1_CONN_ID}
    OVSDB.Get Flow Entries On Node    ${OS_CMP2_CONN_ID}
    Verify IPV6 NA responder flows    ${OS_COMPUTE_1_IP}    @{NETWORKS}[0]    @{PORTS}[0]    4    2
    Verify IPV6 NA responder flows    ${OS_COMPUTE_1_IP}    @{NETWORKS}[1]    @{PORTS}[1]    4    2
    Verify IPV6 NA responder flows    ${OS_COMPUTE_2_IP}    @{NETWORKS}[0]    @{PORTS}[2]    4    2
    Verify IPV6 NA responder flows    ${OS_COMPUTE_2_IP}    @{NETWORKS}[1]    @{PORTS}[3]    4    2

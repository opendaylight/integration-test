...
VNI Based L3 Forwarding With BGPVPN Network Association
    ...
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[2]    @{VNI6_SUBNETS}[2]    @{VNI6_SUBNET_CIDRS}[2]
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[3]    @{VNI6_SUBNETS}[3]    @{VNI6_SUBNET_CIDRS}[3]
...
VNI Based L3 Forwarding With BGPVPN With Irt Ert
    ...
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[4]    @{VNI6_SUBNETS}[4]    @{VNI6_SUBNET_CIDRS}[4]
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[5]    @{VNI6_SUBNETS}[5]    @{VNI6_SUBNET_CIDRS}[5]
...
Start Suite
    ...
    ${net1_additional_args}=    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${VNI6_NET0_IPV6_ADDR_POOL}
    ${net2_additional_args}=    BuiltIn.Catenate    --ip-version=6 --ipv6-address-mode=slaac --ipv6-ra-mode=slaac ${VNI6_NET2_IPV6_ADDR_POOL}
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[0]    @{VNI6_SUBNETS}[0]    ${VNI6_SUBNET_CIDRS[0]}    ${net1_additional_args}
    OpenStackOperations.Create SubNet    @{VNI6_NETWORKS}[1]    @{VNI6_SUBNETS}[1]    @{VNI6_SUBNET_CIDRS}[1]    ${net2_additional_args}

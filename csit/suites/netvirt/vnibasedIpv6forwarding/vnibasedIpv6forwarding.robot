...
VNI Based L3 Forwarding
    ...
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI6_NET_0_PORTS}[0]    @{VNI6_NET_1_PORTS}[0]    @{VNI6_NETWORKS}[0]
    ...    @{VNI6_NETWORKS}[1]    ${VM_IP_NET0}    ${VM_IP_NET1}
...
VNI Based L3 Forwarding With BGPVPN Network Association
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[2]    @{VNI6_SUBNET_CIDRS}[2]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[3]    @{VNI6_SUBNET_CIDRS}[3]
    ${prefix_net2} =    Replace String    @{VNI6_SUBNET_CIDRS}[2]    ::/64    (:[a-f0-9]{,4}){,4}
    ...
    ...    fail_on_none=true    vm_list=@{VNI6_NET_2_VMS}[0]    network=@{VNI6_NETWORKS}[2]    subnet=${prefix_net2}
    ${prefix_net3} =    Replace String    @{VNI6_SUBNET_CIDRS}[3]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=@{VNI6_NET_3_VMS}[0]    network=@{VNI6_NETWORKS}[3]    subnet=${prefix_net3}
    ${VM_IP_NET2} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_2_VMS}[0]    network=@{VNI6_NETWORKS}[2]    subnet=${prefix_net2}
    ${VM_IP_NET3} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_3_VMS}[0]    network=@{VNI6_NETWORKS}[3]    subnet=${prefix_net3}
    ...
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI6_NET_2_PORTS}[0]    @{VNI6_NET_3_PORTS}[0]    @{VNI6_NETWORKS}[2]
    ...    @{VNI6_NETWORKS}[3]    ${VM_IP_NET2}    ${VM_IP_NET3}
...
VNI Based L3 Forwarding With BGPVPN With Irt Ert
    ...
    OpenStackOperations.Poll VM Is ACTIVE    @{VNI6_NET_4_VMS}[0]
    OpenStackOperations.Poll VM Is ACTIVE    @{VNI6_NET_5_VMS}[0]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[4]    @{VNI6_SUBNET_CIDRS}[4]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[5]    @{VNI6_SUBNET_CIDRS}[5]
    ${prefix_net4} =    Replace String    @{VNI6_SUBNET_CIDRS}[4]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=@{VNI6_NET_4_VMS}[0]    network=@{VNI6_NETWORKS}[4]    subnet=${prefix_net4}
    ${prefix_net5} =    Replace String    @{VNI6_SUBNET_CIDRS}[5]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=@{VNI6_NET_5_VMS}[0]    network=@{VNI6_NETWORKS}[5]    subnet=${prefix_net5}
    ${VM_IP_NET4} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_4_VMS}[0]    network=@{VNI6_NETWORKS}[4]    subnet=${prefix_net4}
    ${VM_IP_NET5} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_5_VMS}[0]    network=@{VNI6_NETWORKS}[5]    subnet=${prefix_net5}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Should Not Contain    ${VM_IP_NET4}    None
    Run Keyword If    '${status}' == 'FAIL'    OpenStack CLI    openstack console log show @{VNI6_NET_4_VMS}[0]    30s
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Should Not Contain    ${VM_IP_NET5}    None
    Run Keyword If    '${status}' == 'FAIL'    OpenStack CLI    openstack console log show @{VNI6_NET_5_VMS}[0]    30s
    OpenStackOperations.Copy DHCP Files From Control Node
    BuiltIn.Set Suite Variable    ${VM_IP_NET4}
    BuiltIn.Set Suite Variable    ${VM_IP_NET5}
    BuiltIn.Should Not Contain    ${VM_IP_NET4}    None
    BuiltIn.Should Not Contain    ${VM_IP_NET5}    None
    Wait Until Keyword Succeeds    60s    5s    Verify Vni Segmentation Id and Tunnel Id    @{VNI6_NET_4_PORTS}[0]    @{VNI6_NET_5_PORTS}[0]    @{VNI6_NETWORKS}[4]
    ...    @{VNI6_NETWORKS}[5]    ${VM_IP_NET4}    ${VM_IP_NET5}
...
Start Suite
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[0]    @{VNI6_SUBNET_CIDRS}[0]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Wait For Routes To Propogate    @{VNI6_NETWORKS}[1]    @{VNI6_SUBNET_CIDRS}[1]
    ${prefix_net0} =    Replace String    @{VNI6_SUBNET_CIDRS}[0]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=@{VNI6_NET_0_VMS}[0]    network=@{VNI6_NETWORKS}[0]    subnet=${prefix_net0}
    ${prefix_net1} =    Replace String    @{VNI6_SUBNET_CIDRS}[1]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=@{VNI6_NET_1_VMS}[0]    network=@{VNI6_NETWORKS}[1]    subnet=${prefix_net1}
    ${VM_IP_NET0} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_0_VMS}[0]    network=@{VNI6_NETWORKS}[0]    subnet=${prefix_net0}
    ${VM_IP_NET1} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=@{VNI6_NET_1_VMS}[0]    network=@{VNI6_NETWORKS}[1]    subnet=${prefix_net1}

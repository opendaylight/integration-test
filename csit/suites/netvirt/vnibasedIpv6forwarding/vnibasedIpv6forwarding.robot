...
VNI Based L3 Forwarding With BGPVPN Network Association
    ...
    ${subnet_cidrs}    BuiltIn.Create List    @{VNI6_SUBNETS}[2]    @{VNI6_SUBNETS}[3]
...
VNI Based L3 Forwarding With BGPVPN With Irt Ert
    ...
    ${networks}    BuiltIn.Create List    @{VNI6_NETWORKS}[4]    @{VNI6_NETWORKS}[5]
    ${subnet_cidrs}    BuiltIn.Create List    @{VNI6_SUBNETS}[4]    @{VNI6_SUBNETS}[5]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    OpenStackOperations.Wait For Routes To Propogate    ${networks}    ${subnet_cidrs}
    ${prefix_net4} =    Replace String    @{VNI6_SUBNET_CIDRS}[4]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=${VNI6_NET_4_VMS}    network=@{VNI6_NETWORKS}[4]    subnet=${prefix_net4}
    ${prefix_net5} =    Replace String    @{VNI6_SUBNET_CIDRS}[5]    ::/64    (:[a-f0-9]{,4}){,4}
    ${status}    ${message}    Run Keyword And Ignore Error    BuiltIn.Wait Until Keyword Succeeds    3x    60s    OpenStackOperations.Collect VM IPv6 SLAAC Addresses
    ...    fail_on_none=true    vm_list=${VNI6_NET_5_VMS}    network=@{VNI6_NETWORKS}[5]    subnet=${prefix_net5}
    ${VM_IP_NET4} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=${VNI6_NET_4_VMS}    network=@{VNI6_NETWORKS}[4]    subnet=${prefix_net4}
    ${VM_IP_NET5} =    OpenStackOperations.Collect VM IPv6 SLAAC Addresses    fail_on_none=false    vm_list=${VNI6_NET_5_VMS}    network=@{VNI6_NETWORKS}[5]    subnet=${prefix_net5}
...
Verify Vni Segmentation Id and Tunnel Id
    ...
    ${output} =    OpenStackOperations.Execute Command on VM Instance    ${net1}    ${vm1_ip}    ping6 -c ${PING_COUNT} ${vm2_ip}
...
Start Suite
    ...
    ${subnet_cidrs}    BuiltIn.Create List    @{VNI6_SUBNETS}[0]    @{VNI6_SUBNETS}[1]
    BuiltIn.Wait Until Keyword Succeeds    30s    10s    OpenStackOperations.Wait For Routes To Propogate    ${networks}    ${subnet_cidrs}

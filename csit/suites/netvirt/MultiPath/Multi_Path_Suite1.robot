...
TC01 Verify Distribution of traffic with 3 VM on Compute1 , 2 VM on Compute2
    ...
    Configure_Next_Hop_On_Router    ${ROUTER_NAME}    ${NO_OF_STATIC_IP}    ${VM_IP_LIST}    ${ALLOWED_IP[0]}
    ...
    Verify_VM_Mac    ${OS_COMPUTE_1_IP}    ${ALLOWED_IP[0]}    ${LOCAL_VM_PORT_LIST}    ${REMOTE_VM_PORT_LIST}
    Verify_VM_Mac    ${OS_COMPUTE_2_IP}    ${ALLOWED_IP[0]}    ${REMOTE_VM_PORT_LIST}    ${LOCAL_VM_PORT_LIST}
    Verify_Ping_And_Packet_Count    ${NETWORK_NAME}    ${ALLOWED_IP[0]}    ${VM_IP_DICT.${VM_LIST[5]}}

TC02 Verify Distribution of traffic - 2 VM on Compute1 , 2 VM on Compute2
    ...
    Log    Update the Router with MultiPath Route
    Update Router    ${ROUTER_NAME}    ${NO_ROUTE}
    ...
    Should Match Regexp    ${CTRL_FIB}    ${ALLOWED_IP[0]}\/${MASK[0]}\\s+${OS_COMPUTE_1_IP}
    Should Match Regexp    ${CTRL_FIB}    ${ALLOWED_IP[0]}\/${MASK[0]}\\s+${OS_COMPUTE_2_IP}
    Wait Until Keyword Succeeds    100s    20s    Execute Command on VM Instance    ${NETWORK_NAME}    ${VM_IP_DICT.${VM_LIST[2]}}    sudo ifconfig eth0:0 ${ALLOWED_IP[0]} netmask ${MASK[1]} down
...
Create Setup
    ...
    Comment    Neutron Security Group Create    ${SECURITY_GROUP}
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=ingress    port_range_max=65535    port_range_min=1    protocol=tcp
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=egress    port_range_max=65535    port_range_min=1    protocol=tcp
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=ingress    protocol=icmp
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=egress    protocol=icmp
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=ingress    port_range_max=65535    port_range_min=1    protocol=udp
    Comment    Neutron Security Group Rule Create    ${SECURITY_GROUP}    direction=egress    port_range_max=65535    port_range_min=1    protocol=udp

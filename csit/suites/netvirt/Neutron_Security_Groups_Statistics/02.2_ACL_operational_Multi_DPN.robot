...
*** Test Cases ***
Verify flow stats request and response and drop packets count and bytes for direction both for no rule match data packet for single port. (In Multi-DPN)
    [Setup]    PreConfig
    ...
    log    ${rpc_output}
    Switch Connection    ${devstack_conn_id2}
    ${cmd1}    Write Commands Until Prompt    neutron security-group-list
    Log    ${cmd1}
    ...
    log    ${drops}
    ${packet_drop_ingress}    get from list    ${drops}    0
    ${bytes_ingress}    get from list    ${drops}    1
    ${rpc_output1}    Statistics Call    ${direction[2]}    ${port_2_id}
    Log    ${rpc_output1}
    ...
    log    ${drops}
    ${packet_drop_egress}    get from list    ${drops}    0
    ${bytes_egress}    get from list    ${drops}    1
    ${rpc_output2}    Statistics Call    ${direction[2]}    ${port_2_id}
    Log    ${rpc_output2}
    [Teardown]

(cont...)Verify flow stats request and response and drop packets count and bytes for direction both for no rule match data packet for single port. (In Multi-DPN)
    ${devstack_conn_id}    devstack login    ${TOOLS_SYSTEM_1_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${port_2_id}    Get Port Id    ${port_name[2]}    ${devstack_conn_id}
    ${devstack_conn_id2}    Devstack Login    ${TOOLS_SYSTEM_2_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    Switch Connection    ${devstack_conn_id2}
    ${rpc_output2}    Statistics Call    ${direction[2]}    ${port_2_id}
    Log    ${rpc_output2}
    ${egress_reg}    Set Variable    {"direction":"${direction[1]}","packets":{"invalid-drop-count":[0-9]+,"drop-count":[0-9]+},"bytes":{"invalid-drop-count":0,"drop-count":[0-9]+}
    ${eg_drop}    Should Match Regexp    ${rpc_output2}    ${egress_reg}
    Log    ${eg_drop}
    ${port_1_id}    Get Port Id    ${port_name[1]}    ${devstack_conn_id}
    ${devstack_conn_id1}    Devstack Login    ${TOOLS_SYSTEM_1_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${ingress_reg}    Set Variable    {"direction":"${direction[0]}","packets":{"invalid-drop-count":[0-9]+,"drop-count":[0-9]+},"bytes":{"invalid-drop-count":0,"drop-count":[0-9]+}
    ${ig_drop}    Should Match Regexp    ${rpc_output2}    ${ingress_reg}
    Log    ${ig_drop}
    [Teardown]    Clean

Verify flow stats request and response and drop invalid packets count and bytes for direction both for no rule match data packet for single port. (In Multi-DPN)
    [Setup]    PreConfig
    Create Port    ${Networks[0]}    ${port_name[1]}    ${sg_name}
    Log    Creating port 2
    Create Port    ${Networks[0]}    ${port_name[2]}    ${sg_name}
...
(cont...)Verify flow stats request and response and drop packets count and bytes for direction both for no rule match data invalid packet for single port. (In Multi-DPN)
    ${devstack_conn_id}    devstack login    ${TOOLS_SYSTEM_1_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${port_2_id}    Get Port Id    ${port_name[2]}    ${devstack_conn_id}
    ${devstack_conn_id2}    Devstack Login    ${TOOLS_SYSTEM_2_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    Switch Connection    ${devstack_conn_id2}
...
Verify flow stats request and response and drop packets count and bytes for direction both for port with no security group attached. (In Multi-DPN).
    [Setup]    PreConfig
    ...
    ${rpc_output1}    Statistics Call    ${direction[2]}    ${port_2_id}
    Log    ${rpc_output1}
    ${delete_icmp_egress}    Rule delete    ${TOOLS_SYSTEM_2_IP}    ${sg_name}    ${protocols[0]}    ${direction[1]}
    ${command}    Set Variable    ping -c 4 ${vm1_ip}
    ${ssh_output}    Ssh To Vm    ${devstack_conn_id2}    ${Networks[0]}    ${vm2_ip}    ${command}
    should contain    ${ssh_output}    100%
...
(Cont..)Verify flow stats request and response and drop packets count and bytes for direction both for port with no security group attached. (In Multi-DPN).
    ${devstack_conn_id}    devstack login    ${TOOLS_SYSTEM_1_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    ${port_2_id}    Get Port Id    ${port_name[2]}    ${devstack_conn_id}
    ${devstack_conn_id2}    Devstack Login    ${TOOLS_SYSTEM_2_IP}    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}    ${DEVSTACK_DEPLOY_PATH}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    Switch Connection    ${devstack_conn_id2}

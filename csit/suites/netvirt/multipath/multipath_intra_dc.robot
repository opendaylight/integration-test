...
*** Variables ***
@{NETWORKS}       multipath_network_1
@{SUBNETS}        multipath_subnet_1
${ROUTERS}        multipath_router_1
@{PORT_LIST}      multipath_port_1    multipath_port_2    multipath_port_3    multipath_port_4    multipath_port_5    multipath_port_6
@{VM_LIST}        multipath_vm_1    multipath_vm_2    multipath_vm_3    multipath_vm_4    multipath_vm_5    multipath_vm_6
...
TC01 Verify Distribution of traffic with 3 VM on Compute1 , 2 VM on Compute2
    ...
    \    ...    ${vm_ip}
    ${ctrl_fib} =    KarafKeywords.Issue Command On Karaf Console    ${FIB_SHOW}
    ...
    ${packet_count_before_ping}    ${packet_count_after_ping} =    OpenStackOperations.Get_Packet_Count_Before_And_After_Ping    ${NETWORKS[0]}    ${ALLOWED_IP[0]}    ${VM_IP_DICT.${VM_LIST[5]}}    ${NO_OF_PING_PACKETS}
    ...    ${NO_OF_COMPUTE[2]}
    ${compute_node_ip} =    OpenStackOperations.Get_Compute_Node_That_Forwarded_Traffic    ${packet_count_before_ping}    ${packet_count_after_ping}    ${NO_OF_PING_PACKETS}
...
Create Setup
    ...
    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW_STATE}

...
Verify The Subnet Route When The Network Is Removed From The Vpn
    ...
    VpnOperations.Dissociate L3VPN From Multiple Networks    ${REQ_NO_NET}
    ${output} =    VpnOperations.Get Fib Entries    session
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}
    \    BuiltIn.Should Not Contain    ${output}    ${IP}
    VpnOperations.Associate L3VPN To Multiple Networks    ${REQ_NO_NET}
    ${output} =    VpnOperations.Get Fib Entries    session
...
Verify the subnet route when VSwitch hosting subnet Enterprise Host is restarted on multiple VSwitch topology
    [Documentation]    Verify Enterprise Hosts Reachability OVS Control Plane Restart
    OVSDB.Restart OVSDB    ${OS_COMPUTE_1_IP}
    OVSDB.Restart OVSDB    ${OS_COMPUTE_2_IP}
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Get Ovsdb State    ${OS_COMPUTE_1_IP}
    BuiltIn.Wait Until Keyword Succeeds    120s    20s    OVSDB.Get Ovsdb State    ${OS_COMPUTE_2_IP}
...
Verify the subnet route for one subnet on a single VSwitch
    ...
    ${fib_cmd}    Set Variable    fib-show
    ${output2} =    Issue_Command_On_Karaf_Console    ${fib_cmd}
...
Create Neutron Ports
    ...
    ${REQ_NO_PORTS} =    Evaluate    12
    : FOR    ${index}    IN RANGE    0    ${REQ_NO_PORTS}
    \    BuiltIn.Run Keyword If    ${index} <= 3    Create Port    @{REQ_NETWORKS}[0]    @{REQ_PORT_LIST}[${index}]    sg=${SECURITY_GROUP}
    \    ...    allowed_address_pairs=@{ALLOWED_IP_PORT}
    \    ...    ELSE IF    4 <= ${index} <= 7    Create Port    @{REQ_NETWORKS}[1]    @{REQ_PORT_LIST}[${index}]
    \    ...    sg=${SECURITY_GROUP}    allowed_address_pairs=@{ALLOWED_IP_PORT1}
    \    ...    ELSE    Create Port    @{REQ_NETWORKS}[2]    @{REQ_PORT_LIST}[${index}]    sg=${SECURITY_GROUP}
    \    ...    allowed_address_pairs=@{ALLOWED_IP_PORT2}
...
Create Setup
    ...
    OpenStackOperations.VM Creation Quota Update    30
...
Verify Ping between Inter Intra And Enterprise host
    ...
    ...    ping -c 10 @{VM_IP_NET1}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    BuiltIn.Wait Until Keyword Succeeds    30s    5s    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_NET1}[0]
    ...    ping -c 10 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes

...
@{VM_INSTANCES_REBOOT}    multicast_net1_vm1_1
...
@{REQ_NETWORKS}    multicast_net1    multicast_net2    multicast_net3    multicast_net4    multicast_net5    multicast_net6
...
@{VM_INSTANCES}    multicast_net1_vm1_1    multicast_net1_vm2_1    multicast_net2_vm1_1    multicast_net2_vm2_1    multicast_net3_vm1_1    multicast_net3_vm2_1    multicast_net1_vm3_2
...               multicast_net1_vm4_2    multicast_net2_vm3_2    multicast_net2_vm4_2    multicast_net3_vm3_2    multicast_net3_vm4_2
...
@{REQ_SUBNETS}    multicast_subnet1    multicast_subnet2    multicast_subnet3    multicast_subnet4    multicast_subnet5    multicast_subnet6
...
@{NET_DELETE}     multicast_net1
@{SUBNET_ADD}     multicast_subnet1
...
Delete And Reconfigure neutron port, subnet, network Hosting Enterprise Host
    ...
    OpenStackOperations.Delete Network    @{NET_DELETE}[0]
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements Not At URI    ${NETWORK_URL}    ${NET_DELETE}
    OpenStackOperations.Create Network    @{NET_DELETE}[0]
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${NETWORK_URL}    ${NET_DELETE}
    OpenStackOperations.Create SubNet    @{NET_DELETE}[0]    @{SUBNET_ADD}[0]    ${REQ_NO_CIDR}
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${SUBNETWORK_URL}    ${SUBNET_ADD}
    : FOR    ${port_del}    IN    @{PORT_DELETE}
    \    OpenStackOperations.Create Port    @{NET_DELETE}[0]    ${port_del}    sg=${SECURITY_GROUP}    allowed_address_pairs=@{ALLOWED_IP_PORT}
    Create Vm Instance With Port On Compute Node    ${PORT_DELETE[0]}    ${VM_DELETE[0]}    ${OS_CMP1_HOSTNAME}    sg=${SECURITY_GROUP}
    Create Vm Instance With Port On Compute Node    ${PORT_DELETE[1]}    ${VM_DELETE[1]}    ${OS_CMP2_HOSTNAME}    sg=${SECURITY_GROUP}
    Create Vm Instance With Port On Compute Node    ${PORT_DELETE[2]}    ${VM_DELETE[2]}    ${OS_CMP1_HOSTNAME}    sg=${SECURITY_GROUP}
    Create Vm Instance With Port On Compute Node    ${PORT_DELETE[3]}    ${VM_DELETE[3]}    ${OS_CMP2_HOSTNAME}    sg=${SECURITY_GROUP}
    @{VM_IP_NET1}    ${NET1_DHCP_IP} =    OpenStackOperations.Get VM IPs    @{REQ_VM_INSTANCES_NET1}
    Set Global Variable    @{VM_IP_NET1}
    ${REQ_NO_NET} =    Evaluate    1
    Associate L3VPN To Networks    ${REQ_NO_NET}
    BuiltIn.Log    Creating Sub Interface for    multicast_net1
    : FOR    ${vm_ip}    IN    @{VM_IP_NET1}[0]
    \    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Configure_IP_On_Sub_Interface    ${REQ_NETWORKS[0]}    ${ALLOWED_IP[0]}
    \    ...    ${vm_ip}    ${MASK[1]}
    \    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Verify_IP_Configured_On_Sub_Interface    ${REQ_NETWORKS[0]}    ${ALLOWED_IP[0]}
...
Create Neutron Ports
    ...
    \    Create Port    @{REQ_NETWORKS}[${index}]    @{REQ_PORT_LIST}[${index}]    sg=${SECURITY_GROUP}    allowed_address_pairs=@{ALLOWED_IP_PORT}
    ...
    \    Create Port    @{REQ_NETWORKS}[${index}]    @{REQ_PORT_LIST}[${index}]    sg=${SECURITY_GROUP}    allowed_address_pairs=@{ALLOWED_IP_PORT1}
    ${start} =    Evaluate    ${index}+1
    ${REQ_NO_PORTS_PER_DPN_NET3} =    Evaluate    ${start}+${REQ_NO_PORTS_PER_DPN}
    : FOR    ${index}    IN RANGE    ${start}    ${REQ_NO_PORTS_PER_DPN_NET3}
    \    Create Port    @{REQ_NETWORKS}[${index}]    @{REQ_PORT_LIST}[${index}]    sg=${SECURITY_GROUP}    allowed_address_pairs=@{ALLOWED_IP_PORT2}
...
Create Setup
    ...
    Create Neutron Ports    ${REQ_NO_PORTS}

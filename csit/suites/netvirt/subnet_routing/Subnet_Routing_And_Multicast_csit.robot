...
#Verify the subnet route when the neutron port acting like gateway for Subnet Route is deleted on single VSwitch topology
...
##    \    ${vm_list}    OpenStackOperations.List Nova VMs
##    \    BuiltIn.Wait Until Keyword Succeeds    180s    10s    BuiltIn.Should Not Contain    ${vm_list}    ${vm}
#    BuiltIn.Log    Delete the Ports from DPNs
#    : FOR    ${port_del}    IN    @{port_delete}    
#    \    OpenStackOperations.Delete Port    ${port_del}
#    \    ${port_list}    OpenStackOperations.List Ports
#    \    BuiltIn.Wait Until Keyword Succeeds    180s    10s    BuiltIn.Should Not Contain    ${port_list}    ${port_del}
#    BuiltIn.Log    Delete the Network from DPNs 
#    OpenStackOperations.Delete Network    ${net_del}
#    ${net_list}    List Networks
#    BuiltIn.Should Not Contain    ${net_list}    ${net_del}
...
#    OpenStackOperations.Create Network    ${net_del}
#    ${net_list}=    OpenStackOperations.List Networks
#    BuiltIn.Should Contain    ${net_list}    ${net_del}
#    OpenStackOperations.Create SubNet    ${net_del}    ${sub_add}    ${req_no_cidr}
#    ${sub_list}    OpenStackOperations.List Subnets
#    BuiltIn.Should Contain    ${sub_list}    ${sub_add}
#    BuiltIn.Log    Creating Port
#    : FOR    ${port_del}    IN    @{port_delete}
#    \    OpenStackOperations.Create Port    ${net_del}    ${port_del}    sg=${SECURITY_GROUP}
#    \    ${port_list}    OpenStackOperations.List Ports
#    \    BuiltIn.Should Contain    ${port_list}    ${port_del}
...
Verify the subnet route for one subnet on a single VSwitch
    ...
    VERIFY_FIBFLOWTABLE_AFTER_DCGW_TO_EPHOST_PING
    BuiltIn.Wait Until Keyword Succeeds    40s    10s    OpenStackOperations.Verify VM to VM Ping Status    @{REQ_NETWORKS}[2]    ${VM_IP_NET3[1]}    @{ALLOWED_IP}[1]    ${REQ_PING_REGEXP}
    ${cmd1}    Set Variable    fib-show
    ${output2} =    Issue_Command_On_Karaf_Console    ${cmd1}
...
Verify the subnet route for multiple subnets on multi VSwitch topology
    ...
    BuiltIn.Wait Until Keyword Succeeds    40s    10s    OpenStackOperations.Verify VM to VM Ping Status    @{REQ_NETWORKS}[2]    ${VM_IP_NET1[2]}    @{ALLOWED_IP}[3]    ${REQ_PING_REGEXP}   
    VERIFY_FIBFLOWTABLE_AFTER_DCGW_TO_EPHOST_PING
...
Verify that the broadcast traffic is always transmitted over ELAN path for network which is associated to l3vpn
    ...
    ...    ${REQ_PING_REGEXP}
    BuiltIn.Wait Until Keyword Succeeds    40s    10s    OpenStackOperations.Verify VM to VM Ping Status    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET2[2]}
    ...    ${REQ_PING_REGEXP}
    BuiltIn.Log    check ELAN Datapath Traffic For Broadcast Traffic From VM
    BuiltIn.Wait Until Keyword Succeeds    40s    10s    OpenStackOperations.Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET5}
    ...    ${REQ_PING_REGEXP_FAIL}
...
Verify that the multicast traffic is always transmitted over ELAN path for network which is associated to l3vpn
    ...
    BuiltIn.Wait Until Keyword Succeeds    40s    10s    OpenStackOperations.Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET6}
    ...    ${REQ_PING_REGEXP_FAIL}
    ${dumpflow_compute_node1}=    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo tcpdump -ttttnnr /tmp/tcpDumpCN1.pcap | grep ICMP
    BuiltIn.Log    ${dumpflow_compute_node1}
    ${dumpflow_compute_node2}=    Utils.Run Command On Remote System    ${OS_COMPUTE_1_IP}    sudo tcpdump -ttttnnr /tmp/tcpDumpCN1.pcap | grep ARP
...
VERIFY_FIBFLOWTABLE_AFTER_DCGW_TO_EPHOST_PING
    ...
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_NET1}[0]    ping -c 3 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_NET1}[3]    ping -c 3 @{VM_IP_NET1[2]}
    BuiltIn.Should Contain    ${output}    64 bytes
    BuiltIn.Log    "Verification of intra_network_inter_openvswitch network connectivity"
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[0]    ping -c 3 @{VM_IP_NET1[2]}
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[0]    ping -c 3 @{VM_IP_NET2[2]}
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[0]    ping -c 3 @{ALLOWED_IP}[2]
    BuiltIn.Should Contain    ${output}    64 bytes
    BuiltIn.Log    "Verification of inter_network_intra_openvswitch network connectivity"
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[0]    ping -c 3 @{VM_IP_NET2[0]}
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[1]    ping -c 3 @{VM_IP_NET2[1]}
    BuiltIn.Should Contain    ${output}    64 bytes
    BuiltIn.Log    "Verification of inter_network_intra_openvswitch network connectivity between EP Hosts"
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[0]    ping -c 3 @{ALLOWED_IP}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[1]    ping -c 3 @{ALLOWED_IP}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[0]    ping -c 3 @{ALLOWED_IP}[2]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[0]    @{VM_IP_NET1}[2]    ping -c 3 @{ALLOWED_IP}[2]
    BuiltIn.Should Contain    ${output}    64 bytes

    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[0]    ping -c 3 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[1]    ping -c 3 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[0]    ping -c 3 @{ALLOWED_IP}[2]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[1]    @{VM_IP_NET2}[2]    ping -c 3 @{ALLOWED_IP}[2]
    BuiltIn.Should Contain    ${output}    64 bytes

    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[2]    @{VM_IP_NET3}[0]    ping -c 3 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[2]    @{VM_IP_NET3}[1]    ping -c 3 @{ALLOWED_IP}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[2]    @{VM_IP_NET3}[0]    ping -c 3 @{ALLOWED_IP}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS[2]    @{VM_IP_NET3}[2]    ping -c 3 @{ALLOWED_IP}[1]
    BuiltIn.Should Contain    ${output}    64 bytes

...
Delete And Reconfigure neutron port, subnet, network Hosting Enterprise Host
    ...
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements Not At URI    ${NETWORK_URL}    ${REQ_NETWORKS}
    OpenStackOperations.Create Network    @{REQ_NETWORKS}[0]
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${NETWORK_URL}    ${REQ_NETWORKS}
    OpenStackOperations.Create SubNet    @{REQ_NETWORKS}[0]    @{REQ_SUBNETS}[0]    ${REQ_NO_CIDR}
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${SUBNETWORK_URL}    ${REQ_SUBNETS}
    : FOR    ${port_del}    IN    @{PORT_DELETE}

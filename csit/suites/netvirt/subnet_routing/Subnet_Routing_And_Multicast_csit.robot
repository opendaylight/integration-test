...
Delete And Reconfigure neutron port, subnet, network Hosting EP Host
    ...
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements Not At URI    ${NET_URL}    @{net_del}
    ...
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${NET_URL}    @{net_del}
    OpenStackOperations.Create SubNet    ${net_del}    @{sub_add}[0]    ${req_no_cidr}
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${SUBNET_URL}    @{sub_add}
    BuiltIn.Log    Creating Port
    : FOR    ${port_del}    IN    @{port_delete}
    \    OpenStackOperations.Create Port    ${net_del}    ${port_del}    sg=${SECURITY_GROUP}
    \    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${PORT_URL}    ${port_del}
...
Delete And Reconfigure neutron port, subnet, network Hosting EP Host
    ...
    : FOR    ${port_del}    IN    @{port_delete}
    \    OpenStackOperations.Delete Port    ${port_del}
    \    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements Not At URI    ${PORT_URL}    ${port_del}
    BuiltIn.Log    Delete the Network from DPNs

...
Verify the subnet route for multiple subnets on multi VSwitch topology
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Configure_IP_On_Sub_Interface    @{REQ_NETWORKS}[0]    ${NEW_EXTRA_NW_SUBNET}
    ...    @{VM_IP_NET1}[2]    @{MASK}[1]
    BuiltIn.Wait Until Keyword Succeeds    30s    5s    Verify_IP_Configured_On_Sub_Interface    @{REQ_NETWORKS}[0]    ${NEW_EXTRA_NW_SUBNET} 
    ...    @{VM_IP_NET1}[2]
...
Create Setup
    ...
    ${id} =    OpenStackOperations.Get Project Id    ${PROJECT_NAME}
...
Create L3VPN
    ...
    VpnOperations.VPN Create L3VPN    vpnid=${VPN_INSTANCE_ID}    name=@{VPN_NAME}[0]    rd=["${L3VPN_RD}"]    exportrt=["${L3VPN_RD}"]    importrt=["${L3VPN_RD}"]    tenantid=${tenant_id}

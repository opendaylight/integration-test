*** Settings ***
Documentation     Test Suite for Gateway mac based L2L3 seggragation to check
...   the end to end flows for ARP based learning
...
${NEXTHOP}    0.0.0.0
${VPN_INSTANCE_ID}    4ae8cd92-48ca-49b5-94e1-b2921a26111
${VPN_NAME}       vpn1
${CREATE_RD}    ["100:31"]
...
*** Test Cases ***
Verify table Miss entry for GWMAC table 19 points to table 17 dispatcher table
    [Documentation]     To Verify there should be an entry for  dispatcher table in the gateway mac  dump_flows
...
Verify that table miss entry for table 17 should not point to table 81 arp table_
...
Verify ARP eth_type entries and actions for ARP request's are populated on GWMAC table
...
Verify ARP eth_type entries and actions for ARP responses are populated on GWMAC table
...
Verify GWMAC entires are populated with Neutron Router MAC address per network in GWMAC table
    ...
    ${flow_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows}|grep ${TABLE_NO_19}
    Should Contain    ${flow_output}   dl_dst=${gw_mac_addr_2}     actions=goto_table:${L3VPN_TABLE}
    #Add Ping across network
    ${output}=    FIB Output
    Check IP And UUID In FIB Entry    ${output}    ${router_id}    @{DEFAULT_GATEWAY_IPS}[1]    ${NEXTHOP}


Verify GWMAC entires are populated with port MAC address for network to VPN association in GWMAC table
...
Check IP And UUID In FIB Entry
    [Arguments]    ${output_str}    ${rtr_id}    ${prefix}    ${nexthop}
    [Documentation]    Verification of routes entry with correct nexthop
    Log    Output String should contain \\s*${rtr_id}\\s*${prefix}/32\\s*${nexthop} in FIB entry
    Should Match Regexp    ${output_str}    \\s*${rtr_id}\\s*${prefix}/32\\s*${nexthop}
    [Return]    ${output_str}

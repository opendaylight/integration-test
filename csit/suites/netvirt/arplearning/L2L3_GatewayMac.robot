...
Verify table Miss entry for GWMAC table 19 points to table 17 dispatcher table
    [Documentation]     To Verify there should not be an entry for  dispatcher table in the gateway mac  dump_flows
...
Check IP And UUID In FIB Entry
    [Arguments]    ${output_str}    ${prefix}    ${nexthop}
    [Documentation]    Verification of routes entry with correct nexthop
    Log    Output String should contain \\s*${prefix}/32\\s*${nexthop} in FIB entry
    Should Match Regexp    ${output_str}    \\s*${prefix}/32\\s*${nexthop}
    [Return]    ${output_str}

...
Verify GWMAC entires are populated with Neutron Router MAC address per network in GWMAC table
    ...
    VpnOperations.Dissociate L3VPN From Networks    networkid=${network1_id}    vpnid=${vpn_instance}
    ${resp}=    VpnOperations.VPN Get L3VPN    vpnid=${vpn_instance}
    VpnOperations.Dissociate L3VPN From Networks    networkid=${network2_id}    vpnid=${vpn_instance}
    ${resp}=    VpnOperations.VPN Get L3VPN    vpnid=${vpn_instance}
    ...
    Check IP And UUID In FIB Entry    ${output}    ${rtr_id}    @{DEFAULT_GATEWAY_IPS}[1]    ${NEXTHOP}

...
TC2_Verify the pipeline flow from dispatcher table 17 (L3VPN) to table 19
    ...
    ${port_num_1}    Get Port Number    ${subport_id_1}    ${OS_CMP1_CONN_ID}
    ${port_num_2}    Get Port Number    ${subport_id_2}    ${OS_CMP1_CONN_ID} 
    ...
    ${metadata}    Get Metadata    ${OS_CMP1_CONN_ID}    ${port_num_1}
    ...
    #Get ControlNode Connection
    ${gw_mac_addr}   Get Default Mac Addr    ${DEFAULT_GATEWAY_IPS[0]}    ${OS_CMP1_CONN_ID} 
    $D{port_mac_addr}    Get Port Mac     ${PORT_LIST[0]}
...
TC6_Verify that Multiple GWMAC entries in GWMAC table points to FIB table 21 (L3VPN pipeline)
    ...
    #Get ControlNode Connection
    ${gw_mac_addr_1}   Get Default Mac Addr    ${DEFAULT_GATEWAY_IPS[0]}    ${OS_CMP1_CONN_ID} 
    ${gw_mac_addr_2}   Get Default Mac Addr    ${DEFAULT_GATEWAY_IPS[1]}    ${OS_CMP1_CONN_ID}
    ${flow_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows} |grep ${TABLE_NO_19}
    ...
    ${pkt_count_before_ping}    Get Packetcount    br-int    ${OS_CMP1_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_1}
    Log    ${pkt_count_before_ping}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_DPN1}[0]    ping -c 8 @{VM_IP_DPN2}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping}    Get Packetcount    br-int    ${OS_CMP1_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_1}
    Log    ${pkt_count_after_ping}
    ${pkt_diff}    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    Should Be True    ${pkt_diff} > 0
    ${pkt_count_before_ping}    Get Packetcount    br-int    ${OS_CMP1_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_2}
    Log    ${pkt_count_before_ping}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN1}[1]    ping -c 8 @{VM_IP_DPN2}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping}    Get Packetcount    br-int    ${OS_CMP1_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_2}
    Log    ${pkt_count_after_ping}
    ${pkt_diff}    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    Should Be True    ${pkt_diff} > 0
    Log   Verify Ping On Second DPN
    ${pkt_count_before_ping}    Get Packetcount    br-int    ${OS_CMP2_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_1}
    Log    ${pkt_count_before_ping}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_DPN2}[0]    ping -c 8 @{VM_IP_DPN1}[1]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping}    Get Packetcount    br-int    ${OS_CMP2_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_1}
    Log    ${pkt_count_after_ping}
    ${pkt_diff}    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    Should Be True    ${pkt_diff} > 0
    Log   Verify Ping On Second DPN
    ${pkt_count_before_ping}    Get Packetcount    br-int    ${OS_CMP2_CONN_ID}  ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_2}
    Log    ${pkt_count_before_ping}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN2}[1]    ping -c 8 @{VM_IP_DPN1}[0]
    BuiltIn.Should Contain    ${output}    64 bytes
    ${pkt_count_after_ping}    Get Packetcount    br-int    ${OS_CMP2_CONN_ID}   ${TABLE_NO_19} |grep dl_dst=${gw_mac_addr_2}
    Log    ${pkt_count_after_ping}
    ${pkt_diff}    Evaluate    int(${pkt_count_after_ping})-int(${pkt_count_before_ping})
    Should Be True    ${pkt_diff} > 0
...
TC10_Verify GWMAC entires are populated with Neutron Router MAC address per network in GWMAC table
    ...
    ${port_num_2}    Get Port Number    ${subport_id}    ${OS_CMP1_CONN_ID}
   #Get ControlNode Connection 
    ${gw_mac_addr_2}   Get Default Mac Addr    ${DEFAULT_GATEWAY_IPS[1]}    ${OS_CMP1_CONN_ID} 
    ${flow_output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${dump_flows}|grep ${TABLE_NO_19}
    Should Contain    ${flow_output}   dl_dst=${gw_mac_addr_2}     actions=goto_table:${L3VPN_TABLE}
    ...
    #Check IP And UUID In FIB Entry    ${output}       100:31    @{DEFAULT_GATEWAY_IPS}[1]
...
Get Default Mac Addr
    [Arguments]    ${default_gw_ip}   ${conn_id}    
    [Documentation]    Retrieve the port id for the given port name to attach specific vm instance to a particular port
    SSHLibrary.Switch Connection    ${conn_id}
    ${output}=    DevstackUtils.Write Commands Until Prompt   openstack port list | grep -w ${default_gw_ip} | awk '{print $5}'    60s
    Log    ${output}

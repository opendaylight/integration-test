...
Create Neutron Ports
    ...
    \    Create Port    @{REQ_NETWORKS}[${index}]    @{PORTS_LIST}[${index}]    sg=${SECURITY_GROUP}    additional_args=${allowed_address_pairs_args}
    : FOR    ${index}    IN RANGE    0    ${num_of_ports_per_host}
    \    Create Port    @{REQ_NETWORKS}[${index}]    @{PORTS_LIST}[${index}]    sg=${SECURITY_GROUP}    additional_args=${allowed_address_pairs_args}
    Wait Until Keyword Succeeds    3s    1s    Utils.Check For Elements At URI    ${CONFIG_API}/neutron:neutron/ports/    ${PORTS}
    ${PORTS_MACADDR} =    Get Ports MacAddr    ${PORTS_LIST}
    Set Suite Variable    ${PORTS_LIST_MACADDR}
    Update Port    @{PORTS_LIST}[0]    additional_args=--description ${UPDATE_PORT}
    ${output} =    Show Port    @{PORTS_LIST}[0]

...
Create Neutron Subnets
    ...
    : FOR    ${index}    IN RANGE    0    ${NUM_OF_NETWORK}
    \    Run Keyword And Ignore Error    create SubNet   ${REQ_SUBNETS}  
    \   Run Keyword And Ignore Error     Create SubNet    ${REQ_NETWORKS[${index}]}    ${REQ_SUBNETS[${index}]}    ${REQ_SUBNET_CIDR[${index}]}
...
Create Setup
    ...
    ${Dpn1Id}    OVSDB.Get DPID    ${OS_COMPUTE_1_IP}
    ${Dpn2Id}    OVSDB.Get DPID    ${OS_COMPUTE_2_IP}
...
Add Interfaces To Routers
    ...
    ${interface_output} =    Show Router Interface    ${REQ_ROUTER}  
    : FOR    ${INTERFACE}    IN    @{ROUTER_INTERFACE}

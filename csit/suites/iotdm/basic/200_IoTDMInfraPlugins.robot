...
2.00.01 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - HTTP
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/iotdm_plugin_registrations/http/default    verify=True
...
2.00.02 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - CoAP
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/iotdm_plugin_registrations/coap/default    verify=True
...
2.00.03 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - MQTT
    ...
    TemplatedRequests.Post_As_Json_Templated_Check_Not_Included_Subset    folder=${VAR_BASE}/iotdm_plugin_registrations/mqtt/default
...
2.00.04 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - WS
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/iotdm_plugin_registrations/ws/default    verify=True
...
2.00.05 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - Onem2mExample
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/iotdm_plugin_registrations/onem2m_example/default    verify=True
...
3.00.02 Default result of onem2m-plugin-manager-db-api-client-registrations RPC - TSDR
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/db_registrations/tsdr/default    verify=True
...
3.00.03 Default result of onem2m-plugin-manager-db-api-client-registrations RPC - SimpleAdapter
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/db_registrations/simple_adapter/default    verify=True
...
4.00.01 Default result of onem2m-plugin-manager-simple-config-client-registrations RPC - Onem2mExample
    [Documentation]    Verifies the result of RPC and looks for Onem2mExample registration
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/simpleconfig_registrations/default    verify=True
...
5.00.01 Default Communication channel for HTTP plugin
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/communication_channels/http/default    verify=True
...
5.00.02 Default Communication channel for CoAP plugin
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/communication_channels/coap/default    verify=True
...
5.00.03 Default Communication channel for MQTT plugin
    ...
    TemplatedRequests.Post_As_Json_Templated_Check_Not_Included_Subset    folder=${VAR_BASE}/communication_channels/mqtt/default
...
5.00.04 Default Communication channel for WS plugin
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/communication_channels/ws/default    verify=True
...
5.00.05 Default Communication channel for Onem2mExample plugin
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/communication_channels/onem2m_example/default    verify=True
...
7.00 Test missing configuration of HTTP provider module
    ...
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=missing_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing secure-connection
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=missing_sec_conn_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing server-port
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=missing_server_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
7.01 Test invalid values in configuration of HTTP provider module
    ...
    # Invalid server-port
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=invalid_server_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid security-level
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=invalid_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid secure-connection
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/http_cfg    req_file_base_name=invalid_sec_conn_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
7.02 Test missing configuration of CoAP provider module
    ...
    # Missing security-level
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=missing_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing secure-connection
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=missing_sec_conn_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing server-port
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=missing_server_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
7.03 Test invalid values in configuration of CoAP provider module
    ...
    # Invalid server-port
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=invalid_server_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid security-level
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=invalid_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid secure-connection
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/coap_cfg    req_file_base_name=invalid_sec_conn_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
7.04 Test missing configuration of MQTT provider module
    ...
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=missing_mqtt_broker_ip_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing MQTT broker port
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=missing_mqtt_broker_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Missing security level
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=missing_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
7.05 Test invalid values in configuration of MQTT provider module
    ...
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=invalid_mqtt_broker_ip_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid security level
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=invalid_sec_level_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
    # Invalid MQTT broker port number
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/mqtt_cfg    req_file_base_name=invalid_mqtt_broker_port_req    verify=${false}    expected_status_codes=${NEGATIVE_RESULTS}
...
9.06 Test onem2m-simple-config:iplugin-cfg-key-del deletes single data
    ...
    ${payload} =    Set Variable    {"input": {"plugin-name":"Onem2mExample", "instance-name":"default", "cfg-key":"testKey2"}}
    TemplatedRequests.Post_As_Json_To_Uri    uri=${uri}    data=${payload}    expected_status_codes=${NEGATIVE_RESULTS}
...
9.09 Test onem2m-simple-config:iplugin-cfg-put to add wrong values and expect error
    ...
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/simple_config/iplugin_cfg_put    req_file_base_name=non_reg_plugin_req    rsp_file_name=post_non_reg_plugin_rsp    verify=${true}    expected_status_codes=${NEGATIVE_RESULTS}
    # non-registered instance
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/simple_config/iplugin_cfg_put    req_file_base_name=non_reg_instance_req    rsp_file_name=post_non_reg_instance_rsp    verify=${true}    expected_status_codes=${NEGATIVE_RESULTS}
...
9.13 Test IoTDM BUG 7593
    ...
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/simple_config/multi_delete    req_file_base_name=delete_non_existing_req    rsp_file_name=post_delete_non_existing_rsp    verify=${true}    expected_status_codes=${NEGATIVE_RESULTS}
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/simple_config/multi_delete    req_file_base_name=delete_non_existing_req    rsp_file_name=post_delete_non_existing_rsp    verify=${true}    expected_status_codes=${NEGATIVE_RESULTS}
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/negative_tcs/simple_config/multi_delete    req_file_base_name=delete_non_existing_req    rsp_file_name=post_delete_non_existing_rsp    verify=${true}    expected_status_codes=${NEGATIVE_RESULTS}
...
Verify Default PluginData Output
    ...
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/http/default    verify=True
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/coap/default    verify=True
    TemplatedRequests.Post_As_Json_Templated_Check_Not_Included_Subset    folder=${VAR_BASE}/plugin_data/mqtt/default
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/ws/default    verify=True
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/onem2m_example/default    verify=True
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/tsdr/default    verify=True
    TemplatedRequests.Post_As_Json_Templated_Subset    folder=${VAR_BASE}/plugin_data/simple_adapter/default    verify=True

...
...             "karaf-archive-url" : "https://nexus.opendaylight.org/service/local/repo_groups/staging/content/org/opendaylight/controller/features-extras/1.8.0-Carbon/features-extras-1.8.0-Carbon.kar",
...             "features-to-install": [
...                 {
...                     "feature-name" : "odl-jolokia"
...                 }
...             ]
...         }
...     }

${odl_jolokia_uninstall}
...     {
...         "input": {
...             "karaf-feature-loader-name" : "KarafFeatureLoaderDefault",
...             "karaf-archive-name" : "features-extras-1.8.0-Carbon"
...         }
...     }
...
6.01 Change port number of CoAP provider plugin
    ...
    # todo sleep should be deleted and test should pass. It is added because of race condition - coap is not configured if we don`t wait
    sleep    1s
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/coap/changed_port    session=ClusterManagement__session_1    verify=True
...
6.05 Restart IoTDM and verify configuration of all plugins
    ...
    run keyword if      ${USING_CLUSTER} == "true"
    ...    Restart Cluster
    ...    ELSE
    ...    Restart Local
...
Check PluginManager RPC
    [Documentation]      Tests if the PluginManager's RPC calls are available. This check is used as a test if the
    ...                  tested IoTDM instance is ready.
    ${resp} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-plugin-manager:onem2m-plugin-manager-plugin-data    headers=${headers}
    ${status_code} =    Status Code    ${resp}
    Should Be Equal As Integers    ${status_code}    200

Run_keyword_and_fail_if_passed
    [Documentation]     Negates keyword return status
    [Arguments]  ${keyword}     @{args}
    ${ret} =   BuiltIn.run_keyword_and_return_status   ${keyword}    @{args}
    run keyword if  ${ret}
    ...     fail

Wait_Until_Keyword_Fails
    [Documentation]     Executes keyword and retries every ${interval} if keyvord passed.
    ...                 Execution is repeated util keyword fails or timeout is reached.
    [Arguments]  ${timeout}     ${interval}     ${keyword}     @{args}
    BuiltIn.Wait_Until_Keyword_Succeeds     ${timeout}      ${interval}   Run_keyword_and_fail_if_passed      ${keyword}   @{args}

Prepare Restart Settings
    [Documentation]     Sets global variable USING_CLUSTER according to provided parameters
    Set global variable     ${USING_CLUSTER}    "true"
    Variable should exist   ${LOCAL_KARAF_DIR}
    Set global variable     ${USING_CLUSTER}    "false"
...
Start
    ...
    Run keyword and ignore error    Prepare Restart Settings
    ...
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_BASE}/put_coap    session=ClusterManagement__session_1
...
Restart Local
    [Documentation]     Restarts IoTDM running on local machine, LOCAL_KARAF_DIR variable must be set
    Log     Restarting local IoTDM instance
    run process     ${LOCAL_KARAF_DIR}/target/assembly/bin/stop
    Wait_Until_Keyword_Fails    90 sec  1 sec   Check PluginManager RPC
    run process     ${LOCAL_KARAF_DIR}/target/assembly/bin/start
    BuiltIn.Wait_Until_Keyword_Succeeds     90 sec  1 sec   Check PluginManager RPC

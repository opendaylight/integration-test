...
Configure Mqtt
    ${payload} =    Set Variable    {"onem2m-protocol-mqtt-providers" : [{"mqtt-provider-instance-name": "MqttProviderDefault","mqtt-client-config": {"mqtt-broker-port" : 1883,"mqtt-broker-ip" : "127.0.0.1","security-level" : "l0"}}]}
...
1.00 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC
    ...
    sleep    1s
    ${data} =    Set Variable    {"input": {}}
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-iotdm-plugin-registrations    data=${data}    headers=${headers}
...
3.00 Default result of onem2m-plugin-manager-simple-config-client-registrations RPC
    [Documentation]    Verifies the result of RPC and looks for Onem2mExample registration
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-simple-config-client-registrations    headers=${headers}
    Set Suite Variable    ${response}
    Check Response Succesfull With Correct Data    ${response}    Onem2mExample
...
4.00 Default result of onem2m-plugin-manager-plugin-data RPC
    ...
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    @{list}
...
4.01 Plugin data of HTTP
    ...
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    data=${payload}    headers=${headers}
...
5.00 Default result of onem2m-plugin-manager-communication-channels RPC
    ...
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-communication-channels    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    @{list}
...
5.01 Communication channel for HTTP plugins
    ...
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-communication-channels    data=${payload}    headers=${headers}
...
6.00 Store current outputs of PluginManager RPC calls
    ...
    ${response_suite} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    headers=${headers}
...
6.01 Change port number of HTTP provider plugin
    ...
    ${response} =    RequestsLibrary.Get Request    ClusterManagement__session_1    restconf/config/onem2m-protocol-http:onem2m-protocol-http-providers/HttpProviderDefault    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    HttpProviderDefault    "server-port":7777
    ...
    sleep    1s
    ${payload} =    Set Variable    {"input": {"plugin-name":"http(s)-base"}}
    ${response1} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    data=${payload}    headers=${headers}
    ...
    Should Be Equal    ${value}    ${value1}
    ${json_value1} =    find json children by value    http(s)-base    ${response_suite.json()['output']['onem2m-plugin-manager-plugins-table']}
    ${value1} =    Set Variable    ${json_value1['onem2m-plugin-manager-plugin-instances'][0]['plugin-configuration']['onem2m-protocol-http:server-config']['server-port']}
...
6.03 Change port number of MQTT provider plugin
    ...
    ${payload} =    Set Variable    {"onem2m-protocol-mqtt-providers" : [{"mqtt-provider-instance-name": "MqttProviderDefault","mqtt-client-config": {"mqtt-broker-port" : 7778,"mqtt-broker-ip" : "127.0.0.1","security-level" : "l0"}}]}
    ${response} =    RequestsLibrary.Put Request    ClusterManagement__session_1    restconf/config/onem2m-protocol-mqtt:onem2m-protocol-mqtt-providers/MqttProviderDefault    data=${payload}    headers=${headers}
    ${status_code} =    Status Code    ${response}
    Should Be True    199 < ${status_code} < 299
    ${response} =    RequestsLibrary.Get Request    ClusterManagement__session_1    restconf/config/onem2m-protocol-mqtt:onem2m-protocol-mqtt-providers/MqttProviderDefault    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    MqttProviderDefault    "mqtt-broker-port":7778
    # todo sleep should be deleted and test should pass. It is added because of race condition - mqtt is not configured if we don`t wait
    sleep    1s
    ${payload} =    Set Variable    {"input": {"plugin-name":"mqtt"}}
    ${response1} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    data=${payload}    headers=${headers}
    Check Response Succesfull With Correct Data    ${response1}    "port":7778    "mqtt-broker-port":7778
    ${json_value1} =    Get Json Value    ${response1.content}    /output/onem2m-plugin-manager-plugins-table/0/onem2m-plugin-manager-plugin-instances/0/plugin-configuration/onem2m-protocol-mqtt:mqtt-client-config
    ${json_value} =    Get Json Value    ${response.content}    /onem2m-protocol-mqtt-providers/0/mqtt-client-config
    ${value} =    Get Json Value    ${json_value}    /mqtt-broker-port
    ${value1} =    Get Json Value    ${json_value1}    /mqtt-broker-port
    Should Be Equal    ${value}    ${value1}
    ${value} =    Get Json Value    ${json_value}    /security-level
    ${value1} =    Get Json Value    ${json_value1}    /security-level
    Should Be Equal    ${value}    ${value1}
    ${value} =    Get Json Value    ${json_value}    /mqtt-broker-ip
    ${value1} =    Get Json Value    ${json_value1}    /mqtt-broker-ip
    Should Be Equal    ${value}    ${value1}
    ${value} =    Get Json Value    ${json_value}    /mqtt-broker-port
    ${value1} =    Get Json Value    ${json_value1}    /mqtt-broker-port
    Should Be Equal    ${value}    ${value1}
    ${json_value1} =    find json children by value    mqtt    ${response_suite.json()['output']['onem2m-plugin-manager-plugins-table']}
    ${value1} =    Set Variable    ${json_value1['onem2m-plugin-manager-plugin-instances'][0]['plugin-configuration']['onem2m-protocol-mqtt:mqtt-client-config']['mqtt-broker-port']}
    Should Not Be Equal    ${value1}    ${value}
...
6.06 Restart IoTDM and verify configuration of all plugins
    ...
    ${payload} =    Set Variable    {"input": {"plugin-name": "Onem2mExample"}}
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${PLUGIN_CONTEXT}:onem2m-plugin-manager-plugin-data    data=${payload}    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    "port":7778    "plugin-name":"Onem2mExample"
    ${response} =    RequestsLibrary.Get Request    ClusterManagement__session_1    restconf/config/onem2m-protocol-http:onem2m-protocol-http-providers/HttpProviderDefault    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    HttpProviderDefault    "server-port":7777
    ${response} =    RequestsLibrary.Get Request    ClusterManagement__session_1    restconf/config/onem2m-protocol-mqtt:onem2m-protocol-mqtt-providers/MqttProviderDefault    headers=${headers}
    Check Response Succesfull With Correct Data    ${response}    MqttProviderDefault    "mqtt-broker-port":7778
    #todo check for ws and coap when feature is added to IoTDM project
...
9.00 Test onem2m-simple-config:iplugin-cfg-get doesn`t contain anything in key-val-list
    ...
    ${payload} =    Set Variable    {"input": {"plugin-name":"Onem2mExample", "instance-name":"default"}}
    ${resp} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-get    data=${payload}    headers=${headers}
...
9.04 Test onem2m-simple-config:iplugin-cfg-key-put set single data
    ...
    ${resp} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-key-put    data=${payload}    headers=${headers}
    ${status_code} =    Status Code    ${resp}
    Should Be Equal As Integers    ${status_code}    200
...
9.05 Test onem2m-simple-config:iplugin-cfg-key-del deletes single data
...
9.06 Test onem2m-simple-config:iplugin-cfg-put to add wrong values and epect error
...
9.07 Test onem2m-simple-config:iplugin-cfg-get-startup contains expected data
    [Documentation]    Check if startup content contains only expected data
    ${payload} =    Set Variable    {"input": {"plugin-name":"Onem2mExample", "instance-name":"default"}}
    ${resp} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-get-startup    data=${payload}    headers=${headers}
    ${status_code} =    Status Code    ${resp}
...
9.08 Test iplugin-cfg-get-startup-config and iplugin-cfg-get-running-config should equal
    [Documentation]    Tests if StartupConfig and running config are the same in content
    ${resp1} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-get-startup-config    headers=${headers}
    ${status_code} =    Status Code    ${resp1}
    Should Be Equal As Integers    ${status_code}    200
    ${resp2} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-get-running-config    headers=${headers}
...
9.10 Test onem2m-simple-config:iplugin-cfg-del to delete all the values
    ...
    ${status_code} =    Status Code    ${resp}
    Should Be Equal As Integers    ${status_code}    200
    ${resp} =    RequestsLibrary.Post Request    ClusterManagement__session_1    ${REST_CONTEXT}/onem2m-simple-config:iplugin-cfg-get-startup-config    headers=${headers}
    ${status_code} =    Status Code    ${resp}
    Should Be Equal As Integers    ${status_code}    200
...
Check Response Succesfull With Correct Data
    ...
    [Documentation]    Checks if ${response} code is 200 and content conatis all @{checked}
    Should Be Equal As Strings    ${response.status_code}    200
    Should Contain All Sub Strings    ${response.content}    @{checked}

Find Json Children By Value
    [Arguments]    ${value}    ${json}
    [Documentation]    Iterates through ${json} list and search for specific ${value}. Returns first child from list
    ...    in which we can find the ${value}.
...
Check Common Data On Specific Path
    ...
    [Documentation]    Checks if ${json_value} common data are as expected
    Log    ${json_value}

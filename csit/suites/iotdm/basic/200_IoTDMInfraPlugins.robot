...
2.00.01 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - HTTP
    ...
    ${expr} =    Set Variable    "output"."registered-iotdm-plugins-table"[?"plugin-name" == `http(s)-base`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/iotdm_plugin_registrations/http/default    verify=True    jmespath_expr=${expr}
...
2.00.02 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - CoAP
    ...
    ${expr} =    Set Variable    "output"."registered-iotdm-plugins-table"[?"plugin-name" == `coap(s)-base`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/iotdm_plugin_registrations/coap/default    verify=True    jmespath_expr=${expr}
...
2.00.04 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - WS
    ...
    ${expr}    Set Variable    "output"."registered-iotdm-plugins-table"[?"plugin-name" == `ws`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/iotdm_plugin_registrations/ws/default    verify=True    jmespath_expr=${expr}
...
2.00.05 Default result of onem2m-plugin-manager-iotdm-plugin-registrations RPC - Onem2mExample
    ...
    ${expr}    set variable    "output"."registered-iotdm-plugins-table"[?"plugin-name" == `Onem2mExample`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/iotdm_plugin_registrations/onem2m_example/default    verify=True    jmespath_expr=${expr}
...
3.00.02 Default result of onem2m-plugin-manager-db-api-client-registrations RPC - TSDR
    ...
    ${expr}    Set Variable    "output"."registered-db-api-client-plugins-table"[?"plugin-name" == `Onem2mTsdrProvider`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/db_registrations/tsdr/default    verify=True    jmespath_expr=${expr}
...
3.00.03 Default result of onem2m-plugin-manager-db-api-client-registrations RPC - SimpleAdapter
    ...
    ${expr}    Set Variable    "output"."registered-db-api-client-plugins-table"[?"plugin-name" == `Onem2mSimpleAdapterProvider`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/db_registrations/simple_adapter/default    verify=True    jmespath_expr=${expr}
...
4.00.01 Default result of onem2m-plugin-manager-simple-config-client-registrations RPC - Onem2mExample
    [Documentation]    Verifies the result of RPC and looks for Onem2mExample registration
    ${expr}    Set Variable    "output"."registered-simple-config-client-plugins-table"[?"plugin-name" == `Onem2mExample`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/simpleconfig_registrations/default    verify=True    jmespath_expr=${expr}
...
5.00.01 Default Communication channel for HTTP plugin
    ...
    ${expr}    Set Variable    "output"."onem2m-communication-channel-protocols"[?"communication-channel-protocol" == `http`] | [0]."onem2m-communication-channel-addresses"[?"communication-channel-address" == `0.0.0.0`] | [0]."onem2m-communication-channel-ports"[?"onem2m-communication-channel-port" == `8282`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/communication_channels/http/default    verify=True    jmespath_expr=${expr}
...
5.00.02 Default Communication channel for CoAP plugin
    ...
    ${expr}    Set Variable    "output"."onem2m-communication-channel-protocols"[?"communication-channel-protocol" == `coap`] | [0]."onem2m-communication-channel-addresses"[?"communication-channel-address" == `0.0.0.0`] | [0]."onem2m-communication-channel-ports"[?"onem2m-communication-channel-port" == `5683`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/communication_channels/coap/default    verify=True    jmespath_expr=${expr}
...
5.00.04 Default Communication channel for WS plugin
    ...
    ${expr}    Set Variable    "output"."onem2m-communication-channel-protocols"[?"communication-channel-protocol" == `websocket`] | [0]."onem2m-communication-channel-addresses"[?"communication-channel-address" == `0.0.0.0`] | [0]."onem2m-communication-channel-ports"[?"onem2m-communication-channel-port" == `8888`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/communication_channels/ws/default    verify=True    jmespath_expr=${expr}
...
5.00.05 Default Communication channel for Onem2mExample plugin
    ...
    ${expr}    Set Variable    "output"."onem2m-communication-channel-protocols"[?"communication-channel-protocol" == `http`] | [0]."onem2m-communication-channel-addresses"[?"communication-channel-address" == `0.0.0.0`] | [0]."onem2m-communication-channel-ports"[?"onem2m-communication-channel-port" == `8283`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/communication_channels/onem2m_example/default    verify=True    jmespath_expr=${expr}
...
Verify Default PluginData Output
    ...
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `http(s)-base`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/http/default    verify=True    jmespath_expr=${expr}
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `coap(s)-base`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/coap/default    verify=True    jmespath_expr=${expr}
    TemplatedRequests.Post_As_Json_Templated_Check_Not_Included    folder=${VAR_BASE}/plugin_data/mqtt/default
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `ws`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/ws/default    verify=True    jmespath_expr=${expr}
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `Onem2mExample`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/onem2m_example/default    verify=True    jmespath_expr=${expr}
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `Onem2mTsdrProvider`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/tsdr/default    verify=True    jmespath_expr=${expr}
    ${expr} =    Set Variable    "output"."onem2m-plugin-manager-plugins-table"[?"plugin-name" == `Onem2mSimpleAdapterProvider`] | [0]
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/plugin_data/simple_adapter/default    verify=True    jmespath_expr=${expr}
...
Verify Negative Configuration Scenario
    ...
    Should Be Equal As Integers    ${response.status_code}    ${response.status_code}

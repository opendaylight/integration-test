...
1.01 Bundle loader loads new feature
    ...
    #${nieco_divne} =    NexusKeywords.Deploy_Test_Tool    itplugin1    onem2m-core    ${EMPTY}
...
1.02 Bundle loader fails to load same feature using new feature name
    ...
    Set Local Variables    NewFeature2    ${filename_api}    ${filename_impl}
...
1.03 Bundle loader update feature should fail
    ...
    ${body} =    OperatingSystem.Get File    ${VAR_BASE}/pluginLoader/jar/load_plugin/post_data.json
    Set Local Variables    NewFeature1    ${filename_api2}    ${filename_impl2}
    ${body} =    Replace Variables    ${body}
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    restconf/operations/iotdmbundleloader:feature-put/    data=${body}    headers=${headers}
    ${status_code} =    Status Code    ${response}
    Should Be Equal As Integers    ${status_code}    500
...
4.00 Bundle loader tests multiple cases of missing configuration loading new feature
    ...
    Set Local Variables    filename_api=${filename_api}    filename_impl=${filename_impl}
...
4.01 Bundle loader tests multiple cases of invalid values loading new feature
    ...
    ${status_code} =    Status Code    ${response}
    Should Be Equal As Integers    ${status_code}    500
    Should Contain    ${response.content}    does not exist
...
4.08 Bundle loader tests loading a feature with missing function
    ...
    Set Local Variables    NewFeature1    ${filename_impl_invalid}    ${filename_api_invalid}
    ${body} =    OperatingSystem.Get File    ${VAR_BASE}/pluginLoader/jar/load_plugin/post_data.json
    ${body} =    Replace Variables    ${body}
    ${response} =    RequestsLibrary.Post Request    ClusterManagement__session_1    restconf/operations/iotdmbundleloader:feature-put/    data=${body}    headers=${headers}
    ${status_code} =    Status Code    ${response}
    Should Be Equal As Integers    ${status_code}    500
    Should Contain All Sub Strings    ${response.content}    ErrorMessage: BundleLoader: BundleLoaderInstanceDefault, Feature: NewFeature1, Failed to start bundle:
    ...    RuntimeException: Exception throws for purposes of testing of negative scenarios
...
5.00 Bundle loader loads feature, and verify weather it is registered using registerPluginHttp
    ...
    TODO
    ${filename_api_plugin} =    Set Variable    itiotdmplugin-api-0.1.0-SNAPSHOT.jar
    ${filename_impl_plugin} =    Set Variable    itiotdmplugin-impl-0.1.0-SNAPSHOT.jar
    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/jar/load_plugin    {'FEATURE': 'NewFeature3', 'PWD': '${PWD}', 'FILENAMEIMPL': '${filename_impl_plugin}', 'FILENAMEAPI': '${filename_api_plugin}'}    ClusterManagement__session_1
    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/jar/clean    session=ClusterManagement__session_1
...
#    Set Test Variable    ${KAR}    itplugin1-features-0.1.0-SNAPSHOT.kar
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/install    {'PWD': '${pwd}', 'FEATURE': 'odl-itplugin1', 'KAR':'${KAR}'}
#    ...    ClusterManagement__session_1
#    Get Plugin Id And Create Response    ${VAR_BASE}/pluginLoader/kar/list/one_feature    Itplugin1
#    ${response} =    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/list/one_feature    {'PWD': '${pwd}', 'FEATURE': 'odl-itplugin1', 'KAR':'itplugin1-features-0.1.0-SNAPSHOT.kar'}    ClusterManagement__session_1    True
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/list_startup/one_feature    {'PWD': '${pwd}', 'FEATURE': 'odl-itplugin1', 'KAR':'itplugin1-features-0.1.0-SNAPSHOT.kar'}    ClusterManagement__session_1    True
#    #Check Karaf Repo And Features    ${response}
#
...
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/install    {'PWD': '${pwd}', 'FEATURE': 'odl-itplugin2', 'KAR':'itplugin2-features-0.1.0-SNAPSHOT.kar'}
#    ...    ClusterManagement__session_1
#    Get Plugin Id And Create Response    ${VAR_BASE}/pluginLoader/kar/list/one_feature
#    ${response} =    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/list/one_feature    {'WORKSPACE': '${WORKSPACE}'}    ClusterManagement__session_1    True
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/list_startup/one_feature    {'WORKSPACE': '${WORKSPACE}'}    ClusterManagement__session_1    True
#    #Check Karaf Repo And Features    ${response}
#
...
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/reload    {'PWD': '${pwd}', 'FEATURE': 'itplugin1-features-0.1.0-SNAPSHOT'}
#    ...    ClusterManagement__session_1
#    TemplatedRequests.Post_As_Json_Templated    ${VAR_BASE}/pluginLoader/kar/reload    {'PWD': '${pwd}', 'FEATURE': 'itplugin2-features-0.1.0-SNAPSHOT'}
#    ...    ClusterManagement__session_1
#
...
Start
    ...
    Open_Connection_To_ODL_System    ${ODL_SYSTEM_IP}
    ${PWD} =    SSHLibrary.Execute_Command    pwd
    Set Suite Variable    ${pwd}
    ${filename_impl} =    Set Variable    itplugin1-impl-0.1.0-SNAPSHOT.jar
    ${filename_api} =    Set Variable    itplugin1-api-0.1.0-SNAPSHOT.jar
    ${filename_impl2} =    Set Variable    itplugin2-impl-0.1.0-SNAPSHOT.jar
    ${filename_api2} =    Set Variable    itplugin2-api-0.1.0-SNAPSHOT.jar
    ${filename_impl_invalid} =    Set Variable    itplugininvalid1-impl-0.1.0-SNAPSHOT.jar
    ${filename_api_invalid} =    Set Variable    itplugininvalid1-api-0.1.0-SNAPSHOT.jar
    Set Suite Variable    ${filename_impl}
    Set Suite Variable    ${filename_api}
    Set Suite Variable    ${filename_impl2}
    Set Suite Variable    ${filename_api2}
    Set Suite Variable    ${filename_impl_invalid}
    Set Suite Variable    ${filename_api_invalid}
...
Check karaf Itplugin1
    [Arguments]    ${response}
    ${repo} =    KarafKeywords.Execute Controller Karaf Command On Background    repo-list |grep "odl-itplugin1-0.1.0-SNAPSHOT"
    ${feature_list} =    KarafKeywords.Execute Controller Karaf Command On Background    feature:list |grep "odl-itplugin1-0.1.0-SNAPSHOT"
    Should Not Be Empty    ${repo}
    Should Not Be Empty    ${feature_list}

Check Karaf Repo And Features
    [Arguments]    ${response}
    [Documentation]    Checks if karaf features and repository contain new feature
    Run keyword if    '${response}'=='Itplugin1'    Check karaf Itplugin1    ${response}

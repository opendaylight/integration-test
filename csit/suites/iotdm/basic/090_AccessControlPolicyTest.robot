...
1.0 Test whether default ACP exist
    #    Valid Input for AE without name, Can I remove this?
    [Documentation]    The defalut ACP is stored under each CSE's  cseid/_defaultACP
    # ${attr} =    Set Variable    "api":"jb","apn":"jb2","or":"http://hey/you","rr":true
    # ${r} =    Create Resource    ${iserver}    InCSE1    ${rt_ae}    ${attr}
    # Response Is Correct    ${r}
    ${iserver1} =    Modify Headers Origin    ${iserver}    //iotsandbox.cisco.com:10000
    ${r} =    Retrieve Resource    ${iserver1}    InCSE1/_defaultACP
    ${text} =    Text    ${r}
    LOG    ${text}
...
1.1 Create ACP without context, test whether all the reponse mandatory attribtues are exist.
    ...
    ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
    ${r}=    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp1
    ${status_code} =    Status Code    ${r}
...
1.2 Create ACP with valid acip(ipv4)
    [Documentation]    After Created, test whether all the mandatory attribtues are exist.
    ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35,"acco":[{"acip":{"ipv4":["127.0.0.1"]}}]},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
    ${r}=    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp2
    ${status_code} =    Status Code    ${r}
    Should Be Equal As Integers    ${status_code}    201
    ${text} =    Text    ${r}
    Should Contain    ${text}    "ct":    "lt":    "ty"
...
1.3 Create ACP with invalid acip(ipv4)
    ...
    ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35,"acco":[{"acip":{"ipv4":["127.0.01"]}}]},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
    ${error}=    Run Keyword And Expect Error    *    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp3
...
1.4 Create ACP with valid acip(ipv6)
    [Documentation]    After Created, test whether all the mandatory attribtues are exist.
    ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35,"acco":[{"acip":{"ipv6":["2001:db8:0:0:0:ff00:42:8329"]}}]},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
    ${r}=    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp4
    ${status_code} =    Status Code    ${r}
    Should Be Equal As Integers    ${status_code}    201
    ${text} =    Text    ${r}
    Should Contain    ${text}    "ct":    "lt":    "ty"
...
1.5 Create ACP with invalid acip(ipv6)
    ...
    ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35,"acco":[{"acip":{"ipv6":["2001:db8:0:0:0:ff00:42"]}}]},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
    ${error}=    Run Keyword And Expect Error    *    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp3
    Should Start with    ${error}    Cannot create this resource [400]
    Should Contain    ${error}    not a valid Ipv6 address

# 2.1 Create Invalid ACP with Invalid orinin
#     ${iserver1} =    Modify Headers Origin    ${iserver}    //localhost:100001
#     ${attr} =    Set Variable    "pv":{"acr":[{"acor" : ["111","222"],"acop":35,"acco":[{"acip":{"ipv6":["2001:db8:0:0:0:ff00:42:8329"]}}]},{"acor" : ["111","222"],"acop":35}]},"pvs":{"acr":[{"acor" : ["111","222"],"acop":7},{"acor" : ["111","222"],"acop":9}]}
#     ${error}=    Run Keyword And Expect Error    *    Create Resource    ${iserver}    InCSE1    ${rt_acp}    ${attr}    Acp3
#     Should Start with    ${error}    Cannot create this resource [400]

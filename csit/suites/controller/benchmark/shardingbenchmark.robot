...
Benchmark_Sharding
    ...
    ${cmd}=    BuiltIn.Set_Variable    python shardbenchmark.py --hosts ${ip_list} --port ${RESTCONFPORT} --warmups ${WARMUPS} --runs ${RUNS} --test-type ${TEST_TYPE} --datastore ${DATA_STORE} --shard-type ${SHARD_TYPE} --dataitems ${DATAITEMS} --shards ${SHARDS} --operations ${OPERATIONS} --putspertx ${PUTSPERTX} --listeners ${LISTENERS} --csv ${CSV_OUT_FILE} --fulldata ${DATA_LOG_FILE} &> ${STDOUT_LOG}

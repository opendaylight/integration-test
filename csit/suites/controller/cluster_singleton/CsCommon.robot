...
Monitor_Owner_And_Candidates_Stability
    ...
    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    ${monitoring_duration}    Verify_Owner_And_Candidates_Stable    ${owner_index}

Register_Singleton_Constant_On_Nodes
    [Arguments]    ${index_list}    ${constant_prefix}
    [Documentation]    Register a candidate application on given nodes.
    : FOR    ${index}    IN    @{index_list}
    \    Register_Singleton_And_Update_Expected_Candidates    ${index}    ${constant_prefix}${index}
...
Get_And_Save_Present_CsOwner_And_CsCandidates
    ...
    ${cs_owner}    ${cs_candidates}    ClusterManagement.Get_Owner_And_Candidates_For_Device    ${DEVICE_NAME}    ${DEVICE_TYPE}    ${node_to_ask}
...
Rejoin_Isolated_Node_And_Verify

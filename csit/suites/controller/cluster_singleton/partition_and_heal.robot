...
Verify_Master_And_Candidates_Stable
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List
    : FOR    ${index}    IN    @{index_list}
    \    BuiltIn.Run_Keyword_If    "${index}" == "${isolated_node}"     Log To Console      verify on isolated node
    \    BuiltIn.Run_Keyword_Unless    "${index}" == "${isolated_node}"    Log To Console     verify on other nodes

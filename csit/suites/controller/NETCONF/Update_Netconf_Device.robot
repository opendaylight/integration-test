...
Mount Netconf Device
    ...
    Log    ${body}
    ${resp}    RequestsLibrary.Post Request    session    ${REST_CONT_CONF}/${REST_Sim-Dev_Mount}    data=${body}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    204
    ${resp}    RequestsLibrary.Get Request    session    ${REST_CONT_OPER}/node/${tt-sim-dev}

Netconf Device Base CFG
    Create Session    session    http://${CONTROLLER}:${RESTCONFPORT}
    ${resp}    RequestsLibrary.Get Request   session    ${REST_CONT_CONF}/${REST_Sim-DEV_MOUNT}
    Log    ${resp.content}
...
Update Netconf Device CFG
    Create Session    session    http://${CONTROLLER}:${RESTCONFPORT}
    ${resp}    RequestsLibrary.Get Request   session    ${REST_CONT_OPER}/${REST_SIM-DEV_MOUNT}
    Log    ${resp}
    ${resp}    RequestsLibrary.Put Request   session    ${REST_SIM-DEV_CONF}    data=${update_cfg_xml}
    Log    ${resp}
    Should Be Equal As Strings    ${resp.status_code}    200

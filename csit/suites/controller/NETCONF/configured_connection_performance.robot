...
...    netconf devices.
...    The suite should work as follows:
...    Generate configruation for karaf (karaf is the application container that runs ODL. Google OSGi for more information on this). This configuration comes in the form of XML files placed in the "data" directory of any ODL karaf distribution.
...   Let ODL connect automatically to these generated devices. The netconf-testtool keeps on running in the background on the TOOLS system while karaf does it's thing to connect to these devices.
...   Measure the time it took for each device to connect and also the sum of all connections.
...
Setup_Everything
    ...
    RequestsLibrary.Create_Session    config    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Check_Netconf_Up_And_Running
    ...
    ${response}=    RequestsLibrary.Get_Request    config    restconf/config/network-topology:network-topology/topology/topology-netconf/node/controller-config/yang-ext:mount/config:modules/module/odl-sal-netconf-connector-cfg:sal-netconf-connector/controller-config
    BuiltIn.Log    ${response.text}
    ${status}=    BuiltIn.Run_Keyword_And_Return_Status    BuiltIn.Should_Not_Contain    ${response.text}    data model content does not exist
    ${status_mp}=    BuiltIn.Run_Keyword_And_Return_Status    BuiltIn.Should_Not_Contain    ${response.text}    Mount point does not exist
    BuiltIn.Run_Keyword_Unless    ${status}    BuiltIn.Fail
    BuiltIn.Run_Keyword_Unless    ${status_mp}    BuiltIn.Fail

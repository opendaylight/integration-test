...
Add_Data
    ...
    # SSHLibrary.Switch_Connection    ${odl_system_ssh_index}
    ${command} =    BuiltIn.Set_Variable    patch_cars_sr2.py --segment-size=${SEGMENT_SIZE} --iterations=${ITERATIONS} --move-per-iter=${MOVE_PER_ITER}
    ${stdout}    ${rc} =    SSHLibrary.Execute_Command    ${command}    return_rc=True
...
Verify_Data_Is_Restored
    ...
    TemplatedRequests.Get_As_Json_Templated    folder=${CAR_VAR_DIR}    verify=True    iterations=${SEGMENT_SIZE}    iter_start=${first_id}

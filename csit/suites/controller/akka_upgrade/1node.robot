...
Install_Older_Odl
    ...
    # Download.
    SSHKeywords.Execute_Command_Should_Pass    mkdir -p "${ALTERNATIVE_BUNDLEFOLDER_PARENT}" && cd "${ALTERNATIVE_BUNDLEFOLDER_PARENT}" && wget -N "${PREVIOUS_ODL_RELEASE_ZIP_URL}"    stderr_must_be_empty=False
    # Unzip and detect bundle folder name.
    ${alternative_bundlefolder} =    SSHKeywords.Execute_Command_Should_Pass    cd "${ALTERNATIVE_BUNDLEFOLDER_PARENT}" && unzip "${PREVIOUS_ODL_RELEASE_ZIP_URL}" && rm "${PREVIOUS_ODL_RELEASE_ZIP_URL} && ls -1"

...
Install_Older_Odl
    ...
    # Download.
    SSHLibrary.Execute_Command    cd "${ALTERNATIVE_BUNDLEFOLDER_PARENT}" && wget -N "${PREVIOUS_ODL_RELEASE_ZIP_URL}"
    # Unzip and detect bundle folder name.
    ${alternative_bundlefolder} =    SSHLibrary.Execute_Command    cd "${ALTERNATIVE_BUNDLEFOLDER_PARENT}" && unzip "${PREVIOUS_RELEASE_ZIP_URL}" && rm "${PREVIOUS_RELEASE_ZIP_URL} && ls -1"
    ...
    ${vanilla_line} =    SSHLibrary.Execute_Command    grep 'featuresBoot' "${cfg_vanilla}" | grep -v 'featuresBootAsynchronous'
    ${orig_line} =    SSHLibrary.Execute_Command    grep 'featuresBoot' "${cfg_orig}" | grep -v 'featuresBootAsynchronous'
    # Replace the newly installed line.
    SSHLibrary.Execute_Command    sed -i 's/${vanilla_line}/${orig_line}/g' "${cfg_vanilla}"
    # TODO: Add check to verify edit was successful.
...
Transfer_Persisted_Data
    ...
    SSHLibrary.Switch_Connection    ${odl_system_ssh_index}
    SSHLibrary.Execute_Command    mv "${alternative_bundlefolder}/snapshots" "${WORKSPACE}/${BUNDLEFOLDER}/" && mv "${alternative_bundlefolder}/journal" "${WORKSPACE}/${BUNDLEFOLDER}/"
...
Setup_Suite
    ...
    BuiltIn.Set_Suite_Variable    \${odl_system_ssh_index}    ${conenction.index}

...
Start_All_And_Sync
    ...
    \    ${rc} =    ClusterManagement.Check_Bash_Command_On_Member    command=sudo netstat -pnatu | grep 2550 | grep LISTEN    member_index=${index}
    \    BuiltIn.Run Keyword If    ${rc}>0    ClusterManagement.Check_Bash_Command_On_Member    command=pid=$(grep org.apache.karaf.main.Main | grep -v grep | tr -s ' ' | cut -f2 -d' '); sudo /usr/lib/jvm/java-1.8.0/bin/jstack -l ${pid}    member_index=${index}

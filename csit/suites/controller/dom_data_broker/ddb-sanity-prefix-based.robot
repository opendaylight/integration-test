...
Resource          ${CURDIR}/../../../libraries/DdbCommons.robot
Resource          ${CURDIR}/../../../libraries/SetupUtils.robot

*** Variables ***
${PREF_BASED_SHARD}     default
${SHARD_TYPE}     config
${TRANSACTION_RATE_1K}    ${1000}
${DURATION_10S}    ${10}
${SIMPLE_TX}      ${False}
${CHAINED_TX}     ${True}
${MODULE_SHARD_PREFIX}    ${EMPTY}
...
Remove_Leader_Prefix_Shard_Replica_And_Add_It_Back
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    15s    3s    Verify_Shard_Replica_Removed    ${old_leader}    ${shard_name}    ${shard_type}
    ${actual_leader}    ${actual_follower_list} =    BuiltIn.Wait_Until_Keyword_Succeeds    15s    3s    ClusterManagement.Get_Leader_And_Followers_For_Shard    shard_name=${shard_name}
    ...    shard_type=${shard_type}    member_index_list=${follower_list}
    ClusterAdmin.Add_Shard_Replica    ${old_leader}    ${shard_name}    ${shard_type}
    BuiltIn.Wait_Until_Keyword_Succeeds    15s    3s    ClusterManagement.Get_Leader_And_Followers_For_Shard    shard_name=${shard_name}    shard_type=${shard_type}    member_index_list=${all_indices}
...
Remove_Follower_Prefix_Shard_Replica_And_Add_It_Back
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    15s    3s    Verify_Shard_Replica_Removed    ${follower1}    ${shard_name}    ${shard_type}
...
Become_Prefix_Leader
    ...
    ${shard_name} =    BuiltIn.Set_Variable    ${PREF_BASED_SHARD}
    ${shard_type} =    BuiltIn.Set_Variable    ${SHARD_TYPE}
    ${old_leader}    ${follower_list} =    ClusterManagement.Get_Leader_And_Followers_For_Shard    shard_name=${PREF_BASED_SHARD}    shard_type=${SHARD_TYPE}
    ${follower1} =    Collections.Get_From_List    ${follower_list}    ${0}
    ClusterAdmin.Make_Leader_Local    ${follower1}    ${shard_name}    ${shard_type}
    ${leader}    ${follower_list} =    BuiltIn.Wait_Until_Keyword_Succeeds    30s    3s    Verify_Shard_Leader_Elected    ${shard_name}
...
Produce_Transactions
    ...
    MdsalLowlevelPy.Start_Produce_Transactions_On_Nodes    ${all_ip_list}    ${MODULE_SHARD_PREFIX}    ${DURATION_10S}    ${TRANSACTION_RATE_1K}    chained_flag=${SIMPLE_TX}

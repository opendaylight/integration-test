...
Cycle_Leaders
    [Documentation]    Remove and add shard replica until member-1 is the leader again.
...
Cycle_Leader_Replica_Land_Member_1
    [Documentation]    Remove shard replica from current leader and add it back. Fail if the new leader is not member-1.
    ...
    ${new_leader}    ${new_followers} =    BuiltIn.Wait_Until_Keyword_Succeeds    60s    5s    ClusterManagement.Get_Leader_And_Followers_For_Shard    shard_name=${shard_name}!!    shard_type=${shard_type}    member_index_list=${all_indices}

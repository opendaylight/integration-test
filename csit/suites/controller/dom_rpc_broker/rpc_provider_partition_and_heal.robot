...
Invoke_Rpc_On_Each_Node
    ...
    \    ${constant} = BuiltIn.Set_Variable_If    ${index} in ${INSTALLED_RPC_MEMEBER_IDX}    Verify_Local_Rpc_Invoked    ${index}
    \    ...    ELSE    Verify_Any_Remote_Rpc_Invoked    ${index}
    BuiltIn.Set_Suite_Variable    ${non_rpc_node_idx}    ${index}
...
Invoke_Rpc_On_Isolated_Node
    ...
    ...    is expected.
    Verify_Local_Rpc_Invoked    ${isolated_idx}
...
Isolate_Member_Without_Registered_Rpc
    ...
    ClusterManagement.Isolate_Member_From_List_Or_All    ${non_rpc_node_idx}
...
Verify_Rpc_Fails_On_Isolated_Member_Without_Rpc
    ...
    WaitForFailure.Confirm_Keyword_Fails_Within_Timeout    15s    2s    MdsalLowlevel.Get_Constant    ${non_rpc_node_idx}

Rejoin_Isolated_Member_Without_Registered_Rpc
    [Documentation]    Rejoin isolated node.
    ClusterManagement.Rejoin_Member_From_List_Or_All    ${non_rpc_node_idx}

Verify_Rpc_Again_Passes_On_Member_Without_Rpc
    [Documentation]    Verify rpc works after the node rejoin.
    ${constant} =    BuiltIn.Wait_Until_Keyword_Succeeds    7x    2s    MdsalLowlevel.Get_Constant    ${non_rpc_node_idx}
...
Verify_Local_Rpc_Invoked
    [Arguments]    ${node_idx}
    ${constant} =    MdsalLowlevel.Get_Constant    ${node_idx}
    BuiltIn.Should_Be_Equal_As_Strings    ${CONSTANT_PREFIX}${node_idx}    ${constant}

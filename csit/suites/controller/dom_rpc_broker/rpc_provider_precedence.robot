...
Resource    ${CURDIR}/../../../libraries/TemplatedRequests
...
Register_Rpc_On_Each_Node
    ...
    \    TemplatedRequests.Register_Constant    ${index}    ${NODE${index}_CONSTANT}

Invoke_Rpc_On_Each_Node
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List    
    : FOR    ${index}    IN    @{index_list}
    \    ${constant} =    TemplatedRequests.Get_Constant    ${index}
    \    BuiltIn.Should_Be_Equal    "${NODE${index}_CONSTANT}"    ${constant}

Unregister_Rpc_On_Node1
    TemplatedRequests.Unregister_Constant    1

Invoke_Rpc_On_Node1
    ${allowed_values} =    BuiltIn.Create_List    ${NODE2_CONSTANT}    ${NODE3_CONSTANT}
    ${constant} =    TemplatedRequests.Get_Constant   1
...
Reregister_Rpc_On_Node1
    TemplatedRequests.Register_Constant    1    ${NODE1_CONSTANT}

...
Invoke_Rpc_Node_With_Unregistered_Rpc
    ...
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List
    : FOR    ${index}    IN    @{index_list}
    \    Continue For Loop If    "${index}" == "${UNREGISTERED_RPC_NODE}"
...
Invoke_Rpc_On_Remaining_Nodes
    ...
    \    Continue For Loop If    "${index}" == "${UNREGISTERED_RPC_NODE}"
    \    ${constant} =    MdsalLowlevel.Get_Constant    ${index}
    \    BuiltIn.Should_Be_Equal_As_Strings    ${NODE${index}_CONSTANT}    ${constant}

Reregister_Rpc_On_Node1
    MdsalLowlevel.Register_Constant    ${UNREGISTERED_RPC_NODE}    ${CONSTANT_PREXIX}${UNREGISTERED_RPC_NODE}
...
Deregister_Rpc_On_Each_Node
    ${index_list} =    ClusterManagement__Given_Or_Internal_Index_List
    : FOR    ${index}    IN    @{index_list}
    \    MdsalLowlevel.Deregister_Constant    ${index}

...
Invoke_Rpc_On_Each_Node
    ...
    DrbCommons.Verify_Context_Constant_On_Active_Nodes

Isolate_One_Node
    [Documentation]    Isolate one node with registered rpc.
    ${isolated_idx} =    DrbCommons.Get_Context_Constant_Index_From_Node    ${TESTED_MEMBER_WITHOUT_RPC_IDX}
...
Invoke_Rpc_On_Each_Node_Again
    ...
    ...    local instance the local value is expected. If invoked on the node with no local instance, any remote
    ...    value is expected.
    BuiltIn.Wait_Until_Keyword_Succeeds    11s    1s    DrbCommons.Verify_Contexted_Constant_On_Active_Nodes

...
*** Test Cases ***
Wait_For_Rpcs
    [Documentation]    Issue (invalid) RPC requests until 401 goes away (or timeout expires).
    ...    https://lists.opendaylight.org/pipermail/integration-dev/2016-February/005824.html
    BuiltIn.Wait_Until_Keyword_Succeeds    120s    1s    Check_Rpc_Readiness
...
Add_And_Verify_Zero_Cars
    [Documentation]    This attempts to create container with empty list of cars.
    ...    This is to test this corner case data generation and see impact in datastore.
    TemplatedRequests.Post_As_Json_Templated    folder=${VAR_BASE}/cars    iterations=0
    TemplatedRequests.Get_As_Json_Templated    folder=${VAR_BASE}/cars    verify=True    iterations=0
...
Check_Rpc_Readiness
    ...
    ${status}    ${message} =    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Post_As_Xml_To_Uri    uri=restconf/operations/car-purchase:buy-car    data=${EMPTY}

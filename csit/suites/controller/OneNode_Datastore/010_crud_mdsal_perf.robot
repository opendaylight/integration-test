...
Delete People
    ...
    ${rsp}=    RequestsLibrary.DELETE On Session    session    ${peopleurl}
    Should Be Equal As Numbers    200    ${rsp.status_code}
    ${rsp}=    RequestsLibrary.GET On Session    session    ${peopleurl}
    Should Contain    ${DELETED_STATUS_CODES}    ${rsp.status_code}

Delete CarPeople
    [Documentation]    Remove car-people entries from the datastore
    ${rsp}=    RequestsLibrary.DELETE On Session    session    ${carpeopleurl}
    Should Be Equal As Numbers    200    ${rsp.status_code}
    ${rsp}=    RequestsLibrary.GET On Session    session    ${carpeopleurl}
    Should Contain    ${DELETED_STATUS_CODES}    ${rsp.status_code}

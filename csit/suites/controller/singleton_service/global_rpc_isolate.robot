...
#    BuiltIn.Wait_Until_Keyword_Succeeds    70s    10s    ShardStability.Shards_Stability_Get_Details    ${DEFAULT_SHARD_LIST}    member_index_list=${old_brt_successors}
    BuiltIn.Wait_Until_Keyword_Succeeds    120s    5s    Verify_Owner_Elected    ${True}    ${old_brt_owner}    ${idx}
...
Get_Present_Brt_Owner_And_Successors
    ...
    ${current_owner}    ${current_successors}=    ClusterManagement.Get_Owner_And_Successors_For_Device    ${SERVICE}    org.opendaylight.mdsal.ServiceEntityType    ${node_to_ask}
    BuiltIn.Run_Keyword_If    ${store}    BuiltIn.Set_Suite_Variable    ${brt_owner}    ${current_owner}
    BuiltIn.Run_Keyword_If    ${store}    BuiltIn.Set_Suite_Variable    ${brt_successors}    ${current_successors}
    BuiltIn.Return_From_Keyword    ${current_owner}    ${current_successors}

...
Verify_New_Car_Leader_Elected
    ...
    ${idx}=    Collections.Get_From_List    ${old_car_followers}    0
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_Leader_Elected    ${True}    ${old_car_leader}    ${idx}
    Get_Car_Shard_Leader_And_Followers    ${idx}    store=${True}
...
Verify_Owner_Elected
    [Arguments]    ${new_elected}    ${old_leader}    ${node_to_ask}
    [Documentation]    Verify new leaderr was elected or remained the same.
    ${leader}     ${followers}=    Get_Car_Shard_Leader_And_Followers    ${node_to_ask}

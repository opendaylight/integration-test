...
Check Contents Of Leader Shards
    ...
    wait until keyword succeeds    3    1    Check Cars    ${CURRENT_CAR_LEADER}    ${PORT}    100
    wait until keyword succeeds    3    1    Check People    ${CURRENT_CAR_LEADER}    ${PORT}    100
    wait until keyword succeeds    3    1    Check CarPeople    ${CURRENT_CAR_LEADER}    ${PORT}    100

Check Contents Of First Follower Shards
    [Documentation]    Check all content using first follower
    wait until keyword succeeds    3    1    Check Cars    @{CAR_FOLLOWERS}[0]    ${PORT}    100
    wait until keyword succeeds    3    1    Check People    @{CAR_FOLLOWERS}[0]    ${PORT}    100
    wait until keyword succeeds    3    1    Check CarPeople    @{CAR_FOLLOWERS}[0]    ${PORT}    100

Check Contents Of Second Follower Shards
    [Documentation]    Check all content using second follower
    wait until keyword succeeds    3    1    Check Cars    @{CAR_FOLLOWERS}[1]    ${PORT}    100
    wait until keyword succeeds    3    1    Check People    @{CAR_FOLLOWERS}[1]    ${PORT}    100
    wait until keyword succeeds    3    1    Check CarPeople    @{CAR_FOLLOWERS}[1]    ${PORT}    100
...
Check Contents Of New Leader Shards
    [Documentation]    Check all content using Leader
    Log    ${NEW_CAR_LEADER}
    wait until keyword succeeds    3    1    Check Cars    ${NEW_CAR_LEADER}    ${PORT}    100
    wait until keyword succeeds    3    1    Check People    ${NEW_CAR_LEADER}    ${PORT}    100
    wait until keyword succeeds    3    1    Check CarPeople    ${NEW_CAR_LEADER}    ${PORT}    100

...
Purchase 100 More Cars Using New Follower1
    [Documentation]    RePurchase 100 cars using Follower1
    Buy Cars And Verify    @{CAR_FOLLOWERS}[0]    ${100}    ${100}

Check Contents Of New Leader Shards After Repurchase
    [Documentation]    Check all content using Leader
    Log    ${NEW_CAR_LEADER}
    Check Elements In Shards  ${NEW_CAR_LEADER}    200

Check Contents Of New First Follower Shards
    [Documentation]    Check all content using first follower
    Check Elements In Shards  @{CAR_FOLLOWERS}[0]    200

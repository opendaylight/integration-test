...
Add_Cars_On_Tipping_Follower
    ...
    TemplatedRequests.Put_As_Json_Templated    folder=${VAR_DIR}/cars    session=${car_tipping_follower_session}    iterations=${CAR_ITEMS}    iter_start=${MAJORITY_START_I}
...
Set_Variables_For_Shard
    ...
    ${tipping_follower_session} =    Collections.Get_From_List    ${sessions}    0
    BuiltIn.Set_Suite_Variable    \${${shard_name}_tipping_follower_session}    ${tipping_follower_session}
    ${last_follower_session} =    Collections.Get_From_List    ${sessions}    -1
    BuiltIn.Set_Suite_Variable    \${${shard_name}_last_follower_session}    ${last_follower_session}
    ...
    ${revive_list} =    Collections.Get_Slice_From_List    ${original_follower_indices}    ${half_followers}    ${number_followers}

...
Add_Cars_To_Leader_And_Verify
    ...
    \    BuiltIn.Wait_Until_Keyword_Succeeds    10s    2s    TemplatedRequests.Get_As_Json_Templated    folder=${VAR_DIR}/cars    session=${session}    verify=True    iterations=${car_items}
...
Add_People_To_First_Follower_And_Verify
    ...
    : FOR    ${session}    IN    @{ClusterManagement__session_list}
    \    BuiltIn.Wait_Until_Keyword_Succeeds    10s    2s    TemplatedRequests.Get_As_Json_Templated    folder=${VAR_DIR}/people    session=${session}    verify=True    iterations=${people_items}
...
Buy_Cars_On_Leader_And_Verify
    ...
    : FOR    ${session}    IN    @{ClusterManagement__session_list}
    \    BuiltIn.Wait_Until_Keyword_Succeeds    10s    2s    TemplatedRequests.Get_As_Json_Templated    folder=${VAR_DIR}/car-people    session=${session}    verify=True    iterations=${total_iterations}

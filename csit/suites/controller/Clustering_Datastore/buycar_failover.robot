...
Reboot_1
    ...
    ClusterManagement.Kill_Single_Member    3    confirm=True
    ClusterManagement.Start_Single_Member    3    wait_for_sync=True    timeout=${MEMBER_START_TIMEOUT}
...
Reboot_2
    [Documentation]    Previous car-people Leader is rebooted (without persistence cleanup).
    ${index_to_reboot} =    Collections.Remove_From_List    ${list_to_reboot}    0
    ClusterManagement.Kill_Single_Member    2    confirm=True
    ClusterManagement.Start_Single_Member    2    wait_for_sync=True    timeout=${MEMBER_START_TIMEOUT}
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    2s    ClusterManagement.Verify_Leader_Exists_For_Each_Shard    shard_name_list=${SHARD_NAME_LIST}    shard_type=config
...
Reboot_3
    [Documentation]    Previous car-people Leader is rebooted (without persistence cleanup).
    ${index_to_reboot} =    Collections.Remove_From_List    ${list_to_reboot}    0
    ClusterManagement.Kill_Single_Member    1    confirm=True
    ClusterManagement.Start_Single_Member    1    wait_for_sync=True    timeout=${MEMBER_START_TIMEOUT}
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    2s    ClusterManagement.Verify_Leader_Exists_For_Each_Shard    shard_name_list=${SHARD_NAME_LIST}    shard_type=config

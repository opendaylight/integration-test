...
Reboot_1
    ...
    Collections.Remove_Values_From_List    ${list_to_reboot}    ${people_leader_index}    ${people_first_follower_index}    
...
Reboot_3
    [Documentation]    Previous car-people Leader is rebooted (without persistence cleanup).
    ClusterManagement.Kill_Single_Member    ${people_first_follower_index}    confirm=True
    ClusterManagement.Start_Single_Member    ${people_first_follower_index}    wait_for_sync=True    timeout=${MEMBER_START_TIMEOUT}
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    2s    ClusterManagement.Verify_Leader_Exists_For_Each_Shard    shard_name_list=${SHARD_NAME_LIST}    shard_type=config

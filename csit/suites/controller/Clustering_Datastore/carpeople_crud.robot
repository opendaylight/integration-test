...
Setup
    ...
    Log    ${operational}
    ${config} =    TemplatedRequests.Get_As_Json_From_Uri    uri=/jolokia/read/org.opendaylight.controller:type=DistributedConfigDatastore,Category=ShardManager,name=shard-manager-config    session=ClusterManagement__session_1
    Log    ${config}

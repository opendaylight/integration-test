...
TC4_TC16_TC18
    [Documentation]    Testcase Ids 7.1.4 7.1.16 7.1.18
    [Tags]    Nightly
    ${exp_result}    ConvertToInteger    1
    ...
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET1[2]}
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET2[1]}    ${VM_IP_NET2[3]}
...
TC2
    ...
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET1[2]}
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET2[2]}
...
TC3
    ...
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET2[0]}
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ${VM_IP_NET2[1]}
...
TC7
    ...
    Associate L3VPN To Networks    ${Req_no_of_net}
    Log    "STEP 2 : PING VNF11 <-> VNF23 AND VNF12 <-> VNF24"
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET2[2]}
    Wait Until Keyword Succeeds    60s    10s    Verify Ping    @{NETWORKS}[0]    ${VM_IP_NET1[1]}    ${VM_IP_NET2[3]}
...
TC8_TC9
    [Documentation]    Testcase Ids 7.1.8 7.1.9
    [Tags]    Nightly
    ${exp_result}    ConvertToInteger    1
...
Create Setup
    ...
    #${output}=    Issue Command On Karaf Console    ${BGP_SHOW}
    #Log     ${output}
...
Delete Setup
    ...
    #Log    Delete BGP neighbor config
    #${output}=    Issue Command On Karaf Console    ${BGP_NEIGHBOR_DELETE_CMD}
    #${output}=    Issue Command On Karaf Console    ${BGP_SHOW}
    #Log     ${output}

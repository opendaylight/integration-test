...
SF218_RT5_TC1 Verification Of Intra_Network_Intra_Openvswitch network connectivity
...
SF218_RT5_TC4_SF218_RT5_TC16_SF218_RT5_TC18
...
Delete Setup
    ...
    ${Req_no_of_net} =    Evaluate    2
    Dissociate L3VPN From Networks    ${Req_no_of_net}
    Log    DELETE L3VPN
    ${Req_no_of_L3VPN} =    Evaluate    1
...
Dissociate L3VPN From Networks
    ...
    ${devstack_conn_id} =    Get ControlNode Connection
    ${NUM_OF_NET}    Convert To Integer    ${NUM_OF_NET}
    :FOR   ${index}   IN RANGE   0   ${NUM_OF_NET}
    \    ${network_id} =    Get Net Id    ${NETWORKS[${index}]}    ${devstack_conn_id}
...
Verify Ping
    ...
    ${output}=    Wait Until Keyword Succeeds    20s    1s    Execute Command on VM Instance    ${NETWORK}    ${VM_IP1}    ping -c 3 ${VM_IP2}

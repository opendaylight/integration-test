...
Unification Termination Point Inside
    ...
    ${request}    KeywordsAndVariables.Prepare Unification Inside Topology Request    ${UNIFICATION_NT_AGGREGATE_INSIDE}    network-topology-model    node    ovsdb:ofport    network-topo:1
...
Unification Termination Point Inside Inventory
    [Documentation]    Test aggregate inside operation on termination points
    ${request}    KeywordsAndVariables.Prepare Unification Inside Topology Request    ${UNIFICATION_NT_AGGREGATE_INSIDE}    opendaylight-inventory-model    node    flow-node-inventory:port-number    openflow-topo:1
    ${resp}    Basic Aggregation    ${request}    network-topology:network-topology/topology/topo:1
    Should Contain    ${resp.content}    <topology-id>topo:1</topology-id>
    ${response_xml}    Get Element    ${resp.content}    xpath=.//topology[topology-id='topo:1']
    ${response_xml}    Element to String    ${response_xml}

...
Unification Scripting Node
    ...
    ${request}    Insert Target Field    ${request}    0    l3-unicast-igp-topology:igp-node-attributes/isis-topology:isis-node-attributes/isis-topology:ted/isis-topology:te-router-id-ipv4    0
    ${request}    Insert Target Field    ${request}    1    l3-unicast-igp-topology:igp-node-attributes/isis-topology:isis-node-attributes/isis-topology:ted/isis-topology:te-router-id-ipv4    0
    ${request}    Insert Scripting into Request    ${request}    javascript    if (originalItem.getLeafNode().get(0).getValue().indexOf("192.168.1.1") > -1 && newItem.getLeafNode().get(0).getValue().indexOf("192.168.1.3") > -1 || originalItem.getLeafNode().get(0).getValue().indexOf("192.168.1.3") > -1 && newItem.getLeafNode().get(0).getValue().indexOf("192.168.1.1") > -1) {aggregable.setResult(true);} else { aggregable.setResult(false);}
    ${resp}    Send Basic Request    ${request}    network-topology:network-topology/topology/topo:1
    Should Contain    ${resp.content}    <topology-id>topo:1</topology-id>
    Should Contain X Times    ${resp.content}    <node-id>node:    9
    : FOR    ${index}    IN RANGE    1    10
...
Unification Scripting Node Inventory
    ...
    ${request}    Insert Target Field    ${request}    0    flow-node-inventory:ip-address    0
    ${request}    Insert Target Field    ${request}    1    flow-node-inventory:ip-address    0
    ${request}    Insert Scripting into Request    ${request}    javascript    if (originalItem.getLeafNode().get(0).getValue().indexOf("192.168.1.2") > -1 && newItem.getLeafNode().get(0).getValue().indexOf("192.168.1.4") > -1 || originalItem.getLeafNode().get(0).getValue().indexOf("192.168.1.4") > -1 && newItem.getLeafNode().get(0).getValue().indexOf("192.168.1.2") > -1) {aggregable.setResult(true);} else { aggregable.setResult(false);}
    ${resp}    Send Basic Request    ${request}    network-topology:network-topology/topology/topo:1
    Should Contain    ${resp.content}    <topology-id>topo:1</topology-id>
    Should Contain X Times    ${resp.content}    <node-id>node:    9
    : FOR    ${index}    IN RANGE    1    10

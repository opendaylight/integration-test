...
Link Computation Aggregation Inside
    ...
    ${request}    KeywordsAndVariables.Prepare Unification Inside Topology Request    ${UNIFICATION_NT_AGGREGATE_INSIDE}    network-topology-model    node    network-topology-pcep:path-computation-client/network-topology-pcep:ip-address    network-topo:6
    ${request}    KeywordsAndVariables.Insert Link Computation Inside    ${request}    ${LINK_COMPUTATION_INSIDE}    n:network-topology-model    network-topo:6
    ...
    Should Contain X Times    ${resp.content}    <node-ref>pcep:26</node-ref>    1
    Should Contain X Times    ${resp.content}    <node-ref>pcep:27</node-ref>    1
    Should Contain X Times    ${resp.content}    <node-ref>pcep:30</node-ref>    1
    ${node}    Get Element    ${resp.content}    xpath=.//node/supporting-node[node-ref='pcep:28']/..
    ${node_id}    Get Element Text    ${node}    xpath=./node-id
    ${node}    Element to String    ${node}
    Should Contain X Times    ${node}    <supporting-node>    2
    Should Contain    ${node}    <node-ref>pcep:28</node-ref>
    Should Contain    ${node}    <node-ref>pcep:29</node-ref>
    ...
    ${node}    Get Element    ${resp.content}    xpath=.//node/supporting-node[node-ref='pcep:30']/..
    ${node_id}    Get Element Text    ${node}    xpath=./node-id
    ${node}    Get Element    ${resp.content}    xpath=.//node/supporting-node[node-ref='pcep:29']/..
...
Link Computation Filtration
    ...
    ${request}    KeywordsAndVariables.Prepare Filtration Topology Request    ${FILTRATION_NT}    network-topology-model    node    network-topo:6
    ...
    Should Contain X Times    ${resp.content}    <node-ref>pcep:28</node-ref>    1
    Should Contain X Times    ${resp.content}    <node-ref>pcep:29</node-ref>    1
    Should Contain X Times    ${resp.content}    <node-ref>pcep:30</node-ref>    1
...
Link Computation Aggregation Filtration
    ...
    ${request}    KeywordsAndVariables.Prepare Unification Filtration Inside Topology Request    ${UNIFICATION_FILTRATION_NT}    network-topology-model    node    network-topology-pcep:path-computation-client/network-topology-pcep:ip-address    network-topo:6
    ${request}    KeywordsAndVariables.Prepare Unification Filtration Topology Request    ${request}    network-topology-model    network-topology-pcep:path-computation-client/network-topology-pcep:ip-address    network-topo:1
    ${request}    KeywordsAndVariables.Insert Filter    ${request}    ${FILTER_IPV4}    network-topology-pcep:path-computation-client/network-topology-pcep:ip-address
    ${request}    KeywordsAndVariables.Set IPV4 Filter    ${request}    192.168.2.1/24
    ${request}    KeywordsAndVariables.Insert Link Computation    ${request}    ${LINK_COMPUTATION}    n:network-topology-model    network-topo:6    network-topo:1
    ${resp}    Basic Filtration    ${request}    network-topology:network-topology/topology/topo:1
    Should Contain    ${resp.content}    <topology-id>topo:1</topology-id>

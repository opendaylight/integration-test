...
Filtration Range Number Termination Point Update Inventory
    ...
    ${nodeConnector}    Create Openflow Node Connector    openflow:3:2    4
    Basic Request Put    ${nodeConnector}    opendaylight-inventory:nodes/node/openflow:2/node-connector/openflow:3:2
    ${resp}    Basic Request Get And Test    ${nodeConnector}    network-topology:network-topology/topology/topo:1    <node-id>node:    5
    ...
    ${node}    Element to String    ${node}
    Should Contain X Times    ${node}    <termination-point>    1
    Should Contain X Times    ${node}    <tp-ref>/network-topology:network-topology/topology/openflow-topo:1/node/of-node:2/termination-point/tp:3:1</tp-ref>    1
    [Teardown]    Run Keywords    Report_Failure_Due_To_Bug    5955    AND    Test Teardown With Underlay Topologies Refresh    network-topology:network-topology/topology/topo:1
...
Link Computation Aggregation Inside Update NT
    ...
    Should Be Equal As Strings    ${source}    ${node_29}
    Should Be Equal As Strings    ${destination}    ${node_30}
    [Teardown]    Run Keywords    Report_Failure_Due_To_Bug    6004    AND    Test Teardown With Underlay Topologies Refresh    network-topology:network-topology/topology/topo:1
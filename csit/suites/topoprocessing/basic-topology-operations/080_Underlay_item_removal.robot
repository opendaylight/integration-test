...
*** Variables ***
${OVERLAY_TOPO_URL}    network-topology:network-topology/topology/topo:1
...
Unification Node Removal NT
    ...
    ${resp}    Wait Until Keyword Succeeds    40x    250ms    Basic Request Get And Test    ${OVERLAY_TOPO_URL}    <supporting-node>    9
    ...
    Wait Until Keyword Succeeds    40x    250ms    Basic Request Get And Test    ${OVERLAY_TOPO_URL}    <supporting-node>    8
...
Unification Node Removal Inventory
    ...
    Send Basic Request And Test If Contain X Times    ${request}    network-topology:network-topology/topology/topo:1    <node-id>node:    7
    #Remove an underlay aggregated node, preserving the overlay node

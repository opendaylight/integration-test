...
...
...               Copyright (c) 2016,2019 Cisco Systems, Inc. and others. All rights reserved.
...
...
@{excluded_dirs}       experimental         draft        xr/601
...
Prepare_Yang_Files_To_Test
    ...
    ${dirs_to_exclude} =     Create List
    :FOR      ${directory}    IN      @{excluded_dirs}
    \   @{unwanted_dirs} =    Collections.Get Matches     ${yang_dirs_to_process}     *${directory}*    case_insensitive=True
    \   ${dirs_to_exclude} =  Combine_Lists     ${dirs_to_exclude}      ${unwanted_dirs}
    Log       ${dirs_to_exclude}
    :FOR      ${directory}    IN      @{dirs_to_exclude}
    \   Collections.Remove Values From List         ${yang_dirs_to_process}       ${directory}
    Log       ${yang_dirs_to_process}
    ${p_option_value} =    BuiltIn.Catenate    SEPARATOR=:    @{dirs_to_process}
    ${yang_files_to_validate} =    Get_Yang_Files_From_Dirs    ${yang_dirs_to_process}
...
Deploy_And_Start_Odl_Yang_Validator_Utility
    ...
    \    ${yang_file_path}   ${yang_file_name} =   OperatingSystem.Split Path       ${yang_file}
    \    ${logfile} =    NexusKeywords.Install_And_Start_Java_Artifact    component=yangtools    artifact=${TEST_TOOL_NAME}    suffix=jar-with-dependencies    tool_options=-p ${p_option_value}:${yang_file_path} ${yang_file} -v

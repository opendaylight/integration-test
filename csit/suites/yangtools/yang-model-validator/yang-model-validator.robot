...
Deploy_And_Start_Odl_Yang_Validator_Utility
    ...
    FOR    ${yang_file}    IN    @{YANG_FILES_TO_VALIDATE}
        ...
        ${parser_path} =    Fetch From Left    ${relative_path}    /
        Log To Console    working through this: -p ${path} -r -d ${yang_file}
        ${logfile} =    NexusKeywords.Install_And_Start_Java_Artifact    component=yangtools    artifact=${TEST_TOOL_NAME}    suffix=jar-with-dependencies    tool_options=-p ${parser_path} -r -d ${yang_file}

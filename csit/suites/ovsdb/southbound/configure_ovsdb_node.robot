...
Get Operational Topology
    ...
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements At URI    ${OPERATIONAL_TOPO_API}/topology/ovsdb:1    ${NODE_LIST}

Create a Bridge
    [Documentation]    This will create bridge on the specified OVSDB node.
    OVSDB.Add Bridge To Ovsdb Node    ${ODL_SYSTEM_IP}    ${BRIDGE}    0000000000000004
...
Get Operational Topology with Bridge
    ...
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements At URI    ${OPERATIONAL_TOPO_API}/topology/ovsdb:1    ${list}
...
Create Port and Attach to a Bridge
    ...
    OVSDB.Add Termination Point    ${TOOLS_SYSTEM_IP}:${OVSDB_PORT}    ${BRIDGE}    ${PORT}    10.0.0.10
...
Get Operational Topology after Deletion of Port
    ...
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements Not At URI    ${OPERATIONAL_TOPO_API}/topology/ovsdb:1    ${list}
...
Get Operational Topology to make sure the connection has been deleted
    [Documentation]    This request will fetch the operational topology from the connected OVSDB nodes
    @{list} =    BuiltIn.Create List    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements Not At URI    ${OPERATIONAL_TOPO_API}/topology/ovsdb:1    ${list}
...
Get Configuration Topology to make sure the connection has been deleted
    ...
    @{list} =    BuiltIn.Create List    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements Not At URI    ${CONFIG_TOPO_API}/topology/ovsdb:1    ${NODE_LIST}
...
Create OVSDB NODE HOST1
    ...
    ${url} =    ${CONFIG_TOPO_API}/topology/ovsdb:1/
    BuiltIn.Log    URL is ${url}
    BuiltIn.Log    data: ${body}
    ${resp} =    RequestsLibrary.Post Request    session    ${url}    data=${body}
    BuiltIn.Log    ${resp.content}
...
Create QOS entry
    ...
    BuiltIn.Log    URL is ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:qos-entries/${QOS}/
    BuiltIn.Log    data: ${body}
...
Create Queue entry to the queues list of a ovsdb node
    ...
    BuiltIn.Log    URL is ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:queues/${QUEUE}/
    BuiltIn.Log    data: ${body}

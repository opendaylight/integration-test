...
Get bridge setup
    ...
    [Tags]    OVSDB netvirt
    ${output}   Wait Until Keyword Succeeds    20s    2s    Run Command On Remote System    ${TOOLS_SYSTEM_IP}    sudo ovs-vsctl show
    Log    ${output}
...
Get the bridge flows
    ...
    [Tags]    OVSDB netvirt
    ${output}   Wait Until Keyword Succeeds    20s    2s    Run Command On Remote System    ${TOOLS_SYSTEM_IP}    sudo ovs-ofctl -O Openflow13 dump-flows br-int
    Log    ${output}

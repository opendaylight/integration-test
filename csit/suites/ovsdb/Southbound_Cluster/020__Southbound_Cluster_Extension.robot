...
Check Entity Owner Status And Find Owner and Candidate After Fail
    [Documentation]    Check Entity Owner Status and identify owner and candidate.
    ${new_owner}    ${new_candidate_list}    BuiltIn.Run Keyword If    '${ODL_STREAM}' != 'beryllium'    ClusterOvsdb.Get Ovsdb Entity Owner Status For One Device    ovsdb://uuid/${ovsdb_uuid}    ${original_owner}
    ...    ELSE    ClusterOvsdb.Get Ovsdb Entity Owner Status For One Device    ovsdb://uuid/${ovsdb_uuid}    ${original_owner}    ${new_cluster_list}
    BuiltIn.Run Keyword If    '${ODL_STREAM}' != 'beryllium'    Collections.Remove Values From List    ${new_candidate_list}    ${original_candidate}

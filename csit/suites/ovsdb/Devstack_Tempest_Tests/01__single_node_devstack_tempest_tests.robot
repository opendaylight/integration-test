...
*** Variables ***
${ODL_VERSION}                        lithium-SR1
${OPENSTACK_BRANCH}                   stable/liberty
${NETWORKING-ODL_BRANCH}              ${OPENSTACK_BRANCH}
# ${NETWORKING-ODL_GITREPO}             https://github.com/flavio-fernandes/networking-odl
${NETWORKING-ODL_GITREPO}             https://git.openstack.org/openstack/networking-odl
...
Run Devstack Gate Wrapper
    ...
    ${output}=    Write Commands Until Prompt    ./devstack-gate/devstack-vm-gate-wrap.sh    timeout=2700s    #30min
    Log    ${output}
...
Devstack Suite Setup
    ...
    Write Commands Until Prompt    export DEVSTACK_LOCAL_CONFIG="enable_plugin networking-odl ${NETWORKING-ODL_GITREPO} ${NETWORKING-ODL_BRANCH};"
...
Clean DevStack Host In Case It Is Not Sterile
    ...
    # TODO: forcefully kill off any python or java processes, as none should be running

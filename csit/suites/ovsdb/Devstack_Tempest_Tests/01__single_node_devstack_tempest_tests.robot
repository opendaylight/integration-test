...
Testing karaf console with standard headers
    SSHLibrary.Open Connection    ${DEVSTACK_SYSTEM_IP}    port=8101    prompt=opendaylight-user
    Flexible SSH Login    karaf    karaf
    # the prompt is opendaylight-user@root> but let's try to avoid special chars for now
    Write    version
    Read Until    opendaylight-user
    Write    feature:list
    Read Until    opendaylight-user

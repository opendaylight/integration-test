...
Test neutron
    ...
    Log    ${output}
    ${output}=    Write Commands Until Prompt    neutron -v net-create net2_network
    Log    ${output}

Test subnet
    ${output}=    Write Commands Until Prompt    neutron -v subnet-create debuggingIsFun_network 10.0.0.0/24
    Log    ${output}
    ${output}=    Write Commands Until Prompt    neutron -v subnet-create net2_network 10.0.0.0/24
    Log    ${output}
...
Get Net Id
    ${output}=   Write Commands Until Prompt    neutron net-list | grep "debuggingIsFun_network"
    Log    ${output}
    @{output1}=   Write Commands Until Prompt    neutron net-list | grep "debuggingIsFun_network"
    Log    ${output1}
    ${net_id}=    Get From List    ${output1}    0
    Log    ${net_id}
...
Test instance using flavor and image names
    ${output}=   Write Commands Until Prompt     nova boot --image cirros-0.3.4-x86_64-uec --flavor m1.tiny --nic net-id=${net_id} MyFirstInstance
    Log    ${output}

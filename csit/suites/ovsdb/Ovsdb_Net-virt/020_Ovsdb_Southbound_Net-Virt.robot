...
Suite Setup       Create Session    session    http://${CONTROLLER1}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
Suite Setup       Create Session    session    http://${CONTROLLER2}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
Suite Setup       Create Session    session    http://${CONTROLLER3}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
...
Make the OVS instance to listen for connection
    ...
    Run Command On Remote System    ${MININET}    sudo ovs-vsctl set-manager tcp:${CONTROLLER}:${OVSDB_PORT} tcp:${CONTROLLER1}:${OVSDB_PORT} tcp:${CONTROLLER2}:${OVSDB_PORT}
...
Get bridge setup
    ...
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${MININET}    sudo ovs-vsctl show
    Should Contain    ${output}    Manager  "tcp:${CONTROLLER}:${OF_PORT} tcp:${CONTROLLER1}:${OF_PORT} tcp:${CONTROLLER2}:${OF_PORT}"

...
Get bridge setup at controller1
    ...
    Log    ${CONTROLLER1}
    Log    ${CONTROLLER2}
    ${output}   Run Comman d On Remote System    ${CONTROLLER}    sudo ovs-vsctl show
    Log    ${output}
...
Get bridge setup at controller2
    [Documentation]    This request is verifying that the br-int bridge has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER1}    sudo ovs-vsctl show
    Log    ${output}
    Should Contain    ${output}    Controller "tcp:${CONTROLLER1}:${OF_PORT}"
    Should Contain    ${output}    Bridge br-int

Get bridge setup at controller3
    [Documentation]    This request is verifying that the br-int bridge has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER2}    sudo ovs-vsctl show
    Log    ${output}
    Should Contain    ${output}    Controller "tcp:${CONTROLLER2}:${OF_PORT}"
    Should Contain    ${output}    Bridge br-int

Get port setup  at  controller1
    [Documentation]    This will check the port br-int has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER}    sudo ovs-vsctl show
    Log    ${output}
    Should Contain    ${output}    Port br-int

Get port setup  at  controller2
    [Documentation]    This will check the port br-int has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER1}    sudo ovs-vsctl show
    Log    ${output}
...
Get interface setup at controller1
    [Documentation]    This verify the interface br-int has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER}    sudo ovs-vsctl show
    Should Contain    ${output}    Interface br-int

Get interface setup at conroller2
    [Documentation]    This verify the interface br-int has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER1}    sudo ovs-vsctl show
    Should Contain    ${output}    Interface br-int

Get interface setup at controller3
    [Documentation]    This verify the interface br-int has been created
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER2}    sudo ovs-vsctl show
    Should Contain    ${output}    Interface br-int
...
Get the bridge flows
    ...
    [Tags]    OVSDB netvirt
    ${output}    Run Command On Remote System    ${CONTROLLER}    sudo ovs-ofctl -O Openflow13 dump-flows br-int

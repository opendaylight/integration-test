...
Uninstall the feature
   [Documentation]    Unstalling the openstack feature
   Uninstall a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_1_IP}
   Uninstall a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_2_IP}
   Uninstall a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_3_IP}

Install the openstack feature
   [Documentation]    Installing the openstack feature
   Install a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_1_IP}
   Install a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_2_IP}
   Install a Feature    odl-ovsdb-openstack    ${ODL_SYSTEM_3_IP}
...
Checking the Karaf Log
    Open Connection    ${ODL_SYSTEM_1_IP}    port=${KARAF_SHELL_PORT}    prompt=${KARAF_PROMPT}    timeout=60
    Flexible SSH Login    ${KARAF_USER}    ${KARAF_PASSWORD}
    Write    log:tail
    Read Until    /bridge/br-int
    Close Connection
...
Ping Mininet with Controllers
    ...
    ${pingresult}    Run Command On Remote System    ${MININET}    ping ${ODL_SYSTEM_1_IP} -c 10
    ${pingresult}    Run Command On Remote System    ${MININET}    ping ${ODL_SYSTEM_2_IP} -c 10
    ${pingresult}    Run Command On Remote System    ${MININET}    ping ${ODL_SYSTEM_3_IP} -c 10
    Should Not Contain    ${pingresult}    ${PING_NOT_CONTAIN}

Check Cluster status
    [Documentation]    Check Status of cluster
    ${node-id}    Create Dictionary    node-id    ovsdb://uuid/
    Wait Until Keyword Succeeds    10s    2s    Check Item Occurrence At URI In Cluster    ${original_cluster_list}    ${node-id}    ${OPERATIONAL_TOPO_API}

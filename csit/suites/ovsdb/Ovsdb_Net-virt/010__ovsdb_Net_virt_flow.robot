...
Ensure controller is running
    [Documentation]    Check if the controllers are running before sending restconf requests
    [Tags]    Check controller reachability
    @{node_list}      Create Dictionary    ovsdb://uuid/=1
    Wait Until Keyword Succeeds    300s    2s    Check Item Occurrence At URI In Cluster    ${original_cluster_list}    ${node_list}    ${OPERATIONAL_TOPO_API}
...
Check External Net for Tenant
    ...
    ${result}    Create Dictionary    response=200
    Wait Until Keyword Succeeds    300s    4s    Check Item Occurrence At URI In Cluster       ${original_cluster_list}    ${result}    ${ODLREST}/networks
...
Create External Net for Tenant
    ...
    Log    ${Data}
    ${result}    Create Dictionary    response=201
    ${controller_index}    1
    Wait Until Keyword Succeeds    300s    4s    Put And Check At URI In Cluster       ${original_cluster_list}    ${controller_index}    ${ODLREST}/networks    ${Data}

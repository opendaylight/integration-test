...
Bug 7414 Same Endpoint Name
    [Documentation]    There are two real issues to validate. The first is that multiple port creations can be
    ...    made in the same rest request. Bug 7414 will turn up in this case when the port names are the same
    ...    and the transaction will be accepted but the port creation will not make it down to the ovs.
    [Tags]    7414
    [Setup]    Run Keywords    Clean OVSDB Test Environment    ${TOOLS_SYSTEM_1_IP}
    ...
    Should Not Contain    ${ovs_2_output}    local_ip="${TOOLS_SYSTEM_1_IP}", remote_ip="${TOOLS_SYSTEM_2_IP}"
...
Bug 7414 Different Endpoint Name
    ...
    # connect two ovs
    Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    sudo ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
    Run Command On Remote System    ${TOOLS_SYSTEM_2_IP}    sudo ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
    Wait Until Keyword Succeeds    5s    1s    Verify OVS Reports Connected    ${TOOLS_SYSTEM_1_IP}
    Wait Until Keyword Succeeds    5s    1s    Verify OVS Reports Connected    ${TOOLS_SYSTEM_2_IP}
    # add brtest to both
    Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    sudo ovs-vsctl add-br ${BRIDGE}
    Run Command On Remote System    ${TOOLS_SYSTEM_2_IP}    sudo ovs-vsctl add-br ${BRIDGE}
    ...
    Log    ${resp.content}
    # check that each ovs has the correct endpoint
    ${ovs_1_output}=    Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    sudo ovs-vsctl show
    ${ovs_2_output}=    Run Command On Remote System    ${TOOLS_SYSTEM_2_IP}    sudo ovs-vsctl show
    Should Contain    ${ovs_1_output}    local_ip="${TOOLS_SYSTEM_1_IP}", remote_ip="${TOOLS_SYSTEM_2_IP}"
    Should Not Contain    ${ovs_1_output}    local_ip="${TOOLS_SYSTEM_2_IP}", remote_ip="${TOOLS_SYSTEM_1_IP}"
    Should Contain    ${ovs_2_output}    local_ip="${TOOLS_SYSTEM_2_IP}", remote_ip="${TOOLS_SYSTEM_1_IP}"
    Should Not Contain    ${ovs_2_output}    local_ip="${TOOLS_SYSTEM_1_IP}", remote_ip="${TOOLS_SYSTEM_2_IP}"
    [Teardown]    Run Keywords    RequestsLibrary.Delete Request    session    ${CONFIG_TOPO_API}    data=${body}
    ...    AND    Clean OVSDB Test Environment    ${TOOLS_SYSTEM_1_IP}
    ...    AND    Clean OVSDB Test Environment    ${TOOLS_SYSTEM_2_IP}
...
Modify Multi Port Body
    ...
    ${ovs_1_ovsdb_uuid}=    Get OVSDB UUID    ${TOOLS_SYSTEM_1_IP}
    ...
    ${body}    Replace String    ${body}    OVS_1_IP    ${TOOLS_SYSTEM_1_IP}

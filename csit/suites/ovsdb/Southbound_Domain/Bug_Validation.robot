...
Bug OVSDB-439
    ...
    Wait Until Keyword Succeeds    5s    1s    Verify OVS Reports Connected    ${TOOLS_SYSTEM_IP}
    ${ovs_uuid}=    Get OVSDB UUID    ${TOOLS_SYSTEM_IP}
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    -A INPUT -d ${TOOLS_SYSTEM_IP} -j DROP
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    -A OUTPUT -d ${TOOLS_SYSTEM_IP} -j DROP
    Wait Until Keyword Succeeds    60s    10s    Verify OVS Not Connected    ${TOOLS_SYSTEM_IP}
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    -D INPUT -d ${TOOLS_SYSTEM_IP} -j DROP
    Modify Iptables On Remote System    ${ODL_SYSTEM_IP}    -D OUTPUT -d ${TOOLS_SYSTEM_IP} -j DROP

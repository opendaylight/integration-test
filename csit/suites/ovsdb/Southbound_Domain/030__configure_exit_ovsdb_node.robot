...
${SOUTHBOUND_CONFIG_API}    ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:%2F%2F${ODL_TOOLS_SYSTEM_IP}:${OVSDB_PORT}
...
Create a Topology in OVSDB node
    ...
    Run Command On Remote System    ${ODL_TOOLS_SYSTEM_IP}    sudo ovs-vsctl add-br ${BRIDGE}
    Run Command On Remote System    ${ODL_TOOLS_SYSTEM_IP}    sudo ovs-vsctl add-port ${BRIDGE} vx1 -- set Interface vx1 type=vxlan options:remote_ip=192.168.1.11
    Run Command On Remote System    ${ODL_TOOLS_SYSTEM_IP}    sudo ovs-vsctl set-manager ptcp:6634
...
Connect to OVSDB Node
    ...
    ${sample1}    Replace String    ${sample}    127.0.0.1    ${ODL_TOOLS_SYSTEM_IP}
...
Create bridge of already added bridge
    ...
    ${sample2}    Replace String    ${sample1}    127.0.0.1    ${ODL_TOOLS_SYSTEM_IP}
...
Get Operational Topology after Deletion of OVSDB Node
    ...
    @{list}    Create List    ovsdb://${ODL_TOOLS_SYSTEM_IP}:${OVSDB_PORT}    ${BRIDGE}    vxlanport    vx1
    Wait Until Keyword Succeeds    8s    2s    Check For Elements Not At URI    ${OPERATIONAL_TOPO_API}    ${list}
...
Configure Exit OVSDB Node Suite Teardown
    ...
    Clean OVSDB Test Environment    ${ODL_TOOLS_SYSTEM_IP}    ${BRIDGE}
    Delete All Sessions
...
Get Operational Topology to verify the ovs instance is connected to the controller
    ...
    @{list} =    BuiltIn.Create List    ovsdb://uuid    "remote-ip":"${TOOLS_SYSTEM_IP}"    "local-port":6640
...
Verify OVS Not In Config Topology
    ...
    Utils.Check For Elements Not At URI    ${CONFIG_TOPO_API}    ${NODE_LIST}
...
Get Config Topology to verify the manually added bridge is not added to the config datastore
    ...
    BuiltIn.Should Not Contain    ${resp.content}    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}/bridge/${BRIDGE1}

Create a Bridge through controller
    [Documentation]    This will create bridge on the specified OVSDB node.
    Ovsdb.Add Bridge To Ovsdb Node    ${TOOLS_SYSTEM_IP}    ${BRIDGE2}    0000000000000002
...
Create bridge of already added bridge
    ...
    Ovsdb.Add Bridge To Ovsdb Node    ${TOOLS_SYSTEM_IP}    ${BRIDGE1}    0000000000000001
...
Suite Teardown
    ...
    @{urls} =    Builtin.Create List    ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:%2F%2Fuuid%2F${ovsdb_uuid}%2Fbridge%2F${BRIDGE1}
    ....    ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:%2F%2Fuuid%2F${ovsdb_uuid}%2Fbridge%2F${BRIDGE2}

...
Create a Bridge through controller
    ...
    ${sample0}    Replace String    ${sample}     ovsdb://127.0.0.1:61644/bridge/br01    ovsdb://${ovsdb_uuid}/bridge/${BRIDGE2}
    ${sample1}    Replace String    ${sample0}    tcp:127.0.0.1:6633    tcp:${ODL_SYSTEM_IP}:6633
    ...
    Log    URL is ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb://${ovsdb_uuid}/bridge/${BRIDGE2}
    Log    data: ${body}
    ${resp}    RequestsLibrary.Put Request    session    ${CONFIG_TOPO_API}/topology/ovsdb:1/node/ovsdb:%2F%2F${ovsdb_uuid}%2Fbridge%2F${BRIDGE2}    data=${body}
    Log    ${resp.content}
...
Get Config Topology to verify the entry added to the config datastore
    ...
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error
    Should Contain    ${resp.content}    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}/bridge/${BRIDGE2}
...
Get Config Topology to verify the entry of existing bridge added to the config datastore
    ...
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error
    Should Contain    ${resp.content}    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}/bridge/${BRIDGE1}
...
OVSDB Connection Manager Suite Teardown
    ...
    Clean OVSDB Test Environment    ${TOOLS_SYSTEM_IP}    ${BRIDGE1}
    Clean OVSDB Test Environment    ${TOOLS_SYSTEM_IP}    ${BRIDGE2}
    Delete All Sessions
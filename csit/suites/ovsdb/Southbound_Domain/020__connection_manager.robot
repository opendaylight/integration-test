...
Connecting an OVS instance to the controller
    ...
    Run Command On Remote System    ${MININET}    sudo ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
...
Get Config Topology to verify the entry added to the config datastore
    ...
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error
    Should Contain    ${resp.content}    ovsdb://${ODL_TOOLS_SYSTEM_IP}:${OVSDB_PORT}/bridge/${BRIDGE2}
...
Get Config Topology to verify the entry of existing bridge added to the config datastore
    ...
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error
    Should Contain    ${resp.content}    ovsdb://${ODL_TOOLS_SYSTEM_IP}:${OVSDB_PORT}/bridge/${BRIDGE1}

Delete bridge manually
    Run Command On Remote System    ${ODL_TOOLS_SYSTEM_IP}    sudo ovs-vsctl del-br ${BRIDGE2}
...
OVSDB Connection Manager Suite Teardown
    ...
    Clean OVSDB Test Environment    ${ODL_TOOLS_SYSTEM_IP}    ${BRIDGE1}
    Clean OVSDB Test Environment    ${ODL_TOOLS_SYSTEM_IP}    ${BRIDGE2}
    Delete All Sessions
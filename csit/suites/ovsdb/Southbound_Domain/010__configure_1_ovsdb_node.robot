...
Get Configuration Topology to make sure the connection has been deleted
    ...
    @{list} =    BuiltIn.Create List    ovsdb://${TOOLS_SYSTEM_IP}:${OVSDB_PORT}
    BuiltIn.Wait Until Keyword Succeeds    8s    2s    Utils.Check For Elements Not At URI And Print    ${CONFIG_TOPO_API}/topology/ovsdb:1    ${NODE_LIST}
...
Create OVSDB NODE HOST1
    ...
    ${uri} =    ${CONFIG_TOPO_API}/topology/ovsdb:1/

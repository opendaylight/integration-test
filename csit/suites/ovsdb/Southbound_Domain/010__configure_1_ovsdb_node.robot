...
Create OVSDB NODE HOST1
    ...
    Log    URL is /restconf/config/network-topology:network-topology/topology/ovsdb:1/
    Log    data: ${body}
    ${resp}    RequestsLibrary.Post Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1    data=${body}
    Log    ${resp.content}
...
Create QOS entry
    ...
    Log    URL is /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:qos-entries/QOS-1/
    Log    data: ${body}
    ${resp}    RequestsLibrary.Put Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:qos-entries/QOS-1/    data=${body}
    Log    ${resp.content}
...
Create Queue entry to the queues list of a ovsdb node
    ...
    Log    URL is /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:queues/QUEUE-1/
    Log    data: ${body}
    ${resp}    RequestsLibrary.Put Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:queues/QUEUE-1/    data=${body}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Create a QOS and a Linked queue entry to a OVSDB Node
    ...
    Log    URL is /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1
    Log    data: ${body}
    ${resp}    RequestsLibrary.Put Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1    data=${body}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete a Queue entry from a Qos entry
    ...
    ${resp}    RequestsLibrary.Delete Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:qos-entries/QOS-1/queue-list/0/
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error

Delete a QoS entry from a node
    [Documentation]    This request will Delete a QoS entry from a node.
    ${resp}    RequestsLibrary.Delete Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:qos-entries/QOS-1/
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error

Delete a Queue entry from an ovsdb node
    [Documentation]    This request will Delete a Queue entry from an ovsdb node
    ${resp}    RequestsLibrary.Delete Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1/ovsdb:queues/QUEUE-1/
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error

Delete the OVSDB Node HOST1
    [Documentation]    This request will delete the OVSDB node
    ${resp}    RequestsLibrary.Delete Request    session    /restconf/config/network-topology:network-topology/topology/ovsdb:1/node/ovsdb:HOST1
    Should Be Equal As Strings    ${resp.status_code}    200    Response    status code error

...
ConditionalBackupRestoreCheck keyword
    [Documentation]    Demostrates how the ConditionalBackupRestoreCheck keyword can be used together with the flag "y -v BR_TESTING_ENABLED:false/true" in order to add backup-restore verification to existing testcases
...
BackupRestoreCheck keyword
    ...
    ${body}    OperatingSystem.Get File    ${SERVICE_FUNCTIONS_FILE}
    Add Elements To URI From File    ${SERVICE_FUNCTIONS_URI}    ${SERVICE_FUNCTIONS_FILE}
    Run Keyword And Expect Error    *    BackupRestoreCheck    -    -    ../variables/backuprestore/json_prefilter.conf    -
    ${resp}    RequestsLibrary.Get Request    session    ${SERVICE_FUNCTIONS_URI}
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}

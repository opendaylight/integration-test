...
${flowfilterIcmpCodedata}    "vtn-flow-filter": [{"condition": "cond_1","index": "1", "vtn-pass-filter": {}, "vtn-flow-action": [{ "order": "1", "vtn-set-icmp-code-action":{"code": "0"}},{"order": "2","vtn-set-icmp-type-action": {"type": "8"}}]}]	
...
Add a vtn flowfilter with inet4src and inet4dst
    ...
    Wait_Until_Keyword_Succeeds    12s    1s    Mininet Ping Should Not Succeed    h1    h3
    #Wait_Until_Keyword_Succeeds    12s    1s    Mininet Ping Should Succeed    h1    h3
...
Verify inet4src and inet4dst of vtn flowfilter
    ...
    Wait_Until_Keyword_Succeeds    10s    1s    Verify Flow Entries for Flowfilter    ${DUMPFLOWS_OF10}    @{inet_actions}
...
Verify icmp action for vtn flowfilter
    ...
    [Tags]    exclude
    Wait_Until_Keyword_Succeeds    6s    1s    Verify Flow Entries for Flowfilter    ${DUMPFLOWS_OF10}    @{icmp_action}
...
Add a vtn flowfilter with dscp
    ...
    Wait_Until_Keyword_Succeeds    6s    1s    Mininet Ping Should Succeed    h1    h3

Verify dscp action for vtn flowfilter
    [Documentation]    Verify actions in Flow Enties for dscp
    [Tags]    exclude
    Wait_Until_Keyword_Succeeds    12s    1s    Verify flowactions    ${dscp_flow}    ${DUMPFLOWS_OF10}
...
Verify Removed Flow Entry For Inet After Drop Action
    ...
    [Tags]    exclude
    Wait_Until_Keyword_Succeeds    6s    1s    Verify flowactions    ${drop_action}    ${DROP_DUMPFLOWS}

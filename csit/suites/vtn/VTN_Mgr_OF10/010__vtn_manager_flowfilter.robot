...
${flowfilterIcmpCodedata}    "vtn-flow-filter": [{"condition": "cond_1", "vtn-pass-filter": {}, "vtn-flow-action": [{"order": "1", "vtn-set-icmp-code-action":{"code": "11"}},{"order": "2","vtn-set-icmp-type-action": {"type": "22"}}],"index": "2"}]
...
${flowfilterDscpdata}   "vtn-flow-filter":[{"condition": "cond_1","vtn-pass-filter": {},"vtn-flow-action": [{"order": "1","vtn-set-inet-dscp-action": {"dscp":"32"}}],"index":"3"}]
...
Verify inet4src and inet4dst of vtn flowfilter
    ...
    Verify Flow Entries for Flowfilter    ${DUMPFLOWS_OF10}    @{inet_actions}
...
Verify icmp action for vtn flowfilter
    ...
    Verify Flow Entries for Flowfilter    ${DUMPFLOWS_OF10}     @{icmp_action}
...
Verify dscp action for vtn flowfilter
    ...
    Verify flowactions     ${dscp_flow}    ${DUMPFLOWS_OF10}
...
Verify Removed Flow Entry For Inet After Drop Action
    ...
    [Tags]    exclude
    Verify Removed Flow Entry for Inet Drop Flowfilter    @{inet_actions}    ${DUMPFLOWS_OF10}

...
Verification of TSDR ElasticSearch Feature Installation
    ...
    Start Tsdr Suite
    Ping All Hosts
#    Wait Until Keyword Succeeds    5x    30 sec    Check Available values from Elastic Search   FLOWTABLESTATS  2285
...
Getting all Tables from ELK Plugin
    [Documentation]    Getting Flow Table Stats Values from ELK plugin
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketLookup    openflow:1      openflow:1
    Append To List    ${tsdr_pl}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketLookup    openflow:2      openflow:2
    Append To List    ${tsdr_pl}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketLookup    openflow:3      openflow:3
    Append To List    ${tsdr_pl}    ${ret}
    Set Suite Variable    @{tsdr_pl}

    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    ActiveFlows    openflow:1      openflow:1
    Append To List    ${tsdr_af}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    ActiveFlows    openflow:2      openflow:2
    Append To List    ${tsdr_af}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    ActiveFlows    openflow:3      openflow:3
    Append To List    ${tsdr_af}    ${ret}
    Set Suite Variable    @{tsdr_af}

    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketMatch    openflow:1      openflow:1
    Append To List    ${tsdr_pm}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketMatch    openflow:2      openflow:2
    Append To List    ${tsdr_pm}    ${ret}
    ${ret}=     Retrieve Value From Elastic Search  FLOWTABLESTATS    PacketMatch    openflow:3      openflow:3
    Append To List    ${tsdr_pm}    ${ret}

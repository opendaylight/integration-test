...
Verifying TSDR Data Store For Netflow Entries
    ...
    ${metric_log}=    Verify the Metrics Syslog on Cassandra Client    grep NID=127.0.0.1 | grep DC=NETFLOW
    ...
    Should Contain    ${netflow}    engine_type=${engine_type}
    Should Contain    ${netflow}    engine_id=${engine_id}
...
Initialize Netflow
    ...
    Wait Until Keyword Succeeds    24x    10 sec    Iperf All Hosts Cassandra    \\d{2}

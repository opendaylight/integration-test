...
Verification of FlowStats-Attributes on Cassandra Data Store
    ...
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:1*DC=FLOWSTATS*MN=PacketCount*RK=Node:openflow:1,Table:0,Flow:${flow}*
    \   Append To List    ${tsdr_op1_pc}    ${ret_val1}
   
    : FOR    ${flow}    IN    @{openflow_2}
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:2*DC=FLOWSTATS*MN=PacketCount*RK=Node:openflow:2,Table:0,Flow:${flow}*
    \   Append To List    ${tsdr_op2_pc}    ${ret_val1}

    : FOR    ${flow}    IN    @{openflow_3}
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:3*DC=FLOWSTATS*MN=PacketCount*RK=Node:openflow:3,Table:0,Flow:${flow}*
    \   Append To List    ${tsdr_op3_pc}    ${ret_val1}

    : FOR    ${flow}    IN    @{openflow_1}
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:1*DC=FLOWSTATS*MN=ByteCount*RK=Node:openflow:1,Table:0,Flow:${flow}*
    \   Append To List    ${tsdr_op1_bc}    ${ret_val1}

    : FOR    ${flow}    IN    @{openflow_2}
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:2*DC=FLOWSTATS*MN=ByteCount*RK=Node:openflow:2,Table:0,Flow:${flow}*
    \   Append To List    ${tsdr_op2_bc}    ${ret_val1}

    : FOR    ${flow}    IN    @{openflow_3}
    \   ${ret_val1}=    Verify the Metrics Attributes on Cassandra Client    *NID=openflow:3*DC=FLOWSTATS*MN=ByteCount*RK=Node:openflow:3,Table:0,Flow:${flow}*

...
Verification of Full Record Text for Netflow Dumps
    ...
    Should Contain    ${out}    engine_type=${engine_type}
    Should Contain    ${out}    engine_id=${engine_id}
...
Initialize Netflow 
    ...
    COMMENT    Install a Feature    odl-tsdr-netflow-statistics-collector  ${ODL_SYSTEM_IP}    ${KARAF_SHELL_PORT}    60
    Wait Until Keyword Succeeds    24x    10 sec    Check Karaf Log Has Messages     NetFlow Data Colletor Initialized
    Start Tsdr Suite
    Ping All Hosts
    Configure Netflow
    Wait Until Keyword Succeeds    24x    10 sec    Iperf All Hosts HSQLDB    1\\d+

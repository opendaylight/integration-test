...
Add Pre-define Role
    ...
    ${jsonbody}    Read JSON From File    ${PREDEFINE_ROLE_FILE}
    Add Elements To URI From File    ${PREDEFINE_ROLE_URI}     ${jsonbody}
...
Add Pre-define Node
    [Documentation]    Add Pre-define Role
    Create Session    session    http://${CONTROLLER}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    ${jsonbody}    Read JSON From File    ${PREDEFINE_NODE_FILE}

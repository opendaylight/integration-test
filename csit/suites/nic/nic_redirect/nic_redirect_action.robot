...
Verify NIC Redirect CLI Command and Mininet Ping
    ...
    ${source_macaddress}=    Fetch Mac Address    h1
    ${destination_macaddress}=    Fetch Mac Address    h5
...
Install Bridge Utils
    ...
    SSHLibrary.Execute Command    apt-cache policy bridge-utils
    SSHLibrary.Execute Command    sudo apt-get install bridge-utils
    SSHLibrary.Execute Command    dpkg -p bridge-utils
    Write    apt-cache policy bridge-utils
    Read Until    mininet
...
Fetch Mac Address
    [Arguments]    ${host}
    [Documentation]    Getting the host mac address.
    # .... dummy reading ...
    write    ${host} ifconfig -a | grep HWaddr
    ${macaddr_data}    Read Until    mininet>
    # ... actual reading ....
    write    ${host} ifconfig -a | grep HWaddr
    ${macaddr_data}    Read Until    mininet>
    ${macaddress}=    Split String    ${macaddr_data}    ${SPACE}
    ${macaddr}=    Get from List    ${macaddress}    ${index}
    ${macaddr}=    Convert To Lowercase    ${macaddr}
    [Return]    ${macaddr}

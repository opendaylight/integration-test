...
Install Bridge Utils
    ${conn_id}=    SSHLibrary.Open Connection    ${MININET}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=300s
    #SSHLibrary.Login_With_Public_Key    ${ODL_SYSTEM_USER}    ${USER_HOME}/.ssh/${SSH_KEY}    any
    Wait Until Keyword Succeeds    20s    1s    Write    sudo apt-get install bridge-utils
    # To get os-details
    Write    lsb_release -a
    Read Until    mininet
    Write    cat /etc/os-release
    Read Until    mininet>
    # installation
    Write    sudo yum install -q -y
    Read Until    mininet>

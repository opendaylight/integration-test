...
Start Mininet
    ...
    Read Until    mininet>
    #......temp checking
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 ip addr del 10.0.0.6/8 dev srvc1-eth0
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 brctl addbr br0
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 brctl addif br0 srvc1-eth0
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 brctl addif br0 srvc1-eth1
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 ifconfig br0 up
    Wait Until Keyword Succeeds    60s    1s    Write    srvc1 tc qdisc add dev srvc1-eth1 root netem delay 200ms
    #Log    ******* Dump-flows ********
    #write    ${DUMPFLOWS}
    #${result}    Read Until    mininet>
    #Log    ${result}

...
${redirect_topo}     sudo mn --mac --controller=remote,ip=${ODL_SYSTEM_IP} --custom redirect_test.py --topo mytopo2
${index}          7
${sourec_index}    0
${destination_index}    1
${action}    redirect
...
Verify NIC Redirect CLI Command and Mininet Ping
    ...
    ${id}=    Create Intent From Karaf Console For Redirect    00:00:00:00:00:01    00:00:00:00:00:05    ${action}    ${service_name}
...
Install Bridge Utils
    ...
    Wait Until Keyword Succeeds    120s    30s    SSHLibrary.Execute Command    sudo apt-get install bridge-utils
    SSHLibrary.Execute Command    dpkg -L bridge-utils
    SSHLibrary.Execute Command    dpkg -L bridge-utils | grep bin
    SSHLibrary.Execute Command    dpkg -L bridge-utils | grep etc
...
Start Mininet
    ...
    Read Until    mininet>
    #......temp checking
    Wait Until Keyword Succeeds    120s    5s    Write    brctl show
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 ip addr del 10.0.0.6/8 dev srvc1-eth0
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 brctl addbr br0
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 brctl addif br0 srvc1-eth0
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 brctl addif br0 srvc1-eth1
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 ifconfig br0 up
    Wait Until Keyword Succeeds    120s    5s    Write    srvc1 tc qdisc add dev srvc1-eth1 root netem delay 200ms
    #Log    ******* Dump-flows ********
    #write    ${DUMPFLOWS}
    #${result}    Read Until    mininet>
    #Log    ${result}
...
Issue Command On Karaf Console
    [Arguments]    ${cmd}    ${controller}=${ODL_SYSTEM_IP}    ${karaf_port}=${KARAF_SHELL_PORT}    ${timeout}=500

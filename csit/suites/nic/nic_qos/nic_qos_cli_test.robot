...
Verify OF Renderer QOS Constraint CLI
    [Documentation]    Verification of NIC Console command for QOS Constraint.
    [Tags]    NIC
    ${id}=    Add Qos Configuration    @{qos_intent1}
    : FOR    ${intent}    IN    @{all_valid_intent}
    \    ${idd}=    Add Qos From Karaf Console    @{intent}
    \    Append To List    ${all_intents_ids}    ${idd}
    Switch Connection    ${mininet_conn_id}
    Mininet Ping Should Succeed    h1    h2

Verify dscp action
    [Documentation]    Verify actions in dump-flows for dscp

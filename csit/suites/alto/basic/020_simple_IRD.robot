...
Check the simple IRD information
    ...
    Wait Until Keyword Succeeds    10s    2s    Check GET Response Code Equals 200    /${ALTO_SIMPLE_IRD_INFO}
    ...
    Wait Until Keyword Succeeds    10s    2s    Check GET Response Code Equals 200    /${RESOURCE_POOL_BASE}/${context_id}

Create two IRDs
    [Documentation]    Create two IRDs and verify their existence
    Create An IRD    ${DEFAULT_CONTEXT_ID}    ${THE_FIRST_IRD_RESOURCE_ID}
    Wait Until Keyword Succeeds    10s    2s    Check GET Response Code Equals 200    /${ALTO_OPERATIONAL_IRD_INSTANCE}/${THE_FIRST_IRD_RESOURCE_ID}
    Create An IRD    ${DEFAULT_CONTEXT_ID}    ${THE_SECOND_IRD_RESOURCE_ID}
    Wait Until Keyword Succeeds    10s    2s    Check GET Response Code Equals 200    /${ALTO_OPERATIONAL_IRD_INSTANCE}/${THE_SECOND_IRD_RESOURCE_ID}
...
Add one IRD configuration entry in one IRD instance
    ...
    Wait Until Keyword Succeeds    10s    2s    Add An IRD Configuration Entry    ${THE_FIRST_IRD_RESOURCE_ID}    ${DEFAULT_CONTEXT_ID}    ${RESOURCE_IN_FIRST_IRD}    ${BASE_URL}
    Wait Until Keyword Succeeds    10s    2s    Add An IRD Configuration Entry    ${THE_SECOND_IRD_RESOURCE_ID}    ${DEFAULT_CONTEXT_ID}    ${RESOURCE_IN_SECOND_IRD}    ${BASE_URL}
...
Create An IRD
    ...
    ${resp}    RequestsLibrary.Put Request    session    /${ALTO_CONFIG_IRD_INSTANCE_CONFIG}/${IRD_id}    data=${body}    headers=${HEADERS}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Add An IRD Configuration Entry
    ...
    ${resp}    RequestsLibrary.Put Request     session    /${ALTO_CONFIG_IRD_INSTANCE_CONFIG}/${IRD_id}/ird-configuration-entry/${resource_id}    data=${body}    headers=${HEADERS}
    should Be Equal As Strings    ${resp.status_code}    200

...
Check GET Response Code Equals 200
    ...
    ${resp}    RequestsLibrary.GET On Session    session    ${uri_without_ip_port}
...
Create An IRD
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    /${ALTO_CONFIG_IRD_INSTANCE_CONFIG}/${IRD_id}    data=${body}
...
Add An IRD Configuration Entry
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    /${ALTO_CONFIG_IRD_INSTANCE_CONFIG}/${IRD_id}/ird-configuration-entry/${resource_id}    data=${body}
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}

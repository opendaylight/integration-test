...
...               This suite assumes the .zip file is stll present on ${ODL_SYSTEM_IP} in the deafult working directory.
...
Distribution_Size
    ...
    ${ls_output} =    SSHKeywords.Execute_Command_Should_Pass    ls -lAn    ${BUNDLEFOLDER}.zip
    ${size} =    SSHKeywords.Execute_Command_Should_Pass    echo '${ls_output}' | cut -d ' ' -f 5

*** Settings ***
Documentation     Verification that it is possible to run filled nongroup request from collection tab.  
...
Step_05_run
    ...
    @{row_number_1_data}=    Create List    ${Put_Method_NAME}    ${Success_STATUS}    ${Name_1}    ${row_number_4}   
    Set Suite Variable    @{row_number_1_data}    
    @{row_number_2_data}=    Create List    ${Get_Method_NAME}    ${Success_STATUS}    ${Name_2}    ${row_number_5}
    Set Suite Variable    @{row_number_2_data}
    @{row_number_3_data}=    Create List    ${Put_Method_NAME}    ${Error_STATUS}    ${Name_3}    ${row_number_6}
    Set Suite Variable    @{row_number_3_data}

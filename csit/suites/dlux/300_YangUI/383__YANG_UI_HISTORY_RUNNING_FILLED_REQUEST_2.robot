...
Step_06
    ...
    ...    -3rd row - success sent data elements, Param1 key = <<p1>> in code mirror,
    ...    -4th row: - success sent data elements, Param2 key = <<p3>> in code mirror,
    Step_06_run

Step_07
    [Documentation]    Fill the parametrized request from row [3, 4] to form and execute 
    ...    the request by clicking Send button.
    ...    Result
    ...    The page should contain: 
    ...    - 5th row: - success sent data elements, Param1 value v1 in code mirror,
    ...    - 6th row: - success sent data elements, Param2 value v3 in code mirror,
   Step_07_run
...
Step_06_run
    ...
    @{row_1_data}=    Create List    ${Param_Name_1}    ${EMPTY}    ${EMPTY}    ${row_number_3}    ${row_number_5}    ${Param_Value_1}    ${Topology_Id_0}    ${Node_Id_0}    ${Link_Id_0}      
    Set Suite Variable    @{row_1_data}             
    @{row_2_data}=    Create List    ${Param_Name_1}    ${EMPTY}    ${Param_Name_3}    ${row_number_4}    ${row_number_6}    ${Param_Value_3}    ${EMPTY}    ${EMPTY}    ${Link_Id_0}    
...
Step_07_run
    ...
    \    Verify History Table Row Content    ${run_req_row}    ${Put_Method_NAME}    ${Success_STATUS}
    \    Verify Sent Data Elements Presence In History Table Row    ${run_req_row}
    \    Open History Table Sent Data Box    ${run_req_row}
    \    Verify History Sent Box Data Presence In Code Mirror    ${run_req_row}    ${param_value}
    ...
    ${url_topo}=    Return History Table Row Url    ${row_number_3}
    ...
    ${url_link}=    Return History Table Row Url    ${row_number_4}    
    ...
    Compare History Table Row Url And Variable    ${row_number_6}    ${url_topo_and_link}

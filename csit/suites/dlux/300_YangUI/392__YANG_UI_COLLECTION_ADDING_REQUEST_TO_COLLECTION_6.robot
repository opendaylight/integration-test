*** Settings ***
Documentation     Verification that requests are added to collections with name and if you choose group 
...    and then delete it from group input, request is not added to group.
...
Step_07
    [Documentation]    Navigate to HISTORY tab. (POST removed from operations selectbox) PUT topology-id with valid data. Topology id = t0.
...
Step_08
    ...
    ...    Add request to Collection with name to an existing group, then erase group name from group input field.
    ...    Name value: N4. Group value: G3. Clear History data. Navigate to COLLECTION tab.
    ...    Result
...
Step_09
    ...
    ...    Add request to Collection with name to an existing group, then erase group name from group input field. 
    ...    Name value: N5. Group value: G2. Clear History data. Navigate to COLLECTION tab.
    ...    Result
...
Step_10
    [Documentation]    Navigate to HISTORY tab. PUT topology-id withno data.
    ...    Add request to Collection with name to an existing group, then erase group name from group input field. 
    ...    Name value: N6. Group value: G1. Clear History data. Navigate to COLLECTION tab.
    ...    Result
...
Step_07_run
    ...
    Verify Collection Table Group Row Content    3    1    ${Put_Method_NAME}    ${Name_3}    ${Success_STATUS}
...
Step_08_run
    ...
    Insert Text To Input Field    ${Add_To_Collection_Box_Group_Name_New_INPUT}    ${EMPTY}
    Click Element    ${Add_To_Collection_Box_Add_BUTTON}    
    If History Table Contains Data Then Clear History Data
    Close Form In CustomContainer Area    ${Testing_Root_API_Topology_List_Topology_Delete_BUTTON}    ${Testing_Root_API_Topology_List_Topology_Id_INPUT}
    Click Element    ${COLLECTION_TAB}
...
Step_09_run
    ...
    If History Table Contains Data Then Clear History Data
    Click Element    ${COLLECTION_TAB}
    Verify Collection Table Nongroup Row Content    2    ${Put_Method_NAME}    ${Name_5}    ${Error_STATUS}
    Verify No Sent No Received Data Elements Presence In Collection Table Nongroup Row    2
...
Step_10_run
    ...
    Close Form In CustomContainer Area    ${Testing_Root_API_Topology_List_Topology_Delete_BUTTON}    ${Testing_Root_API_Topology_List_Topology_Id_INPUT}
    Click Element    ${COLLECTION_TAB}
    Verify Collection Table Nongroup Row Content    3    ${Put_Method_NAME}    ${Name_6}    ${Success_STATUS}

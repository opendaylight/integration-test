...
Step_08 
    [Documentation]    Click "Add new parameter" button. Insert value into Name input
    ...    field so that this value is already used and any value into Value input field.
    ...    Data: name: p1, value: v3. Click Save button.
    ...    Result
    ...    The application does not allow you to add a parameter with the same name
    ...    as already existing parameter has. The application will show an alert icon.
    Step_08_run
...
Step_08_run
    @{param_names}=    Create List    ${Param_Name_1}    ${Param_Name_2}
    : FOR    ${param_name}    IN    @{param_names}        
    \    Click Element    ${Add_New_Parameter_BUTTON}    
    \    Fill Add New Parameter Box    ${param_name}    ${Param_Value_3}
    \    Verify Add_New_Parameter_Box Visibility
    \    Page Should Contain Element    ${Add_New_Parameter_Form_Param_Check_ICON}
    \    Click Element    ${Add_New_Parameter_Box_Close_BUTTON}

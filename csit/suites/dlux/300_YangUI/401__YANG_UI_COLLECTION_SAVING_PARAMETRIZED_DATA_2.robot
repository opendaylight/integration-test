*** Settings ***
Documentation     Verification that when group edited parametrized request is saved, 
...
Step_05
    ...
    ...    1st row request: Name value: N1, Group value: G1, 2nd row request: Name value: N2, Group value: G2,
    ...    3rd row request: Name value: N3, Group value: G2.
    ...    Navigate to collection tab.
    ...    Result
    ...    The page should contain: - G1: 1st row: - name N1, - success sent data elements,
    ...    - G1: 2nd row: - name N2, - success sent data elements,
    ...    - G2: 1st row: - name N3, - success sent data elements.      
    Step_05_run
...
Step_08
    ...
    ...    Result
    ...    The page should contain: Param1 key = <<p1>> in  G1: 3rd row code mirror..
    ...    Param2 key = <<p2>> in  G1: 4th row code mirror.. Param3 key = <<p3>> in  G2: 2nd row code mirror..
   Step_08_run
...
Step_07_run
    ...
    Open Collection Table Group Sent Data Box    ${group_number_1}    ${row_number_1}
    ...
    Click Element    ${Collection_Table_Group_Row_Sent_Data_Box_Save_Parametrized_Data_BUTTON}
    ...
    Open Collection Table Group Sent Data Box    ${group_number_1}    ${row_number_2}
    ${Collection_Table_Group_Row_Sent_Data_BOX}=    Set Variable    ${Collection_Table_List_Group_ROW}${Sent_Data_BOX}
    ${Collection_Table_Group_Row_Sent_Data_Box_Path_WRAPPER}=    Set Variable    ${Collection_Table_Group_Row_Sent_Data_BOX}${Sent_Data_Box_Path_WRAPPER}    
    ${Collection_Table_Group_Row_Sent_Data_Box_Topology_Id_Path_Wrapper_INPUT}=    Set Variable    ${Collection_Table_Group_Row_Sent_Data_Box_Path_WRAPPER}${Sent_Data_Box_Topology_Id_Path_Wrapper_INPUT}    
    ...
    Open Collection Table Group Sent Data Box    ${group_number_2}    ${row_number_1}
    ${Collection_Table_Group_Row_Sent_Data_BOX}=    Set Variable    ${Collection_Table_List_Group_ROW}${Sent_Data_BOX}
    ${Collection_Table_Group_Row_Sent_Data_Box_Path_WRAPPER}=    Set Variable    ${Collection_Table_Group_Row_Sent_Data_BOX}${Sent_Data_Box_Path_WRAPPER}    
    ${Collection_Table_Group_Row_Sent_Data_Box_Topology_Id_Path_Wrapper_INPUT}=    Set Variable    ${Collection_Table_Group_Row_Sent_Data_Box_Path_WRAPPER}${Sent_Data_Box_Topology_Id_Path_Wrapper_INPUT}    
...
Step_08_run
    Click Element    ${COLLECTION_TAB}
    @{groups}=    Create List    ${group_number_1}    ${group_number_1}    ${group_number_2}
    @{rows}=    Create List    ${row_number_3}     ${row_number_4}    ${row_number_2}    
    @{names}=    Create List    ${Name_1}    ${Name_2}    ${Name_3}
    ${index}    Evaluate    0
    : FOR    ${group}    IN    @{groups}
    \    ${row}=     Get From List    ${rows}   ${index}                
    ...
    ${index}    Evaluate    0
    @{param_keys}=    Create List     ${parameter_key_1}     ${parameter_key_3}     ${parameter_key_5}
    : FOR    ${group}    IN    @{groups}
    \    ${row}=     Get From List    ${rows}   ${index}                
    \    ${pram_key}=    Get From List    ${param_keys}    ${index}                
    \    Open Collection Table Group Sent Data Box    ${group}    ${row}
    \    Verify Collection Group Sent Box Data Presence In Code Mirror    ${group}   ${row}    ${pram_key}
    \    ${index}    Evaluate    ${index}+1

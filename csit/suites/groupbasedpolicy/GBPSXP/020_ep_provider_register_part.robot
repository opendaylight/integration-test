...
Test Setup        Prepare ssh tooling
...
Create sxp node
    ...
    # create sxp-node in topology
    Add Elements To URI From File    ${SXP_TOPOLOGY_NODE_CONFIG_URI}/node/${sxp_node_id}    ${sxp_node_config_file}
    # wait till it appears in DS/operational
    Wait until keyword succeeds    5    1    Check if sxp node is enabled    ${session_arg}    ${SXP_TOPOLOGY_NODE_OPERATIONAL_URI}/node/${sxp_node_id}

...
Find Features On ODL System
    ...
    \    Set Suite Variable    ${CURRENT_INSTALLATION_COUNT}    ${0}
    \    Continue For Loop If    '${ip}' == '${GBP_MASTER_IP}'
    \    Log    current IP:${ip}
    \    ${all_installed}=    Check Features Installed On IP    ${ip}
    \    Log    all_in:${all_installed}
    \    ${number_of_installations}=    Run Keyword If    ${all_installed}    ${number_of_installations+1}

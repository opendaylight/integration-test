...
#    ${esc_feature_name}    Regexp Escape    ${feature_name}
    ${result}=    Evaluate    re.match(r'${feature_name}\|.+?\|x?\|', '''${output}''').group(0)    re
    Log    ${result}
    ${result}=    Evaluate    re.match(r'[-\\w]+\|.+?\|x?\|', '''${output}''').group(0)    re
    Log    ${result}
    ${result}=    Evaluate    re.search(r'|x|', '''${output}''').group(0)    re
    Log    ${result}
    ${result}=    Evaluate    re.search(r'||', '''${output}''').group(0)    re
    Log    ${result}

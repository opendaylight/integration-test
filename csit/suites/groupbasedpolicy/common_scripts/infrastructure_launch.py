    ...
    for sw in switches:
        ...
        for host in hosts:
            if host['switch'] == sw['name']:
                ...
                print "Created container: %s with IP: %s. Connect using docker attach %s, disconnect with 'ctrl-p-q'." % (
                host['name'], host['ip'], host['name'])

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print "Usage: Please, specify IP of ODL in the argument."
        sys.exit()
    if len(sys.argv) > 2:
        print "Usage: Please, specify only IP of ODL in arguments."
        sys.exit()
    ...
    if sw_index in range(0, len(switches) + 1):
        ...
        if sw_type == 'gbp':
            ...
            print "*****************************"
            sf = "sudo " + os.path.dirname(os.path.realpath(__file__)) + "/sf-config.sh " + sw_name
            doCmd(sf)

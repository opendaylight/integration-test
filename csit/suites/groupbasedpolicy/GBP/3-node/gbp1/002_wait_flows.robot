...
Wait For Flows
    ${passed} =  Run Keyword And Return Status    ConnUtils.Wait For Flows On Switch  ${GBP1}  sw1
    Run Keyword Unless    ${passed}    Fatal Error    Flows not created on sw1!
    ${passed} =  Run Keyword And Return Status    ConnUtils.Wait For Flows On Switch  ${GBP2}  sw2
    Run Keyword Unless    ${passed}    Fatal Error    Flows not created on sw2!
    ${passed} =  Run Keyword And Return Status    ConnUtils.Wait For Flows On Switch  ${GBP3}  sw3

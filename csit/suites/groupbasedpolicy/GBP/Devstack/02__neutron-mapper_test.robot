...
Test Create Security Group Rule With Remote Group ID
    ...
    ${remote_ip_prefix}    Set Variable    ${REMOTE_IP_PREFIX}
    ${uuid}    Create Security Group Rule    ${SERVER_SG}
    ...    --direction ingress --ethertype IPv4 --protocol udp --port-range-min 8080 --port-range-max 8090 --remote-group-id ${CLIENT_SG}
    ...    verbose=FALSE
    ${group_rule_id}    Should Match Regexp    ${uuid}    ${UUID_PATTERN}
    Set Global Variable    ${GROUP_RULE_ID}    ${group_rule_id}
...
Verify Created Server Port
    ...
    ${status}    Run Keyword And Ignore Error    Verify Neutron Port    ${SERVER_MAC_ADDR}    ${SERVER_PORT_IP}    ${SERVER_SG_ID}
...
Verify Created Client Port
    ...
    ${status}    Run Keyword And Ignore Error    Verify Neutron Port    ${CLIENT_MAC_ADDR}    ${CLIENT_PORT_IP}    ${CLIENT_SG_ID}
    Return From Keyword If    '${status}' == 'PASS'

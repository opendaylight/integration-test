...
Test neutron router-create
    ...
    ${router_id}       Wrap Command and Execute
    ...    neutron router-create ${ROUTER_NAME} | grep -w id | awk '{print $4}'
    Should Not Be Empty    ${router_id}
...
Test neutron port-create and verify
    ...
    \    ${endpoint-l3_path}  Get EndpointL3 Path    ${ROUTER_ID}    ${port_ip}
...
Verify Contract
    ...
    ...    ${classif_inst_json['classifier-instance'][0]['parameter-value']}    _
...
Verify Server Endpoint Group
    [Documentation]    Verifies data generated by security group and security group rules.
    ${epg_path}   Get Endpoint Group Path    ${TENANT_ID}    ${SERVER_SG_ID}
    ${endpoint_group}        Get Data From URI    session    ${epg_path}    headers=${headers}
    Should Not Be Empty    ${endpoint_group}
    ${endpoint_group_json}    To Json    ${endpoint_group}
    ${provider_named_selectors}   Set Variable  ${endpoint_group_json['endpoint-group'][0]['consumer-named-selector']}
    Set Global Variable  ${selector_found}    FALSE
    : FOR    ${selector}    IN    @{provider_named_selectors}
    \    Run Keyword If    "${selector['contract'][0]}" == "${GROUP_RULE_ID}"  Set Global Variable  ${selector_found}    TRUE
    Should Be Equal As Strings    ${selector_found}    TRUE
...
Wrap Command and Execute
    ...
    ...    cd /home/vagrant/devstack && source openrc admin admin && ${cmd}

...
Test neutron subnet-create and verify
    ...
    ${subnet_id}    Create Neutron Entity And Return ID
    ...    neutron subnet-create ${NETWORK_NAME} ${SUBNET_IP_PREFIX} --name ${SUBNET_NAME}
...
Test neutron security-group-create
    ...
    \    ${security_group_id}     Create Neutron Entity And Return ID
...
Test neutron security-group-rule-create
    ...
    ${group_rule_id}       Create Neutron Entity And Return ID
    ...    neutron security-group-rule-create ${SERVER_SG} --direction ingress --ethertype IPv4 --protocol tcp --port-range-min 80 --port-range-max 90 --remote-ip-prefix ${remote_ip_prefix}
    Set Global Variable    ${GROUP_RULE_ID}     ${group_rule_id}

Test neutron router-create
    [Documentation]    Creates neutron router.
    ${router_id}       Create Neutron Entity And Return ID    neutron router-create ${ROUTER_NAME}
...
Start Suite
    ...
    SSHLibrary.Open Connection    ${ip}    prompt=${prompt}   alias=${DEVSTACK_CONN_ALIAS}
...
Clean Suite
    ...
    @{commands}  Set Variable  neutron security-group-delete client_sg    neutron security-group-delete server_sg
    ...  neutron port-delete ${CLIENT_PORT_NAME}  neutron port-delete ${SERVER_PORT_NAME}

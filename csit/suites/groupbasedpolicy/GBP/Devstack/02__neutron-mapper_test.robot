...
Test Create Network
    ...
    ${uuid}    Create Network    net123
    ${l2_fd_id}     Should Match Regexp  ${uuid}  ${UUID_PATTERN}
    ${l2_fd_path}   Get L2 Flood Domain Path        ${TENANT_ID}     ${l2_fd_id}
    ${l2_fd_path}   Set Variable    restconf/config/neutron:neutron
    ${l2_fd}        Get Data From URI    session    ${l2_fd_path}    headers=${headers}
    Log    ${l2_fd}
    ${policy}       Get Data From URI    session    restconf/config/policy:tenants    headers=${headers}
    Log    ${policy}

...
${GBP_POLICY}        /restconf/config/policy:tenants

*** Test Cases ***
Run Devstack Gate Wrapper
    Write Commands Until Prompt    unset GIT_BASE
    ${output}=    Write Commands Until Prompt    env
    Log    ${output}
...
Set OVS
    ...
    Log    ${output}
    ${output}=    Write Commands Until Prompt    sudo ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
    ${output}=    Write Commands Until Prompt    sudo ovs-vsctl set-controller br-int tcp:${ODL_SYSTEM_IP}:6653
    Sleep   60s
    ${output}=    Write Commands Until Prompt    sudo ovs-vsctl show
    Log    ${output}
...
Testing ODL restconf port 8181 with different headers
    ...
    Wait Until Keyword Succeeds    5x    5 s    Get Data From URI    session    ${GBP_POLICY}    headers=${HEADERS_YANG_JSON}
    Delete All Sessions
...
Devstack Suite Setup
    ...
    #Write Commands Until Prompt    export DEVSTACK_LOCAL_CONFIG+="ODL_NETVIRT_DEBUG_LOGS=True;"

    Write Commands Until Prompt    export DEVSTACK_LOCAL_CONFIG+="ODL_MODE=external;ODL_MGR_IP=${ODL_SYSTEM_IP};ODL_PORT=8080;"

...
Neutron exploration
    ...
    SSHLibrary.Put File    ${CURDIR}/local.conf    /opt/stack/new/devstack/local.conf    mode=0755
    ${results}    Write Commands Until Prompt    git checkout stable/kilo
    Log    ${results}
    ${results}    Write Commands Until Prompt    ./stack.sh >/dev/null 2>&1  &&  echo success
    Log    ${results}
...
Neutron exploration
    Create Session    session    http://${ODLIP_test}:8181    auth=${AUTH}    headers=${HEADERS_YANG_JSON}
    Wait Until Keyword Succeeds    120x    5 s    Get Data From URI    session    ${ODL_BOOT_WAIT_URL}    headers=${headers}

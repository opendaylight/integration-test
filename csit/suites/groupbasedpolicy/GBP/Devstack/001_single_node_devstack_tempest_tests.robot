...
Test2
    ...
    ${results}    Write Commands Until Prompt    ps aux | grep java
    Log    ${results}
    ${results}    Write Commands Until Prompt    sudo curl -v http://admin:admin@localhost:8181/restconf/operational/network-topology:network-topology/topology/netvirt:1
    Log    ${results}

Neutron exploration
    Create Session    session    http://${ODLIP}:8181    auth=${AUTH}    headers=${HEADERS_YANG_JSON}

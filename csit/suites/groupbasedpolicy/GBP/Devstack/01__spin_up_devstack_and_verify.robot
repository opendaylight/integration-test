...
Show devstack dir
    ${output}    Write Commands Until Prompt    ls -R /opt/stack
    LOG    ${output}
...
Put GBP Goodness In To The Mix
    ...
    Install a Feature    odl-groupbasedpolicy-base             ${DEVSTACK_SYSTEM_IP}    timeout=120
    Sleep    60s
    Install a Feature    odl-groupbasedpolicy-ofoverlay        ${DEVSTACK_SYSTEM_IP}    timeout=120
    Sleep    60s
    Install a Feature    odl-groupbasedpolicy-neutronmapper    ${DEVSTACK_SYSTEM_IP}    timeout=120
    Sleep    30s
...
Cat log
    ...
    ${output}=    Write Commands Until Prompt     path=$(find /opt/stack/ -name karaf.log)
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     cat $path
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     path=$(find /opt/stack/opendaylight -name karaf.log)
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     cat $path
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     path=$(find /opt/stack/new/ -name karaf.log)
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     cat $path
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     path=$(find /opt/stack/new/opendaylight -name karaf.log)
    LOG    ${output}
    ${output}=    Write Commands Until Prompt     cat $path
    LOG    ${output}
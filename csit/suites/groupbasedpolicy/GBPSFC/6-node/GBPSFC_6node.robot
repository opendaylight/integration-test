...
Setup Node
    ...
    ${stderr}    SSHLibrary.Execute Command     ${VM_HOME_FOLDER}${/}${VM_SCRIPTS_FOLDER}/execute-in-ve.sh python
    ...    ${VM_HOME_FOLDER}${/}${VM_SCRIPTS_FOLDER}//infrastructure_launch.py    return_stderr=True
    ...    return_stdout=False
    Should Be Empty    ${stderr}
    Run Keyword If    "${GBPSFC}" == "${GBPSFC3}"
    ...    Render SF Flows    ${GBPSFC2}    ${stderr}
    ...    ELSE IF    "${GBPSFC}" == "${GBPSFC5}"
    ...    Render SF Flows    ${GBPSFC4}    ${stderr}
    Should Be Empty    ${stderr}

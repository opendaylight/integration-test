...
Setup Node
    ...
    ${stdout}    ${stderr}    ${rc}    SSHLibrary.Execute Command    git clone https://github.com/yyang13/ovs_nsh_patches.git    return_stderr=True    return_stdout=True    return_rc=True
    Log    ${stdout}
    Log    ${stderr}
    ${stdout}    ${stderr}    ${rc}    SSHLibrary.Execute Command    cd ovs_nsh_patches    return_stderr=True    return_stdout=True    return_rc=True
    Log    ${stdout}
    Log    ${stderr}
    ...
    ${stdout}    ${stderr}    ${rc}    SSHLibrary.Execute Command    dmesg    return_stderr=True    return_stdout=True    return_rc=True
    Log    ${stdout}
    Log    ${stderr}
    ${stdout}    ${stderr}    ${rc}    SSHLibrary.Execute Command    cat /var/log/openvswitch/ovs-ctl.log    return_stderr=True    return_stdout=True    return_rc=True
    Log    ${stdout}
    Log    ${stderr}

...
Setup_GBPSFC_6node
    ...
    SSHLibrary.Put Directory    ${CURDIR}/init_scripts    ${VM_HOME_FOLDER}${/}${VM_SCRIPTS_FOLDER}/
    ...
    # TODO 'cat'->'sudo' x2
    ${stdout}    ${stderr}=    SSHLibrary.Execute Command    cat ${VM_HOME_FOLDER}${/}${VM_SCRIPTS_FOLDER}/infrastructure_launch.py
    ...    return_stderr=True
    Log    ${stdout}
    Log    ${stderr}
    ${stdout}    ${stderr}=    SSHLibrary.Execute Command    cat ${VM_HOME_FOLDER}${/}${VM_SCRIPTS_FOLDER}/get-nsps.py
    ...    return_stderr=True
    Log    ${stdout}
    Log    ${stderr}
    SSHLibrary.Close Connection

Teardown_GBPSFC_6node
    [Arguments]    ${GBPSFC}    ${timeout}=3s
    ConnUtils.Connect and Login    ${GBPSFC}    timeout=${timeout}

...
TC26 7.2.1 Verification of intra_router_intra_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[2]    ${VM_IP_NET3[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[3]    ${VM_IP_NET4[0]}    ${VM_IP_NET4[1]}
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Get Fib Entries    session
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}    @{REQ_SUBNET_CIDR_TESTAREA2}
...
TC27 7.2.2 Verification of intra_router_inter_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[2]    ${VM_IP_NET3[0]}    ${VM_IP_NET3[2]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[3]    ${VM_IP_NET4[0]}    ${VM_IP_NET4[2]}
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Get Fib Entries    session
...
TC28 7.2.3 Verification of inter_router_intra_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[0]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[1]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[0]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[1]}
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Get Fib Entries    session
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
    \    Should Contain    ${output}    ${IP}
...
TC29 7.2.4 Verification of inter_router_inter_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[2]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
...
#    ${VM_INSTANCES} =    Create List    @{VM_INSTANCES_DPN1}    @{REQ_VM_INSTANCES_NET2}
...
#    ${VM_IP_NET1_New}    ${VM_IP_NET2_New}    ${VM_IP_NET3_New}    ${VM_IP_NET4_New}    Wait Until Keyword Succeeds    300s    10s    Verify VMs received IP
#    ${output}=    Wait Until Keyword Succeeds    60s    10s    Get Fib Entries    session
...
#    \    Should Contain    ${output}    ${IP}
#
#TC39 7.2.14 Verify manual TEP recreation with modified TEP IPs for L3VPNoVxLAN
#    [Documentation]    Testcase Id 7.2.14
#    [Tags]    Nightly
#    ${exp_result}    ConvertToInteger    1
#    Log    "Testcases covered as per Testplan ${\n} 7.2.14 Verify manual TEP recreation with modified TEP IPs for L3VPNoVxLAN ${\n}"
#    Log    "STEP 2 : PING VNF11 <-> VNF33 VNF22<-> VNF44 VNF13<-> VNF31 VNF14<-> VNF41"
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[2]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
...
#    ${dpn_id1}    Get DPID    ${OS_COMPUTE_1_IP}
#    ${dpn_id2}    Get DPID    ${OS_COMPUTE_2_IP}
...
#
#TC40 7.2.15 Verify TEP UP/DOWN events for L3VPNoVxLAN VNFs
#    [Documentation]    Testcase Id 7.2.15
#    [Tags]    Nightly
#    ${exp_result}    ConvertToInteger    1
#    Log    "Testcases covered as per Testplan ${\n} 7.2.15      Verify TEP UP/DOWN events for L3VPNoVxLAN VNFs ${\n}"
#    Log    "STEP 2 : PING VNF11 <-> VNF33 VNF22<-> VNF44 VNF13<-> VNF31 VNF14<-> VNF41"
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[2]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
...
#    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
#    \    Should Contain    ${output}    ${IP}
#    Log    "STEP 5 : PING VNF11 <-> VNF33 VNF22<-> VNF44 VNF13<-> VNF31 VNF14<-> VNF41"
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[2]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
#
...
#    Wait Until Keyword Succeeds    60s    10s    ${output}=    Get Fib Entries    session
#    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
#    \    Should Not Contain    ${output}    ${IP}
...
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${LOOPBACK_IP}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${LOOPBACK_IP1}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${LOOPBACK_IP1}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${LOOPBACK_IP1}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[2]    ${VM_IP_NET3[0]}    ${LOOPBACK_IP1}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[3]    ${VM_IP_NET4[0]}    ${LOOPBACK_IP1}
...
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[3]}    ${VM_IP_NET4[0]}
#    Wait Until Keyword Succeeds    60s    10s    ${output} =    Get Fib Entries    session
#    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
#    \    Should Contain    ${output}    ${IP}
...
#    \    Should Contain    ${output}    ${IP}
#
#TC23 7.2.23 Verify ASR DCGW BGP session down from CSC
#    [Documentation]    Verify ASR DCGW BGP session down from CSC
#    ${exp_result}    ConvertToInteger    1
#    Log    "Testcases covered as per Testplan ${\n} 7.2.23 Verify ASR DCGW BGP session down from CSC"
#    Log    "STEP 2 : PING VNF11 <-> VNF33 VNF22<-> VNF44 VNF13<-> VNF31 VNF14<-> VNF41"
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[2]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[1]}    ${VM_IP_NET4[3]}
#    Wait Until Keyword Succeeds    180s    10s    Verify Ping Success    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[2]}    ${VM_IP_NET3[0]}
...
Add Interfaces To Routers
    ...
    \    ${subnet_id} =    Get Subnet Id    ${INTERFACE}    ${devstack_conn_id}
    \    Should Contain    ${interface_output}    ${subnet_id}
    ${GWMAC_ADDRS_ROUTER1}    ${GWIP_ADDRS_ROUTER1} =    Get Gateway MAC And IP Address    ${REQ_ROUTERS[0]}
    Log    ${GWMAC_ADDRS_ROUTER1}
    Set Suite Variable    ${GWMAC_ADDRS_ROUTER1}
    Log    ${GWIP_ADDRS_ROUTER1}
    Set Suite Variable    ${GWIP_ADDRS_ROUTER1}
    ${GWMAC_ADDRS_ROUTER2}    ${GWIP_ADDRS_ROUTER2} =    Get Gateway MAC And IP Address    ${REQ_ROUTERS[1]}
...
Change Br-ext Ip
    ...
    [Documentation]    Get DpnId from server and return
    ${conn_handle}    SSHLibrary.Open Connection    ${serverip}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=30s
    #Set Client Configuration    prompt=#

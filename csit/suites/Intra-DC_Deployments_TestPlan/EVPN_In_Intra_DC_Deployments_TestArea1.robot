...
TC15 7.1.15 Verify TEP UP/DOWN events for L3VPNoVxLAN VNFs
    ...
    ${dpn_id1}    Get DPID    ${OS_COMPUTE_1_IP}
    ${output}=    Wait Until Keyword Succeeds    30s    1s    Bring Down Br-ext    ${OS_COMPUTE_1_IP}
    log    ${output}
    Wait Until Keyword Succeeds    180s    10s    Verify ITM Status    DOWN
    ${output}=    Wait Until Keyword Succeeds    30s    1s    Bring Up Br-ext    ${OS_COMPUTE_1_IP}
    log    ${output}
...
Bring Down Br-ext
    [Arguments]    ${ip}
    [Documentation]    Bring down the external Bridge.
    ${output} =    Run Command On Remote System    ${ip}    ifconfig br-ext down
    [Return]    ${output}

Bring Up Br-ext
    [Arguments]    ${ip}
    [Documentation]    Bring down the external Bridge.
    ${output} =    Run Command On Remote System    ${ip}    ifconfig br-ext up
    [Return]    ${output}

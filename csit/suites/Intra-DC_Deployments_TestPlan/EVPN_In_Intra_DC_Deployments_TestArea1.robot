...
Verification of intra_network_intra_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET1[1]}    , 0% packet loss
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET2[1]}    , 0% packet loss
...
Verification of intra_network_inter_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET1[2]}    , 0% packet loss
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET2[2]}    , 0% packet loss
    ${output}=    Get Fib Entries    session
...
Verification of inter_network_intra_openvswitch network connectivity
    ...
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET2[0]}    , 0% packet loss
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[1]}    ${VM_IP_NET2[1]}    , 0% packet loss
    ${output}=    Get Fib Entries    session
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}
    \    Should Contain    ${output}    ${IP}
...
Verification of subnet route and VNF as gateway for enterprise network and after VNF reboot
    ...
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[3]}    ${VM_IP_NET1[0]}    , 0% packet loss
    Wait Until Keyword Succeeds    40s    10s    Verify VM to VM Ping Status    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[3]}    @{EXTRA_NW_IP}[0]    , 0% packet loss

*** Settings ***
Documentation      Test Suite for SF218 EVPN In Inter DC Deployments with CBA \ NON CBA based ODL Cluster TESTAREA4
...
CREATE TEST TOPOLOGY FOR TEST AREA 4
    ...
    ${VM_IP_NET1}    ${VM_IP_NET2}    ${VM_IP_NET3}    ${VM_IP_NET4}    ${VM_IP_NET5}    ${VM_IP_NET6}    ${VM_IP_NET7}    ${VM_IP_NET8}    Wait Until Keyword Succeeds    300s    10s    Verify VMs received IP
...
TC63 7.4.1 Verify network connectivity between VNF_Network to VNF_Router on intra_openvswitch for L3VPNoVxLAN1
    ...
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}    @{REQ_SUBNET_CIDR_TESTAREA4}
...
TC69_TC70 7.4.7_7.4.8 Verify deletion and recreation of L3VPNoVxLAN1 and its impact on L3VPNoVxLAN2
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.7 Verify deletion L3VPNoVxLAN which has networks associated ${\n} 7.4.8 Verify recreation of L3VPNoVxLAN and re associate the networks ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32, VNF21<-> VNF42"
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[2]}
    ${output}=    Get Fib Entries    session
...
TC71_TC72 7.4.9_7.4.10 Verify manual TEP deletion and recreation scenario for L3VPNoVxLAN VNFs
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.9 Verify manual TEP deletion scenario for L3VPNoVxLAN VNFs ${\n} 7.4.10 Verify manual TEP recreation scenario for L3VPNoVxLAN VNFs ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32, VNF21<-> VNF42"
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[2]}
    ${output}=    Get Fib Entries    session
...
TC69 7.4.11     Verify manual TEP recreation with modified TEP IPs for L3VPNoVxLAN VNFs
    [Documentation]    Testcase Id 7.4.11
    [Tags]    Nightly
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.11 Verify manual TEP recreation with modified TEP IPs for L3VPNoVxLAN VNFs ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32 VNF21<-> VNF42 "
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[1]}
    ...
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
    \    Should Contain    ${output}    ${IP}
    Log    "STEP 6 : PING VNF11 <-> VNF32 VNF21<-> VNF42 "
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[1]}
...
TC70 7.4.12     Verify TEP UP/DOWN events for L3VPNoVxLAN VNFs
    [Documentation]    Testcase Id 7.4.12
    [Tags]    Nightly
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.12      Verify TEP UP/DOWN events for L3VPNoVxLAN VNFs ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32 VNF21<-> VNF42 "
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[1]}
    ...
    : FOR    ${IP}    IN    @{VM_IP_NET1}    @{VM_IP_NET2}    @{VM_IP_NET3}    @{VM_IP_NET4}
    \    Should Contain    ${output}    ${IP}
    Log    "STEP 5 : PING VNF11 <-> VNF32 VNF21<-> VNF42 "
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[1]}



TC73 7.4.13 Verify ASR route updates for L3VPNoVxLAN
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.13 Verify ASR route updates for L3VPNoVxLAN ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32, VNF21<-> VNF42"
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[2]}
    ${output}=    Get Fib Entries    session
...
TC74_TC75 7.4.14_7.4.15
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.14 Verify ASR DCGW deletion from CSC ${\n} 7.4.15 Verify ASR DCGW re addition from CSC ${\n}"
...
TC76 7.4.16 Verify ASR DCGW BGP session down from CSC
    [Documentation]    Verify ASR DCGW BGP session down from CSC
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.16 Verify ASR DCGW BGP session down from CSC"
...
TC77 7.4.17 Verify nova migration for L3VPNoVxLAN VNFs
    ${exp_result}    ConvertToInteger    1
    Log    "Testcases covered as per Testplan ${\n} 7.4.17 Verify nova migration for L3VPNoVxLAN VNFs ${\n}"
    Log    "STEP 2 : PING VNF11 <-> VNF32, VNF21<-> VNF42"
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[0]    ${VM_IP_NET1[0]}    ${VM_IP_NET3[1]}
    Wait Until Keyword Succeeds    180s    10s    Verify Ping    @{REQ_NETWORKS}[1]    ${VM_IP_NET2[0]}    ${VM_IP_NET4[2]}
    ${output}=    Get Fib Entries    session
...
    SSHLibrary.Login    ${OS_USER}    ${LOGIN_PSWD}
    ${CmdOut}    Write Commands Until Prompt    sudo ovs-ofctl show -O Openflow13 br-int | head -1 | awk -F "dpid:" '{ print $2 }'
...
Change Br-ext Ip
    ...
    Set Client Configuration    prompt=#
    SSHLibrary.Login    ${OS_USER}    ${LOGIN_PSWD}
    ${CmdOut}    Write Commands Until Prompt    sudo ifconfig br-ext ${ip} netmask 255.255.255.0
    SSHLibrary.Close Connection
...
Bring Down Br-ext
    ...
    Set Client Configuration    prompt=#
    SSHLibrary.Login    ${OS_USER}    ${LOGIN_PSWD}
    ${CmdOut}    Write Commands Until Prompt    ifconfig br-ext down
    SSHLibrary.Close Connection
...
Bring Up Br-ext
    ...
    Set Client Configuration    prompt=#
    SSHLibrary.Login    ${OS_USER}    ${LOGIN_PSWD}
    ${CmdOut}    Write Commands Until Prompt    ifconfig br-ext up
    SSHLibrary.Close Connection

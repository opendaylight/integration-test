...
Create Nova VMs
    ...
    \    Create Vm Instance With Port On Compute Node    ${REQ_PORT_LIST[${index}]}    ${VM_INSTANCES_DPN1[${index}]}    ${OS_COMPUTE_1_IP}    flavor=m1.tiny    sg=${SECURITY_GROUP}
    ${start} =    Evaluate    ${index}+1
    ${NUM_OF_VMS_PER_DPN} =    Evaluate    ${start}+${NUM_OF_VMS_PER_DPN}
    : FOR    ${index}    IN RANGE    ${start}    ${NUM_OF_VMS_PER_DPN}
    \    Create Vm Instance With Port On Compute Node    ${REQ_PORT_LIST[${index}]}    ${VM_INSTANCES[${index}]}    ${OS_COMPUTE_2_IP}    flavor=m1.tiny    sg=${SECURITY_GROUP}
    List Nova VMs
...
Delete Setup
    ...
    : FOR    ${INTERFACE}    IN    @{ROUTER1_INTERFACE_TESTAREA5}
    \    Remove Interface    ${REQ_ROUTERS[0]}    ${INTERFACE}
    : FOR    ${INTERFACE}    IN    @{ROUTER2_INTERFACE_TESTAREA5}
    \    Remove Interface    ${REQ_ROUTERS[1]}    ${INTERFACE}
    Log    Delete Routers
    : FOR    ${index}    IN    @{REQ_ROUTERS}
    \    Delete Router    ${index}
    Log    Delete the VM instances
    : FOR    ${VmInstance}    IN    @{VM_INSTANCES_TEST_TOPOLOGY_5}
    \    Delete Vm Instance    ${VmInstance}
    Log    Delete neutron ports
    : FOR    ${Port}    IN    @{PORT_LIST_TEST_TOPOLOGY_5}
    \    Delete Port    ${Port}
    Log    Delete subnets

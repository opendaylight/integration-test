...
Delete Setup
    ...
    : FOR    ${VmInstance}    IN    @{VM_INSTANCES_TEST_TOPOLOGY_4}
...
Create Routers
    ...
    \    Create Router    ${ROUTERS[${index}]}
    ${router_output} =    List Router
    Log    ${router_output}
    :FOR   ${index}   IN RANGE   0   ${NUM_OF_ROUTERS}
    \    Should Contain    ${router_output}    ${ROUTERS[${index}]}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${CONFIG_API}/neutron:neutron/routers/    ${ROUTERS}

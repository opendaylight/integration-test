...
Isolation of SXP service follower Test
    [Documentation]    Test SXP connection switchover while peer are communicating via virtual ip
...
Isolation of SXP service follower Test Listener Part
    [Documentation]    Test SXP binding propagation while peer are communicating via virtual ip
    ${controller_index}    Get Active Controller
    Setup Custom SXP Cluster    ${VIRTUAL_IP}    listener
...
Setup Custom SXP Cluster Session
    ...
    \    Issue Command On Karaf Console    log:set DEBUG org.opendaylight.sxp.route    ${ODL_SYSTEM_${i+1}_IP}
    ${controller_index}    Get Active Controller
...
Setup SXP Cluster Bindings
    ...
    [Documentation]    Adds bindings to cluster nodes
    : FOR    ${i}    IN RANGE    1    25
...
Isolate SXP Controller
    ...
    \    Wait Until Keyword Succeeds    240    1    Sync_Status_Should_Be_True    ${controller_index+1}
    Wait Until Keyword Succeeds    60    1    Check Cluster is Connected    ${CLUSTER_NODE_ID}    mode=${cluster_mode}    session=controller${active_follower}
    Wait Until Keyword Succeeds    60    1    Check Device is Connected    ${DEVICE_NODE_ID}    ${VIRTUAL_IP}    ${peer_mode}
    ...    session=${DEVICE_SESSION}
...
Isolate SXP Controller With Bindings
    ...
    Wait Until Keyword Succeeds    60    1    Check Bindings    ${node}    ${session}
    Flush_Iptables_From_List_Or_All
    : FOR    ${controller_index}    IN RANGE    ${NUM_ODL_SYSTEM}
    \    Wait Until Keyword Succeeds    240    1    Sync_Status_Should_Be_True    ${controller_index+1}
    Wait Until Keyword Succeeds    60    1    Check Cluster is Connected    ${CLUSTER_NODE_ID}    mode=${cluster_mode}    session=controller${active_follower}
...
Check Device is Connected
    ...
    [Documentation]    Checks if SXP device is connected to cluster via virtual ip

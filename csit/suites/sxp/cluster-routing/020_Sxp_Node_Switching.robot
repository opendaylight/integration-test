...
Isolation Of SXP Service Follower W/O Bindings Listener Test
    ...
    UnIsolate SXP Controller    ${cluster_owner}    listener
...
Isolation Of SXP Service Follower W/O Bindings Speaker Test
    ...
    UnIsolate SXP Controller    ${cluster_owner}    speaker
...
Isolation Of SXP Service Follower Listener Test
    ...
    UnIsolate SXP Controller With Bindings    ${cluster_owner}    ${DEVICE_NODE_ID}    listener
...
Isolation Of SXP Service Follower Speaker Test
    ...
    UnIsolate SXP Controller With Bindings    ${cluster_owner}    ${CLUSTER_NODE_ID}    speaker
...
UnIsolate SXP Controller
    [Arguments]    ${controller_index}    ${peer_mode}
    [Documentation]    UnIsolate cluster-owner and perform check that Device is still connected
    ${cluster_mode} =    Sxp.Get Opposing Mode    ${peer_mode}
...
UnIsolate SXP Controller With Bindings
    [Arguments]    ${controller_index}    ${node}    ${peer_mode}
    [Documentation]    UnIsolate cluster owner and perform checks that bindings are propagated
    ${cluster_mode} =    Sxp.Get Opposing Mode    ${peer_mode}
    ClusterManagement.Flush_Iptables_From_List_Or_All
    BuiltIn.Wait Until Keyword Succeeds    240    1    ClusterManagement.Sync_Status_Should_Be_True    ${controller_index}

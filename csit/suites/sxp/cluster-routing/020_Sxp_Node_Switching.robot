...
Isolation Of SXP Service Follower W/O Bindings Listener Test
    ...
    Isolate SXP Controller    ${cluster_owner}    listener
    Un-Isolate SXP Controller    ${cluster_owner}    listener
...
Isolation Of SXP Service Follower W/O Bindings Speaker Test
    ...
    ${cluster_owner} =    SxpClusterLib.Get Owner Controller
    Isolate SXP Controller    ${cluster_owner}    speaker
    Un-Isolate SXP Controller    ${cluster_owner}    speaker
...
Isolation Of SXP Service Follower Listener Test
    ...
    Isolate SXP Controller With Bindings    ${cluster_owner}    ${DEVICE_NODE_ID}    listener
    Un-Isolate SXP Controller With Bindings    ${cluster_owner}    ${DEVICE_NODE_ID}    listener
...
Isolation Of SXP Service Follower Speaker Test
    ...
    Isolate SXP Controller With Bindings    ${cluster_owner}    ${CLUSTER_NODE_ID}    speaker
    Un-Isolate SXP Controller With Bindings    ${cluster_owner}    ${CLUSTER_NODE_ID}    speaker
...
Un-Isolate SXP Controller
    [Arguments]    ${controller_index}    ${peer_mode}
    [Documentation]    Un-Isolate cluster-owner and perform check that Device is still connected.
    ${cluster_mode} =    Sxp.Get Opposing Mode    ${peer_mode}
...
Isolate SXP Controller With Bindings
    ...
    BuiltIn.Wait Until Keyword Succeeds    60    1    Check Device is Connected    ${DEVICE_NODE_ID}    ${VIRTUAL_IP}    ${peer_mode}
    ...    session=${DEVICE_SESSION}
    BuiltIn.Wait Until Keyword Succeeds    30    1    Check Bindings    ${node}    session=ClusterManagement__session_${active_follower}

Un-Isolate SXP Controller With Bindings
    [Arguments]    ${controller_index}    ${node}    ${peer_mode}
    [Documentation]    Un-isolate cluster owner and perform checks that bindings are propagated.
    ${cluster_mode} =    Sxp.Get Opposing Mode    ${peer_mode}
    ClusterManagement.Flush_Iptables_From_List_Or_All
    BuiltIn.Wait Until Keyword Succeeds    240    1    ClusterManagement.Sync_Status_Should_Be_True    ${controller_index}
    BuiltIn.Wait Until Keyword Succeeds    60    1    SxpClusterLib.Check Cluster is Connected    ${CLUSTER_NODE_ID}    mode=${cluster_mode}    session=ClusterManagement__session_${controller_index}

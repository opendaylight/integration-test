...
Isolation Of SXP Service Follower Listener Test
    ...
    Check Connections And Bindings    ${cluster_owner}    ${DEVICE_NODE_ID}    listener
    ${active_follower} =    Isolate SXP Controller With Bindings    ${cluster_owner}    ${DEVICE_NODE_ID}    listener
    Check Connections And Bindings    ${active_follower}    ${DEVICE_NODE_ID}    listener
    [Teardown]    SxpClusterLib.Clean SXP Cluster
...
Isolation Of SXP Service Follower Speaker Test
    ...
    Check Connections And Bindings    ${cluster_owner}    ${CLUSTER_NODE_ID}    speaker
...
Check Connections And Bindings
    [Arguments]    ${controller_index}    ${node}    ${peer_mode}
    [Documentation]    Check that connection is established between device and the cluster and bindings are propagated
    ${cluster_mode} =    Sxp.Get Opposing Mode    ${peer_mode}
    BuiltIn.Wait Until Keyword Succeeds    60    1    SxpClusterLib.Check Cluster is Connected    ${CLUSTER_NODE_ID}    mode=${cluster_mode}    session=ClusterManagement__session_${active_follower}
    BuiltIn.Wait Until Keyword Succeeds    60    1    Check Device is Connected    ${DEVICE_NODE_ID}    ${VIRTUAL_IP}    ${peer_mode}
    ...    session=${DEVICE_SESSION}
    BuiltIn.Wait Until Keyword Succeeds    30    1    Check Bindings    ${node}    session=ClusterManagement__session_${controller_index}

...
Isolation of SXP service follower Test Listener Part
    ...
    Setup Custom SXP Cluster    ${VIRTUAL_IP}    listener    ${CLUSTER_NODE_ID}    controller${controller_index}
    : FOR    ${i}    IN RANGE    0    ${SAMPLES}
    \    ${controller_index}    Get Active Controller
    \    Isolate SXP Controller    ${controller_index}    ${DEVICE_NODE_ID}    listener    ${DEVICE_SESSION}

Isolation of SXP service follower Test Speaker Part
    [Documentation]    Test SXP binding propagation only if Controller with SCS is isolated
    Setup Custom SXP Cluster    ${VIRTUAL_IP}    speaker    ${DEVICE_NODE_ID}    ${DEVICE_SESSION}
    : FOR    ${i}    IN RANGE    0    ${SAMPLES}
    \    ${controller_index}    Get Active Controller
    \    Isolate SXP Controller    ${controller_index}    ${CLUSTER_NODE_ID}    speaker
...
Setup Custom SXP Cluster
    [Arguments]    ${peer_address}    ${peer_mode}    ${node}    ${session}

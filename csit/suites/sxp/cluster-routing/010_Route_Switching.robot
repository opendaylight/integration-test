...
Setup Custom SXP Cluster Session
    ...
    Issue Command On Karaf Console    log:set DEBUG org.opendaylight.sxp.route    ${ODL_SYSTEM_1_IP}
    Issue Command On Karaf Console    log:set DEBUG org.opendaylight.sxp.route    ${ODL_SYSTEM_2_IP}
    Issue Command On Karaf Console    log:set DEBUG org.opendaylight.sxp.route    ${ODL_SYSTEM_3_IP}
    ${mac_addresses}    Map Followers To Mac Addresses
    Set Suite Variable    ${MAC_ADDRESS_TABLE}    ${mac_addresses}
    ${controller_index}    Get Active Controller

    ${ifconfig}    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    ifconfig    ${ODL_SYSTEM_USER}    ${ODL_SYSTEM_PASSWORD}
    LOG    ${ifconfig}
    ${ifconfig}    Run Command On Remote System    ${ODL_SYSTEM_2_IP}    ifconfig    ${ODL_SYSTEM_USER}    ${ODL_SYSTEM_PASSWORD}
    LOG    ${ifconfig}
    ${ifconfig}    Run Command On Remote System    ${ODL_SYSTEM_3_IP}    ifconfig    ${ODL_SYSTEM_USER}    ${ODL_SYSTEM_PASSWORD}
    LOG    ${ifconfig}

    ${v_ip}    Generate Virtual Ip    ${ODL_SYSTEM_1_IP}
    Set Suite Variable    ${VIRTUAL_IP_1}    ${v_ip}
    Set Suite Variable    ${VIRTUAL_INTERFACE_1}    eth0:1

    ${v_ip}    Generate Virtual Ip    ${VIRTUAL_IP_1}
    Set Suite Variable    ${VIRTUAL_IP_2}    ${v_ip}
    Set Suite Variable    ${VIRTUAL_INTERFACE_2}    eth0:2

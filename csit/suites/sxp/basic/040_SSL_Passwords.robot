...
Setup SXP Environment Local
    ...
    Setup SXP Session
    ${rc}    Run And Return Rc    test -d ${ssl_stores} || mkdir -p ${ssl_stores}
    Should Be Equal As Integers    ${rc}    0
    ${rc}    ${output}    Run And Return Rc And Output    ${CURDIR}/../scripts/generate-keys.sh ${ssl_stores}
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
    : FOR    ${node}    IN RANGE    1    ${node_range}
    \    ${SSL}    Create Dictionary    truststore=${ssl_stores}/csit-truststore-${node}    keystore=${ssl_stores}/csit-keystore-${node}    password=admin123

...
Update Messages Test
    ...
    Wait Until Keyword Succeeds    15    1    Check Domain Sharing Removal
    : FOR    ${node}    IN RANGE    2    5
...
Local Binding Update Test
    ...
    Wait Until Keyword Succeeds    15    1    Check Domain Sharing Removal Local
...
Binding Replacement Test
    [Documentation]    Test if Node 7 contains only bindings from global domain
    [Tags]    SXP    Domains    Filtering
    Wait Until Keyword Succeeds    15    1    Check Initialized Removal
    Add Binding    400    35.35.35.35/32    127.0.0.4
    Add Binding    450    35.35.35.35/32    127.0.0.1    ${DOMAIN_3}
    Wait Until Keyword Succeeds    15    1    Check Removal Part One
    Add Filters Removal
    Wait Until Keyword Succeeds    15    1    Check Removal Part Two
    Delete Binding    450    35.35.35.35/32    127.0.0.1    ${DOMAIN_3}
    Wait Until Keyword Succeeds    15    1    Check Removal Part Three
...
Add Filters Removal
    ...
    ${domain_1_xml}    Add Domains    global
    ${domains}    Combine Strings    ${domain_1_xml}
    ${entry1}    Get Filter Entry    10    permit    ps=ge,300
    ${entries}    Combine Strings    ${entry1}
...
Check Domain Sharing Removal Local
    [Documentation]    TODo
    ${resp}    Get Bindings    127.0.0.5
    Should Not Contain Binding    ${resp}    20    20.20.20.5/32
    Should Contain Binding    ${resp}    20    20.20.5.5/32
...
Check Removal Part One
    [Documentation]    TODo
    ${resp}    Get Bindings    127.0.0.7
    Should Contain Binding    ${resp}    300    25.25.25.25/32
    Should Contain Binding    ${resp}    450    35.35.35.35/32

Check Removal Part Two
    [Documentation]    TODo
    ${resp}    Get Bindings    127.0.0.8
    Should Contain Binding    ${resp}    300    25.25.25.25/32
    Should Contain Binding    ${resp}    450    35.35.35.35/32

Check Removal Part Three
    [Documentation]    TODo
    ${resp}    Get Bindings    127.0.0.7
    Should Contain Binding    ${resp}    300    25.25.25.25/32

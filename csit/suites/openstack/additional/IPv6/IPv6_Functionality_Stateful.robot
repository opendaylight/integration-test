...
...               functionality in dhcpv6-stateful mode
...
${image}          fedora
${flavor}         fedora
@{IPv4_NETWORKS_NAME}    NET1_IPV4    NET2_IPV4    NET3_IPV4
@{IPv4_SUBNETS_NAME}    SUBNET1_IPV4    SUBNET2_IPV4    SUBNET3_IPV4
@{IPv4_SUBNETS_RANGE}    20.0.0.0/24    40.0.0.0/24    50.0.0.0/24
@{NET_1_VM_INSTANCES}    IPv4_VM1    IPv4_VM2
@{NET1_VM_INSTANCE}    IPv4_VM11    IPv4_VM22
@{Tcp_SG}         tcp_sg
@{port_Nub}       123    30    48
@{SG1}            ipv4_sg
${password}       cubswin:)
...
Create Secuirty rule for tcp
    ...
    Neutron Security Group Rule Create    @{Tcp_SG}[0]    direction=ingress    protocol=tcp
...
Create IPv6 Network1 Components
    ...
    Create SubNet    @{NETWORKS_IPV6}[1]    @{SUBNETS_NAME}[1]    @{SUBNETS_RANGE}[1]    ${net1_additional_args}
    ...
    Neutron Security Group Rule Create    @{sg_list}[0]    direction=ingress    protocol=tcp
    Create Fedora VM for dhcpv6-stateful    @{IPv4_NETWORKS_NAME}[1]    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[0]    sg=@{sg_list}[0]    image=fedora    flavor=fedora
    Create Fedora VM for dhcpv6-stateful    @{IPv4_NETWORKS_NAME}[1]    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[1]    sg=@{sg_list}[0]    image=fedora    flavor=fedora
    Create Fedora VM for dhcpv6-stateful    @{IPv4_NETWORKS_NAME}[1]    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[2]    sg=@{sg_list}[0]    image=fedora    flavor=fedora
...
Create IPv6 Network2 Components
    ...
    Add Router Interface    ${ROUTERS}    @{SUBNETS_NAME}[2]
    ${rc}    ${default_sgs}    Run And Return Rc And Output    openstack security group list --project admin | grep "default" | awk '{print $2}'
    ${sg_list}=    Split String    ${default_sgs}    \n
    ${length}=    Get Length    ${sg_list}
    Set Suite Variable    ${sg_list}
    ...
    Neutron Security Group Rule Create    @{sg_list}[0]    direction=ingress    protocol=tcp
    Neutron Security Group Rule Create    @{sg_list}[0]    direction=ingress    protocol=tcp    ethertype=IPv6
    Create Fedora VM for dhcpv6-stateful    @{IPv4_NETWORKS_NAME}[0]    @{NETWORKS_IPV6}[2]    @{NET2-VM}[0]    sg=@{sg_list}[0]    image=fedora    flavor=fedora
...
Create IPv4 Network Components
    ...
    Create Network    @{IPv4_NETWORKS_NAME}[2]
    Create SubNet    @{IPv4_NETWORKS_NAME}[2]    @{IPv4_SUBNETS_NAME}[2]    @{IPv4_SUBNETS_RANGE}[2]
    ${rc}    ${default_sgs}    Run And Return Rc And Output    openstack security group list --project admin | grep "default" | awk '{print $2}'
    ${sg_list}=    Split String    ${default_sgs}    \n
    ${length}=    Get Length    ${sg_list}
    Set Suite Variable    ${sg_list}
    Set Suite Variable    ${length}
    Create Vm Instances    @{IPv4_NETWORKS_NAME}[2]    ${NET_1_VM_INSTANCES}    image=cirros    flavor=cirros    sg=@{sg_list}[0]
...
Check Communication from dhcp
    ...
    ${src_ip}=    Collect Fedora_IPV6    @{NET1-VM}[1]
    ${net_id}=    Get Net Id    @{NETWORKS_IPV6}[0]
    ${output}=    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ping6 ${src_ip}    ]>
...
Check Basic Functionality between vm's in same node(ICMP-Communication)
    ...
    ${First_VM}    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${Second_VM}    Collect Fedora_IPV6    @{IPV6_VM}[2]
    ${dest_ip}    Create List    ${First_VM}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${First_VM}    ${dest_ip}
...
Check Basic Functionality between vm's in different node(ICMP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    ${des_ip_11}=    Create List    ${des_ip_1}
    ${des_ip_21}=    Create List    ${des_ip_2}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_1}    ${des_ip_21}
...
Check Basic Functionality across vm's in different networks(ICMP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{NET2-VM}[0]
    ${des_ip_11}=    Create List    ${des_ip_1}
    ${des_ip_21}=    Create List    ${des_ip_2}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[2]    ${des_ip_1}    ${des_ip_21}
...
Check Basic Functionality between vm's in same node(TCP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[0]

Check Basic Functionality between vm's in different node(TCP-Communication)
    [Documentation]    To Check TCP communication between VM's in the same network within the same node using default-sg
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[0]
...
Check Basic Functionality across vm's in different networks(TCP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{NET2-VM}[0]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{NETWORKS_IPV6}[2]    ${des_ip_1}    @{port_Nub}[0]
...
Check Basic Functionality between vm's in same node(UDP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[0]    additional_args=-u
...
Check Basic Functionality across vm's in different networks(UDP-Communication)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{NET2-VM}[0]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{NETWORKS_IPV6}[2]    ${des_ip_1}    @{port_Nub}[0]    additional_args=-u
...
Check SecurityGroup Functionality for ICMP within same network
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_11}=    Create List    ${des_ip_1}
    ${des_ip_21}=    Create List    ${des_ip_2}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[0]    ${des_ip_2}    ${des_ip_11}
...
Check SecurityGroup Functionality for ICMP across different networks
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{NET2-VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_11}=    Create List    ${des_ip_1}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[3]
...
Check SecurityGroup Functionality for UDP across different networks
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{NET2-VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[0]    ${des_ip_2}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[0]    additional_args=-u
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[1]
    ...    AND    Remove Security Group From VM    @{NET2-VM}[0]    @{SECURITY_GROUP}[2]
    ...    AND    Delete SecurityGroup    @{SECURITY_GROUP}[1]
    ...    AND    Delete SecurityGroup    @{SECURITY_GROUP}[2]
...
Check Remote-SG Functionality for ICMP protocol with three vm's
    ...
    ${des_ip_0}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    ${des_ip_11}=    Create List    ${des_ip_1}
    ${des_ip_21}=    Create List    ${des_ip_2}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    ${des_ip_11}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    ${des_ip_21}    ping_should_succeed=False
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[2]
...
Check Remote-SG Functionality for TCP protocol with three vm's
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[0]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[0]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[0]    nc_should_succeed=False
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[1]
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[1]    @{SECURITY_GROUP}[3]
...
Check Remote-SG Functionality for UDP protocol with three vm's
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[2]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[0]    additional_args=-u
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[0]    additional_args=-u
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[0]    additional_args=-u
...
Check SG Functionality for TCP protocol within a given range
    ...
    ${des_ip_0}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[1]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[2]    nc_should_succeed=False
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[0]
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[1]    @{SECURITY_GROUP}[0]
    ...    AND    Delete SecurityGroup    @{SECURITY_GROUP}[0]
...
Check SG Functionality for UDP protocol within a given range
    ...
    ${des_ip_0}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[1]    additional_args=-u
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_0}    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{port_Nub}[2]    additional_args=-u
    ...    nc_should_succeed=False
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[1]
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[1]    @{SECURITY_GROUP}[1]
    ...    AND    Delete SecurityGroup    @{SECURITY_GROUP}[1]
...
Dynamic protocol change (ICMP to TCP)
    ...
    ${des_ip_1}=    Collect Fedora_IPV6    @{IPV6_VM}[0]
    ${des_ip_2}=    Collect Fedora_IPV6    @{IPV6_VM}[1]
    ${des_ip_11}=    Create List    ${des_ip_1}
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}
    Delete All Security Group Rules    @{SECURITY_GROUP1}[2]
    ...
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_1}    @{NETWORKS_IPV6}[1]    ${des_ip_2}    @{port_Nub}[1]
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    ${des_ip_2}    ${des_ip_11}    ping_should_succeed=False
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[2]
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[1]    @{SECURITY_GROUP}[2]
    ...    AND    Delete SecurityGroup    @{SECURITY_GROUP}[2]
...
Dynamic protocol change (ANY to ICMP)
    ...
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[0]    @{IPV6_VM}[0]    ${des_ip_1}
    Delete All Security Group Rules    @{SECURITY_GROUP1}[3]
    ...
    Test Netcat Operations Between Fedora_IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[0]    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[1]    @{port_Nub}[1]
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[0]    ${des_ip_1}    ping_should_succeed=False
    [Teardown]    Run Keywords    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP}[3]
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[1]    @{SECURITY_GROUP}[3]
...
Create Vm with two NIC Cards
    ...
    ${net1_additional_args}=    Catenate    --ip-version=6 --ipv6-address-mode=dhcpv6-stateful --ipv6-ra-mode=dhcpv6-stateful
    Create SubNet    @{NETWORKS_IPV6}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]    ${net1_additional_args}
    Neutron Security Group Rule Create    @{sg_list}[0]    direction=ingress    port_range_max=65535    port_range_min=1    protocol=tcp
    ${VM1}=    Create List    @{NET1_VM_INSTANCE}[1]
    ${rc}    ${output}=    Run And Return Rc And Output    openstack server create --image fedora --flavor fedora --nic net-id=@{IPv4_NETWORKS_NAME}[2] --nic net-id=@{NETWORKS_IPV6}[0] --security-group @{sg_list}[0] --key-name vm_keys @{NET1_VM_INSTANCE}[1]
    : FOR    ${vm}    IN    @{VM1}
    \    Poll VM Is ACTIVE    ${vm}
    ${NET1_VM_IPS}    Collect IP    @{VM1}[0]
    Set Suite Variable    ${NET1_VM_IPS}
    Should Not Contain    ${NET1_VM_IPS}    None
    : FOR    ${vm}    IN    @{VM1}
    ...
    Log    ${net_id}
    Switch Connection    ${devstack_conn_id}
    ${output}=    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ssh -i ~/vm_key -o ConnectTimeout=10 -o StrictHostKeyChecking=no fedora@@{NET1_VM_IPS}[1] -o UserKnownHostsFile=/dev/null    $
    Log    ${output}
    ...
    Write    exit
    [Teardown]    Run Keywords    Delete Vm Instance    @{NET1_VM_INSTANCE}[1]
    ...    AND    Delete SubNet    @{SUBNETS_NAME}[0]
    ...    AND    Delete SubNet    @{IPv4_SUBNETS_NAME}[2]
    ...    AND    Delete Network    @{IPv4_NETWORKS_NAME}[2]
    ...    AND    Delete Network    @{NETWORKS_IPV6}[0]
...
Create IPv6 and IPv4 vm's with the same SG
    ...
    Add Security Group To VM    @{NET_1_VM_INSTANCES}[0]    @{Tcp_SG}[0]
    Add Security Group To VM    @{NET_1_VM_INSTANCES}[1]    @{Tcp_SG}[0]
    ...
    ${des_ip_1}=    Create List    @{IPV6_VM}[1]
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[0]    ${des_ip_1}
    ${des_ip_2}=    Create List    @{NET_1_VM_INSTANCES}[1]
    Test Operations From Fedora IPV6_Vm Instance    @{IPv4_NETWORKS_NAME}[0]    @{NET_1_VM_INSTANCES}[1]    ${des_ip_2}
    ...
    Test Operations From Fedora IPV6_Vm Instance    @{NETWORKS_IPV6}[1]    @{IPV6_VM}[0]    ${des_ip_1}
    ${des_ip_2}=    Create List    @{NET_1_VM_INSTANCES}[1]
    Test Operations From Fedora IPV6_Vm Instance    @{IPv4_NETWORKS_NAME}[0]    @{NET_1_VM_INSTANCES}[0]    ${des_ip_2}    ping_should_succeed=False
    ...
    ...    AND    Remove Security Group From VM    @{IPV6_VM}[0]    @{SECURITY_GROUP1}[5]0

...
*** Test Cases ***
Kill ovs-vswitchd in control node-No. 3
    [Documentation]    Kill ovsdb-server in control node
    #State transition of related component: No. 3
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Create Virtual LAN(No.3)
    ...
    ${stdout}=    Execute Command    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 |    awk '{print $3 $6 $7 $8}'    ${DEFAULT_LINUX_PROMPT_STRICT}
    Close Connection
    Log    ${stdout}
    ...
    SSHKeywords.Flexible SSH Login    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}
    SSHLibrary.Set Client Configuration    timeout=${default_devstack_prompt_timeout}
    ${flows}=    Execute Command    sudo ovs-ofctl dump-flows br-int -OOpenFlow13 |    awk '{print $3 $6 $7 $8}'    ${DEFAULT_LINUX_PROMPT_STRICT}
    Close Connection
    Log    ${flows}
...
Refer list of virtual LAN(No.3)
...
Delete virtual LAN(No.3)
    ...
    #Router operations(State transition of related component: No. 3)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 3)
...
Delete the security rule(No. 3 )
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No.8
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovsdb-server in control node-No. 8
    [Documentation]    Kill ovsdb-server in control node
    Kill ovsdb-server in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovsdb-server in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovsdb-server in control node    ${OS_CONTROL_NODE_3_IP}
    #Virtual LAN operations((DHCP: Enabled)Includes subnet settings)
...
Refer list of virtual LAN(No.8)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.8)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 8)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 8)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete the security rule(No. 8)
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No. 13
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovs-vswitchd in control node-No. 13
    [Documentation]    Kill ovsdb-server in control node
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Refer list of virtual LAN(No.13)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.13)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 13)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 13)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete the security rule(No. 13)
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No. 6
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovs-vswitchd in control node-No. 6
    [Documentation]    Kill ovsdb-server in control node
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Refer list of virtual LAN(No.6)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.6)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 6)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 6)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete the security rule(No. 6)
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No. 4
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovs-vswitchd in control node-No. 4
    [Documentation]    Kill ovsdb-server in control node
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Refer list of virtual LAN(No.4)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.4)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 4)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 4)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete the security rule(No. 4)
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No. 11
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovs-vswitchd in control node-No. 11
    [Documentation]    Kill ovsdb-server in control node
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Refer list of virtual LAN(No.11)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.11)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 11)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 11)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete the security rule(No. 11)
    ...
    ${DEL_ODL3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/security-rules
    Should Not Be Equal    ${ODL3}    ${DEL_ODL3}
    #State transition of related component: No. 9
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network

Kill ovs-vswitchd in control node-No. 9
    [Documentation]    Kill ovsdb-server in control node
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_1_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_2_IP}
    Kill ovs-vswitchd in control node    ${OS_CONTROL_NODE_3_IP}
...
Refer list of virtual LAN(No.9)
    [Documentation]    Refer list of virtual LANs.
    Create Network    @{NETWORKS_NAME}[0]
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[0]
    ${output}=    Show Network    @{NETWORKS_NAME}[0]
...
Delete virtual LAN(No.9)
    ...
    ${Output3}=    Write Commands Until Prompt    curl -v --user "admin":"admin" -H "Content-type: application/json" -X GET http://localhost:8181/restconf/config/neutron:neutron/
    Should Not Contain    ${Output3}    @{NETWORKS_NAME}[0]
    #Router operations(State transition of related component: No. 9)
    [Teardown]    Run Keywords    Get Test Teardown Debugs
    ...    AND    Clear L2_Network
...
Disconnect to external network(No. 9)
    [Documentation]    Check that you can disconnect to external network
    Create Network    @{NETWORKS_NAME}[0]    additional_args=--external --provider-network-type=flat --provider-physical-network=public
    Create SubNet    @{NETWORKS_NAME}[0]    @{SUBNETS_NAME}[0]    @{SUBNETS_RANGE}[2]    additional_args=--gateway 100.64.2.13 --allocation-pool start=100.64.2.18,end=100.64.2.248
    ${output}=    Show Network    @{NETWORKS_NAME}[0]

...
Wait For Routes To Propogate
    ...
    ${output} =    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ip -o link    ]>
    Log    ${output}
    ${output} =    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ip -o addr    ]>
    Log    ${output}

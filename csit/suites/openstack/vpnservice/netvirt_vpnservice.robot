...
@{VM_HOSTNAMES}    vm11    vm21    vm12    vm22
...
Test Setup
    ...
    IP Assignment test for Vm Instance    ${VM_HOSTNAMES}
...
IP Assignment test for Vm Instance
    [Arguments]       ${VM_HOSTNAMES}
    [Documentation]    check ip assignment for vm instance
    ${devstack_conn_id}=    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    : FOR    ${vm_name}    IN    @{VM_HOSTNAMES}
    \    ${output}=    Write Commands Until Prompt    nova console-log ${vm_name} | grep "debug start" -A10    90s
    \    Log    ${output}

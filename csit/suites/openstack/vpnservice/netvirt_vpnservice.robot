...
Test Case7-Disassociate the Network2 from VPN3 and validate the routes are updated and VMs between Network1 and Network2 are not reachable.
    Dissociate Network From VPN3    ${NETWORKS_VPN3[1]}    ${VPN_CONFIG_DIR}    ${networkid}    ${REST_CON}  
...
Test Case8-Create Router1 and associate 2 interfaces to the Router
    ...
    Add Interfaces To Router    @{SUBNETS}    ${ROUTERS[0]}    ${INTERFACE}    

Test Case9-Associate VPN1 and VPN2 to Router1 and validate that the VMs are pingable to each other. 
    Associtate VPN To Router    @{PRI_VPNIDS}    ${vpnid}   ${ROUTERS[0]}    ${VPN_CONFIG_DIR}    ${REST_CON}
...
Test Cleanup
    Delete Router Interfaces    @{SUBNETS}    ${ROUTERS[0]}    ${INTERFACE}
...
Dissociate Network From VPN3
    [Arguments]    ${NETWORKS_VPN3[1]}    ${VPN_CONFIG_DIR}    ${networkid}    ${REST_CON}  
...
Add Interfaces To Router
    [Arguments]    @{SUBNETS}    ${ROUTERS[0]}    ${INTERFACE}    
    [Documentation]    TC8-STEP2 Assocaite 2 interfaces to router
    : FOR    ${INTERFACE}    IN    @{SUBNETS}
    \    Add Router Interface    ${ROUTERS[0]}    ${INTERFACE}

Associtate VPN To Router
    [Arguments]    @{PRI_VPNIDS}    ${vpnid}   ${ROUTERS[0]}    ${VPN_CONFIG_DIR}    ${REST_CON}

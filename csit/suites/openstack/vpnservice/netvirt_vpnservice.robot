...
@{VM_HOSTNAMES_1}    vm11    vm12
@{VM_HOSTNAMES_2}    vm21    vm22
...
Test Initialize
    ...
    Add Ssh Allow Rule
    Create Vm Instances with netid and zone    ${NETWORKS[0]}    ${VM_INSTANCES[0]}    ${OS_COMPUTE_1_IP}    sg=csit
    Create Vm Instances with netid and zone    ${NETWORKS[0]}    ${VM_INSTANCES[1]}    ${OS_COMPUTE_2_IP}    sg=csit
    Create Vm Instances with netid and zone    ${NETWORKS[1]}    ${VM_INSTANCES[2]}    ${OS_COMPUTE_1_IP}    sg=csit
    Create Vm Instances with netid and zone    ${NETWORKS[1]}    ${VM_INSTANCES[3]}    ${OS_COMPUTE_2_IP}    sg=csit
    Ping Vm From DHCP Namespace    ${NETWORKS[0]}    ${NET_10_VM_IPS[0]}
    Ping Vm From DHCP Namespace    ${NETWORKS[0]}    ${NET_10_VM_IPS[1]}
    Ping Vm From DHCP Namespace    ${NETWORKS[1]}    ${NET_20_VM_IPS[0]}
    Ping Vm From DHCP Namespace    ${NETWORKS[1]}    ${NET_20_VM_IPS[1]}
    IP Assignment test for Vm Instance    ${VM_HOSTNAMES_1}
    IP Assignment test for Vm Instance    ${VM_HOSTNAMES_2}
    Connectivity Tests From Vm Instance In NETWORK    ${NETWORKS[0]}    ${NET_10_VM_IPS}
    Connectivity Tests From Vm Instance In NETWORK    ${NETWORKS[1]}    ${NET_20_VM_IPS}
...
Get ComputeNode Connection
    [Arguments]    ${OS_COMPUTE_NODE_IP}
    [Documentation]    Get compute node console connection
    ${compute_conn_id}=    SSHLibrary.Open Connection    ${OS_COMPUTE_NODE_IP}    prompt=${DEFAULT_LINUX_PROMPT_STRICT}
    Utils.Flexible SSH Login    ${OS_USER}    ${DEVSTACK_SYSTEM_PASSWORD}
    SSHLibrary.Set Client Configuration    timeout=30s
    Source Password    force=yes
    [Return]    ${compute_conn_id}

IP Assignment test for Vm Instance
    [Arguments]       ${VM_INSTANCES}
    [Documentation]    check ip assignment for vm instance and show iptables on compute node
    ${devstack_conn_id}=    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    : FOR    ${vm_name}    IN    @{VM_INSTANCES}
    \    ${output}=    Write Commands Until Prompt    nova console-log ${vm_name} | grep "debug start" -A10    300s
    \    Log    ${output}
    Write Commands Until Prompt    sudo iptables -S    30s

Connectivity Tests From Vm Instance In NETWORK
    [Arguments]       ${NETWORK}    ${NET_VM_IPS}
    [Documentation]    Login to the vm instance and check vm details
    ${dst_ip_list}=    Create List    @{NET_VM_IPS}[1]
    Log    ${dst_ip_list}
    Get OvsDebugInfo
    Test Operations From Vm Instance    ${NETWORK}    @{NET_VM_IPS}[0]    ${dst_ip_list}

Test Operations From Vm Instance
    [Arguments]    ${net_name}    ${src_ip}    ${list_of_local_dst_ips}    ${user}=cirros    ${password}=cubswin:)
...
Create Vm Instances with netid and zone
    [Arguments]    ${net_name}    ${vm_instance_name}    ${compute_node}    ${image}=cirros-0.3.4-x86_64-uec    ${flavor}=m1.nano    ${sg}=default
    [Documentation]    Create X Vm Instance with the net id of the Netowrk.
    ${devstack_conn_id}=    Get ControlNode Connection
    Switch Connection    ${devstack_conn_id}
    ${net_id}=    Get Net Id    ${net_name}    ${devstack_conn_id}
    ${hostname_compute_node}=    Run Command On Remote System    ${compute_node}    hostname
    ${output}=    Write Commands Until Prompt    nova boot --image ${image} --flavor ${flavor} --nic net-id=${net_id} ${vm_instance_name} --availability-zone nova:${hostname_compute_node} --security-groups ${sg}    30s
    Log    ${output}
...
Create Nova VMs
    ...
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[0]}    ${VM_INSTANCES[0]}    ${OS_COMPUTE_1_IP}    sg=csit
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[1]}    ${VM_INSTANCES[1]}    ${OS_COMPUTE_2_IP}    sg=csit
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[2]}    ${VM_INSTANCES[2]}    ${OS_COMPUTE_1_IP}    sg=csit
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[3]}    ${VM_INSTANCES[3]}    ${OS_COMPUTE_2_IP}    sg=csit

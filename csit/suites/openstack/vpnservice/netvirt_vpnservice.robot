...
Create Vm Instances with netid and zone
    [Arguments]    ${net_name}    ${vm_instance_names}    ${compute_node}    ${image}=cirros-0.3.4-x86_64-uec    ${flavor}=m1.nano    ${sg}=default
    ...
    \    ${output}=    Write Commands Until Prompt    nova boot --image ${image} --flavor ${flavor} --nic net-id=${net_id} ${VmElement} --availability-zone nova:${hostname_compute_node} --security-groups ${sg}    30s
    \    Log    ${output}

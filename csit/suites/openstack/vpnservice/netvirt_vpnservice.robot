...
@{NET_10_VM_IPS}    30.1.1.3
@{NET_20_VM_IPS}    40.1.1.3
...
Create Neutron Networks
    [Documentation]    Pretest-Create two networks
    Create Network    ${NETWORKS[0]}    --provider:network_type local
    Create Network    ${NETWORKS[1]}    --provider:network_type local
    List Networks

Create Neutron Subnets
    [Documentation]    Pretest-Create two subnets for previously created networks
    Create SubNet    ${NETWORKS[0]}    ${SUBNETS[0]}    ${SUBNET_CIDR[0]}
    Create SubNet    ${NETWORKS[1]}    ${SUBNETS[1]}    ${SUBNET_CIDR[1]}
    List Subnets

Create Neutron Ports
    [Documentation]    Pretest-Create four ports under previously created subnets
...
Check OpenDaylight Neutron Ports
    [Documentation]    Pretest-Checking OpenDaylight Neutron API for known ports
    ${resp}    RequestsLibrary.Get Request    session    ${NEUTRON_PORTS_API}
    Log    ${resp.content}
    Should be Equal As Strings    ${resp.status_code}    200

Create Nova VMs
    [Documentation]    Pretest-Boot VM instances on previously created network ports
...
Create VPN Instances
    [Documentation]    TC1-Creates VPN1 and VPN2 Instance through restconf
    [Tags]    Post
	${tenantid}=    Get Tenant ID From Nova List    ${VM_INSTANCES[0]}
...
Associate Networks To L3VPN
    [Documentation]    TC1-Associate Networks To L3VPN
    [Tags]    Post
...
Check VM Accessibility Across VPNs Before Import_Export
    [Documentation]    TC1-VM accessibility check Across the VPNs without import/export
    Check Ping From VM    ${NETWORKS[0]}    ${NET_10_VM_IPS[0]}    ${NET_20_VM_IPS[0]}    ${ping_fail}
	Log    Failure due to bug 6400

Exchange VPN Routes Through Import_Export
    [Documentation]   TC2-Import/Export Routes between VPN1 and VPN2
    [Tags]    Post
	${tenantid}=    Get Tenant ID From Nova List    ${VM_INSTANCES[0]}
...
Check VM accessibility across VPNs within DPN1
    [Documentation]    TC2-VM accessibility check Across the VPNs within DPN1
...
Check VM accessibility across VPNs within DPN2
    [Documentation]    TC2-VM accessibility check Across the VPNs within DPN2
...
Check VM Accessibility Between VPN On Other DPN
    [Documentation]    TC3-VM accessibility check VPN2 can access VPN1 on other DPN
...
Delete Neutron Port from VPN
   [Documentation]  TC5- Delete a Neutron port in VPN1 
   Delete Port    ${PORT_LIST[0]}

Check VPN Route After Neutron Port Delete
    [Documentation]   TC5-Validate that the route for deleted port is withdrawn from VPN1 and VPN2 
    [Tags]    exclude
    Log    This test will be added in the next patch
	
Create Neutron Networks for VPN3
    [Documentation]    TC6-Create two networks for VPN3
...
Create Neutron Subnets for VPN3
    [Documentation]    TC6-Create two subnets for previously created networks for VPN3
...
Create Neutron Ports for VPN3
    [Documentation]    TC6-Create four ports under previously created subnets for VPN3
...
Check OpenDaylight Neutron Ports for VPN3
    [Documentation]    TC6-Checking OpenDaylight Neutron API for known ports for VPN3
    ${resp}    RequestsLibrary.Get Request    session    ${NEUTRON_PORTS_API}
    Log    ${resp.content}
    Should be Equal As Strings    ${resp.status_code}    200

Create Nova VMs VPN3
    [Documentation]    TC6-Boot VM instances on previously created network ports for VPN3
...
Create L3VPN for VPN3
    ...
    [Tags]    Post
	${tenantid}=    Get Tenant ID From Nova List    ${VM_INSTANCES_VPN3[0]}

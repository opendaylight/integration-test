...
${EXT_RT2}        destination=50.1.1.0/24,nexthop=20.1.1.3    #TODO    PUT in array
...
Check ELAN Datapath Traffic Within The Networks
    ...
    ${output}=    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id} ssh ${user}@${NET10_VM_IPS[0]} -o ConnectTimeout=10 -o StrictHostKeyChecking=no    d:
    Log    ${output}
...
Add Multiple Extra Routes and check Data path before L3VPN Creation
    ...
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[0]    ifconfig eth0:1

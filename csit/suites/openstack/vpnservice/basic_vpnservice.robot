...
SSH Vm Instance13 From Network_1
    [Documentation]    Check reachability of vm instances by pinging to them.
    Get OvsDebugInfo
    SSH Namespace Network From Stack    ${NETWORKS[0]}    10.1.1.5

SSH Vm Instance1 From Network_1
    [Documentation]    Check reachability of vm instances by pinging to them.
    Get OvsDebugInfo
    SSH Namespace Network From Stack    ${NETWORKS[0]}    @{NET_1_VM_IPS}[0]

SSH Vm Instance1 From Network_2
    [Documentation]    Check reachability of vm instances by pinging to them.
    Get OvsDebugInfo
    SSH Namespace Network From Stack    ${NETWORKS[1]}    @{NET_2_VM_IPS}[0]
...
SSH Namespace Network From Stack
    ...
    ${output}=    Run Command On Remote System    ${OS_CONTROL_NODE_IP}    sudo ip netns exec qdhcp-${net_id} ssh ${user}@${src_ip} -o ConnectTimeout=10 -o StrictHostKeyChecking=no
    Log    ${output}
...
Get DPID For Compute Node
    ...
    ${output}=    Run Command On Remote System    ${ip}    sudo ovs-ofctl show -O Openflow13 br-int | head -1 | awk -F "dpid:" '{print $2}'    user=stack    prompt=$
    Log    ${output}
...
Get Ethernet Adapter From Compute Node
    ...
    ${adapter}=    Run Command On Remote System    ${ip}    /usr/sbin/ip addr show    user=stack    prompt=$
    Log    ${adapter}
    ${adapter}=    Run Command On Remote System    ${ip}    /usr/sbin/ip addr show | grep ${ip} | cut -d " " -f 11    user=stack    prompt=$
    [Return]    ${adapter}

...
Get Ethernet Adapter From Compute Node
    ...
    ${adapter}=    Run Command On Remote System    ${ip}    ip addr show    user=stack    prompt=$
    Log    ${adapter}
    ${adapter}=    Run Command On Remote System    ${ip}    ip addr show | grep ${ip} | cut -d " " -f 11    user=stack    prompt=$
    [Return]    ${adapter}

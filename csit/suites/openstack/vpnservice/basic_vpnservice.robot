...
Check ELAN Datapath Traffic Within The Networks
    ...
    Log    ${resp.content}
    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}
    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}
...
Verify L3 Dapath with Router
    ...
    # TODO FIB validation and flowtable Validation
    ${resp} =    RequestsLibrary.Get Request    session    /restconf/config/odl-fib:fibEntries/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${NET10_VM_IPS}
    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${NET20_VM_IPS}
    #    Datapath test across the networks using router for L3.
    ...
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}
...
Verify L3VPN Datapath with Router association
    ...
    Wait Until Keyword Succeeds    5s    1s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}
...
Delete Router Interfaces
    ...
    ${router_list} =    Create List    ${ROUTERS[0]}
    Wait Until Keyword Succeeds    3s    1s    Check For Elements Not At URI    ${NEUTRON_ROUTERS_API}    ${router_list}
    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}
    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}

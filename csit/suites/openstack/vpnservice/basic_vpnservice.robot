...
Verify L3VPN Datapath With Router Association
    ...
    ${vm_instances} =    Create List    @{NET10_VM_IPS}    @{NET20_VM_IPS}
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    /restconf/config/l3vpn:vpn-interfaces/    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}
...
Verify L3VPN datapath with Networks Association
    ...
    ${vm_instances} =    Create List    @{NET10_VM_IPS}    @{NET20_VM_IPS}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}
    Wait Until Keyword Succeeds    30s    5s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${vm_instances}
    Get OvsDebugInfo
    Log    Check datapath from network1 to network2
    ${dst_ip_list} =    Create List    @{NET10_VM_IPS}[1]    @{NET20_VM_IPS}
...
Dissociate L3VPN From Networks
    ...
    Log    "Verify FIB entries"
    ${vm_instances} =    Create List    @{NET10_VM_IPS}    @{NET20_VM_IPS}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements Not At URI    /restconf/config/odl-fib:fibEntries/    ${vm_instances}
...
Delete L3VPN with unknowID
    ...
    ${resp} =    RequestsLibrary.Post Request    session    ${REST_CON_OP}/neutronvpn:deleteL3VPN    ${json_body}
    Log    ${resp.content}

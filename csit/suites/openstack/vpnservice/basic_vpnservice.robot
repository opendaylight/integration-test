...
Check ELAN Datapath Traffic Within The Networks
    ...
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[0]    ping -c 3 @{NET1_VM_IPS}[1]
...
Check L3_Datapath Traffic Across Networks With Router
    ...
    Log    ${other_dst_ip_list}
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[0]    ping -c 3 @{NET2_VM_IPS}[0]
    #est Operations From Vm Instance    ${NETWORKS[0]}    @{NET10_VM_IPS}[0]    ${dst_ip_list}    l2_or_l3=l3    list_of_external_dst_ips=${other_dst_ip_list}
...
Add Multiple Extra Routes and check Data path before L3VPN Creation
    ...
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[0]    ifconfig
    ...
    Log    ${output}
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[1]    ping -c 3 @{NET10_VM_IPS}[0]
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET10_VM_IPS}[1]    ping -c 3 @{EXTRA_NW_IP}[0]

...
Create L3VPN
    ...
    ${tenant_id} =    Get Tenant ID From Network     ${NETWORKS[0]}
    Log    ${tenant_id}
...
Wait For Routes To Propogate On Node
    ${output}=    Run Command On Remote System    ${OS_CONTROL_NODE_IP}    ip route
    Log    ${output}
    Should Contain    ${output}    @{SUBNET_CIDR}[0]
    Should Contain    ${output}    @{SUBNET_CIDR}[1]
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ip route 
    Log    ${output}
    Should Contain    ${output}    @{SUBNET_CIDR}[0]
    Should Contain    ${output}    @{SUBNET_CIDR}[1]
    ${output}=    Run Command On Remote System    ${OS_COMPUTE_2_IP}    ip route
    Log    ${output}
    Should Contain    ${output}    @{SUBNET_CIDR}[0]
    Should Contain    ${output}    @{SUBNET_CIDR}[1]

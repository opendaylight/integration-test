...
Check ELAN Datapath Traffic Within The Networks
    ...
    Get Packet Count From FlowTable     ${OS_COMPUTE_1_IP}     ${ELAN_DMACTABLE}
    ...
    Should Contain    ${output}    ${PING_PASS}
    Get Packet Count From FlowTable     ${OS_COMPUTE_1_IP}     ${ELAN_DMACTABLE}
    Get Packet Count From FlowTable     ${OS_COMPUTE_2_IP}     ${ELAN_DMACTABLE}
...
Delete Router And Router Interfaces With L3VPN
    ...
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    120s    20s    Check For Elements Not At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances}
    Wait Until Keyword Succeeds    120s    20s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}
    Wait Until Keyword Succeeds    120s    20s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}
...
Verify L3VPN datapath with Networks Association
    ...
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    120s    20s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances} 
    Wait Until Keyword Succeeds    120s    20s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_1_IP}    ${vm_instances} 
    Wait Until Keyword Succeeds    120s    20s    Verify Flows Are Present For L3VPN    ${OS_COMPUTE_2_IP}    ${vm_instances} 
    Get OvsDebugInfo

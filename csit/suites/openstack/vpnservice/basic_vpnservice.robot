...
Delete Router And Router Interfaces With L3VPN
    ...
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    90s    10s    Check For Elements Not At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances}
    Wait Until Keyword Succeeds    60s    10s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}
    Wait Until Keyword Succeeds    60s    10s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}
...
Verify L3VPN datapath with Networks Association
    ...
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances} 
...
Dissociate L3VPN From Networks
    ...
    Log    "Verify FIB entries"
    ${vm_instances} =    Create List    @{VM_IP_NET10}    @{VM_IP_NET20}
    Wait Until Keyword Succeeds    60s    10s    Check For Elements Not At URI    ${CONFIG_API}/odl-fib:fibEntries/    ${vm_instances} 
    Wait Until Keyword Succeeds    60s    10s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_1_IP}
    Wait Until Keyword Succeeds    60s    10s    Verify Flows Are Present For ELAN    ${OS_COMPUTE_2_IP}

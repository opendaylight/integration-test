...
${ADD_ARG_SSH5}    --ingress --ethertype IPv4 --dst-port 20:25 --protocol tcp
@{ADD_PARAMS}     ingression    IPv4    20    25    tcp
${ADD_ARG_SSH6}    --ingress --ethertype IPv4 --dst-port 25:-1 --protocol tcp
${ADD_ARG_SSH7}    --ingress --ethertype IPv4 --dst-port -1:20 --protocol tcp
...
Neutron Rule Creation With Invalid Parameters
    ...
    ${rc}    ${output} =    OpenStackOperations.OpenStack CLI    openstack security group rule create ${additional_args} ${sg_name}
    BuiltIn.Log    ${output}

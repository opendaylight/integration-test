...
#    ${output} =    BuiltIn.Wait Until Keyword Succeeds    420s    10s    OpenStack CLI    openstack console log show @{NET_2_VMS}[1]
    ...
    ${flow_output1} =    Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    sudo ovs-ofctl -O OpenFlow13 dump-flows br-int
    ${flow_output2} =    Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    sudo ovs-ofctl -O OpenFlow13 dump-flows br-int

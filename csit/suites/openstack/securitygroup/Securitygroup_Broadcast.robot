...
Verify Subnet Broadcast traffic between the VMs hosted on same compute node in Single Network
    ...
    ${get_pkt_count_before_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP1_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET1_BCAST_IP}   ${VM2_SUBMETA}
    ...
    ${get_pkt_count_after_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP1_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET1_BCAST_IP}    ${VM2_SUBMETA}
    ${pkt_diff} =    Evaluate    int(${get_pkt_count_after_bcast})-int(${get_pkt_count_before_bcast})
    Should Be Equal As Numbers    ${pkt_diff}    ${PACKET_COUNT}
...
Verify Subnet Broadcast traffic between the VMs hosted on Different compute node in Single Network
    ...
    ${get_pkt_count_before_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP2_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET1_BCAST_IP}   ${VM3_SUBMETA}
    ${output} =    Wait Until Keyword Succeeds    180s    10s     OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c 5 ${SUBNET1_BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ...
    ${get_pkt_count_after_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP2_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET1_BCAST_IP}    ${VM3_SUBMETA}
    ${pkt_diff} =    Evaluate    int(${get_pkt_count_after_bcast})-int(${get_pkt_count_before_bcast})
    Should Be Equal As Numbers    ${pkt_diff}    ${PACKET_COUNT}
...
Verify Subnet Broadcast traffic between the VMs hosted on same compute node in Multi Network
    ...
    ${get_pkt_count_before_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP1_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET2_BCAST_IP}   ${VM4_SUBMETA}
    ...
    ${get_pkt_count_after_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP1_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET2_BCAST_IP}    ${VM4_SUBMETA}
    ${pkt_diff} =    Evaluate    int(${get_pkt_count_after_bcast})-int(${get_pkt_count_before_bcast})
...
Verify Subnet Broadcast traffic between the VMs hosted on Different compute node in Multi Network
    ...
    ${get_pkt_count_before_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP2_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET2_BCAST_IP}   ${VM5_SUBMETA}
    ${output} =    Wait Until Keyword Succeeds    180s    10s     OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c 5 ${SUBNET2_BCAST_IP}
    ...
    ${get_pkt_count_after_bcast} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    SubnetBcast Packetcount    ${OS_CMP2_CONN_ID}    ${br_name}    ${ACL_Anti_Spoofing_Table}    ${SUBNET2_BCAST_IP}    ${VM5_SUBMETA}
    ${pkt_diff} =    Evaluate    int(${get_pkt_count_after_bcast})-int(${get_pkt_count_before_bcast})
    Should Be True    ${pkt_diff}==0
...
#    ${output} =    BuiltIn.Wait Until Keyword Succeeds    420s    10s    OpenStack CLI    openstack console log show @{NET_2_VMS}[1]
    ...
    ${flow-compute1} =    Run Command On Remote System    ${OS_COMPUTE_1_IP}    ${DUMP_FLOW}
    ${flow-compute2} =    Run Command On Remote System    ${OS_COMPUTE_2_IP}    ${DUMP_FLOW}
...
SubnetBcast Packetcount
    ...
    Switch Connection    ${conn_id}
    ${output} =    OpenStackOperations.OpenStack CLI    sudo ovs-ofctl dump-flows -O Openflow13 ${br_name} | grep ${TABLE_NO} | grep ${SUBNET_BCAST_IP}| grep ${VM_SUBMETA}
    @{output_list} =    String.Split String    ${output}    \r\n

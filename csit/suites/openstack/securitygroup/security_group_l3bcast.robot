...
@{NET_1_VMS}      sg_net1_vm1    sg_net1_vm2    sg_net1_vm3
@{NET_2_VMS}      sg_net2_vm1    sg_net2_vm2
#${ACL_ANTI_SPOOFING_TABLE}    table=240 //need to add this in variable.robot
...
Verify Network Broadcast traffic between the VMs hosted on same compute node in Single Network
    [Documentation]    This TC is to verify Network Broadcast traffic between the VMs hosted on same compute node ... in same Network
    ...
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
...
Verify Network Broadcast traffic between the VMs hosted on Different compute node in Single Network
    [Documentation]    This TC is to verify Network Broadcast traffic between the VMs hosted on different compute ... node in same Network
    ${get_pkt_count_before_bcast} =    Bcast Packetcount    ${OS_COMPUTE2_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c ${PACKET_COUNT} ${BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
...
Verify Network Broadcast traffic between the VMs hosted on same compute node in Multi Network
    [Documentation]    This TC is to verify Network Broadcast traffic between the VMs hosted on same compute node ... in different Network
    ${get_pkt_count_before_bcast} =    Bcast Packetcount    ${OS_COMPUTE1_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c ${PACKET_COUNT} ${BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
...
Verify Network Broadcast traffic between the VMs hosted on Different compute node in Multi Network
    [Documentation]    This TC is to verify Network Broadcast traffic between the VMs hosted on different compute ... node in different Network
    ${get_pkt_count_before_bcast} =    Bcast Packetcount    ${OS_COMPUTE2_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c ${PACKET_COUNT} ${BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
...
Bcast Packetcount
    ...
    ${output} =    Run Command On Remote System And Log    ${OS_COMPUTE_IP}    ${DUMP_FLOW} | grep table=${TABLE_NO} | grep ${BCAST_IP}

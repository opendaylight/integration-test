...
Verify Network Broadcast traffic between the VMs hosted in Single Network
    ...
    Check L3Broadcast in Same Subnet    ${OS_COMPUTE1_IP}    ${ACL_ANTI_SPOOFING_TABLE}   ${BCAST_IP}
...
Verify Network Broadcast traffic between the VMs hosted in Multi Network
    ...
    Check L3Broadcast in Different Subnet    ${OS_COMPUTE1_IP}    ${ACL_ANTI_SPOOFING_TABLE}   ${BCAST_IP}
...
Create Setup
    ...
    @{VM_IPS}=    BuiltIn.Create List    VM1_NET1_DPN1_IP_Address    VM2_NET1_DPN1_IP_Address    VM3_NET1_DPN1_IP_Address    VM1_NET2_DPN1_IP_Address    VM2_NET2_DPN1_IP_Address
    : FOR    ${vm_ips}    ${vms}    IN ZIP    ${VM_IPS}    ${VMS}
    \    ${temp}    ${ips_and_console_log[1]} =    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Get VM IPs
    \    ...    ${vms}
    \    BuiltIn.Set Suite Variable    ${${vm_ips}}    ${temp}
    ${VM1_DPN1_Enable_Bcast} =    OpenStackOperations.Execute Command on VM Instance    ${NETWORKS[0]}    ${VM1_NET1_DPN1_IP_Address}    ${ENABLE_BCAST}
	${VM3_DPN1_Enable_Bcast} =    OpenStackOperations.Execute Command on VM Instance    ${NETWORKS[1]}    ${VM1_NET2_DPN1_IP_Address}    ${ENABLE_BCAST}
...
Check L3Broadcast in Same Subnet
    [Arguments]    ${OS_COMPUTE_IP}    ${ACL_ANTI_SPOOFING_TABLE}   ${BCAST_IP}
    [Documentation]    Verify the l3 broadcast requests are hitting to antispoofing table in same subnet     
    ${get_pkt_count_before_bcast} =     Get Broadcast Packet Count    ${OS_COMPUTE_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET1_DPN1_IP_Address}    ping -c ${PACKET_COUNT} ${BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
    ${get_pkt_count_after_bcast} =     Get Broadcast Packet Count    ${OS_COMPUTE_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
...
Check L3Broadcast in Different Subnet
    ...
    ${get_pkt_count_before_bcast} =    Get Broadcast Packet Count    ${OS_COMPUTE_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${VM1_NET2_DPN1_IP_Address}    ping -c ${PACKET_COUNT} ${BCAST_IP}
    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}
    ${bcast_egress} =    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_IP}    ${DUMP_FLOW} | grep table=${ACL_ANTI_SPOOFING_TABLE} | grep ${BCAST_IP}
    ${get_pkt_count_after_bcast} =    Get Broadcast Packet Count    ${OS_COMPUTE_IP}    ${ACL_ANTI_SPOOFING_TABLE}    ${BCAST_IP}
    ${pkt_diff} =    Evaluate    int(${get_pkt_count_after_bcast})-int(${get_pkt_count_before_bcast})

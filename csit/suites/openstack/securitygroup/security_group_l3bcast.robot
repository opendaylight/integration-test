...
Verify Network Broadcast traffic between the VMs hosted in Single Network
    ...
    Wait Until Keyword Succeeds    30s    5s    Verify L3Broadcast With Antispoofing Table    ${OS_COMPUTE_IP}    ${EGRESS_ACL_TABLE}    ${BCAST_IP}
    ...    @{VM_IPS}[0]    same
...
Verify L3Broadcast With Antispoofing Table
    ...
    ${get_pkt_count_before_bcast} =    Get Packet Count In Table For IP    ${OS_COMPUTE_IP}    ${EGRESS_ACL_TABLE}    ${BCAST_IP}
    ...
    ${get_pkt_count_after_bcast} =    Get Packet Count In Table For IP    ${OS_COMPUTE_IP}    ${EGRESS_ACL_TABLE}    ${BCAST_IP}

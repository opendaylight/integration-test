...
Verify Subnet Broadcast traffic between the VMs hosted on same compute node in Multi Network
    ...
    ...    @{VM_IPS}[0]    different    ${VM4_SUBMETA}    noping
...
Verify Subnet Broadcast traffic between the VMs hosted on Different compute node in Multi Network
    ...
    ...    @{VM_IPS}[0]    different    ${VM5_SUBMETA}    noping
...
Verify L3Broadcast With Antispoofing Table
    [Arguments]    ${OS_COMPUTE_IP}    ${EGRESS_ACL_TABLE}    ${BCAST_IP}    ${vm_ip}    ${subnet_var}    ${vm_submeta}=''    ${ping_response}
    ...
    BuiltIn.Run Keyword If    '${ping_response}'=='noping'    BuiltIn.Should Contain    ${output}    ${NO_PING_REGEXP}
    ...    ELSE    BuiltIn.Should Contain    ${output}    ${PING_REGEXP}

...
TC_16_Verify acl flow is is getting created for Remote Ip using remote group id rules in Custom SG
    ...
    [Tags]    Regression
    ${ingress_icmprule_flow}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[1]| grep @{PROTOCOL_NAME}[0]
    log    ${ingress_icmprule_flow}
    Should Contain    ${ingress_icmprule_flow}    icmp
    ${egress_icmprule_flow}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[0]| grep @{PROTOCOL_NAME}[0]
    log    ${egress_icmprule_flow}
    Should Contain    ${egress_icmprule_flow}    icmp
    ${Remote_ACL_Tables_VM1_Egress}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep @{ACL_Remote_ACL_Tables}[0] | grep nw_dst=
    log    ${Remote_ACL_Tables_VM1_Egress}
    Should Contain    ${Remote_ACL_Tables_VM1_Egress}    ${vm2_ip_dpn2}
    ${Remote_ACL_Tables_VM1_Ingress}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_1_IP}    ${dump_flows} | grep @{ACL_Remote_ACL_Tables}[1] | grep nw_src=
    log    ${Remote_ACL_Tables_VM1_Ingress}
    Should Contain    ${Remote_ACL_Tables_VM1_Ingress}   ${vm2_ip_dpn2}
    log   Compute node Login and validation
    ${ingress_tcprule_flow}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[1]| grep @{PROTOCOL_NAME}[1]
    log    ${ingress_tcprule_flow}
    Should Contain    ${ingress_tcprule_flow}    tcp
    ${egress_tcprule_flow}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[0]| grep @{PROTOCOL_NAME}[1]
    log   ${egress_tcprule_flow}
    Should Contain    ${egress_tcprule_flow}    tcp
    ${Remote_ACL_Tables_VM2_Egress}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}    ${dump_flows} |grep @{ACL_Remote_ACL_Tables}[0] | grep nw_dst=
    log    ${Remote_ACL_Tables_VM2_Egress}
    Should Contain    ${Remote_ACL_Tables_VM2_Egress}    ${vm1_ip_dpn1}
    ${Remote_ACL_Tables_VM2_Ingress}    Utils.Run Command On Remote System And Log    ${OS_COMPUTE_2_IP}     ${dump_flows} |grep @{ACL_Remote_ACL_Tables}[1] | grep nw_src=
    log     ${Remote_ACL_Tables_VM2_Ingress}

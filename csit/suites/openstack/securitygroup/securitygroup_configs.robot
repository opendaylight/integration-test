...
TC_16_Verify acl flow is is getting created for Remote Ip using remote group id rules in Custom SG
    ...
    [Tags]    Regression
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IPs     ${OS_CNTL_CONN_ID}    ${DPN1_VM_NAMES[0]}
    ${VM1_IP}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IPs     ${OS_CMP1_CONN_ID}    ${DPN2_VM_NAMES[0]}
    log    >>>> Verify the Flows for TCp and ICMP
    ...
    Should Contain    ${Remote_ACL_Tables_VM1_Egress}    ${VM2_IP}
    Should Contain    ${Remote_ACL_Tables_VM1_Ingress}    ${VM2_IP}
    log   Compute node Login and validation
    ${OS_CMP1_CONN_ID}    SSHLibrary.Open Connection    ${OS_COMPUTE_2_IP}
    SSHLibrary.Login    stack    stack
#   SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    ${ingress_tcprule_flow}     BuiltIn.Wait Until Keyword Succeeds    30s    10s    Write Commands Until Prompt And Log      ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[1]| grep @{PROTOCOL_NAME}[1]
    ${egress_tcprule_flow}     BuiltIn.Wait Until Keyword Succeeds    30s    10s    Write Commands Until Prompt And Log      ${dump_flows} | grep @{ACL_Rule_Based_Filter_Tables}[0]| grep @{PROTOCOL_NAME}[0]
    Should Contain    ${ingress_tcprule_flow}    tcp
    Should Contain    ${egress_tcprule_flow}    tcp
    log    >>>> Table validation for remote Ip table 216/246
    ${Remote_ACL_Tables_VM2_Egress}    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Write Commands Until Prompt And Log      ${dump_flows} | grep @{ACL_Remote_ACL_Tables}[0] | grep nw_dst=
    ${Remote_ACL_Tables_VM2_Ingress}    BuiltIn.Wait Until Keyword Succeeds    30s    10s    Write Commands Until Prompt And Log     ${dump_flows} | grep @{ACL_Remote_ACL_Tables}[1] | grep nw_src=
    Should Contain    ${Remote_ACL_Tables_VM2_Egress}    ${VM1_IP}
    Should Contain    ${Remote_ACL_Tables_VM2_Ingress}    ${VM1_IP}
...
Create Setup
    ...
    OpenStackOperations.Neutron Security Group Rule Create    @{SECURITY_GROUP}[0]    direction=ingress     protocol=icmp     remote_group_id=@{SECURITY_GROUP}[0]
    OpenStackOperations.Neutron Security Group Rule Create    @{SECURITY_GROUP}[0]    direction=egress      protocol=icmp     remote_group_id=@{SECURITY_GROUP}[0]

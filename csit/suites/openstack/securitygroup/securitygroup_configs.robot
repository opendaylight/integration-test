...
TC_16_Verify acl flow is is getting created for Remote Ip using remote group id rules in Custom SG
    ...
    Should Contain    ${Remote_ACL_Tables_VM1_Egress}    @{vm2_ip_dpn2}[0]
    Should Contain    ${Remote_ACL_Tables_VM1_Ingress}   @{vm2_ip_dpn2}[0]
    ...
    Should Contain    ${Remote_ACL_Tables_VM2_Egress}    @{vm1_ip_dpn1}[0]
    Should Contain    ${Remote_ACL_Tables_VM2_Ingress}   @{vm1_ip_dpn1}[0]
...
Create Setup
    ...
    ${vm2_ip_dpn2}    Wait Until Keyword Succeeds    240s    10s    OpenStackOperations.Get VM IP    true    @{DPN2_VM_NAMES}[0]

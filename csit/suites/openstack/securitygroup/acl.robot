...
TC3_Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    ...
    Sleep    210
    @{VM_IP1}    ${dhcp_ip1}    @{VM_IP_DPN1}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${VM_NAMES[0]}
    @{VM_IP2}    ${dhcp_ip2}    @{VM_IP_DPN2}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${VM_NAMES[1]}
    @{VM_IP3}    ${dhcp_ip3}    @{VM_IP_DPN3}    Wait Until Keyword Succeeds    240 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${VM_NAMES[2]}
...
Create Neutron Ports
    ...
    Create Port    ${REQ_NETWORKS[0]}    ${PORT_LIST[0]}    sg=${SECURITY_GROUP[0]}
    Create Port    ${REQ_NETWORKS[1]}    ${PORT_LIST[1]}    sg=${SECURITY_GROUP[0]}
    Create Port    ${REQ_NETWORKS[0]}    ${PORT_LIST[2]}    sg=${SECURITY_GROUP[0]}
    Create Port    ${REQ_NETWORKS[1]}    ${PORT_LIST[3]}    sg=${SECURITY_GROUP[0]}
    Create Port    ${REQ_NETWORKS[0]}    ${PORT_LIST[4]}    sg=${SECURITY_GROUP[0]}
    Create Port    ${REQ_NETWORKS[1]}    ${PORT_LIST[5]}    sg=${SECURITY_GROUP[0]}

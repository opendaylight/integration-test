...
TC1_Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    [Documentation]    Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    Create Setup
    ...
    Create Vm Instance With Ports    ${PORTS[4]}    ${PORTS[5]}    ${VM_NAMES[2]}    ${OS_CMP1_HOSTNAME}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]} 
    @{VM_IP_DPN1}    Wait Until Keyword Succeeds    440 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${VM_NAMES[0]}
    @{VM_IP_DPN2}    Wait Until Keyword Succeeds    440 sec    60 sec    Get VM IP Address    ${OS_CMP2_CONN_ID}    ${VM_NAMES[1]}
    @{VM_IP_DPN3}    Wait Until Keyword Succeeds    440 sec    60 sec    Get VM IP Address    ${OS_CMP1_CONN_ID}    ${VM_NAMES[2]}
...
TC2_Verify ARP request generated from Spoofed IP for the VM
    ...
    ${get_pkt_count_before_arp}     Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm1_metadata}|grep arp_sha
    ${get_arp_drop_pkt_before}    Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
    ${cmd}    Set Variable    sudo arping -s ${SPOOF[0]} -c ${PACKET_COUNT} \ ${RANDOM_IP}
    ${output}    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN1[1]}    ${cmd}
    ${ovs1_output}    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ${get_pkt_count_after_arp}     Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm1_metadata}|grep arp_sha
    ${get_arp_drop_pkt_after}    Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
...
TC3_Verify ARP request generated from Spoofed MAC for the VM
    ...
    ${get_arp_drop_pkt_before}    Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
    ${cmd}    Set Variable    sudo arping -I eth0 -c ${PACKET_COUNT} \ ${RANDOM_IP}
    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN1[1]}    ${cmd}
    ${ovs1_output}    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ${get_pkt_count_after_arp}     Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm1_metadata}|grep arp_sha
    ${get_arp_drop_pkt_after}    Get Packetcount    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
...
TC4_Verify ARP request generated from Spoofed IP and spoofed MAC for the VM
...
TC5_Verify Ping - Valid MAC and Valid IP for the VM by clearing the ARP Cache
    [Documentation]    Verify Ping - Valid MAC and Valid IP for the VM by clearing the ARP Cache
    Switch Connection    ${OS_CMP2_CONN_ID}
...
TC6_Verify IPv4 unicast traffic with MAC Address and IP Address (Valid MAC + Valid IP)
    [Documentation]    Verify IPv4 unicast traffic with MAC Address and IP Address (Valid MAC + Valid IP)
    Switch Connection    ${OS_CMP2_CONN_ID}
    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    Capture Flows    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp_nw_src}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep nw_src
    ${get_pkt_count_before_arp}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep arp_sha
    ${cmd}    Set Variable    ping -c 5 ${VM_IP_DPN3[0]} -I eth0
    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN2[1]}    ${cmd}
    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    Capture Flows    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ...
    ${get_pkt_count_after_arp}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep arp_sha
    ${pkt_diff}    Evaluate    int(${get_pkt_count_after_arp_nw_src})-int(${get_pkt_count_before_arp_nw_src})
    Should Be True    ${pkt_diff} > ${PACKET_COUNT_ZERO}
    ${pkt_diff_arp_sha}    Evaluate    int(${get_pkt_count_before_arp})-int(${get_pkt_count_after_arp})
    Should Not Be Equal As Numbers    ${pkt_diff_arp_sha}    ${PACKET_COUNT_ZERO}

TC7_Verify Ping - Spoofed IP and Valid Mac for the VM by clearing the ARP cache
    [Documentation]    Verify Ping - Spoofed IP and Valid Mac for the VM by clearing the ARP cache
    Switch Connection    ${OS_CMP2_CONN_ID}
    ${cmd}    Set Variable    sudo arp -a
    ${output}    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN2[1]}    ${cmd}
    ...
    ${get_pkt_count_before_arp_nw_src}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep nw_src
    ${get_pkt_count_before_arp}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep arp_sha
    ${get_pkt_drop_before_arp}    Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
    ${cmd}    Set Variable    sudo arp -a
    ${output}    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN2[1]}    ${cmd}
    ...
    ${get_pkt_count_after_arp_nw_src}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep nw_src
    ${get_pkt_count_after_arp}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep arp_sha
    ${get_pkt_drop_after_arp}    Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
    ${pkt_diff}    Evaluate    int(${get_pkt_count_after_arp_nw_src})-int(${get_pkt_count_before_arp_nw_src})
...
TC8_Verify IPv4 unicast traffic by SPOOFing IP Address (Invalid IP + Valid MAC)
    [Documentation]    Verify IPv4 unicast traffic by SPOOFing IP Address (Invalid IP + Valid MAC)
    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    Capture Flows    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp_nw_src}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep nw_src
    ${get_pkt_drop_before_arp}    Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    arp|grep goto_table:217
...
TC9_Verify Ping - Vaild IP and Spoofed Mac for the VM by clearing the ARP cache
    [Documentation]    Verify Ping - Vaild IP and Spoofed Mac for the VM by clearing the ARP cache
    Switch Connection    ${OS_CMP2_CONN_ID}
    ${cmd}    Set Variable    sudo arp -a
    ${output}    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
    ...    ${VM_IP_DPN2[1]}    ${cmd}
    ...
    : FOR    ${item}    IN    @{arp_ip}
    \    ${cmd}    Set Variable    sudo arp -d ${item}
    \    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}    ${VM_IP_DPN2[1]}    ${cmd}
    \    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}    ${VM_IP_DPN2[1]}    ${cmd}
    \    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}    ${VM_IP_DPN2[1]}    ${cmd}
    ${cmd}    Set Variable    sudo ifconfig eth0 down
    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[1]}
...
TC10_Verify IPv4 unicast traffic by SPOOFing MAC Address (Invalid MAC + Valid IP)
    [Documentation]    Verify IPv4 unicast traffic by SPOOFing MAC Address (Invalid MAC + Valid IP)
...
TC11_Verify Ping - Spoofed IP and Spoofed Mac for the VM by clearing the ARP cache
    [Documentation]    Verify Ping - Spoofed IP and Spoofed Mac for the VM by clearing the ARP cache
...
TC12_Verify IPv4 unicast traffic by Spoofing MAC Address and IP Address After port security enabled false
    [Documentation]    Verify IPv4 unicast traffic by Spoofing MAC Address and IP Address After port security enabled false
    ${FLOW_DUMP_CMD}    Set Variable    sudo ovs-ofctl dump-flows -O Openflow13 br-int
    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    Capture Flows    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp_nw_src}     Get Packetcount    ${OS_CMP2_CONN_ID}    ${BR_NAME}    ${TABLE_NO}    ${vm2_metadata}|grep nw_src

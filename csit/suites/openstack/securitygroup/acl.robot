...
@{AVAILIBITY_ZONE}    ${OS_CMP1_HOSTNAME}    ${OS_CMP2_HOSTNAME}
...
TC1_Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    Create Setup
    Create Vm Instance With Ports    ${PORTS[0]}    ${PORTS[1]}    ${VM_NAMES[0]}    ${AVAILIBITY_ZONE[0]}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]}
    Create Vm Instance With Ports    ${PORTS[2]}    ${PORTS[3]}    ${VM_NAMES[1]}    ${AVAILIBITY_ZONE[1]}    flavor=m1.tiny    sg=${SECURITY_GROUP[0]}
    ...
    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${REQ_NETWORKS[0]}    ${VM_IP_DPN1[1]}    ${DHCP_CMD}

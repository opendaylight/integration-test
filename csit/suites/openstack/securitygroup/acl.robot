...
Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Execute Command on VM Instance    @{REQ_NETWORKS}[0]    @{VM_IP_DPN3}[0]    ${DHCP_CMD}
    ...
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.Execute Command on VM Instance    ${REQ_NETWORKS[1]}    @{VM_IP_DPN1}[1]    ${cmd}
    Capture Flows    ${OS_CMP1_CONN_ID}    ${BR_NAME}    ${FLOW_DUMP_CMD}
...
Create Setup
    ...
    OpenStackOperations.Create Port    ${REQ_NETWORKS[0]}    @{PORTS}[4]    sg=@{SECURITY_GROUP}[0]
    OpenStackOperations.Create Port    ${REQ_NETWORKS[1]}    @{PORTS}[5]    sg=@{SECURITY_GROUP}[0]
...
Get Two Port VM IP Address
    ...
    SSHLibrary.SSHLibrary.Switch Connection    ${conn_id}

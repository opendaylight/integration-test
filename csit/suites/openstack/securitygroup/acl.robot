...
Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    ...
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_after_arp}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_2_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    |grep ${VM1_METADATA}|grep arp_sha
...
Verify ARP request generated from Spoofed IP for the VM
    ...
    ...    ${arp_int_up_cli}
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp}    OvsManager.Get Packet Count From Table    ${OS_CMP1_CONN_ID}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    |grep ${VM1_METADATA}|grep arp_sha
    ${get_arp_drop_pkt_before}    OvsManager.Get Packet Count From Table    ${OS_CMP1_CONN_ID}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    |grep arp|grep goto_table:217

...
Verify ARP request Valid MAC and Valid IP for the VM Egress Table
    ...
    ${get_pkt_count_before_arp}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    ${VM1_METADATA}|grep arp_sha
    ${arping_cli} =    BuiltIn.Set Variable    sudo arping -I eth0 -c ${PACKET_COUNT} \ ${RANDOM_IP}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.SSHLibrary.Execute Command on VM Instance    ${REQ_NETWORKS[1]}    @{VM_IP_DPN1}[1]    ${arping_cli}
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_after_arp}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    ${VM1_METADATA}|grep arp_sha
...
Verify ARP request generated from Spoofed IP for the VM
    ...
    ...    ${arp_int_up_cli}
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp}    OvsManager.Get Packet Count From Table    ${OS_CMP1_CONN_ID}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    ${VM1_METADATA}|grep arp_sha
    ${get_arp_drop_pkt_before}    OvsManager.Get Packet Count From Table    ${OS_CMP1_CONN_ID}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    arp|grep goto_table:217
    ...
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_after_arp}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    ${VM1_METADATA}|grep arp_sha
    ${get_arp_drop_pkt_after}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    arp|grep goto_table:217
...
Verify ARP request generated from Spoofed MAC for the VM
    ...
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
    ${get_pkt_count_before_arp}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    ${VM1_METADATA}|grep arp_sha
    ${get_arp_drop_pkt_before}    OvsManager.Get Packet Count From Table    ${OS_COMPUTE_1_IP}    ${INTEGRATION_BRIDGE}    ${TABLE_NO}    arp|grep goto_table:217
    ${arping_cli} =    BuiltIn.Set Variable    sudo arping -I eth0 -c ${PACKET_COUNT} \ ${RANDOM_IP}
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.SSHLibrary.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN1}[1]    ${arping_cli}
    SSHLibrary.Switch Connection    ${OS_CMP1_CONN_ID}
    SSHLibrary.Execute Command    ${FLOW_DUMP_CMD}
...
Verify ARP request generated from Spoofed IP and spoofed MAC for the VM
    ...
    ${arp_int_up_cli} =    BuiltIn.Set Variable    sudo ifconfig eth0 down
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.SSHLibrary.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN1}[1]    ${arp_int_up_cli}
    ${arp_mac_int_cli} =    BuiltIn.Set Variable    sudo ifconfig eth0 hw ether @{SPOOF_MAC_ADDRESS}[1]
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.SSHLibrary.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN1}[1]    ${arp_mac_int_cli}
    ${arp_int_down_cli} =    BuiltIn.Set Variable    sudo ifconfig eth0 @{VM_IP_DPN1}[0]/24 up
    BuiltIn.Wait Until Keyword Succeeds    60s    10s    OpenStackOperations.SSHLibrary.Execute Command on VM Instance    @{REQ_NETWORKS}[1]    @{VM_IP_DPN1}[1]    ${arp_int_down_cli}
...
Create Setup
    ...
    OpenStackOperations.Neutron Security Group Rule Create    @{SECURITY_GROUP}[0]    direction=ingress    protocol=icmp    remote-ip=0.0.0.0/0
    OpenStackOperations.Neutron Security Group Rule Create    @{SECURITY_GROUP}[0]    direction=egress    protocol=icmp    remote-ip=0.0.0.0/0

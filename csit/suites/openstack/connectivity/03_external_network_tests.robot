...
Create External Network And Subnet
    Run Keyword If    '${OPENSTACK_BRANCH}'=='stable/mitaka'    Create Network    ${external_net_name}    --router:external --provider:network_type=vlan --provider:physical_network=${PUBLIC_PHYSICAL_NETWORK} --provider:segmentation_id=${network1_vlan_id}
    ...    ELSE    Create Network    ${external_net_name}    --external --provider-network-type vlan --provider-physical-network ${PUBLIC_PHYSICAL_NETWORK} --provider-segment ${network1_vlan_id}

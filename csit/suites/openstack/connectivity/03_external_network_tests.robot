...
Check Vm Instances Have Ip Address
    ...
    \    ${FLOATING_VM_IPS}    ${FLOATING_DHCP_IP}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_FLOATING}
    \    ${SNAT_VM_IPS}    ${SNAT_DHCP_IP}    Wait Until Keyword Succeeds    180s    10s    Verify VMs Received DHCP Lease    @{VM_INSTANCES_SNAT}
    \    ${FLOATING_VM_LIST_LENGTH}=    Get Length    ${VM_IPS}
    \    ${SNAT_VM_LIST_LENGTH}=    Get Length    ${VM_IPS}
...
SNAT - Send UDP to External Gateway From VM Instance (without Floating IP)
    ...
    Test Netcat Operations From Vm Instance    network_1    @{SNAT_VM_IPS}[0]    ${external_gateway}    UDP

SNAT - Send TCP to External Gateway From VM Instance (without Floating IP)
    [Documentation]    Login to the VM instance and test ping to the external gateway.
    Test Netcat Operations From Vm Instance    network_1    @{SNAT_VM_IPS}[0]    ${external_gateway}    TCP

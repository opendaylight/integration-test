...
Connectivity Tests From Vm Instances In l2_network_1
    ...
    Log    ${dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    @{NET_1_VM_IPS}[1]    ${dst_ip_list}
    ${dst_ip_list}=    Create List    @{NET_1_VM_IPS}[0]    @{DHCP_IPS}[0]     @{NET_1_VM_IPS}[1]
    Log    ${dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    @{NET_1_VM_IPS}[2]    ${dst_ip_list}
...
Connectivity Tests From Vm Instances In l2_network_2
    ...
    Log    ${dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    @{NET_2_VM_IPS}[1]    ${dst_ip_list}
    ${dst_ip_list}=    Create List    @{NET_2_VM_IPS}[0]    @{DHCP_IPS}[1]     @{NET_2_VM_IPS}[1]
    Log    ${dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    @{NET_2_VM_IPS}[2]    ${dst_ip_list}

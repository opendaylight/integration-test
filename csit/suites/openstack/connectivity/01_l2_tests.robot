...
Connectivity Tests From Vm Instances In l2_network_2
    [Documentation]    Logging to the vm instances in l2_network_2
    : FOR    ${vm_instance}    IN    @{NET_2_VM_INSTANCES}
    \    Connectivity Tests From A Vm Instance    l2_network_2    ${vm_instance}    ${NET_2_VM_IPS}    @{DHCP_IPS}[1]

Delete A Vm Instance
    [Documentation]    Delete Vm instances using instance names.
    ${del_vm_ip}=    Get Vm Instance Ip    @{NET_1_VM_INSTANCES}[0]
    Set Suite Variable    ${del_vm_ip}
    Delete Vm Instance    @{NET_1_VM_INSTANCES}[0]

No Ping For Deleted Vm
    [Documentation]    Check non reachability of deleted vm instances by pinging to them.
    ${output}=    Ping From DHCP Should Not Succeed    l2_network_1    ${del_vm_ip}

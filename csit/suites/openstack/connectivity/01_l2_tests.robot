...
Create Networks
    ...
    Run Command On Remote System    ${ODL_SYSTEM_IP}    sudo tcpdump -ni eth0 port 6640 -s 0 -w /tmp/ovsdb_packet_capture.pcap &
...
Show PKT CAP Output
    [Tags]    vishaldebug
    Run Command On Remote System    ${ODL_SYSTEM_IP}    sudo kill $(pgrep tcpdump)
    ${pkt_cap_output}=    Run Command On Remote System    ${ODL_SYSTEM_IP}    sudo tcpdump -vvv -r /tmp/ovsdb_packet_capture.pcap    prompt_timeout=120s
    Log    ${pkt_cap_output}

...
Create VLAN Network (l2_network_1)
    ...
    Run Keyword If    ${feature_check1} == 'False' or ${feature_check2} == 'False'    Create Network    @{NETWORKS_NAME}[0]    --provider:network_type=vlan --provider:physical_network=${PUBLIC_PHYSICAL_NETWORK} --provider:segmentation_id=${network1_vlan_id}

...
Migrate VMs
    Migrate VM      @{NET_1_VM_INSTANCES}[0]
    Migrate VM      @{NET_1_VM_INSTANCES}[1]
    Migrate VM      @{NET_2_VM_INSTANCES}[0]
    Migrate VM      @{NET_2_VM_INSTANCES}[1]
...
Ping2 Vm Instance1 In l2_network_2
    [Documentation]    Check reachability of vm instances by pinging to them.
    Ping Vm From DHCP Namespace    l2_network_2    @{NET2_VM_IPS}[0]
    [Teardown]    Run Keywords    Show Debugs    @{NET_2_VM_INSTANCES}
     ...    AND    Get Test Teardown Debugs

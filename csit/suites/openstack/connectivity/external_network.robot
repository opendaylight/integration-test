...
Suite Setup
    ...
    ${dst_ip}=    BuiltIn.Create List    ${EXTERNAL_PNF}
    Run Keyword And Ignore Error    OpenStackOperations.Test Operations From Vm Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[0]    ${dst_ip}    ttl=1
    Run Keyword And Ignore Error    OpenStackOperations.Test Operations From Vm Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[1]    ${dst_ip}    ttl=1

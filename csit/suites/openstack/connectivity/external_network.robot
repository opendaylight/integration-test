...
Ping Vm Instance1 Floating IP From SNAT VM Instance1
    ...
    ${dst_ip}=    Create List    @{VM_FLOATING_IPS}[0]
    ...
    OpenStackOperations.Test Operations From Vm Instance    @{NETWORKS}[0]    @{NET2_SNAT_VM_IPS}[0]    ${dst_ip}    ttl=1    ping_should_succeed=${expect_ping_to_work}
...
Ping Vm Instance1 Floating IP From SNAT VM Instance2
    ...
    OpenStackOperations.Test Operations From Vm Instance    @{NETWORKS}[0]    @{NET2_SNAT_VM_IPS}[1]    ${dst_ip}    ttl=1    ping_should_succeed=${expect_ping_to_work}

...
Ping Internet Site from Vm Instance 1 After Floating IP Assignment
    ...
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[0]    curl "http://google.com"
    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[0]    ping 8.8.8.8
    OpenStackOperations.Test Operations From Vm Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[0]    8.8.8.8

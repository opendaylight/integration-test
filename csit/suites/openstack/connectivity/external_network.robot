...
@{NETWORKS}       l3_ext_net    l3_ext_net2
@{SUBNETS}        l3_ext_sub    l3_ext_sub2
@{ROUTERS}        l3_ext_router    l3_ext_router2
@{FIP_VMS}        fip_vm_1    fip_vm_2
@{SNAT_VMS}       snat_vm_1    snat_vm_2    snat_vm_3
...
Create Vm Instances
    ...
    ${snat_vms2} =    Create List    @{SNAT_VMS}[0]    @{SNAT_VMS}[1]
    OpenStackOperations.Create Vm Instances    @{NETWORKS}[0]    ${snat_vms2}    sg=${SECURITY_GROUP}
    ${snat_vms2} =    Create List    @{SNAT_VMS}[2]
    OpenStackOperations.Create Vm Instances    @{NETWORKS}[1]    ${snat_vms2}    sg=${SECURITY_GROUP}
...
SNAT - TCP connection to External Gateway From SNAT VM Instance3
    [Documentation]    Login to the VM instance and test TCP connection to the controller via SNAT
    OpenStackOperations.Test Netcat Operations From Vm Instance    @{NETWORKS}[1]    @{SNAT_VM_IPS}[2]    ${EXTERNAL_GATEWAY}

SNAT - UDP connection to External Gateway From SNAT VM Instance3
    [Documentation]    Login to the VM instance and test UDP connection to the controller via SNAT
    OpenStackOperations.Test Netcat Operations From Vm Instance    @{NETWORKS}[1]    @{SNAT_VM_IPS}[2]    ${EXTERNAL_GATEWAY}    -u

...
TCP connection to Vm Instance3 Floating IP From Vm Instance2 With Floating IP (FIP-FIP in the same compute node)
    [Documentation]    Login to the VM instance and test TCP connection though floating IP from another VM instance with FIP
    ${dst_ip} =    BuiltIn.Create List    @{VM_FLOATING_IPS}[2]
    OpenStackOperations.Test Netcat Operations From Vm Instance    @{NETWORKS}[0]    @{NET1_FIP_VM_IPS}[1]    ${dst_ip}

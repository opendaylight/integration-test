...
Add Ssh Allow Rules
    ...
    # FIXME We need to remove default rules from the created group otherwise they might confuse results
...
No Ping From Vm Instance1 To Vm Instance2
    ...
    Test No Ping From Vm Instance    network_1    @{NET1_VM_IPS}[0]    ${VM2_LIST}
...
No Ping From Vm Instance2 To Vm Instance1
    ...
    Test No Ping From Vm Instance    network_1    @{NET1_VM_IPS}[1]    ${VM1_LIST}
...
Add Additional Security Group To VMs
    ...
    Neutron Security Group Create    additional-sg
    # FIXME Should we actually check rules from the new SG?
    : FOR    ${VM}    IN    @{NET_1_VM_INSTANCES}
    \    Add Security Group To VM    ${VM}    additional-sg

Repeat Verify No Ping From DHCP To Vm Instance1
    [Documentation]    Check non-reachability of vm instances by pinging to them.
    [Tags]    skip_if_transparent
    Ping From DHCP Should Not Succeed    network_1    @{NET1_VM_IPS}[0]

Repeat Verify No Ping From DHCP To Vm Instance2
    [Documentation]    Check non-reachability of vm instances by pinging to them.
    [Tags]    skip_if_transparent
    Ping From DHCP Should Not Succeed    network_1    @{NET1_VM_IPS}[1]

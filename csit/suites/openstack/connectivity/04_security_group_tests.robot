...
Add Additional Security Group To VMs
    ...
    Security Group Create Without Default Security Rules    additional-sg1
    Security Group Create Without Default Security Rules    additional-sg2
    Neutron Security Group Rule Create    additional-sg1    direction=ingress    protocol=icmp    remote_ip_prefix=@{NET1_DHCP_IP}[0]/0    remote_group_id=csit-remote-sgs
    Neutron Security Group Rule Create    additional-sg2    direction=ingress    protocol=icmp    remote_ip_prefix=@{NET1_DHCP_IP}[0]/0    remote_group_id=additional-sg2
    : FOR    ${VM}    IN    @{NET_1_VM_INSTANCES}
    \    Add Security Group To VM    ${VM}    additional-sg1
    \    Add Security Group To VM    ${VM}    additional-sg2

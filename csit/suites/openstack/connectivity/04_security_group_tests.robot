...
Create Vm Instances For l2_network_1
    ...
    Create Vm Instances    l2_network_1    ${NET_1_VM_INSTANCES}    sg=csit-remote-sgs
...
No Ping Vm Instance1 In l2_network_1
...
No Ping Vm Instance2 In l2_network_1
    [Documentation]    Check non-reachability of vm instances by pinging to them.
    Ping Vm From DHCP Namespace    l2_network_1    @{NET1_VM_IPS}[1]

No Connectivity Tests From Vm Instance1 In l2_network_1
    [Documentation]    Login to the vm instance and test some operations
    Test No Ping From Vm Instance    l2_network_1    @{NET1_VM_IPS}[0]    ${NET1_VM_IPS}

No Connectivity Tests From Vm Instance2 In l2_network_1
    [Documentation]    Login to the vm instance and test operations
    Test No Ping From Vm Instance    l2_network_1    @{NET1_VM_IPS}[1]    ${NET1_VM_IPS}

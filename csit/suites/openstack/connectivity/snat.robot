...
Get OS nodes
    ...
    Set To Dictionary    ${OS_NODES}    ${OS_CMP1_SHORT}    ${OS_COMPUTE_1_IP}    ${OS_CMP2_SHORT}    ${OS_COMPUTE_2_IP}   ${OS_ODL1__SHORT}    ${ODL_SYSTEM_1_IP}
    ...                  ${OS_ODL3__SHORT}    ${ODL_SYSTEM_2_IP}    ${OS_ODL3__SHORT}    ${ODL_SYSTEM_3_IP}
...
Add Ssh Allow Rule
    [Documentation]    all TCP/UDP/ICMP packets for this suite
...
Block Traffic on NAPT switch node
    ...
    Utils.Run Command On Remote System    ${OS_NODES['${router_node_shortname}']}    sudo iptables -I OUTPUT 1 -p tcp --dport 6653 -j DROP
...
Verify Migration
    ...
    OpenStackOperations.Test Operations From Vm Instance    ${NETWORK}    @{SNAT_VM_IPS}[0]    ${EXTERNAL_IPS}
    [Teardown]    Run Keywords    Utils.Run Command On Remote System    ${OS_NODES['${router_node_shortname}']}    sudo iptables -D OUTPUT -p tcp --dport 6653 -j DROP
...
Get NAPT Switch Node
    ...
    Get ControlNode Connection
    ${command} =    BuiltIn.Set Variable    sudo docker exec -i opendaylight_api /opt/opendaylight/bin/client -u karaf display-napt-switches |grep ${router_id} | awk '{print $3}'
    ${output} =    Utils.Run Command On Controller    ${ODL_SYSTEM_1_IP}    cmd=${command}
    [Return]    ${output}

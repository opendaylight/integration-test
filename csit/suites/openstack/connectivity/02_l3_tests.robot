...
Connectivity Tests From Vm Instances In network_1
    ...
    ${net_id}=    Get Net Id    network_1
    ${dst_ip_list}=    Create List    50.0.0.2     50.0.0.4
    Log    ${dst_ip_list}
    ${other_dst_ip_list}=    Create List    60.0.0.2      60.0.0.4
    Log    ${other_dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    50.0.0.3    ${dst_ip_list}    l2_or_l3=l3     other_dst_ip_list=${other_dst_ip_list}

Connectivity Tests From Vm Instances In network_2
    [Documentation]    Logging to the vm instance using generated key pair.
    ${net_id}=    Get Net Id    network_2
    ${dst_ip_list}=    Create List    60.0.0.2       60.0.0.4
    Log    ${dst_ip_list}
    ${other_dst_ip_list}=    Create List    50.0.0.2    50.0.0.4
    Log    ${other_dst_ip_list}
    Test Operations From Vm Instance      ${net_id}    60.0.0.3    ${dst_ip_list}    l2_or_l3=l3     other_dst_ip_list=${other_dst_ip_list}

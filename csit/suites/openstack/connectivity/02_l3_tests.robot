...
Ping Vm Instance1 In network_2 From network_1
    ...
    Ping Vm From DHCP Namespace    network_1      @{NET_2_VM_INSTANCES}[0]

Ping Vm Instance2 In network_2 From network_1
    [Documentation]    Check reachability of vm instances by pinging to them after creating routers.
    Ping Vm From DHCP Namespace    network_1    @{NET_2_VM_INSTANCES}[1]

Ping Vm Instance3 In network_2 From network_1
    [Documentation]    Check reachability of vm instances by pinging to them after creating routers.
    Ping Vm From DHCP Namespace    network_1    @{NET_2_VM_INSTANCES}[2]

Ping Vm Instance1 In network_1 From network_2
    [Documentation]    Check reachability of vm instances by pinging to them after creating routers.
    Ping Vm From DHCP Namespace    network_2    @{NET_1_VM_INSTANCES}[0]

Ping Vm Instance2 In network_1 From network_2
    [Documentation]    Check reachability of vm instances by pinging to them after creating routers.
    Ping Vm From DHCP Namespace    network_2    @{NET_1_VM_INSTANCES}[1]

Ping Vm Instance3 In network_1 From network_2
    [Documentation]    Check reachability of vm instances by pinging to them after creating routers.
    Ping Vm From DHCP Namespace    network_2    @{NET_1_VM_INSTANCES}[2]

Connectivity Tests From Vm Instance1 In network_1
    [Documentation]     Login to the VM instance and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_INSTANCES}[0]    ${dst_list}      dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance2 In network_1
    [Documentation]    Login to the vm instance and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_INSTANCES}[1]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance3 In network_1
    [Documentation]    Login to the vm instance  and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_INSTANCES}[2]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance1 In network_2
    [Documentation]    Login to the vm instance and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_INSTANCES}[0]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance2 In network_2
    [Documentation]    Logging to the vm instance using generated key pair.
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_INSTANCES}[1]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance3 In network_2
    [Documentation]    Logging to the vm instance using generated key pair.
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_INSTANCES}[2]    ${dst_list}     dhcp_ips=${DHCP_IPS}

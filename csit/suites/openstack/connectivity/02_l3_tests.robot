...
Create Vm Instances For network_3
    [Documentation]    Create Four Vm instances using flavor and image names for a network.
    Create Vm Instances    network_3    ${NET_3_VM_INSTANCES}
...
Connectivity Tests From Vm Instance1 In network_1
    ...
    ${dst_list}=    Create List    ${NET3_DHCP_IP}     ${NET2_DHCP_IP}     ${NET1_DHCP_IP}
    Log    ${dst_list}
...
Connectivity Tests From Vm Instance1 In network_2
    [Documentation]    Login to the vm instance and test operations
    ${dst_list}=    Create List    ${NET3_DHCP_IP}     ${NET1_DHCP_IP}     ${NET2_DHCP_IP}
    Log    ${dst_list}

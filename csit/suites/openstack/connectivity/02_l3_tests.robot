...
Connectivity Tests From Vm Instance1 In network_1
    ...
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_IPS}[0]    ${dst_list}      dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance2 In network_1
    [Documentation]    Login to the vm instance and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_IPS}[1]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance3 In network_1
    [Documentation]    Login to the vm instance  and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_1    @{NET_1_VM_IPS}[2]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance1 In network_2
    [Documentation]    Login to the vm instance and test operations
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_IPS}[0]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance2 In network_2
    [Documentation]    Logging to the vm instance using generated key pair.
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_IPS}[1]    ${dst_list}     dhcp_ips=${DHCP_IPS}

Connectivity Tests From Vm Instance3 In network_2
    [Documentation]    Logging to the vm instance using generated key pair.
    ${dst_list}=    Create List     @{NET_1_VM_INSTANCES}    @{NET_2_VM_INSTANCES}
    Log    ${dst_list}
    Test Operations From Vm Instance    network_2    @{NET_2_VM_IPS}[2]    ${dst_list}     dhcp_ips=${DHCP_IPS}

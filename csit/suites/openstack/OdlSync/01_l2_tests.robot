...
Stop Odl And Delete DB
    ...
    ${OUTPUT}=    Write Commands Until Prompt    /tmp/distribution-karaf-0.5.2-SNAPSHOT/bin/stop    1s
    Log    ${OUTPUT}
    ${cmd}=    Set Variable    rm -rf /tmp/distribution-karaf-0.5.2-SNAPSHOT/{data,journal,snapshots}
    Log    ${cmd}
...
Start Odl
    ...
    ${odl_conn_id}=    Get OdlNode Connection
    Switch Connection    ${odl_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    /tmp/distribution-karaf-0.5.2-SNAPSHOT/bin/start    1s
    Log    ${OUTPUT}
    [Teardown]

Reconnect Ports
    [Documentation]    Re-connect tap ports
    ${cmd3}=    Set Variable    sudo ovs-vsctl show
    ${cmd1}=    Set Variable    sudo ovs-ofctl dump-ports-desc br-int -OOpenFlow13 | grep tap | awk -F ")" '{print$1}' | awk -F "(" '{print$2}' > tap_if
    Log    ${cmd1}
    ${cmd2}=    Set Variable    for interface in $(cat tap_if); do sudo ovs-vsctl add-port br-int $interface; done
    Log    ${cmd2}
    ${control_conn_id}=    Get ControlNode Connection
    Switch Connection    ${control_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd1}    1s
    Log    ${OUTPUT}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd2}    1s
    Log    ${OUTPUT}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd3}    1s
    Log    ${OUTPUT}
    ${compute1_conn_id}=    Get ComputeNode Connection    ${OS_COMPUTE_1_IP}
    Switch Connection    ${compute1_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd1}    1s

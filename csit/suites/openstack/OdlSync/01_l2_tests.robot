...
Stop Odl And Delete DB
    ...
    ${OUTPUT}=    Write Commands Until Prompt    ${KARAF_HOME}/bin/stop    2s
    Log    ${OUTPUT}
    ${cmd}=    Set Variable    rm -rf ${KARAF_HOME}/{data,journal,snapshots}
    Log    ${cmd}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd}    2s
    Log    ${OUTPUT}
...
Start Odl
    [Documentation]    Start running ODL and wait for sync to complete
    ${odl_conn_id}=    Get OdlNode Connection
    Switch Connection    ${odl_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    ${KARAF_HOME}/bin/start    2s
    Log    ${OUTPUT}
    ${netvirt}=    Create Dictionary    netvirt:1=1
    Wait Until Keyword Succeeds    300s    4s    ClusterManagement.Check_Item_Occurrence_Member_List_Or_All    uri=${OPERATIONAL_NODES_NETVIRT}    dictionary=${netvirt}
    sleep    5m

Reconnect Ports
    [Documentation]    Re-connect tap ports
    ${cmd}=    Set Variable    sudo ovs-vsctl show    # only for debug purpose
    ${control_conn_id}=    Get ControlNode Connection
    Switch Connection    ${control_conn_id}
    ...
    \    Write Commands Until Prompt    sudo ovs-vsctl add-port br-int ${interface}    2s
    ${compute1_conn_id}=    Get ComputeNode Connection    ${OS_COMPUTE_1_IP}
    Switch Connection    ${compute1_conn_id}

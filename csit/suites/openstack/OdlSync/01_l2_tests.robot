...
Stop Odl And Delete DB
    ...
    ${OUTPUT}=    Write Commands Until Prompt    /tmp/distribution-karaf-0.6.0-SNAPSHOT/bin/stop    1s
    Log    ${OUTPUT}
    ${cmd}=    Set Variable    rm -rf /tmp/distribution-karaf-0.6.0-SNAPSHOT/{data,journal,snapshots}
    Log    ${cmd}
...
Start Odl
    ...
    ${odl_conn_id}=    Get OdlNode Connection
    Switch Connection    ${odl_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    /tmp/distribution-karaf-0.6.0-SNAPSHOT/bin/start    1s
    Log    ${OUTPUT}

...
Delete OF Rules
    ...
    ${cmd1}=    Set Variable    sudo ovs-vsctl show | grep "Interface \\"tap" | awk -F "\\"" '{print$2}'>tap_if
    ${cmd2}=    Set Variable    sudo ovs-vsctl del-br br-int
    Log    ${cmd1}
...
Reconnect Ports
    ...
    ${cmd2}=    Set Variable    for i in $(cat tap_if); do sudo ovs-vsctl add-port br-int $i;done
    Log    ${cmd2}
    ${control_conn_id}=    Get ControlNode Connection
    Switch Connection    ${control_conn_id}
    ${OUTPUT}=    Write Commands Until Prompt    ${cmd1}    1s

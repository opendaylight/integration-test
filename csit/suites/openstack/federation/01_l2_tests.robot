...
Ping Vm Instance1 From Site A To Site B Should Not Succeed
    ...
    ${netId}=    Get Network Id    ${siteB_index}    ${net_name}
    Ping From DHCP Should Not Succeed    l2_network_1    @{NET1_VM_IPS_SITE_B}[0]    site_index=1    ${netId}

Ping Vm Instance1 From Site B To Site A Should Not Succeed
    [Documentation]    Try to ping vms in 2 sites before connecting the sites- should not succeed.
    ${netId}=    Get Network Id    ${siteA_index}    ${net_name}
    Ping From DHCP Should Not Succeed    l2_network_1    @{NET1_VM_IPS_SITE_A}[0]    site_index=2    ${netId}
...
Ping Vm Instance1 In l2_network_1 From Site A To Site B
    ...
    ${netId}=    Get Network Id    ${siteB_index}    ${net_name}
    Ping Vm From DHCP Namespace    l2_network_1    @{NET1_VM_IPS_SITE_B}[0]    site_index=1    ${netId}

Ping Vm Instance1 In l2_network_1 From Site B To Site A
    [Documentation]    Check reachability of vm instances by pinging to them from one site to another.
    ${netId}=    Get Network Id    ${siteA_index}    ${net_name}
    Ping Vm From DHCP Namespace    l2_network_1    @{NET1_VM_IPS_SITE_A}[0]    site_index=2    ${netId}
...
Connectivity Tests From Vm Instance1 In Site A To Vm Instance1 In Site B
    ...
    Test Operations From Vm Instance    l2_network_1    @{NET1_VM_IPS_SITE_A}[0]    ${NET1_VM_IPS_SITE_B}[0]    site_index=1

Connectivity Tests From Vm Instance1 In Site B To Vm Instance1 In Site A
    [Documentation]    Login to the vm instance using generated key pair.
    Test Operations From Vm Instance    l2_network_1    @{NET1_VM_IPS_SITE_B}[0]    ${NET1_VM_IPS_SITE_A}[0]    site_index=2

...
Config Federation Sites
    ...
    : FOR    ${i}    IN RANGE    1    3
    \    ${odl_ip}=    Catenate    SEPARATOR=    ODL_SYSTEM_    ${i}    _IP
    \    Create Session    session    http://${${odl_ip}}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS}
    \    ${data}=    Set Variable    {"federation-service-config:federation-sites-config": {"federation-service-config:federation-site-config": [{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${ODL_SYSTEM_1_IP}","federation-service-config:mqUserPwd": "federation","federation-service-config:controlQueueName": "CONTROL_QUEUE_${ODL_SYSTEM_1_IP}","federation-service-config:mqUser": "federation","federation-service-config:broker-ip": "${ODL_SYSTEM_1_IP}"},{"federation-service-config:mqPortNumber": "5672","federation-service-config:site-ip": "${ODL_SYSTEM_2_IP}","federation-service-config:mqUserPwd": "federation","federation-service-config:controlQueueName":"CONTROL_QUEUE_${ODL_SYSTEM_2_IP}","federation-service-config:mqUser": "federation","federation-service-config:broker-ip": "${ODL_SYSTEM_2_IP}"}]}}
...
Check Vm Instances Have Ip Address
    ...
    ${status}    ${message}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    60s    5s    Verify Ip
    ...    1    true    @{NET_1_VM_INSTANCES}
    ${status}    ${message}    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    60s    5s    Verify Ip
    ...    2    true    @{NET_1_VM_INSTANCES}
    ${NET1_VM_IPS_SITE_A}    ${NET1_DHCP_IP_SITE_A}    Verify Ip    1    false    @{NET_1_VM_INSTANCES}
    ${NET1_VM_IPS_SITE_B}    ${NET1_DHCP_IP_SITE_B}    Verify Ip    2    false    @{NET_1_VM_INSTANCES}
    ${VM_IPS}=    Collections.Combine Lists    ${NET1_VM_IPS_SITE_A}    ${NET1_VM_IPS_SITE_B}
    ...
    Should Not Contain    ${NET1_DHCP_IP_SITE_A}    None
    Set Suite Variable    ${NET1_VM_IPS_SITE_B}
    Should Not Contain    ${NET1_VM_IPS_SITE_B}    None
    Should Not Contain    ${NET1_DHCP_IP_SITE_B}    None
...
Ping Vm Instance1 From Site A To Site B Should Not Succeed
    ...
    Ping From DHCP Should Not Succeed    l2_network_1    @{NET1_VM_IPS_SITE_B}[0]    site_index=1    netId=${netId}

Ping Vm Instance1 From Site B To Site A Should Not Succeed
    [Documentation]    Try to ping vms in 2 sites before connecting the sites- should not succeed.
    ${netId}=    Get Network Id    1    l2_network_1
    Ping From DHCP Should Not Succeed    l2_network_1    @{NET1_VM_IPS_SITE_A}[0]    site_index=2    netId=${netId}
...
Ping Vm Instance1 In l2_network_1 From Site A To Site B
    ...
    ${netId}=    Get Network Id    1    l2_network_1
    Ping Vm From DHCP Namespace    l2_network_1    @{NET1_VM_IPS_SITE_B}[0]    site_index=1    netId=${netId}

Ping Vm Instance1 In l2_network_1 From Site B To Site A
    [Documentation]    Check reachability of vm instances by pinging to them from one site to another.
    ${netId}=    Get Network Id    2    l2_network_1
    Ping Vm From DHCP Namespace    l2_network_1    @{NET1_VM_IPS_SITE_A}[0]    site_index=2    netId=${netId}

...
@{NET_1_VM_INSTANCES}    VmInstance1_net_1
@{NET_2_VM_INSTANCES}    VmInstance1_net_2
@{NET_1_VM_IPS}    90.0.0.3
@{NET_2_VM_IPS}    100.0.0.3
...
Login to Vm Instances In net_1 Using Ssh
    ...
    Switch Connection    ${devstack_conn_id}
    ${net_id}=    Get Net Id    net_1
    Ssh Vm Instance    ${net_id}    90.0.0.3

Ping Vm Instance From Instance In net_1
    [Documentation]    Check reachability of vm instances by pinging.
    ${output}=    Ping From Instance    100.0.0.3
    Should Contain    ${output}    64 bytes

Ping Dhcp Server In net_2 From Instance In net_1
    [Documentation]    ping the dhcp server from instance.
    ${output}=    Ping From Instance    100.0.0.2
    Should Contain    ${output}    64 bytes
    Close Vm Instance
...
Login to Vm Instances In net_2 Using Ssh
    ...
    Switch Connection    ${devstack_conn_id}
    ${net_id}=    Get Net Id    net_2
    Ssh Vm Instance    ${net_id}    100.0.0.3

Ping Vm Instance From Instance In net_2
    [Documentation]    Check reachability of vm instances by pinging.
    ${output}=    Ping From Instance    90.0.0.3
    Should Contain    ${output}    64 bytes

Ping Dhcp Server In net_1 From Instance In net_2
    [Documentation]    ping the dhcp server from instance.
    ${output}=    Ping From Instance    90.0.0.2
    Should Contain    ${output}    64 bytes
    Close Vm Instance

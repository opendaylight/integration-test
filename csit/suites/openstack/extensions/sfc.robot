...
${HTTP_SUCCESS}    open
${HTTP_FAILURE}    Operation timed out
...
Test Communication From Vm Instance1 In net_1 No SF
    ...
    ${curl_resp}    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 80    user=root    password=opnfv
    ...    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Should Contain    ${curl_resp}    ${HTTP_SUCCESS}
    ${curl_resp}    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 81    user=root    password=opnfv
    ...    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Should Contain    ${curl_resp}    ${HTTP_SUCCESS}
...
Test Communication From Vm Instance1 In net_1 Port 80 via SF
    ...
    Wait Until Keyword Succeeds    3x    10s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 80    ${HTTP_SUCCESS}
    Wait Until Keyword Succeeds    3x    10s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 81    ${HTTP_SUCCESS}
    ...
    Wait Until Keyword Succeeds    3x    10s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 80    ${HTTP_FAILURE}
...
Test Communication From Vm Instance1 In net_1 Port 81 via SF
    ...
    Start Vxlan Tool in SF    @{NETWORKS}[0]    ${SF1_IP}    args=--do forward --interface eth0 --output eth1 --verbose off --block 81
    Wait Until Keyword Succeeds    3x    10s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 80    ${HTTP_SUCCESS}
    Wait Until Keyword Succeeds    3x    10s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 81    ${HTTP_FAILURE}
    ...    cmd_timeout=60s
...
Test Communication From Vm Instance1 For Symmetric Chain
    ...
    Wait Until Keyword Succeeds    8x    20s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} -p 2000 @{NET1_VM_IPS}[1] 82    ${HTTP_SUCCESS}
    ...    cmd_timeout=80s
    ...
    Start Vxlan Tool in SF    @{NETWORKS}[0]    ${SF1_IP}    args=--do forward --interface eth1 --output eth0 --verbose off
    Wait Until Keyword Succeeds    8x    20s    Access Http And Check Status    @{NETWORKS}[0]    ${NC_COMMAND} -p 2000 @{NET1_VM_IPS}[1] 82    ${HTTP_FAILURE}
    ...    cmd_timeout=80s
...
Check Vm Instances Have Ip Address And Ready For Test
    ...
    BuiltIn.Wait Until Keyword Succeeds    300s    60s    OpenStackOperations.Check If Instance Is Ready For Ssh Login Using Password    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    user=root
    ...    password=opnfv    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Wait Until Keyword Succeeds    300s    60s    OpenStackOperations.Check If Instance Is Ready For Ssh Login Using Password    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    user=root
    ...    password=opnfv    OS_SYSTEM_PROMPT=#    console=root
    OpenStackOperations.Show Debugs    @{NET_1_VMS}
    OpenStackOperations.Get Suite Debugs
...
Start Applications on VM Instances For Test
    ...
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    ${WEBSERVER_80}    user=root    password=opnfv    OS_SYSTEM_PROMPT=#
    ...    console=root
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    ${WEBSERVER_81}    user=root    password=opnfv    OS_SYSTEM_PROMPT=#
    ...    console=root
    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    ${WEBSERVER_82}    user=root    password=opnfv    OS_SYSTEM_PROMPT=#
    ...    console=root
...
Access Http And Check Status
    ...
    ${curl_resp}    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${curl_command}    cmd_timeout=${cmd_timeout}    user=root
    ...    password=opnfv    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Should Contain    ${curl_resp}    ${ret_code}

Execute Command on VM Instance
    [Arguments]    ${net_name}    ${vm_ip}    ${cmd}    ${user}=root    ${password}=cubswin:)    ${OS_SYSTEM_PROMPT}=$
    ...    ${console}=root    ${cmd_timeout}=30s
...
Check If Console Is VmInstance
    [Arguments]    ${console}=root    ${OS_SYSTEM_PROMPT}=$

...
Check If Instances Are Ready For Test
    BuiltIn.Wait Until Keyword Succeeds    300s    30s    OpenStackOperations.Check If NonCirros Instance Is Ready For Ssh Login    @{NETWORKS}[0]    ${SF1_IP}    user=ubuntu
    BuiltIn.Wait Until Keyword Succeeds    300s    30s    OpenStackOperations.Check If NonCirros Instance Is Ready For Ssh Login    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    user=ubuntu
    BuiltIn.Wait Until Keyword Succeeds    300s    30s    OpenStackOperations.Check If NonCirros Instance Is Ready For Ssh Login    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    user=ubuntu
...
Test Communication From Vm Instance1 For Symmetric Chain
    ...
    ${curl_resp}    OpenStackOperations.Execute Command on NonCirros VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${CURL_COMMAND} http://@{NET1_VM_IPS}[1]:82    user=ubuntu
    BuiltIn.Should Not Contain    ${curl_resp}    ${HTTP_SUCCESS}
    BuiltIn.Comment    Test to confirm the DEST->SRC via SF
    Stop Vxlan Tool in SF    @{NETWORKS}[0]    ${SF1_IP}

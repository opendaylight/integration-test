...
Connectivity Tests From Vm Instance1 In net_1
    ...
    ${CURL_OP}      OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[2]    curl -v --connect-timeout 20 http://@{NET1_VM_IPS}[4]:82
    BuiltIn.Should Contain    ${CURL_OP}     server82
    ${CURL_OP}      OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[2]    curl -v --connect-timeout 20 http://@{NET1_VM_IPS}[4] 

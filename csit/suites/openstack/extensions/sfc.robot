...
Configure Service Function VMs
    ...
    OpenstackOperations.Download File On Openstack Node    ${OS_CNTL_CONN_ID}    vxlan_tool.py     ${VXLAN_TOOL_URL}
    BuiltIn.Comment    Copy vxlan_tool script to SFC VM
    OpenStackOperations.Copy File To NonCirros VM Instance    @{NETWORKS}[0]    ${SF1_IP}    /tmp/vxlan_tool.py    user=${CLOUD_USER_NAME}    idfile=/tmp/odlsfctest
    BuiltIn.Comment    Bring up the second Interface for egress
    Execute Command on NonCirros VM Instance    @{NETWORKS}[0]    ${SF1_IP}    sudo ifconfig ens3 up    user=${CLOUD_USER_NAME}    idfile=/tmp/odlsfctest
    [Teardown]    BuiltIn.Run Keywords    OpenStackOperations.Get Test Teardown Debugs
    ...    AND    OpenStackOperations.Get Test Teardown Debugs For SFC
    ...    AND    OpenStackOperations.Exit From Vm Console
...
Create Instances For Testing
    ...
    OpenStackOperations.Create New Flavor For Testing    ${CLOUD_FLAVOR_NAME}    2048    4

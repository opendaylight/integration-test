...
${CURL_COMMAND}    curl -v --connect-timeout 20
${HTTP_SUCCESS}    200 OK
...
Test Communication From Vm Instance1 For Symmetric Chain
    ...
    ${curl_resp}    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${CURL_COMMAND} --local-port 2000 http://@{NET1_VM_IPS}[1]:82
    BuiltIn.Should Contain    ${curl_resp}    ${HTTP_SUCCESS}

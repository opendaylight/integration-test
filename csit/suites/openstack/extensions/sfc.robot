...
Configure Service Function VMs
    ...
    OpenstackOperations.Download File On Openstack Node    ${OS_CNTL_CONN_ID}    /tmp/vxlan_tool.py     "https://git.opendaylight.org/gerrit/gitweb?p=sfc.git;a=blob_plain;f=sfc-test/nsh-tools/vxlan_tool.py;h=1445292405bc17516d03e4a448ac2e0f65d38e79;hb=HEAD"
...
Test Communication From Vm Instance1 For Symmetric Chain
    ...
    Wait Until Keyword Succeeds	   3x   10s    Access Http And Check Status     @{NETWORKS}[0]     ${CURL_COMMAND} --local-port 2000 -m 50s http://@{NET1_VM_IPS}[1]:82     ${HTTP_SUCCESS}    cmd_timeout=60s
    ...
    Start Vxlan Tool in SF    @{NETWORKS}[0]    ${SF1_IP}    args=--do forward --interface ens3 --output ens2 --verbose off
    Wait Until Keyword Succeeds	   3x   10s    Access Http And Check Status     @{NETWORKS}[0]     ${CURL_COMMAND} --local-port 2000 -m 50s http://@{NET1_VM_IPS}[1]:82     ${HTTP_FAILURE}     cmd_timeout=60s

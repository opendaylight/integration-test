...
Test Communication From Vm Instance1 In net_1 No SF
    ...
    ${nc_resp}    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 80    user=root    password=opnfv
    ...    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Should Contain    ${nc_resp}    ${RES_SUCCESS}
    ${nc_resp}    Execute Command on VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    ${NC_COMMAND} @{NET1_VM_IPS}[1] 81    user=root    password=opnfv
    ...    OS_SYSTEM_PROMPT=#    console=root
    BuiltIn.Should Contain    ${nc_resp}    ${RES_SUCCESS}
...
Check Network Reachability
    ...
    ${nc_resp}    Execute Command on VM Instance    ${net_name}    ${vm_ip}    ${command} @{NET1_VM_IPS}[1] ${port}    cmd_timeout=${cmd_timeout}    user=root
    ...    password=opnfv    OS_SYSTEM_PROMPT=#    console=root

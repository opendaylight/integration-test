...
Create Flow Classifiers
    ...
    OpenStackOperations.Create SFC Flow Classifier    FC_81      @{NET1_VM_IPS}[1]    @{NET1_VM_IPS}[0]    tcp    81    dest_vm_port
    OpenStackOperations.Create SFC Flow Classifier    FC_82      @{NET1_VM_IPS}[0]    @{NET1_VM_IPS}[1]    tcp    82    source_vm_port
    OpenStackOperations.Create SFC Flow Classifier    FC_83      @{NET1_VM_IPS}[1]    @{NET1_VM_IPS}[0]    tcp    85    dest_vm_port
...
Test Communication From Vm Instance1 In net_1
    ...
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
    BuiltIn.Should Be True      ${time_value} < 1
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[1]:82
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line     ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
    BuiltIn.Should Be True      ${time_value} < 1
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[1]:83
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line     ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
...
Test Communication From Vm Instance2 In net_1
    [Documentation]    Login to the source VM instance, and send a HTTP GET using curl to the destination VM instance, If the SF handles the traffic there will be a delay.
    ${DEST_VM_LIST}    BuiltIn.Create List    @{NET1_VM_IPS}[1]
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[0]
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line    ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
    BuiltIn.Should Be True      ${time_value} < 1
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[0]:81
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line     ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
    BuiltIn.Should Be True      ${time_value} >= 1
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[0]:82
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line     ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
    BuiltIn.Should Be True      ${time_value} < 1
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[1]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[0]:83
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
    ${last_line}        String.Get Line     ${curl_resp_with_time}       -2
    ${time_value_string}        String.Fetch From Right     ${last_line}       is
    ${time_value}       BuiltIn.Convert To Number      ${time_value_string}    0
...
Update Port Chain
    ...
    OpenStackOperations.Update SFC Port Chain Removing A Flow Classifier    SFCPC1      FC_80
    OpenStackOperations.Update SFC Port Chain Removing A Flow Classifier    SFCPC1      FC_81
    OpenStackOperations.Update SFC Port Chain With A New Flow Classifier    SFCPC1      FC_82
    OpenStackOperations.Update SFC Port Chain With A New Flow Classifier    SFCPC1      FC_83

Test Communication From Vm Instance1 In net_1 Post Update
    [Documentation]    Login to the source VM instance, and send a HTTP GET using curl to the destination VM instance, If the SF handles the traffic, there will be delay causing the time for curl to be higher.
    ${DEST_VM_LIST}    BuiltIn.Create List    @{NET1_VM_IPS}[1]
    ${curl_resp_with_time}      OpenStackOperations.Execute Command on CentOS VM Instance    @{NETWORKS}[0]    @{NET1_VM_IPS}[0]    export TIMEFORMAT="Time taken is %R";time curl -v http://@{NET1_VM_IPS}[1]
    BuiltIn.Should Contain    ${curl_resp_with_time}    200
...
Test Communication From Vm Instance2 In net_1 Post Update
    [Documentation]    Login to the source VM instance, and send a HTTP GET using curl to the destination VM instance, If the SF handles the traffic there will be a delay.

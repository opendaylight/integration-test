...
@{VPN_INSTANCE_NAME}    4ae8cd92-48ca-49b5-94e1-b2921a2661c7
                        ...  4ae8cd92-48ca-49b5-94e1-b2921a266112
...
Create VMs
    ...
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[0]}
    ...  ${VM_INSTANCES[0]}    ${OS_COMPUTE_1_IP}
    Create Vm Instance With Port On Compute Node    ${PORT_LIST[1]}
    ...  ${VM_INSTANCES[1]}    ${OS_COMPUTE_2_IP}
...
The VMs are active
    ...
    ${rc}= Execute Command  nova list | grep ${VM_INSTANCES[0]} |
    ...  grep -i active  return_stdout=False  return_rc=True
    Should Be Equal As Integers  ${rc}  0
    ${rc}= Execute Command  nova list | grep ${VM_INSTANCES[1]} |
    ...  grep -i active  return_stdout=False  return_rc=True
    Should Be Equal As Integers  ${rc}  0
...
Then VMs are not reachable
    ...
    Run Keyword And Expect Error  100% packet loss  Check ping between VM1 and VM2
    ...      ${NETWORKS[0]}     ${VM_INSTANCES[0]}     ${VM_INSTANCES[1]}
...
BGP Route is added between VPN1 and VPN2
    ...
    ${resp}    RequestsLibrary.Put Request    session    ${REST_CON}bgp:bgp-router/
    ...  data=${body}
    Log    ${resp.content}
...
Neighbouring routes is added between VPN1 and VPN2
    ...
    ${resp}    RequestsLibrary.Put Request    session    ${REST_CON}bgp:bgp-neighbors/
    ...  data=${body}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    204
...
Check ping between VM1 and VM2
    ...
    ${output}=    Write Commands Until Expected Prompt    sudo ip netns exec qdhcp-${net_id}
    ...  ssh ${user}@${vm1_ip} -o ConnectTimeout=10 -o StrictHostKeyChecking=no    d:

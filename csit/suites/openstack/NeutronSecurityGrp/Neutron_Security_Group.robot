...
Neutron Setup Creation
    ...
    ${port_id}    Create Port    ${NETWORK}    ${PORT1}    ${ADD_ARGMS}
    Log    ${port_id}
    Log    "Creating ports"
    ${port_id}    Create Port    ${NETWORK}    ${PORT2}    ${ADD_ARGMS}
    Log    ${port_id}
...
Neutron Rule Creation With Invalid Parameters
    ...
    ${output}=    Set Variable    Neutron Security Group Rule Create    ${SecurityGroupName}    &{Kwargs}
    Log    ${output}

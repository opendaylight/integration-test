...
Create Security Rule and Validate
    ...
    ${OUTPUT}    ${RULE_ID}    Neutron Security Group Rule Create    ${SGP_SSH}    &{Kwargs}
    Log    ${OUTPUT}
    Log    ${RULE_ID}
...
Neutron Rule Creation With Invalid Parameters
    ...
    ${output}=    Run Keyword If    '${OPENSTACK_BRANCH}'=='stable/mitaka'    Write Commands Until Prompt    neutron security-group-rule-create ${SecurityGroupName} ${additional_args}
    ...    ELSE    Write Commands Until Prompt    openstack security group rule create ${Security_group_name} ${additional_args}
    Log    ${output}

...
TC01 Verify TUNNEL creation
    ...
    [Tags]    Post 
    Log    "Get the dpn ids"
    ${resp}    vpn.Create Tunnel   srcip=${TOOLS_SYSTEM_IP}    dstip=${TOOLS_SYSTEM_IP_2}    ovs_pwd=${OVSPWD}
    Log To Console    ${resp}
...
TC08 Verify E2E ELAN connectivity across DPNs with VxLAN
    ...
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_IP2}    ${EXP_STR}
    Log To Console    ${ping_output}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX2}    ${PING_IP4}    ${EXP_STR}
    Log To Console    ${ping_output}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
...
TC12 Verify end to end data path within same network and from one network to the other 
    ...
    Verify Flows Are Present    ${TOOLS_SYSTEM_IP}    ${swdumpflows}
    Verify Flows Are Present    ${TOOLS_SYSTEM_IP_2}    ${swdumpflows}

    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_IP2}    ${EXP_STR}
    Log To Console    ${ping_output}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_IP3}    ${EXP_STR}
    Log To Console    ${ping_output}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_IP4}    ${EXP_STR}
    Log To Console    ${ping_output}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
...
TC14 Verify Deletion of vpn instance
    ...
    Sleep    180
    Wait Until Keyword Succeeds    12s    2s    Verify Flows Are Removed    ${TOOLS_SYSTEM_IP}    ${swdumpflows}
    Wait Until Keyword Succeeds    12s    2s    Verify Flows Are Removed    ${TOOLS_SYSTEM_IP_2}    ${swdumpflows}
    #Verify Flows Are Removed    ${TOOLS_SYSTEM_IP}    ${swdumpflows}
    #Verify Flows Are Removed    ${TOOLS_SYSTEM_IP_2}    ${swdumpflows}
...
TC29 Add Multiple Extra Routes and check Data path before L3VPN Creation
   ...
   ${resp}    Create_OpenStack_Entity    ${OS_CONTROL_NODE_IP}    ${STK_UNAME}    ${STK_PWD}    ${ADD_EXTRA_ROUTES}
    ...
    ${output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX1}    ${CONFIG_EXTRA_ROUTE_IP1}    ${EXP_STR_IFCONFIG}
    Log    ${output}

    Log    "Configure extra route ip on VM22 - extra route host-2"
    ${output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP_2}    ${MININET_PASSWORD}    ${VM_INDX2}    ${CONFIG_EXTRA_ROUTE_IP2}    ${EXP_STR_IFCONFIG}
    Log    ${output}

    Log    "Verify datapath - extra route host-1"
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX2}    ${PING_EXTRA_ROUTE_IP1}    ${EXP_STR}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP_2}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_EXTRA_ROUTE_IP1}    ${EXP_STR}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}

    Log    "Verify datapath - extra route host-2"
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP}    ${MININET_PASSWORD}    ${VM_INDX2}    ${PING_EXTRA_ROUTE_IP2}    ${EXP_STR}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
    ${ping_output}    vpn.Run Command Nova Vm    ${TOOLS_SYSTEM_IP_2}    ${MININET_PASSWORD}    ${VM_INDX1}    ${PING_EXTRA_ROUTE_IP2}    ${EXP_STR}
    Log    ${ping_output}
    Should Match Regexp    ${ping_output}    ${PING_REGEX}
...
TC23 Verify E2E Datapath after restart of OVS
    ...
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP}    ${STOP_CMD}
    Log    ${output}
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP_2}    ${STOP_CMD}
    Log    ${output}
    Sleep    5
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP}    ${START_CMD}
    Log    ${output}
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP_2}    ${START_CMD}
    Log    ${output}
    Sleep    90
    Log    "Checking the OVS state and Flows after restart"
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP}    ${ovs-show}
    Log    ${output}
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP_2}    ${ovs-show}
    Log    ${output}
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP}    ${swdumpflows}
    Log    ${output}
    ${output}    vpn.Remotevmexec    ${TOOLS_SYSTEM_IP_2}    ${swdumpflows}
    Log    ${output}

...
Create And Configure Keystone Db 
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}       Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     keystone.*     keystone    ${HAPROXY_HOSTNAME}    keystone

Create And Configure Keystone Db Other Nodes
    [Arguments]    ${os_node_cxn}     ${mysql_user}     ${mysql_pass}     ${host_name}
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     keystone.*     keystone    ${host_name}      keystone
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     keystone.*     keystone    localhost     keystone
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     keystone.*     keystone    ${HAPROXY_HOSTNAME}   keystone
...
Install Configure Keystone
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/keystone/keystone.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${OS_CONTROL_1_HOSTNAME},openstack:rabbit@${OS_CONTROL_2_HOSTNAME},openstack:rabbit@${OS_CONTROL_3_HOSTNAME}"
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/keystone/keystone.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${host_name}"

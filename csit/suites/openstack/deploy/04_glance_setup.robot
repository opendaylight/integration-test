...
Create And Configure Glance Db 
    ...
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance    ${host_name}      glance
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance     localhost     glance
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance     ${HAPROXY_HOSTNAME}      glance
...
Install Configure Glance
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-api.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${OS_CONTROL_1_HOSTNAME},openstack:rabbit@${OS_CONTROL_2_HOSTNAME},openstack:rabbit@${OS_CONTROL_3_HOSTNAME}"
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-api.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${host_name}"
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-api.conf     keystone_authtoken    memcached_servers      ${OS_CONTROL_1_IP}:11211,${OS_CONTROL_2_IP}:11211,${OS_CONTROL_3_IP}:11211
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-api.conf     keystone_authtoken    memcached_servers      ${host_name}:11211
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-registry.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${OS_CONTROL_1_HOSTNAME},openstack:rabbit@${OS_CONTROL_2_HOSTNAME},openstack:rabbit@${OS_CONTROL_3_HOSTNAME}"
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-registry.conf    DEFAULT     transport_url    "rabbit://openstack:rabbit@${host_name}"
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-registry.conf     keystone_authtoken    memcached_servers      ${OS_CONTROL_1_IP}:11211,${OS_CONTROL_2_IP}:11211,${OS_CONTROL_3_IP}:11211
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/glance/glance-registry.conf     keystone_authtoken    memcached_servers      ${host_name}:11211
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install Rpm Package    ${os_node_cxn}      nfs-utils

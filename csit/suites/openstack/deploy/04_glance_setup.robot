...
Install Glance
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install Configure Glance      ${OS_CONTROL_2_IP}     ${OS_CONTROL_2_HOSTNAME}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install Configure Glance      ${OS_CONTROL_3_IP}     ${OS_CONTROL_3_HOSTNAME}
...
Create And Configure Glance Db 
    ...
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance    ${host_name}     ${mysql_pass}
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance     localhost    ${mysql_pass}
    Grant Privileges To Mysql Database     ${os_node_cxn}    ${mysql_user}     ${mysql_pass}     glance.*     glance     ${HAPROXY_HOSTNAME}     ${mysql_pass}
...
Create Openstack Elements
    ...
    Create User     glance     default    glance
    Role Add     service     glance     admin
    Create Service    glance      "Image Service"      image
    Create Endpoint    RegionOne    ${host_name}     image     public     9292
    Create Endpoint    RegionOne    ${host_name}     image     internal   9292
    Create Endpoint    RegionOne    ${host_name}     image     admin     9292
...
Install Configure Glance
    ...
    Chmod    ${os_node_cxn}     /etc/glance/glance-api.conf      640
    Chown    ${os_node_cxn}     /etc/glance/glance-api.conf      root:glance

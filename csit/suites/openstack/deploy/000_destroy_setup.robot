...
Destroy Openstack
    ...
    Run Command    ${os_node_cxn}    sudo yum remove -y haproxy
    Run Command    ${os_node_cxn}    sudo rm -rf /etc/nagios /etc/yum.repos.d/packstack_* /root/.my.cnf
    Run Command    ${os_node_cxn}    sudo rm -rf /etc/my.cnf.d /var/lib/mysql/ /var/lib/glance /var/lib/nova /etc/nova /etc/swift /etc/keystone /etc/haproxy /etc/openvswitch /etc/httpd
    Run Command    ${os_node_cxn}    sudo rm -rf /var/log/nova /var/log/neutron /var/log/rabbitmq /var/log/mariadb
    Run Command    ${os_node_cxn}    sudo rm -rf /var/lib/mysql/ /var/lib/glance /var/lib/nova /etc/nova /etc/swift

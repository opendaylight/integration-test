...
Configure Cluster Root Node
    ...
    Grant Privileges To Mysql Database      ${os_node_cxn}    root     mysql    *.*      ${hostuser}      haproxy    mysql
...
Add HAPROXY Entry for DB
    ...
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " default_backend os-db-vms"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      $'\n'backend os-db-vms
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " option httpchk"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " stick-table type ip size 2"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " stick on dst"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " timeout server 90s"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera1 ${OS_CONTROL_1_IP}:3306 check inter 1s port 9200"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera2 ${OS_CONTROL_2_IP}:3306 check inter 1s port 9200"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera3 ${OS_CONTROL_3_IP}:3306 check inter 1s port 9200"
...
Configure Mysql Cluster Check
    ...
    Grant Process To Mysql Database      ${os_node_cxn}    root     mysql    *.*      clustercheck_user      localhost     my_clustercheck_password
    Install Rpm Package    ${os_node_cxn}     xinetd
    Touch File     ${os_node-cxn}    /etc/xinetd.d/galera-monitor 

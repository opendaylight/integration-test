...
Configure Cluster Root Node
    ...
    Grant Privileges To Mysql Database      ${os_node_cxn}    root     mysql    *.*      ${hostuser}      ${HAPROXY_HOSTNAME}    mysql
...
Add HAPROXY Entry for DB
    ...
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      $'\n'backend os-db-vms
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " balance leastconn"
    ...
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera1 ${OS_CONTROL_1_IP}:3306 check port 9200"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera2 ${OS_CONTROL_2_IP}:3306 check port 9200"
    Append To File     ${os_node_cxn}      /etc/haproxy/haproxy.cfg      " server galera3 ${OS_CONTROL_3_IP}:3306 check port 9200"

...
Create Required Image and Flavor
    ...
    Run Command In Local Node   export https_proxy=${PROXY_TEST};wget "https://download.fedoraproject.org/pub/fedora/linux/releases/27/CloudImages/x86_64/images/Fedora-Cloud-Base-27-1.6.x86_64.qcow2" -O /tmp/fedora.qcow2;unset https_proxy
...
Setup Passwordless Ssh
    ...
    Run Command In Local Node     scp /tmp/vm_key ${user_name}@${node_ip}:~ -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null
    Run Command In Local Node     scp /tmp/vm_key.pub ${user_name}@${node_ip}:~ -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null

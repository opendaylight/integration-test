...
Install Neutron
    ...
    Run Keyword If    2 > ${NUM_CONTROL_NODES}      Sync Db And Start     ${OS_CONTROL_1_IP}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Sync Db And Start     ${OS_CONTROL_1_IP}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Sync Db And Start     ${OS_CONTROL_2_IP}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Sync Db And Start     ${OS_CONTROL_3_IP}
    ...
    Run Keyword If    '${ENABLE_COMPUTE_IN_CONTROL}' == 'yes'      Install Configure Neutron Compute      ${OS_CONTROL_1_IP}      ${OS_CONTROL_1_IP}      ${OS_CONTROL_1_IP}
    Run Keyword If    1 < ${NUM_COMPUTE_NODES}      Install Configure Neutron Compute      ${OS_COMPUTE_1_IP}      ${OS_COMPUTE_1_IP}      ${OS_CONTROL_1_HOSTNAME} 
    Run Keyword If    1 < ${NUM_COMPUTE_NODES}      Install Configure Neutron Compute      ${OS_COMPUTE_2_IP}      ${OS_COMPUTE_2_IP}      ${OS_CONTROL_1_HOSTNAME}
...
Install Configure Neutron
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Crudini Edit     ${os_node_cxn}       /etc/neutron/neutron.conf     DEFAULT     dhcp_agents_per_network     3
...
Install Configure Neutron Compute
    [Arguments]    ${os_node_cxn}     ${host_ip}     ${host_name}
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      auth_type     password
    Run Keyword If    2 > ${NUM_CONTROL_NODES}        Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      auth_url      http://${host_name}:35357
    Run Keyword If    2 < ${NUM_CONTROL_NODES}        Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      auth_url      http://${HAPROXY_HOSTNAME}:35357
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      username    neutron
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      password    neutron
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      user_domain_name     Default
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      project_domain_name     Default
    ...
    Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      region_name    RegionOne
    Run Keyword If    2 > ${NUM_CONTROL_NODES}       Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      url     http://${host_name}:9696
    Run Keyword If    2 < ${NUM_CONTROL_NODES}       Crudini Edit     ${os_node_cxn}       /etc/nova/nova.conf      neutron      url     http://${HAPROXY_HOSTNAME}:9696
...
Install OVS And Configure
    ...
    Run Command      ${os_node_cxn}      neutron-odl-ovs-hostconfig --config-file=/etc/neutron/neutron.conf --debug --noovs_dpdk --bridge_mappings="flat1:br-flat1,flat2:br-flat2,physnet1:br-physnet1" --local_ip ${host_ip}

...
Install Nova
    ...
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Generic HAProxy Entry      ${HAPROXY_IP}     ${HAPROXY_IP}     9696     neutron_api
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install OVS And Configure      ${OS_CONTROL_1_IP}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install OVS And Configure      ${OS_CONTROL_2_IP}
    Run Keyword If    2 < ${NUM_CONTROL_NODES}      Install OVS And Configure      ${OS_CONTROL_3_IP}
    Run Keyword If    1 < ${NUM_COMPUTE_NODES}      Install OVS And Configure      ${OS_COMPUTE_1_IP}
    Run Keyword If    1 < ${NUM_COMPUTE_NODES}      Install OVS And Configure      ${OS_COMPUTE_2_IP}
...
Create Openstack Elements
    [Arguments]    ${host_name}
    Source Local File 
    Create User     neutron    default   neutron
    Role Add     service     neutron   admin
    Create Service    nova    "Networking service"       network
    Create Endpoint    RegionOne    ${host_name}     compute    public      9696
    Create Endpoint    RegionOne    ${host_name}     compute    internal    9696
    Create Endpoint    RegionOne    ${host_name}     compute    admin       9696
...
Install OVS And Configure
    ...
    Run Command      ${os_node_cxn}      neutron-odl-ovs-hostconfig 
    Run Keyword If    2 < ${NUM_CONTROL_NODES}       Run Command      sudo ovs-vsctl set-manager tcp:${OS_CONTROL_1_IP}:6640 tcp:${OS_CONTROL_2_IP}:6640 tcp:${OS_CONTROL_3_IP} 

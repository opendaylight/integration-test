...
Get_Current_Term_Of_Shard_At_Member
    ...
    ${value_data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${verify_restconf}
    ${current_term_value} =    Collections.Get_From_Dictionary    ${value_data_object}    CurrentTerm
    [Return]    ${current_term_value}

Get_Last_Index_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${value_data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${verify_restconf}
    ${LastIndex} =    Collections.Get_From_Dictionary    ${value_data_object}    LastIndex
    [Return]    ${LastIndex}

Get_Last_Applied_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${value_data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${verify_restconf}
    ${LastApplied} =    Collections.Get_From_Dictionary    ${value_data_object}    LastApplied
    [Return]    ${LastApplied}

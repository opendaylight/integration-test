...
Get_Current_Term_Of_Shard_At_Member
    ...
    ${current_term_value} =    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    CurrentTerm    ${verify_restconf}
    [Return]    ${current_term_value}

Get_Last_Index_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${LastIndex} =    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    LastIndex    ${verify_restconf}
    [Return]    ${LastIndex}

Get_Last_Applied_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${LastApplied} =    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    LastApplied    ${verify_restconf}
    [Return]    ${LastApplied}

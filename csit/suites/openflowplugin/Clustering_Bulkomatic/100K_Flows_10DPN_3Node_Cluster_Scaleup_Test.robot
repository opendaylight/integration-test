...
Resource          ../../../libraries/ClusterPerformanceScaleup.robot
...
Check Shards Status And Initialize Variables
    ...
    ClusterPerformanceScaleup.Run_Karaf_Command_On_List_Or_All    ${karaf_log_level}
...
Check Switch State Prior To Connection
    ...
    ${status}=    Run Keyword And Return Status    Wait Until Keyword Succeeds    ${mininet_timeout}    2s    ClusterPerformanceScaleup.Verify_Switch_Connections_Running_On_Member    0
    ...    ${switch_state_pre_connection}    ${Follower_Node_1}
    Run Keyword If    '${status}' == 'False'    ClusterPerformanceScaleup.Clean_And_Stabilize_Mininet_System    ${switch_state_pre_connection}    ${TOOLS_SYSTEM_IP}
    ${status}=    Run Keyword And Return Status    Wait Until Keyword Succeeds    ${mininet_timeout}    2s    ClusterPerformanceScaleup.Verify_Switch_Connections_Running_On_Member    0
...
Start Mininet Connect To Follower Node
    ...
    Wait Until Keyword Succeeds    ${mininet_timeout}    1s    ClusterPerformanceScaleup.Stabilize_Mininet_System    ${switch_state_pre_connection}    ${TOOLS_SYSTEM_IP}
...
Verify Stable Switch State Post Connection
    ...
    Wait Until Keyword Succeeds    ${mininet_timeout}    2s    ClusterPerformanceScaleup.Verify_Switch_Connections_Running_On_Member    0    ${switch_state_pre_connection}    ${Follower_Node_1}
    Wait Until Keyword Succeeds    ${mininet_timeout}    2s    ClusterPerformanceScaleup.Verify_Switch_Connections_Running_On_Member    ${switch_count}    ${switch_state_post_connection}    ${Follower_Node_1}
...
Verify Flows In Oper DS
    ...
    Wait Until Keyword Succeeds    ${oper_ds_timeout}    2s    Check Flows Inventory Oper DS    ${switch_count}    ${flow_count_after_add}    ${Inventory_Leader}

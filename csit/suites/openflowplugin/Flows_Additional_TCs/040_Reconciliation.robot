...
${setcntl_cmd}    sudo ovs-vsctl set-controller s1 tcp:${CONTROLLER}:6653
${delcntl_cmd}    sudo ovs-vsctl del-controller s1
...
Log Switch Details
    Write    sudo ovs-ofctl dump-flows s${switchid} -O OpenFlow13
...
Fill Switch
    ...
    \    SSHLibrary    Write     sudo ovs-ofctl add-flow s${switchid} \"${flow}\" -O OpenFlow13

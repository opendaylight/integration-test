...
Add a group
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    ${REST_CONTEXT_CF}/group/1    headers=${HEADERS_XML}    data=${body}
    Log    ${resp.content}
    BuiltIn.Should_Match    "${resp.status_code}"    "20?"

Verify after adding group config
    [Documentation]    Get the group stat in config
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT_CF}/group/1
    Log    ${resp.content}
...
Add a flow that includes a group
    ...
    Set Suite Variable    ${body}
    ${resp}    RequestsLibrary.PUT On Session    session    ${REST_CONTEXT_CF}/table/0/flow/1    headers=${HEADERS_XML}    data=${body}
    Log    ${resp.content}
    BuiltIn.Should_Match    "${resp.status_code}"    "20?"
...
Verify after adding flow config
    ...
    [Tags]    Get
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT_CF}/table/0/flow/1
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Remove the flow
    ...
    ${resp}    RequestsLibrary.DELETE On Session    session    ${REST_CONTEXT_CF}/table/0/flow/1
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete the group
    ...
    [Tags]    Delete
    ${resp}    RequestsLibrary.DELETE On Session    session    ${REST_CONTEXT_CF}/group/1
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

...
Add a meter
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    ${REST_CONTEXT_CF}/meter/1    headers=${HEADERS_XML}    data=${body}
    Log    ${resp.content}
    BuiltIn.Should_Match    "${resp.status_code}"    "20?"

Verify after adding meter config
    [Documentation]    Get the meter stat in config
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT_CF}/meter/1
    Log    ${resp.content}
...
Add a flow that includes a meter
    ...
    Set Suite Variable    ${body}
    ${resp}    RequestsLibrary.PUT On Session    session    ${REST_CONTEXT_CF}/table/0/flow/2    headers=${HEADERS_XML}    data=${body}
    Log    ${resp.content}
    BuiltIn.Should_Match    "${resp.status_code}"    "20?"
...
Verify after adding flow config
    ...
    [Tags]    Get
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT_CF}/table/0/flow/2
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Remove the flow
    ...
    ${resp}    RequestsLibrary.DELETE On Session    session    ${REST_CONTEXT_CF}/table/0/flow/2
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete the meter
    ...
    [Tags]    Delete
    ${resp}    RequestsLibrary.DELETE On Session    session    ${REST_CONTEXT_CF}/meter/1
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

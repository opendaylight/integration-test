...
Get node 1 inventory
    ...
    ${list}    Create List    @{SW_CAPABILITIES}    ${SW_VENDOR}    ${SW_IPADDRESS}    ${SW_HARDWARE}    openflow%3A1%3A1
    ...    openflow%3A1%3A2
...
Get node 2 inventory
    [Documentation]    Get the inventory for a node
    ${list}    Create List    @{SW_CAPABILITIES}    ${SW_VENDOR}    ${SW_IPADDRESS}    ${SW_HARDWARE}    openflow%3A2%3A1
    ...    openflow%3A2%3A2    openflow%3A2%3A3
...
Get node 3 inventory
    [Documentation]    Get the inventory for a node
    ${list}    Create List    @{SW_CAPABILITIES}    ${SW_VENDOR}    ${SW_IPADDRESS}    ${SW_HARDWARE}    openflow%3A3%3A1
    ...    openflow%3A3%3A2    openflow%3A3%3A3
...
Remove Port
    ...
    Read Until    mininet>
    @{list}    Create List    openflow%3A2%3A1
...
Add Port
    ...
    Read Until    mininet>
    @{list}    Create List    openflow%3A2%3A4

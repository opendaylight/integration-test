...
Get list of nodes
    ...
    Wait Until Keyword Succeeds    10s    2s    Ensure All Nodes Are In Response    ${REST_CONTEXT}    ${node_list}

Check No Link Down
    [Documentation]    Check there is no link down. We have 8 ports in total: s1=2, s2=3, s3=3.
    Wait Until Keyword Succeeds    10s    2s    Check For Specific Number Of Elements At URI    ${REST_CONTEXT}    "link-down":false    8
...
Get node 1 inventory
    ...
    ...    openflow:1:2
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}/node/openflow:1    ${list}
...
Get node 2 inventory
    ...
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}/node/openflow:2    ${list}
...
Get node 3 inventory
    ...
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}/node/openflow:3    ${list}
...
Link Down
    ...
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}/node/openflow:1/node-connector/openflow:1:1    ${list}
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}/node/openflow:2/node-connector/openflow:2:3    ${list}
...
Remove Port
    ...
    Wait Until Keyword Succeeds    10s    2s    Check For Elements Not At URI    ${REST_CONTEXT}    ${list}
...
Add Port
    ...
    Wait Until Keyword Succeeds    10s    2s    Check For Elements At URI    ${REST_CONTEXT}    ${list}

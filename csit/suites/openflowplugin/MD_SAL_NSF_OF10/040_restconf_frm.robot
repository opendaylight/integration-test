...
Add a flow - Output to physical port#
    ...
    ${resp}    RequestsLibrary.PUT On Session    session    ${CONFIG_NODES_API}/node/openflow:1/table/0/flow/152    headers=${HEADERS_XML}    data=${BODY2}
    BuiltIn.Should_Match    "${resp.status_code}"    "20?"

Verify after adding flow config - Output to physical port#
    [Documentation]    Verify the flow
    ${resp}    RequestsLibrary.GET On Session    session    ${CONFIG_NODES_API}/node/openflow:1/table/0
...
Remove a flow - Output to physical port#
    ...
    ${resp}    RequestsLibrary.DELETE On Session    session    ${CONFIG_NODES_API}/node/openflow:1/table/0/flow/152
    Should Be Equal As Strings    ${resp.status_code}    200

...
Check Linear Topology After Owner Fail
    [Documentation]    Check Linear Topology.
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check Linear Topology    ${SWITCHES}

Remove Configuration In Owner and Verify After Fail
...
Check Flows In Operational DS After Owner Is Stopped
    [Documentation]    Check Flows in Operational DS after Owner is disconnected.
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check Number Of Flows    ${less_flows}    ${new_owner}

Check Groups In Operational DS After Owner Is Stopped
    [Documentation]    Check Groups in Operational DS after Owner is disconnected.
    Check Number Of Groups    ${less_groups}    ${new_owner}

Check Flows In Switch After Owner Is Disconnected
    [Documentation]    Check Flows in switch after Owner is disconnected
    MininetKeywords.Check Flows In Mininet    ${mininet_conn_id}    ${less_flows}
...
Check Flows After Owner Restar In Operational DS
    [Documentation]    Check Flows in Operational DS after mininet is reconnected.
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check Number Of Flows    ${all_flows}
...
Check Linear Topology After Adding Back Flow
    [Documentation]    Check Linear Topology after owner is restored.
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check Linear Topology    ${SWITCHES}
...
Check Flows In Operational DS After Controller Restarts
    ...
    BuiltIn.Wait Until Keyword Succeeds    300s    2s    Check Number Of Flows    ${less_flows}

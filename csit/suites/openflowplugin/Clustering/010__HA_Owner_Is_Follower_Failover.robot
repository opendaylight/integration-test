...
Add Flow In Leader And Verify
    ...
    ${uri}=    Set Variable    /restconf/config/opendaylight-inventory:nodes/node/openflow:1/table/0/flow/1
    ${body}=    OperatingSystem.Get File    ${CURDIR}/../../../variables/openflowplugin/simple_flow_1.json
    Put And Check At URI In Cluster    ${original_cluster_list}    ${original_leader}    ${uri}    ${body}
    ${dictionary}=    Create Dictionary    10.0.1.0/24=1
    Wait Until Keyword Succeeds    5s    1s    Check Item Occurrence At URI In Cluster    ${original_cluster_list}    ${dictionary}    ${body}

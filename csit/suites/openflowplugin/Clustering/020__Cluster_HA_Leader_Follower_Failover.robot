...
Get inventory Leader Before Leader Restart
    ...
    ${inventory_leader}    ${inventory_followers}     Get InvConfig Shard Status  ${original_cluster_list}
...
Start Mininet Connect To Leader
    ...
    ${mininet_conn_id}=    Start Mininet Single Controller    ${TOOLS_SYSTEM_IP}  ${ODL_SYSTEM_${inventory_leader}_IP} 
...
Add Flows In Leader and Verify Before Leader Restart
    ...
    Add Sample Flow And Verify    ${original_cluster_list}    ${inventory_leader}
...
Modify Flows In Leader and Verify Before Leader Restart
    ...
    Modify Sample Flow and Verify    ${original_cluster_list}   ${inventory_leader}
...
Delete Flows In Leader and Verify Before Leader Restart
    ...
    Delete Sample Flow and Verify    ${original_cluster_list}   ${inventory_leader}
...
Send RPC Add to Leader and Verify Before Leader Restart
    ...
    Send RPC Add Sample Flow and Verify    ${original_cluster_list}    ${inventory_leader}
...
Send RPC Delete to Leader and Verify Before Leader Restart
    ...
    Send RPC Delete Sample Flow and Verify    ${original_cluster_list}    ${inventory_leader}
...
Send RPC Add to Follower Node1 and Verify Before Leader Restart
    ...
    Send RPC Add Sample Flow and Verify    ${original_cluster_list}    ${follower_node_1}
...
Send RPC Delete to Follower Node2 and Verify Before Leader Restart
    ...
    Send RPC Delete Sample Flow and Verify    ${original_cluster_list}    ${follower_node_2}
...
Restart Leader From Cluster Node
    ...
    Kill Multiple Controllers           ${inventory_leader}
    Start Specific Controller     ${START_TIMEOUT}      ${inventory_leader_list}
...
Start Mininet Connect To Follower Node1
    ...
    ${mininet_conn_id}=    Start Mininet Single Controller    ${TOOLS_SYSTEM_IP}  ${ODL_SYSTEM_${follower_node_1}_IP}
    Set Suite Variable    ${mininet_conn_id}
...
Add Flows In Follower Node2 and Verify Before Follower Restart
    ...
    Add Sample Flow And Verify    ${original_cluster_list}    ${follower_node_2}
...
Modify Flows In Follower Node2 and Verify Before Follower Restart
    ...
    Modify Sample Flow and Verify    ${original_cluster_list}   ${follower_node_2}
...
Delete Flows In Follower Node2 and Verify Follower Restart
    ...
    Delete Sample Flow and Verify    ${original_cluster_list}   ${follower_node_2}
...
Restart Follower Node2
    [Documentation]    Kill Follower Node2 and Start it Up.
    Kill Multiple Controllers       ${follower_node_2}
    Start Specific Controller     ${START_TIMEOUT}      ${inventory_follower_node_2}
...
Start Mininet Connect To Follower Node2
    [Documentation]    Start mininet with connection to cluster Follower Node1.
    ${mininet_conn_id}=    Start Mininet Single Controller    ${TOOLS_SYSTEM_IP}  ${ODL_SYSTEM_${follower_node_2}_IP}
    Set Suite Variable    ${mininet_conn_id}
...
Add Flows In Follower Node1 and Verify Before Cluster Restart
    ...
    Add Sample Flow And Verify    ${original_cluster_list}    ${follower_node_1}
...
Modify Flows In Follower Node1 and Verify Before Cluster Restart
    ...
    Modify Sample Flow and Verify    ${original_cluster_list}   ${follower_node_1}
...
Delete Flows In Follower Node1 and Verify Before Cluster Restart
    ...
    Delete Sample Flow and Verify    ${original_cluster_list}   ${follower_node_1}
...
Send RPC Add to Follower Node2 and Verify Before Cluster Restart
    [Documentation]    Add Flow in Follower and verify it gets applied from all Controller instances.
    Send RPC Add Sample Flow and Verify    ${original_cluster_list}    ${follower_node_2}

Send RPC Delete to Follower Node1 and Verify Before Cluster Restart
    [Documentation]    Delete Flow in Follower and verify it gets removed from all Controller instances.
    Send RPC Delete Sample Flow and Verify    ${original_cluster_list}    ${follower_node_1}
...
Restart Full Cluster
    ...
    Kill Multiple Controllers         @{original_cluster_list}
    Start Multiple Controllers       ${START_TIMEOUT}        @{original_cluster_list}

...
Start Suite
    ...
    RequestsLibrary.Create Session    ${CTRL_REST_SESSION}    http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}    auth=${AUTH}    headers=${HEADERS_XML}
...
Is Jolokia Available
    ...
    RequestsLibrary.Create Session    n1    http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}
    RequestsLibrary.Create Session    n2    http://${ODL_SYSTEM_2_IP}:${RESTCONFPORT}
    RequestsLibrary.Create Session    n3    http://${ODL_SYSTEM_3_IP}:${RESTCONFPORT}

    : FOR    ${i}    IN RANGE    1    4
    \    ${resp_jolokia_status}=    RequestsLibrary.Get Request    ${i}    ${JOLOKIA_STATUS}

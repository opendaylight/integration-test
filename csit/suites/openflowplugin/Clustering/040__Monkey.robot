...
Start Suite
    ...
    SSHLibrary.Login With Public Key    ${TOOLS_SYSTEM_USER}    ${USER_HOME}/.ssh/id_rsa    any
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    sudo apt-get update -y
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    sudo apt-get install -y python-pip python-devel gcc libffi-devel python-devel openssl-devel   return_stdout=True    return_stderr=True    return_rc=True

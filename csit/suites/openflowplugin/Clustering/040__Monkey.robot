...
${START_MONKEY_CMD}      sudo python ${MONKEY_SCRIPT_FILE} --monkey-count ${MONKEY_COUNT} --interval ${MONKEY_INTERVAL} --run-time ${RUN_TIME} --logfile ${MONKEY_LOG_FILE} --dumpfile ${MONKEY_DUMP_FILE} --cluster-nodes ${ODL_SYSTEM_1_IP} ${ODL_SYSTEM_2_IP} ${ODL_SYSTEM_3_IP} --debug
...
Execute Monkey Tests
    ...
    SSHLibrary.Switch Connection    ${monkey_conn_id}
...
Get Monkey Test Logs
    ...
    ${log_file_content}=    Get Monkey Log    ${MONKEY_LOG_FILE}
    ${dump_log_file_content}=    Get Monkey Log    ${MONKEY_DUMP_FILE}
...
Start Suite
    ...
    ${monkey_conn_id}=    SSHLibrary.Open Connection    ${TOOLS_SYSTEM_IP}    prompt=${TOOLS_SYSTEM_PROMPT}
    SSHLibrary.Login With Public Key    ${TOOLS_SYSTEM_USER}    ${USER_HOME}/.ssh/id_rsa    any
    BuiltIn.Set Suite Variable    ${monkey_conn_id}
...
Get Monkey Log
    [Arguments]    ${log_file}
    SSHLibrary.File Should Exist    ${log_file}

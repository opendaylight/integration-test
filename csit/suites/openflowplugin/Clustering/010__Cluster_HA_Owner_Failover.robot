...
Check Shards Status Before Fail
    ...
    ClusterOpenFlow.Check OpenFlow Shards Status    ${ClusterManagement__member_index_list}
...
Check Entity Owner Status And Find Owner and Candidate Before Fail
    ...
    ${original_owner}    ${original_candidate_list}    ClusterOpenFlow.Get OpenFlow Entity Owner Status For One Device    ${ClusterManagement__member_index_list}    openflow:1
...
Reconnect Extra Switches To Candidate And Check Entity Owner
    ...
    ${owner}    ${candidate_list}    ClusterOpenFlow.Get OpenFlow Entity Owner Status For One Device    ${owner_list}    openflow:2
    Should Be Equal    ${owner}    ${original_candidate}
    ${owner}    ${candidate_list}    ClusterOpenFlow.Get OpenFlow Entity Owner Status For One Device    ${owner_list}    openflow:3
    Should Be Equal    ${owner}    ${original_candidate}
...
Check Network Operational Information Before Fail
    ...
    ClusterOpenFlow.Check OpenFlow Network Operational Information For Sample Topology    ${ClusterManagement__member_index_list}
...
Add Configuration In Owner and Verify Before Fail
    ...
    ClusterOpenFlow.Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_owner}
...
Modify Configuration In Owner and Verify Before Fail
    ...
    ClusterOpenFlow.Modify Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_owner}
...
Delete Configuration In Owner and Verify Before Fail
    ...
    ClusterOpenFlow.Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_owner}

Add Configuration In Candidate and Verify Before Fail
    [Documentation]    Add Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_candidate}

Modify Configuration In Candidate and Verify Before Fail
    [Documentation]    Modify Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Modify Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_candidate}
...
Delete Configuration In Candidate and Verify Before Fail
    ...
    ClusterOpenFlow.Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_candidate}

Send RPC Add to Owner and Verify Before Fail
    [Documentation]    Add Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Send RPC Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_owner}

Send RPC Delete to Owner and Verify Before Fail
    [Documentation]    Delete Flow in Owner and verify it gets removed from all instances.
    ClusterOpenFlow.Send RPC Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_owner}
...
Send RPC Add to Candidate and Verify Before Fail
    ...
    ClusterOpenFlow.Send RPC Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_candidate}
...
Send RPC Delete to Candidate and Verify Before Fail
    ...
    ClusterOpenFlow.Send RPC Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${original_candidate}

Modify Network And Verify Before Fail
    [Documentation]    Take a link down and verify port status in all instances.
    ClusterOpenFlow.Take OpenFlow Device Link Down and Verify    ${ClusterManagement__member_index_list}

Restore Network And Verify Before Fail
    [Documentation]    Take the link up and verify port status in all instances.
    ClusterOpenFlow.Take OpenFlow Device Link Up and Verify    ${ClusterManagement__member_index_list}
...
Check Entity Owner Status And Find Owner and Candidate After Fail
    ...
    ${new_owner}    ${new_candidate_list}    ClusterOpenFlow.Get OpenFlow Entity Owner Status For One Device    ${new_cluster_list}    openflow:1
...
Check Shards Status After Recover
    [Documentation]    Create original cluster list and check Status for all shards in OpenFlow application.
    ClusterOpenFlow.Check OpenFlow Shards Status After Cluster Event    ${ClusterManagement__member_index_list}

Check Entity Owner Status After Recover
    [Documentation]    Check Entity Owner Status and identify owner and candidate.
    ${new_owner}    ${new_candidates_list}    ClusterOpenFlow.Get OpenFlow Entity Owner Status For One Device    ${ClusterManagement__member_index_list}    openflow:1
    Set Suite Variable    ${new_owner}
...
Add Configuration In Owner and Verify After Recover
    [Documentation]    Add Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${new_owner}

Modify Configuration In Owner and Verify After Recover
    [Documentation]    Modify Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Modify Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${new_owner}

Delete Configuration In Owner and Verify After Recover
    [Documentation]    Delete Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${new_owner}
...
Send RPC Add to Owner and Verify After Recover
    [Documentation]    Add Flow in Owner and verify it gets applied from all instances.
    ClusterOpenFlow.Send RPC Add Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${new_owner}

Send RPC Delete to Owner and Verify After Recover
    [Documentation]    Delete Flow in Owner and verify it gets removed from all instances.
    ClusterOpenFlow.Send RPC Delete Sample Flow and Verify    ${ClusterManagement__member_index_list}    ${new_owner}
...
Check No Network Operational Information
    ...
    ClusterOpenFlow.Check No OpenFlow Network Operational Information    ${ClusterManagement__member_index_list}

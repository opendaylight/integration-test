...
Start Mininet Multiple Connections
    ...
    ${mininet_conn_id}=    Start Mininet Multiple Controllers    ${original_cluster_list}
...
Check Network Operational Information
    [Documentation]    Check device is in operational topology in all cluster instances. Inventory should show 1x node_id per device 1x node_id per connector.
    ...    Topology should show 2x node_id per device + 3x node_id per connector.
...
Add Flow In Owner and Verify
    ...
    Put And Check At URI In Cluster    ${original_cluster_list}    ${original_owner}    ${config_uri}    ${body}    ${HEADERS}
...
Add Flow In Candidate and Verify
    ...
    ${body}=    OperatingSystem.Get File    ${CURDIR}/../../../variables/openflowplugin/sample_flow_1.json
    ${dictionary}=    Create Dictionary    10.0.1.0/24=1
    Put And Check At URI In Cluster    ${original_cluster_list}    ${original_candidate}    ${config_uri}    ${body}    ${HEADERS}
    Wait Until Keyword Succeeds    10s    1s    Check Item Occurrence At URI In Cluster    ${original_cluster_list}    ${dictionary}    ${operational_uri}
...
Send RPC Add Flow to Owner and Verify
    ...
    ${dictionary}=    Create Dictionary    10.0.1.0/24=1
    ${resp}    RequestsLibrary.Post Request    controller${original_owner}    ${rpc_uri}    ${body}    ${HEADERS}
    Log    ${resp.content}
...
Send RPC Add Flow to Candidate and Verify
    ...
    ${body}=    OperatingSystem.Get File    ${CURDIR}/../../../variables/openflowplugin/add_flow_rpc.json
    ${dictionary}=    Create Dictionary    10.0.1.0/24=1
    ${resp}    RequestsLibrary.Post Request    controller${original_candidate}    ${rpc_uri}    ${body}    ${HEADERS}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Wait Until Keyword Succeeds    10s    1s    Check Item Occurrence At URI In Cluster    ${original_cluster_list}    ${dictionary}    ${operational_uri}
...
Check No Network Operational Information
    [Documentation]    Check device is not in operational topology in all cluster instances.

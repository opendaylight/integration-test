...
${orig_json_config_table_add}    sal_table_add_config.json
${orig_json_config_flows_add}    sal_table_flow_config.json
...
Initial Current Term Verification
    ...
    ${current_term_value}    ${data_object}    ${LastIndex}    ${LastApplied}    Get Lastindex And Lastapplied On Leader Node
    ${current_term_value_before}    Set Variable    ${current_term_value}
    BuiltIn.Log to console    Current Term is ${current_term_value_before}
    BuiltIn.Set Suite Variable    ${current_term_value_before}

Add Bulk Flow From Follower Get Bulk Flows And Verify In Leader Node
    ...
    Log    Initiate get operation and check flow count across cluster nodes
...
Current Term Verification After Adding Bulk Flow
    ...
    ${current_term_value}    ${data_object}    ${LastIndex}    ${LastApplied}    Get Lastindex And Lastapplied On Leader Node
    ${current_term_value_after}    Set Variable    ${current_term_value}
...
Delete All Flows From Follower Node And Verify No Flows In Leader Node
...
Check Shards Status And Initialize Variables
    ...
    ${temp_json_config_table_add}    BulkomaticKeywords.Set DPN And Flow Count In Json Add    ${orig_json_config_table_add}    ${switch_count}    ${flow_count_per_switch}
    ${temp_json_config_flows_add}    BulkomaticKeywords.Set DPN And Flow Count In Json Add    ${orig_json_config_flows_add}    ${switch_count}    ${flow_count_per_switch}
...
Get Lastindex And Lastapplied On Leader Node
    [Documentation]    Initiate get request for CurrentTerm value on Leader Node
    ${current_term_value}    ${data_object}    ClusterManagement.Get_Current_Term_Of_Shard_At_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${verify_restconf}
    ...
    [Return]    ${current_term_value}    ${data_object}    ${LastIndex}    ${LastApplied}

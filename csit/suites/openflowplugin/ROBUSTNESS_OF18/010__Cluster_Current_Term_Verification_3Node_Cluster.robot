...
Get_Current_Term_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${current_term_value}    ${data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${Current_Term}    ${verify_restconf}
    [Return]    ${current_term_value}    ${data_object}

Get_Last_Index_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${LastIndex}    ${data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${Last_Index}    ${verify_restconf}
    [Return]    ${LastIndex}

Get_Last_Applied_Of_Shard_At_Member
    [Documentation]    Find a leader and followers in the inventory config shard
    ${LastApplied}    ${data_object}    ClusterManagement.Get_Raft_Property_From_Shard_Member    ${shard_name}    ${shard_type}    ${Inventory_Leader}    ${Last_Applied}    ${verify_restconf}
    [Return]    ${LastApplied}

...
Library           Collections
Library           String
Library           ../../../libraries/OVSFlowDumpParser.py
...
${FLOW_BLASTER_FILE}             ${CURDIR}/../../../../tools/odl-mdsal-clustering-tests/clustering-performance-test/flow_config_blaster.py
${FLOW_BLASTER_TEMPLATE_FILE}    ${CURDIR}/../../../variables/openflowplugin/blast_flow_template.json
${FLOW_BLASTER_ARGS}             --auth --no-delete --flows ${FLOW_BLAST_COUNT} --host
...
RPC FlatBatch And FRM Failover Isolate With New Flows
   ...
   ${ds_leader_ip}=    BuiltIn.Set Variable    ${ODL_SYSTEM_${ds_leader_idx}_IP}
   ${follower_idx}=    BuiltIn.Evaluate    ${ds_followers_idx_list}[0]
   ${old_ds_leader_ip}=    Set Variable    ${ds_leader_ip}
   ...
   ClusterKeywords.Wait For Controller Sync     10 m    ${old_ds_leader_ip}
   BuiltIn.Should Be True       '${status_flows}' == 'PASS'
   BuiltIn.Should Be True       '${status_elections_sw}' == 'PASS'
...
Blast Flows
    ...
    SSHLibrary.Login With Public Key    ${DEFAULT_USER}    ${USER_HOME}/.ssh/id_rsa    any
    SSHLibrary.Start Command    python ${FLOW_BLASTER_FILE} ${FLOW_BLASTER_ARGS} ${ODL_SYSTEM_${odl_system_num}_IP} --file=${template_file}
    BuiltIn.Run Keyword Unless      ${current_conn_id}==${None}    SSHLibrary.Switch Connection    ${current_conn_id}
...
#    \    BuiltIn.Should Be Equal As Integers    ${resp.status_code}    200
#
#Log Active SSH Connection
#    ${active_connection}=    SSHLibrary.Get Connection
#    BuiltIn.Log    ${active_connection.alias} with index -> ${active_connection.index}

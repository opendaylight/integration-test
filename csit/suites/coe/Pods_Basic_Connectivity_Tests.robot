...
*** Variables ***
@{BB_NAMES}       ss1-busybox1    ss1-busybox2    ss1-busybox3    ss2-busybox4
...
Verify L2 Connectivity Between Pods
    ...
    Coe.Create Pods    ss1    ${BUSY_BOXES[0]}    ${BB_NAMES[0]}
    Coe.Create Pods    ss1    ${BUSY_BOXES[1]}    ${BB_NAMES[1]}
...
Verify L3 Connectivity Between Pods
    ...
    Coe.Create Pods    ss1    ${BUSY_BOXES[2]}    ${BB_NAMES[2]}
    Coe.Create Pods    ss2    ${BUSY_BOXES[3]}    ${BB_NAMES[3]}
    BuiltIn.Wait Until Keyword Succeeds    55s    2s    Coe.Check Pod Status Is Running
    Coe.Collect Pod Names and Ping

...
Create_Pods
    ...
    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    kubectl create -f ${yaml}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}

Ping_Pods
    [Arguments]    ${pods}
    [Documentation]    Ping pods to check connectivity between them
    ${lines} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    kubectl get pods -o wide    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}    ${DEFAULT_LINUX_PROMPT}
    ...
    \    ${ping} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_1_IP}    kubectl exec -it ${pod_1} -- ping -c 3 ${pod_ip}    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}

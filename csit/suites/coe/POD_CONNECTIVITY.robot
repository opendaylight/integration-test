...
*** Test Cases ***
ansible inventory
    ${a} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_IP}    ansible --version    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    log    ${a}
    ${b} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_2_IP}    ansible --version    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    log    ${b}
    ${c} =    Utils.Run Command On Remote System    ${TOOLS_SYSTEM_3_IP}    ansible --version    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    log    ${c}
    ...
    ${play}=    OperatingSystem.Get File    ${CURDIR}/../../variables/coe/coe_play.yaml
    ${play}=    String.Replace String    ${play}    confpath    ${CURDIR}/../../variables/coe/odlovs-cni.conf .j2
    OperatingSystem.Create File    ${CURDIR}/../../variables/coe/coe_play.yaml    ${play}

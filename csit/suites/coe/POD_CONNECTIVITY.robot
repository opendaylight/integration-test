...
Tunnel_Creation
    ...
    [Documentation]    Creates ITM tunnels between the nodes
    @{commands}=    Create List    sudo ovs-vsctl \ \ \ set O . other_config:local_ip=${NODE_IP}    sudo ovs-vsctl \ \ \ set O . external_ids:br-name=${bridge}    sudo ovs-vsctl show
    : FOR    ${cmd}    IN    @{commands}
    \    Run Command On Remote System    ${NODE_IP}    ${cmd}    ${NODE_USER}    ${NODE_PASSWORD}

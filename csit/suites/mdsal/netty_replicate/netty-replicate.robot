...
Setup_suite
    ...
    ClusterManagement.Run_Bash_Command_On_List_Or_All    command=pwd
    ClusterManagement.Run_Bash_Command_On_List_Or_All    command=ls
    ClusterManagement.Run_Bash_Command_On_List_Or_All    command=cat ${karaf_home}/configuration/initial/module-shards.conf
    ...
    ClusterManagement.Run_Bash_Command_On_List_Or_All    command=cat ${karaf_home}/configuration/initial/module-shards.conf
    ClusterManagement.Run_Bash_Command_On_List_Or_All    command=hostname -I
...
Setup_Test
    ...
    #KarafKeywords.Setup_Karaf_Keywords
    #ClusterManagement.Install_Feature_On_List_Or_All    features-controller odl-mdsal-exp-replicate-netty odl-restconf-nb-bierman02 odl-netconf-clustered-topology odl-clustering-test-app
    #BuiltIn.Sleep    10s
...
Open_Sink_Connection
    ...
    BuiltIn.Set_Variable    ${netty_sink_session_alias}
...
Put_Config_And_Verify
    ...
    #BuiltIn.Run_Keyword_If    ${netty_source_session_alias} == ${EMPTY}    BuiltIn.Fail    Could not put config, session to source node is not opened.
...
Delete_Config_And_Verify
    ...
    #BuiltIn.Run_Keyword_If    ${netty_source_session_alias} == ${EMPTY}    BuiltIn.Fail    Could not delete config, session to source node is not opened
    TemplatedRequests.Delete_Templated    ${template_folder}    mapping=${mapping}    session=${netty_source_session_alias}
    BuiltIn.Run_Keyword_If    ${verify_on_source}    Verify_Config_Is_Not_Present    template_folder=${template_folder}    mapping=${mapping}    session=${netty_source_session_alias}
    BuiltIn.Run_Keyword_If    ${verify_on_sink}    Verify_Config_Is_Not_Present    template_folder=${template_folder}    mapping=${mapping}    session=${netty_sink_session_alias}

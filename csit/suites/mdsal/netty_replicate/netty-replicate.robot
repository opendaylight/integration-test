...
*** Variables ***
@{REQUIRED_FEATURES}    features-controller    odl-mdsal-exp-replicate-netty    odl-restconf-nb-bierman02    odl-netconf-clustered-topology
...
Setup_suite
    ...
    ${features_string} =    BuiltIn.Catenate    @{REQUIRED_FEATURES}
...
Teardown_Suite
    [Documentation]    Reconfigure back clustering and clear data.
    ${members_index_list} =    List_Indices_Or_All
    FOR    ${cluster_member_index}    IN    @{members_index_list}
        ${member_ip_address} =    ClusterManagement.Resolve_Ip_Address_For_Member    ${cluster_member_index}
        Run_Bash_Command_On_Member    command=pushd ${karaf_home} && rm -rf ./configuration/initial/ && popd    member_index=${cluster_member_index}
        # restore old cluster configuration files
        Run_Bash_Command_On_Member    command=pushd ${karaf_home} && tar -xvf /tmp/config_backup.tar && popd    member_index=${cluster_member_index}
        Run_Bash_Command_On_Member    command=pushd ${karaf_home} && rm -f /tmp/config_backup.tar && popd    member_index=${cluster_member_index}
    END

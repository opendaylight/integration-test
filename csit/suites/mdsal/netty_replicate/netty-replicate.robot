...
Replicate_config_Update
    ...
    NettyReplication.Setup_Netty_Replication
    &{mapping} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping}
    &{mapping} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_updated}
...
Replicte_Config_Deletion
    ...
    &{mapping} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping}
    Delete_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping}
...
Replicate_Multiple_Changes_to_Config
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_1}
    &{mapping_2} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_2}
    &{mapping_2_updated} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_2_updated}
    Delete_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_1}
...
Replicate_Multiple_Changes_to_Multiple_Sinks
    ...
    &{mapping_1} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_1}    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}
    &{mapping_2} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_2}    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}
    &{mapping_2_updated} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_2_updated}    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}
    Delete_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping_1}
...
Sink_Catch_Up_After_Opening_Connection
    ...
    &{mapping} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping}    sink_node_indexes=@{EMPTY}
    ${netty_sink_session_alias} =    Resolve_Http_Session_For_Member    member_index=@{DEFAULT_NETTY_SINK_NODE_INDEXES}[0]
    Verify_Config_Is_Not_Present    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping}    session=${netty_sink_session_alias}    wait_time=5s
    NettyReplication.Open_Sink_Connection
    Verify_Config_Is_Present    ${CARPEOPLE_DEV_FOLDER}/people    mapping=${mapping}    session=${netty_sink_session_alias}
...
Reconnect_After_Lost_Connection
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping}    sink_node_indexes=@{EMPTY}
    ${netty_sink_session_alias} =    Resolve_Http_Session_For_Member    member_index=@{DEFAULT_NETTY_SINK_NODE_INDEXES}[0]
    Verify_Config_Is_Not_Present    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping}    session=${netty_sink_session_alias}    wait_time=5s
...
Change_Replication_Source
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_older}
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_newer}    source_node_index=${ADDITIONAL_SOURCE_NODE_INDEX}
    # check if data from old source was forgotten (removed)
    ${netty_sink_session_alias} =    Resolve_Http_Session_For_Member    member_index=@{DEFAULT_NETTY_SINK_NODE_INDEXES}[0]
    Verify_Config_Is_Not_Present    ${CARPEOPLE_DEV_FOLDER}/people    ${mapping_older}    session=${netty_sink_session_alias}    removal=True
...
Put_Config_And_Verify
    ...
    TemplatedRequests.Put_As_Json_Templated    ${template_folder}    session=${netty_source_session_alias}    iterations=${iterations}
...
Verify_Config_Is_Present
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Json_Templated    ${template_folder}    verify=True    session=${session}    iterations=${iterations}
...
Delete_Config_And_Verify
    ...
    ${netty_source_session_alias} =    Resolve_Http_Session_For_Member    member_index=${source_node_index}
    TemplatedRequests.Delete_Templated    ${template_folder}    session=${netty_source_session_alias}    iterations=${iterations}

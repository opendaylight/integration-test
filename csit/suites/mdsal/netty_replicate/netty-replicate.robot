...
Replicate_config_Update
    ...
    NettyReplication.Setup_Netty_Replication
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    iterations=${5}    j_offset=${0}
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    iterations=${5}    j_offset=${5}
...
Replicate_Multiple_Changes_to_Config
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    iterations=${7}    j_offset=${0}
    &{mapping_2_updated} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    iterations=${7}    j_offset=${2}
    Delete_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people
...
Replicate_Multiple_Changes_to_Multiple_Sinks
    ...
    &{mapping_1} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}    iterations=${5}    j_offset=${0}
    &{mapping_2} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}    iterations=${7}    j_offset=${0}
    &{mapping_2_updated} =    BuiltIn.Create_Dictionary
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    sink_node_indexes=@{MULTIPLE_SINK_NODES_INDEXES}    iterations=${8}    j_offset=${2}
    Delete_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people
...
Change_Replication_Source
    ...
    Put_Config_And_Verify    ${CARPEOPLE_DEV_FOLDER}/people    source_node_index=${ADDITIONAL_SOURCE_NODE_INDEX}    iterations=${8}
...
Put_Config_And_Verify
    [Arguments]    ${template_folder}    ${mapping}={}    ${source_node_index}=${DEFAULT_NETTY_SOURCE_NODE_INDEX}    @{sink_node_indexes}=${DEFAULT_NETTY_SINK_NODE_INDEXES}    ${iterations}=${1}    ${j_offset}=${0}
    ...
    TemplatedRequests.Put_As_Json_Templated    ${template_folder}    session=${netty_source_session_alias}    iterations=${iterations}    j_offset=${j_offset}
    Verify_Config_Is_Present    template_folder=${template_folder}    mapping=${mapping}    session=${netty_source_session_alias}    iterations=${iterations}    j_offset=${j_offset}
    FOR    ${sink_node_index}    IN    @{sink_node_indexes}
        ...
        Verify_Config_Is_Present    template_folder=${template_folder}    mapping=${mapping}    session=${netty_sink_session_alias}    iterations=${iterations}    j_offset=${j_offset}
    END

Verify_Config_Is_Present
    [Arguments]    ${template_folder}    ${session}    ${mapping}={}    ${iterations}=${1}    ${j_offset}=${0}
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Json_Templated    ${template_folder}    verify=True    session=${session}    iterations=${iterations}    j_offset=${j_offset}

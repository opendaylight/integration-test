...
*** Variables ***
${REPLICATE_SOURCE_INDEX}    1
${REPLICATE_SINK_INDEX}    2
...
Single_Config_Change_Replication
    ...
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device    DEVICE_PORT=1830    DEVICE_IP=192.168.20.20    DEVICE_USER=admin    DEVICE_PASSWORD=admin
...
Sink_Catch_Up_After_Opening_Connection
    [Documentation]    Sink should catch up to changes made on source node before connecting sink
...
Reconnect_After_Lost_Connection
    [Documentation]    Test if changes made during lost connection are resent to reconnected sink.
    Setup_Netty_Replication
    ClusterMangament.Isolate_Member_From_List_Or_All    ${1}
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device    DEVICE_PORT=1830    DEVICE_IP=192.168.20.20    DEVICE_USER=admin    DEVICE_PASSWORD=admin
    TemplatedRequests.Put_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping} verify=True session=${source_odl_session_alias}
    ClusterMangament.Rejoin_Member_From_List_Or_All    ${1}
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping} verify=True session=${sink_odl_session_alias}
...
Setup_Test
    ...
    ClusterManagment.Clean_Journals_Data_And_Snapshots_On_List_Or_All
...
Setup_Netty_Replication
    [Arguments]    ${source_memeber_index}=${REPLICATE_SOURCE_INDEX} ${sink_member_index}=${REPLICATE_SINK_INDEX}
...
Open_Source_Connection
    [Arguments]    ${cluster_member_index}=${REPLICATE_SINK_INDEX}
...
Close_Source_Connection
    ...
    KarafKeywords.Execute_Controller_Karaf_Command_On_Background    config:update member_index=${cluster_member_index}
    ${source_odl_session_alias} = ${EMPTY}

Open_Sink_Connection
    [Arguments]    ${cluster_member_index}=${REPLICATE_SOURCE_INDEX}

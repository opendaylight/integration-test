...
Single_Config_Change_Replication
    ...
    ${result} =    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=False    session=${source_odl_session_alias}
    ${result} =    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=False    session=${sink_odl_session_alias}

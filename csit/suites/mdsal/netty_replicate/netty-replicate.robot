...
Single_Config_Change_Replication
    ...
    Verify_Feature_Is_Installed     odl-netconf-clustered-topology    controller=${DEFAULT_SOURCE_INDEX}
    Verify_Bundle_Is_Installed    odl-netconf-clustered-topology    controller=${DEFAULT_SOURCE_INDEX}
    Install_A_Feature    odl-netconf-clustered-topology    controller=${DEFAULT_SOURCE_INDEX}
    Setup_Netty_Replication
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device-1
...
Sink_Catch_Up_After_Opening_Connection
    ...
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device-2
    BuiltIn.Log_Variables
    ${connections} =    SSHLibrary.Get_Connections
    BuiltIn.Log    ${connections}
    TemplatedRequests.Put_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${source_odl_session_alias}
...
Reconnect_After_Lost_Connection
    ...
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device-3
    BuiltIn.Log_Variables
    ${connections} =    SSHLibrary.Get_Connections
    BuiltIn.Log    ${connections}
    TemplatedRequests.Put_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${source_odl_session_alias}

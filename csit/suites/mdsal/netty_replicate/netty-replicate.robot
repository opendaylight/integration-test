...
Replicte_Config_Deletion
    ...
    TemplatedRequests.Delete_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${source_odl_session_alias}
    ${uri} =    TemplatedRequests.Resolve_Text_From_Template_Folder    folder=${NETCONF_DEV_FOLDER}     base_name=location    extension=uri    mapping=${mapping}
    Utils.No_Content_From_URI    ${source_odl_session_alias}    ${uri}
    Utils.No_Content_From_URI    ${sink_odl_session_alias}    ${uri}
    ${result} =    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=True    session=${source_odl_session_alias}
    ${result} =    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=True    session=${sink_odl_session_alias}
...
Change_Replication_Source
    ...
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device-4    DEVICE_IP=${DEVICE_IP}    DEVICE_PORT=1212    DEVICE_USER="Newer"    DEVICE_PASSWORD="Newer"
    TemplatedRequests.Put_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${source_odl_session_alias}
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=True    session=${source_odl_session_alias}
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    verify=True    session=${sink_odl_session_alias}
...
Put_Config_And_Verify
    ...
    TemplatedRequests.Put_As_Xml_Templated    ${template_folder}    mapping=${mapping}    session=${source_odl_session_alias}
    BuiltIn.Run_Keyword_If    ${verify_on_source}    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${template_folder}     mapping=${mapping}    verify=True    session=${source_odl_session_alias}
    BuiltIn.Run_Keyword_If    ${verify_on_sink}    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    TemplatedRequests.Get_As_Xml_Templated    ${template_folder}     mapping=${mapping}    verify=True    session=${sink_odl_session_alias}

...
Sink_Catch_Up_After_Opening_Connection
    ...
    Verify_Config_Is_Not_Present    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${netty_sink_session_alias}    time_lenght=5s
...
Reconnect_After_Lost_Connection
    ...
    Setup_Netty_Replication
    &{mapping} =    BuiltIn.Create_Dictionary    DEVICE_NAME=new-netconf-device-2    DEVICE_IP=${DEVICE_IP}    DEVICE_PORT=${DEVICE_PORT}    DEVICE_USER=${DEVICE_USER}    DEVICE_PASSWORD=${DEVICE_PASSWORD}
    Verify_Config_Is_Not_Present    ${NETCONF_DEV_FOLDER}    ${mapping}    session=${netty_sink_session_alias}    time_lenght=5s
...
Verify_Config_Is_Not_Present
    [Arguments]    ${template_folder}    ${mapping}    ${session}    ${until_removal}=False    ${time_lenght}=0s
    ...
    ...    time_lenght - Retries for specific time if config is not present the whole time (in case of config addition when config might appear after some time)
    BuiltIn.Should_Not_Be_Empty    ${session}    Could not verify, session to node is not opened
    ...
    BuiltIn.Run_Keyword_If    ${until_removal}    BuiltIn.Wait_Until_Keyword_Succeeds    10x    3s    Utils.No_Content_From_URI    ${session}    ${uri}
    ...    ELSE_IF    ${time_lenght} != 0s    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    10s    1s    Utils.No_Content_From_URI    ${session}    ${uri}

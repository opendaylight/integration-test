...
Sink_Catch_Up_After_Opening_Connection
    ...
    ${netty_sink_session_alias} =    Resolve_Http_Session_For_Member    member_index=${DEFAULT_NETTY_SINK_NODE_INDEX}
...
Reconnect_After_Lost_Connection
    ...
    NettyReplication.Setup_Netty_Replication
    ClusterManagement.Isolate_Member_From_List_Or_All    ${DEFAULT_NETTY_SOURCE_NODE_INDEX}    protocol=tcp    sport=9999
    ...
    ClusterManagement.Rejoin_Member_From_List_Or_All    ${DEFAULT_NETTY_SOURCE_NODE_INDEX}    protocol=tcp    sport=9999
    #Verify_Config_Is_Not_Present    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${netty_sink_session_alias}    wait_time=5s
    Verify_Config_Is_Present    ${NETCONF_DEV_FOLDER}    mapping=${mapping}    session=${netty_sink_session_alias}
    # In case test fails before reachicng rejoin
    [Teardown]    ClusterManagement.Rejoin_Member_From_List_Or_All    ${DEFAULT_NETTY_SOURCE_NODE_INDEX}    protocol=tcp    sport=9999
...
Change_Replication_Source
    ...
    NettyReplication.Setup_Netty_Replication    source_node_index=${ADDITIONAL_SOURCE_NODE_INDEX}
    ...
    Verify_Config_Is_Not_Present    ${NETCONF_DEV_FOLDER}    ${mapping_older}    ${DEFAULT_NETTY_SINK_NODE_INDEX}
...
Delete_Config_And_Verify
    ...
    FOR    ${sink_node_index}    IN    @{sink_node_indexes}
        ${netty_sink_session_alias} =    Resolve_Http_Session_For_Member    member_index=${sink_node_index}
        Verify_Config_Is_Not_Present    template_folder=${template_folder}    mapping=${mapping}    session=${netty_sink_session_alias}    until_removal=True
    END

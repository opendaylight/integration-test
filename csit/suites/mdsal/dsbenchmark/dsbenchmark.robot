...
Set Karaf Log Levels
    ...
    ${status_write}    ${message_write}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Write    log:set ${ODL_LOG_LEVEL}
    ${status_wait}    ${message_wait}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Read Until Prompt
    KarafKeywords.Restore_Current_SSH_Connection_From_Index    ${current_SSH_connection}

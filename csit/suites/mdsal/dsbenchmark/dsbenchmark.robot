...
Start_Benchmark_Tool
    ...
    ${command}=    BuiltIn.Set_Variable    python ${tool} --host ${CONTROLLER} --port ${RESTCONFPORT} --warmup ${WARMUP} --runs ${RUN} --total ${COUNT} --inner ${INNER} --txtype ${TX_TYPE} --ops ${OP_COUNT} --optype ${OP_TYPE} --plot ${FILTER} --units ${UNITS} ${tool_args} &> ${tool_log_name}
    BuiltIn.Log    ${command}
    ${output}=    SSHLibrary.Write    ${command}
    ${status}    ${message}=    BuiltIn.Run Keyword And Ignore Error    Read Until Prompt

...
...
...               This test suite uses the odl-dsbenchmark-impl feature controlled via dsbenchmark.py tool (see the
...
...               for testing the MD-SAL Data Store performance.
...
...
...               (Start Measurement, Wait For Results) and basic cheks for test runs results
...               (Check Results). Finally it provides total numbers per operation type 
...               (by default in the perf_per_struct.csv, perf_per_ops.csv files)
...               suitable for plotting in system test environment ... see also
...
...               Included totals can be filtered using the FILTER parameter. 
...               Because of the way how graphs are build it is recomended to keep 
...               all test suite variables as defined for 1st build.

...
${tool_plot1}    perf_per_struct.csv
${tool_plot2}    perf_per_ops.csv
...
Wait For Results
    [Documentation]    Wait until data to plot are available
    Wait_Until_File_Is_Created    ${tool_plot1}    ${TEST_TIMEOUT}
    Wait_Until_File_Is_Created    ${tool_plot2}    5s

Store Results
    [Documentation]    Store data to plot
    Store_File    ${tool_plot1}
    Store_File    ${tool_plot2}
...
Start_Benchmark_Tool
    [Documentation]    Start the benchmark tool.
    ${command}=    BuiltIn.Set_Variable    python ${tool} --host ${CONTROLLER} --port ${RESTCONFPORT} --warmup ${WARMUP} --runs ${RUN} --total ${COUNT} --inner ${ELEMENTS} --txtype ${TX_TYPE} --ops ${OP_COUNT} --optype ${OP_TYPE} --plot ${PLOT_FILTER} ${tool_args} &> ${tool_log}
    BuiltIn.Log    ${command}

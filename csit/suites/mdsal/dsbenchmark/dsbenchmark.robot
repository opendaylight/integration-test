...
Start_Benchmark_Tool
    ...
    ${command}=    BuiltIn.Set_Variable    python ${benchmark_tool} --host ${CONTROLLER} --port ${RESTCONFPORT} --total ${TOTAL_NUMBER} --warmup ${WARMUP} --runs ${RUNS} ${benchmark_tool_arguments} &> ${benchmark_tool_log}
    BuiltIn.Log    ${command}

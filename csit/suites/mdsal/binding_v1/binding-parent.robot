...
${MAVEN_OPTIONS}     -Dmaven.compile.fork=true -DskipTests -Dcheckstyle.skip=true -Dmaven.javadoc.skip=true -Dmaven.site.skip=true -DgenerateReports=false
...
Install_Maven
    [Documentation]    Download and unpack Maven (default version), prepare launch command with proper Java version.
    ...    Optionally, perform a multipatch build to populate local Maven repository with patched artifacts.
    NexusKeywords.Install_Maven
    ${patches} =    BuiltIn.Get_Variable_Value    \${PATCHES_TO_BUILD}    ${EMPTY}    # Avoid failure if the variable is not set.
    BuiltIn.Pass_Execution_If    """${patches}""" == ""    No post-install build requested.
    ${branches} =    BuiltIn.Get_Variable_Value    \${BRANCH}    ${EMPTY}
    BuiltIn.Run_Keyword_If    """${branches}""" == ""    BuiltIn.Fail    BRANCH needs to be specified for multipatch builds.
    ...
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    export WORKSPACE='${WORKSPACE}' && export BRANCH='${branches}' && export PATCHES_TO_BUILD='${patches}' && bash '${script_name}'
...
Run_Maven
    ...
    Run_Maven    pom_file=${POM_FILENAME}
...
Run_Maven
    [Arguments]    ${pom_file}=pom.xml
    [Documentation]    Determine arguments to use and call mavan command with given pom file.
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    mkdir -p '${MAVEN_REPOSITORY_PATH}'
    ${maven_additional_options} =    BuiltIn.Set_Variable    -Dmaven.repo.local=${MAVEN_REPOSITORY_PATH} -Dorg.ops4j.pax.url.mvn.localRepository=${MAVEN_REPOSITORY_PATH}
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    ${maven_bash_command} clean install dependency:tree -V -B -DoutputFile=dependency_tree.log -s './settings.xml' -f '${POM_FILENAME}' ${MAVEN_OPTIONS} ${maven_additional_options} > '${MAVEN_OUTPUT_FILENAME}'

...
${MAVEN_SETTINGS_URL}    http://git.opendaylight.org/gerrit/gitweb?p=integration/distribution.git;a=blob_plain;f=pom.xml;hb=refs/heads/master -O settings.xml
${MAVEN_VERSION}    3.3.9
...
Install_Maven
    ...
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    wget -N '${MAVEN_SETTINGS_URL}'    stderr_must_be_empty=False
...
#    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    cp -v 'settings.xml' '${MAVEN_REPOSITORY_PATH}'
    ${maven_options} =    BuiltIn.Set_Variable    -X -s './settings.xml' -Dmaven.repo.local=${MAVEN_REPOSITORY_PATH} -Dorg.ops4j.pax.url.mvn.localRepository=${MAVEN_REPOSITORY_PATH} -DskipTests -Dcheckstyle.skip=true -Dmaven.javadoc.skip=true -Dmaven.site.skip=true -DgenerateReports=false
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    ${maven_bash_command} clean install dependency:tree -V -B -DoutputFile=dependency_tree.log -f '${POM_FILENAME}' ${maven_options} > '${MAVEN_OUTPUT_FILENAME}'

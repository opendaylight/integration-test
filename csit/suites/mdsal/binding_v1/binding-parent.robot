...
Install_Maven
    ...
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    export WORKSPACE='${WORKSPACE}' && export BRANCH='${branches}' && export PATCHES_TO_BUILD='${patches}' && bash '${script_name}'    stderr_must_be_empty=False
...
Run_Maven
    ...
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    ${maven_bash_command} clean install dependency:tree -V -B -DoutputFile=dependency_tree.log -s './settings.xml' -f '${POM_FILENAME}' ${MAVEN_OPTIONS} ${maven_repository_options} > '${MAVEN_OUTPUT_FILENAME}'

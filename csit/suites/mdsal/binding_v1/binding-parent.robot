...
Prepare_Yang_Files_To_Test
    ...
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    mkdir src/main
...
Run_Maven
    ...
    ${final_pom} =    TemplatedRequests.Resolve_Text_From_Template_File    filepath=${BINDING_TEMPLATE_FILEPATH}    mapping={"BINDING_PARENT_VERSION":"${binding_parent_version}"}
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    echo '${final_pom}' > '${POM_FILENAME}'
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    mvn clean install dependency:tree -V -B -DoutputFile=dependency_tree.log -f '${POM_FILENAME}' -DskipTests -Dcheckstyle.skip=true -Dmaven.javadoc.skip=true -Dmaven.site.skip=true -DgenerateReports=false > tee '${MAVEN_OUTPUT_FILENAME}'

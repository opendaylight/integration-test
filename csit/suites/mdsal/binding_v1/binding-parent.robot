...
${MAVEN_OPTIONS}    -Dmaven.repo.local=/tmp/r -Dorg.ops4j.pax.url.mvn.localRepository=/tmp/r -DskipTests -Dcheckstyle.skip=true -Dmaven.javadoc.skip=true -Dmaven.site.skip=true -DgenerateReports=false 
...
#    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    set -o pipefail && ${maven_bash_command} clean install dependency:tree -V -B -DoutputFile=dependency_tree.log -f '${POM_FILENAME}' ${MAVEN_OPTIONS} | tee '${MAVEN_OUTPUT_FILENAME}'
    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    ${maven_bash_command} clean install dependency:tree -V -B -s 'settings.xml' -DoutputFile=dependency_tree.log -f '${POM_FILENAME}' ${MAVEN_OPTIONS} > '${MAVEN_OUTPUT_FILENAME}'

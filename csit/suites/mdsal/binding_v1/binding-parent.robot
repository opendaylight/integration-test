...
${JAVA_OPTIONS}    -Xmx3g    # Note that '-Xmx=2g' is wrong syntax.
...
Install_Maven
    ...
    ${java_actual_options} =    BuiltIn.Set_Variable_If    """${JDKVERSION}""" == "openjdk7"    ${JAVA_OPTIONS} ${JAVA_PERM}    ${JAVA_OPTIONS}
    BuiltIn.Set_Suite_Variable    \${maven_bash_command}    export JAVA_HOME='${java_home}' && export MAVEN_OPTS='${java_actual_options}' && ./${maven_directory}/bin/mvn
...
#    SSHKeywords.Execute_Command_At_Cwd_Should_Pass    cp -v 'settings.xml' '${MAVEN_REPOSITORY_PATH}'
    ${maven_options} =    BuiltIn.Set_Variable    -Dmaven.repo.local=${MAVEN_REPOSITORY_PATH} -Dorg.ops4j.pax.url.mvn.localRepository=${MAVEN_REPOSITORY_PATH}

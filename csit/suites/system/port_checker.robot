...
Get List Of Available Features
    ...
    \    @{feature_line}=    Split String    ${line}
    \    ${feature_name}=    Set Variable    @{feature_line}[0]
    # \    ${feature}    Fetch From Left    ${line}    ${SPACE}
    # need to make sure the prompt doesn't end up as a feature
    \    Run Keyword If    "opendaylight-user" not in "${feature_name}"    Append To List    ${ODL_FEATURES}    ${feature_name}
...
Install Feature And Wait Until Ports Have All Come Up
    ...
    \    Issue Command On Karaf Console    feature:install ${feature}    timeout=180s

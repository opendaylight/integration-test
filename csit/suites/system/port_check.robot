...
@{features_override}    odl-openflowplugin-flow-services-li    odl-vpnservice-openstack    odl-netide-rest
# @{features_override}

# common port numbers that will be bound to by multiple odl-features (numerical order)
@{clustering}                                   2550
@{openflow}                                     6633    6653
@{ovsdb}                                        6640
@{web-portal}                                   8080
...
# features (alphabetical order) where
@{ports-odl-aaa-api}
@{ports-odl-aaa-authn}    @{clustering}    @{web-portal}    @{restconf}
@{ports-odl-openflowplugin-flow-services-li}    @{clustering}    @{openflow}
@{ports-odl-vpnservice-openstack}    @{clustering}    @{web-portal}    @{restconf}    @{openflow}    @{ovsdb}

...
...               http authentication methods, based on credential pybot arguments.
...
...
Put_Updater
    [Documentation]    Open SSH session to updater VM, copy the utility there, including dependency.
    BuiltIn.Run_Keyword_If    ${UPDATER_COLOCATED}    BuiltIn.Set_Suite_Variable    ${UPDATERVM_IP}    ${CONTROLLER}
    ...
    SSHLibrary.Put_File    ${CURDIR}/../../../libraries/Counter.py    ${UPDATERVM_WORKSPACE}/

Save_And_Enable_Tcp_Rw_Reuse
    [Documentation]    Temporarily enable TCP port reuse on Updater VM to allow for high rate of TCP connections.  Do not start failing fast.
    ${old_value} =    SSHLibrary.Execute_Command    cat /proc/sys/net/ipv4/tcp_tw_reuse
    # The next line may be skipped on failure.
...
Restore_Tcp_Rw_Reuse
    [Setup]    Run_Even_When_Failing_Fast
    [Documentation]    Restore the old value if enabling TCP reuse was successful on Updater VM.
...
Updater
    ...
    # ${cmd} =    echo "still alive"
    ${cmd} =    BuiltIn.Evaluate    '''import collections\nprint dir(collections)\nquit()\n'''
    ${output} =    SSHLibrary.Execute_Command    python -c '${cmd}'
    ...
    Set_Hop    ${iteration}
    SSHLibrary.Switch_Connection    updater
    ${response} =    SSHLibrary.Execute_Command    taskset 0x00000001 python ${UPDATERVM_WORKSPACE}/updater.py --workers '${workers}' --odladdress '${CONTROLLER}' --user '${RESTCONF_USER}' --password '${RESTCONF_PASSWORD}' --scope '${RESTCONF_SCOPE}' --pccaddress '${FIRST_PCC_IP}' --pccs '${PCCS}' --lsps '${LSPS}' --hop '${hop}' --timeout '${UPDATER_TIMEOUT}' --refresh '${UPDATER_REFRESH}' --reuse '${RESTCONF_REUSE}' 2>&1
    BuiltIn.Log    ${response}

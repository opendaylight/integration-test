...
PCEP Sessions Flapped with LSP updates
    ...
    FOR    ${devices}    IN RANGE    ${INIT_PCC_DEVICE_COUNT}    ${MAX_PCC_DEVICE_COUNT+1}    ${PCC_DEVICE_INCREMENT}
        Flap Pcc Mock Continuously with given PCC sessions    127.1.0.0    ${devices}    1
    END
...
PCEP Sessions Flapped alongside LSP updates
    ...
    FOR    ${devices}    IN RANGE    ${INIT_PCC_DEVICE_COUNT}    ${MAX_PCC_DEVICE_COUNT+1}    ${PCC_DEVICE_INCREMENT}
        Flap Pcc Mock parallelly with given PCC sessions    127.1.0.0    ${devices}    1
        BGPcliKeywords.Stop_Console_Tool_And_Wait_Until_Prompt
    END
    [Teardown]    Run Keywords    Kill all pcc mock simulator processes    AND    BGPcliKeywords.Stop_Console_Tool_And_Wait_Until_Prompt
...
Flap Pcc Mock Continuously with given PCC sessions
...
Flap Pcc Mock parallelly with given PCC sessions
    [Arguments]    ${mock-ip}=${FIRST_PCC_IP}    ${pccs}=${PCCS}    ${lsps}=${LSPS}
    SSHLibrary.Switch_Connection    pccmock
...
Kill all pcc mock simulator processes
    Kill Process    /home/${ODL_SYSTEM_USER}/${mock-location}
    Kill Process    bash -c sh /tmp/mock.sh
    Kill Process    sh /tmp/mock.sh

Get Running Process Id
    [Arguments]    ${keyword}    # The keyword to use when searching for a process.
    [Documentation]    Looks up the process ID (pid) of a running process.
    Comment    Print out all running process for debugging
    ${rc}    ${std_out}    Execute SSH Command    .    sudo ps aux | grep -v grep
    Log    ${std_out}
    ${rc}    ${std_out}    Execute SSH Command    .    sudo ps aux | grep '${keyword}' | grep -v grep | awk '{ print $2 }'
    Comment    For return to be scalar
    ${std_out}    Convert List    ${std_out}
    @{pid}    String.Split To Lines    ${std_out}
    ${len}    Get Length    ${pid}
    ${is_running}    Set Variable If    ${len} == 0    False    True
    [Return]    ${pid}    ${is_running}    # Returns the process ID (or empty) and a boolean representing if the process is running.

Kill Process
    [Arguments]    ${keyword}
    [Documentation]    Kills (-9) ALL processes on the remote system that match the given keyword.
    ${process_id}    ${is_running}    Get Running Process Id    ${keyword}
    FOR    ${pid}    IN    @{process_id}
        Run Keyword If    ${is_running}    Log    Killing ${pid}
        Run Keyword If    ${is_running}    Execute SSH Command    .    sudo kill -9 ${pid}
    END

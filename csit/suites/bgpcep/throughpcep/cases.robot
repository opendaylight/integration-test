...
Updater_with delegate
    ...
    [Tags]    critical
    Updater    10    1    127.1.0.0    ${PCCS}    ${LSPS}    True
    ...    false    127.1.0.1    2
    Verify    10
    [Teardown]    Do_Not_Start_Failing_If_This_Failed
...
Stop_Pcc_Mock
    ...
    [Teardown]    Run Keywords    Kill all pcc mock simulator processes
    ...    AND    Builtin.Wait_Until_Keyword_Succeeds    ${PCEP_READY_VERIFY_TIMEOUT}    5s    Pcep_Off
...
PCEP Sessions Flapped with LSP updates
    ...
    END
    [Teardown]    Run Keywords    Kill all pcc mock simulator processes
    ...    AND    BGPcliKeywords.Stop_Console_Tool_And_Wait_Until_Prompt
...
Select_MOCK_Machine
    ...
    Run Keyword If    '${USE_TOOLS_SYSTEM}' == 'False'    Run Keywords    Pccmock_From_Odl_System
    ...    AND    Updater_From_Odl_System
...
Updater
    ...
    ...    ${delegate}=true    ${pccip}=${none}    ${tunnel_no}=${none}
...
Flap Pcc Mock sessions parallelly with LSP updates
    ...
    FOR    ${i}    IN RANGE    ${PARALLEL_ITERATION}
        ...
        ${i} =    Evaluate    ${i}+1
        Run Keyword If    '${pid}'!= ""    Updater    ${i}    1    127.1.0.0
        ...    ${pccs}    ${lsps}    True
    END
    BGPcliKeywords.Stop_Console_Tool_And_Wait_Until_Prompt

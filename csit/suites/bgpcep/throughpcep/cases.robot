...
PCEP_sessions_from_multiple_machines
    ...
    # Builtin.Set_Suite_Variable    ${size}    ${30}
    ${size1} =    BuiltIn.Evaluate    int(${size})+int(${LSPS})
    Builtin.Set_Suite_Variable    ${size}    ${size1}
    BuiltIn.Log    ${size}
    Builtin.Wait_Until_Keyword_Succeeds    120s    1s    Pcep_On
    ${size2} =    BuiltIn.Evaluate    int(${LSPS})
    Builtin.Set_Suite_Variable    ${size}    ${size2}
    ...
    Kill all pcc mock simulator processes    pccmock_toolsvm
...
Download_Pccmock_Log
    ...
    [Setup]    Run_Even_When_Failing_Fast
    SSHLibrary.Execute Command    tar -zcvf ${LOG_PATH}/mock_log.tar.gz /tmp/throughpcep*
    SSHLibrary.Get_File    ${LOG_PATH}/mock_log.tar.gz    mock_log.tar.gz

...
Library           ${CURDIR}/../../../libraries/BgpRpcClient.py
...
Do_Something_1
    ...
    [Setup]    Two_Peers_Setup_With_App_Peer
    &{mapping}    BuiltIn.Create_Dictionary    PREFIX=1.1.1.1/32    APP_RIB=${APP_PEER_ID}
    TemplatedRequests.Post_As_Xml_Templated    ${BGP_VAR_FOLDER}${/}routing_policy${/}route    mapping=${mapping}    session=${CONFIG_SESSION}
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_ExaBgp_Received_Updates    ${exa_proxy_1}    1
    ${msg}     BgpRpcClient.Exa_Get_Update_Message    proxy=${exa_proxy_1}
    BuiltIn.Log    ${msg}
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_ExaBgp_Received_Updates    ${exa_proxy_2}    1
    ${msg}     BgpRpcClient.Exa_Get_Update_Message    proxy=${exa_proxy_2}
    BuiltIn.Log    ${msg}
    [Teardown]    Two_Peers_Teardown_With_App_Peer
...
Two_Peers_Setup_With_App_Peer
    ...
    Configure_App_Peer
    ${exa_proxy_1}=    BgpRpcClient.Exa_Get_New_Proxy      ${TOOLS_SYSTEM_IP}
    ${exa_proxy_2}=    BgpRpcClient.Exa_Get_New_Proxy      ${ODL_SYSTEM_IP}
    BuiltIn.Set_Suite_Variable    ${exa_proxy_1}
    BuiltIn.Set_Suite_Variable    ${exa_proxy_2}
...
Verify_ExaBgp_Received_Updates
    [Arguments]      ${proxy_obj}     ${expcount}
    ${count_recv}=    BgpRpcClient.exa_get_received_update_count    proxy=${proxy_obj}

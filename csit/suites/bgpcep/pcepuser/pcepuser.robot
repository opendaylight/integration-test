...
Compare_Topology
    ...
    BuiltIn.Log    ${resp}
    ${normresp}=    Normalize_And_Save_Json    ${resp}    ${name}    ${DIRECTORY_FOR_EXPECTED_RESPONSES}
    ...
    ${resp_session_state}=    Run Keyword And Ignore Error    TemplatedRequests.Get_As_Json_Templated    ${NODE_SESSION_STATE_FOLDER}    ${mapping}    ${CONFIG_SESSION}
    Run Keyword And Ignore Error    BuiltIn.Log    ${resp_session_state}

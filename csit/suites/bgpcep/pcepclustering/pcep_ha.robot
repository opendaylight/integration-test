...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --reconnect 1 --local-address ${TOOLS_SYSTEM_IP} --remote-address ${ODL_SYSTEM_1_IP}:4189,${ODL_SYSTEM_2_IP}:4189,${ODL_SYSTEM_3_IP}:4189 --log_level TRACE 2>&1 | tee pccmock.log
    BuiltIn.Log    ${command}
...
Verify_Data_Reported
    ...
    ${output}=    Wait Until Keyword Succeeds    6x    10s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}

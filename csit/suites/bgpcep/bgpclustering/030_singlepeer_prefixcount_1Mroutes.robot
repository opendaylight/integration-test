...
...               RIB is not examined.
...               <dst-id> is recognized during the suite run.
...
Configure_Netconf_Device
    [Documentation]    Configures and verifies netconf device configuration. If configuration is not successful, it de-configure the device fefore the next attempt.
    ...
    # After the netconf device is configured, odl starts downloading schemas. Ifthe downloading will not finish within akka timeout, more tries needed, 3 is based on a user experience.
...
Reconfigure_ODL_To_Accept_Connection
    ...
    # sleep needed, because until odl really starts to accept incomming bgp connection, it takes some time; and often the next Start_Talking_BGP_Speaker tc failed if bgp peer wants to connect to quicly after configuring the peer
...
Wait_For_Stable_Talking_Ipv4_Topology_1
    [Documentation]    Wait until ${EXAMPLE_IPV4_TOPOLOGY_PREF}${dos_leader} becomes stable. This is done by checking stability of prefix count.
...
Check_Talking_Ipv4_Topology_Count_1
    [Documentation]    Count the routes in ${EXAMPLE_IPV4_TOPOLOGY_PREF}${dos_leader} and fail if the count is not correct.
    [Tags]    critical
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
...
Wait_For_Stable_Ipv4_Topology_After_Listening_1
    [Documentation]    Wait until ${EXAMPLE_IPV4_TOPOLOGY_PREF}${dos_leader} becomes stable again.
    [Tags]    critical

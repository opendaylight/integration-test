...
Check_For_Empty_Ipv4_Topology_On_All_Nodes
    [Documentation]    Check the topology is empty on amm 3 nodes.
...
Check_For_Empty_Ipv4_Topology_On_Node
    [Arguments]    ${session}    ${topology}
    [Documentation]    Check the topology is empty on given node.
    PrefixCounting.Check_Ipv4_Topology_Is_Empty    session=${session}    topology=${topology}

...
Configure_Netconf_Device
    ...
    CompareStream.Run_Keyword_If_At_Least_Carbon    BuiltIn.Pass_Execution
...
Delete_Netconf_Device_Configuration
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    #${skip_tc}=    CompareStream.Set_Variable_If_At_Least_Carbon    ${True}    ${False}
    #BuiltIn.Pass_Execution_If    ${skip_tc}==${True}    Openconfig usage does not need netconf connector
    CompareStream.Run_Keyword_If_At_Least_Carbon    BuiltIn.Pass_Execution
    &{mapping}    BuiltIn.Create_Dictionary    DEVICE_NAME=${DEVICE_NAME}    DEVICE_PORT=1830    DEVICE_IP=${rib_owner_node_id}    DEVICE_USER=admin    DEVICE_PASSWORD=admin

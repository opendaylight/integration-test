...
Isolate_Current_Owner_Member
    ...
    ClusterManagement.Isolate_Member_From_List_Or_All    ${pcep_owner}
    BuiltIn.Set Suite variable    ${old_bm_owner}    ${pcep_owner}
    BuiltIn.Set Suite variable    ${old_bm_candidates}    ${pcep_candidates}
...
Get_Example_Pcep_Owner
    ...
    ...    pcep-topology    org.opendaylight.mdsal.ServiceEntityType    1
...
Start_Pcc_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --reconnect 1 --local-address ${TOOLS_SYSTEM_IP} --remote-address ${ODL_SYSTEM_1_IP}:4189,${ODL_SYSTEM_2_IP}:4189,${ODL_SYSTEM_3_IP}:4189 --reconnect 1 --state-sync-avoidance 2 2 2 --log-level TRACE 2>&1 | tee pccmock.log
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    started
    ${status}    ${output}=    SSHLibrary.Read_Until    ${until_phrase}
    BuiltIn.Log    ${output}

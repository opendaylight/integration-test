...
Start_Bmp_Mock_3
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_3_IP} --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    successfully established.
...
Verify_Data_Reported
    ...
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${CONFIG_SESSION1}    verify=True
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${CONFIG_SESSION2}    verify=True
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${CONFIG_SESSION3}    verify=True
    BuiltIn.Log    ${output}

...
Start_Bmp_Mock
    ...
    SSHLibrary.Read_Until_Regexp    ${until_phrase}
    SSHLibrary.Read_Until_Regexp    ${until_phrase}
...
Verify_Data_Reported_2
    [Documentation]    Verifies if the tool reported expected data
    Verify_Data_Reported    ip=${ODL_SYSTEM_1_IP}    session_verify=${CONFIG_SESSION1}

Verify_Data_Reported_3
    [Documentation]    Verifies if the tool reported expected data
    Verify_Data_Reported    ip=${ODL_SYSTEM_2_IP}    session_verify=${CONFIG_SESSION2}

Verify_Data_Reported_4
    [Documentation]    Verifies if the tool reported expected data
    Verify_Data_Reported    ip=${ODL_SYSTEM_3_IP}    session_verify=${CONFIG_SESSION3}
...
Verify_Data_Reported
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}    session=${session_verify}
    ...    verify=True

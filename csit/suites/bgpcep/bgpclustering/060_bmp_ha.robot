...
*** Test Cases ***
Install Bmp Feature
    ${status}    ${results} =    BuiltIn.Run_Keyword_And_Ignore_Error    KarafKeywords.Install_A_Feature_Using_Active_Connection    odl-bgpcep-bmp
    BuiltIn.Log    ${results}
    ${status}    ${results} =    BuiltIn.Run_Keyword_And_Ignore_Error    KarafKeywords.Install_A_Feature_Using_Active_Connection    odl-bgpcep-bmp-config-example
    BuiltIn.Log    ${results}
    &{mapping}    BuiltIn.Create_Dictionary    TOOL_IP=${TOOLS_SYSTEM_IP}
    RequestsLibrary.Create_Session    config_session    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}
    BuiltIn.Wait_Until_Keyword_Succeeds    15x    10s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_FEAT_DIR}    mapping=${mapping}
    ...    verify=True    session=config_session

Get Example Bgp Bm Owner
    ...
    ${bm_owner}    ${bm_candidates}=    ClusterManagement.Get_Owner_And_Successors_For_device    bmp-monitors    org.opendaylight.mdsal.ServiceEntityType    1
...
Verify_Data_Reported
    ...
    ${output}=    Wait Until Keyword Succeeds    10x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}

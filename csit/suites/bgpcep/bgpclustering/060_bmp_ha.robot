...
${CONFIG_SESSION1}    config-session1
${CONFIG_SESSION2}    config-session2
${CONFIG_SESSION3}    config-session3
...
Get Example Bgp Rib Owner
    ...
    BuiltIn.Log    ${rib_candidates}
    BuiltIn.Log    ${rib_candidates}[0]
    BuiltIn.Log    ${rib_candidates}[1]
...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${rib_owner_node_id}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
...
Start_Bmp_Mock
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_1_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    successfully established.
...
Start_Bmp_Mock_2
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_2_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    successfully established.
...
Start_Bmp_Mock_3
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_3_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    successfully established.
...
Setup_Everything
    ...
    RequestsLibrary.Create_Session    ${CONFIG_SESSION1}    http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}    auth=${AUTH}
    RequestsLibrary.Create_Session    ${CONFIG_SESSION2}    http://${ODL_SYSTEM_2_IP}:${RESTCONFPORT}    auth=${AUTH}
    RequestsLibrary.Create_Session    ${CONFIG_SESSION3}    http://${ODL_SYSTEM_3_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Verify_Data_Reported
    ...
    ${output}=    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}    session=${CONFIG_SESSION1}
    ...    verify=True
    BuiltIn.Log    ${output}
    ${output}=    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}    session=${CONFIG_SESSION2}
    ...    verify=True
    BuiltIn.Log    ${output}
    ${output}=    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}    session=${CONFIG_SESSION3}
    ...    verify=True
    BuiltIn.Log    ${output}

...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} -ra ${ODL_SYSTEM_1_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${until_phrase}=    Set Variable    successfully established.
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} -ra ${ODL_SYSTEM_2_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} -ra ${ODL_SYSTEM_3_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    SSHLibrary.Set_Client_Configuration    timeout=30s
    SSHLibrary.Write    ${command}
...
Verify_Data_Reported
    ...
    &{mapping}    BuiltIn.Create_Dictionary    TOOL_IP=127.0.0.2
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${CONFIG_SESSION2}    verify=True
    &{mapping}    BuiltIn.Create_Dictionary    TOOL_IP=127.0.0.3
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}

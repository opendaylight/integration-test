...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${rib_owner_node_id}:12345,${ip2}:12345,${ip3} --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
...
Verify_Data_Reported
    ...
    ...    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True
...
Verify_Odl_Status
    ...
    BuiltIn.Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_1_IP}
    BuiltIn.Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_2_IP}
    BuiltIn.Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_3_IP}
    BuiltIn.Log    ${output}

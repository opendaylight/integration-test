...
#${kill_command}    ps axf | grep org.apache.karaf | grep -v grep | awk '{print \"kill -9 \" $1}' | sh
#${living_session}    session
#${living_session2}    session2
#${living_session3}    session3
...
Get Example Bgp Rib Owner
    ...
    ${status}    ${data}=    Run Keyword And Ignore Error    TemplatedRequests.Get_As_Json_From_Uri    uri=restconf/operational/entity-owners:entity-owners    session=ClusterManagement__session_1
    Log    ${data}
    Run Keyword And Ignore Error    Log    ${data.json()}
    ${rib_owner}    ${rib_candidates}=    ClusterManagement.Get_Owner_And_Successors_For_device    example-bmp-monitor    org.opendaylight.mdsal.ServiceEntityType    1

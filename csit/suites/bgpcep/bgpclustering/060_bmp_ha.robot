...
Reconfigure_ODL_To_Accept_Connection
    ...
    &{mapping}    Create Dictionary    IP=${TOOLS_SYSTEM_1_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${RIB_INSTANCE}
...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_1_IP} --remote_address ${ODL_SYSTEM_1_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
...
Setup_Everything
    ...
    ${tools_system_conn_id}=    SSHLibrary.Open_Connection    ${TOOLS_SYSTEM_1_IP}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=6s
...
Verify_Data_Reported
    [Arguments]    ${ip}=${TOOLS_SYSTEM_1_IP}
    [Documentation]    Verifies if the tool reported expected data
    &{mapping}    BuiltIn.Create_Dictionary    TOOL_IP=${ip}
    ${output}=    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}    mapping=${mapping}    session=${CONFIG_SESSION}
    ...    verify=True
    BuiltIn.Log    ${output}
    BuiltIn.Log    ${CONFIG_SESSION}

...
Delete_Bgp_Peer_Configuration
    ...
    TemplatedRequests.Delete_Templated    ${BGP_PEER_FOLDER}    mapping=${mapping}    session=ClusterManagement__session_1
...
Verify_Data_Reported
    ...
    ...    mapping=${mapping}    session=ClusterManagement__session_1    verify=True
    Log    ${output}
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=ClusterManagement__session_2    verify=True
    Log    ${output}
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=ClusterManagement__session_3    verify=True
    Log    ${output}
...
Verify_Tools_Connection
    ...
    ${status}    ${rsp}=    Run Keyword And Ignore Error    RequestsLibrary.Get Request    ClusterManagement__session_1    ${PEER_CHECK_URL}${TOOLS_SYSTEM_IP}    timeout=10

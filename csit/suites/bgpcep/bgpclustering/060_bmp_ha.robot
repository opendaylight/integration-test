...
${BGP_PEER_FOLDER}    ${CURDIR}/../../../variables/bgpclustering
${HOLDTIME}       180
...
Reconfigure_ODL_To_Accept_Connection
    ...
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_PEER_FOLDER}/bgp_peer_openconf    mapping=${mapping}    session=${living_session}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Start_Bmp_Mock
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_1_IP}:12345,${ODL_SYSTEM_2_IP}:12345,${ODL_SYSTEM_3_IP}:12345 --routers_count 1 --peers_count 1 --pre_policy_routes 2 --post_policy_routes 2 --log_level TRACE 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
...
Reconfigure_BMP_To_Accept_Connection1
    [Documentation]    Configure BMP Monitor connection to routers to active.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    &{mapping}    Create Dictionary    IP=${TOOLS_SYSTEM_IP}    BMP_OPENCONFIG=${BMP_INSTANCE}
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_PEER_FOLDER}/bmp_monitor    mapping=${mapping}    session=${living_session}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Reconfigure_BMP_To_Accept_Connection2
    [Documentation]    Configure BMP Monitor connection to routers to active.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    &{mapping}    Create Dictionary    IP=${ODL_SYSTEM_1_IP}    BMP_OPENCONFIG=${BMP_INSTANCE}
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_PEER_FOLDER}/bmp_monitor    mapping=${mapping}    session=${living_session}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Reconfigure_BMP_To_Accept_Connection3
    [Documentation]    Configure BMP Monitor connection to routers to active.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    &{mapping}    Create Dictionary    IP=${ODL_SYSTEM_2_IP}    BMP_OPENCONFIG=${BMP_INSTANCE}
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_PEER_FOLDER}/bmp_monitor    mapping=${mapping}    session=${living_session}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Reconfigure_BMP_To_Accept_Connection4
    [Documentation]    Configure BMP Monitor connection to routers to active.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    &{mapping}    Create Dictionary    IP=${ODL_SYSTEM_3_IP}    BMP_OPENCONFIG=${BMP_INSTANCE}
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_PEER_FOLDER}/bmp_monitor    mapping=${mapping}    session=${living_session}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Verify_Data_Reported_4
    [Documentation]    Verifies if the tool reported expected data
    Verify_Data_Reported
    Verify_Tools_Connection
...
Delete_Bgp_Peer_Configuration
    ...
    TemplatedRequests.Delete_Templated    ${BGP_PEER_FOLDER}/bgp_peer_openconf    mapping=${mapping}    session=${living_session}
...
Setup_Everything
    ...
    #RequestsLibrary.Create_Session    ${living_session}    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Verify_Data_Reported
    ...
    Log    ${response.json()}
    ${response}    RequestsLibrary.Get_Request    alias=${living_session}    uri=/restconf/operational/bgp-rib:bgp-rib/rib/example-bgp-rib    headers=${ACCEPT_JSON}
    Log    ${response.json()}
...
Verify_Tools_Connection
    ...
    ${status}    ${rsp}=    Run Keyword And Ignore Error    RequestsLibrary.Get Request    ${living_session}    ${PEER_CHECK_URL}${TOOLS_SYSTEM_IP}    timeout=10
    ...
    Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${ODL_SYSTEM_1_IP}
    Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${ODL_SYSTEM_2_IP}
    Log    ${output}
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${ODL_SYSTEM_3_IP}
    Log    ${output}

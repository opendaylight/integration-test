...
Verify_Data_Reported_1
    ...
    Verify_Data_Reported    session_verify=${CONFIG_SESSION}

Verify_Data_Reported_2
    [Documentation]    Verifies if the tool reported expected data
    ${inc_ip}=    Increment_ip    ip=${TOOLS_SYSTEM_IP}    increment=1
    BuiltIn.Log    ${inc_ip}
    RequestsLibrary.Create_Session    ${CONFIG_SESSION_2}    http://${ODL_SYSTEM_2_IP}:${RESTCONFPORT}    auth=${AUTH}
    Verify_Data_Reported    ip=${inc_ip}    session_verify=${CONFIG_SESSION_2}

Verify_Data_Reported_3
    [Documentation]    Verifies if the tool reported expected data
    ${inc_ip}=    Increment_ip    ip=${TOOLS_SYSTEM_IP}    increment=2
    RequestsLibrary.Create_Session    ${CONFIG_SESSION_3}    http://${ODL_SYSTEM_3_IP}:${RESTCONFPORT}    auth=${AUTH}
    BuiltIn.Log    ${inc_ip}
    Verify_Data_Reported    ip=${inc_ip}    session_verify=${CONFIG_SESSION_2}
...
Start_Bmp_Mock
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} -ra ${ODL_SYSTEM_1_IP}:12345,${ODL_SYSTEM_2_IP}:12345,${ODL_SYSTEM_3_IP}:12345 --routers_count 1 --peers_count 1 --log_level INFO 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    ...
    Log    ${output}
    ${status}    ${output}=    Run Keyword And Ignore Error    Get File    ${CURDIR}/bmpmock.log
    Log    ${output}

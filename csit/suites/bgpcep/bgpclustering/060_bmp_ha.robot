...
Verify BMP Feature
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    6x    10s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_FEAT_DIR}    mapping=${mapping}    session=${SESSION1}
    ...    verify=True
...
Start_Bmp_Mock
    ...
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} --remote_address ${ODL_SYSTEM_1_IP}:12345,${ODL_SYSTEM_2_IP}:12345,${ODL_SYSTEM_3_IP}:12345 --routers_count 1 --peers_count 1 --log_level DEBUG 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
...
Delete_Bgp_Peer_Configuration
    ...
    TemplatedRequests.Delete_Templated    ${BGP_PEER_FOLDER}    mapping=${mapping}    session=${SESSION1}
...
#   SSHKeywords.Flexible_Mininet_Login    ${TOOLS_SYSTEM_USER}
    ...
    RequestsLibrary.Create_Session    ${SESSION3}    http://${ODL_SYSTEM_3_IP}:${RESTCONFPORT}    auth=${AUTH}
    RequestsLibrary.Create_Session    ${SESSION1}    http://${ODL_SYSTEM_1_IP}:${RESTCONFPORT}    auth=${AUTH}
    RequestsLibrary.Create_Session    ${SESSION2}    http://${ODL_SYSTEM_2_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Verify_Data_Reported
    ...
    ...    mapping=${mapping}    session=${SESSION1}    verify=True
    Log    ${output}
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${SESSION2}    verify=True
    Log    ${output}
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}
    ...    mapping=${mapping}    session=${SESSION3}    verify=True
    Log    ${output}
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    Wait Until Keyword Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_BMP_DIR}/monitor/example-bmp-monitor
    ...    mapping=${mapping}    session=${SESSION1}    verify=True
    Log    ${output}
...
Verify_Tools_Connection
    ...
    ${status}    ${rsp}=    Run Keyword And Ignore Error    RequestsLibrary.Get Request    ${SESSION1}    ${PEER_CHECK_URL}${TOOLS_SYSTEM_IP}    timeout=10

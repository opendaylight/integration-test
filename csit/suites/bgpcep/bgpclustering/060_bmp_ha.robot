...
Repeat_Start_Bmp_Mock
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    10x    5s    Start_Bmp_Mock

Verify_Data_Reported_1
    [Documentation]    Verifies if the tool reported expected data
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    Verify_Data_Reported    session_verify=node1

Verify_Data_Reported_2
    [Documentation]    Verifies if the tool reported expected data
    RequestsLibrary.Create_Session    node2    http://${ODL_SYSTEM_2_IP}:${RESTCONFPORT}    auth=${AUTH}
    ${inc_ip}=    Increment_ip    ip=${TOOLS_SYSTEM_IP}    increment=1
    Verify_Data_Reported    ip=${inc_ip}    session_verify=node2

Verify_Data_Reported_3
    [Documentation]    Verifies if the tool reported expected data
    RequestsLibrary.Create_Session    node3    http://${ODL_SYSTEM_3_IP}:${RESTCONFPORT}    auth=${AUTH}
    ${inc_ip}=    Increment_ip    ip=${TOOLS_SYSTEM_IP}    increment=2
    Verify_Data_Reported    ip=${inc_ip}    session_verify=node3
...
Increment_Ip
    ...
    ${new_value}=    Evaluate    str(int(${splitip}[-1])+1)
...
Start_Bmp_Mock
    [Documentation]    Starts bmp mock
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local_address ${TOOLS_SYSTEM_IP} -ra ${ODL_SYSTEM_1_IP}:12345,${ODL_SYSTEM_2_IP}:12346,${ODL_SYSTEM_3_IP}:12347 --routers_count 1 --peers_count 1 --log_level TRACE 2>&1 | tee ${BMP_LOG_FILE}
    BuiltIn.Log    ${command}
    ...
    BuiltIn.Run_Keyword_And_Ignore_Error    SSHLibrary.Read_Until    ${until_phrase}
    BuiltIn.Run_Keyword_And_Ignore_Error    SSHLibrary.Read_Until    ${until_phrase}

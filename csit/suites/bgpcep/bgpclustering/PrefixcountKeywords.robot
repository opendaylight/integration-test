...
Set_Shard_Leaders_Location_And_Verify
    ...
    ${expected_location} =    BuiltIn.Set_Variable_If    "${requested_shard_localtion}" == "local"    ${current_rib_owner}    @{current_rib_candidates}[0]
    ShardStability.Set_Shard_Location    ${expected_location}
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    5s    ShardStability.Verify_Shard_Leader_Located_As_Expected    ${expected_location}
    ${init_shard_details} =    ShardStability.Shards_Stability_Get_Details    ${SHARD_MONITOR_LIST}
    BuiltIn.Set_Suite_Variable    ${init_shard_details}

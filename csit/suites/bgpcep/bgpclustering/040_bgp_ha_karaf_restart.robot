...
Get_Neighbor_Configuration
    [Documentation]    Verifying datastore configuration for DEBUG purposes.
    &{mapping}    Create Dictionary    IP=${TOOLS_SYSTEM_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${RIB_INSTANCE}
    ${status}    ${output}    Run Keyword And Ignore Error    TemplatedRequests.Get_As_Json_Templated    ${BGP_PEER_FOLDER}    mapping=${mapping}    session=ClusterManagement__session_1    http_timeout=5
    Log    ${output}
    ${status}    ${output}    Run Keyword And Ignore Error    TemplatedRequests.Get_As_Json_Templated    ${BGP_PEER_FOLDER}    mapping=${mapping}    session=ClusterManagement__session_2    http_timeout=5
    Log    ${output}
    ${status}    ${output}    Run Keyword And Ignore Error    TemplatedRequests.Get_As_Json_Templated    ${BGP_PEER_FOLDER}    mapping=${mapping}    session=ClusterManagement__session_3    http_timeout=5
    Log    ${output}

...
Reconfigure_ODL_To_Accept_Connection
    ...
    &{mapping}    Create Dictionary    DEVICE_NAME=${DEVICE_NAME}    BGP_NAME=${BGP_PEER_NAME}    IP=${ODL_SYSTEM_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}
...
Reconfigure_ODL_To_Accept_Connection2
    [Documentation]    Configure BGP peer module with initiate-connection set to false.
...
Reconfigure_ODL_To_Accept_Connection3
    [Documentation]    Configure BGP peer module with initiate-connection set to false.
    &{mapping}    Create Dictionary    IP=${ODL_SYSTEM_IP}    RIB=${RIB_INSTANCE}
    TemplatedRequests.Post_As_Xml_Templated    ${BGP_VARIABLES_FOLDER}${/}peer_session/create    mapping=${mapping}    session=${CONFIG_SESSION}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Reconfigure_ODL_To_Accept_Connection4
    [Documentation]    Configure BGP peer module with initiate-connection set to false.
    &{mapping}    Create Dictionary    IP=${TOOLS_SYSTEM_IP}    RIB=${RIB_INSTANCE}
    TemplatedRequests.Post_As_Xml_Templated    ${BGP_VARIABLES_FOLDER}${/}peer_session/create    mapping=${mapping}    session=${CONFIG_SESSION}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

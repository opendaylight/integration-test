...
${PROTOCOL_OPENCONFIG}    ${RIB_INSTANCE}
${BASIC_URI}    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}

*** Test Cases ***
Wait_For_Empty_Topology
    ${response}=    Wait_Until_Keyword_Succeeds    180s    2s    RequestsLibrary.Get Request    ${CONFIG_SESSION}    /restconf/operational/network-topology:network-topology/topology/example-ipv4-topology
...
Reconfigure_ODL_To_Accept_Connection
    ...
    Wait_Until_Keyword_Succeeds    180s    5s    TemplatedRequests.Put_As_Xml_Templated    ${BGP_VARIABLES_FOLDER}${/}bgp_peer    mapping=${mapping}    session=${CONFIG_SESSION}
...
Check_Bgp_Peer_Configuration
    ...
    Log    ${output}
    ${response}=    RequestsLibrary.Get Request    ${CONFIG_SESSION}    /restconf/operational/network-topology:network-topology/topology/example-ipv4-topology
    BuiltIn.Log    ${response.status_code}
    BuiltIn.Log    ${response.text}
    ${response}=    RequestsLibrary.Get Request    ${CONFIG_SESSION}    /restconf/operational/bgp-rib:bgp-rib/rib/example-bgp-rib
    BuiltIn.Log    ${response.status_code}
    BuiltIn.Log    ${response.text}
...
Start_Talking_BGP_speaker
    ...
    BGPSpeaker.Start_BGP_Speaker    --amount 1 --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --firstprefix=8.0.0.1 --${BGP_TOOL_LOG_LEVEL}
...
Setup_Everything
    ...
    RequestsLibrary.Create_Session    ${CONFIG_SESSION}    ${BASIC_URI}    auth=${AUTH}

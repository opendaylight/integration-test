...
TC_LA_Start_Bgp_Speaker_And_Verify_Connected
    ...
    [Tags]    critical
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid only for versions fluorine and above.
    ${speaker_args}    BuiltIn.Set_Variable    --amount 1 --listen --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --debug
    ${output}    BGPSpeaker.Start_BGP_Speaker_And_Verify_Connected    ${speaker_args}    ${CONFIG_SESSION}    speaker_ip=${TOOLS_SYSTEM_IP}
    BuiltIn.Log    ${output}

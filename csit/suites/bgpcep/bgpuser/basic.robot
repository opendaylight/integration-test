...
Reconfigure_ODL_With_Peer_Group_To_Accept_Connection
    [Documentation]    Configure BGP peer module with initiate-connection set to false.
    CompareStream.Run_Keyword_If_At_Least_Fluorine    Configure_Peer_Group
    &{mapping}    Create Dictionary    DEVICE_NAME=${DEVICE_NAME}    BGP_NAME=${BGP_PEER_NAME}    IP=${TOOLS_SYSTEM_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    PEER_GROUP_NAME=${PEER_GROUP}
    ...    INITIATE=false    BGP_RIB=${RIB_INSTANCE}    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${PROTOCOL_OPENCONFIG}    RIB_INSTANCE_NAME=${RIB_INSTANCE}
...
Reconfigure_ODL_With_Peer_Group_Without_Ipv4_Unicast
    [Documentation]    Configure BGP peer module with initiate-connection set to false.
    CompareStream.Run_Keyword_If_At_Least_Fluorine    Configure_Peer_Group    peer_group=peer_group_without_ipv4
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

Check_For_Empty_Topology_After_Deconfiguration
...
Configure_Peer_Group
    [Arguments]    ${peer_group}=peer_group

...
TC_LA_Reconfigure_Odl_To_Accept_Connection
    [Documentation]    Configure neighbor with local-address.
...
TC_LA_Start_Bgp_Speaker_With_Neighbor_Address_And_Verify_Connected
    [Documentation]    Verify that peer is present in odl's rib. Peer is configured with local-address
    [Tags]    critical
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid only for versions fluorine and above.

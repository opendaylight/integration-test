...
TC_LA_Start_Bgp_Speaker_With_Neighbor_Address_And_Verify_Not_Connected
    ...
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid only for versions fluorine and above.
    ${speaker_args}    BuiltIn.Set_Variable    --amount 1 --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --${BGP_TOOL_LOG_LEVEL}
...
TC_LA_Reconfigure_Odl_To_Accept_Connection_Correct
    ...
    &{mapping}    Create Dictionary    IP=${LOCAL_ADDRESS}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${PROTOCOL_OPENCONFIG}

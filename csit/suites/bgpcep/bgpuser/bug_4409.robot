...
Set Karaf Log Levels
    ...
    ${current_SSH_connection}=    SSHLibrary.Switch Connection    ${KarafKeywords__controller_index}
    ${status_write}    ${message_write}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Write    log:set ${ODL_LOG_LEVEL} org.opendaylight.bgpcep
    ${status_wait}    ${message_wait}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Read Until Prompt
    ${status_write}    ${message_write}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Write    log:set ${ODL_LOG_LEVEL} org.opendaylight.protocol
    ${status_wait}    ${message_wait}=    BuiltIn.Run Keyword And Ignore Error    SSHLibrary.Read Until Prompt

...
...               BGP peers in different roles (iBGP, iBGP, iBGP RR-client).
...
${eBGP_PEER1_FIRST_PREFIX_IP}    8.3.0.0
${eBGP_PEER2_FIRST_PREFIX_IP}    8.4.0.0
${PREFIX_LEN}    28
...
${eBGP_PEER1_COMMAND}    python play.py --firstprefix ${BGP_PEER1_FIRST_PREFIX_IP} --prefixlen ${BGP_PEER1_PREFIX_LEN} --amount ${BGP_PEER1_PREFIX_COUNT} --myip=${BGP_PEER1_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --asnumber ${BGP_PEER1_AS} --${BGP_PEER_LOG_LEVEL} --logfile ${BGP_PEER1_LOG_FILE}
...
${eBGP_PEER2_COMMAND}    python play.py --firstprefix ${BGP_PEER2_FIRST_PREFIX_IP} --prefixlen ${BGP_PEER2_PREFIX_LEN} --amount ${BGP_PEER2_PREFIX_COUNT} --myip=${BGP_PEER2_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --asnumber ${BGP_PEER2_AS}--${BGP_PEER_LOG_LEVEL} --logfile ${BGP_PEER2_LOG_FILE}
...
TC4
    [Documentation]    Two eBGP peers 
    BuiltIn.Sleep    3s

TC4_Configure_Two_eBGP_Peers
    [Documentation]    Configure eBGP peers
    [Tags]    critical
...
TC4_eBGP_Peer1_Check_Log_For_Introduced_Prefixes
    [Documentation]    Check incomming updates for new routes
    [Tags]    critical
    Switch_To_BGP_Peer1_Console
    BuiltIn.Wait_Until_Keyword_Succeeds    60s    1s    Check_File_For_Word_Count    ${eBGP_PEER1_LOG_FILE}    nlri_prefix_received:    ${eBGP_PEER2_PREFIX_COUNT}
    Check_File_For_Word_Count    ${BGP_PEER1_LOG_FILE}    nlri_prefix_received: ${eBGP_PEER2_FIRST_PREFIX_IP}/${eBGP_PEER2_PREFIX_LEN}    1
    Check_File_For_Word_Count    ${BGP_PEER1_LOG_FILE}    withdrawn_prefix_received:    0

TC4_eBGP_Peer2_Check_Log_For_Introduced_Prefixes
    [Documentation]    Check incomming updates for new routes
    [Tags]    critical
    Switch_To_BGP_Peer2_Console
    BuiltIn.Wait_Until_Keyword_Succeeds    60s    1s    Check_File_For_Word_Count    ${eBGP_PEER2_LOG_FILE}    nlri_prefix_received:    ${eBGP_PEER1_PREFIX_COUNT}
    Check_File_For_Word_Count    ${BGP_PEER2_LOG_FILE}    nlri_prefix_received: ${eBGP_PEER1_FIRST_PREFIX_IP}/${eBGP_PEER1_PREFIX_LEN}    1
    Check_File_For_Word_Count    ${BGP_PEER2_LOG_FILE}    withdrawn_prefix_received:    0
    [Teardown]    Report_Failure_Due_To_Bug    4791

TC4_Disconnect_eBGP_Peer1
    [Documentation]    Stop BGP peer & store logs
    [Tags]    critical
    Switch_To_BGP_Peer1_Console
    Stop_Console_Tool
    Store_File_To_Workspace    ${eBGP_PEER1_LOG_FILE}    tc4_${eBGP_PEER1_LOG_FILE}
    Log_Example_IPv4_Topology

TC4_eBGP_Peer2_Check_Log_For_Withdrawn_Prefixes
    [Documentation]    Check incomming updates for withdrawn routes
    [Tags]    critical
    Switch_To_BGP_Peer2_Console
    BuiltIn.Wait_Until_Keyword_Succeeds    60s    1s    Check_File_For_Word_Count    ${BGP_PEER2_LOG_FILE}    withdrawn_prefix_received:    ${BGP_PEER1_PREFIX_COUNT}
    Check_File_For_Word_Count    ${eBGP_PEER2_LOG_FILE}    withdrawn_prefix_received: ${BGP_PEER1_FIRST_PREFIX_IP}/${BGP_PEER1_PREFIX_LEN}    1
    Log_Example_IPv4_Topology

TC4_Disconnect_eBGP_Peer2
    [Documentation]    Stop BGP peer & store logs
    [Tags]    critical
    Switch_To_BGP_Peer2_Console
    Stop_Console_Tool
    Store_File_To_Workspace    ${eBGP_PEER2_LOG_FILE}    tc4_${eBGP_PEER2_LOG_FILE}
    Log_Example_IPv4_Topology

TC4_Delete_eBGP_Peers_Configuration
    [Documentation]    Delete all previously configured BGP peers.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'NAME': 'example-ebgp-peer1'}

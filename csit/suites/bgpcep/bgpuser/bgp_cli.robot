...
Test_Cli
    [Arguments]    ${bgp_rib_value}=0    ${ibgp-1}=0    ${ibgp-2}=0
    [Documentation]    Verifies values of bgp-cli returns
    [Tags]    critical
    BuiltIn.Wait Until Keyword Succeeds    3x    1s    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Should_Contain    ${ret}    TotalPrefixes|${bgp_rib_value}
    BuiltIn.Wait Until Keyword Succeeds    3x    1s    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.1
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Should_Contain    ${ret}    Installed|${ibgp-1}
    BuiltIn.Wait Until Keyword Succeeds    3x    1s    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.2
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Should_Contain    ${ret}    Installed|${ibgp-2}
    BuiltIn.Wait Until Keyword Succeeds    3x    1s    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -peer-group application-peers
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Log    ${ret}

...
Test_Cli
    ...
    BuiltIn.Log    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    '${output}'.replace(' ','')    TotalPrefixes|6
    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.1
    BuiltIn.Log    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    '${output}'.replace(' ','')    Prefixes|Installed|3
    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.2
    BuiltIn.Log    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    '${output.strip()}'.replace(' ','')    Prefixes|Installed|3

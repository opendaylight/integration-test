...
Test_Cli
    ...
    BuiltIn.Log    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${output.strip()}    TotalPrefixes|6
    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.1
    BuiltIn.Log    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${output.strip()}    Prefixes|Installed|3

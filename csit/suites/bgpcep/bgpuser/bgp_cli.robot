...
Test_Cli
    ...
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${ret}    TotalPrefixes|6
    ${output}=    KarafKeywords.Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor 127.0.0.1
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${ret}    Prefixes|Installed|3
...
Normalize_String
    [Arguments]    ${string}
    ${string}=   String.Replace_String   ${string}    ${SPACE}    ${\n}
    [Return]    ${string}

...
TC2_Check_Cli_Returns
    [Documentation]    Tests results of commands through odl-bgpcep-bgp-cli
    [Tags]    critical
    BuiltIn.Repeat_Keyword    3 times    Test_Cli    6     3     3
...
Test_Cli
    ...
    [Tags]    critical
    ${output}=    BuiltIn.Wait_Until_Keyword_Succeeds    3x    1s    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${ret}    TotalPrefixes|${bgp_rib_value}
    ...
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${ret}    Installed|${ibgp-1}
    ...
    BuiltIn.Log    ${output}
    ${ret}=    Normalize_String    ${output}
    BuiltIn.Run_Keyword_And_Ignore_Error    BuiltIn.Should_Contain    ${ret}    Installed|${ibgp-2}

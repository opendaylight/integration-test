...
Test_Cli
    ...
    [Tags]    critical
    CompareStream.Run_Keyword_If_At_Least_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    Test_Bgp_Rib    expcount=${bgp_rib_value}    test=Family|IPV4UNICASTTotalPaths|${bgp_rib_value}TotalPrefixes|
...
Test_Bgp_Rib
    [Arguments]    ${expcount}    ${test}=Family|IPV4UNICASTActive|truePrefixes|Installed|    ${ip}=
    [Documentation]    Checks for expected number of prefixes on main rib or neighbors
    ...    It uses karaf command through odl-bgpcep-bgp-cli. Compares output strings, fails if strings do not match.

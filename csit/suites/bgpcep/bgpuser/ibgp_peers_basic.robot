...
...
...               Test Case 4: Two iBGP(play.py) RR-client peers configured with route-reflector-cluster-id.
...               Each of them introduces 3 prefixes.
...               Expected result: controller forwards updates towards both peers and each of the
...               routes contains their respective cluster-ids. For this we check exact output of
...               adj-rib-out on each of the peers, containing prefixes and cluster-id from the
...               other peer.
...
...
TC4_Connect_BGP_Peers
    ...
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid for version fluorine and above.
    SSHLibrary.Switch Connection    bgp_peer1_console
    BGPCliKeywords.Start_Console_Tool    ${BGP_PEER1_COMMAND} --cluster=${BGP_PEER1_IP}    ${BGP_PEER1_OPTIONS}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEFAULT_TOPOLOGY_CHECK_TIMEOUT}    ${DEFAULT_TOPOLOGY_CHECK_PERIOD}    BgpOperations.Check_Example_IPv4_Topology_Content    {"prefix":"${BGP_PEER1_FIRST_PREFIX_IP}/${PREFIX_LEN}"}
    SSHLibrary.Switch Connection    bgp_peer2_console
...
TC4_BGP_Peer1_Check_Log_For_Introduced_Prefixes
    ...
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid for version fluorine and above.
    #TMP DEBUG purposes
    ${output}    TemplatedRequests.Get_As_Json_Templated    ${BGP_VARIABLES_FOLDER}../bgpfunctional/bgppolicies/rib_state    session=${CONFIG_SESSION}
    BuiltIn.Log    ${output}
    ${output}    TemplatedRequests.Get_As_Json_Templated    ${BGP_VARIABLES_FOLDER}../bgpfunctional/bgppolicies/topology_state    session=${CONFIG_SESSION}
    BuiltIn.Log    ${output}
    #TMP END
    &{mapping}    BuiltIn.Create_Dictionary    IP=${BGP_PEER1_IP}    BGP_RIB_OPENCONFIG=${PROTOCOL_OPENCONFIG}    PEER_NUMBER=2    CLUSTER_ID=${BGP_PEER2_IP}
...
TC4_BGP_Peer2_Check_Log_For_Introduced_Prefixes
    ...
    [Tags]    critical
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid for version fluorine and above.
    &{mapping}    BuiltIn.Create_Dictionary    IP=${BGP_PEER2_IP}    BGP_RIB_OPENCONFIG=${PROTOCOL_OPENCONFIG}    PEER_NUMBER=1    CLUSTER_ID=${BGP_PEER1_IP}
    TemplatedRequests.Get_As_Json_Templated    ${BGP_VARIABLES_FOLDER}${/}cluster_id/peer_rib_out    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True

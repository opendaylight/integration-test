...
TC_LAS_Reconfigure_Odl_To_Accept_Connection
    ...
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_VARIABLES_FOLDER}${/}local_as/ebgp_peer    mapping=${mapping}    session=${CONFIG_SESSION}
...
TC_LAS_Start_iBgp_Speaker_And_Verify_Connected
    ...
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid only for versions fluorine and above.
    SSHLibrary.Switch Connection    ibgp_peer1_console
    ${speaker_args}    BuiltIn.Set_Variable    --firstprefix ${iBGP_PEER1_FIRST_PREFIX_IP} --prefixlen ${PREFIX_LEN} --amount 1 --myip=${iBGP_PEER1_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --${BGP_TOOL_LOG_LEVEL}
    # DEBUG
    BuiltIn.Sleep    10s
    ...
    BuiltIn.Log    ${rib}
    ${output}    BGPSpeaker.Start_BGP_Speaker_And_Verify_Connected    ${speaker_args}    ${CONFIG_SESSION}    speaker_ip=${iBGP_PEER1_IP}
    BuiltIn.Log    ${output}
...
TC_LAS_Start_eBgp_Speaker_And_Verify_Connected
    ...
    CompareStream.Run_Keyword_If_Less_Than_Fluorine    BuiltIn.Pass_Execution    Test case valid only for versions fluorine and above.
    SSHLibrary.Switch Connection    ebgp_peer1_console
    ${speaker_args}    BuiltIn.Set_Variable    --firstprefix ${eBGP_PEER1_FIRST_PREFIX_IP} --prefixlen ${PREFIX_LEN} --amount 1 --asnumber=${eBGP_AS} --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --${BGP_TOOL_LOG_LEVEL}
    ${output}    BGPSpeaker.Start_BGP_Speaker_And_Verify_Connected    ${speaker_args}    ${CONFIG_SESSION}    speaker_ip=${eBGP_PEER1_IP}
    BuiltIn.Log    ${output}
...
TC_LAS_Verify_iBGP_Rib_Out
    ...
    TemplatedRequests.Get_As_Json_Templated    ${BGP_VARIABLES_FOLDER}${/}local_as/adj_rib_out    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True
...
TC_LAS_Delete_Bgp_Peer_Configurations
    ...
    TemplatedRequests.Delete_Templated    ${BGP_VARIABLES_FOLDER}${/}local_as/ebgp_peer    mapping=${mapping}    session=${CONFIG_SESSION}
    [Teardown]    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed

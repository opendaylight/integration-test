...
Start Suite
    ...
    ...    return_rc=True
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    env DEBIAN_FRONTEND noninteractive sudo apt-get install -y --force-yes -qq exabgp    return_stdout=True    return_stderr=True     return_rc=True
    #${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    sudo pip install --upgrade pip    return_stdout=True    return_stderr=True     return_rc=True

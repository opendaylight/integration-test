...
Reconfigure_ODL_To_Accept_Connections
    ...
    \    ${individual_ip}=    BuiltIn.Evaluate    "127.0.0." + ${index}
    \    ${template_as_string}=    BuiltIn.Set_Variable    {'NAME': 'example-bgp-peer-${index}', 'IP': '${individual_ip}', 'PEER_PORT': '${BGP_TOOL_PORT}', 'HOLDTIME': '${HOLDTIME_CHANGE_COUNTER}', 'INITIATE': 'false'}
...
Wait_For_Data_Change_Counter_Ready
    ...
    [Setup]    FailFast.Run_Even_When_Failing_Fast
    BuiltIn.Run_Keyword_Unless    ${change_counter_ok}    Builtin.Wait_Until_Keyword_Succeeds    60s    1s    Check_Change_Counter
    [Teardown]    FailFast.Do_Not_Start_Failing_If_This_Failed
...
Check_Topology_Is_Empty
    ...
    Utils.Run_Keyword_If_Status_Is_Ok    Builtin.Should_Be_Equal_As_Strings    ${count}    0

Get_Change_Count
    [Arguments]    ${stop_at_http_error}
    [Documentation]    Get the count of changes from BGP change counter. Fail on a HTTP
    ...    error if ${stop_at_http_error} is set.
...
Check_Change_Counter
    [Documentation]    Make a request to change counter's value and fail if that request was not successful.
    Get_Change_Count    True
...
Check_Topology_Stability
    ...
    ${actual_count_text}=    Get_Change_Count    ${stop_at_http_error}
...
Wait_For_Topology_Stability
    ...
    Builtin.Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    Check_Topology_Stability    expect_stable=${expect_stable}    stop_at_http_error=${stop_at_http_error}

...
...               TODO: Figure out it configurable.
...
Reconfigure_ODL_To_Accept_Connections
    ...
    \    ${peer_ip}=    BuiltIn.Evaluate    str(netaddr.IPAddress('127.0.0.1') + ${index})    modules=netaddr
...
Start_Talking_BGP_Manager
    ...
    BGPSpeaker.Start_BGP_Manager    --amount=${COUNT_CHANGE_COUNTER} --multiplicity=${MULTIPLICITY} --myip=127.0.0.1 --myport=${BGP_TOOL_PORT} --peerip=${CONTROLLER} --peerport=${ODL_BGP_PORT}

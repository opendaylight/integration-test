...
${BGP_APP_PEER_INITIAL_POST_COMMAND}    python bgp_app_peer.py --host ${ODL_SYSTEM_IP} --port ${RESTCONFPORT} --command post --count 1 --prefix 8.0.1.0 --prefixlen 28 --${BGP_APP_PEER_LOG_LEVEL}
${BGP_APP_PEER_PUT_COMMAND}    python bgp_app_peer.py --host ${ODL_SYSTEM_IP} --port ${RESTCONFPORT} --command put --count ${COUNT} --batch ${BATCH} --prefix 8.0.1.0 --prefixlen 28 --${BGP_APP_PEER_LOG_LEVEL}
...
Start_BGP_Application_Peer_To_Introduce_Routes
    ...
    Wait_Until_Console_Tool_Finish    10s
    Store_File_To_Workspace    bgp_app_peer.log    bgp_app_peer_initial_post.log
    Start_Console_Tool    ${BGP_APP_PEER_PUT_COMMAND}    ${BGP_APP_PEER_OPTIONS}

Wait_For_Stable_Ipv4_Topology
    [Documentation]    Wait until example-ipv4-topology becomes stable. This is done by checking stability of prefix count. Take into account the batch size.
    ${repetitions} =    BuiltIn.Evaluate    ${REPETITIONS_APP_PEER_PREFIX_COUNT} + ${BATCH} / 100
    PrefixCounting.Wait_For_Ipv4_Topology_Prefixes_To_Become_Stable    timeout=${bgp_filling_timeout}    period=${CHECK_PERIOD_APP_PEER_PREFIX_COUNT}    repetitions=${repetitions}    excluded_count=0

Check_Ipv4_Topology_Count
    [Documentation]    Count the routes in example-ipv4-topology and fail if the count is not correct.
    [Tags]    critical
    PrefixCounting.Check_Ipv4_Topology_Count    ${COUNT_APP_PEER_PREFIX_COUNT}
...
Check_Bgp_Peer_Updates_For_New_Prefixes
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check_File_For_Word_Count    bgp_peer.log    total_received_nlri_prefix_counter: ${COUNT_APP_PEER_PREFIX_COUNT}    2
...
Check_Bgp_Peer_Updates_For_Prefix_Withdrawals
    ...
    BuiltIn.Wait Until Keyword Succeeds    30s    1s    Check_File_For_Word_Count    bgp_peer.log    total_received_withdrawn_prefix_counter: ${COUNT_APP_PEER_PREFIX_COUNT}    2
...
Check_File_For_Word_Count
    ...
    ${output_log}=    SSHLibrary.Execute_Command    grep -o '${word}' ${file_name} | wc -w
    BuiltIn.Log    ${output_log}

...
${BGP_APP_PEER_ADD_COMMAND}    python bgp_app_peer.py --host ${ODL_SYSTEM_IP} --port ${RESTCONFPORT} --command add --count ${remaining_prefixes} --prefix 10.0.0.0 --prefixlen 28 --${BGP_APP_PEER_LOG_LEVEL}
...
Start_BGP_Application_Peer_To_Prefill_Routes
    ...
    Wait_Until_Console_Tool_Finish    ${bgp_filling_timeout/10}
...
Wait_For_Ipv4_Topology_Is_Filled
    [Documentation]    Wait until example-ipv4-topology reaches the target prfix count.
    BuiltIn.Wait Until Keyword Succeeds    ${bgp_filling_timeout/10}    10s    PrefixCounting.Check_Ipv4_Topology_Count    ${COUNT_APP_PEER_PREFIX_COUNT}
    [Teardown]    Report_Failure_Due_To_Bug    4689

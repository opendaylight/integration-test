...
BGP_Application_Peer_Prefill_Routes
    ...
    Start_Console_Tool    ${BGP_APP_PEER_INITIAL_COMMAND} ${scritp_uri_opt}    ${BGP_APP_PEER_OPTIONS}
...
BGP_Application_Peer_Introduce_Single_Routes
    ...
    SSHLibrary.Switch Connection    bgp_app_peer_console
    Start_Console_Tool    python bgp_app_peer.py --host ${ODL_SYSTEM_IP} --port ${RESTCONFPORT} --command add --count ${remaining_prefixes} --prefix 12.0.0.0 --prefixlen 28 --${BGP_APP_PEER_LOG_LEVEL} --stream=${ODL_STREAM} ${scritp_uri_opt}    ${BGP_APP_PEER_OPTIONS}
    Wait_Until_Console_Tool_Finish    ${bgp_filling_timeout}
...
Setup_Everything
    ...
    ${scritp_uri_opt}=    CompareStream.Set_Variable_If_At_Most_Boron    ${Empty}    --uri config/bgp-rib:application-rib/${BGP_APP_PEER_ID}/tables/bgp-types:ipv4-address-family/bgp-types:unicast-subsequent-address-family/

...
Setup_Everything
    ...
    Utils.Log_Message_To_Controller_Karaf    Starting the BGP ingestion test with ${COUNT} route(s) using change counter
...
Wait_For_Topology_To_Become_Empty
    ...
    Builtin.Run_Keyword_If    ${wasfilled}    Wait_For_Topology_To_Become_Unstable    ${timeout}    ${refresh}
...
Check_Topology_Stability
    ...
    ${actual_count}=    Builtin.Set_Variable    ${response.text}
    Builtin.Set_Suite_Variable    ${current_count}    ${actual_count}
    Builtin.Run_Keyword_If    ${expectstable}    Builtin.Should_Be_Equal    ${expected_count}    ${actual_count}
    Builtin.Run_Keyword_Unless    ${expectstable}    Builtin.Should_Not_Be_Equal    ${expected_count}    ${actual_count}
...
Wait_For_Topology_Stability
    ...
    ${actual}=    Builtin.Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    Check_Topology_Stability    expectstable=${expectstable}    passonhttperror=${stopathttperror}
    Fail_If_Status_Is_Wrong

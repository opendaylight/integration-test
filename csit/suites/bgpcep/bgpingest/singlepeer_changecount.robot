...
${RESULTS_FILE_NAME}    bgp-performance-changecount.csv
...
Start_Talking_BGP_speaker
    ...
    BGPSpeaker.Start_BGP_speaker    --amount ${COUNT_CHANGE_COUNT} --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --insert=${INSERT} --withdraw=${WITHDRAW} --prefill ${PREFILL} --${SCENARIO} --${BGP_TOOL_LOG_LEVEL} --results ${RESULTS_FILE_NAME}
...
Store_Results_For_Talking_BGP_Speaker
    ...
    Store_File_To_Workspace    ${RESULTS_FILE_NAME}    ${RESULTS_FILE_NAME}
    Store_File_To_Workspace    ${RESULTS_FILE_NAME}    talking-${RESULTS_FILE_NAME}
    Store_File_To_Workspace    ${BGPSpeaker__OUTPUT_LOG}    talking-${BGPSpeaker__OUTPUT_LOG}
...
Start_Listening_BGP_Speaker
    ...
    BGPSpeaker.Start_BGP_speaker    --amount ${COUNT_CHANGE_COUNT} --listen --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --insert=${INSERT} --withdraw=${WITHDRAW} --prefill ${PREFILL} --${SCENARIO} --${BGP_TOOL_LOG_LEVEL} --results ${RESULTS_FILE_NAME}
...
Store_Results_For_Listening_BGP_Speaker
    [Setup]    FailFast.Run_Even_When_Failing_Fast
    Store_File_To_Workspace    ${RESULTS_FILE_NAME}    ${RESULTS_FILE_NAME}
    Store_File_To_Workspace    ${RESULTS_FILE_NAME}    listening-${RESULTS_FILE_NAME}
    Store_File_To_Workspace    ${BGPSpeaker__OUTPUT_LOG}    listening-${BGPSpeaker__OUTPUT_LOG}
...
Setup_Everything
    ...
    Builtin.Set_Suite_Variable    ${bgp_filling_timeout}    ${count*6/10000+20}

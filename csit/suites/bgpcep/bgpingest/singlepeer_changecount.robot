...
Wait_For_Talking_Topology
    ...
    Wait_For_Topology_To_Become_Stable    timeout=${timeout_for_topology_filling}
...
Check_For_Empty_Topology_After_Talking
    ...
    [Tags]    critical
    Wait_For_Topology_To_Become_Unstable    timeout=180s
...
Wait_For_Topology_Stability
    [Arguments]    ${timeout}    ${check_period}    ${count}    ${expect_stable}
    ...
    Utils.Wait_For_Data_To_Satisfy_Keyword    ${timeout}    ${check_period}    ${count}    Get_Change_Count    Check_Topology_Stability    ${expect_stable}

Wait_For_Topology_To_Become_Stable
    [Arguments]    ${timeout}    ${check_period}=${CHECK_PERIOD_CHANGE_COUNTER} s    ${count}=${PASS_COUNT_CHANGE_COUNTER}    ${stop_at_http_error}=True
    [Documentation]    Wait until no more changes are happening in the topology.
    Wait_For_Topology_Stability    ${timeout}    ${check_period}    ${count}    expect_stable=True

Wait_For_Topology_To_Become_Unstable
    [Arguments]    ${timeout}    ${check_period}=${CHECK_PERIOD_CHANGE_COUNTER} s
    [Documentation]    Wait until changes start happening in the topology.
    Wait_For_Topology_Stability    ${timeout}    ${check_period}    1    expect_stable=False

Wait_For_Topology_To_Become_Empty
    [Arguments]    ${timeout}=10s    ${refresh}=${CHECK_PERIOD_CHANGE_COUNTER} s    ${count}=${PASS_COUNT_CHANGE_COUNTER}
    [Documentation]    Wait until topology becomes empty.
    Wait_For_Topology_To_Become_Stable    ${timeout}    ${refresh}    ${count}

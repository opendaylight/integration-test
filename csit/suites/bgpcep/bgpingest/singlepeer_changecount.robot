...
${HOLDTIME}       180
${COUNT}          100000
${CHECK_PERIOD_CHANGE_COUNTER}    5
${CHECK_PERIOD}    ${CHECK_PERIOD_CHANGE_COUNTER}
${current_count}    -1
...
Reconfigure_ODL_To_Accept_Connection
    ...
    Log_Message_To_Controller_Karaf    Reconfiguring ODL to accept a connection
...
Reconfigure_Data_Change_Counter
    [Documentation]    Configure data change counter to count transactions affecting example-ipv4-topology instead
    ...    example-linkstate-topology.
    Log_Message_To_Controller_Karaf    Reconfiguring data change counter to start counting transactions in topology
    ${template_as_string}=    BuiltIn.Set_Variable    {}
    Builtin.Wait_Until_Keyword_Succeeds    60s    1s    Put_Xml_Template_Folder_Config_Via_Restconf    ${directory_with_template_folders}${/}change_counter    ${template_as_string}
...
Start_Talking_BGP_speaker
    ...
    Log_Message_To_Controller_Karaf    Starting talking BGP speaker
    BuiltIn.Log    ${command}
...
Check_Talking_Connection_Is_Established
    ...
    Log_Message_To_Controller_Karaf    Checking the BGP speaker is connected
...
Wait_For_Talking_Topology
    ...
    Log_Message_To_Controller_Karaf    Waiting for BGP topology to become filled
...
Check_Talking_Topology_Count
    ...
    [Tags]    critical
    Log_Message_To_Controller_Karaf    Checking that all routes are in the topology
    Check_Topology_Count    ${COUNT}

Kill_Talking_BGP_Speaker
    [Documentation]    Abort the Python speaker. Also, attempt to stop failing fast.
    [Setup]    FailFast.Run_Even_When_Failing_Fast
    Log_Message_To_Controller_Karaf    Stopping the BGP speaker
    Kill_BGP_Speaker
...
Check_For_Empty_Topology_After_Talking
    ...
    [Tags]    critical
    Log_Message_To_Controller_Karaf    Waiting for topology to become empty
...
Start_Listening_BGP_Speaker
    ...
    Log_Message_To_Controller_Karaf    Starting listening BGP speaker
    Builtin.Log    ${command}
    ${output}=    SSHLibrary.Write    ${command}
    Read_And_Fail_If_Prompt_Is_Seen
...
Reconfigure_ODL_To_Initiate_Connection
    ...
    Log_Message_To_Controller_Karaf    Reconfiguring ODL to initiate the connection
...
Check_Listening_Connection_Is_Established
    [Documentation]    See TCP (BGP) connection in established state.
    Log_Message_To_Controller_Karaf    Checking that the BGP speaker is now connected
    Check_Speaker_Is_Connected

Wait_For_Listening_Topology
    [Documentation]    Wait until example-ipv4-topology becomes stable.
    Log_Message_To_Controller_Karaf    Waiting for the topology to fill up
    Wait_For_Topology_To_Become_Stable    ${timeout}    ${CHECK_PERIOD}

Check_Listening_Topology_Count
    [Documentation]    Count the routes in example-ipv4-topology and fail if the count is not correct.
    [Tags]    critical
    Log_Message_To_Controller_Karaf    Checking that all the routes are in the topology
    Check_Topology_Count    ${COUNT}
...
Delete_Bgp_Peer_Configuration
    ...
    Log_Message_To_Controller_Karaf    Deleting the BGP speaker configuration from ODL
...
Setup_Everything
    ...
    Flexible_SSH_Login    ${MININET_USER}    ${MININET_PASSWORD}
    ...
    ${count}=    Convert_To_Integer    ${COUNT}
    Set_Suite_Variable    ${timeout}    ${count/25+60} s
    # Report that the suite is being run.
    Log_Message_To_Controller_Karaf    Starting the BGP ingestion test with ${COUNT} route(s) using change counter
...
Fail_If_Status_Is_Wrong
    ...
    Return_From_Keyword_If    ${response_code} == 200
    Builtin.Log    ${response_content}
    Fail    The topology got wedged
...
Wait_For_Topology_To_Become_Empty
    ...
    Run_Keyword_If    ${wasfilled}    Wait_For_Topology_To_Become_Unstable    ${timeout}    ${refresh}
    Wait_For_Topology_To_Become_Stable    ${timeout}    ${refresh}    stopathttperror=${wasfilled}
...
Check_Topology_Is_Empty
    ...
    Run_Keyword_If    ${response_code} == 200    Should_Be_Equal_As_Strings    ${count}    0
...
Check_Topology_Stability
    ...
    ...    selected by passing ${expectunstable} = 0) or there are changes since last call
    ...
    Set_Suite_Variable    ${response_code}    ${response.status_code}
    Run_Keyword_Unless    ${passonhttperror}    Fail_If_Status_Is_Wrong
    Run_Keyword_If    ${response_code} <> 200    Set_Suite_Variable    ${response_content}    ${response.text}
    Run_Keyword_If    ${response_code} <> 200    Return_From_Keyword    -1
    ${expected_count}=    Set Variable    ${current_count}
    ${actual_count}=    Set Variable    ${response.text}
    Set_Suite_Variable    ${current_count}    ${actual_count}
    Run_Keyword_If    ${expectstable}    Should Be Equal    ${expected_count}    ${actual_count}
    Run_Keyword_Unless    ${expectstable}    Should Not Be Equal    ${expected_count}    ${actual_count}

Wait_For_Topology_To_Become_Stable
    [Arguments]    ${timeout}    ${check_period}    ${stopathttperror}=True
...
Wait_For_Topology_Stability
    [Arguments]    ${timeout}    ${check_period}    ${expectstable}    ${stopathttperror}=False
    ...
    Sleep    ${check_period}
    ${actual}=    Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    Check_Topology_Stability    expectstable=${expectstable}    passonhttperror=${stopathttperror}
    Fail_If_Status_Is_Wrong
...
Wait_For_Topology_To_Become_Unstable
    ...
    Wait_For_Topology_Stability    ${timeout}    ${check_period}    expectstable=False    stopathttperror=True
...
Get_Topology_Count_Core
    ...
    Run_Keyword_If    ${response_code} <> 200    Set_Suite_Variable    ${response_content}    ${response.text}
    Run_Keyword_If    ${response_code} <> 200    Return_From_Keyword    -1
    ${actual_count}=    Evaluate    len(re.findall('${prefix_pattern}', '''${response.text}'''))    modules=re
    Return_From_Keyword    ${actual_count}
...
Read_And_Fail_If_Prompt_Is_Seen
    ...
    Dump_Player_Error_Log
    Fail    The prompt was seen but it should not be seen.

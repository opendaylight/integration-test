...
Setup_Everything
    ...
    Log_Message_To_Controller_Karaf    Starting the BGP ingestion test with ${COUNT} route(s) using topology polling
...
Check_Topology_Is_Empty
    ...
    Evaluate    gc.collect()    modules=gc
...
Check_Topology_Is_Stable
    ...
    Evaluate    gc.collect()    modules=gc
    Run_Keyword_If    ${response_code} == 200    Should Be Equal    ${expected_count}    ${actual_count}
...
Wait_For_Topology_To_Become_Stable
    ...
    Sleep    ${check_period}
    ${actual}=    Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    Check_Topology_Is_Stable
    Fail_If_Status_Is_Wrong

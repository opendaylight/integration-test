...
Reconfigure_ODL_To_Accept_Connection
    ...
    ${template_as_string}=    BuiltIn.Set_Variable    {'IP': '${MININET}', 'HOLDTIME': '${HOLDTIME_PREFIX_COUNT}', 'INITIATE': 'false'}
...
Start_Talking_BGP_speaker
    ...
    ${command}=    BuiltIn.Set_Variable    python play.py --amount ${COUNT_PREFIX_COUNT} --myip=${MININET} --myport=17900 --peerip=${CONTROLLER} --peerport=1790 2>${player_error_log}
...
Check_Talking_Topology_Count
    ...
    Check_Topology_Count    ${COUNT_PREFIX_COUNT}
...
Start_Listening_BGP_Speaker
    ...
    ${command}=    BuiltIn.Set_Variable    python play.py --amount ${COUNT_PREFIX_COUNT} --listen --myip=${MININET} --myport=17900 --peerip=${CONTROLLER} 2>${player_error_log}
...
Reconfigure_ODL_To_Initiate_Connection
    ...
    ${template_as_string}=    BuiltIn.Set_Variable    {'IP': '${MININET}', 'HOLDTIME': '${HOLDTIME_PREFIX_COUNT}', 'INITIATE': 'true'}
    ConfigViaRestconf.Put_Xml_Template_Folder_Config_Via_Restconf    ${directory_with_template_folders}${/}bgp_peer    ${template_as_string}
...
Setup_Everything
    ...
    Utils.Log_Message_To_Controller_Karaf    Starting the BGP ingestion test with ${COUNT_PREFIX_COUNT} route(s) using topology polling
...
Check_Topology_Is_Empty
    [Arguments]    ${stop_at_http_error}=True
    ...
    Builtin.Run_Keyword_If    ${stop_at_http_error}    Fail_If_Status_Is_Wrong
...
Wait_For_Topology_To_Become_Empty
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    ${timeout}    ${refresh}    Check_Topology_Is_Empty    stop_at_http_error=${stop_at_http_error}
    Fail_If_Status_Is_Wrong

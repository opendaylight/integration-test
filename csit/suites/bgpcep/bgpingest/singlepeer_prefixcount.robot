...
Wait_For_Topology_Count_0
    ...
    KarafKeywords.Log_Message_To_Controller_Karaf    DEBUG: Checking that all routes are withdrawn, starting ...)
    ${actual}=    Builtin.Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    BGPKeywords.Check_Topology_Count    0
    KarafKeywords.Log_Message_To_Controller_Karaf    DEBUG: Checking that all routes are withdrawn, status    ${actual}
    BuiltIn.Log    ${actual}
    builtin

...
Start_Talking_BGP_speaker
    ...
    KarafKeywords.Log_Message_To_Controller_Karaf    DEBUG: BGP Speaker is going to speek
    BGPSpeaker.Start_BGP_speaker    --amount ${COUNT_PREFIX_COUNT} --myip=${MININET} --myport=${BGP_TOOL_PORT} --peerip=${CONTROLLER} --peerport=${ODL_BGP_PORT} --withdrawal yes --idle=60
...
Wait_For_Topology_Count_0
    ...
    KarafKeywords.Log_Message_To_Controller_Karaf    DEBUG: Staring to check that all routes are withdrawn.
    KarafKeywords.Log_Message_To_Controller_Karaf    Count_Prefixes_In_Topology_Data
    ${actual}=    Builtin.Wait_Until_Keyword_Succeeds    ${timeout}    ${check_period}    BGPKeywords.Check_Topology_Count    0
    KarafKeywords.Log_Message_To_Controller_Karaf    DEBUG: Stopping the check that all routes are withdrawn.
    KarafKeywords.Log_Message_To_Controller_Karaf    Count_Prefixes_In_Topology_Data
    BuiltIn.Log    ${actual}

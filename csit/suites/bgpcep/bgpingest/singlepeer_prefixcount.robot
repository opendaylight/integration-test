...
Start_Talking_BGP_Speaker
    ...
    Get_Sysstat_Results
    BGPSpeaker.Start_BGP_Speaker    --amount 100000 --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --insert=${INSERT} --withdraw=${WITHDRAW} --prefill ${PREFILL} --update ${UPDATE} --${BGP_TOOL_LOG_LEVEL} --results ${RESULTS_FILE_NAME}
...
Get_Sysstat_Results
    ${output}    Set Variable    no results
    ${status}    ${output}    Run_Keyword_And_Ignore_Error    SSHKeywords.Run_Keyword_With_Ssh    ${ODL_SYSTEM_IP}    SSHLibrary.Write    ls /etc/var/sysstat
    Log    ${output}
    ${status}    ${output}    Run_Keyword_And_Ignore_Error    SSHKeywords.Run_Keyword_With_Ssh    ${TOOLS_SYSTEM_IP}    SSHLibrary.Write    ls /etc/var/sysstat
    Log    ${output}
    ${status}    ${output}    Run_Keyword_And_Ignore_Error    SSHKeywords.Run_Keyword_With_Ssh    ${ODL_SYSTEM_IP}    SSHLibrary.Write    sar -A -f /etc/var/sysstat/sa*
    Log    ${output}
    ${status}    ${output}    Run_Keyword_And_Ignore_Error    SSHKeywords.Run_Keyword_With_Ssh    ${TOOLS_SYSTEM_IP}    SSHLibrary.Write    sar -A -f /etc/var/sysstat/sa*
    Log    ${output}

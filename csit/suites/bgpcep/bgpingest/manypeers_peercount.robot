...
Check_Logs_For_Updates
    ...
    \    ${expected_prefixcount} =   BuiltIn.Evaluate    ${COUNT_PREFIX_COUNT_MANY} / ${MULTIPLICITY_PREFIX_COUNT_MANY}
    \    ${expected_string} =   BuiltIn.Set_Variable    total_received_nlri_prefix_counter: ${expected_prefixcount}
    \    BuiltIn.Wait_Until_Keyword_Succeeds    ${bgp_filling_timeout}    1s    Check_File_For_Occurence    ${BGP_PEERS_LOG_FILE_NAME}    ${bgp_peer_label}    ${expected_string}    2
    # FIXME: Calculation correct when divided without fraction only?

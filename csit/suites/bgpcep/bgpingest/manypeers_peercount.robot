...
*** Test Cases ***
Check_That_Ipv4_Topology_Is_Empty
    [Documentation]    Wait and check that the example-ipv4-topology is empty.
    BuiltIn.Wait_Until_Keyword_Succeeds    120s    1s    Check_Ipv4_Topology    0
...
Reconfigure_ODL_To_Accept_Connections
    ...
    \    ${template_as_string} =    BuiltIn.Set_Variable    {'NAME': '${peer_name}', 'IP': '${peer_ip}', 'PEER_PORT': '${BGP_TOOL_PORT}', 'HOLDTIME': '${HOLDTIME_PREFIX_COUNT_MANY}', 'PEER_ROLE': 'rr-client', 'INITIATE': 'false'}
...
Start_Talking_BGP_Manager
    ...
    BGPSpeaker.Start_BGP_Manager    --amount=${COUNT_PREFIX_COUNT_MANY} --multiplicity=${MULTIPLICITY_PREFIX_COUNT_MANY} --myip=${FIRST_PEER_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --logfile=${BGP_PEERS_LOG_FILE_NAME} --${BGP_TOOL_LOG_LEVEL}

Wait_For_Ipv4_Topology
    [Documentation]    Wait until example-ipv4-topology reaches the target prefix count.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${bgp_filling_timeout}    period=${CHECK_PERIOD_PREFIX_COUNT_MANY}    Check_Ipv4_Topology    ${COUNT_PREFIX_COUNT_MANY}
...
Check_Logs_For_Updates
    ...
    \    ${expected_prefixcount} =   BuiltIn.Evaluate    ${COUNT_PREFIX_COUNT_MANY} - ${COUNT_PREFIX_COUNT_MANY} / ${MULTIPLICITY_PREFIX_COUNT_MANY}
    \    ${expected_string} =   BuiltIn.Set_Variable    total_received_nlri_prefix_counter: ${expected_prefixcount}
    \    BuiltIn.Wait_Until_Keyword_Succeeds    ${timeout}    1s    Check_File_For_Occurence    ${BGP_PEERS_LOG_FILE_NAME}    ${bgp_peer_label}    ${expected_string}    2
    \    ${timeout} =    BuiltIn.Set_Variable    20s
    # FIXME: Calculation of ${expected_prefixcount} correct just when the ${COUNT_PREFIX_COUNT_MANY} is a multiplication of ${MULTIPLICITY_PREFIX_COUNT_MANY}
...
Wait_For_Empty_Ipv4_Topology
    [Documentation]    Wait until example-ipv4-topology reaches the target prefix count.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${bgp_emptying_timeout}    ${CHECK_PERIOD_PREFIX_COUNT_MANY}    Check_Ipv4_Topology    0
...
Check_Ipv4_Topology
    ...
    BuiltIn.Log    ${current_time}: prefix count is ${actual_count} of ${count}     console=yes

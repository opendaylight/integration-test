...
Check_Bgp_Peer_Updates_For_Prefilled_Routes
    ...
    BuiltIn.Wait Until Keyword Succeeds    ${bgp_filling_timeout}    1s    Check_File_For_Word_Count    bgp_peer.log    total_received_nlri_prefix_counter: ${PREFILL}    2    or_more=True

BGP_Application_Peer_Introduce_Single_Routes
    [Documentation]    Start BGP application peer tool and introduce routes.
    SSHLibrary.Switch Connection    bgp_app_peer_console
    Start_Console_Tool    pypy bgp_trial_peer.py --host ${ODL_SYSTEM_IP} --port ${RESTCONFPORT} --command add --count ${remaining_prefixes} --batchsize 1 --prefix 12.0.0.0 --prefixlen 28    ${BGP_APP_PEER_OPTIONS}
    Wait_Until_Console_Tool_Finish    ${bgp_filling_timeout}
...
Check_Bgp_Peer_Updates_For_All_Routes
    [Documentation]    Count the routes introduced by updates.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Fast_Failing
    BuiltIn.Wait Until Keyword Succeeds    ${bgp_filling_timeout}    1s    Check_File_For_Word_Count    bgp_peer.log    total_received_nlri_prefix_counter: ${COUNT_APP_PEER_PREFIX_COUNT}    2    or_more=True
...
Check_Bgp_Peer_Updates_For_Prefix_Withdrawals
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Fast_Failing
    BuiltIn.Wait Until Keyword Succeeds    ${bgp_emptying_timeout}    1s    Check_File_For_Word_Count    bgp_peer.log    total_received_withdrawn_prefix_counter: ${COUNT_APP_PEER_PREFIX_COUNT}    2    or_more=True

...
#    \    ${peer_ip} =    BuiltIn.Evaluate    str(ipaddr.IPAddress('${FIRST_PEER_IP}') + ${index} - 1)    modules=ipaddr
    ...
    \    TemplatedRequests.Put_As_Xml_Templated    ${POLICIES_VAR}/npath/${peer_type}    mapping=${mapping}    session=${CONFIG_SESSION}

Start_Exabgps
    ${start_cmd}    BuiltIn.Set_Variable    ${CMD} exabgp.cfg > exa2.log &
...
Delete_Bgp_Peer_Configuration
    ...
    \    TemplatedRequests.Delete_Templated    ${POLICIES_VAR}/npath/${peer_type}    mapping=${mapping}    session=${CONFIG_SESSION}

Deconfigure_App_Peer
    [Documentation]    Revert the BGP configuration to the original state: without application peer
    &{mapping}    BuiltIn.Create_Dictionary    APP_PEER_NAME=${APP_PEER_NAME}    IP=${ODL_SYSTEM_IP}    BGP_RIB_OPENCONFIG=${RIB_INSTANCE}    AMOUNT=1
...
Stop_All_Peers
    Run Keyword And Ignore Error    BGPcliKeywords.Store_File_To_Workspace    exa2.log    exa2.log
...
Upload_Config_Files
    ...
    SSHLibrary.Put_File    ${POLICIES_VAR}/exabgp.cfg    .

...
Topology_Precondition
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    300    1    Compare_Topology    ${off_json}    010_Precondition.json
...
Topology_Intercondition
    ...
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    5s    1s    Compare_Topology    ${default_json}    050_Intercondition.json
...
Topology_Updated
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_succeeds    10s    1s    Compare_Topology    ${updated_json}    060_Topology_Updated.json
...
Topology_Unauthorized_3
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    Compare_Topology    ${offjson}    070_Unauthorized_4.json
...
Compare_Topology
    ...
    BuiltIn.Log    ${normexp}
    ${resp}=    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    BuiltIn.Log    ${resp}

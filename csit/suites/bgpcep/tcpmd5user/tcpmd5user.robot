...
Topology_Unauthorized_1
    ...
    [Tags]    critical
    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_off   session=${CONFIG_SESSION}    verify=True
...
Topology_Intercondition
    ...
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    CODE=${path_name_code}
    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_on    ${mapping}   ${CONFIG_SESSION}    verify=True
...
Topology_Updated
    ...
    [Tags]    critical
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    CODE=${path_name_code}
    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_on_updated   session=${CONFIG_SESSION}    verify=True
...
Set_It_Up
    ...
    ${code}=    Evaluate    binascii.b2a_base64('pcc_' + '${TOOLS_SYSTEM_IP}' + '_tunnel_1')[:-1]    modules=binascii
    BuiltIn.Set_Suite_Variable    ${path_name_code}    ${code}
    FailFast.Do_Not_Fail_Fast_From_Now_On

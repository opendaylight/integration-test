...
Topology_Precondition
    ...
    [Tags]    critical
    ${off_json_normalized}=    Normalize_And_Save_Expected_Json    ${off_json}    010_Precondition.json    ${directory_for_expected_responses}
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    300    1    Compare_Topology    ${off_json_normalized}    010_Precondition.json
...
Start_Secure_Pcc_Mock
    ...
    ${resp}=    Read_And_Fail_If_Prompt_Is_Seen
...
Topology_Unauthorized_1
    ...
    [Tags]    critical
    ${off_json_normalized}=    Normalize_And_Save_Expected_Json    ${off_json}    020_Unauthorized_1.json    ${directory_for_expected_responses}
...
Set_Wrong_Password
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    Replace_Password_Xml_Element_In_Pcep_Client_Module    password=changeme
    ${password_line}=    CompareStream.Run_Keyword_If_Less_Than_Oxygen    Construct_Password_Element_Line_Using_Password    password=changeme
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    Replace_Password_Xml_Element_In_Pcep_Client_Module_Less_Than_Oxygen    ${password_line}

Topology_Unauthorized_2
    [Documentation]    The same logic as Topology_Unauthorized_1 as incorrect password was provided to ODL.
    [Tags]    critical
    ${off_json_normalized}=    Normalize_And_Save_Expected_Json    ${off_json}    040_Unauthorized_3.json    ${directory_for_expected_responses}
...
Set_Correct_Password
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    Replace_Password_Xml_Element_In_Pcep_Client_Module    password=topsecret
    ${password_line}=    CompareStream.Run_Keyword_If_Less_Than_Oxygen    Construct_Password_Element_Line_Using_Password    password=topsecret
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    Replace_Password_Xml_Element_In_Pcep_Client_Module_Less_Than_Oxygen    ${password_line}

Topology_Intercondition
    [Documentation]    Compare pcep-topology to filled one, which includes a tunnel from pcc-mock.
    ${default_json_normalized}=    Normalize_And_Save_Expected_Json    ${default_json}    050_Intercondition.json    ${directory_for_expected_responses}
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    5s    1s    Compare_Topology    ${default_json_normalized}    050_Intercondition.json
...
Topology_Updated
    ...
    [Tags]    critical
    ${updated_json_normalized}=    Normalize_And_Save_Expected_Json    ${updated_json}    060_Topology_Updated.json    ${directory_for_expected_responses}
    ${response}=    BuiltIn.Wait_Until_Keyword_succeeds    5s    1s    Compare_Topology    ${updated_json_normalized}    060_Topology_Updated.json
...
Unset_Password
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    Unset_Password_Xml_Element_In_Pcep_Client_Module    password=${EMPTY}
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    Replace_Password_Xml_Element_In_Pcep_Client_Module_Less_Than_Oxygen    password_element=${EMPTY}
...
Topology_Unauthorized_3
    ...
    [Tags]    critical
    ${offjson_normalized}=    Normalize_And_Save_Expected_Json    ${offjson}    070_Unauthorized_4.json    ${directory_for_expected_responses}
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    5s    1s    Compare_Topology    ${offjson_normalized}    070_Unauthorized_4.json
...
Topology_Postcondition
    ...
    [Tags]    critical
    ${offjson_normalized}=    Normalize_And_Save_Expected_Json    ${offjson}    080_Postcondition.json    ${directory_for_expected_responses}
    WaitForFailure.Verify_Keyword_Does_Not_Fail_Within_Timeout    10    1    Compare_Topology    ${offjson_normalized}    080_Postcondition.json
...
Set_It_Up
    ...
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    BuiltIn.Run_Keyword_If    """${USE_NETCONF_CONNECTOR}""" == """False"""    Install_Netconf_Connector
...
Tear_It_Down
    ...
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    BuiltIn.Run_Keyword_If    """${USE_NETCONF_CONNECTOR}""" == """False"""    Uninstall_Netconf_Connector
    BuiltIn.Log    ${diff}
...
Compare_Topology
    ...
    ${normresp}=    Normalize_And_Save_Expected_Json    ${resp.text}    ${filename}    ${directory_for_expected_responses}
...
Unset_Password_Xml_Element_In_Pcep_Client_Module
    [Arguments]    ${password}
    [Documentation]    Send restconf PUT to unset the config module.
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    PASSWD=${password}
    TemplatedRequests.Put_As_Xml_Templated    ${directory_with_template_folders}${/}pcep_topology_client_module_empty    mapping=${mapping}
...
Replace_Password_Xml_Element_In_Pcep_Client_Module_Less_Than_Oxygen
    ...
    TemplatedRequests.Put_As_Xml_Templated    ${directory_with_template_folders}${/}pcep_topology_cm_lt_oxygen    mapping=${mapping}

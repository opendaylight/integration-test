...
Update_Delegated
    ...
    ${response}=    RequestsLibrary.Post Request    /restconf/operations/network-topology-pcep:update-lsp    ${update_delegated_xml}
...
Set_It_Up
    ...
    RequestsLibrary.Create_Session    ${CURRENT_SESSION}    http://${ODL_SYSTEM_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Compare_Topology
    ...
    ${resp}=    RequestsLibrary.Get_Request    ${CURRENT_SESSION}    /restconf/operational/network-topology:network-topology/topology/pcep-topology
    BuiltIn.Log    ${resp}
    ...
    Run Keyword And Ignore Error    ${normresp}    ${normexp}

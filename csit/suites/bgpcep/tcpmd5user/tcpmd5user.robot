...
Compare_Topology
    ...
    ${status}    ${output}=    Run Keyword And Ignore Error    CompareStream.Run_Keyword_If_At_Least_Oxygen
    ...    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    pcep:node-state -topology-id pcep-topology -node-id pcc://${ODL_SYSTEM_IP}
    Run Keyword And Ignore Error    Log    ${output}
        ${status}    ${output}=    Run Keyword And Ignore Error    CompareStream.Run_Keyword_If_At_Least_Oxygen
    ...    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    pcep:node-state -topology-id pcep-topology -node-id pcc://${TOOLS_SYSTEM_IP}
    Run Keyword And Ignore Error    Log    ${output}

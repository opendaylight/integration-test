...
Verify Pcep Session State Data
    ...
    ${rsp}=    TemplatedRequests.Get_Templated    ${DIR_WITH_TEMPLATES}${/}${exprspfile}.${ODL_STREAM}    ${mapping}    session=${CONFIG_SESSION}
    ${rsp_normalized} =    norm_json.normalize_json_text    ${rsp}    keys_with_volatiles=${keys_with_volatiles}
    TemplatedRequests.Verify_Response_Templated    ${rsp_normalized}    ${DIR_WITH_TEMPLATES}${/}${exprspfile}.${ODL_STREAM}    mapping=${mapping}    normalize_json=True
#

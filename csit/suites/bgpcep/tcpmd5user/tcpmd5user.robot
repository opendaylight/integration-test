...
Topology_Intercondition
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_on    ${mapping}   ${CONFIG_SESSION}    verify=True
    CompareStream.Run_Keyword_If_At_Least_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_on_state    ${mapping}   ${CONFIG_SESSION}    verify=True
...
Topology_Updated
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    TemplatedRequests.Get_As_Json_Templated    ${DIR_WITH_TEMPLATES}${/}default_on_updated_state    ${mapping}   ${CONFIG_SESSION}    verify=True
...
Verify Pcep Session State Data
    ...
    ${keys_with_volatiles}=    BuiltIn.Create_List    odl-pcep-stateful-stats:last-received-rpt-msg-timestamp    session-duration    last-sent-msg-timestamp

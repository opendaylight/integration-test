...
#${CONNECTOR_FEATURE}    odl-netconf-connector-ssh
...
Topology_Precondition
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    300    1    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    Compare_Topology    ${off_json}    010_Precondition.json    ${response}
...
Topology_Intercondition
    ...
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    Compare_Topology    ${default_json}    050_Intercondition.json    ${response}
...
Topology_Updated
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_succeeds    5s    1s    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    Compare_Topology    ${updated_json}    060_Topology_Updated.json    ${response}
...
Topology_Unauthorized_3
    ...
    [Tags]    critical
    ${response}=    BuiltIn.Wait_Until_Keyword_Succeeds    10s    1s    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    Compare_Topology    ${offjson}    070_Unauthorized_4.json    ${response}
...
Tear_It_Down
    ...
    ${status}    ${pccmocklog}=    Run Keyword And Ignore Error    OperatingSystem.Run    cat pccmock.log
    Run Keyword And Ignore Error    BuiltIn.Log    ${pccmocklog}
    ${status}    ${diff}=    Run Keyword And Ignore Error    OperatingSystem.Run    diff -dur ${directory_for_expected_responses} ${directory_for_actual_responses}
...
Compare_Topology
    [Arguments]    ${expected}    ${name}    ${response}=${EMPTY}
    ...
    BuiltIn.Log    ${normexp}
    ${resp}=    Run Keyword If    "${response}"!="${EMPTY}"   Set Variable    ${response}
    ...    ELSE    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${NETWORK_TOPO_URI}topology/pcep-topology
    BuiltIn.Log    ${resp}

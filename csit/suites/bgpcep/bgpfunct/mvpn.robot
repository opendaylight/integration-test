...
Odl_To_Play_Template
    ...
    BgpRpcClient.play_clean
    Run Keyword If    '${totest}' == 'intra_ipv6'    Remove_Configured_Routes    ipv6
    ...    ELSE    Remove_Configured_Routes
    ${update}    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    Get_Update_Message
...
Play_To_Odl_Template
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_ADJ_RIB_IN}    session=${CONFIG_SESSION}    verify=True
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_EFFECTIVE_RIB_IN}    session=${CONFIG_SESSION}    verify=True
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_LOC_RIB}    session=${CONFIG_SESSION}    verify=True
    BgpRpcClient.play_send    ${withdraw_hex}
    ${ipv}    BuiltIn.Set_Variable_If    '${totest}' == 'intra_ipv6'    ipv6    ipv4
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/empty_routes_${ipv}    mapping=${MVPN_APP_PEER}    session=${CONFIG_SESSION}    verify=True
...
Remove_Configured_Routes
    ...
    TemplatedRequests.Delete_Templated    ${MVPN_DIR}/empty_routes_${ipv}    mapping=${MVPN_APP_PEER}    session=${CONFIG_SESSION}

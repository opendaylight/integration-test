...
Play_To_Odl_Template
    ...
    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_ADJ_RIB_IN}    session=${CONFIG_SESSION}    verify=True
    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_EFFECTIVE_RIB_IN}    session=${CONFIG_SESSION}    verify=True
    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/${totest}/rib    mapping=${MVPN_LOC_RIB}    session=${CONFIG_SESSION}    verify=True
    BgpRpcClient.play_send    ${withdraw_hex}
    ${ipv}    BuiltIn.Set_Variable_If    '${totest}' == 'intra_ipv6'    ipv6    ipv4
    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/empty_routes_${ipv}    mapping=${MVPN_APP_PEER}    session=${CONFIG_SESSION}    verify=True
...
Withdraw_Route_And_Verify
    ...
    BgpRpcClient.play_send    ${withdraw_hex}
    TemplatedRequests.Get_As_Json_Templated    ${MVPN_DIR}/empty_routes    mapping=${MVPN_APP_PEER}    session=${CONFIG_SESSION}    verify=True

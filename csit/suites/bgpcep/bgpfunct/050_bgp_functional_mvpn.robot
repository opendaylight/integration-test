...
#    Odl_To_Play_intra_as_impmsi_ad
#        [Template]    Odl_To_Play_Template
#        intra_as_impmsi_ad

Play_To_Odl_intra_as_impmsi_ad
...
Start_Suite
    ...
    BuiltIn.Set_Suite_Variable    ${MVPN_CONF_URL}    /restconf/config/bgp-rib:application-rib/${app_rib}/tables/bgp-types:ipv4-address-family/bgp-mvpn:mcast-vpn-subsequent-address-family/bgp-mvpn-ipv4:mpvn-routes
    BuiltIn.Set_Suite_Variable    ${MVPN_LOC_RIB}    /restconf/operational/bgp-rib:bgp-rib/rib/${bgp_rib}/loc-rib/tables/bgp-types:ipv4-address-family/bgp-mvpn:mcast-vpn-subsequent-address-family/bgp-mvpn-ipv4:mpvn-routes
...
Start_Bgp_Peer
    ...
    BGPSpeaker.Start_BGP_Speaker    --amount 0 --myip=${TOOLS_SYSTEM_IP} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --${BGP_TOOL_LOG_LEVEL} --mvpn --wfr 1
...
Odl_To_Play_Template
    ...
    ${resp}=    RequestsLibrary.Post_Request    ${CONFIG_SESSION}    ${MVPN_CONF_URL}    data=${post_data_xml}    headers=${HEADERS_XML}
...
Get_Update_Content
    ...
    ${resp}=    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${MVPN_LOC_RIB}    headers=${HEADERS_XML}
    BuiltIn.Log    ${resp.content}
...
Loc_Rib_Presence
    ...
    ${status}    ${rsp}    Run Keyword And Ignore Error    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    ${MVPN_LOC_RIB}    headers=${HEADERS}

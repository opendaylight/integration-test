...
Verify_No_Routes
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    3s    TemplatedRequests.Get_As_Json_Templated    ${GR_FOLDER}${/}empty_route    session=${CONFIG_SESSION}    verify=True
...
Verify_Update_Message
    ...
    ${actual} =    BuiltIn.Wait_Until_Keyword_Succeeds    5x    3s    Get_Update_Message    peer=${peer}
    BgpOperations.Verify_Two_Hex_Messages_Are_Equal    ${expected}    ${actual}

Get_Update_Message
    [Arguments]    ${peer}=${PEER1_IP}
    ${update} =    Run_Keyword_If    "${peer}" == "${PEER1_IP}"    BgpRpcClient1.play_get

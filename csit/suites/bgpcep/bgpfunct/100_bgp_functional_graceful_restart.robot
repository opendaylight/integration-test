...
${GRACEFUL_RESTART_FOLDER}    DIR}/../../../variables/bgpuser/
...
Configure_BGP_Peers
    ...
    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true    PEER_TYPE=INTERNAL    RR_CLIENT=false
    ...
    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true    PEER_TYPE=EXTERNAL    AS_NUMBER=${eBGP_PEER1_AS}
    TemplatedRequests.Put_As_Xml_Templated    ${GRACEFUL_RESTART_FOLDER}${/}peers    mapping=${mapping}    session=${CONFIG_SESSION}
    &{mapping}    BuiltIn.Create_Dictionary    IP=${eBGP_PEER2_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}
    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true    PEER_TYPE=EXTERNAL    AS_NUMBER=${eBGP_PEER2_AS}
    TemplatedRequests.Put_As_Xml_Templated    ${GRACEFUL_RESTART_FOLDER}${/}peers    mapping=${mapping}    session=${CONFIG_SESSION}
...
Post_Graceful_Restart_To_Internal_Peer
    ...
    TemplatedRequests.Put_As_Xml_Templated    ${GRACEFUL_RESTART_FOLDER}${/}restart    mapping=${mapping}    session=${CONFIG_SESSION}
...
Delete_Bgp_Peers_Configuration
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    &{mapping}    BuiltIn.Create_Dictionary    IP=${iBGP_PEER1_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}
    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true    PEER_TYPE=EXTERNAL    RR_CLIENT=false

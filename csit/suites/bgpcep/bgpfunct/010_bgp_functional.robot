...
L3vpn Ipv4
    ...
    [Teardown]    Stop_Tool_And_verify   ${L3VPN_URL}    ${L3VPN_RSPEMPTY}

Exa To Send Route Request
    [Documentation]     Sends route requests and checks fi exabgp receives it
    [Setup]    Start Tool And Verify      ${DEFAUTL_RPC_CFG}
    ExaClient.announce     announce route-refresh ipv4 unicast
    # toto some verification on receiving updates
    [Teardown]    Stop_Tool

Odl To Send Route Request
    [Documentation]     Sends route requests and checks fi exabgp receives it
    [Setup]    Start Tool And Verify      ${DEFAUTL_RPC_CFG}
    ExaClient.clean_received_route_refresh_count

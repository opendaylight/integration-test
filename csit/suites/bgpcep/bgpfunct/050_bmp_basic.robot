...
Start_Bmp_Mock
    [Documentation]    Execute bmp-mock on Mininet, fail is Open is not sent, keep it running for next tests.
    ${command}=    NexusKeywords.Compose_Full_Java_Command    -jar ${filename} --local-address ${TOOLS_SYSTEM_IP} --remote-address ${ODL_SYSTEM_IP} 2>&1 | tee bmpmock.log
    Log    ${command}
    Write    ${command}
...
Stop_Bmp_Mock
    [Documentation]    Send ctrl+c to bmp-mock, fails if no prompt is seen
...
Tear_It_Down
    ...
    SSHLibrary.Get_File    bmpmock.log
    ${bmpmocklog}=    Run    cat bmpmock.log
    Log    ${bmpmocklog}

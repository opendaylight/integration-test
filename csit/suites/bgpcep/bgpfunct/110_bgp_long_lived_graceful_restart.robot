...
TC_2
    ...
    [Documentation]    Prerequistes: One peer with two routes was just killed.
    ...    Restart killed peer with just one route. Verify only one route is present in loc-rib.
    Start_Bgp_Peer    prefix=${SECOND_PREFIX}    myip=${PEER2_IP}    port=${PEER2_PORT}    as_number=${PEER2_AS}    log_name=ll_gr_tc2.out    amount=0    grace=0
    Verify_Routes    dir=ll_gr/ipv4_1
    BuiltIn.Sleep    5s
    Verify_Empty_Hex_Message
    [Teardown]    Teardown_TC    ll_gr_tc2.out
...
Verify_Hex_Message
    ...
    ${expected} =    TemplatedRequests.Resolve_Text_From_Template_File    ${GR_FOLDER}${/}${file_dir}    message.hex
...
Verify_Empty_Hex_Message
    ...
    BuiltIn.Should_Be_Equal    ${hex}    ${EMPTY}

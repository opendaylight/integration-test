...
Verify_Odl_Advertize_Empty_Message_To_Peer_Without_LL_GR_Enabled
    ...
    [Setup]    Setup_TC_LL_GR_Route
    Start_Bgp_Peer    prefix=${SECOND_PREFIX}    myip=${PEER2_IP}    port=${PEER2_PORT}    as_number=${PEER2_AS}    log_name=ll_gr_tc3.out    amount=0
    ...    grace=0
    Verify_Routes    dir=ll_gr/ipv4_1
...
Verify_Route_With_No_LL_GR_Community_Removal
    ...
    [Setup]    Setup_TC3
    Kill_Talking_BGP_Speakers    log_name=ll_gr_tc4.out
    BuiltIn.Sleep    5s
    Verify_Routes    retry=8x    interval=1s
    [Teardown]    Teardown_TC    ll_gr_tc4.out    0s
...
Setup_TC4
    ...
    Verify_Peer_Connected    ip=${PEER1_IP}
...
Verify_Peer_Connected
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    3s    TemplatedRequests.Get_As_Json_Templated    ${LL_GR_FOLDER}${/}empty_peer_rib    session=${CONFIG_SESSION}    mapping=${mapping}    verify=True

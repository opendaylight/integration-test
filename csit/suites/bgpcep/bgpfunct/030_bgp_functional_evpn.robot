...
Odl_To_Play_bidir_pim_tree
    [Template]    Odl_To_Play_Template
    bidir_pim_tree

Play_To_Odl_bidir_pim_tree
    [Template]    Play_To_Odl_Template
    bidir_pim_tree
...
Odl_To_Play_Template
    ...
    BuiltIn.Log    ${aupdate}
    ${status}    Run Keyword And Ignore Error    BuiltIn.Should_Be_Equal_As_Strings    ${aupdate}    ${announce_hex}
    BuiltIn.Log    ${status}
    BgpRpcClient.play_clean
    Remove_Configured_Routes
    ${wupdate} =    BuiltIn.Wait_Until_Keyword_Succeeds    4x    2s    Get_Update_Content
    BuiltIn.Log    ${wupdate}
    ${status}    Run Keyword And Ignore Error    BuiltIn.Should Be Equal As Strings    ${wupdate}    ${withdraw_hex}
    BuiltIn.Log    ${status}
...
Loc_Rib_Presence
    ...
    ${rsp} =    RequestsLibrary.Get_Request    ${CONFIG_SESSION}    /restconf/operational/bgp-rib:bgp-rib/rib/example-bgp-rib    headers=${HEADERS}

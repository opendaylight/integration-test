...
Verify_Test_Preconditions
    ${resp} =    RequestsLibrary.GET On Session    alias=${CONFIG_SESSION}    url=${EVPN_CONF_URL}?content=config
...
Remove_Configured_Routes
    ...
    ${rsp} =    RequestsLibrary.GET On Session    alias=${CONFIG_SESSION}    url=${EVPN_LOC_RIB}    headers=${HEADERS}
    Log    ${rsp.content}
    ${rsp} =    RequestsLibrary.GET On Session
    ...    alias=${CONFIG_SESSION}
    ...    url=${EVPN_CONF_URL}?content=config

...
${OPENCONFIG_PEER_URI}    /restconf/config/openconfig-network-instance:network-instances/network-instance/global-bgp/openconfig-network-instance:protocols/protocol/openconfig-policy-types:BGP/${PROTOCOL_OPENCONFIG}/bgp/neighbors/neighbor
${DEFAULT_BGPCEP_LOG_LEVEL}    INFO

*** Test Cases ***
Configure_App_Peer
    [Documentation]    Configures bgp application peer. Openconfig is used for carbon and above.
    &{mapping}    BuiltIn.Create_Dictionary    DEVICE_NAME=${DEVICE_NAME}    APP_PEER_NAME=${APP_PEER_NAME}    RIB_INSTANCE_NAME=${RIB_INSTANCE}    APP_PEER_ID=${ODL_SYSTEM_IP}    BGP_RIB_OPENCONFIG=${PROTOCOL_OPENCONFIG}    IP=${ODL_SYSTEM_IP}
...
L3vpn_Ipv4_From_Odl
    ...
    ${route_dir}=    CompareStream.Set_Variable_If_At_Least_Carbon    ${BGP_L3VPN_DIR}/route_openconfig    ${BGP_L3VPN_DIR}/route
    TemplatedRequests.Post_As_Xml_Templated    ${route_dir}    mapping=${mapping}    session=${CONFIG_SESSION}
    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_ExaBgp_Received_Update    ${route_dir}/exa-expected.json
...
Teardowm_With_Remove_Route
    ...
    ${route_dir}=    CompareStream.Set_Variable_If_At_Least_Carbon    ${BGP_L3VPN_DIR}/route_openconfig    ${BGP_L3VPN_DIR}/route
    TemplatedRequests.Delete_Templated    ${route_dir}    mapping=${mapping}    session=${CONFIG_SESSION}
    Teardown_Simple

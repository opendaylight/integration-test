*** Settings ***
Documentation     Functional test for bgp - l3vpn-ipv4
...
...
...               It uses exabgp odl and exabgpa as bgp peers. Route advertized from odl
...               are configured via application peer. Route advertised from exabgp is
...
Start_Tool
    ...
    BuiltIn.Log_To_Console    Start Tool with ${cmd} ${cfg_file}
    ${output}=    SSHLibrary.Write    ${cmd} ${cfg_file}
    BuiltIn.Log    ${output}
    BuiltIn.Log_To_Console    ${output}

Start_Tool_With_Verification
    [Arguments]    ${cfg_file}
    [Documentation]    Starts the tool and verifies started
    Start_Tool    ${cfg_file}
    SSHLibrary.Set_Client_Configuration    timeout=5s
    ${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${output}
    BuiltIn.Log_To_Console    ${output}
    BuiltIn.Run_Keyword_If    '${status}' != 'PASS'   Builtin.Fail    The prompt was seen but it was not expected yet

Start_Tool_And_Verify_Started
    [Arguments]    ${cfg_file}
    [Documentation]    Starts the tool and verifies started
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    1s    Start_Tool_With_Verification      ${cfg_file}
    #Start_Tool    ${cfg_file}
    #${status}    ${output}=    BuiltIn.Run_Keyword_And_Ignore_Error    SSHLibrary.Read Until Prompt
    #BuiltIn.Log    ${output}
    #BuiltIn.Log_To_Console    ${output}
    #BuiltIn.Return_From_Keyword_If    '${status}' != 'PASS'    ${Empty}

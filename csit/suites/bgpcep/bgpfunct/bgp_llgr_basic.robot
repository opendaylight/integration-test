...
${BINARY_URL}    https://github.com/osrg/gobgp/releases/download/v2.18.0/gobgp_2.18.0_linux_386.tar.gz
...
Download_Gobgp_Binary
    ...
    ${download-url} =      Set Variable        ${BINARY_URL}
    ${cmd1} =     Set Variable      wget ${download-url}
    ${cmd2} =     Set Variable      tar -xzf ${FILE_NAME}
    ${cmd3} =     Set variable      Log ${CURDIR}
    SSHKeywords.Open_Connection_To_Tools_System    ${TOOLS_SYSTEM_IP}
    SSHLibrary.Execute Command        ${cmd1}
    SSHLibrary.Execute Command        ${cmd3}
    SSHLibrary.File Should Exist        ./${FILE_NAME}
    SSHLibrary.Execute Command        ${cmd2}

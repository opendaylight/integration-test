...
Verify_Odl_Sent_Route_Request_Oxygen
    ...
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_IP}    AND
    BuiltIn.Log    ${output}    AND
    ${output}=    Normalize_String    ${output}    AND
...
Normalize_String
    ...
    Log    ${ret}
    ${ret2}=    BuiltIn.Run_Keyword_And_Ignore_Error    Evaluate    ''.join(${string})
    Log    ${ret2}
    ${ret2}=    BuiltIn.Run_Keyword_And_Ignore_Error    String.Remove_String    ${ret2}    ${SPACE}    \r
    Log    ${ret2}
    [Return]    ${ret}

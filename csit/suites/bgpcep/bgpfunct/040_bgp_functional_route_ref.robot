...
Exa_To_Send_Route_Request
    ...
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_ExaBgp_Received_Updates    ${nr_configured_routes}
...
Odl_To_Send_Route_Request
    ...
    CompareStream.Run_Keyword_If_Less_Than_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    5x    2s    Verify_Odl_Sent_Route_Request    1
...
Verify_ExaBgp_Received_Updates_Oxygen
    [Arguments]    ${expcount}
    [Documentation]    Gets numebr of received update requests and compares with given expected count using odl-bgpcep-bgp-cli
    ${status}    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_IP}
    BuiltIn.Log    ${output}
    ${output}=    Normalize_String    ${output}
    BuiltIn.Should_Contain    ${output}    UPDATE|${expcount}

Normalize_String
    [Arguments]    ${string}
    [Documentation]    Removes irrelevant spaces from the input string variable
    ${ret}=    String.Remove_String    ${string}    ${SPACE}
    Log    ${ret}
    ${status}    ${ret2}=    BuiltIn.Run_Keyword_And_Ignore_Error    Evaluate    ''.join(${string})
    Log    ${ret2}
    ${status}    ${ret2}=    BuiltIn.Run_Keyword_And_Ignore_Error    String.Remove_String    ${ret2}    ${SPACE}    \r    \n
    Log    ${ret2}
    ${status}    ${ret3}=    BuiltIn.Run_Keyword_And_Ignore_Error    Convert To String    ${string}
    Log    ${ret3}
    ${status}    ${ret3}=    BuiltIn.Run_Keyword_And_Ignore_Error    Evaluate    ''.join(${ret3})
    Log    ${ret3}
    ${status}    ${ret3}=    BuiltIn.Run_Keyword_And_Ignore_Error    String.Remove_String    ${ret3}    ${SPACE}    \r    \n
    Log    ${ret3}
    [Return]    ${ret}

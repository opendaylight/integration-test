...
Exa_To_Send_Route_Request
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    Verify_Odl_Received_Updates    ${nr_configured_routes}
...
Odl_To_Send_Route_Request
    ...
    CompareStream.Run_Keyword_If_At_Least_Oxygen    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    Verify_Odl_Received_Route_Request    1
...
Verify_Odl_Received_Updates
    ...
    BuiltIn.Should_Contain    ${output2}    ${expstate2}
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    RIB_INSTANCE_NAME=${RIB_INSTANCE}
    ${ret}=    TemplatedRequests.Get_As_Xml_Templated    folder=${BGP_RR_VAR_FOLDER}/operational    mapping=${mapping}    session=${CONFIG_SESSION}
    Log    ${ret}
...
Verify_Odl_Received_Route_Request
    ...
    ${output2}=    String.Remove String    ${output}    ${SPACE}    \r    \n
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    COUNT=${expcount}
    ${expstate}    TemplatedRequests.Resolve_Text_From_Template_File    folder=${BGP_RR_VAR_FOLDER}/state/    file_name=data_rr.txt    mapping=${mapping}
    ${expstate2}=    String.Remove String    ${expstate}    ${SPACE}    \r    \n
    BuiltIn.Should_Contain    ${output2}    ${expstate2}
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    RIB_INSTANCE_NAME=${RIB_INSTANCE}
    ${ret}=    TemplatedRequests.Get_As_Xml_Templated    folder=${BGP_RR_VAR_FOLDER}/operational    mapping=${mapping}    session=${CONFIG_SESSION}

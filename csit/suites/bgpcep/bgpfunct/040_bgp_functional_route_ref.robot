...
Verify_Odl_Received_Updates
    ...
    ${ret}=    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_RR_VAR_FOLDER}/operational_updates    mapping=${mapping}    session=${CONFIG_SESSION}    verify=False
...
Verify_Odl_Received_Updates_Cli
    ...
    ${status}    Run Keyword And Ignore Error    BuiltIn.Should_Contain    ${output}    ${expstate}
...
Verify_Odl_Received_Route_Refresh
    ...
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    RIB_INSTANCE_NAME=${RIB_INSTANCE}    COUNT=${expcount}
    ${ret}=    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_RR_VAR_FOLDER}/operational_route_refresh    mapping=${mapping}    session=${CONFIG_SESSION}    verify=False
...
Verify_Odl_Received_Route_Refresh_Cli
    ...
    ${output}=    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_SYSTEM_IP}
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    COUNT=${expcount}
    ${expstate}    TemplatedRequests.Resolve_Text_From_Template_File    folder=${BGP_RR_VAR_FOLDER}/operational_cli/    file_name=data_route_refresh.txt    mapping=${mapping}
    @{lines}=    String.Split To Lines    ${output}
    @{lines}=    Evaluate    [x.rstrip() for x in @{lines}]
    ${output}=    Catenate    SEPARATOR=\n    @{lines}
    BuiltIn.Should_Contain    ${output}    ${expstate}

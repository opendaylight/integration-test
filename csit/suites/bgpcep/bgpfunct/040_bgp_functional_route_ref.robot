...
Verify_Odl_Received_Updates
    ...
    BuiltIn.Log    ${output}
    ${output} =    BuiltIn.Evaluate    string.Template('''${output}'''.rstrip()).safe_substitute(${mapping})    modules=string
...
Verify_Odl_Received_Route_Request
    ...
    BuiltIn.Log    ${output}
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}
    ${expstate}    TemplatedRequests.Resolve_Text_From_Template_File    folder=${BGP_RR_VAR_FOLDER}/state/    file_name=data.txt    mapping=${mapping}
    BuiltIn.Should_Contain    ${output}    ${expstate}

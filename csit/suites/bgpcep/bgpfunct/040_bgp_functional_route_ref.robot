...
Verify_Odl_Received_Updates
    ...
    ${status}    ${ret}=    Run Keyword And Ignore Error    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    TemplatedRequests.Get_As_Json_Templated    folder=${BGP_RR_VAR_FOLDER}/operational_updates    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True
    Log    ${ret}

...
Verify_Odl_Received_Route_Request
    ...
    &{mapping}    BuiltIn.Create_Dictionary    IP=${TOOLS_SYSTEM_IP}    RIB_INSTANCE_NAME=${RIB_INSTANCE}    COUNT=${expcount}
    ${ret}=    BuiltIn.Wait_Until_Keyword_Succeeds    3x    5s    TemplatedRequests.Get_As_Xml_Templated    folder=${BGP_RR_VAR_FOLDER}/operational_rr    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True

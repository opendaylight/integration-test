...
Reconfigure_ODL_To_Accept_Connections2
    [Documentation]    Configure BGP peer modules with initiate-connection set to false.
    &{mapping}    Create Dictionary    IP=127.0.0.2    HOLDTIME=${HOLDTIME}
    ...    PEER_PORT=${BGP_TOOL_PORT}    INITIATE=false    BGP_RIB=${RIB_INSTANCE}    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${RIB_INSTANCE}
    ...    RIB_INSTANCE_NAME=${RIB_INSTANCE}
    TemplatedRequests.Put_As_Xml_Templated    ${BGP_VAR_FOLDER}/bgp_peer    mapping=${mapping}    session=${CONFIG_SESSION}
...
Verify_Rib_Status
    ...
    BuiltIn.Log    ${output}
    ${output}    TemplatedRequests.Get_As_Json_From_Uri    restconf/operational/bgp-rib:bgp-rib/rib/example-bgp-rib    session=${CONFIG_SESSION}
    BuiltIn.Log    ${output}
...
Start_Suite
    ...
    Upload_Config_Files
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    ip addr show
    ...
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    docker network ls
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    cat /etc/network/interfaces
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    cat /etc/sysconfig/network-scripts/ifcfg-eth0
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo ip -6 route add local ::2/128 dev lo
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo sh -c 'echo "NETWORKING_IPV6=yes" >> /etc/sysconfig/network'
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo sh -c 'echo "IPV6INIT=yes" >> /etc/sysconfig/network-scripts/ifcfg-eth0'
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo sh -c 'echo "IPV6ADDR=2607:f0d0:1002:0011:0000:0000:0000:0002" >> /etc/sysconfig/network-scripts/ifcfg-eth0'
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo sh -c 'echo "IPV6_DEFAULTGW=2607:f0d0:1002:0011:0000:0000:0000:0001" >> /etc/sysconfig/network-scripts/ifcfg-eth0'
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    sudo /etc/init.d/network restart
    BuiltIn.Sleep    30s
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${stdout}    Run Keyword And Ignore Error    SSHLibrary.Read_Until_Prompt
    ${status}    Run Keyword And Ignore Error    BuiltIn.Log    ${stdout}
    ${status}    ${output}    Run Keyword And Ignore Error    SSHLibrary.Write    cat /etc/sysconfig/network-scripts/ifcfg-eth0
    ${stdout}    SSHLibrary.Read_Until_Prompt
    BuiltIn.Log    ${stdout}
    ${status}    ${stdout}    Run Keyword And Ignore Error    SSHLibrary.Read_Until_Prompt

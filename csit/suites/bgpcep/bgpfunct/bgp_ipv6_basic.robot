...
${IPV6_IP_2}    2607:f0d0:1002:0011:0000:0000:0000:0003
...
Reconfigure_Basic_Odl
    &{mapping}    Create Dictionary    IP=${IPV6_IP}    HOLDTIME=${HOLDTIME}
    ...    PEER_PORT=${BGP_TOOL_PORT}    INITIATE=false    BGP_RIB=bgp-example    PASSIVE_MODE=true    BGP_RIB_OPENCONFIG=${RIB_INSTANCE}
    ...    RIB_INSTANCE_NAME=${RIB_INSTANCE}
    TemplatedRequests.Post_As_Xml_Templated    ${BGP_VAR_FOLDER}/ipv6/basic_config    mapping=${mapping}    session=${CONFIG_SESSION}
...
Verify_Rib_Status
    BuiltIn.Sleep    10s
    ${output}    KarafKeywords.Safe_Issue_Command_On_Karaf_Console    bgp:operational-state -rib bgp-example
    BuiltIn.Log    ${output}

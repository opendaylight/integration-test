...
Reconfigure_ODL_To_Accept_Connections_4
    ...
    ${resp} =    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/openconfig-network-instance:network-instances
    Log    ${resp.text}
    ${resp} =    RequestsLibrary.Get Request    session    ${CONFIG_API}/openconfig-network-instance:network-instances
    Log    ${resp.text}

...
@{L3VPN_RT_CHECK}    true    false    false
...
Play_To_Odl_l3vpn_rt_arg
    ...
    Play_To_Odl_Non_Removal_BgpRpcClient2    rt_constrain_type_0    ${RT_CONSTRAIN_DIR}

Send_Route_To_Odl
    [Documentation]    Send l3vpn route to app peer, and than checks that app peer advertizes this route.
    Send_Route_To_Odl   l3vpn_rt_arg    ${RT_CONSTRAIN_DIR}
...
#    ${folder}=    BuiltIn.Set_Variable    ${dir}/${totest}/app
#    ${filename_uri}=    BuiltIn.Set_Variable    location.uri
#    ${filename_data}=    BuiltIn.Set_Variable    post_data.xml
#    ${uri} =    Resolve_Text_From_File    folder=${folder}    file_name=${filename_uri}    mapping=${APP_PEER}
#    ${data} =    Resolve_Text_From_File   folder=${folder}    file_name=${filename_data}    mapping=${APP_PEER}
#    ${headers} =    TemplatedRequests.Join_Two_Headers    first=${HEADERS_XML}    second=${ACCEPT_EMPTY}
#    BuiltIn.Log_Many    ${folder}    ${filename_uri}    ${filename_data}    ${uri}    ${data}    ${headers}
#    RequestsLibrary.Post_Request    alias=${CONFIG_SESSION}    uri=${uri}    data=${data}    headers=${headers}

Resolve_Text_From_File
    [Arguments]    ${folder}    ${file_name}    ${mapping}={}
    ${file_path}=    BuiltIn.Set Variable    ${folder}${/}${file_name}

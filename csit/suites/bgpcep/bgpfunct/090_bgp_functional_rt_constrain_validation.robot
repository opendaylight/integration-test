...
Play_To_Odl_remove_rt
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    Verify_Empty_Reported_Data    ${uri}    ${L3VPN_RSPEMPTY}
    ${update} =    BgpRpcClient4.play_get
    BuiltIn.Should_Be_Equal    ${update}    ${Empty}
...
Verify_Empty_Reported_Data
    ...
    CompareStream.Run_Keyword_If_At_Most_Fluorine    TemplatedRequests.Get_As_Json_Templated    ${L3VPN_IPV4_DIR}${/}${L3VPN_RSPEMPTY}    session=${CONFIG_SESSION}    verify=True
...
Verify_Empty_Data_Neon
    ...
    TemplatedRequests.Get_As_Json_Templated    ${L3VPN_IPV4_DIR}${/}empty_route    session=${CONFIG_SESSION}    verify=True

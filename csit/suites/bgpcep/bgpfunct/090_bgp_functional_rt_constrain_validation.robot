...
Start_Bgp_Peers
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    BuiltIn.Sleep    5s
    ${command}=    BuiltIn.Set_Variable    python play.py --amount 0 --myip=127.0.0.2 --asnumber=65000 --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --port=${port} --usepeerip --debug --allf --wfr 1 &> ${filename} &
    BuiltIn.Log    ${command}

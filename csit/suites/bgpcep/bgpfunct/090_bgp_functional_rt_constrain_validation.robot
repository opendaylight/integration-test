...
Library           SSHLibrary
Library           String
Library           ../../../libraries/BgpRpcClient.py    127.0.0.2    WITH NAME    BgpRpcClient2
Library           ../../../libraries/BgpRpcClient.py    127.0.0.3    WITH NAME    BgpRpcClient3
Library           ../../../libraries/BgpRpcClient.py    127.0.0.4    WITH NAME    BgpRpcClient4
...
&{RT_CONSTRAIN_APP_PEER}    IP=${ODL_1_IP}    BGP_RIB=${RIB_NAME}
...
Play_To_Odl_remove_rt
    Play_To_Odl_Routes_Removal_Template_BgpRpcClient3    ${BGPRPCCLIENT_LIB_PATH}    ${ODL_3_IP}    BgpRpcClient3    rt_constrain_type_0    ${RT_CONSTRAIN_DIR}
...
Delete_Bgp_Peers_Configuration
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    : FOR    ${i}    IN    @{ODL_refs/changes/31/75531/22_INDICES}
    \    &{RT_CONSTRAIN_ODL_CONFIG}=    BuiltIn.Create Dictionary    IP=${ODL_${i}_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    INITIATE=false
    \    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true
...
Start_Suite
    ...
    ${conn_id} =    SSHLibrary.Open Connection    ${ODL_1_IP}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=6s
    Builtin.Set_Suite_Variable    ${conn_id}
    SSHKeywords.Flexible_Controller_Login
    RequestsLibrary.Create_Session    ${CONFIG_SESSION}    http://${ODL_1_IP}:${RESTCONFPORT}    auth=${AUTH}
...
Start_Bgp_Peer
    ...
    ${command}=    BuiltIn.Set_Variable    python play.py --amount 0 --myip=${ip} --myport=${BGP_TOOL_PORT} --peerip=${ODL_1_IP} --peerport=${ODL_BGP_PORT} --debug --allf --wfr 1 &> ${filename} &
    BuiltIn.Log    ${command}

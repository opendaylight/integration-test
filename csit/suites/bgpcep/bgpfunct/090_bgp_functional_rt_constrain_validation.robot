...
Check_Presence_Of_l3vpn_Route_In_Node_2_Effective_Rib_In_Table
    ...
    &{effective_rib_in}=    BuiltIn.Create_Dictionary    PATH=peer/bgp:%2F%2F${ODL_3_IP}/effective-rib-in    BGP_RIB=${RIB_NAME}
    &{adj_rib_in}=    BuiltIn.Create_Dictionary     PATH=peer/bgp:%2F%2F${ODL_3_IP}/adj-rib-in    BGP_RIB=${RIB_NAME}
    &{adj_rib_out}=    BuiltIn.Create_Dictionary    PATH=peer/bgp:%2F%2F${ODL_3_IP}/adj-rib-out    BGP_RIB=${RIB_NAME}
    ${mappings}=    BuiltIn.Create List    ${effective_rib_in}    ${adj_rib_in}    ${adj_rib_out}
    BuiltIn.Set_Suite_variable    ${mappings}
    : FOR    ${mapping}    IN    @{mappings}
    \    BuiltIn.Run_Keyword_And_Continue_On_Failure    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated
        ...    ${RT_CONSTRAIN_DIR}/ext_l3vpn_rt_arg/rib    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True
...
Play_To_Odl_remove_rt
    ...
    : FOR    ${mapping}    IN    @{mappings}
    \    BuiltIn.Run_Keyword_And_Continue_On_Failure    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated
    \    ...    ${L3VPN_IPV4_DIR}    mapping=${mapping}    session=${CONFIG_SESSION}    verify=True
    : FOR    ${mapping}    IN    @{mappings}
    \    ${uri}=    Resolve_Text_From_File    ${L3VPN_RIB_URI}    location.uri    mapping=${mapping}
    \    BuiltIn.Run_Keyword_And_Continue_On_Failure    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    Verify_Reported_Data    ${uri}    ${L3VPN_RSPEMPTY}
    ${update}=    BgpRpcClient4.play_get
    BuiltIn.Should_Be_Equal    ${update}    ${Empty}

...
${ODL_3_IP}       127.0.0.3
${ODL_4_IP}       127.0.0.4
@{BGP_PEER_TYPES}    external    internal    external
@{BGP_PEER_AS_NUMBERS}    65000    64496    65000
...
Reconfigure_ODL_To_Accept_Connection
    ...
    \    TemplatedRequests.Put_As_Xml_Templated    ${EBGP_DIR}/ebgp_peer    mapping=${ODL_CONFIG}    session=${CONFIG_SESSION}
...
Play_To_Odl_l3vpn_rt_arg
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${RT_CONSTRAIN_DIR}/l3vpn_rt_arg/rib    mapping=${EFFECTIVE_RIB_IN}    session=${CONFIG_SESSION}
    ...    verify=True
...
Delete_Bgp_Peers_Configuration
    ...
    \    &{ODL_CONFIG}=    BuiltIn.Create Dictionary    IP=${ODL_${i}_IP}    TYPE=${type}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    INITIATE=false
    \    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true
    \    TemplatedRequests.Delete_Templated    ${EBGP_DIR}/ebgp_peer    mapping=${ODL_CONFIG}    session=${CONFIG_SESSION}

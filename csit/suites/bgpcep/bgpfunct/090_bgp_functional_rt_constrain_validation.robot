...
${RPC_CLIENT_SCRIPT}    ${CURDIR}/../../../../tools/fastbgp/bgp_rpc_client.py
...
Test_Bgp_Rpc_Client_Script
    ${output}=    SSHLibrary.Write    ./bgp_rpc_client.py
...
Start_Suite
    ...
    SSHLibrary.Put_File    ${RPC_CLIENT_SCRIPT}    .
...
Start_Bgp_Peer
    ...
    ${command}=    BuiltIn.Set_Variable    python play.py --amount 0 --myip=${ip} --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --port=${port} --debug --allf --wfr 1 &> ${filename} &
    BuiltIn.Log    ${command}

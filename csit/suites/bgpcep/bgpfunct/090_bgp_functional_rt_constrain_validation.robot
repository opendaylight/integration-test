...
Reconfigure_ODL_To_Accept_Connection
    ...
    &{RT_CONSTRAIN_ODL_CONFIG}=    BuiltIn.Create Dictionary    IP=${ODL_2_IP}    HOLDTIME=${HOLDTIME}    PEER_PORT=${BGP_TOOL_PORT}    INITIATE=false
    ...    BGP_RIB=${RIB_NAME}    PASSIVE_MODE=true
    TemplatedRequests.Put_As_Xml_Templated    ${RT_CONSTRAIN_DIR}/bgp_peer_2    mapping=${RT_CONSTRAIN_ODL_CONFIG}    session=${CONFIG_SESSION}
    : FOR    ${i}    IN    @{ODL_IP_INDICES_ALL}
...
Start_Bgp_Peers
    ...
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    BuiltIn.Sleep    5s
    ${command}=    BuiltIn.Set_Variable    python play.py --amount 0 --myip=127.0.0.2 --asnumber=65000 --myport=${BGP_TOOL_PORT} --peerip=${ODL_SYSTEM_IP} --peerport=${ODL_BGP_PORT} --port=8002 --usepeerip --debug --allf --wfr 1 &> play.py.090.2 &
    BuiltIn.Log    ${command}
    ${output}=    SSHLibrary.Write    ${command}
    : FOR    ${i}    IN    @{ODL_IP_INDICES_3_4}
...
Play_To_Odl_l3vpn_rt_arg
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    TemplatedRequests.Get_As_Json_Templated    ${RT_CONSTRAIN_DIR}/l3vpn_rt_arg/rib    mapping=${LOC_RIB}    session=${CONFIG_SESSION}
    ...    verify=True
...
Play_To_Odl_rt_constrain_type_1
    [Documentation]    Sends RT route from node 3 to odl and then checks that odl advertizes l3vpn route from previous TC.
    Play_To_Odl_Non_Removal_BgpRpcClient4    rt_constrain_type_1    ${RT_CONSTRAIN_DIR}
    ${announce}=    OperatingSystem.Get_File    ${RT_CONSTRAIN_DIR}/l3vpn_rt_arg/announce_l3vpn_rt_arg.hex
    ${announce_hex}=    String.Remove_String    ${announce}    \n
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    Get_Update_Message_And_Compare_With_Hex_BgpRpcClient4    ${announce_hex}    true
...
Play_To_Odl_remove_rt
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    Get_Update_Message_And_Compare_With_Hex_BgpRpcClient3    ${withdraw_hex}    true

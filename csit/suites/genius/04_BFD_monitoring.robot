...
BFD_TC03 Verify that in controller tunnel status is down when ITM tunnel interface is brought down.
    [Documentation]    Verify that in controller tunnel status is down when ITM tunnel interface is brought down.
...
BFD_TC05 Verify BFD tunnel monitoring interval can be changed.
    ...
    ${oper_int}    RequestsLibrary.Post Request    session    ${CONFIG_API}/itm-config:tunnel-monitor-interval/    data=${INTERVAL_5000}
    SLEEP    5

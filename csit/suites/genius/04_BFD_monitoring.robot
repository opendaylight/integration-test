...
BFD_TC05 Verify BFD tunnel monitoring interval can be changed.
    ...
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At Uri    ${OPERATIONAL_API}/itm-config:tunnel-monitor-interval/    ${Bfd_updated_value}
    Wait Until Keyword Succeeds    30s    5s    Check For Elements At Uri    ${CONFIG_API}/itm-config:tunnel-monitor-interval/    ${Bfd_updated_value}
    ...
    Wait Until Keyword Succeeds    20s    5s    Verify OVS Output    ${Bridge-1}
    SSHLibrary.Switch Connection    ${conn_id_2}
    Wait Until Keyword Succeeds    20s    5s    Verify OVS Output    ${Bridge-2}

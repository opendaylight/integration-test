...
BFD_TC05 Verify BFD tunnel monitoring interval can be changed.
    ...
    ${tun_name}    Wait Until Keyword Succeeds    20s    4s    Ovs Tunnel Get    list interface ${Bridge-1}
    ...
    ${tun_name}    Wait Until Keyword Succeeds    20s    4s    Ovs Tunnel Get    list interface ${Bridge-2}
    Wait Until Keyword Succeeds    20s    5s    OVSDB.Verify Ovs-vsctl Output    list interface ${tun_name}    5000    ovs_system=${TOOLS_SYSTEM_IP}

...
Test Teardown     Dump DS At TC End
...
BFD_TC05 Verify BFD tunnel monitoring interval can be changed.
    ...
    Should Contain    ${respjson}    ${TUNNEL_MONI_PROTO}
    Log    "Verifying BFD Interval on Switch BR1"
    SSHLibrary.Switch Connection    ${conn_id_1}
    Execute Command    sud ovs-vsctl del-port BR1 tap8ed70586-6c
    SLEEP    1
    ...
    Log    "Verifying BFD Interval on Switch BR2"
...
Dump DS At TC End
    ...
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/odl-interface-meta:if-indexes-interface-map/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ...
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${CONFIG_API}/id-manager:id-pools/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${CONFIG_API}/network-topology:network-topology/topology/ovsdb:1/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/network-topology:network-topology/topology/ovsdb:1/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${CONFIG_API}/vpnservice-dhcp:designated-switches-for-external-tunnels/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${CONFIG_API}/neutron:neutron/security-groups/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${int_resp}    RequestsLibrary.Get Request    session    ${CONFIG_API}/neutron:neutron/security-rules/
    ${respjson}    RequestsLibrary.To Json    ${int_resp.content}    pretty_print=True
    Log    ${respjson}
    ${oper_int}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/itm-config:tunnel-monitor-interval/
    ${respjson}    RequestsLibrary.To Json    ${oper_int.content}    pretty_print=True

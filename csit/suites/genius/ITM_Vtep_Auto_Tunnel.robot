...
Verify TEP in controller and transport zone in OVSDB table of compute nodes
    ...
    BuiltIn.Wait Until Keyword Succeeds    2x    15 sec    Genius.Verify Tunnel Status as UP    ${DEFAULT_TRANSPORT_ZONE}
...
Verify TEPs with transport zone configured from OVS will be added to corresponding transport zone
    ...
    ${get_nohosted_data} =    BuiltIn.Wait Until Keyword Succeeds    2x    15 sec    Utils.Get Data From URI    session    ${TEP_NOT_HOSTED_ZONE_URL}
    ...
    ${get_hosted_data} =    BuiltIn.Wait Until Keyword Succeeds    2x    15 sec    Utils.Get Data From URI    session    ${TRANSPORT_ZONE_ENDPOINT_URL}/${TRANSPORT_ZONE}
    BuiltIn.Should Contain    ${get_hosted_data}    ${TRANSPORT_ZONE}
    BuiltIn.Should Contain    ${get_hosted_data}    ${DPN1}
    BuiltIn.Wait Until Keyword Succeeds    2x    15 sec    Genius.Verify Tunnel Status as UP    ${TRANSPORT_ZONE}

...
Verify TEP in controller and transport zone in OVSDB table of compute nodes
    ...
    OVSDB.Check Ovs Version Is Higher Than    @{COMPUTE-NODE-LIST}    ${OVS_VERSION}
    ...
    ${ITM_DATA} =    Utils.Get Data From URI    ${POST_URL}
    ...
    ${status}    BuiltIn.Run Keyword And Return Status    Utils.Get Data From URI    ${TUNNEL_NAME}${DPN2}/${DPN1}/${TUNNEL_TYPE_KEY_VAL}
    BuiltIn.Should Be True    '${status}' == 'True'
    ${status}    BuiltIn.Run Keyword And Return Status    Utils.Get Data From URI    ${TUNNEL_NAME}${DPN1}/${DPN2}/${TUNNEL_TYPE_KEY_VAL}
    BuiltIn.Should Be True    '${status}' == 'True'
...
Verify TEPs with transport zone configured from OVS will be added to corresponding transport zone
    ...
    ${status} =    BuiltIn.Run Keyword And Return Status    Utils.Get Data From URI    ${GET_TRANSPORT_ZONE}
    BuiltIn.Should Be True    '${status}' == 'True'
    Change Transport Zone In Compute    @{COMPUTE-NODE-LIST}[0]    ${TRANSPORT_ZONE}
    ${get_nohosted_data} =    BuiltIn.Wait Until Keyword Succeeds    1 min    5 sec    Utils.Get Data From URI    ${TEPNOTHOSTED_ZONE}
    BuiltIn.Should Contain Any    ${get_nohosted_data}    ${TRANSPORT_ZONE}    ${DPN1}
    Utils.Post Elements To URI From File    ${POST_URL}    ${FILE_DIR}
    ${get_hosted_data} =    Utils.Get Data From URI    ${GET_TRANSPORT_ZONE}
    ...
    ${config_data} =    Utils.Get Data From URI    ${GET_TRANSPORT_ZONE}
...
Verify other-config-key and transport zone value in controller operational datastore
    ...
    ${controller-data} =    Utils.Get Data From URI    ${GET_NETWORK_TOPOLOGY}
...
Delete transport zone on OVS and check ovsdb update to controller
    ...
    ${default_zone_data} =    Utils.Get Data From URI    ${GET_DEFAULT_ZONE}

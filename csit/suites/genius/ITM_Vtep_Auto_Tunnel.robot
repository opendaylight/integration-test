...
Verify TEP in controller and transport zone in OVSDB table of compute nodes
    ...
    Comment    ${dpn} =    OVSDB.Get DPID    ${TOOLS_SYSTEM_1_IP}
    Comment    ${dpn} =    BuiltIn.Convert To String    ${dpn}
    Comment    BuiltIn.Set Suite Variable    ${DPN1}    ${dpn}
    Comment    ${dpn} =    OVSDB.Get DPID    ${TOOLS_SYSTEM_2_IP}
    Comment    ${dpn} =    BuiltIn.Convert To String    ${dpn}
    Comment    BuiltIn.Set Suite Variable    ${DPN2}    ${dpn}
    Comment    ${ip} =    OvsManager.Get OVS Local Ip    ${TOOLS_SYSTEM_1_IP}
    Comment    BuiltIn.Set Suite Variable    ${LOCAL_IP_COMPUTE1}    ${ip}
    Comment    ${ip} =    OvsManager.Get OVS Local Ip    ${TOOLS_SYSTEM_2_IP}
    Comment    BuiltIn.Set Suite Variable    ${LOCAL_IP_COMPUTE2}    ${ip}
...
Verify TEPs with transport zone configured from OVS will be added to corresponding transport zone
    ...
    Comment    : FOR    ${node_number}    IN RANGE    2    ${NUM_TOOLS_SYSTEM}+1
    Comment    \    Change Transport Zone In Compute    ${TOOLS_SYSTEM_${node_number}_IP}    ${TRANSPORT_ZONE}
    ...
    Change Transport Zone In Compute    ${TOOLS_SYSTEM_3_IP}    ${TRANSPORT_ZONE}
    BuiltIn.Wait Until Keyword Succeeds    3x    10 sec    Genius.Verify Tunnel Status as Up

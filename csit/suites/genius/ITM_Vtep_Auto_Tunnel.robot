...
Verify TEPs with transport zone configured from OVS will be added to corresponding transport zone
    ...
    ${status} =    BuiltIn.Run Keyword And Return Status    Utils.Get Data From URI    session    ${TRANSPORT_ZONE_ENDPOINT}/${DEFAULT_TRANSPORT_ZONE}
    BuiltIn.Should Be True    '${status}' == 'True'
    Change Transport Zone In Compute    @{COMPUTE-NODE-LIST}[0]    ${TRANSPORT_ZONE}
    ${get_nohosted_data} =    BuiltIn.Wait Until Keyword Succeeds    1 min    5 sec    Utils.Get Data From URI    session    ${TEP_NOT_HOSTED_ZONE}
    ...
    ${get_hosted_data} =    Utils.Get Data From URI    session    ${TRANSPORT_ZONE_ENDPOINT}/${TRANSPORT_ZONE}
    ...
    ${ip1} =    OvsManager.Get OVS Local Ip    @{COMPUTE-NODE-LIST}[0]
    ${ip2} =    OvsManager.Get OVS Local Ip    @{COMPUTE-NODE-LIST}[1]
    ${config_data} =    Utils.Get Data From URI    session    ${TRANSPORT_ZONE_ENDPOINT}/${TRANSPORT_ZONE}
...
Delete transport zone on OVS and check ovsdb update to controller
    ...
    ${default_zone_data} =    Utils.Get Data From URI    session    ${TRANSPORT_ZONE_ENDPOINT}/${DEFAULT_TRANSPORT_ZONE}

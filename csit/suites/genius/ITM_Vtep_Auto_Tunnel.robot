...
#    ${output} =    Issue Command On Karaf Console    ${TEP_SHOW}
#    Should Not Contain   ${output}   ${DEFAULT_TRANSPORT_ZONE}
#    Post Tunnel Data    /restconf/config/itm:transport-zones/
#
...
#    : FOR    ${node}    IN    @{COMPUTE-NODE-LIST}
#    \    Restart OVSDB    ${node}
#    ${tunnels_post_reebot} =   Wait Until Keyword Succeeds   5 min   5 sec    Get Csc Datastore    ${TUNNEL_CONF_LIST}
#    Should Be Equal    ${tunnels_post_reebot}    ${tunnels_post_reebot}
#    ${output} =    OpenStackOperations.Execute Command on VM Instance    @{NETWORKS}[0]    ${vm1_ip}    ping -c 3 ${vm2_ip}
#    Should Contain    ${output}    ${PING_REGEXP}
#    Reconnect Css With Zone Change    @{COMPUTE-NODE-LIST}[0]    ${DEFAULT_TRANSPORT_ZONE}    ${IP1}
#    Reconnect Css With Zone Change    @{COMPUTE-NODE-LIST}[1]    ${DEFAULT_TRANSPORT_ZONE}    ${IP2}
    Clean Config

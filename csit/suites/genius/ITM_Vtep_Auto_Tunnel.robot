...
Verify TEP IP and transport zone in OVSDB table of compute nodes
    ...
    ${ITM_DATA} =    Get Controller Data    ${POST_URL}
    ...
    Get Controller Data    ${TUNNEL_NAME}${DPN2}/${DPN1}/${TUNNEL_TYPE_KEY_VAL}
    Get Controller Data    ${TUNNEL_NAME}${DPN1}/${DPN2}/${TUNNEL_TYPE_KEY_VAL}
...
Verify TEPs with transport zone configured from OVS will be added to corresponding transport zone
    ...
    Post Tunnel Data    ${POST_URL}    ${FILE_DIR}
...
Get Controller Data
    [Arguments]    ${url}
    [Documentation]    Get REST call data and return the content
    ${resp} =    RequestsLibrary.Get Request    session    ${url}
    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    200
    [Return]    ${resp.content}

Post Tunnel Data
    [Arguments]    ${uri}    ${file_dir}

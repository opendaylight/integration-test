...
Delete and Verify VTEP -No Vlan
    ...
    Comment    : FOR    ${node}    IN    @{TOOLS_SYSTEM_ALL_IPS}
    Comment    \    ${cmd} =    BuiltIn.Set Variable    tep:delete @{DPN_ID_LIST}[${i}] ${port_name} @{VLAN}[0] ${node} ${subnet}/24 null ${itm_created[0]}
    Comment    \    KarafKeywords.Issue Command On Karaf Console    ${cmd}
    Comment    \    KarafKeywords.Issue Command On Karaf Console    tep:commit
    Comment    \    ${i} =    Evaluate    ${i}+1
    Comment    ${output}    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW}
    Comment    BuiltIn.Should Not Contain    ${output}    ${itm_created[0]}

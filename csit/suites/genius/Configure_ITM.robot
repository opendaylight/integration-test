...
Delete and Verify VTEP -No Vlan
    ...
    \    ${i} =    Evaluate    ${i}+1
    Comment    ${cmd}    Set Variable    tep:delete ${Dpn_id_1} @{PORT}[0] @{VLAN}[0] ${TOOLS_SYSTEM_IP} ${subnet}/24 null ${itm_created[0]}
    Comment    ${cmd2}    Set Variable    tep:delete ${Dpn_id_2} @{PORT}[1] @{VLAN}[0] ${TOOLS_SYSTEM_2_IP} ${subnet}/24 null ${itm_created[0]}
    Comment    KarafKeywords.Issue Command On Karaf Console    ${cmd}
    Comment    KarafKeywords.Issue Command On Karaf Console    tep:commit
    Comment    KarafKeywords.Issue Command On Karaf Console    ${cmd2}
    Comment    KarafKeywords.Issue Command On Karaf Console    tep:commit

...
Create VTEP - Vlan and Gateway
    ...
    ${substr} =    BuiltIn.Should Match Regexp    ${TOOLS_SYSTEM_IP}    ${REGEX_IPV4}
...
Create Vteps IPv6
    ...
    ${substr} =    Should Match Regexp    @{tools_ips}[0]    [0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:[0-9a-fA-F]{1,4}:
    ${SUBNET_IPV6} =    Catenate    ${substr}0
    Set Suite Variable    ${SUBNET_IPV6}
    ${body} =    Genius.Set Json    ${vlan}    ${gateway_ip}    ${SUBNET_IPV6}    ${tools_ips}
...
Verify Ietf Interface State
    Comment    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_API}/ietf-interfaces:interfaces-state/
    Comment    ${respjson}    RequestsLibrary.To Json    ${resp.content}    pretty_print=True
    Comment    BuiltIn.Log    ${respjson}
    Comment    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    200
    Comment    : FOR    ${dpn}    IN    @{DPN_ID_LIST}
    Comment    \    BuiltIn.Should Contain    ${resp.content}    ${dpn}
    Comment    ${all_tunnels}    Genius.Get Tunnels List
    Comment    : FOR    ${tun}    IN    @{all_tunnels}
    Comment    \    BuiltIn.Should Contain    ${resp.content}    ${tun}

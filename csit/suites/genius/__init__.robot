...
Start Suite
    ...
    ${ovs_path}=    set Variable    /etc/openvswitch/conf.d
    Login With Public Key    ${TOOLS_SYSTEM_USER}    ${USER_HOME}/.ssh/${SSH_KEY}    any
    Log    ${conn_id_2}
    ${std_out}=    Execute Command    sudo rm -rf \ \ \ ${ovs_path}
    log    ${std_out}
    ${std_out}=    Execute Command    sudo service openvswitch-switch restart
    log    ${std_out}

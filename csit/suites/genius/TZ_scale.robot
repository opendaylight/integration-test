...
Dumb Test1
    ...
    \    SSHLibrary.Execute Command    sudo docker run --name container${count} -e MODE=none -itd --cap-add NET_ADMIN jhershbe/centos7-ovs-multimode

    BuiltIn.Sleep   15s

    : FOR    ${count}    IN RANGE     1     10
    \    SSHLibrary.Execute Command    sudo docker exec container${count} modprobe openvswitch
    ...
    \    Log    ${out}
    \    ${log}=     SSHLibrary.Execute Command    modinfo openvswitch
    \    Log    ${out}

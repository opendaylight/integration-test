...
Delete Interface
    ...
    BuiltIn.Wait Until Keyword Succeeds    40    10    OVSDB.Verify Dump Flows For Specific Table    ${TOOLS_SYSTEM_1_IP}    ${EMPTY}    False
...
Start Suite for HA
    ...
    Comment    ${conn_id_1} =    SSHLibrary.Open Connection    ${TOOLS_SYSTEM_IP}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=30s
    Comment    SSHKeywords.Flexible_SSH_Login    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    Comment    BuiltIn.Set Suite Variable    ${conn_id_1}
    Comment    ${conn_id_2} =    SSHLibrary.Open Connection    ${TOOLS_SYSTEM_2_IP}    prompt=${DEFAULT_LINUX_PROMPT}    timeout=30s
    Comment    SSHKeywords.Flexible_SSH_Login    ${DEFAULT_USER}    ${DEFAULT_PASSWORD}
    Comment    BuiltIn.Set Suite Variable    ${conn_id_2}
    : FOR    ${i}    IN RANGE    1    ${NUM_TOOLS_SYSTEM}
    \    Create Bridge    ${TOOLS_SYSTEM_${i}_IP}    ${Bridge}

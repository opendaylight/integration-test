...
Take Down ODL1
    ...
    Verify OVS Configuration    ${ODL_SYSTEM_2_IP}

Bring Up ODL1
    ClusterManagement.Start Single Member    1
    Verify OVS Configuration    ${ODL_SYSTEM_1_IP}
...
Create Bridge
    ...
    Run Command On Remote System    ${ovs_ip}    sudo ovs-vsctl set bridge ${bridge} protocols=OpenFlow13
    Run Command On Remote System    ${ovs_ip}    sudo ifconfig ${bridge} up
    Run Command On Remote System    ${ovs_ip}    sudo ovs-vsctl add-port ${bridge} tap8ed70586-6c -- set Interface tap8ed70586-6c type=tap
    Run Command On Remote System    ${ovs_ip}    sudo ovs-vsctl set-manager tcp:${ODL_SYSTEM_IP}:6640
    Run Command On Remote System    ${ovs_ip}    sudo ovs-vsctl set-controller ${bridge} tcp:${ODL_SYSTEM_1_IP}:${ODL_OF_PORT_6653} tcp:${ODL_SYSTEM_2_IP}:${ODL_OF_PORT_6653} tcp:${ODL_SYSTEM_3_IP}:${ODL_OF_PORT_6653}
    Run Command On Remote System And Log    ${ovs_ip}    sudo ovs-vsctl show
    Run Command On Remote System And Log    ${ovs_ip}    sudo ovs-vsctl show list Controller
...
Create VTEP and Verify
    ...
    ...    ${type}
    Run Command On Remote System And Log    ${TOOLS_SYSTEM_1_IP}    sudo ovs-vsctl show
    Comment    Wait Until Keyword Succeeds    2 min    2    Verify Tunnel Status    TZA    ${ODL_SYSTEM_1_IP}
...
Verify OVS Configuration
    [Arguments]    ${odl_ip}
    Wait Until Keyword Succeeds    2 min    2    Verify Tunnel Status    TZA    ${odl_ip}
    Verify IETF Interfaces

...
Create VTEP and Verify
    ...
    ${type} =    Set Variable    odl-interface:tunnel-type-vxlan
    ${tunnel-1} =    Wait Until Keyword Succeeds    70    20    Get Tunnel Ha    ${Dpn_id_1}    ${Dpn_id_2}
    ...    ${type}    session
    ${tunnel-2} =    Wait Until Keyword Succeeds    70    20    Get Tunnel Ha    ${Dpn_id_2}    ${Dpn_id_1}
    ...    ${type}    session
    Run Command On Remote System And Log    ${TOOLS_SYSTEM_1_IP}    sudo ovs-vsctl show
...
Get Tunnel Ha
    [Arguments]    ${src}    ${dst}    ${type}    ${session}
    ${resp} =    RequestsLibrary.Get Request    ${session}    ${CONFIG_API}/itm-state:tunnel-list/internal-tunnel/${src}/${dst}/${type}/
    Comment    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
    Comment    BuiltIn.Should Contain    ${resp.content}    ${src}
    Comment    BuiltIn.Should Contain    ${resp.content}    ${dst}

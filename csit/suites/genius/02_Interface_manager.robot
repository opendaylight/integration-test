...
Create l2vlan trunk interface
    ...
    ${post_resp}    RequestsLibrary.Post    session    ${REST_CON}/ietf-interfaces:interfaces/    data=${body}
    ...
    ${get_resp}    RequestsLibrary.Get    session    ${REST_CON}/ietf-interfaces:interfaces/    headers=${ACCEPT_XML}
...
Bind service on Interface
    ...
    log    ${body}
    ${post_resp}    RequestsLibrary.Post    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/    data=${body}
    log    ${post_resp.content}
    log    ${post_resp.status_code}
    Should Be Equal As Strings    ${post_resp.status_code}    204
    Log    >>>>> Verifying Binded interface <<<<<
    ${get_resp}    RequestsLibrary.Get    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/    headers=${ACCEPT_XML}
    log    ${get_resp.content}
    log    ${get_resp.status_code}
    should be equal as strings    ${get_resp.status_code}    200
...
unbind service on interface
    ...
    ${del_response_1}    RequestsLibrary.Delete    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/bound-services/${service-priority-1}/
    log    ${del_response_1.content}
    log    ${del_response_1.status_code}
    should be equal as strings    ${del_response_1.status_code}    200
    ${get_resp1}    RequestsLibrary.Get    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/bound-services/${service-priority-1}/    headers=${ACCEPT_XML}
    log    ${get_resp1}
    ...
    log    ${no-table-21}
    ${del_response_2}    RequestsLibrary.Delete    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/bound-services/${service-priority-2}/
    log    ${del_response_2.content}
    log    ${del_response_2.status_code}
    should be equal as strings    ${del_response_2.status_code}    200
    ${get_resp2}    RequestsLibrary.Get    session    ${REST_CON}/interface-service-bindings:service-bindings/services-info/${interface_name}/bound-services/${service-priority-2}/    headers=${ACCEPT_XML}
    log    ${get_resp2}
...
Delete l2vlan trunk interface
    ${del_resp}    RequestsLibrary.Delete    session    ${REST_CON}/ietf-interfaces:interfaces/
    Should Be Equal As Strings    ${del_resp.status_code}    200
    ${get_del_resp}    RequestsLibrary.Get    session    ${REST_CON}/ietf-interfaces:interfaces/    headers=${ACCEPT_XML}
    Log    ${get_del_resp.content}
    log    ${get_del_resp.status_code}
    Should Be Equal As Strings    ${get_del_resp.status_code}    404
    ${get_del_operresp}    RequestsLibrary.Get    session    ${REST_OPER}/ietf-interfaces:interfaces/    headers=${ACCEPT_XML}
...
get operational interface
    ...
    ${get_oper_resp}    RequestsLibrary.Get    session    ${REST_OPER}/ietf-interfaces:interfaces-state/interface/${interface_name}/    headers=${ACCEPT_XML}

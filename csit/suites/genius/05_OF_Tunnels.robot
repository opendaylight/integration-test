...
Delete and verify OF tunnels on 1 DPN and non-OF tunnel on another DPN
    ...
    Remove All Elements At URI And Verify    ${CONFIG_API}/itm:transport-zones/transport-zone/${itm_created[0]}/
    SLEEP    5    
    #Wait Until Keyword Succeeds    40    10    Verify Data Base after Delete    ${Dpn_id_1}    ${Dpn_id_2}    ${ovs_of_tunnel_1}    ${ovs_of_tunnel_2}
    Display OVS Show    ${conn_id_1}
    Display OVS Show    ${conn_id_2}
    Display OVS Show    ${conn_id_3}
...
Configure and verify OF based tunnels on all 3 DPNs
    ...
    ${type}    set variable    odl-interface:tunnel-type-vxlan    
#    #${tunnel-1}    Wait Until Keyword Succeeds    40    10    Get Tunnel    ${Dpn_id_1}    ${Dpn_id_2}
#    #${type}    Set Global Variable    ${tunnel-1}    
#    #${tunnel-2}    Wait Until Keyword Succeeds    40    10    Get Tunnel    ${Dpn_id_2}    ${Dpn_id_1}
#    #${type}    Set Global Variable    ${tunnel-2}
#    #${tunnel-3}    Wait Until Keyword Succeeds    40    10    Get Tunnel    ${Dpn_id_3}    ${Dpn_id_1}    
#    #${type}    Set Global Variable    ${tunnel-3}    
    ${tunnel-type}=     Set Variable    type: vxlan    
    Get Data From URI    session    ${CONFIG_API}/itm-state:dpn-endpoints/DPN-TEPs-info/${Dpn_id_1}/    
    Get Data From URI    session    ${CONFIG_API}/itm-state:dpn-endpoints/DPN-TEPs-info/${Dpn_id_2}/    

#    #Log    >>>>OVS Validation in Switch 1 for Tunnel Created<<<<< 
#    #Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${conn_id_1}    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${tunnel-1}    ${tunnel-type}
#    #Log    >>>>OVS Validation in Switch 2 for Tunnel Created<<<<<
#    #Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${conn_id_2}    ${TOOLS_SYSTEM_2_IP}    ${TOOLS_SYSTEM_IP}    ${tunnel-2}    ${tunnel-type}

    Log    >>>>OVS Validation in Switch 1 for Tunnel Created<<<<<
    ${tunnel-type}=    Set Variable    type: vxlan
    Wait Until Keyword Succeeds    40    10    Ovs Verification For OF Tunnels    ${conn_id_1}    ${TOOLS_SYSTEM_IP}    ${ovs_of_tunnel_1}    ${tunnel-type}

    Log    >>>>OVS Validation in Switch 2 for Tunnel Created<<<<<
    Wait Until Keyword Succeeds    40    10    Ovs Verification For OF Tunnels    ${conn_id_2}    ${TOOLS_SYSTEM_2_IP}    ${ovs_of_tunnel_2}    ${tunnel-type}

    Log    >>>>OVS Validation in Switch 3 for Tunnel Created<<<<<
    Wait Until Keyword Succeeds    40    10    Ovs Verification For OF Tunnels    ${conn_id_3}    ${TOOLS_SYSTEM_3_IP}    ${ovs_of_tunnel_3}    ${tunnel-type}
...
Configure and verify OF based tunnels on two of 3 DPNs
    ...
    ...    ${Dpn_id_1}    ${TOOLS_SYSTEM_IP}    ${Dpn_id_2}    ${TOOLS_SYSTEM_2_IP}    ${Dpn_id_3}    ${TOOLS_SYSTEM_3_IP}
    ${type}    set variable    odl-interface:tunnel-type-vxlan    
#    ${tunnel-1}    Wait Until Keyword Succeeds    40
...
Delete and verify OF tunnels on two of 3 DPNs
    ...
    Remove All Elements At URI And Verify    ${CONFIG_API}/itm:transport-zones/transport-zone/${itm_created[0]}/
    SLEEP    5    
    #Wait Until Keyword Succeeds    40    10    Verify Data Base after Delete    {Dpn_id_1}    ${Dpn_id_2}    ${tunnel-1}    ${tunnel-2}
...
Verify OF Tunnel ports gets deleted when TEP is deleted from DPN
    ...
    ${body}    set json    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${TOOLS_SYSTEM_3_IP}    ${vlan}    ${gateway-ip}    ${subnet}    ${file_name} 
    Log    ${body}

    # Trying to delete vtep of 3rd OVS
...
Verify OF Tunnel ports gets deleted when all TEP are deleted on other DPNs
    ...
    ${body}    Set Json for 1 DPN    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${TOOLS_SYSTEM_3_IP}    ${vlan}    ${gateway-ip}    ${subnet}
    ...    ${file_name}

    # Delete vtep of 2nd & 3rd OVS
    ${resp}    RequestsLibrary.Put Request    session    ${CONFIG_API}/itm:transport-zones/transport-zone/TZA    data=${body}
...
Verify update/modification of OF based tunnels on one DPN of 2 DPNs
    ...
    Update Vteps    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${TOOLS_SYSTEM_3_IP}    ${vlan}    ${gateway-ip}    ${file_name2}    2
    Sleep    5

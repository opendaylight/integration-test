...
table entry
    ...
    ${result} =    Utils.Run Command On Remote System And Log    ${TOOLS_SYSTEM_1_IP}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${INTEGRATION_BRIDGE}
...
no table0 entry
    ...
    ${ovs-check} =    Utils.Run Command On Remote System And Log    ${TOOLS_SYSTEM_1_IP}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${INTEGRATION_BRIDGE}
...
no goto_table entry
    ...
    ${ovs-check1} =    Utils.Run Command On Remote System And Log    ${TOOLS_SYSTEM_1_IP}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${INTEGRATION_BRIDGE}
...
table0 entry
    ...
    ${ovs-check} =    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-ofctl -O OpenFlow13 dump-flows ${INTEGRATION_BRIDGE}

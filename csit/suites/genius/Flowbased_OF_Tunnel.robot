...
OFT Verify Vteps Created
    ...
    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Update Dpnid list and get OfTeps    odl-interface:tunnel-type-vxlan    dpn-teps-state    ${dpn_id_list}
    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Verify response code of of tep config    ${dpn_id_list}
    ${num_switches} =    BuiltIn.Get Length    ${dpn_id_list}
    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Verify OFTep status as UP    ${num_switches}
...
OFT Verify Vteps Deleted
    ...
    BuiltIn.Run Keyword If    ${existing_tep_len} > 0    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Verify OFTep status as UP    ${existing_tep_len}
    ${tep_show_output} =    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW}
    ${tep_show_state_output} =    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW_OFPORTS}
    FOR    ${tools_system_index}    IN RANGE    ${deleted_tep_len}
    ${tep_show_state_output_1} =    KarafKeywords.Issue Command On Karaf Console    ${TEP_SHOW_OFPORTS}
    ...
    BuiltIn.Wait Until Keyword Succeeds    60    5    Utils.No Content From URI    session    ${CONFIG_API}/itm-state:dpn-tep-config/@{dpn_id_list}/
...
OFT Verify Vteps Deleted at Dpn Teps State per Interface
    ...
    ${status} =    BuiltIn.Run Keyword And Return Status    Genius.Get Ofteps    ${src_dpn_id}    ${dst_dpn_id}    odl-interface:tunnel-type-vxlan    dpn-teps-state
...
OF Tunnels Start Suite
    ...
    Run Command On Remote System And Log    ${ODL_SYSTEM_${controller_index+1}_IP}    sed -i -- 's/<itm-of-tunnels>true/<itm-of-tunnels>true/g' ${GENIUS_IFM_CONFIG_FLAG}
    END

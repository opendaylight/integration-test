...
BFD_TC02 Enable BFD Monitoring And Verify On Controller
    [Documentation]    Enable BFD monitoring in branches lesser than neon and verify that BFD is enabled in the controller.
    Comment    ${branch} =    CompareStream.Set_Variable_If_At_Least_Neon    ATELAST NEON    LESS THAN NEON
    Comment    BuiltIn.Run Keyword If    '${branch}' == 'ATLEAST NEON'    Enable BFD And Verify
    Run_Keyword_If_At_Least_Neon    Enable BFD And Verify    ${EMPTY}

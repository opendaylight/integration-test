...
Create and Verify VTEP -No Vlan
    ...
    ${check-1}    Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${mininet1_conn_id_1}    ${TOOLS_SYSTEM_IP}
    ...    ${TOOLS_SYSTEM_2_IP}    ${tunnel-1}    ${tunnel-type}
    Log    ${check-1}
    Log    >>>>OVS Validation in Switch 2 for Tunnel Created<<<<<
    ${check-2}    Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${mininet2_conn_id_1}    ${TOOLS_SYSTEM_2_IP}
...
Create and Verify VTEP-Vlan
    ...
    Log    >>>>OVS Validation in Switch 1 for Tunnel Created<<<<<
    ${check-1}    Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${mininet1_conn_id_1}    ${TOOLS_SYSTEM_IP}
    ...    ${TOOLS_SYSTEM_2_IP}    ${tunnel-3}    ${tunnel-type}
    Log    ${check-1}
    Log    >>>>OVS Validation in Switch 2 for Tunnel Created<<<<<
    ${check-2}    Wait Until Keyword Succeeds    40    10    Ovs Verification 2 Dpn    ${mininet2_conn_id_1}    ${TOOLS_SYSTEM_2_IP}
    ...    ${TOOLS_SYSTEM_IP}    ${tunnel-4}    ${tunnel-type}
    Log    ${check-2}
...
Get ITM
    ...
    ...    ${TOOLS_SYSTEM_IP}    ${Dpn_id_2}    BR2-eth1    ${TOOLS_SYSTEM_2_IP}

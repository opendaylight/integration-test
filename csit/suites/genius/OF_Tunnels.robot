...
Create and Verify TZ with OFT TEPs
    ...
    OFT Create Vteps    ${NO_VLAN}    ${gateway_ip}    ${DPN_ID_LIST}    ${TOOLS_SYSTEM_ALL_IPS}
...
Create and Verify TZ with single OFT TEPs
    ...
    OFT Create Vteps    ${NO_VLAN}    ${gateway_ip}    ${dpn_ids}    ${tools_ips}
...
*** Keywords ***
OFT Create Vteps
    [Arguments]    ${vlan_id}    ${gateway_ip}    ${dpn_ids}    ${tools_ips}
    [Documentation]    Creates VTEPs for selected tools systems in ODL.
    ${body} =    OFT Set Json    ${vlan_id}    ${gateway_ip}    ${SUBNET}    ${dpn_ids}    ${tools_ips}

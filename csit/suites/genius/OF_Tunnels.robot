...
OFT Verify Vteps Created
    ...
    Comment    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Update Dpn id List And Get Tunnels    odl-interface:tunnel-type-vxlan    dpn-teps-state
    ...    ${dpn_id_list}
    Comment    BuiltIn.Wait Until Keyword Succeeds    60    5    Genius.Verify Response Code Of Dpn End Point Config API    ${dpn_id_list}
    ...
    Comment    BuiltIn.Wait Until Keyword Succeeds    40    10    OFT OVS Verify Egress Flows Created per Switch    ${tools_ip_list}[${tools_system_index}]    ${tun_ip_list}
    ...    ${port_numbers}
    END
...
OFT OVS Verify Ingress Flows Created per Switch
    ...
    Comment    FOR    ${tun_src}    IN    @{tun_src_list}
    Comment    BuiltIn.Should Contain    ${flows_table0_output}    tun_src=${tun_src},
    Comment    END
...
OF Tunnels Start Suite
    ...
    Run Command On Remote System And Log    ${ODL_SYSTEM_${controller_index+1}_IP}    sed -i -- 's/<itm-of-tunnels>false/<itm-of-tunnels>true/g' ${GENIUS_IFM_CONFIG_FLAG}
    END
...
OFT Verify Tunnel status as UP
    ...
    ${lines_of_state_up} =    String.Get Lines Containing String    ${no_of_tunnels}    Up

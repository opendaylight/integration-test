...
OFT Create Vteps using Auto Tunnels
    ...
    \    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-vsctl set O . other_config:local_ip=${tools_ip}
...
OFT Verify Vteps Created
    ...
    Wait Until Keyword Succeeds    60    5    OFT Verify TZ    ${dpn_ids}    ${tools_ips}
...
OFT Verify TZ
    ...
    ${switch_data} =    BuiltIn.Create List    ${DEFAULT_GATEWAY}    @{dpn_ids}    @{tools_ips}
    Genius.Get ITM    ${DEFAULT_TRANSPORT_ZONE}    ${DEFAULT_SUBNET}    ${DEFAULT_VLAN}    ${switch_data}
...
OFT Verify Vteps Created at Tep Show
    ...
    BuiltIn.Should Contain    ${output}    ${DEFAULT_SUBNET}
    BuiltIn.Should Contain    ${output}    ${DEFAULT_GATEWAY}
    BuiltIn.Should Contain    ${output}    ${DEFAULT_VLAN}
...
OFT Delete Vteps using Auto Tunnels
    ...
    : FOR    ${tools_ip}    IN    @{tools_ips}
    \    Utils.Run Command On Remote System And Log    ${tools_ip}    sudo ovs-vsctl remove O . other_config local_ip

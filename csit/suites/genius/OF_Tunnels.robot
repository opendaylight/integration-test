...
Create and Verify OFT TEPs
    ...
    OF_Tunnels.Create Vteps using Auto Tunnels    @{TOOLS_SYSTEM_ALL_IPS}
    OF_Tunnels.Verify Vteps Created    ${DPN_ID_LIST}    ${TOOLS_SYSTEM_ALL_IPS}

Delete and Verify OFT TEPs
    [Documentation]    Deletes TEPs set to use OF based Tunnels and verifies.
    OF_Tunnels.OVS Verify Tunnels Created    @{TOOLS_SYSTEM_ALL_IPS}
    OF_Tunnels.Delete Vteps using Auto Tunnels    @{TOOLS_SYSTEM_ALL_IPS}
    OF_Tunnels.Verify Vteps Deleted    ${DPN_ID_LIST}    ${TOOLS_SYSTEM_ALL_IPS}
...
Create and Verify single OFT TEPs
    ...
    OF_Tunnels.Create Vteps using Auto Tunnels    @{tools_ips}
    OF_Tunnels.Verify Vteps Created    ${dpn_ids}    ${tools_ips}
    OF_Tunnels.Create Vteps using Auto Tunnels    @{TOOLS_SYSTEM_ALL_IPS}[-1]
    OF_Tunnels.Verify Vteps Created    ${DPN_ID_LIST}    ${TOOLS_SYSTEM_ALL_IPS}
...
Delete and Verify single OFT TEPs
    ...
    OF_Tunnels.Delete Vteps using Auto Tunnels    @{deleted_tools_ip_list}
    ${deleted_dpn_id_list} =    BuiltIn.CreateList    @{DPN_ID_LIST}[0]
    OF_Tunnels.Verify Vteps Deleted    ${deleted_dpn_id_list}    ${deleted_tools_ip_list}
...
Verify Reference Count with Selective BFD
    ...
    OF_Tunnels.Create Vteps using Auto Tunnels    @{TOOLS_SYSTEM_ALL_IPS}[0]
    BuiltIn.Wait Until Keyword Succeeds    40    10    OF_Tunnels.OVS Verify Tunnels Created    @{TOOLS_SYSTEM_ALL_IPS}[0]
    ...
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Set BFD State per Switch    @{DPN_ID_LIST}[${num}]    ${dst_dpn_id_list}
    \    ...    ${True}
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Verify BFD State per Switch    @{DPN_ID_LIST}[${num}]    ${dst_dpn_id_list}
    \    ...    ${True}
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Verify Reference Count per Source    @{DPN_ID_LIST}[${num}]    ${dst_dpn_id_list}
    \    ...    1
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Verify P2P per Source    @{DPN_ID_LIST}[${num}]    ${dst_dpn_id_list}
    \    ...    ${True}
...
Delete and Verify OFT TEPs with Non-Zero Reference Count
    ...
    BuiltIn.Wait Until Keyword Succeeds    40    10    OF_Tunnels.OVS Verify Tunnels Created    @{TOOLS_SYSTEM_ALL_IPS}
    ${len} =    BuiltIn.Get Length    ${DPN_ID_LIST}
    : FOR    ${num}    IN RANGE    ${len}
    \    ${dst_dpn_id_list} =    BuiltIn.Create List    @{DPN_ID_LIST}
    \    Collections.Remove From List    ${dst_dpn_id_list}    ${num}
    ...
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Verify P2P per Source    ${src_dpn_id}    ${DPN_ID_LIST}
    \    ...    ${False}
...
*** Keywords ***
Create Vteps using Auto Tunnels
...
Verify Vteps Created
    ...
    BuiltIn.Wait Until Keyword Succeeds    40    10    OF_Tunnels.OVS Verify Tunnels Created    @{tools_ip_list}
    ...
    \    BuiltIn.Wait Until Keyword Succeeds    40    10    OF_Tunnels.OVS Verify Ingress Flows Created per Switch    @{tools_ip_list}[${num}]    ${tun_src_list}
    BuiltIn.Comment    TODO: Check egress flows (table 95)

OVS Verify Tunnels Created
    [Arguments]    @{tools_ip_list}
...
OVS Verify Ingress Flows Created per Switch
...
Delete Vteps using Auto Tunnels
    [Arguments]    @{tools_ip_list}
...
Verify Vteps Deleted
    ...
    \    ${dst_dpn_id_list} =    BuiltIn.Create List    @{DPN_ID_LIST}
    \    Collections.Remove From List    ${dst_dpn_id_list}    ${num}
    \    BuiltIn.Wait Until Keyword Succeeds    60    5    OF_Tunnels.Verify Vteps Deleted at Dpn Teps State per Interface    @{dpn_id_list}[${num}]    ${dst_dpn_id_list}
    ...
    \    ${tun_src_list} =    BuiltIn.CreateList    @{tools_ip_list}
    \    Collections.Remove From List    ${tun_src_list}    ${num}
    \    BuiltIn.Wait Until Keyword Succeeds    40    10    OF_Tunnels.OVS Verify Ingress Flows Deleted per Switch    @{tools_ip_list}[${num}]    ${tun_src_list}
    \    BuiltIn.Comment    TODO: Check egress flows (table 95)

Verify Vteps Deleted at Dpn Teps State per Interface
...
OVS Verify Ingress Flows Deleted per Switch
...
Verify BFD State per Switch
    [Arguments]    ${src_dpn_id}    ${dst_dpn_id_list}    ${enable}
    [Documentation]    Verifies BFD State in ODL for all src-dst dpn pair for a given src dpn.
...
Set BFD State per Switch
    [Arguments]    ${src_dpn_id}    ${dst_dpn_id_list}    ${enable}
    [Documentation]    Sets BFD State in ODL for all src-dst dpn pair for a given src dpn.
...
Verify Reference Count per Source
    [Arguments]    ${src_dpn_id}    ${dst_dpn_id_list}    ${value}    ${condition}=\=\=
    [Documentation]    Verifies the value of Reference Count in ODL for all src-dst dpn pair for a given src dpn.
    : FOR    ${dst_dpn_id}    IN    @{dst_dpn_id_list}
...
Verify P2P per Source
    [Arguments]    ${src_dpn_id}    ${dst_dpn_id_list}    ${is_created}
    [Documentation]    Verifies P2P is created/deleted in ODL for all src-dst dpn pair for a given src dpn.
    ${input} =    BuiltIn.Create Dictionary
    ${body} =    BuiltIn.Create Dictionary
    Collections.Set To Dictionary    ${body}    input=${input}
    : FOR    ${dst_dpn_id}    IN    @{dst_dpn_id_list}

...
@{CONNECTION_IDS}    ${conn_id_1}    ${conn_id_2}    ${conn_id_3}
...
Verify Creation and Deletion of OF based tunnels on 2 DPNs
    ...
    Genius.Create Vteps    ${dpn_Id_1}    ${dpn_Id_2}    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${vlan}    ${gateway-ip}    ${FILENAME1}
    ${ovs_of_tunnel_1} =    Get Tunnel From OVS Show    ${conn_id_1}    ${Bridge-1}
    BuiltIn.Should Match Regexp    ${ovs_of_tunnel_1}   ${TUN} 
    ${ovs_of_tunnel_2} =    Get Tunnel From OVS Show    ${conn_id_2}    ${Bridge-2}
    BuiltIn.Should Match Regexp    ${ovs_of_tunnel_2}   ${TUN} 
    ...
    BuiltIn.Wait Until Keyword Succeeds    40    10    OVSDB.Ovs Verification For Tunnels    ${conn_id_2}    True    local_ip="${TOOLS_SYSTEM_2_IP}"    ${REMOTE_FLOW}    ${ovs_of_tunnel_2}
    ...    ${tunnel-type}
    ...
    BuiltIn.Should Not Contain    ${output}    ${DISABLE}
...
Verify creation and deletion of OF tunnel on 1 DPN and non-OF tunnel on another DPN
    ...
    Genius.Create Vteps    ${dpn_Id_1}    ${dpn_Id_2}    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}    ${vlan}    ${gateway-ip}    ${FILENAME2}
    ${ovs_of_tunnel_1} =    Get Tunnel From OVS Show    ${conn_id_1}    ${Bridge-1}
    BuiltIn.Should Match Regexp    ${ovs_of_tunnel_1}   ${TUN}
    ${ovs_of_tunnel_2} =    Get Tunnel From OVS Show    ${conn_id_2}    ${Bridge-2}
    BuiltIn.Should Match Regexp    ${ovs_of_tunnel_2}   ${TUN}
    ...
    ...    remote_ip="${TOOLS_SYSTEM_IP}"    ${ovs_of_tunnel_2}    ${tunnel-type}
    BuiltIn.Wait Until Keyword Succeeds    40    10    OVSDB.Ovs Verification For Tunnels    ${conn_id_1}    True    local_ip="${TOOLS_SYSTEM_IP}"    ${ovs_of_tunnel_1}
    ...    ${tunnel-type}
    ${url-2} =    BuiltIn.Set Variable    ${OPERATIONAL_API}/network-topology:network-topology/
    ${resp} =    BuiltIn.Wait Until Keyword Succeeds    40    10    Get Network Topology with Tunnel    @{Bridge}[0]    @{Bridge}[1]
    ...    ${ovs_of_tunnel_1}    ${ovs_of_tunnel_2}    ${url-2}
    ...
    ${output} =    KarafKeywords.Issue Command On Karaf Console    ${VXLAN_SHOW}
    BuiltIn.Should Not Contain    ${output}    ${DISABLE}
    BuiltIn.Should Contain    ${output}    ${ENABLED}
    : FOR    ${connection_id}    IN    @{CONNECTION_IDS}
    \    Utils.Remove All Elements At URI And Verify    ${CONFIG_API}/itm:transport-zones/transport-zone/${itm_created[0]}/
    \    OVSDB.Ovs Verification For Tunnels    ${connection_id}    False    ${REMOTE_FLOW}    

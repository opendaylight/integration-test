...
Delete VTEP with Non-Zero Reference Count
    [Documentation]    Verify BFD monitoring can be enabled for OF tunnels.
    Comment    TODO
...
OFT Set Json
    ...
    Collections.Set To Dictionary    ${transport_zone_json}    tunnel-type=odl-interface:tunnel-type-vxlan
    Collections.Set To Dictionary    ${transport_zone_json}    zone-name=TZA
...
OFT Verify TZ Created
    ...
    ${parameters} =    BuiltIn.Create List    ${vlan}    ${gateway_ip}    ${subnet}    @{dpn_ids}    @{tools_ips}
    Utils.Check For Elements At URI    ${TUNNEL_TRANSPORTZONE}/transport-zone/@{itm_created}[0]    ${parameters}
...
OFT OVS Verify Tunnels Created
    ...
    \    ${matches} =    String.Get Regex Matches    ${output}    ${OFT_OVSIF_REGEX}    1    2
    \    ...    3
    \    BuiltIn.Log    ${matches}

...
OFT Verify Vteps Deleted at Dpn Teps State per Interface
    ...
    \    ${status} =    Run Keyword And Return Status    Genius.Get Tunnel    ${dst_dpn_id}    ${src_dpn_id}    ${OFT_TUNNEL_TYPE}
    \    ...    dpn-teps-state
    \    BuiltIn.Should Be True    ${status} == ${False}
...
OFT Verify Vteps Deleted at Tunnels State
    ...
    : FOR    ${tools_ip}    IN    @{tools_ip_list}
    \    BuiltIn.Should Not Contain    ${resp.content}    ${tools_ip}
...
OFT Verify BFD State per Switch
    ...
    : FOR    ${dst_dpn_id}    IN    @{dst_dpn_id_list}
    \    ${resp} =    RequestsLibrary.Get Request    session    ${CONFIG_API}/itm-state:dpn-teps-state/dpns-teps/${src_dpn_id}/remote-dpns/${dst_dpn_id}
    \    BuiltIn.Should Contain    ${resp.content}    <monitoring-enabled>${enable}</monitoring-enabled>
...
OFT Set BFD State per Switch
    ...
    \    ${resp} =    RequestsLibrary.Post Request    session    ${OPERATIONS_API}/itm-rpc:set-bfd-param-on-tunnel    data=${data}
...
OFT Verify Reference Count per Source
    ...
    : FOR    ${dst_dpn_id}    IN    @{dst_dpn_id_list}
    \    ${resp} =    RequestsLibrary.Get Request    ${OPERATIONAL_API}/odl-itm-meta:monitoring-ref-count/monitored-tunnels/${src_dpn_id}/${dst_dpn_id}
    \    ${matches} =    String.Get Regexp Matches    ${resp.content}    <reference-count>(\\d+)</reference-count>    1
    \    BuiltIn.Log    ${matches}
...
OFT Verify P2P Created per Source
    ...
    \    Collections.Set To Dictionary    ${input}    destination-node=${dst_dpn_id}
    \    ${data} =    BuiltIn.Evaluate    json.dumps(${body}, indent=4)    json
    \    ${resp} =    RequestsLibrary.Post Request    session    ${OPERATIONS_API}/itm-rpc:get-watch-port-for-tunnel    data=${data}
    \    BuiltIn.Should Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}
...
OFT Verify P2P Deleted per Source
    ...
    \    ${data} =    BuiltIn.Evaluate    json.dumps(${body}, indent=4)    json
    \    ${resp} =    RequestsLibrary.Post Request    session    ${OPERATIONS_API}/itm-rpc:get-watch-port-for-tunnel    data=${data}
    \    BuiltIn.Should Not Be Equal As Strings    ${resp.status_code}    ${RESP_CODE}

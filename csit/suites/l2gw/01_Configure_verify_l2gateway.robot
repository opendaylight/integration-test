...
TC15 Update Existing L2Gateway With 2nd Port And Verify
    [Documentation]    Add the new port ${NS_PORT3} to existing L2GW that has an existing connection. ${NS_PORT1} is already attached, L2GW API all ports in update cmd
    ${output}=    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Update And Verify L2Gateway    ${L2GW_NAME1}    ${NS_PORT1}    ${NS_PORT3}

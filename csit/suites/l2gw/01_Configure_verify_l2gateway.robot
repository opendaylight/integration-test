...
TC15 Update Existing L2Gateway With 2nd Port And Verify
    [Documentation]    Add the new port ${HWVTEP_PORT_3} to existing L2GW that has an existing connection. ${HWVTEP_PORT_1} is already attached, L2GW API all ports in update cmd
    ${output}=    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Update And Verify L2Gateway    ${L2GW_NAME1}    ${HWVTEP_PORT_1}    ${HWVTEP_PORT_3}

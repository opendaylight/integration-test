...
TC14 Delete L2GW For Second Network Verify Ping
    ...
    ${output}=    Wait Until Keyword Succeeds    60s    20s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[2]}
    ...    ping -c 3 ${port_ip_list[3]}
    Log    ${output}
    ${output}=    Wait Until Keyword Succeeds    60s    20s    Execute Command on VM Instance    ${NET_1}    ${port_ip_list[0]}
    ...    ping -c 3 ${port_ip_list[1]}
    Log    ${output}

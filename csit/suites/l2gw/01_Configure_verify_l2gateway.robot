...
TC15 Update Existing L2Gateway With 2nd Port And Verify
    [Documentation]    Add the new port to existing L2GW connection.
    ${output}=    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Update And Verify L2Gateway    ${L2GW_NAME1}    ${NET_1}    ${HWVTEP_PORT_3}

...
TC10 Update Hwvtep Port Create Nova Vm On Second Network And L2Gw Creation
...
TC10 Update Hwvtep Port Create Nova Vm On Second Network And L2Gw Creation a
...
TC10 Update Hwvtep Port Create Nova Vm On Second Network And L2Gw Creation b
    Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Verify Vtep List    ${hwvtep_conn_id}    ${TUNNEL_TABLE}    enable="true"
    ${phy_port_out}=    Get Vtep List    ${PHYSICAL_PORT_TABLE}
...
TC11 Dhcp Ip Allocation And Ping Validation
    Wait Until Keyword Succeeds    180s    10s    L2GatewayOperations.Namespace Dhclient Verify    ${HWVTEP_NS2}    ${NS_TAP2}    ${port_ip_list[3]}
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[2]}
    ...    ping -c 3 ${port_ip_list[3]}
    Log    ${output}
    Should Not Contain    ${output}    ${PACKET_LOSS}

TC11 Dhcp Ip Allocation And Ping Validation a
...
TC12 Ping Between The Networks Which Should Fail
    Wait Until Keyword Succeeds    30s    5s    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS2}    ${port_mac_list[3]}    ${port_ip_list[0]}

TC13 Ping Between OVs and Ns
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[2]}
    ...    ping -c 3 ${port_ip_list[1]}
    Log    ${output}

...
TC18 Readd L2gw For Second Network
    ...
    ${output}=    L2GatewayOperations.Create Verify L2Gateway Connection    ${L2GW_NAME2}    ${NET_2}
    Log    ${output}
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[4]}
    ...    ping -c 3 ${port_ip_list[5]}
    Log    ${output}
...
TC20 Delete Manger And Verify
    ${output}=    Exec Command    ${hwvteo_conn_id}    sudo vtep-ctl del-manager

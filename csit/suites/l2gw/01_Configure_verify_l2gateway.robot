...
TC02 Create First Set Of Network Subnet And Ports
    ...
    Wait Until Keyword Succeeds    60s    5s    OpenStackOperations.Create SubNet    ${NET_1}    ${SUBNET_1}    ${SUBNET_RANGE1}
    ...
    Wait Until Keyword Succeeds    25s    5s    OpenStackOperations.Create And Configure Security Group    ${SECURITY_GROUP_L2GW}
    Wait Until Keyword Succeeds    25s    5s    OpenStackOperations.Create Port    ${NET_1}    ${OVS_PORT_1}    sg=${SECURITY_GROUP_L2GW}
    Wait Until Keyword Succeeds    25s    5s    OpenStackOperations.Create Port    ${NET_1}    ${HWVTEP_PORT_1}    sg=${SECURITY_GROUP_L2GW}
...
TC04 Create Vms On Compute Node
    ...
    Wait Until Keyword Succeeds    25s    5s    OpenStackOperations.Verify VM Is ACTIVE    ${OVS_VM1_NAME} 
    Wait Until Keyword Succeeds    25s    5s    OpenStackOperations.List Nova VMs 
#    ${vm_ip}=    Wait Until Keyword Succeeds    360s    10s    L2GatewayOperations.Verify Nova VM IP    ${OVS_VM1_NAME}
    ${vm_ip}=    Wait Until Keyword Succeeds    180s    10s    OpenStackOperations.Get VMs IPAddress    ${OVS_VM1_NAME}
    Log    ${vm_ip}
...
#    OpenStackOperations.Create SubNet    ${NET_2}    ${SUBNET_2}    ${SUBNET_RANGE2}

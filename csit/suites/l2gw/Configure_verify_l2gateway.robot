...
*** Test Cases ***
TC1 Configure Hwvtep Manager And Verify
    L2GatewayOperations.Add Vtep Manager And Verify    ${ODL_IP}

TC2 Configure Ovs Manager Controller And Verify
    L2GatewayOperations.Add Ovs Bridge Manager Controller And Verify

TC3 Create First Set Network Subnet Port And Vms
...
TC4 Update Port For Hwvtep Configuration
    L2GatewayOperations.Update Port For Hwvtep    ${HWVTEP_PORT_1}

TC5 Attach Hwvtep Port To Namespace
...
TC7 Update First Set Ovs Port And Create Vm
...
TC14 Ping Verification From Namespace Tap To Ovs Vm
    L2GatewayOperations.Verify Ping In Namespace    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC14a Ping Verification From Namespace Tap To Ovs Vm
    L2GatewayOperations.Verify Ping In Namespace1    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC14b Ping Verification From Namespace Tap To Ovs Vm
    L2GatewayOperations.Verify Ping In Namespace2a    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC14c Ping Verification From Namespace Tap To Ovs Vm
    L2GatewayOperations.Verify Ping In Namespace2b    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC15 Create Second Set Neutron Network Subnet Ports and Verify
    ...
    OpenStackOperations.Create Neutron Port With Additional Params    ${NET_2}    ${OVS_PORT_2}
    OpenStackOperations.Create Neutron Port With Additional Params    ${NET_2}    ${HWVTEP_PORT_2}
    ${port_mac}=    Get Port Mac    ${OVS_PORT_2}
    ${port_ip}=    Get Port Ip    ${OVS_PORT_2}
    Append To List    ${port_mac_list}    ${port_mac}
    Append To List    ${port_ip_list}    ${port_ip}
    ${port_mac}=    Get Port Mac    ${HWVTEP_PORT_2}
    ${port_ip}=    Get Port Ip    ${HWVTEP_PORT_2}
    Append To List    ${port_mac_list}    ${port_mac}
    Append To List    ${port_ip_list}    ${port_ip}

TC16 Update Second Set Ports In Controller Add To Hwvtep and Ovs
    ...
    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Attach Port To Hwvtep Namespace    ${port_mac_list[3]}    ${HWVTEP_NS1}    ${NS_TAP2}

TC17 Create Second Set L2Gateway Connection And Verify
    ...
    Get L2gw Debug Info
    Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Verify Vtep List    ${TUNNEL_TABLE}    enable="true"
    Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Verify Vtep List    ${UCAST_MACS_REMOTE_TABLE}    ${port_mac_list[2]}
    ${phy_port_out}=    Get Vtep List    ${PHYSICAL_PORT_TABLE}
    L2GatewayOperations.Validate Regexp In String    ${phy_port_out}    ${VLAN_BINDING_REGEX}    2

TC18 Dhcp Ip Allocation To The Namespace Second Set On Vtep And Ovs
    Wait Until Keyword Succeeds    10s    2s    L2GatewayOperations.Namespace Dhclient Verify    ${HWVTEP_NS1}    ${NS_TAP2}    ${port_ip_list[3]}
...
TC20 Cleanup L2Gateway Connection Itm Tunnel Port Subnet And Network

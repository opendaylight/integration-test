...
TC04_A Create Vms On Both Compute Nodes
...
TC05 Create Itm Tunnel L2Gateway And Connection And Verify
...
TC06_A Dhcp Ip Allocation For Hwvtep Tap Port
    L2GatewayOperations.Namespace Dhclient Verify    ${HWVTEP_NS1}    ${NS_TAP1}    ${port_ip_list[1]}
    ${dhcp_status}=    Set Variable    ASSIGNED

TC06_B Static Ip Allocation If Above Test Fails
    Run Keyword If    '${dhcp_status}' == '${EMPTY}'    Namespace Static Ip Assign    ${HWVTEP_NS1}    ${NS_TAP1}    ${port_ip_list[1]}

TC07_A Verify Ping From Compute Node Vms To Hwvtep
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_1}    ${port_ip_list[0]}
...
TC07_B Ping Verification From Namespace Tap Port To Ovs Vms In Background With Default Timeout
    L2GatewayOperations.Verify Ping In Namespace Background    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC07_C Ping Verification From Namespace Tap To Ovs Vm Normal With Extra Timeout
    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}
    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[2]}    #TC8 Ping Verification From Namespace Tap To Ovs Vm Normal Ping    #    L2GatewayOperations.Verify Ping In Namespace_A
    ...    # ${HWVTEP_NS1}    ${port_mac_list[1]}    # ${port_ip_list[0]}    #TC15 Create Second Set Neutron Network Subnet Ports and Verify    #    OpenStackOperations.Create Network
    ...    # ${NET_2}    ${NET_ADDT_ARG}${NET_2_SEGID}    #    ${output}=    OpenStackOperations.List Networks    #
    ...    # Should Contain    ${output}    # ${NET_2}    #    OpenStackOperations.Create SubNet    ${NET_2}
    ...    # ${SUBNET_2}    ${SUBNET_RANGE2}    # ${SUBNET_ADDT_ARG}    #    ${output}=    OpenStackOperations.List Subnets
    ...    #    Should Contain    # ${output}    ${SUBNET_2}    #    OpenStackOperations.Create Neutron Port With Additional Params
    ...    # ${NET_2}    ${OVS_PORT_2}    #    OpenStackOperations.Create Neutron Port With Additional Params    ${NET_2}    ${HWVTEP_PORT_2}
    ...    #    ${port_mac}=    # Get Port Mac    ${OVS_PORT_2}    #    ${port_ip}=
    ...    # Get Port Ip    ${OVS_PORT_2}    #    Append To List    ${port_mac_list}    ${port_mac}
    ...    #    Append To List    # ${port_ip_list}    ${port_ip}    #    ${port_mac}=
    ...    # Get Port Mac    ${HWVTEP_PORT_2}    #    ${port_ip}=    Get Port Ip    ${HWVTEP_PORT_2}
    ...    #    Append To List    # ${port_mac_list}    ${port_mac}    #    Append To List
    ...    # ${port_ip_list}    ${port_ip}    #    #TC16 Update Second Set Ports In Controller Add To Hwvtep and Ovs    #    L2GatewayOperations.Update Port For Hwvtep
    ...    # ${HWVTEP_PORT_2}    #    # OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS_PORT_2}    ${OVS_VM2_NAME}    ${OVS_IP}
    ...    #    ${vm_ip}=    # Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Verify Nova VM IP
    ...    # ${OVS_VM2_NAME}    #    # Log    ${vm_ip}    #    Should Contain
    ...    # ${vm_ip[0]}    ${port_ip_list[2]}    #    Wait Until Keyword Succeeds    30s    2s
    ...    # L2GatewayOperations.Attach Port To Hwvtep Namespace    ${port_mac_list[3]}    # ${HWVTEP_NS1}    # ${NS_TAP2}    #    #TC17 Create Second Set L2Gateway Connection And Verify
    ...    #    ${output}=    # L2GatewayOperations.Create Verify L2Gateway    ${HWVTEP_BRIDGE}    ${NS_PORT2}    ${L2GW_NAME2}
    ...    #    Log    # ${output}    #    ${output}=    L2GatewayOperations.Create Verify L2Gateway Connection
    ...    # ${L2GW_NAME2}    ${NET_2}    #    Log    ${output}    #
    ...    # Get L2gw Debug Info    #    # Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Verify Vtep List
    ...    # ${TUNNEL_TABLE}    enable="true"    #    Wait Until Keyword Succeeds    30s    1s
    ...    # L2GatewayOperations.Verify Vtep List    ${UCAST_MACS_REMOTE_TABLE}    # ${port_mac_list[2]}    #    ${phy_port_out}=    Get Vtep List
    ...    # ${PHYSICAL_PORT_TABLE}    #    # L2GatewayOperations.Validate Regexp In String    ${phy_port_out}    ${VLAN_BINDING_REGEX}    2
    ...    #    #TC18 Dhcp Ip Allocation To The Namespace Second Set On Vtep And Ovs    #    Wait Until Keyword Succeeds    10s    2s
    ...    # L2GatewayOperations.Namespace Dhclient Verify    ${HWVTEP_NS1}    # ${NS_TAP2}    # ${port_ip_list[3]}
    #

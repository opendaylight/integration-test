...
TC09 Additional Network Subnet And Port
...
TC10 Verify Addition Of Ports To Ovs And Tor
...
TC15 Verify Between Networks
    Wait Until Keyword Succeeds    30s    5s    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS2}    ${port_mac_list[3]}    ${port_ip_list[1]}

TC16 Verify Between Networks From Ovs
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[2]}
    ...    ping -c 3 ${port_ip_list[1]}
...
TC Delete Port From Compute Node1
...
TC Readd Ports
    OpenStackOperations.Create Port    ${NET_1}    ${OVS_PORT_1}    sg=${SECURITY_GROUP_L2GW}
    OpenStackOperations.Create Port    ${NET_1}    ${HWVTEP_PORT_1}    sg=${SECURITY_GROUP_L2GW}
    ${port_mac}=    Get Port Mac    ${OVS_PORT_1}    #port_mac[3]
    ${port_ip}=    Get Port Ip    ${OVS_PORT_1}    #port_ip[3]
    Append To List    ${port_mac_list}    ${port_mac}
    Append To List    ${port_ip_list}    ${port_ip}
    ${port_mac}=    Get Port Mac    ${HWVTEP_PORT_1}    #port_mac[4]
    ${port_ip}=    Get Port Ip    ${HWVTEP_PORT_1}    #port_ip[4]
    Append To List    ${port_mac_list}    ${port_mac}
    Append To List    ${port_ip_list}    ${port_ip}
    L2GatewayOperations.Update Port For Hwvtep    ${HWVTEP_PORT_1}
    ...
    Wait Until Keyword Succeeds    30s    5s    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS1}    ${port_mac_list[4]}    ${port_ip_list[3]}

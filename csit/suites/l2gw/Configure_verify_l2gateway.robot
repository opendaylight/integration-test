...
TC2 Create First Set Network Subnet Port And Vms
    ...
    # Security Group Finished
    OpenStackOperations.Create Port    ${NET_1}    ${OVS_PORT_1}    sg=sg-vpnservice
    OpenStackOperations.Create Port    ${NET_1}    ${HWVTEP_PORT_1}    sg=sg-vpnservice
    OpenStackOperations.Create Port    ${NET_1}    ${OVS2_PORT_1}    sg=sg-vpnservice
...
Testcase To Verify Ping Between Compute Nodes
...
Static Ip Allocation If The Above Testcase Fails
...
Testcase To Verify Ping From ComputeNodes To Hwvtep
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_1}    ${port_ip_list[0]}
...
TC8_redo Ping Verification From Namespace Tap To Ovs Vm Normal Ping Extra Timeout
    L2GatewayOperations.Verify Ping In Namespace_B    ${HWVTEP_NS1}    ${port_mac_list[1]}    ${port_ip_list[0]}

TC10 Test
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance_1    ${NET_1}    ${port_ip_list[0]}

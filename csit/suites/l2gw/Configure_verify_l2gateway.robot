...
TC1 Connect Hwvtep And Ovs To Controller Verify
    Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Add Vtep Manager And Verify    ${ODL_IP}
    Wait Until Keyword Succeeds    30s    1s    L2GatewayOperations.Add Ovs Bridge Manager Controller And Verify
...
TC4 Create Itm L2Gateway Connection And Verify
    ...
    Verify Vtep List    ${UCAST_MACS_REMOTE_TABLE}    ${port_mac_list[0]}
...
TC5 Dhcp Ip Allocation To The Namespace On Vtep And Ovs
    Wait Until Keyword Succeeds    10s    2s    L2GatewayOperations.Namespace Dhclient Verify    ${HWVTEP_NS1}    ${NS_TAP1}    ${port_ip_list[1]}
...
TC9 Create Second Set L2Gateway Connection And Verify
    ${output}=    L2GatewayOperations.Create L2Gateway    ${HWVTEP_BRIDGE}    ${NS_PORT2}    ${L2GW_NAME2}
    Log    ${output}
    ${output}=    L2GatewayOperations.Create L2Gateway Connection    ${L2GW_NAME2}    ${NET_2}
    Log    ${output}
    Get L2gw Debug Info
    L2GatewayOperations.Verify Vtep List    ${TUNNEL_TABLE}    enable="true"
    Verify Vtep List    ${UCAST_MACS_REMOTE_TABLE}    ${port_mac_list[2]}
    ${phy_port_out}=    Get Vtep List    ${PHYSICAL_PORT_TABLE}

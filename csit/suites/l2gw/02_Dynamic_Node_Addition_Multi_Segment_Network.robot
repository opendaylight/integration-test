...
04 Verify Dhcp Ip allocation to Sr-iov Ports
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${DHCP_HWVTEP_TABLE}    true    ${mac_list}
...
14 Delete Vm On Compute And Verify Mac Sync
    ...
    Wait Until Keyword Succeeds    12s    3s    L2GatewayOperations.Verify Remote Macs Not In Hwvtep DB    ${hwvtep_conn_id}    ${NET1_ID}    ${mac_list}
    Wait Until Keyword Succeeds    12s    3s    L2GatewayOperations.Verify Remote Macs Not In Hwvtep DB    ${hwvtep2_conn_id}    ${NET1_ID}    ${mac_list}
...
Hwvtep Suite Setup
    [Documentation]    Configured hwvtep in the Tool System VM

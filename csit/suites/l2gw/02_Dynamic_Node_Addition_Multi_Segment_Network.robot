...
10 Verify Ping Among All Nodes
    ...
    L2GatewayOperations.Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS1} nohup ping ${port_ip_list[2]} </dev/null &>/dev/null &    ${HWVTEP_IP}
    L2GatewayOperations.Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP2_NS1} nohup ping ${port_ip_list[0]} </dev/null &>/dev/null &    ${HWVTEP2_IP}
...
11 Add New Compute With Vm And Verify
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS2_PORT_1}    ${OVS2_VM1_NAME}    ${OS_CMP2_HOSTNAME}    sg=${SECURITY_GROUP_L2GW}
...
13 Verify Ping And Remote Macs On All Nodes
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP2_CONN_ID}    ${dst_mac_list}    ${NET_1_SEGID}    True    4
...
17 Delete L2gateway For Second Hwvtep And Verify
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${mac_list}    ${NET_1_SEGID}    False    2

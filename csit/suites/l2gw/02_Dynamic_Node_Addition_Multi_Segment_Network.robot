...
@{HWVTEP_NS_LIST}    ${HWVTEP_NS1}    ${HWVTEP_NS2}    # List of namespace names to configure in the hwvtep nodes
@{HWVTEP2_PORT_LIST}    ${NS2_PORT1}    ${NS2_PORT2}
@{HWVTEP2_NS_LIST}    ${HWVTEP2_NS1}    ${HWVTEP2_NS2}
${HWVTEP_BRIDGE}    br-ovs1
...
04 Verify Dhcp Ip allocation to Sr-iov Ports
    ...
    Verify Entries In Flow Table    ${OS_CMP1_CONN_ID}    ${DHCP_HWVTEP_TABLE}    true    ${mac_list}
...
05 Verify Hwvtep Db Entries
    ...
    Wait Until Keyword Succeeds    30s    10s    Check For Elements At URI    ${HWVTEP_NETWORK_TOPOLOGY}    ${LS_entries}    session
...
07 Add New Vm On Compute Node And Verify Ping
    ...
    Log    ${output}
    Verify Ping Is Successful    ${output}
    Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS1} nohup ping -c500 ${port_ip_list[2]} &    ${HWVTEP_IP}
...
10 Verify Ping Among All Nodes
    ...
    ...    ${hwvtep2_conn_id}
    Start Command In Hwvtep    ${NETNS_EXEC} $${HWVTEP2_NS1} nohup ping -c500 ${port_ip_list[0]} &    ${HWVTEP2_IP}
...
16 Verify Ping And MAC Sync
    ...
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_1}    ${port_ip_list[3]}
    ...    ping -c 3 ${port_ip_list[1]}
    Log    ${output}
    Verify Ping Is Successful    ${output}
    ${mac_list}=    Create List    ${port_mac_list[0]}    ${port_mac_list[1]}    ${port_mac_list[2]}    ${port_mac_list[3]}    unknown-dst

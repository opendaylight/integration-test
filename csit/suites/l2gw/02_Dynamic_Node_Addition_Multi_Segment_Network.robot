...
03 Verify VXLAN Tunnels
    ...
    Wait Until Keyword Succeeds    20s    2s    L2GatewayOperations.Verify Ovs Tunnel    ${HWVTEP_IP}    ${OS_CNTL_IP}    ${NET_1_SEGID}
    ...    ${hwvtep_conn_id}    ${OS_CNTL_CONN_ID}
    ${output}=    VpnOperations.ITM Get Tunnels
    Log    ${output}
    Should Contain    ${output}    physicalswitch/${HWVTEP_BRIDGE}
    ${list}=    Create List    ${OS_CNTL_IP}    ${HWVTEP_IP}
...
04 Verify Dhcp Ip allocation to Sr-iov Ports
    ...
    L2GatewayOperations.Verify Entries In Flow Table    ${OS_CNTL_CONN_ID}    ${DHCP_HWVTEP_TABLE}    true    ${mac_list}
...
07 Add New Vm On Compute Node And Verify Ping
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS_PORT_1}    ${OVS_VM1_NAME}    ${OS_CNTL_HOSTNAME}    sg=${SECURITY_GROUP_L2GW}
    @{vm_name}=    Create List    ${OVS_VM1_NAME}
    Wait Until Keyword Succeeds    180s    15s    L2GatewayOperations.Verify Nova VM IP    ${port_ip_list}    @{vm_name}
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CNTL_CONN_ID}    ${dst_mac_list}    ${NET_1_SEGID}    True    1
...
08 Preprovision New Hwvtep Node And Verify
    ...
    ${list}=    Create List    ${OS_CNTL_IP}    ${HWVTEP_IP}    ${HWVTEP2_IP}
...
09 Verify Tunnels And Dhcp To New Hwvtep Node
    ...
    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Verify Ovs Tunnel    ${HWVTEP2_IP}    ${OS_CNTL_IP}    ${NET1_SEG_ID}
...
11 Add New Compute With Vm And Verify
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS2_PORT_1}    ${OVS2_VM1_NAME}    ${OS_CMP1_HOSTNAME}    sg=${SECURITY_GROUP_L2GW}
...
12 Verify Vxlan Tunnels
    ...
    Wait Until Keyword Succeeds    30s    2s    L2GatewayOperations.Verify Ovs Tunnel    ${HWVTEP_IP}    ${OS_CMP1_IP}    ${NET1_SEG_ID}
    ...
    ${list}=    Create List    ${OS_CNTL_IP}    ${OS_CMP1_IP}    ${HWVTEP_IP}    ${HWVTEP2_IP}
...
13 Verify Ping And Remote Macs On All Nodes
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${dst_mac_list}    ${NET_1_SEGID}    True    3
...
14 Delete Vm On Compute And Verify Mac Sync
    ...
    Wait Until Keyword Succeeds    6s    2s    L2GatewayOperations.Verify Entries In Flow Table    ${OS_CNTL_CONN_ID}    ${DEST_MAC_TABLE}    False
    ...    ${mac_list}
    ${dst_mac_list}=    Create List    ${port_mac_list[0]}    ${port_mac_list[1]}    ${port_mac_list[2]}    ${port_mac_list[3]}    ${port_mac_list[4]}
...
15 Create The Deleted VM On Different Compute
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS2_PORT_1}    ${OVS2_VM1_NAME}    ${OS_CNTL_HOSTNAME}    sg=${SECURITY_GROUP_L2GW}
    @{vm_name}=    Create List    ${OVS2_VM1_NAME}
    Wait Until Keyword Succeeds    180s    15s    L2GatewayOperations.Verify Nova VM IP    ${port_ip_list}    @{vm_name}

...
Suite Setup       BuiltIn.Run Keywords    Basic Suite Setup    Network Suite Setup
...
TC02 Verify Hwvtep Node DB Entries
    ...
    Verify Entries in Hwvtep DB Dump    ${hwvtep_conn_id}    ${NET1_ID}    ${hwvtep_bridge}    ${DEF_CONN_SEG_ID}    ${mac_entries}    ${port_list}
    ${LS_entries}=    Create List    ${NET1_ID}    ${port_mac_list[0]}
    Wait Until Keyword Succeeds    30s    15s    Check For Elements At URI    ${HWVTEP_NETWORK_TOPOLOGY}    ${LS_entries}    session
...
TC05 Verify Flow And Group Entries
    ...
    Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS1} nohup ping -c20 ${port_ip_list[0]} &    ${HWVTEP_IP}
...
TC06 Configure L2gateway Connection to Second Network
    ...
    Log    ${output}
    ${NET2_ID} =    Get Net Id    ${NET_2}
    Set Suite Variable    ${NET2_ID}
    ${mac_entries}=    Create List    ${port_mac_list[1]}    unknown-dst
    ${port_list}=    Create List    ${NS_PORT2}
    Verify Entries in Hwvtep DB Dump    ${hwvtep_conn_id}    ${NET2_ID}    ${hwvtep_bridge}    ${DEF_CONN_SEG_ID}    ${mac_entries}    ${port_list}
...
TC07 Verify Ping Within Second Network and Flows
    ...
    ${output}=    Wait Until Keyword Succeeds    60s    10s    Execute Command on VM Instance    ${NET_2}    ${port_ip_list[1]}
...
TC08 Verify Flows, Groups And Ping From Hwvtep Namespace
    ...
    Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS2} nohup ping -c20 ${port_ip_list[1]} &    ${HWVTEP_IP}
    ${dst_mac_list}=    Create List    ${BM_PORT2_MAC}    ${port_mac_list[1]}
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${dst_mac_list}    ${NET_2_SEGID}    True
...
TC10 Delete L2-gateway-connection with Second Network And Verify
    ...
    ${mac_entries}=    Create List    ${port_mac_list[1]}    unknown-dst
    ${port_list}=    Create List    ${NS_PORT2}
    Verify Entries Not in Hwvtep DB Dump    ${hwvtep_conn_id}    ${NET2_ID}    ${hwvtep_bridge}    ${DEF_CONN_SEG_ID}    ${mac_entries}    ${port_list}
    ${LS_entries}=    Create List    ${NET2_ID}    ${port_mac_list[1]}
    Wait Until Keyword Succeeds    30s    15s    Check For Elements Not At URI    ${HWVTEP_NETWORK_TOPOLOGY}    ${LS_entries}    session
    ${dst_mac_list}=    Create List    ${BM_PORT2_MAC}
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${dst_mac_list}    ${NET_2_SEGID}    False    0
...
TC12 Delete All L2gateway Connections and Verify Ping
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${dst_mac_list}    ${NET_1_SEGID}    False    0
    ${dst_mac_list}=    Create List    ${BM_PORT2_MAC}
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CMP1_CONN_ID}    ${dst_mac_list}    ${NET_2_SEGID}    False    0
...
TC13 Verify Hwvtep DB After Deletion of L2GWs
    ...
    ${mac_entries}=    Create List    ${port_mac_list[0]}    unknown-dst
    ${port_list}=    Create List    ${NS_PORT1}
    Verify Entries Not in Hwvtep DB Dump    ${hwvtep_conn_id}    ${NET1_ID}    ${hwvtep_bridge}    ${DEF_CONN_SEG_ID}    ${mac_entries}    ${port_list}
    ${mac_entries}=    Create List    ${port_mac_list[1]}    unknown-dst
    ${port_list}    Create List    ${NS_PORT2}
    Verify Entries Not in Hwvtep DB Dump    ${hwvtep_conn_id}    ${NET2_ID}    ${hwvtep_bridge}    ${DEF_CONN_SEG_ID}    ${mac_entries}    ${port_list}
    ${LS_entries}=    Create List    ${NET1_ID}    ${port_mac_list[0]}    ${NET2_ID}    ${port_mac_list[1]}
    Wait Until Keyword Succeeds    30s    3s    Check For Elements Not At URI    ${HWVTEP_NETWORK_TOPOLOGY}    ${LS_entries}    session
...
TC14 Disconnect Hwvtep Node And Verify
    ...
    Log    ${output}
    ${output}=    Exec Command    ${hwvtep_conn_id}    sudo vtep-ctl list Manager
    Log    ${output}
...
Basic Suite Setup
    ...
    Hwvtep Suite Setup
    Set Log Level For L2gw    TRACE
    Enable Controller Dhcp

Basic Suite Teardown
    [Documentation]    Basic Suite Teardown required for HWVTEP Test Suite
    Hwvtep Suite Teardown
...
Network Suite Setup
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS_PORT_2}    ${OVS_VM2_NAME}    ${OS_CMP1_HOSTNAME}

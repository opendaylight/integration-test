...
TC05 Verify Flow And Group Entries
    ...
    Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS1} ping -c20 ${port_ip_list[0]} &    ${HWVTEP_IP}
...
TC08 Verify Flows, Groups And Ping From Hwvtep Namespace
    ...
    Start Command In Hwvtep    ${NETNS_EXEC} ${HWVTEP_NS2} ping -c20 ${port_ip_list[1]} &    ${HWVTEP_IP}
    ${dst_mac_list}=    Create List    ${BM_PORT2_MAC}    ${port_mac_list[1]}
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CNTL_CONN_ID}    ${dst_mac_list}    ${NET_2_SEGID}    True
...
TC10 Delete L2-gateway-connection with Second Network And Verify
    ...
    L2GatewayOperations.Verify Flow And Group Entries    ${OS_CNTL_CONN_ID}    ${dst_mac_list}    ${NET_2_SEGID}    False    0
...
TC14 Disconnect Hwvtep Node And Verify
    ...
    Utils.Check For Elements Not At URI    ${HWVTEP_NETWORK_TOPOLOGY}    ${list_to_check}    session
...
Setup Network Configuration
    ...
    ${OS_CNTL_HOSTNAME}     ${NET_1}    ${NET_ADDT_ARG} ${NET_1_SEGID}
    ...
    OpenStackOperations.Create Vm Instance With Port On Compute Node    ${OVS_PORT_2}    ${OVS_VM2_NAME}    ${OS_CNTL_HOSTNAME}
    ...
    @{VM_list}=    Create List    ${OVS_VM1_NAME}    ${OVS_VM1_NAME}
    : FOR    ${VM}    IN    @{VM_list}
    \    ${vm_ip}=    Wait Until Keyword Succeeds    60s    2s    L2GatewayOperations.Verify Nova VM IP    ${VM}
    \    Log    ${vm_ip}
    \    ${vm_ip_string}=    Encode String To Bytes    ${vm_ip}    UTF-8
    \    List Should Contain Value    ${port_ip_list}    ${vm_ip_string}

Cleanup Network Configuration
    [Documentation]    Cleanup of openstack networks and VMs

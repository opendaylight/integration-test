...
TC00 Setup test case
    ...
    Collections.Set To Dictionary    ${NS_PORT_INFO1}    conn_id    ${hwvtep_conn_id_1}
    Collections.Set To Dictionary    ${NS_PORT_INFO2}    conn_id    ${hwvtep_conn_id_1}
    Setup Hwvtep    ${ODL_IP}    ${hwvtep_conn_id_1}    ${HWVTEP_IP}    ${GW_DEV_NAME1}    @{GW_NS_PORTS1}
    # L2GW#1 LogicalSwitch Index
    Set Suite Variable    ${ls_index1}    ${0}

    # Setup L2GW#2 initialize and create PhysicalSwitch
    Collections.Set To Dictionary    ${NS_PORT_INFO3}    conn_id    ${hwvtep_conn_id_2}
    Setup Hwvtep    ${ODL_IP}    ${hwvtep_conn_id_2}    ${HWVTEP2_IP}    ${GW_DEV_NAME2}    @{GW_NS_PORTS2}
    # L2GW#2 LogicalSwitch Index
    Set Suite Variable    ${ls_index2}    ${0}

TC01 GATEWAY Configuration L2GW:1, PS:1, Interface:1
    # Create a VM and connect it to one VLAN network

    # Craete Network/Subnet/VM Instance
    OpenStackOperations.Create Network    ${NET_NAME1}    ${NET_ADDT_ARG} ${NET_SEGID1}
    OpenStackOperations.Create Subnet    ${NET_NAME1}    ${SUBNET_NAME1}    ${SUBNET_RANGE_IP1}    ${SUBNET_ADDT_ARG}
    OpenStackOperations.Create Vm Instance    ${NET_NAME1}    ${VM_NAME1}
    ${vm_ip}=    Wait Until Keyword Succeeds    240s    4s    L2GatewayOperations.Verify Nova VM IP    ${VM_NAME1}
    Log    ${vm_ip}
    ${vm_conn_id}=    OpenStackOperations.Create VM Instance Session    ${NET_NAME1}    ${vm_ip}
    &{VM_INFO}=    Create Dictionary    type=vm    net_name=${NET_NAME1}    ip=${vm_ip}    conn_id=${vm_conn_id}

    # Create L2GW GATEWAY/CONNECTION
    L2GatewayOperations.Create Verify L2Gateway    ${GW_DEV_NAME1}    ${GW_DEV_IF1}    ${GW_NAME1}
    L2GatewayOperations.Create Verify L2Gateway Connection    ${GW_NAME1}    ${NET_NAME1}     --default-segmentation-id ${GW_CONN_SEGID_VID1}
    Set Suite Variable    ${ls_index1}    ${ls_index1+1}
    L2GatewayOperations.Verify L2GW Node    ${hwvtep_conn_id_1}    ${GW_DEV_NAME1}    ${GW_DEV_IF1}    ${ls_index1}    ${NET_SEGID1}   ${GW_CONN_SEGID_VID1}
    Collect All Ovs Information    ${ctrl_conn_id}    ${cn_conn_id}    ${hwvtep_conn_id_1}    ${hwvtep_conn_id_2}

    # Verify
    Verify Traffic    ${VM_INFO}    ${NS_PORT_INFO1}
    Verify Traffic    ${NS_PORT_INFO1}    ${VM_INFO}
...
TC02 GATEWAY Configuration L2GW:1, PS:1, Interface:2
    # Create a VM and connect to two VLAN networks (one L2GW)
    ...
    # Create L2GW GATEWAY/CONNECTION
    L2GatewayOperations.Create Verify L2Gateway    ${GW_DEV_NAME1}    '${GW_DEV_IF1};${GW_DEV_IF2}'    ${GW_NAME1}
    L2GatewayOperations.Create Verify L2Gateway Connection    ${GW_NAME1}    ${NET_NAME1}    --default-segmentation-id ${GW_CONN_SEGID_VID1}
    Set Suite Variable    ${ls_index1}    ${ls_index1+1}
    L2GatewayOperations.Verify L2GW Node    ${hwvtep_conn_id_1}    ${GW_DEV_NAME1}    ${GW_DEV_IF1}    ${ls_index1}    ${NET_SEGID1}   ${GW_CONN_SEGID_VID1}
    L2GatewayOperations.Verify L2GW Node    ${hwvtep_conn_id_1}    ${GW_DEV_NAME1}    ${GW_DEV_IF2}    ${ls_index1}    ${NET_SEGID1}   ${GW_CONN_SEGID_VID1}
    Collect All Ovs Information    ${ctrl_conn_id}    ${cn_conn_id}    ${hwvtep_conn_id_1}    ${hwvtep_conn_id_2}

    # Verify
    Verify Traffic    ${VM_INFO}    ${NS_PORT_INFO1}
    Verify Traffic    ${NS_PORT_INFO1}    ${VM_INFO}
    Verify Traffic    ${VM_INFO}    ${NS_PORT_INFO2}
    Verify Traffic    ${NS_PORT_INFO2}    ${VM_INFO}
...
TC04 GATEWAY Configuration L2GW:2, PS:2, Interface:2
    # Create a VM and connect to two VLAN networks (two L2GWs)
    ...
    # Create L2GW GATEWAY/CONNECTION
    L2GatewayOperations.Create Verify L2Gateway    ${GW_DEV_NAME1}    ${GW_DEV_IF1}    ${GW_NAME1}
    L2GatewayOperations.Create Verify L2Gateway    ${GW_DEV_NAME2}    ${GW_DEV_IF3}    ${GW_NAME2}
    L2GatewayOperations.Create Verify L2Gateway Connection    ${GW_NAME1}    ${NET_NAME1}    --default-segmentation-id ${GW_CONN_SEGID_VID1}
    Set Suite Variable    ${ls_index1}    ${ls_index1+1}
    L2GatewayOperations.Create Verify L2Gateway Connection    ${GW_NAME2}    ${NET_NAME1}    --default-segmentation-id ${GW_CONN_SEGID_VID2}
    Set Suite Variable    ${ls_index2}    ${ls_index2+1}
    L2GatewayOperations.Verify L2GW Node    ${hwvtep_conn_id_1}    ${GW_DEV_NAME1}    ${GW_DEV_IF1}    ${ls_index1}    ${NET_SEGID1}    ${GW_CONN_SEGID_VID1}
    L2GatewayOperations.Verify L2GW Node    ${hwvtep_conn_id_2}    ${GW_DEV_NAME2}    ${GW_DEV_IF3}    ${ls_index2}    ${NET_SEGID1}    ${GW_CONN_SEGID_VID2}
    Collect All Ovs Information    ${ctrl_conn_id}    ${cn_conn_id}    ${hwvtep_conn_id_1}    ${hwvtep_conn_id_2}

    # Verify
    Verify Traffic    ${VM_INFO}    ${NS_PORT_INFO1}
    Verify Traffic    ${NS_PORT_INFO1}    ${VM_INFO}
    Verify Traffic    ${VM_INFO}    ${NS_PORT_INFO3}
    Verify Traffic    ${NS_PORT_INFO3}    ${VM_INFO}
...
Ping From VM Instance
    ...
    ${output}=    OpenStackOperations.Execute Command on VM Instance Session    ${vm_conn_id}
    ...    sudo /bin/sh -c "arp -d ${to_ip};ping -c ${num} ${to_ip} > /tmp/ping.txt 2>&1 &"
    [Return]    ${output}
...
Ping Command Wait From VM Instance
    ...
    \    ${output}=    OpenStackOperations.Execute Command on VM Instance Session    ${vm_conn_id}
    \    ...    sudo /bin/sh -c "ps -eo pid,comm | grep [p]ing | wc -l"
...
Tcp Send VM Instance
    ...
    ${output}=    Wait Until Keyword Succeeds    120s    10s    OpenStackOperations.Execute Command on VM Instance
    ...    ${net_name}    ${from_ip}    sudo echo ${string} | nc -w 1 ${to_ip} 18080
    [Return]    ${output}
...
Tcp Recv VM Instance
    ...
    ${output}=    Wait Until Keyword Succeeds    120s    10s    OpenStackOperations.Execute Command on VM Instance
    ...    ${net_name}    ${ip}    sudo /bin/sh -c "nc -l -p 18080 2>&1 >/tmp/tempfile &"
    [Return]    ${output}
...
Tcp Read VM Instance
    ...
    ${ip}=    Collections.Get From Dictionary    ${to}    ip
    ${output}=    Wait Until Keyword Succeeds    120s    10s    OpenStackOperations.Execute Command on VM Instance
    ...    ${net_name}    ${ip}    sudo /bin/sh -c "killall nc;cat /tmp/tempfile;rm -f /tmp/tempfile"
    Log    ${output}
    [Return]    ${output}
...
Udp Send VM Instance
    ...
    ${output}=    Wait Until Keyword Succeeds    120s    10s    OpenStackOperations.Execute Command on VM Instance
    ...    ${net_name}    ${from_ip}    sudo echo ${string} | nc -u -w 1 ${to_ip} 18081
    [Return]    ${output}
...
Udp Recv VM Instance
    ...
    ${ip}=    Collections.Get From Dictionary    ${to}    ip
    ${output}=    Wait Until Keyword Succeeds    120s    10s    OpenStackOperations.Execute Command on VM Instance
    ...    ${net_name}    ${ip}    sudo /bin/sh -c "nc -u -l -p 18081 2>&1 >/tmp/tempfile &"
    [Return]    ${output}
...
Add Hwvtep Namespace
    ...
    \    L2GatewayOperations.Attach Port To Hwvtep Namespace    ${mac}    ${ns_name}    ${tap_port_name}    ${ip}    ${vlan}    ${conn_id}
...
Setup Hwvtep
    ...
    \    ${ns_name}=    Collections.Get From Dictionary    ${dict}    ns_name
    \    ${vlan}=    Get From Dictionary Present Default    ${dict}    vlan
    \    L2GatewayOperations.Attach Port To Hwvtep Namespace With IP And VLAN    ${mac}    ${ns_name}    ${tap_port_name}    ${ip}    ${vlan}    ${conn_id}

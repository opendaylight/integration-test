...
TC31 Connect Hwvtep Manager For First Tor
    L2GatewayOperations.Add Vtep Manager And Verify    ${hwvtep2_conn_id}    ${ODL_IP}

TC32 Create Network Subnet And Ports For Hwvtep
...
TC34 Create L2Gateway And Connection Between First Hwvtep Namespaces
    ${output}=    L2GatewayOperations.Create Verify L2Gateway    ${HWVTEP_BRIDGE}    "${NS2_PORT1};${NS2_PORT2}"    ${L2GW_NAME2}
    Log    ${output}
...
TC37 Ping Verification In Hwvtep From First Port To Second
    Wait Until Keyword Succeeds    30s    5s    L2GatewayOperations.Verify Ping In Namespace Extra Timeout    ${HWVTEP_NS1}    ${port_mac_list[3]}    ${port_ip_list[4]}
...
TC39 Ovs Manager Addition
    L2GatewayOperations.Add Ovs Bridge Manager Controller And Verify    ${ovs2_conn_id}

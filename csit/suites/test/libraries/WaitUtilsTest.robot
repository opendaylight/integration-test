...
SlACHTSC_Too_Many_Sleeps
    ...
    BuiltIn.Run_Keyword_And_Expect_Error    Not possible to succeed within deadline. Last result: No result yet.    WaitUtils.Stateless_Assert_Closure_Has_To_Succeed_Consecutively    timeout=1.4s    period=0.5s    count=4    assertor=${ScalarClosures__identity}
...
SlACHTSC_Slow_Assertor
    ...
    BuiltIn.Run_Keyword_And_Expect_Error    Not possible to succeed within deadline. Last result: None    WaitUtils.Stateless_Assert_Closure_Has_To_Succeed_Consecutively    timeout=1.4s    period=0.5s    count=3    assertor=${assertor}
...
SfACHTSC_Too_Many_Sleeps
    [Documentation]    There are too many sleeps to meet deadline.
    BuiltIn.Run_Keyword_And_Expect_Error    Not possible to succeed within deadline. Last result: No result yet.    WaitUtils.Stateful_Assert_Closure_Has_To_Succeed_Consecutively    timeout=1.4s    period=0.5s    count=4    assertor=${countdown_quick}
    ...    initial_state=4

SfACHTSC_Slow_Assertor
    [Documentation]    Assertor takes additional time, deadline is encountered.
    BuiltIn.Run_Keyword_And_Expect_Error    Not possible to succeed within deadline. Last result: still_alive    WaitUtils.Stateful_Assert_Closure_Has_To_Succeed_Consecutively    timeout=1.4s    period=0.5s    count=3    assertor=${countdown_slow}
    ...    initial_state=3
...
GASSVHTSCBD_Happy
    ...
    ${date_deadline} =    DateTime.Add_Time_To_Date    ${date_now}    1
...
GASSVHTSCBD_Sleeps_Too_Long
    ...
    BuiltIn.Should_Be_Equal    ${result}    Not possible to succeed within deadline. Last result: No result yet.
...
GASSVHTSCBD_Slow_Getter
    ...
    BuiltIn.Should_Be_Equal    ${state}    1
    BuiltIn.Should_Be_Equal    ${status}    FAIL
    BuiltIn.Should_Be_Equal    ${result}    Not possible to succeed within deadline. Last result: Validated stable: 1

...
Create Key
    ...
    ${add_key}=    Merge_And_Wrap_input    ${eid_json}    ${authkey_json}
...
Attempt To Read Non-Existing Key
    ...
    ${get_key}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.255/32
    Post Log Check    ${LFM_RPC_API}:get-key    ${get_key}    404

Read Key
    [Documentation]    Read an existing key for an IPv4 EID
    ${get_key}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.1/32
...
Update Key
    ...
    ${update_key}=    Merge_And_Wrap_input    ${eid_json}    ${authkey_json}
...
Delete Key
    ...
    ${remove_key}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.1/32
...
Attempt To Read Non-Existing Mapping
    ...
    ${get_mapping}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.255/32
    Check Mapping Removal    ${get_mapping}

Read Mapping
    [Documentation]    Read an existing mapping for an IPv4 EID
    ${get_mapping}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.1/32
...
Delete Mapping
    ...
    ${remove_mapping}=    Get_LispAddress_JSON_And_Wrap_input    ipv4:192.0.2.1/32

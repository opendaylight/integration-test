...
Get nodeconnector for the root node
    ...
    ${TOPO_TREE_FANOUT}    Convert To Integer    ${TOPO_TREE_FANOUT}
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT}/node/openflow:1    expected_status=200
    Log    ${resp.content}
...
Check Every Nodes
    [Arguments]    ${numnodes}
    ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT}    expected_status=200
...
Check Every Nodes Stats
    [Arguments]    ${numnodes}
    FOR    ${IND}    IN RANGE    1    ${numnodes+1}
        ${resp}    RequestsLibrary.GET On Session    session    ${REST_CONTEXT}/node/openflow:${IND}
        ...    expected_status=200
        Log    ${resp.content}

...
...               Verification that running get requests from collections tab using run request button with all checkboxes
...               in history settings selected results in correct displaying of API and form content and status and time values of
...               get requests in history.
...               Verification that unselecting fill form with received data on history select checkbox in history list
...               settings results in correct displaying of API and form content and status and time values of get requests in history.
...               Verification that running get requests from collections tab using run request button with save received data
...               checkbox in history setings unselected results in correct displaying of API and form content and status
...               and time values of get requests in history.
...               Verification that running get requests from collections tab using run request button with save base response
...               data checkbox in history setings unselected results in correct displaying of API and form content and status
...               and time values of get requests in history.
...
Run requests from collections using run request button and verify these requests are present in history and verify API and form content and status and time values 1
    ...
    YangmanKeywords.Compare Indexed History Request Operation Label And Verify Url Label Contains Given Key    GET    ${reversed_keys}
...
Run requests from collections using run request button and verify these requests are present in history and verify API and form content and status and time values 2
    : FOR    ${index}    IN RANGE    0    2
    \    YangmanKeywords.Run Collection Indexed Request Via Run Request Button    ${collection_id_0}    ${index}
    YangmanKeywords.Navigate To History Tab
...
Run requests from collections using run request button and verify these requests are present in history and verify API and form content and status and time values 3
    YangmanKeywords.Select And Expand Collection    0
    : FOR    ${index}    IN RANGE    0    2
    \    YangmanKeywords.Run Collection Indexed Request Via Run Request Button    ${collection_id_0}    ${index}
    YangmanKeywords.Navigate To History Tab
    YangmanKeywords.Verify Number Of History Requests Displayed Equals To Number Given    10
    YangmanKeywords.Compare Indexed History Request Operation Label And Verify Url Label Contains Given Key    GET    ${reversed_keys}
    YangmanKeywords.Verify History Requests With Given Indeces Contain Data In Api And No Data In Form And Do Not Contain Status And Time Data    ${reversed_keys}    ${group_id}    0    1

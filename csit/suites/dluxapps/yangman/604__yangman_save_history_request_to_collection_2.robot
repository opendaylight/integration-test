...
...               using Save to collection button results in the collections being displayed in the Collections tab with correct names.
...
Execute successful Put and Get and Delete operations and save each to new collection using save to collection button and verify collections are displayed in collections tab
...
Verify collection contains all requests that has been saved to it
    ...
    \    ${operation_name}=    BuiltIn.Set Variable    @{operation_names}[${index}]

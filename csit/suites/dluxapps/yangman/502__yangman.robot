...
Verify Operations Presence In Operation Select Menu
    ...
    Selenium2Library.Page Should Contain Element    ${Get_Option}
    Selenium2Library.Page Should Contain Element    ${Put_Option}
    Selenium2Library.Page Should Contain Element    ${Post_Option}
    Selenium2Library.Page Should Contain Element    ${Delete_Option}       
...
Select Each Operation And Verify That Code Mirrors Has Been Displayed Correctly
    ...
    \    Run Keyword If    "${operation_name}"=="${Put_Operation_Name}" or "${operation_name}"=="${Post_Operation_Name}" 
    \    ...    BuiltIn.Run Keywords    
    \    ...    YangmanKeywords.Verify Sent Data CM Is Displayed    AND
    \    ...    YangmanKeywords.Verify Received Data CM Is Displayed
    \    Run Keyword If    "${operation_name}"=="${Get_Operation_Name}" or "${operation_name}"=="${Delete_Operation_Name}"
    \    ...    BuiltIn.Run Keywords    
    \    ...    YangmanKeywords.Verify Sent Data CM Is Not Displayed    AND
    \    ...    YangmanKeywords.Verify Received Data CM Is Displayed

...
...               and saving delete requests from history to collections results in correct displaying of API and form content
...
Verify requests in collections tab have correct operation label and API and form and status and time content
    YangmanKeywords.Compare Collection Indexed Request Operation Label And Verify Url Label Contains Given Key    ${collection_id_0}    DELETE    ${keys}
...
Unselect save base response data checkbox and save requests from history to collections and verify status and time values of new requests are threedots
    ...
    YangmanKeywords.Compare Collection Indexed Request Operation Label And Verify Url Label Contains Given Key    ${collection_id_1}    DELETE    ${keys}

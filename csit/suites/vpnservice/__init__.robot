...
Start Suite
    ...
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns1 ip link set dev tap-port-1 up
    ${resp}     Execute Command    sudo /sbin/ip link set dev tap79ad0001-19 up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns1 ifconfig tap-port-1 20.1.1.2 netmask 255.255.255.0
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns1 ifconfig tap-port-1 hw ether 00:16:3E:19:F7:8B
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns1 ip link set dev lo up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns1 ip route add default via 20.1.1.1
    ...
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns2 ip link set dev tap-port-2 up
    ${resp}     Execute Command    sudo /sbin/ip link set dev tap79ad0002-19 up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns2 ifconfig tap-port-2 20.1.1.3 netmask 255.255.255.0
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns2 ifconfig tap-port-2 hw ether 00:16:3E:BB:9B:0F
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns2 ip link set dev lo up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns2 ip route add default via 20.1.1.1
    ...
    ${resp}    Sleep    3
    Log    ${output}
    ${output}    Execute Command    sudo ifconfig
    ${resp}    Sleep    3
    Log    ${output}
    ${output}    Execute Command    sudo /sbin/ip netns exec ns1 ifconfig
    ${resp}    Sleep    3
    Log    ${output}
    ${output}    Execute Command    sudo /sbin/ip netns exec ns2 ifconfig
    ${resp}    Sleep    3
    Log    ${output}
    ...
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns3 ip link set dev tap-port-3 up
    ${resp}     Execute Command    sudo /sbin/ip link set dev tap79ad0003-19 up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns3 ifconfig tap-port-3 30.1.1.4 netmask 255.255.255.0
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns3 ifconfig tap-port-3 hw ether 00:16:3E:92:F5:F8
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns3 ip link set dev lo up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns3 ip route add default via 30.1.1.1
    ...
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns4 ip link set dev tap-port-4 up
    ${resp}     Execute Command    sudo /sbin/ip link set dev tap79ad0004-19 up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns4 ifconfig tap-port-4 30.1.1.5 netmask 255.255.255.0
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns4 ifconfig tap-port-4 hw ether 00:16:3E:19:57:58
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns4 ip link set dev lo up
    ${resp}     Execute Command    sudo /sbin/ip netns exec ns4 ip route add default via 30.1.1.1
    ...
    ${resp}    Sleep    3
    Log    ${output}
    ${output}    Execute Command    sudo /sbin/ip netns exec ns3 ifconfig
    ${resp}    Sleep    3
    Log    ${output}
    ${output}    Execute Command    sudo /sbin/ip netns exec ns4 ifconfig
    ${resp}    Sleep    3
    Log    ${output}

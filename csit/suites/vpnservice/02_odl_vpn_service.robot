...
Set Json fields
    ...
    Log    ${body}
    ${resp}    OperatingSystem.Create File    ${VPN_CONFIG_DIR}/${json-file}    ${body}
Reset Json fields
    [Arguments]    ${json-file}
    ${body}    OperatingSystem.Get File    ${VPN_CONFIG_DIR}/${json-file}
    ${substr}    Should Match Regexp    ${MININET}    [0-9]\{1,3}\.[0-9]\{1,3}\.[0-9]\{1,3}\.
    ${subnet}    Catenate    SEPARATOR=    ${substr}0    \\    /24
    Log    ${subnet}
    ${body}    replace string    ${body}    "prefix": "${subnet}"    "prefix": "1.1.1.1"
    ${body}    replace string    ${body}    "ip-address": "${MININET}"    "ip-address": "2.2.2.2"
    ${body}    replace string    ${body}    "ip-address": "${MININET1}"    "ip-address": "3.3.3.3"
    Log    ${body}
    ${resp}    OperatingSystem.Create File    ${VPN_CONFIG_DIR}/${json-file}    ${body}
Delete All VxLan Tunnels

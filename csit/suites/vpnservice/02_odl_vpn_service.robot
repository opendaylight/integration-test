...
TC04 Verify neutron subnet creation
    ...
    ${resp}    Post Json    session    ${ODLREST}subnets/    data=${subnet_1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}subnets/    data=${subnet_2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
...
TC06 Verify neutron port creation 
    ...
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${port_1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${port_2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${port_3}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${port_4}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
...
TC08 Verify vpn service creation 
    ...
    ${resp}    Post Json    odlsession    ${REST_CON1}/neutronvpn:createL3VPN/    data=${l3vpn}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    200
...
TC10 Verify Associate network to vpn service and check from DB 
    ...
    ${netid1}    Post Json    odlsession    ${REST_CON1}/neutronvpn:associateNetworks    data=${ass_diss_conf_1}
    Log    ${netid1.content}
    Log    ${netid1.status_code}
    Should Be Equal As Strings    ${netid1.status_code}    ${GET_RESP_CODE}
    ${netid2}    Post Json    odlsession    ${REST_CON1}/neutronvpn:associateNetworks    data=${ass_diss_conf_2}
    Log    ${netid2.content}

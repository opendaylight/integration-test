...
${BRIDGE1}    BR1
${BRIDGE2}    BR2
${ns1ping1}    sudo ip netns exec ns1 ping -c 3 20.0.0.3
${ns1ping2}    sudo ip netns exec ns1 ping -c 3 30.0.0.4
${ns1ping3}    sudo ip netns exec ns1 ping -c 3 30.0.0.5
${ovs_restart}    sudo /etc/init.d/openvswitch-switch restart
${ovs-show}    sudo ovs-vsctl show
${ns-show}    sudo ip netns show
${ns-ifconfig}    sudo ifconfig
...
TC11 Verify end to end data path within same network and from one network to the other 
    ...
    [Tags]    Datapath
    Log    "Getting the OVS config"
    Switch Connection    ${mininet1_conn_id_1}
    Log    ${mininet1_conn_id_1}
    SSHLibrary.Write    ${ovs-show}
    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    Log To Console    ${output}
    Log    ${output}
    SSHLibrary.Write    ${ns-show}
    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    Log To Console    ${output}
    Log    ${output}
    SSHLibrary.Write    ${ns-ifconfig}
    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    Log To Console    ${output}
    Log    ${output}
    ...
    \    SSHLibrary.Write    ${ns1ping1}
    \    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    \    Log To Console    ${output}
    \    Log    ${output}
    \    ${resp}    Sleep    ${DELAY_BEFORE_PING}
    \    SSHLibrary.Write    ${ns1ping2}
    \    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    \    Log To Console    ${output}
    \    Log    ${output}
    \    ${resp}    Sleep    ${DELAY_BEFORE_PING}
    \    SSHLibrary.Write    ${ns1ping3}
    \    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    \    Log To Console    ${output}
    \    Log    ${output}
...
TC12 Verify end to end data path after the restart of OVS 
    [Documentation]   Verify end to end data path within same network and from one network to the other    
    [Tags]    RestartOVS
    Log    "Restarting OVS1 and OVS2"
    Switch Connection    ${mininet1_conn_id_1}
    Log    ${mininet1_conn_id_1}
    SSHLibrary.Write    ${ovs_restart}
    ${output}=    Read Until    ${DEFAULT_LINUX_PROMPT}
    Log To Console    ${output}
    Log    ${output}
    Switch Connection    ${mininet2_conn_id_1}
...
TC15 Verify data path after delete and re-create VPN 
...
TC16 Verify neutron Port Deletion
...
TC17 Verify neutron Subnet Deletion
...
TC18 Verify neutron network Deletion

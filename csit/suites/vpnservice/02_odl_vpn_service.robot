...
TC01 Create and Verify vxlan Tunnel
    ...
    Create VxLan Tunnel
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPR}odl-interface-meta:bridge-ref-info/
    Log    ${resp.content}
    Log To Console    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/itm:transport-zones/
    Log    ${resp.content}
    Log To Console    ${resp.content}
...
TC02 Verify neutron network creation
    ...
    ${resp}    Post Json    session    ${ODLREST}networks/    data=${neutron_test_network_1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}networks/    data=${neutron_test_network_2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
...
TC04 Verify neutron subnet creation
    ...
    ${resp}    Post Json    session    ${ODLREST}subnets/    data=${neutron_test_subnet_1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}subnets/    data=${neutron_test_subnet_2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
...
TC05 Verify fetching available subnet
    ...
    ${resp}    RequestsLibrary.Get    session    ${ODLREST}subnets/
    Log    ${resp.content}
...
TC06 Verify neutron port creation 
    ...
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${neutron_test_port_1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${neutron_test_port_2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${neutron_test_port_3}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
    ${resp}    Post Json    session    ${ODLREST}ports/    data=${neutron_test_port_4}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    201
...
TC07 Verify fetching available ports
    ...
    ${resp}    RequestsLibrary.Get    session    ${ODLREST}ports/
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    ${GET_RESP_CODE}
...
TC08 Verify vpn service creation 
    ...
    ${resp}    Post Json    odlsession    ${REST_CON1}/neutronvpn:createL3VPN/    data=${neutron_test_l3vpn}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    200
...
TC09 Verify/fetch vpn service from DB
    ...
    [Tags]    Get
    ${resp}    Post Json    odlsession    ${REST_CON1}/neutronvpn:getL3VPN/    data=${get_delete_l3vpn}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    ${GET_RESP_CODE}
...
TC10 Verify Associate network to vpn service and check from DB 
    ...
    ${netid1}    Post Json    odlsession    ${REST_CON1}/neutronvpn:associateNetworks    data=${net_ass_diss_conf_1}
    Log    ${netid1.content}
    Log    ${netid1.status_code}
    Should Be Equal As Strings    ${netid1.status_code}    ${GET_RESP_CODE}
    ${netid2}    Post Json    odlsession    ${REST_CON1}/neutronvpn:associateNetworks    data=${net_ass_diss_conf_2}
    Log    ${netid2.content}
    ...
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/l3vpn:vpn-instances/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/odl-l3vpn:vpn-instance-to-vpn-id/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/ietf-interfaces:interfaces/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPER}/ietf-interfaces:interfaces-state/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/l3vpn:vpn-interfaces/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPER}/l3vpn:vpn-interfaces/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPER}/l3nexthop:l3nexthop/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/odl-fib:fibEntries/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPER}/odl-l3vpn:vpn-instance-op-data/    headers=${ACCEPT_XML}
    Log    ${resp.content}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_OPER}/odl-l3vpn:prefix-to-interface/    headers=${ACCEPT_XML}
    Log    ${resp.content}

TC11 Verify Dissociate Network from VPN instance
    [Documentation]   Dissociate Network from VPN instance 
    [Tags]    Dissociate
    ${netid1}    Post Json    odlsession    ${REST_CON1}/neutronvpn:dissociateNetworks    data=${ass_diss}
    Log    ${netid1.content}
    Log    ${netid1.status_code}
    Should Be Equal As Strings    ${netid1.status_code}    ${GET_RESP_CODE}
...
TC12 Verify Deletion of vpn instance
    ...
    ${resp}    Post Json    odlsession    ${REST_CON1}/neutronvpn:deleteL3VPN    data=${get_delete_l3vpn}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    200
...
TC13 Verify neutron Port Deletion
    ...
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}ports/${test_neutron_port1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
    Log To Console    ${test_neutron_port2} 
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}ports/${test_neutron_port2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
    Log To Console    ${test_neutron_port3} 
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}ports/${test_neutron_port3}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}ports/${test_neutron_port4}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
...
TC14 Verify neutron Subnet Deletion
    ...
    [Tags]    Delete
    Log    "Creating the Subnet"
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}subnets/${test_neutron_subnet1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}subnets/${test_neutron_subnet2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
...
TC15 Verify neutron network Deletion
    ...
    [Tags]    Delete
    Log    "Delete Netowrk"
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}networks/${test_neutron_network1}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
    ${resp}    RequestsLibrary.Delete    session    ${ODLREST}networks/${test_neutron_network2}
    Log    ${resp.content}
    Log    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
...
Create VxLan Tunnel
    ...
    ${resp}    RequestsLibrary.Post    odlsession    ${REST_CON}/itm:transport-zones/    data=${body}
    ${res}    Sleep    3
    Log    ${resp.content}
    Log To Console    ${resp.status_code}
    Should Be Equal As Strings    ${resp.status_code}    204
...
Delete All VxLan Tunnels
    ${resp}    RequestsLibrary.delete    odlsession    ${REST_CON}/itm:transport-zones/
    Log    ${resp.content}
    Log To Console    ${resp.content}
    Log To Console    ${resp.status_code}
    ${resp}    RequestsLibrary.Get    odlsession    ${REST_CON}/itm:transport-zones/

*** Settings ***
Documentation     Test Suite for ITM - Internal Transport Manager (vxlan tunnel or GRE tunnel)
...
#    Log    >>>>> Checking Network config Topology <<<<<<
#    ${url-1}=    Set Variable    ${REST_CON}/network-topology:network-topology/
#    ${resp}    Wait Until Keyword Succeeds    40    5s    Get Network Topology with Tunnel    ${Bridge-1}    ${Bridge-2}
#    ...    ${tunnel-1}    ${tunnel-2}    ${url-1}
#    Log    ${resp}
...
#    ${resp_3}    RequestsLibrary.Get    session    ${REST_CON}/itm-state:tunnel-list/internal-tunnel/${Dpn_id_1}/${Dpn_id_2}/    headers=${ACCEPT_XML}
#    Log    ${resp_3.content}
#    Should Be Equal As Strings    ${resp_3.status_code}    404
#    ${resp_4}    RequestsLibrary.Get    session    ${REST_CON}/itm-state:tunnel-list/internal-tunnel/${Dpn_id_2}/${Dpn_id_1}/    headers=${ACCEPT_XML}
...
#    ${tunnel-type}=    Set Variable    type: vxlan
#    Log    >>>>> Checking Network Topology <<<<<<
#    ${url-1}=    Set Variable    ${REST_CON}/network-topology:network-topology/
#    ${resp}    Wait Until Keyword Succeeds    40    5s    Get Network Topology with Tunnel    ${Bridge-1}    ${Bridge-2}
#    ...    ${tunnel-5}    ${tunnel-6}    ${url-1}
#    Log    ${resp}
#    ${resp}    RequestsLibrary.Get    session    ${REST_CON}/itm-state:dpn-endpoints/DPN-TEPs-info/${Dpn_id_1}/    headers=${ACCEPT_XML}
#    Log    ${resp.content}
#    Should Be Equal As Strings    ${resp.status_code}    200
...
Get Tunnel
    [Arguments]    ${src}    ${dst}
    ${resp}    RequestsLibrary.Get    session    ${REST_CON}/itm-state:tunnel-list/internal-tunnel/${src}/${dst}/    headers=${ACCEPT_XML}
    ...
    ${Tunnel}    Get From List    ${resp_array}    3
    Log    ${Tunnel}
    [Return]    ${Tunnel}

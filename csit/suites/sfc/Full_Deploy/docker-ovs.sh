...
d_ovs_vsctl () {
    CONTAINER="$1"
    shift
    docker exec "$CONTAINER" ovs-vsctl --timeout=60 "$@"
}
...
connect_namespace_to_container () {
    ...
    # Create namespace
    if [ -z `ip netns list | grep "$NAMESPACE"` ]; then
         ip netns add "$NAMESPACE"
    fi

    # Create a veth pair in namespace.
    ip netns exec "$NAMESPACE" ip link add "$NAMESPACE_IF" type veth peer \
       name "$CONTAINER_IF"
    ip netns exec "$NAMESPACE" ip link set dev "$NAMESPACE_IF" up

    # Move one side to container namespace.
    ip netns exec "$NAMESPACE" ip link set dev "$CONTAINER_IF" netns "$PID"
    ip netns exec "$PID" ip link set dev "$CONTAINER_IF" up

    # And put it in integration bridge
    d_ovs_vsctl "$CONTAINER" add-port br-int "$CONTAINER_IF"

    if [ -n "$ADDRESS" ]; then
        ip netns exec "$NAMESPACE" ip addr add "$ADDRESS" dev "$NAMESPACE_IF"
    fi

    if [ -n "$MACADDRESS" ]; then
        ip netns exec "$NAMESPACE" ip link set dev "$NAMESPACE_IF" \
           address "$MACADDRESS"
    fi
...
spawn_node () {
    ...
    if [ -z `docker images | awk '/^ovs-docker\s/ {print $1}'` ]; then
        echo "$UTIL: Docker image ovs-docker does not exist, creating..."
        docker build -t ovs-docker .
    fi

    CONTAINER=`docker run -itd --cap-add NET_ADMIN --name=ovs-node-"$NODE" ovs-docker`
    ...
    while [ "$STATUS" != "EXITED" ]; do
       STATUS=`docker exec "$CONTAINER" supervisorctl status configure-ovs |\
          awk '{print $2}'`
    done
    CONTAINER_GW=`docker inspect -f '{{ .NetworkSettings.Gateway }}' "$CONTAINER"`
    ...
    # Create a cintainer bridge as endpoint for all tunnels
...
clean() {
     ...
     for ID in `docker ps -a | awk '/ovs-node-[0-9]+$/ {print $1}'`; do
         docker stop "$ID"
         docker rm "$ID"
     done 

     for NS in `ip netns list | grep ovsns`; do
         ip netns del "$NS"
     done

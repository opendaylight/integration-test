...
Resource          ../../../libraries/SFC/SfcUtils.robot
...
Get Flows In Docker Containers
    ${docker_ps}=    DockerSfc.Get Docker Ids Formatted    "{{.Names}}"
    ${docker_name_list}=    Split String    ${docker_ps}    \n
    log    ${docker_name_list}
    ${docker_flows}    DockerSfc.Multiple Docker Exec    ${docker_name_list}    ovs-ofctl dump-flows -OOpenflow13 br-int    OFPST_FLOW
    [Return]    ${docker_flows}
...
Init Suite
    ...
    ${docker_cidr}=    Set Variable    10.250.0.0/24
    ${route_to_docker_net}=    Set Variable    sudo route add -net ${docker_cidr} gw ${TOOLS_SYSTEM_IP}
    ...
    ${docker_net}=    SfcUtils.Get Network From CIDR    ${docker_cidr}
    ${docker_ip_octets}=    SfcUtils.Get Ip Address First Octets    ${docker_net}    3
    Run Command On Remote System    ${ODL_SYSTEM_IP}    sed -i 's/${DOCKER_DEFAULT_SUBNET}/${docker_ip_octets}/g' ${CURDIR}/../../../variables/sfc/${VERSION_DIR}/full-deploy/*    ${ODL_SYSTEM_USER}    prompt=${DEFAULT_LINUX_PROMPT}

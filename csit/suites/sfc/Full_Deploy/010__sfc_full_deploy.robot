...
Init Suite
    ...
    ${stderr}    SSHLibrary.Execute Command    echo ${TOOLS_SYSTEM_PASSWORD}|sudo -S ./setup-docker-image.sh    return_stderr=True    return_stdout=True    return_rc=True
    log    ${stderr}
    ${stderr}    SSHLibrary.Execute Command    echo ${TOOLS_SYSTEM_PASSWORD}|sudo -S ./docker-ovs.sh spawn --nodes=6 --guests=1 --tun=vxlan-gpe --odl=${ODL_SYSTEM_IP}    return_stderr=True    return_stdout=True    return_rc=True
    log    ${stderr}

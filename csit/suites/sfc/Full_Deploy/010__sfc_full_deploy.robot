...
Apply ACLs and Classifiers
    [Documentation]    Apply json file descriptions of ACLs and Classifiers
    ${result}    SSHLibrary.Execute Command    ./setup-ifaces.sh --nodes=6 > >(tee myFile3.log) 2> >(tee myFile3.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
...
Cleanup Suite
    ...
    ${result}    SSHLibrary.Execute Command    ./docker-ovs.sh clean > >(tee myFile3.log) 2> >(tee myFile3.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

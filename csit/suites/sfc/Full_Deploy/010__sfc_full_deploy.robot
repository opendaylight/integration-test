...
Init Suite
    ...
    SSHLibrary.Put File    ${CURDIR}/supervisord.conf    .    mode=0755
    SSHLibrary.Put File    ${CURDIR}/setup-docker-image.sh    .    mode=0755
    ${result}    SSHLibrary.Execute Command    echo ${TOOLS_SYSTEM_PASSWORD}|sudo -S ./setup-docker-image.sh    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    echo ${TOOLS_SYSTEM_PASSWORD}|sudo -S ./docker-ovs.sh spawn --nodes=6 --guests=1 --tun=vxlan-gpe --odl=${ODL_SYSTEM_IP}    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

...
Switch Ips In Json Files
    ...
    \    ${normalized_file}=    OperatingSystem.Normalize Path    ${file}
    \    OperatingSystem.Run    sudo sed -i 's/${ip_old}/${ip_new}/g' ${normalized_file}
...
Init Suite
    ...
    SSHLibrary.Put File    ${CURDIR}/../scripts/docker-ovs.sh    .    mode=0755
    SSHLibrary.Put File    ${CURDIR}/../scripts/Dockerfile    .    mode=0755
    SSHLibrary.Put File    ${CURDIR}/../scripts/setup-docker-image.sh    .    mode=0755
    ...
    ${docker_ip_octets}=    SfcUtils.Get Ip Address First Octets    ${docker_nw}    3
    ...
    ${config_files}=    Create List    ${SERVICE_FUNCTIONS_FILE}    ${SERVICE_FORWARDERS_FILE}    ${SERVICE_FORWARDERS_FILE}    ${SERVICE_NODES_FILE}    ${SERVICE_CHAINS_FILE}
    ...    ${SERVICE_FUNCTION_PATHS_FILE}    ${SERVICE_METADATA_FILE}    ${SERVICE_FUNCTION_ACLS_FILE}    ${SERVICE_CLASSIFIERS_FILE}
    Switch Ips In Json Files    ${DOCKER_DEFAULT_SUBNET}    ${docker_ip_octets}    ${config_files}

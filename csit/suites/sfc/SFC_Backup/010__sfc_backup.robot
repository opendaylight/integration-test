...
Kill Karaf And Backup Karaf Data
    ...
    Active Wait For SFC Features    @{testsuite_features}
...
Init Suite
    ...
    Set Suite Variable    @{testsuite_features}    sfc-netconf    sfc-ovs    sfc-scf-openflow    sfcofl2    sfclisp
    ...    sfc-sb-rest    sfc-ui    sfc-provider    sfc-openflow

Active Wait For SFC Features
    [Arguments]    @{FEATURES_LIST}
    [Documentation]    Wait for a list of features till they are Active in karaf
    ${verification_string}=    Create List    Active
    : FOR    ${feature}    IN    @{FEATURES_LIST}
    \    BuiltIn.Wait Until Keyword Succeeds    60s    2s    KarafKeywords.Check For Elements On Karaf Command Output Message    list | grep ${feature}    ${verification_string}
...
Validate Persistency
    ...
    ...    "service-function-forwarder-ovs:ovs-options"    "dst-port":"6633"    "key":"flow"    "nshc1":"flow"    "nshc2":"flow"    "nshc3":"flow"
    ...    "nshc4":"flow"    "nsi":"flow"    "nsp":"flow"    "remote-ip":"flow"
    ...
    ${elements}=    Create List    "service-function-chains"    "service-function-chain"    "name":"SFC1"    "sfc-service-function"    "name":"dpi-abstract1"
    ...    "type":"dpi"
...
Cleanup Suite
    [Documentation]    Clean up SFC elements and delete sessions
    Remove SFC Elements
    Delete All Sessions

...
Create and Get Service Functions
    ...
    ${jsonfile}    OperatingSystem.Get File    ${SERVICE_FUNCTIONS_FILE}
    Add Elements To URI From File    ${SERVICE_FUNCTIONS_URI}    ${SERVICE_FUNCTIONS_FILE}    ${HEADERS_YANG_JSON}
    Wait Until Keyword Succeeds    6s    1s    ClusterManagement.Check_Json_Member_List_Or_All    uri=${SERVICE_FUNCTIONS_URI}    expected_data=${jsonfile}
...
Verify if the SFC features are installed on each node
    ...
    \    # Ensure there is a ssh session created per node. Otherwise, it becomes unstable
    \    ${member_ip} =    BuiltIn.Set_Variable    ${ODL_SYSTEM_${index}_IP}
    \    Verify If SFC Features Are Loaded    ${member_ip}
    [Teardown]    NONE
...
Init Suite
    ...
    ${status} =    Run Command On Remote System    ${ODL_SYSTEM_1_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${WORKSPACE}/${BUNDLEFOLDER}/bin/status
    ${status} =    Run Command On Remote System    ${ODL_SYSTEM_2_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${WORKSPACE}/${BUNDLEFOLDER}/bin/status
    ${status} =    Run Command On Remote System    ${ODL_SYSTEM_3_IP}    /usr/bin/env JAVA_HOME=${JAVA_HOME} ${WORKSPACE}/${BUNDLEFOLDER}/bin/status
...
Verify If SFC Features Are Loaded
    [Arguments]    ${member_ip}

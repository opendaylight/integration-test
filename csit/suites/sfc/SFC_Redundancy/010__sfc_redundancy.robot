...
Kill leader and check followers status
    ...
    ClusterManagement.Kill_Single_Member    ${leader}
    Verify Shards Status
    ClusterManagement.Start_Single_Member    ${leader}
    Verify Shards Status
...
Complete configuration after failover
    ...
    ClusterManagement.Kill_Single_Member    ${leader}
    Verify Shards Status
    ${second_leader}    ${followers}    Get InventoryConfig Shard Status
    Create Session In Controller    ${second_leader}
    Post Elements To URI As JSON    ${OPERATIONS_CREATE_RSP_URI}    ${CREATE_RSP1_INPUT}
    ${session_sec_leader} =    Resolve_Http_Session_For_Member    ${session}
    ${uri} =    BuiltIn.Set_Variable    /restconf/operational/rendered-service-path:rendered-service-paths/
    ${data_text} =    TemplatedRequests.Get_As_Json_From_Uri    uri=${uri}    session=${session_sec_leader}
    ClusterManagement.Start_Single_Member    ${leader}
    Verify Shards Status
...
Verify Shards Status
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    2s    ClusterManagement.Verify_Leader_Exists_For_Each_Shard    shard_name_list=${SHARD_OPER_LIST}    shard_type=operational    member_index_list=${controller_index_list}
    BuiltIn.Wait_Until_Keyword_Succeeds    30s    2s    ClusterManagement.Verify_Leader_Exists_For_Each_Shard    shard_name_list=${SHARD_CONF_LIST}    shard_type=config    member_index_list=${controller_index_list}

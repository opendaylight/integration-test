...
Add SFC Elements and restart cluster
    ...
    Start_Members_From_List_Or_All    wait_for_sync=False
    : FOR    ${index}    IN RANGE    1    ${NUM_ODL_SYSTEM}+1
    \    ${session} =    Resolve_Http_Session_For_Member    member_index=${index}
    \    Wait until Keyword succeeds    10min    5 sec    TemplatedRequests.Get_As_Json_Templated    session=${session}    folder=${RESTCONF_MODULES_DIR}
    \    ...    verify=False

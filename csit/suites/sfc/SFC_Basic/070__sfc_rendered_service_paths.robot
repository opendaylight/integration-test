...
Basic Environment Setup Tests
    ...
    Wait Until Keyword Succeeds    60s    2s    Check Rendered Service Path Created    SFC1-100
...
Get Rendered Service Path By Name
    ...
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp_name}
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    # The RSP should not be symetric, so only 1 should be created for the SFP
    Check For Specific Number Of Elements At URI  ${SERVICE_FUNCTION_PATHS_STATE_URI}service-function-path-state/${sfp_name}    "sfp-rendered-service-path"    1
    ${elements}=    Create List    "parent-service-function-path":"SFC1-100"    "hop-number":0    "service-index":255
...
Get Rendered Service Path Hop
    ...
    ${sfp_name} =    Set Variable    "SFC1-100"
    ${rsp_name}    Get Rendered Service Path Name    ${sfp_name}
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp_name}/rendered-service-path-hop/0/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ...
    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp_name}/rendered-service-path-hop/2/    ${elements}
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp_name}/rendered-service-path-hop/3/
...
Delete one Rendered Service Path By Name
    ...
    ${resp}    RequestsLibrary.Delete Request    session    ${SERVICE_FUNCTION_PATHS_URI}/service-function-path/${sfp_name}
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
...
Generate RSPs with Random Schedule Algorithm type
    ...
    ${resp}    RequestsLibrary.Delete Request    session    ${SERVICE_FUNCTION_PATHS_URI}
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ...
    Wait Until Keyword Succeeds    60s    2s    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}    ${created_rsps}
...
Generate RSPs with Round Robin Schedule Algorithm type
    ...
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    Wait Until Keyword Succeeds    60s    2s    Check Empty Service Function Paths State
    # Create the SFPs which will create the RSPs with the Round Robin scheduler
    Add Elements To URI From File    ${SERVICE_FUNCTION_PATHS_URI}    ${SERVICE_FUNCTION_PATHS_FILE}
    ${created_rsps} =    Create List    "SFC1-100"    "SFC1-200"    "SFC1-300"    "SFC2-100"    "SFC1-200"
    Wait Until Keyword Succeeds    60s    2s    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}    ${created_rsps}
    ...
    ${path1_hop0}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    ${path1_hop1}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    ${path1_hop2}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
    ${path2_hop0}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    ${path2_hop1}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    ${path2_hop2}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
    ${path3_hop0}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    ${path3_hop1}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    ${path3_hop2}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
    ${path4_hop0}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    ${path4_hop1}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    ${path4_hop2}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
    ${path5_hop0}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    ${path5_hop1}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    ${path5_hop2}    Get JSON Elements From URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
...
Generate RSPs with Shortest Path Schedule Algorithm type
    ...
    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/    ${elements}
    ${elements}=    Create List    "hop-number":1    "service-index":254    "service-function-name":"napt44
    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/    ${elements}
    ${elements}=    Create List    "hop-number":2    "service-index":253    "service-function-name":"firewall
    Check For Elements At URI    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/    ${elements}
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/0/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${fwd_hop1}    Extract Value From Content    ${resp.content}    /rendered-service-path-hop/0/service-function-forwarder
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/1/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${fwd_hop2}    Extract Value From Content    ${resp.content}    /rendered-service-path-hop/0/service-function-forwarder
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp1_name}/rendered-service-path-hop/2/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ...
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp2_name}/rendered-service-path-hop/0/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${fwd_hop1}    Extract Value From Content    ${resp.content}    /rendered-service-path-hop/0/service-function-forwarder
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp2_name}/rendered-service-path-hop/1/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${fwd_hop2}    Extract Value From Content    ${resp.content}    /rendered-service-path-hop/0/service-function-forwarder
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_RSPS_URI}rendered-service-path/${rsp2_name}/rendered-service-path-hop/2/
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${fwd_hop3}    Extract Value From Content    ${resp.content}    /rendered-service-path-hop/0/service-function-forwarder
    Should Be Equal    ${fwd_hop1}    ${fwd_hop2}
    Should Be Equal    ${fwd_hop2}    ${fwd_hop3}

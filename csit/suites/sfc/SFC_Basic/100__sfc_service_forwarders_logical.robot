...
Get one Service Function Forwarder
    ...
    Check For Elements At URI    ${SERVICE_FORWARDERS_URI}service-function-forwarder/sfflogical1    ${elements}

Delete A Service Function Forwarder
    [Documentation]    Delete A Service Function Forwarder. Logical SFF
    [Tags]    include
    Add Elements To URI From File    ${SERVICE_FORWARDERS_URI}    ${SERVICE_FORWARDERS_FILE}
    ${resp}    RequestsLibrary.Get Request    session    ${SERVICE_FORWARDERS_URI}service-function-forwarder/sfflogical1
    Should Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    Remove All Elements At URI    ${SERVICE_FORWARDERS_URI}service-function-forwarder/sfflogical1
    ${resp}    RequestsLibrary.Get Request    session    ${SERVICE_FORWARDERS_URI}service-function-forwarder/sfflogical1
    Should Be Equal As Strings    ${resp.status_code}    404
    ${resp}    RequestsLibrary.Get Request    session    ${SERVICE_FORWARDERS_URI}
...
Init Suite
    ...
    Set Suite Variable    ${SFF_SFFLOG_URI}    /restconf/config/service-function-forwarder:service-function-forwarders/service-function-forwarder/sfflogical1/
    Set Suite Variable    ${SFF_SFFLOG_FILE}    ${CURDIR}/../../../variables/sfc/${VERSION_DIR}/service-function-forwarder-logicallsff1.json

...
Test Traffic in One Service Function Chain with VM Migration
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/sf_hhe/logical_sff;sudo ./test_case_02_move_vm_different_subnet.sh \ -o ${ODL_SYSTEM_IP} > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0
...
Init Suite
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    modinfo openvswitch > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}

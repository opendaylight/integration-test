...
#ip netns exec dovs-napt44 python3 ./sf_hhe.py -i eth0 --name napt44 --port 8000 > napt44_sf_hhe.txt &
...
#ip netns exec dovs-server1 python ./HttpServer.py > dovs-server1.txt &
...
#ip netns exec dovs-server1 tcpdump -i eth0 -wU client1_tcpdump.pcap
...
#ip netns exec dovs-client1 python3 ./HttpClient.py -ip ${server_ip}
...
#server_ip=$(ip netns exec dovs-${server_node} ip addr sh | grep -A 2 ": eth0" | tail -n 1 | grep -o -P '(?<=inet ).*(?=/)')
#ip netns exec dovs-client1 ping -c 1 ${server_ip}
...
#ps -ef | grep "sf_hhe.py" | grep -v grep | awk '{print $2}' | xargs kill
def killProcess(cmd):
...
def execAllCmds(args_dir, chains, url_odl, execute_traffic):
    ...
    print ("To kill all in the namespaces:  ip netns list | grep dovs | xargs -l1 ip netns pids | xargs -l1 kill")
...
Example:
./%(prog)s --odl 172.28.128.3 -d ../sf_hhe """ + args_chain_example + " -rsnt",
        add_help=True)

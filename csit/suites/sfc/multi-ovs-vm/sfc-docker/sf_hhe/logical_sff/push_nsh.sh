...
function push_nsh {
    ...
    client_docker_ping_server_docker="docker exec "${client_node}" ping -c 3 172.19.0.5"
    echo "client_docker_ping_server_docker("${client_docker_ping_server_docker}")"
    eval $client_docker_ping_server_docker

    server_docker_netstat="docker exec "${server_node}" netstat -an"
    echo "server_docker_netstat("${server_docker_netstat}")"
    eval $server_docker_netstat

    server_cat_supervisor_log="docker exec "${server_node}" cat /var/log/supervisor/supervisord.log"
    echo "server_cat_supervisor_log("${server_cat_supervisor_log}")"
    eval $server_cat_supervisor_log

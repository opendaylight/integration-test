...
Init Suite
    ...
    SSHLibrary.Put File    ${CURDIR}/sfc-docker/*    .    mode=0755
    ${result}    SSHLibrary.Execute Command    sudo sfc-docker/provision/setup_ovs.sh > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo sfc-docker/provision/setup_ovs_docker.sh > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo sfc-docker/provision/setup_dovs_bridge.sh > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo sfc-docker/provision/setup_dovs.sh > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}

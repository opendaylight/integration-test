...
Init Suite
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/dovs/;sudo ./dovs.py -d spawn --nodes=6 --guests=1 --odl=${ODL_SYSTEM_IP}    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

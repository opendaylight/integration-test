...
Init Suite
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/provision/;sudo ./setup_dovs_network.sh > myFile.log 2> myFile2.log    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/provision/;cat myFile.log    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/provision/;cat myFile2.log    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo systemctl status network.service    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo journalctl -xe    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo cat /etc/sysconfig/network-scripts/ifcfg-dovsbr0    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    ${result}    SSHLibrary.Execute Command    sudo cat /etc/sysconfig/network-scripts/ifcfg-eth1    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/provision/;sudo ./setup_dovs_dhcp.sh > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    DockerSfc.Docker Ovs Start    nodes=6    guests=1    nets=1    odl_ip=${ODL_SYSTEM_IP}

...
Create and Get Classifiers
    ...
    ${tap2}=    Get Docker Ovs Tap    "dovs-node-5"
    ${content_clas}=    OperatingSystem.Get File    ${SERVICE_CLASSIFIERS_FILE}
    ${content_clas}=    Replace String    ${content_clas}    "v-ovsnsn6g1"    "${tap1}"
...
Init Suite
    ...
    log    ${result}
    Should be equal as integers    ${result[2]}    0
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/dovs/;sudo ./dovs.py -d spawn --nodes=6 --guests=1 --odl=${ODL_SYSTEM_IP} > >(tee ${log_file}) 2> >(tee ${log_file})    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

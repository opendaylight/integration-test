...
Test Traffic in two Service Function Chains
    ...
    [Timeout]    15 minutes
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/sf_hhe/logical_sff;sudo ./test_case_03_traffic_different_subnet.sh -o ${ODL_SYSTEM_IP} > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

Test Traffic in One Service Function Chain with VM Migration
    [Documentation]    Test Traffic involving one Service Function Chain and VM migration
    [Timeout]    15 minutes
    ${result}    SSHLibrary.Execute Command    cd sfc-docker/sf_hhe/logical_sff;sudo ./test_case_04_move_vm_different_subnet.sh \ -o ${ODL_SYSTEM_IP} > >(tee myFile.log) 2> >(tee myFile.log)    return_stderr=True    return_stdout=True    return_rc=True
    log    ${result}
    Should be equal as integers    ${result[2]}    0

...
Start_Data_Change_Event_Listener
    ...
    #Log_Response    ${resp}
    SSHLibrary.Write    curl -v -X GET -u admin:admin http://${ODL_SYSTEM_IP}:${RESTCONFPORT}/${RFC8040_GET_SSE_URI} -H Content-Type: text/event-stream > sse.log
    Sleep    15s
    Log File    sse.log
...
Check_Create_Notification
    ...
    [Tags]    critical
    ${notification} =    OperatingSystem.Get File    sse.log
...
Delete_Data_Change_Notification_And_Check
    ...
    Sleep    15s
    ${sse_log}=    OperatingSystem.Get File    sse.log

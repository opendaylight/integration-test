...
Create_DCN_Stream
    ...
    ${uuid} =    XML.Get_Element_Text    ${resp.content}    stream-name
...
Subscribe_To_DCN_Stream
    ...
    Log_Response    ${resp}
    BuiltIn.Should_Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${location1} =    XML.Get_Element_Text    ${resp.content}    access[2]/location
    ${location2} =    XML.Get_Element_Text    ${resp.content}
    ${STREAM_LOCATION} =    CompareStream.Set_Variable_If_At_Least_Calcium
    ...    ${location1}    ${location2}
    BuiltIn.Log    ${STREAM_LOCATION}
    BuiltIn.Set_Suite_Variable    ${STREAM_LOCATION}
    BuiltIn.Should_Not_Be_Empty    ${STREAM_LOCATION}

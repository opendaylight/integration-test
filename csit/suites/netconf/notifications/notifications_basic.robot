...
Check_Subscribtion
    [Documentation]    Subscribe for notifications.
    [Tags]    critical
    ${resp}=    RequestsLibrary.Get_Request    restconf    ${RESTCONF_GET_SUBSCRIPTION_URI}}    headers=${ACCEPT_XML}
    Log    ${resp.content}
    Log    ${resp.headers}
...
Change_Config
    ...
    Log    ${body}
    ${resp}=    RequestsLibrary.Put_Request    restconf    ${RESTCONF_CONFIG_DATA}    data=${body}
    Should Be Equal As Strings    ${resp.status_code}    200

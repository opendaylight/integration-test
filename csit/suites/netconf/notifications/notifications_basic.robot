...
Check_Notification_Stream
    ...
    Log_Response    ${resp}
    BuiltIn.Should_Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${name} =    XML.Get Element Text    ${resp.content}    streams/0/stream/name
...
Check_Subscribtion
    ...
    Log_Response    ${resp}
    BuiltIn.Should_Contain    ${ALLOWED_STATUS_CODES}    ${resp.status_code}
    ${location_in_header}=    Utils.Extract Value From Content    ${resp.content}    /Location    strip
    ${location} =    XML.Get Element Text    ${resp.content}
    BuiltIn.Log    ${location}
    Should Contain    ${location}    ${location_in_header}

...
Create_Device_Data
    ...
    NetconfViaRestconf.Post_Xml_Template_Folder_Via_Restconf    ${DIRECTORY_WITH_CRUD_TEMPLATES}${/}dataorig    ${template_as_string}
...
Deploy_And_Run_RestPerfClient
    ...
    ${options}=    BuiltIn.Set_Variable    ${options} --destination /restconf/config/network-topology:network-topology/topology/topology-netconf/node/${DEVICE_NAME}/yang-ext:mount/test:cont
    ${options}=    BuiltIn.Set_Variable    ${options} --edit-content ${CURDIR}/../../../variables/netconf/RestPerfClient/request1.json

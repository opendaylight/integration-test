...
Run_RestPerfClient_Directly_On_MDSAL
    [Timeout]    ${DIRECT_MDSAL_TIMEOUT_FOR_TESTCASE} s
    ...
    BuiltIn.Set_Suite_Variable    ${command_prefix}    java -Xmx1G -XX:MaxPermSize=256M -jar ${filename} ${options}
...
Configure_ODL_As_A_Device_On_Netconf
    [Documentation]    Configure ODL as a Netconf device on a Netconf connector.
    NetconfKeywords.Configure_Device_In_Netconf    odl-via-netconf-connector    device_address=${ODL_SYSTEM_IP}    device_port=${ODL_NETCONF_MDSAL_PORT}    device_user=${ODL_NETCONF_USER}    device_password=${ODL_NETCONF_PASSWORD}
    NetconfKeywords.Wait_Device_Connected    odl-via-netconf-connector

Run_RestPerfClient_Through_Netconf_Connector
    [Timeout]    ${NETCONF_CONNECTOR_MDSAL_TIMEOUT_FOR_TESTCASE} s
    ...
    ${command}    BuiltIn.Set_Variable    ${command_prefix} /restconf/config/network-topology:network-topology/topology/topology-netconf/node/odl-via-netconf-connector/yang-ext:mount/car:cars
    BuiltIn.Log    Running restperfclient: ${command}
    Set_Known_Bug_Id    5413
    Execute_Command_Passes    ${command} >${restperfclientlog} 2>&1
    Set_Unknown_Bug_Id
...
Deconfigure_ODL_From_Netconf
    [Documentation]    Deconfigure the ODL attached to a Netconf connector.
    [Setup]    SetupUtils.Setup_Test_With_Logging_And_Without_Fast_Failing
    NetconfKeywords.Remove_Device_From_Netconf    odl-via-netconf-connector
...
Setup_Everything
    ...
    BuiltIn.Set_Suite_Variable    ${DIRECT_MDSAL_TIMEOUT}    ${value}
    ${value}=    BuiltIn.Evaluate    ${DIRECT_MDSAL_TIMEOUT}+60
    BuiltIn.Set_Suite_Variable    ${DIRECT_MDSAL_TIMEOUT_FOR_TESTCASE}    ${value}
    ${value}=    BuiltIn.Evaluate    ${REQUEST_COUNT}/10+10
    BuiltIn.Set_Suite_Variable    ${NETCONF_CONNECTOR_MDSAL_TIMEOUT}    ${value}
    ${value}=    BuiltIn.Evaluate    ${NETCONF_CONNECTOR_MDSAL_TIMEOUT}+60
    BuiltIn.Set_Suite_Variable    ${NETCONF_CONNECTOR_MDSAL_TIMEOUT_FOR_TESTCASE}    ${value}

...
Run_RestPerfClient_Directly_On_MDSAL
    ...
    ${timeout}=    DateTime.Convert_Time    ${DIRECT_MDSAL_TIMEOUT}    result_format=number
...
Run_RestPerfClient_Through_Netconf_Connector
    ...
    ${timeout}=    DateTime.Convert_Time    ${NETCONF_CONNECTOR_MDSAL_TIMEOUT}    result_format=number
    ${command}    BuiltIn.Set_Variable    ${command} --timeout ${timeout}
    BuiltIn.Log    Running restperfclient: ${command}
    Set_Known_Bug_Id    5413
    Execute_Command_Passes    ${command} >${restperfclientlog} 2>&1

...
...               connectivity with the device and see if they work. Then restore the
...               connectivity to the device and check that the affected node sees the operations
...               that were made while its connection to the device was down.
...
...
...               node 1 has no device connection, "Update" while node 2 has no device
...               connection and "Delete" while node 3 has no device connection.
...
Cut_node1_From_Device
    [Documentation]    Simulate node 1 loses connectivity to the device just before device data is created, fail if node 1 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_1_IP}
...
Reconnect_node1_To_Device
    [Documentation]    Simulate node 1 regains connectivity to the device just after device data is created and the change propagated in the cluster, fail if node 1 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_1_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node1
...
Cut_node2_From_Device
    [Documentation]    Simulate node 2 loses connectivity to the device just before device data is modified, fail if node 2 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_2_IP}
...
Reconnect_node2_To_Device
    [Documentation]    Simulate node 2 regains connectivity to the device just after device data is modified and the change propagated in the cluster, fail if node 2 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_2_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node2
...
Cut_node3_From_Device
    [Documentation]    Simulate node 3 loses connectivity to the device just before device data is deleted, fail if node 3 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_3_IP}
...
Reconnect_node3_To_Device
    [Documentation]    Simulate node 3 regains connectivity to the device just after device data is deleted and the change propagated in the cluster, fail if node 3 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${TOOLS_SYSTEM_IP}    ${ODL_SYSTEM_3_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node3
...
*** Keywords ***
FIXME__Remove_Code_Triplication
    # This code is also in "node_outages.robot" and "odl_link_outages.robot"

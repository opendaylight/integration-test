...
Modify_Device_Data
    ...
    NetconfViaRestconf.Activate_NVR_Session    ${NODE_SETTER}
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
    ${status}    ${reply}=    NetconfViaRestconf.Put_Xml_Template_Folder_Via_Restconf    ${directory_with_template_folders}${/}datamod1    ${template_as_string}    return_reply=True
    SetupUtils.Set_Known_Bug_Id    5649
    Set_Suite_Variable    ${bug_5649_absent}    False
    BuiltIn.Should_Not_Contain    ${reply}    NullPointerException
    Set_Suite_Variable    ${bug_5649_absent}    True

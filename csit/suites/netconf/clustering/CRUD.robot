...
Wait_For_Device_To_Become_Visible_For_Configurer
    [Documentation]    Wait until the device becomes visible on congfigurer node.
    NetconfViaRestconf.Activate_NVR_Session    ${NODE_CONFIGURER}
...
Setup_Everything
    ...
    # FIXME: Need to take a look into why this is needed (probably a bug in the suite).
...
Check_Config_Data
    ...
    ${data}=    Utils.Get_Data_From_URI    ${node}    ${url}    headers=${ACCEPT_XML}

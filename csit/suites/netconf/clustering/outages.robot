...
...               This is repeated once for every operation and every node in the cluster.
...
Create_Device_Data_With_node1_Down
    [Documentation]    Send some sample test data into the device and check that the request was accepted.
...
Check_New_Device_Data_Is_Visible_On_Nodes_Without_node1
    [Documentation]    Check that the communication in the cluster works even when node 1 is down
...
Check_Modified_Device_Data_Is_Visible_On_Nodes_Without_node1
    [Documentation]    Get the device data on nodes 2 and 3 and make sure it contains the modified content.
...
Check_Device_Data_Deletion_Is_Visible_On_node1
...
Check_Device_Data_Deletion_Is_Visible_On_Nodes_Without_node2
    [Documentation]    Get the device data and make sure it is empty again.
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${empty_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node3    ${empty_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762
...
Check_Modified_Device_Data_Is_Visible_On_Nodes_Without_node3
    [Documentation]    Get the device data on nodes 1 and 2 and make sure it contains the modified content.
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${modified_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node2    ${modified_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762
...
Check_Device_Data_Deletion_Is_Visible_On_node3

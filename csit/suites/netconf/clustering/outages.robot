...
...               the operations that wewe made while it was down are visible on it as well.
...
...
...
...               The "Create", "Update" and "Delete" cycle is done 3 times and at each time
...               a different node is being brought down and later back up. This is intended to
...               shake out bugs that might appear when all of the nodes in the entire cluster
...
Kill_node1_Before_Modify
    [Documentation]    Simulate node 1 crashes just before device data is modified, fail if node 1 survives.
    ClusterManagement.Kill_Single_Member    1

Modify_Device_Data_With_node1_Down
    [Documentation]    Check that the modification requests work when node 1 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
    TemplatedRequests.Put_As_Xml_Templated    ${directory_with_template_folders}${/}datamod1    ${template_as_string}    session=node2
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4968

Check_Modified_Device_Data_Is_Visible_On_Nodes_Without_node1
    [Documentation]    Check that the device data modification is propagated in the cluster even when node 1 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node2    ${modified_data}
...
Restart_node1_After_Modify_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 1 restarted by admin just after device data is modified and the change propagated in the cluster, fail if node 1 fails to boot.
    ClusterManagement.Start_Single_Member    1
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node1

Check_Modified_Device_Data_Is_Visible_On_node1
    [Documentation]    Check that the device data modification is propagated to node 1 as well.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_BOOT_TIMEOUT}    1s    Check_Config_Data    node1    ${modified_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5761

Kill_node1_Before_Delete
    [Documentation]    Simulate node 1 crashes just before device data is deleted, fail if node 1 survives.
    ClusterManagement.Kill_Single_Member    1

Delete_Device_Data_With_node1_Down
    [Documentation]    Check that the data removal requests work when node 1 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
    TemplatedRequests.Delete_Templated    ${directory_with_template_folders}${/}datamod1    ${template_as_string}    session=node2
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4968

Check_Device_Data_Removal_Is_Visible_On_Nodes_Without_node1
    [Documentation]    Check that the device data removal is propagated in the cluster even when node 1 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node2    ${empty_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node3    ${empty_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762

Restart_node1_After_Delete_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 1 restarted by admin just after device data is deleted and the change propagated in the cluster, fail if node 1 fails to boot.
    ClusterManagement.Start_Single_Member    1
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node1

Check_Device_Data_Removal_Is_Visible_On_node1
    [Documentation]    Check that the device data removal is propagated to node 1 as well.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_BOOT_TIMEOUT}    1s    Check_Config_Data    node1    ${empty_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5761

Kill_node2_Before_Create
    [Documentation]    Simulate node 2 crashes just before device data is created, fail if node 2 survives.
    ClusterManagement.Kill_Single_Member    2

Create_Device_Data_With_node2_Down
    [Documentation]    Check that the create requests work when node 2 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
...
Check_New_Device_Data_Is_Visible_On_Nodes_Without_node2
    [Documentation]    Check that the new device data is propagated in the cluster even when node 2 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${original_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node3    ${original_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762

Restart_node2_After_Create_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 2 restarted by admin just after device data is created and the change propagated in the cluster, fail if node 2 fails to boot.
    ClusterManagement.Start_Single_Member    2
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node2

Check_New_Device_Data_Is_Visible_On_node2
    [Documentation]    Check that the created device data is propagated to node 2 as well.
    [Tags]    critical
...
Kill_node2_Before_Delete
    [Documentation]    Simulate node 2 crashes just before device data is deleted, fail if node 2 survives.
    ClusterManagement.Kill_Single_Member    2

Delete_Device_Data_With_node2_Down
    [Documentation]    Check that the data removal requests work when node 2 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
    TemplatedRequests.Delete_Templated    ${directory_with_template_folders}${/}datamod1    ${template_as_string}    session=node3
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4968

Check_Device_Data_Removal_Is_Visible_On_Nodes_Without_node2
    [Documentation]    Check that the device data removal is propagated in the cluster even when node 2 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${empty_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node3    ${empty_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762

Restart_node2_After_Delete_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 2 restarted by admin just after device data is deleted and the change propagated in the cluster, fail if node 2 fails to boot.
    ClusterManagement.Start_Single_Member    2
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node2

Check_Device_Data_Removal_Is_Visible_On_node2
    [Documentation]    Check that the device data removal is propagated to node 2 as well.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_BOOT_TIMEOUT}    1s    Check_Config_Data    node2    ${empty_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5761

Kill_node3_Before_Create
    [Documentation]    Simulate node 3 crashes just before device data is created, fail if node 3 survives.
    ClusterManagement.Kill_Single_Member    3

Create_Device_Data_With_node3_Down
    [Documentation]    Check that the create requests work when node 3 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
...
Check_New_Device_Data_Is_Visible_On_Nodes_Without_node3
    [Documentation]    Check that the new device data is propagated in the cluster even when node 3 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${original_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node2    ${original_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762

Restart_node3_After_Create_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 3 restarted by admin just after device data is created and the change propagated in the cluster, fail if node 3 fails to boot.
    ClusterManagement.Start_Single_Member    3
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node3

Check_New_Device_Data_Is_Visible_On_node3
    [Documentation]    Check that the created device data is propagated to node 3 as well.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_BOOT_TIMEOUT}    1s    Check_Config_Data    node3    ${original_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5761

Kill_node3_Before_Modify
    [Documentation]    Simulate node 3 crashes just before device data is modified, fail if node 3 survives.
    ClusterManagement.Kill_Single_Member    3

Modify_Device_Data_With_node3_Down
    [Documentation]    Check that the modification requests work when node 3 is down.
    [Tags]    critical
    ${template_as_string}=    BuiltIn.Set_Variable    {'DEVICE_NAME': '${DEVICE_NAME}'}
    TemplatedRequests.Put_As_Xml_Templated    ${directory_with_template_folders}${/}datamod1    ${template_as_string}    session=node1
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4968

Check_Modified_Device_Data_Is_Visible_On_Nodes_Without_node3
    [Documentation]    Check that the device data modification is propagated in the cluster even when node 3 is down.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node1    ${modified_data}
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_CHECK_TIMEOUT}    1s    Check_Config_Data    node2    ${modified_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5762

Restart_node3_After_Modify_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 3 restarted by admin just after device data is modified and the change propagated in the cluster, fail if node 3 fails to boot.
    ClusterManagement.Start_Single_Member    3
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node3

Check_Modified_Device_Data_Is_Visible_On_node3
    [Documentation]    Check that the device data modification is propagated to node 3 as well.
    [Tags]    critical
    BuiltIn.Wait_Until_Keyword_Succeeds    ${DEVICE_BOOT_TIMEOUT}    1s    Check_Config_Data    node3    ${modified_data}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5761

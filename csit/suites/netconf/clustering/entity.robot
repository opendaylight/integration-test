...
Find_And_Shutdown_Device_Entity_Owner
    ...
    ${controller_list}=      ClusterKeywords.Get Controller List
    ${owner}    ${candidates}=    ClusterKeywords.Get Cluster Entity Owner And Candiates Indexes     controller@${controller_list}[0]     openflow    ${DEVICE_NAME} 
...
Check_Owner_Reconfigured
    ...
    ${controller_list}=     ClusterKeywords.Get Controller List
    ${owner}    ${candidates}=     ClusterKeywords.Get Device Entity Owner And Candidates Indexes   controller@{controller_list}[0]   openflow     ${DEVICE_NAME}

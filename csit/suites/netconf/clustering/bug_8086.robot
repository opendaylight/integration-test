...
*** Test Cases ***
Populate_Schema_Directories
    [Documentation]    On each ODL machine, create a directory and copy the car model there.
    ClusterManagement.With_Ssh_To_List_Or_All_Run_Keyword    Populate_Schema_Directory_Over_Active_Connection
...
Configure_Device_On_Netconf
    ...
    [Tags]    critical
    NetconfKeywords.Configure_Device_In_Netconf    ${DEVICE_NAME}    device_type=bug_8086    session=${NODE_CONFIGURER}    schema_directory=${SCHEMA_DIRECTORY}
...
Check_Device_Instance_Count
    ...
    [Document]    Count device instances over ${session} and check the count is ${expected}.
...
Populate_Schema_Directory_Over_Active_Connection
    ...
    ${filename} =    NexusKeywords.DeployArtifact    artifact=clustering-it-model    component=clustering-it-model    name_prefix=clustering-it-model-    name_suffix=-sources.jar
    ${dir} =    BuiltIn.Set_Variable    '${SCHEMA_DIRECTORY}'
    SSHKeywords.Execute_Command_Should_Pass    mkdir -p '${SCHEMA_DIRECTORY}' && unzip -j '${filename}' META-INF/yang/car.yang -d '${SCHEMA_DIRECTORY}/' && rm '${filename}'    stderr_must_be_empty=False

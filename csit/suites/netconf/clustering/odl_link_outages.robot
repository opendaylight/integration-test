*** Settings ***
Documentation     netconf cluster link outage test suite (CRUD operations).
...
...
...               on device data mounted onto a netconf connector while one of the nodes loses
...               connectivity with the other nodes and see if they work. Then rejoin the
...               isolated node up and check that it sees the operations that were made while
...               it was down are visible on it as well.
...
...               The network outage is simulated before each of the "Create", "Update" and
...               "Delete" operations and its repair is simulated after these operations.
...               After the "breakage" is "repaired", a test case makes sure the operation is
...               properly propagated within the cluster.
...
...               Currently each of the 3 operations is done once. "Create" is done while
...               node 1 is isolated, "Update" while node 2 is isolated and "Delete" while
...               node 3 is isolated.
...
Isolate_node1_Before_Create
    [Documentation]    Simulate node 1 gets isolated just before device data is created, fail if node 1 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${ODL_SYSTEM_1_IP}    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_3_IP}
...
Create_Device_Data_With_node1_Down
    ...
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5937
...
Rejoin_node1_After_Create_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 1 netword connectivity is restored by admin just after device data is created and the change propagated in the cluster, fail if node 1 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${ODL_SYSTEM_1_IP}    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_3_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node1
...
Isolate_node2_Before_Modify
    [Documentation]    Simulate node 2 gets isolated just before device data is modified, fail if node 2 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_1_IP}    ${ODL_SYSTEM_3_IP}
...
Rejoin_node2_After_Modify_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 2 netword connectivity is restored just after device data is modified and the change propagated in the cluster, fail if node 2 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_1_IP}    ${ODL_SYSTEM_3_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node2
...
Isolate_node3_Before_Delete
    [Documentation]    Simulate node 3 gets isolated just before device data is deleted, fail if node 3 survives.
    ClusterKeywords.Isolate_a_Controller_From_Cluster    ${ODL_SYSTEM_3_IP}    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_1_IP}
...
Rejoin_node3_After_Delete_And_Dump_Its_Topology_Data
    [Documentation]    Simulate node 3 netword connectivity is restored just after device data is deleted and the change propagated in the cluster, fail if node 3 fails to boot.
    ClusterKeywords.Rejoin_a_Controller_To_Cluster    ${ODL_SYSTEM_3_IP}    ${ODL_SYSTEM_2_IP}    ${ODL_SYSTEM_1_IP}
    ${config_topology}    ${operational_topology}=    Get_Topology    session=node3
...
*** Keywords ***
FIXME__Remove_Code_Duplication
    # This code is also in "node_outages.robot"

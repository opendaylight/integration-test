...
Configure_Device_On_Netconf
    ...
    [Tags]    critical
    NetconfKeywords.Configure_Device_In_Netconf    ${device_name}    device_type=${device_type}    http_timeout=2    device_user=root    device_password=root    device_port=830
...
Run Netopeer Docker Container
    ...
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    docker run -dt -p ${device_port}:830 -v ${USER_HOME}/datastore.xml:/usr/local/etc/netopeer/cfgnetopeer/datastore.xml -v ${USER_HOME}/sb-rsa-key.pub:/root/RSA.pub sdnhub/netopeer netopeer-server -v 3    return_stdout=True    return_stderr=True
    ...    return_rc=True
...
Suite Teardown
    ...
    Comment    RequestsLibrary.Delete_All_Sessions
    Comment    SSHLibrary.Close_All_Connections

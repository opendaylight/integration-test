...
...               the first test case failing), it starts to repeat the query
...               for a minute to see whether it is going "to fix itself" within
...               the minute. If yes, then the testcase will pass, which indicates
...               that the "ODL cooldown" of 1 minute is not long enough to allow
...               for netconf-connector to initialize properly.
...
...               If the first test case passed, then the second test case makes
...               another request ("to make sure the netconf-connector is really
...               up and running") and passes if yes. The real reason for this
...               test design is simplicity of the test cases as I did not want
...               to muddy the code with "skip this test when the previous test
...               case fails" logic. The "to make sure the netconf-connector is
...               really up and running" part of the testing here is just a nice
...               side benefit.

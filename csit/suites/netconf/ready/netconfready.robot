...
*** Variables ***
${netconfready__netconf_is_ready}    False
...
Check_Whether_Netconf_Is_Up_And_Running
    ...
    BuiltIn.Set_Suite_Variable    ${netconfready__netconf_is_ready}    True
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4708

Wait_For_Netconf
    [Documentation]    Wait for the Netconf to go up for configurable time.
    [Tags]    critical
    BuiltIn.Run_Keyword_Unless    ${netconfready__netconf_is_ready}    BuiltIn.Wait_Until_Keyword_Succeeds    ${NETCONFREADY_WAIT}    1s    Check_Netconf_Up_And_Running
    BuiltIn.Set_Suite_Variable    ${netconfready__netconf_is_ready}    True
...
Check_For_Bug_5014
    ...
    [Tags]    critical
    BuiltIn.Pass_Execution_If    ${netconfready__netconf_is_ready}    Netconf was detected to be up and running so bug 5014 did not show up.

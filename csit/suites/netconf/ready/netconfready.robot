...
Check_Whether_Netconf_Is_Up_And_Running
    ...
    BuiltIn.Set_Suite_Variable    ${netconf_ready}    True
    [Teardown]    Utils.Report_Failure_Due_To_Bug    4708

Wait_For_Netconf
    [Documentation]    Wait for the Netconf to go up for configurable time.
    [Tags]    critical
    BuiltIn.Run_Keyword_Unless    ${netconf_ready}    BuiltIn.Wait_Until_Keyword_Succeeds    ${NETCONFREADY_WAIT}    1s    Check_Netconf_Up_And_Running
...
Check_Netconf_Up_And_Running
    ...
    ${response}=    RequestsLibrary.Get    ses    restconf/config/network-topology:network-topology/topology/topology-netconf${pretty_print}

...
${NETCONFREADY_WAIT}    180s
${USE_NETCONF_CONNECTOR}    True
${DEBUG_LOGGING_FOR_EVERYTHING}    False

*** Test Cases ***
Install_Features
    Install_a_Feature    odl-netconf-connector-all
...
Check_Whether_Netconf_Is_Alive
    [Documentation]    If Netconf is dead even after "Check_For_Bug_5014" case, the bug 5383 is the most likely culprit.
    BuiltIn.Should_Be_True    ${netconf_is_ready}
    [Teardown]    Utils.Report_Failure_Due_To_Bug    5383

Check_Whether_Netconf_Can_Pretty_Print
    [Documentation]    Make one pretty-print request to netconf-connector and see if it works. If not, that might be another symptom of bug 5383.
    [Tags]    critical

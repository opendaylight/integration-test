...
${DEVICE_COUNT}    500
${TIMEOUT_FACTOR}    10
${device_type}    topology-netconf-devices
...
Configure_Devices_Onto_Netconf
    ...
    NetconfKeywords.Configure_Devices_In_Netconf    ${device_name_prefix}    devices_type=${device_type}    first_device_port=${DEVICE_COUNT}
    ${timeout}=    BuiltIn.Evaluate    ${DEVICE_COUNT}*${TIMEOUT_FACTOR}
    NetconfKeywords.Perform_Operation_On_Each_Device    Wait_To_Configure_Devices    timeout=${timeout}    first_netconf_device_port=${DEVICE_COUNT}

Get_Data_From_Devices
    [Documentation]    Ask testtool devices for data.
    ${timeout}=    BuiltIn.Evaluate    ${DEVICE_COUNT}*${TIMEOUT_FACTOR}
    NetconfKeywords.Perform_Operation_On_Each_Device    Check_Devices_Data    count=${DEVICE_COUNT}    timeout=${timeout}
...
Deconfigure_Devices_From_Netconf
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    timeout=120s    period=1s    NetconfKeywords.Delete_Whole_Netconf_Topology
...
Deconfigure_Devices
    ...
    NetconfKeywords.Delete_Whole_Netconf_Topology

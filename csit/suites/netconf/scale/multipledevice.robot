...
Configure_Devices_Onto_Netconf
    ...
    NetconfKeywords.Perform_Operation_On_Each_Device    Wait_To_Configure_Devices    timeout=${timeout}    ${test_type}

Get_Data_From_Devices
    [Documentation]    Ask testtool devices for data.
    ${timeout}=    BuiltIn.Evaluate    ${DEVICE_COUNT}*${TIMEOUT_FACTOR}
    NetconfKeywords.Perform_Operation_On_Each_Device    Check_Devices_Data    count=${DEVICE_COUNT}    timeout=${timeout}    ${test_type}

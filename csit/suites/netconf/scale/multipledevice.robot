...
${test_type}    multipledevice
${TOPOLOGY_LOCATION}    topology-netconf
...
Configure_Devices_Onto_Netconf
    ...
    NetconfKeywords.Perform_Operation_On_Each_Device    Wait_To_Configure_Devices    timeout=${timeout}    test_type=${test_type}

Get_Data_From_Devices
    [Documentation]    Ask testtool devices for data.
    ${timeout}=    BuiltIn.Evaluate    ${DEVICE_COUNT}*${TIMEOUT_FACTOR}
    NetconfKeywords.Perform_Operation_On_Each_Device    Check_Devices_Data    count=${DEVICE_COUNT}    timeout=${timeout}    test_type=${test_type}
...
Deconfigure_Devices_From_Netconf
    ...
    NetconfKeywords.Delete_Whole_Netconf_Topology    topology_location=${TOPOLOGY_LOCATION}
    NetconfKeywords.Check_Topology_Completely_Gone

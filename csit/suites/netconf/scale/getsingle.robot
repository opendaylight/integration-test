...
Configure_Devices_Onto_Netconf
    ...
    NetconfKeywords.Perform_Operation_On_Each_Device    Configure_Devices    count=1    timeout=${timeout}
...
Configure_Devices
    ...
    [Documentation]    Operation for configuring the devices in the Netconf subsystem and connecting to it.
    KarafKeywords.Log_Message_To_Controller_Karaf    Connecting devices
    NetconfKeywords.Configure_Devices_In_Netconf    ${device_name_prefix}${current_port}    device_type=${device_type}    device_port=${current_port}    iterations=500
    KarafKeywords.Log_Message_To_Controller_Karaf    Waiting for devices to connect
    NetconfKeywords.Wait_Device_Connected    ${current_name}    timeout=500s    period=2s
    KarafKeywords.Log_Message_To_Controller_Karaf    Devices ${current_name}-all connected

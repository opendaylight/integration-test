...
Find Max Netconf Devices
    ...
    FOR    ${devices}    IN RANGE    ${start}    ${stop+1}    ${increment}
        ...
        Run Keyword If    "${INSTALL_TESTTOOL}"=="True"    NetconfKeywords.Install_And_Start_Testtool    schemas=${schema_dir}    device-count=${devices}
        ...    ELSE    NetconfKeywords.Start_Testtool    ${TESTTOOL_EXECUTABLE}    schemas=${schema_dir}    device-count=${devices}
...
Get Juniper Device Schemas
    ${output} =    OperatingSystem.Run    git clone https://github.com/Juniper/yang.git
    Log    ${output}
    ${output} =    OperatingSystem.Run    mkdir /tmp/junos_19.4R1
    Log    ${output}
    ${rc}    ${output} =    OperatingSystem.Run and Return RC and Output    find yang/19.4/19.4R1/junos -type f -name '*yang' -exec cp {} /tmp/junos_19.4R1/ \\;
    Log    ${output}
    ${output} =    OperatingSystem.Run    find yang/19.4/19.4R1/junos -type f -name '*yang';
    Log    ${output}
    ${output} =    OperatingSystem.Run    cp yang/19.4/19.4R1/common/* /tmp/junos_19.4R1/
    Log    ${output}
    ${output} =    OperatingSystem.Run    sudo yum install -y tree;
    Log    ${output}
    ${output} =    OperatingSystem.Run    tree -L 5
    Log    ${output}
    ${output} =    OperatingSystem.Run    tree -L 3 /tmp/junos_19.4R1;
    Log    ${output}

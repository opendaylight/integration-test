...
Get Juniper Device Schemas
    ...
    Log    ${output}
    ${output} =    OperatingSystem.Run    find yang/19.4/19.4R1/junos -type f -name '*yang' -exec cp {} junos_19.4R1/ \;
    Log    ${output}
    ${output} =    OperatingSystem.Run    cp yang/19.4/19.4R1/common/* /tmp/junos_19.4R1/
    Log    ${output}
    ${output} =    OperatingSystem.Run    sudo yum install -y tree; tree -L 3;
    Log    ${output}

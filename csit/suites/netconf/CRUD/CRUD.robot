...
Check_Device_Gone_After_Delete
    ...
    [Tags]    critical
    BuiltIn.Set_Suite_Variable    ${device_still_there}    True
    Check_Device_Has_No_Netconf_Connector
    BuiltIn.Set_Suite_Variable    ${device_still_there}    False

Check_Device_Gone_After_Delete_And_A_While
    [Documentation]    If the device did not disappear before the delete request returned OK,
    ...    check that it will go away in a reasonable time.
    BuiltIn.Run_Keyword_If    ${device_still_there}     BuiltIn.Wait_Until_Keyword_Succeeds    5s    1s    Check_Device_Has_No_Netconf_Connector

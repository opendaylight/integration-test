...
Check_Device_Going_To_Be_Gone_After_Delete
    ...
    ...    This is an expected behavior as the delete request is sent to the config subsystem which
    ...    then triggers asynchronous destruction of the netconf connector referring the device which
    ...    is reflected in the operational data once completed. This test makes sure this asynchronous
    ...    operation does not take unreasonable amount of time. While we can't be sure if everything
    ...    about the device is gone, we can at least make sure that the netconf connector itself is
    ...    going to disappear.
    [Tags]    critical

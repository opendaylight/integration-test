...
CallHome over SSH with correct credentials
    ...
    Register SSH call-home device in ODL controller    netopeer2    root    root    ${NETOPEER_PUB_KEY}
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    docker-compose up -d    return_stdout=True    return_stderr=True
    ...    return_rc=True
    Wait Until Keyword Succeeds    90s    2s    NetconfCallHomeV2.Check Device Status    CONNECTED
...
CallHome with Incorrect Node-id
    ...
    Apply SSH-based Call-Home configuration
    Register SSH call-home device in ODL controller (APIv1)    incorrect    root    root    ${NETOPEER_PUB_KEY}
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    docker-compose up -d    return_stdout=True    return_stderr=True
    ...    return_rc=True

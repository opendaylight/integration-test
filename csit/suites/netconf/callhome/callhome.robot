*** Settings ***
Documentation     Test suite to verify callhome functionality where the Call Home Server is provisioned with device certificates when docker-compose is invoked. Every test case does a SED operation to search and replace words to cover the happy path and negative scenarios.
...
CallHome with Incorrect global Credentials
    ...
    Wait Until Keyword Succeeds    30s    2s    Run Keyword And Expect Error    *    Check For Elements At URI    ${mount_point_url}
...
Successful CallHome with correct global credentials
    ...
    Wait Until Keyword Succeeds    30s    2s    Check For Elements At URI    ${mount_point_url}    ${netconf_mount_expected_values}

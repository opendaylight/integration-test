...
Getting Netopeer ready for CallHome
    ...
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    curl -L "https://github.com/docker/compose/releases/download/1.11.1/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose    return_stdout=True    return_stderr=True    return_rc=True
    ${stdout}    ${stderr}    ${rc}=    SSHLibrary.Execute Command    chmod +x /usr/local/bin/docker-compose    return_stdout=True    return_stderr=True    return_rc=True

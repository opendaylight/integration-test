*** Settings ***
Documentation     Suite for testing device connector behavior with respect to High Availability.
...
...
...               The results can be different when Owner and Leader are co-located,
...               this suite assumes the initial state is the one that should be tested.
...
...
Start_Configurer
    ...
    ${topology_config_leader_index}    ${candidates} =    ClusterManagement.Get_Leader_And_Candidates_For_Shard    shard_name=topology    shard_type=config

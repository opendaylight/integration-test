...
${CONNECTION_SLEEP}    3.5    # Very large, to ensure connections are established.
...
Locate_Managers
    ...
    ${topology_config_leader_index}    ${candidates} =    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    ClusterManagement.Get_Leader_And_Followers_For_Shard    shard_name=topology    shard_type=config
    ...
    ${netconf_manager_owner_index}    ${candidates} =    BuiltIn.Wait_Until_Keyword_Succeeds    3x    2s    ClusterManagement.Get_Owner_And_Candidates_For_Type_And_Id    type=topology-netconf    id=/general-entity:entity[general-entity:name='topology-manager']    member_index=1
...
Start_Testtool
    ...
    # TODO: Is there an easy way to "prepend" additional keyword (cells) to be executed before the default Test Teardown?
    # TODO: Alternatively, introduce NetconfKeywords.Safe_Install_And_Start_Testtool
    [Teardown]    SSHLibrary.Switch_Connection    ${configurer_connection_index}

...
Locate_Owner
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    ${OWNER_ELECTION_TIMEOUT}    3s    Single_Locate_Owner_Attempt
...
Install_Feature_On_Leader
    ...
    [Teardown]    BuiltIn.Run_Keywords    ClusterManagement.Run_Karaf_Command_On_Member    command=log:set ${DEFAULT_KARAF_LOG_LEVEL}    member_index=${topology_leader_index}    AND    SetupUtils.Teardown_Test_Show_Bugs_If_Test_Failed
...
Single_Locate_Owner_Attempt
    ...
    ...    For the first member listed: Get the actual owner, check candidates size, store owner to suite variable.
    ...    (If the list has less then one item, this Keyword will fail.)
    ...    For other nodes: Get actual owner, check candidates size, compare to the first listed member results.
    ...
    ${require_candidate_list_length} =    BuiltIn.Get_Length    ${index_list}
    ${first_index_listed} =    Collections.Remove_From_List    ${index_list}    ${0}
    # Now ${index_list} contains only the rest of indices.
    ${netconf_manager_owner_index}    ${candidates} =    ClusterManagement.Get_Owner_And_Candidates_For_Type_And_Id    type=topology-netconf    id=/general-entity:entity[general-entity:name='topology-manager']    member_index=${first_index_listed}    require_candidate_list_length=${require_candidate_list_length}
    BuiltIn.Set_Suite_Variable    \${netconf_manager_owner_index}
    : FOR    ${index}    IN    ${index_list}
    \    ${new_owner}    ${new_candidates} =    ClusterManagement.Get_Owner_And_Candidates_For_Type_And_Id    type=topology-netconf    id=/general-entity:entity[general-entity:name='topology-manager']    member_index=${index}    require_candidate_list_length=${require_candidate_list_length}
    ...
    \    BuiltIn.Should_Be_Equal    ${new_candidates}    ${candidates}    Member-${index} candidates ${new_candidates} is not ${candidates}

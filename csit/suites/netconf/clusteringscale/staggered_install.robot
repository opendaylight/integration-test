...
Single_Locate_Owner_Attempt
    [Documentation]    Performs actions on members, one by one:
    ...    For member-1: Get actual owner, check candidates size, store owner to suite variable.
    ...    For other nodes: Get actual owner, check candidates size, compare to member-1 results.
    ${require_candidate_list_length} =    BuiltIn.Get_Length    ${topology_follower_indices}
    ${netconf_manager_owner_index}    ${candidates} =    ClusterManagement.Get_Owner_And_Candidates_For_Type_And_Id    type=topology-netconf    id=/general-entity:entity[general-entity:name='topology-manager']    member_index=${1}    require_candidate_list_length=${require_candidate_list_length}
    BuiltIn.Set_Suite_Variable    \${netconf_manager_owner_index}
    : FOR    ${index}    IN RANGE    2    ${NUM_ODL_SYSTEM}+1  # [2,3] for ${NUM_ODL_SYSTEM}==3

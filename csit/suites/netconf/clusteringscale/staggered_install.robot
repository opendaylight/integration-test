...
Install_Feature_On_Followers
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    1s    ClusterManagement.Install_Feature_On_List_Or_All    feature_name=odl-netconf-clustered-topology    member_index_list=${topology_follower_indices}    timeout=60s
...
Install_Feature_On_Leader
    ...
    ClusterManagement.Run_Karaf_Command_On_Member    command=log:set TRACE    member_index=${topology_leader_index}
    ClusterManagement.Install_Feature_On_Member    feature_name=odl-netconf-clustered-toology    member_index=${topology_leader_index}    timeout=60s
    BuiltIn.Set_Suite_Variable    \${installation_successful}    True
    # FIXME: Do not skip the old teardown.
    [Teardown]    ClusterManagement.Run_Karaf_Command_On_Member    command=log:set INFO    member_index=${topology_leader_index}
...
Retry_Feature_Install_On_Leader
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    1s    ClusterManagement.Install_Feature_On_Member    feature_name=odl-netconf-clustered-toology    member_index=${topology_leader_index}    timeout=60s

...
Resource          ${CURDIR}/../../../libraries/Wait_For_Failure.robot
...
Locate_Leader
    ...
    BuiltIn.Comment    FIXME: Migrate Set_Suite_Variables_For_Shard to ClusterManagement.robot
    CarPeople.Set_Suite_Variables_For_Shard    shard_name=topology    shard_type=config
...
Install_Feature_On_Followers
    ...
    ClusterManagement.Run_Karaf_Command_On_List_Or_All    feature:install odl-netconf-clustered-toology    index_list=${topology_follower_indices}
...
Install_Feature_On_Leader
    ...
    ClusterManagement.Run_Karaf_Command_On_Member    feature:install odl-netconf-clustered-toology    member_index=${topology_leader_index}

Verify_Managers_Are_Stationary
    [Documentation]    Keep checking that Managers do not move for a while.
    Wait_For_Failure.Verify_Keyword_Does_Not_Fail_Within_Timeout    ${OWNER_ELECTION_TIMEOUT}    1s    Check_Manager_Positions

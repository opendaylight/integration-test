...
Locate_Leader
    ...
    BuiltIn.Wait_Until_Keyword_Succeeds    3x    1s    CarPeople.Set_Variables_For_Shard    shard_name=topology    shard_type=config
...
Install_Feature_On_Followers
    ...
    ClusterManagement.Run_Karaf_Command_On_List_Or_All    feature:install odl-netconf-clustered-topology    timeout=300s    member_index_list=${topology_follower_indices}
    # FIXME: Fail if something like "Error executing command: Can't install feature odl-netconf-clustered-toology/0.0.0: null" is encountered.

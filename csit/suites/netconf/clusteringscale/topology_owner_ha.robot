...
Start_Configurer
    [Documentation]    Launch Python utility (while copying output to log file) and wait for devices to be present.
    BuiltIn.Set_Suite_Variable    \${log_filename}    ${SUITE_NAME}-configurer.log
    # TODO: Should things like restconf port/user/password be set from Variables?
    ${command} =    BuiltIn.Set_Variable    python configurer.py --odladdress ${topology_config_leader_ip} --deviceaddress ${TOOLS_SYSTEM_IP} --devices ${DEVICE_SET_SIZE} --disconndelay ${CONFIGURED_DEVICES_LIMIT} --basename ${DEVICE_BASE_NAME} | tee "${log_filename}"

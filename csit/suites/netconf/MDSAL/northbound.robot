...
Fetch_And_Send
    [Arguments]    ${name}
    [Documentation]    Load and send the request from the dataset and return the reply and expected reply data stored in the dataset.
...
Perform_Check
    ...
    [Documentation]    Patch ${expected} to match a Netconf reply, compare with ${actual} and fail if not equal.

Perform_Test
    [Arguments]    ${name}
    [Documentation]    Load and send the request from the dataset and compare the returned reply to the one stored in the dataset.
    ${actual}    ${expected}=    Fetch_And_Send    ${name}
    ${newline}=    BuiltIn.Evaluate    "\\r\\n"
    BuiltIn.Should_Be_Equal    ${actual}    ${newline}${expected}${ODL_NETCONF_PROMPT}
    [Return]    ${actual}

Perform_Test_With_Contains
    [Arguments]    ${name}
    [Documentation]    Load and send the request from the dataset, then check that the reply contains data stored in the dataset.
    ${actual}    ${expected}=    Fetch_And_Send    ${name}
    BuiltIn.Should_Contain    ${actual}    ${expected}
    [Return]    ${actual}

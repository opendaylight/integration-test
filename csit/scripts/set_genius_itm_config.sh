...
cat > ${WORKSPACE}/set_genius_itm_config.sh <<EOF
    ...
    export CONFFILE=\`find /tmp/${BUNDLEFOLDER} -name "*genius-itm*config.xml"\`
    cp \$CONFFILE /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/genius-itm-config.xml
    sed -ie "s/<def-tz-enabled>.*</<def-tz-enabled>\$ODL_DEF_TZ_ENABLED</g" \${CONFFILE}
...
do
        ...
        scp ${WORKSPACE}/set_genius-itm-config.sh ${!CONTROLLERIP}:/tmp/
        ssh ${!CONTROLLERIP} 'bash /tmp/set_genius_itm-config.sh'

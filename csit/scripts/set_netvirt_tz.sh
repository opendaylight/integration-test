...
cat > ${WORKSPACE}/netvirt-elanmanager-config.sh <<EOF

    mkdir -p /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/
    export CONFFILE=\`find /tmp/${BUNDLEFOLDER} -name "*netvirt-elanmanager*config.xml"\`
    cp \$CONFFILE /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/netvirt-elanmanager-config.xml
    sed -ie "s/<auto-config-transport-zones>.*</<auto-config-transport-zones>\$ODL_AUTO_CONFIG_TRANSPORT_ZONES</g" \${CONFFILE}

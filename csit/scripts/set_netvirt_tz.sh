...
cat > ${WORKSPACE}/set_netvirt_tz.sh <<EOF
    ...
    sed -ie "s/<auto-config-transport-zones>.*</<auto-config-transport-zones>\$ODL_AUTO_CONFIG_TRANSPORT_ZONES</g" /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/netvirt-elanmanager-config.xml

...
cat > ${WORKSPACE}/set_itm_direct_tunnels.sh <<EOF
    ...
    sed -i 's/itm-direct-tunnels>false/itm-direct-tunnels>${ITM_DIRECT_TUNNELS}/g' /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/genius-ifm-config.xml
...
do
        ...
        echo "Setting itm-direct-tunnels to ${ITM_DIRECT_TUNNELS} on ${!CONTROLLERIP}"

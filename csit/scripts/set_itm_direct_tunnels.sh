...
cat > ${WORKSPACE}/set_itm_direct_tunnels.sh <<EOF
    ...
    ls /tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/
    ...
    echo $NATCONFFILE
    export ACLCONFFILE=\`find /tmp/${BUNDLEFOLDER} -name "*aclservice*config.xml"\`
    echo $ACLCONFFILE
...
do
        ...
        scp ${WORKSPACE}/test/csit/libraries/netvirt/genius-ifm-config.xml ${!CONTROLLERIP}:/tmp/${BUNDLEFOLDER}/etc/opendaylight/datastore/initial/config/
        ssh ${!CONTROLLERIP} 'bash -x /tmp/set_itm_direct_tunnels.sh'

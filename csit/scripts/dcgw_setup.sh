...
echo "Create the quagga group and quagga user"
    ...
    *)
         ...
         libssl-dev libtool make pkg-config gawk libreadline-dev libc6 libglib2.0-dev -y --force-yes
...
#    cp /tmp/6wind_quagga_build_dir/quagga/bgpd/zebra.conf.sample /opt/quagga/etc/zebra.conf 
    cp /opt/quagga/etc/zebra.conf.sample /opt/quagga/etc/zebra.conf

...
#Install the required softwares for building quagga
    sudo yum -y install apt-get install automake bison flex g++ git libboost1.55-all-dev libevent-dev\
    libssl-dev libtool make pkg-config gawk 
...
#Checkout Compile and Install Quagga
    ...
    sudo sudo addgroup --system quagga

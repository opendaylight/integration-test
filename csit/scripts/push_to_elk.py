...
PUT_URL = 'https://{}:{}{}{}{}'.format(ELK_DB_HOST, ELK_DB_PORT,
        PUT_URL_INDEX, PUT_URL_TYPE, PUT_URL_ID)
print PUT_URL
...
def getVisualization(tesplan, key, fieldlist):
    ...
    vis['description'] = 'visualization of ' + key \
        + ' trends for testplan ' + os.environ['TESTPLAN']
    vis['version'] = 1
    vis['kibanaSavedObjectMeta'] = {'searchSourceJSON': ''}
    searchSourceJSON = {'index': 'opendaylight-*',
        ...
        'key': 'project',
        'value': 'opendaylight',
        }, 'query': {'match': {'project': {'query': 'opendaylight',
                     'type': 'phrase'}}}, '$state': {'store': 'appState'
                }}]}
    ...
    for field in fieldlist:
        seriesParam = {
            ...
            'interpolate': 'linear',
            'lineWidth': 2,
            'data': {'id': str(len(visState['params']['seriesParams'])
                     + 1) + '-' + vis['title'], 'label': field.split('.'
                     )[-1]},
        ...
        agg = {
            ...
            'type': 'sum',
            'schema': 'metric',
            'params': {'field': field, 'customLabel': field.split('.'
                       )[-1]},
            }
...
if BODY['test-type'] == 'performance':
    ...
    for key in BODY['plots']:
        ...
        PUT_URL = \
            'https://{}:{}/.kibana/visualization/{}-{}'.format(ELK_DB_HOST,
                ELK_DB_PORT, os.environ['TESTPLAN'], key)
        print PUT_URL
...
vis = getVisualization(os.environ['TESTPLAN'], 'functional',
                       ['pass-tests', 'failed-tests'])
vis_ids.append(os.environ['TESTPLAN'] + '-functional')
PUT_URL = \
    'https://{}:{}/.kibana/visualization/{}-functional'.format(ELK_DB_HOST,
        ELK_DB_PORT, os.environ['TESTPLAN'])
print PUT_URL
print json.dumps(vis, indent=4)
try:
    r = requests.put(PUT_URL, json=vis)
...
PUT_URL = 'https://{}:{}/.kibana/dashboard/{}'.format(ELK_DB_HOST,
        ELK_DB_PORT, os.environ['TESTPLAN'])
print PUT_URL

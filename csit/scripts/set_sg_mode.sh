...
cat > ${WORKSPACE}/set_sg_mode.sh <<EOF
    ...
    export CONFFILE1=\`find /tmp/${BUNDLEFOLDER} -name "*aclservice*config.xml"\`
    echo "is this the conf file: "
    echo \$CONFFILE1
    echo "another check"
    export CONFFILE2=\$(find /tmp/${BUNDLEFOLDER} -name "*aclservice*config.xml")
    echo "is this the conf file: "
    echo \$CONFFILE2
    echo "another check"

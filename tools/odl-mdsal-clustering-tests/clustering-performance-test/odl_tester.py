...
def _prepare_post(cntl, method, flows, template=None):
    ...
    body = {"flow": flow_list}
    url = "http://" + cntl + ":8181" + odl_node_url + dev_id + "/flow-node-inventory:table=0"
...
def get_flow_ids(controller="127.0.0.1", port=8181):
    ...
    for device_id in device_ids:
        rsp = requests.get(
            url="http://{0}:{1}/rests/data/opendaylight-inventory:nodes/node=%s/flow-node-inventory:table=0?content=nonconfig".format(
                controller, port
            )
            % device_id,
            auth=("admin", "admin"),

...
def _prepare_post(cntl, method, flows, template=None):
    ...
    body = {"flow": flow_list}
    url = 'http://' + cntl + ':' + '8181/restconf/config/opendaylight-inventory:nodes/node/' + dev_id + '/table/0'
...
def _prepare_delete(cntl, method, flows, template=None):
    ...
    url = 'http://' + cntl + ':' + '8181/restconf/config/opendaylight-inventory:nodes/node/' + dev_id + '/table/0/flow/' + str(flow_id)
    req = requests.Request(method, url, headers={'Content-Type': 'application/json'},
...
def get_device_ids(controller='127.0.0.1', port=8181):
    ...
    ids = []
    rsp = requests.get(url='http://{0}:{1}/restconf/operational/opendaylight-inventory:nodes'.format(controller, port), auth=('admin', 'admin'))
...
def get_flow_ids(controller='127.0.0.1', port=8181):
    ...
    for device_id in device_ids:
        rsp = requests.get(url='http://{0}:{1}/restconf/operational/opendaylight-inventory:nodes/node/%s/table/0'.format(controller, port) % device_id, auth=('admin', 'admin'))
        if rsp.status_code != 200:
            return []
        try:

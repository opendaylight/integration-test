...
def _build_post(odl_ip, port, uri, python_data, auth_tuple):
...
def _prepare_add_car(odl_ip, port, item_list, auth_tuple):
    ...
    req = _build_post(odl_ip, port, "config/car:cars", container, auth_tuple)
    return req
...
def _prepare_add_people(odl_ip, port, item_list, auth_tuple):
    ...
    req = _build_post(odl_ip, port, "config/people:people", container, auth_tuple)
    return req
...
def _prepare_add_car_people_rpc(odl_ip, port, item_list, auth_tuple):
    ...
    req = _build_post(odl_ip, port, "operations/car-purchase:buy-car", container, auth_tuple)
    return req
...
def add_car(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    ...
    stdout_logger.info("Add %s car(s) to %s:%s (%s per request)",
...
def add_people(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    ...
    stdout_logger.info("Add %s people to %s:%s (%s per request)",
                       item_count, odl_ip, port, items_per_request)
...
def add_car_people_rpc(odl_ip, port, thread_count, item_count, auth_tuple,
    ...
    stdout_logger.info("Add %s purchase(s) to %s:%s (%s per request)",
                       item_count, odl_ip, port, items_per_request)

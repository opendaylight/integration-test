...
def _request_sender(thread_id, preparing_function, auth_tuple, inqueue=None, exitevent=None, odl_ip="127.0.0.1", port="8181", outqueue=None):
    ...
    while loop:
        try:
            itemlist = inqueue.get(timeout=1)
        except Queue.Empty:
            if exitevent.is_set() and inqueue.empty():
                loop = False
            continue
        req = preparing_function(odl_ip, port, itemlist, auth_tuple)
        prep = req.prepare()
        try:
        ...
        stdout_logger.debug("Elapsed time: %s", rsp.elapsed)
    ...
    for i, v in enumerate(counter):
        if v > 0:
            res[i] = v
    outqueue.put(res)
    stdout_logger.info("Response code(s) got for number of items: %s", res)


def _task_executor(preparing_function, odl_ip="127.0.0.1", port="8181", thread_count=1, item_count=1, items_per_request=1, auth_tuple=True):
    ...
    for i in range(0, item_count, items_per_request):
        itemgroups.append(items[i:i+items_per_request])

    # lets fill the queue with details needed for one http requests
    sendqueue = Queue.Queue()
    for item_list in itemgroups:
        sendqueue.put(item_list)
    ...
    for i in range(int(thread_count)):
        thr = threading.Thread(target=_request_sender, args=(i, preparing_function, auth_tuple),
...
def _help_delete(odl_ip, port, uri):
    ...
    stdout_logger.info("Elapsed time: %s", rsp.elapsed)
...
def delete_people(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    """Delete people container from config datastore, assert success.
...
def delete_car_people(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    """Delete car-people container from config datastore, assert success.
...
def _help_get(odl_ip, port, uri):
    """Send GET to generic URI, assert status code is 200.
...
def get_car(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    """Reads car entries from config datastore, assert success.
...
def get_people(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    """Reads people entries from config datastore, assert success.
...
def get_car_people(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    """Reads car-people entries from config datastore, assert success.
...
def add_car(odl_ip, port, thread_count, item_count, auth_tuple, items_per_request):
    ...
    Args:
        ...
        :param items_per_request: items per request, not used here, just to keep the same api

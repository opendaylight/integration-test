...
def validate_cluster(ipaddress):
    ...
    cluster_status = data['value']
    members = cluster_status['ClusterStatus']
    status = (json.loads(members)['members'])
    member_list = []
    entity_owner_list = []
    ...
    for member in status:
        ...
        shard_manager = "Category=ShardManager,"
        operational = "name=shard-manager-operational,"
        datastore = "type=DistributedOperationalDatastore"
        url1 = base + jolokia + shard_manager + operational + datastore

...
while key != ord('q') and key != ord('Q'):
    ...
    for row, controller in enumerate(controllers):
        ...
        for data_column, shard in enumerate(Shards):
            stdscr.addstr(0, controller_len + 1 + (field_len + 1) * data_column, string.center(shard, field_len), curses.color_pair(1))
    ...
    for data_column, shard_name in enumerate(Shards):
        ...
        if shard_name not in shards_to_exclude:
            ...
            for row, controller in enumerate(controllers):
                ...
                stdscr.addstr(row + 1, controller_len + 1 + (field_len + 1) * data_column, status['txt'], status['color'])
        time.sleep(0.5)
        if odd_or_even % 2 == 0:

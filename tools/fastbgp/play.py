...
class MessageGenerator(object):
    ...
    def open_message(self, version=None, my_autonomous_system=None,
        ...
        if self.grace != 4:
            ...
            # "\x06" Length (6 bytes)
            # "\x00" Restart Flag (customizable - turned on when grace == 2 or 3)
            # "\x05" Restart timer (5sec)
            # "\x00\x01" AFI (IPV4)
            # "\x01"  SAFI (Unicast)
            # "\x00"  Ipv4 Flag (customizable - turned on when grace == 1 or 3)
            optional_parameter_hex = "\x02\x08\x40\x06{}\x00\x01\x01{}".format(
    ...
    def update_message(self, wr_prefixes=None, nlri_prefixes=None,
        ...
        if self.grace != 4 and self.grace != 0 and end_of_rib:
...
def job(arguments, inqueue, storage):
    ...
    logger.debug("Open message: {}".format(binascii.hexlify(msg_in)))

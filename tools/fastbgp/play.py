...
class MessageGenerator(object):
    ...
    def write_results_to_file(self, file_name=None, treshold=None):
        ...
        if performance1 is not None:
            results['straightforward phase'] = performance1
        if performance2 is not None:
            results['mixed traffic phase'] = performance2
    ...
    def compose_update_message(self, PrefixCountToAdd=None, PrefixCountToDel=None):
        ...
        if self.Remaining_Prefixes > self.Remaining_Prefixes_Treshold:
            ...
            # one shot counter update
            if not self.phase1_start_time:
                self.phase1_start_time = time.time()
            # updating final counters - who knows ... maybe finally ;)

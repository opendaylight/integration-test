...
class ReadTracker(object):
    ...
    def decode_update_message(self, msg):
        ...
        if msg_type == 2:
            ...
            total_pa_length = int(binascii.b2a_hex(total_pa_length_hex), 16) - 5  # for suite debugging only (wrongly encoded length) - remove the - 5
    ...
    def wait_for_read(self):
        ...
        if not busy:
            if result[0] == []:
                logger.info("No message received in %ss", wait_timedelta)

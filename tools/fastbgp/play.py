...
class MessageGenerator(object):
    ...
    def compose_update_message(self, withdrawal):
        ...
        if not withdrawal:
            ...
            print 'Update message (without Withdrawn Routes): ' + binascii.b2a_hex(msg_out)
        else:
            msg_out = self.update_message_without_prefix_del_routes + prefix_packed
            print 'Update message (with Withdrawn Routes): ' + binascii.b2a_hex(msg_out)
        self.int_nextprefix += 16  # Hardcoded, as open message specifies such netmask.
        self.updates_to_send -= 1
        print 'Updates to send: ' + self.updates_to_send
        return msg_out

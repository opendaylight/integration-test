...
def parse_arguments():
    ...
    str_help = 'Prefix insertion and withdrawal in a single common UPDATE message'
    parser.add_argument('--single', dest='combined', action='store_const', const=True,
                        default=False, help=str_help)
    ...
    parser.add_argument('--debug', dest='loglevel', action='store_const', const=logging.DEBUG,
                        default=logging.ERROR, help=str_help)
    str_help = 'Name of the file where results are stored for plotting purposes'
    parser.add_argument('--results', default='performance.csv', type=str, help=str_help)
...
class MessageGenerator(object):
    ...
    def __init__(self, args):
        ...
        logging.info('  Remaining number of prefixes to process with parallel withdrawals: ' +
    ...
    def write_results_to_file(self, file_name=None, treshold=None):
        """
        Writes specified results (values) into the file (file_name).
        ...
        # putting performace results
        results = {}
        if performance1 is not None:
            results['Straightforward'] = performance1
        if performance2 is not None:
            results['Mixed traffic'] = performance2
        logging.info('    Performance values: ' + str(results))
        # storing to the file
        first_line = ''
        second_line = ''
        f = open(file_name, 'wt')
        ...
        if write_list:
            ...
            if self.generator.Remaining_Prefixes:
                ...
                if not self.generator.Remaining_Prefixes:  # We have just finished update generation, end-of-rib is due.
                    ...
                    self.generator.write_results_to_file()

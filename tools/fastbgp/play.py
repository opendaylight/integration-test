...
def parse_arguments():
    ...
    str_help = 'Desired change in number of advertised routes in one iteration (one UPDATE message).'
    parser.add_argument('--change', default='1', type=int, help=str_help)
    str_help = 'Max number of IP prefixes in each variable of UPDATE message.'
    parser.add_argument('--items', default='1', type=int, help=str_help)
...
class MessageGenerator(object):
    ...
    def __init__(self, args):
        ...
        self.Prefix_Change_Default = args.change
        self.Prefix_List_Size_Default = args.items
        if self.Prefix_List_Size_Default < self.Prefix_Change_Default:
            self.Prefix_Change_Default = self.Prefix_List_Size_Default  # defined change could not be reache
        self.Sweep_Range_Default = args.sweep
    ...
    def compose_update_message(self):
        ...
        ToAdd = self.Prefix_List_Size_Default
        ToRemove = self.Prefix_List_Size_Default - self.Prefix_Change_Default
        if self.Iteration <= 0:
            ToRemove = 0
        PrefixesToRemove = self.get_prefix_list(self.Iteration - 1, count=ToRemove)
        PrefixesToAdd = self.get_prefix_list(self.Iteration, count=ToAdd)
        if (ToAdd - ToRemove) > self.updates_to_send:
            ToAdd = ToRemove + self.updates_to_send
            PrefixesToAdd = PrefixesToAdd[:ToAdd]
        if (self.Iteration % 100):
            logging.debug('#' * 10 + ' Iteration: ' + str(self.Iteration) + ' (remaining routes: ' + str(self.updates_to_send) + ') ' + '#' * 10)
        else:
            logging.info('#' * 10 + ' Iteration: ' + str(self.Iteration) + ' (remaining routes: ' + str(self.updates_to_send) + ') ' + '#' * 10)
        logging.debug('  Routes going to be added: ' + str(PrefixesToAdd))
        logging.debug('  Routes going to be removed: ' + str(PrefixesToRemove))
        msg_out = self.UPDATE(WR_Prefixes=PrefixesToRemove, NLRI_Prefixes=PrefixesToAdd)
        self.Iteration += 1
        self.updates_to_send -= (ToAdd - ToRemove)
        return msg_out

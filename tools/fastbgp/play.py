...
class ReadTracker(object):
    ...
    def decode_path_attributes(self, path_attributes_hex):
        """Decode an Path Attributes field (rfc4271#section-4.3)
...
#            attr_partial_bit = attr_flags & 32
            ...
            if attr_type_code == 1:
                logger.debug("Attribude type = 1 (ORIGIN, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 2:
                logger.debug("Attribude type = 2 (AS_PATH, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 3:
                logger.debug("Attribude type = 3 (NEXT_HOP, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 4:
                logger.debug("Attribude type = 4 (MULTI_EXIT_DISC, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 5:
                logger.debug("Attribude type = 5 (LOCAL_PREF, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 6:
                logger.debug("Attribude type = 6 (ATOMIC_AGGREGATE, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 7:
                logger.debug("Attribude type = 7 (AGGREGATOR, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
            elif attr_type_code == 14:    # rfc4760#section-3
                logger.debug("Attribude type = 14 (MP_REACH_NLRI, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
                ...
                logger.debug("Attribude type = 15 (MP_UNREACH_NLRI, flags:0x%s)",
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
                address_family_identifier_hex = attr_value_hex[0:2]
                logger.debug("  Address Family Identifier = 0x%s",
                ...
                logger.debug("Unknown attribute type = %s, flags:0x%s)", attr_type_code,
                             binascii.b2a_hex(attr_flags_hex))
                logger.debug("Unknown attribude value = 0x%s", binascii.b2a_hex(attr_value_hex))
        return None

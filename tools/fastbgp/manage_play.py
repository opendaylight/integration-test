...
Only a subset of play.py functionality is supported.
...
def main():
    ...
    str_help = "initial number to add to myip to get BGP ID. Default zero."
    parser.add_argument("--bgpidoffset", default="0", type=int, help=str_help)
    ...
    str_help = "How many utilities are to be started."
    ...
    port_current = mgr_args.myport
    offset_current = mgr_args.bgpidoffset
    processes = []
    while utils_left:
        ...
        util_args.extend(["--bgpidoffset", str(offset_current)])
        util_args.extend(["--firstprefix", str(prefix_current)])
        if mgr_args.listen:
            util_args.append("--listen")
        util_args.extend(["--myip", str(mgr_args.myip)])
        util_args.extend(["--myport", str(port_current)])

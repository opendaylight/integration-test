...
Install Neutron
    ...
    Run Keyword If    2 < ${NUM_NEUTRON_NODES}    Create And Configure Neutron Db Other Nodes    ${OS_NEUTRON_1_IP}    root    mysql    ${OS_NEUTRON_2_HOSTNAME}
    Run Keyword If    2 < ${NUM_NEUTRON_NODES}    Create And Configure Neutron Db Other Nodes    ${OS_NEUTRON_1_IP}    root    mysql    ${OS_NEUTRON_3_HOSTNAME}
    Create Openstack Elements    ${HAPROXY_HOSTNAME}
    Install Configure Neutron    ${OS_NEUTRON_1_IP}    ${OS_NEUTRON_1_IP}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}
    ...    ${HAPROXY_HOSTNAME}
    Run Keyword If    2 < ${NUM_NEUTRON_NODES}    Install Configure Neutron    ${OS_NEUTRON_2_IP}    ${OS_NEUTRON_2_IP}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}
    ...    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}
    Run Keyword If    2 < ${NUM_NEUTRON_NODES}    Install Configure Neutron    ${OS_NEUTRON_3_IP}    ${OS_NEUTRON_3_IP}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}
    ...    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}    ${HAPROXY_HOSTNAME}
    ...
    Run Keyword If    2 < ${NUM_NEUTRON_NODES}    Generic HAProxy Entry    ${HAPROXY_IP}    ${HAPROXY_IP}    9796    neutron_server
    ...    9696    ${OS_NEUTRON_1_IP}    ${OS_NEUTRON_2_IP}    ${OS_NEUTRON_3_IP}
...
Create Openstack Elements
    ...
    Create Endpoint    RegionOne    ${host_name}    network    public    9796    rc_file=/tmp/stackrc
    Create Endpoint    RegionOne    ${host_name}    network    internal    9796    rc_file=/tmp/stackrc
    Create Endpoint    RegionOne    ${host_name}    network    admin    9796    rc_file=/tmp/stackrc

Install Configure Neutron
    [Arguments]    ${os_node_cxn}    ${host_ip}    ${host_name}    ${db_host_name}    ${auth_host_name}    ${odl_host_name}
    ...    ${neutron_host_name}
    ...
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    database    connection    "mysql+pymysql://neutron:neutron@${db_host_name}/neutron"
    ...
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    DEFAULT    dhcp_agents_per_network    ${NUM_NEUTRON_NODES}
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    keystone_authtoken    auth_uri    http://${auth_host_name}:5000
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    keystone_authtoken    auth_url    http://${auth_host_name}:35357
    ...
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    nova    auth_uri    http://${auth_host_name}:5000
    Crudini Edit    ${os_node_cxn}    /etc/neutron/neutron.conf    nova    auth_url    http://${auth_host_name}:35357
    ...
    Crudini Edit    ${os_node_cxn}    /etc/nova/nova.conf    neutron    url    http://${neutron_host_name}:9796
    Crudini Edit    ${os_node_cxn}    /etc/nova/nova.conf    neutron    auth_url    http://${auth_host_name}:35357
    ...
    Crudini Edit    ${os_node_cxn}    /etc/neutron/plugins/ml2/ml2_conf.ini    ml2_odl    url    http://${odl_host_name}:8080/controller/nb/v2/neutron
...
Add ODL As Ovs Manager
    [Arguments]    ${os_node_cxn}
    Wait Until Keyword Succeeds    10x    10s    Check If ODL Is Ready    ${OS_ODL_1_IP}
    ...
    Run Keyword If    2 > ${NUM_ODL_NODES}    Run Command    ${os_node_cxn}    sudo ovs-vsctl add-br br-int

Check If ODL Is Ready
    [Arguments]    ${os_node_cxn}
    Run Command    ${os_node_cxn}    sudo netstat -tunpl | grep java | grep 6640
    Run Command    ${os_node_cxn}    sudo netstat -tunpl | grep java | grep 6653
    Run Command    ${os_node_cxn}    sudo netstat -tunpl | grep java | grep 8080
    Run Command    ${os_node_cxn}    sudo netstat -tunpl | grep java | grep 8181
    Run Command    ${os_node_cxn}    sudo netstat -tunpl | grep java | grep 8185

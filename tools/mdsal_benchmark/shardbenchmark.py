...
parser.add_argument("--dataitems", default=[100], nargs='+', help="Number of per-shard data items.")
parser.add_argument("--shards", default=[1], nargs='+', help="Total number of created shards.")
parser.add_argument("--operations", default=[100], nargs='+', help="Number of operations durinf SOAK-TEST.")
...
if __name__ == "__main__":
               ...
               for sht in args.shard_type:
                   for lis in args.listeners:
                       for nrshards in args.shards:
                           for items in args.dataitems:
                               for opers in args.operations:
                                   for puts in args.putspertx:
                                       request_args = { "TEST_TYPE":test_type, "DATASTORE": ds, "SHARDS": nrshards, "SHARDTYPE": sht, "DATAITEMS": items, "OPERATIONS": opers,
    ...
    if args.csv is not None:
        for test in all_results:
            rdet = test["request"]
            column_name='{}_{}DS_{}_SHARDS{}_ITEMS{}_LISTEN{}_OPER{}_PUTSPERTX{}'.format(rdet["SHARDTYPE"], rdet["DATASTORE"], rdet["TEST_TYPE"], rdet["SHARDS"], rdet["DATAITEMS"], rdet["LISTENERS"], rdet["OPERATIONS"], rdet["PUTSPERTX"] )

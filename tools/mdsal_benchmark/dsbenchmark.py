...
parser.add_argument("--plot", type=str, default='',
                    help="keywords filter for results to be drawn in plot")
...
def store_result(values, tx_type, operation, data_fmt, outer_elem, inner_elem, ops_per_tx, value_name, value):
    """
    Sores a record to the list (dictionary) of values to be written into a csv file for plotting purposes.
    ...
    :param inner_elem: Number of elements in the inner list
    :param ops_per_tx: Number of operations (PUTs, MERGEs or DELETEs) on each transaction
    :param value_name: Value name (result)
    :param value: The (measured) value
    :return: none
    """
    plot_key = (tx_type + '-' + operation + '-' + data_fmt + '-' + str(outer_elem) + '-OUTER-' +
                str(inner_elem) + '-INNER-' + str(ops_per_tx) + '-OP-' + value_name)
...
def write_results_to_file(values, file_name, key_filter):
    ...
    :param values: # the list (dictionary) to be written into the filet
    :param key_filter: A regexp string to filter the results to be put into the file
    :return: none
    """
    first_line = ''
    second_line = ''
    ...
    try:
        for key in sorted(values):
            if re.search(key_filter, key) is not None:
                first_line += key + ', '
...
if __name__ == "__main__":
    ...
    try:
                ...
                for oper in OPERATIONS:
                    ...
                    for elem in INNER_ELEMENTS:
                        ...
                        store_result(PLOT1, tx_type, oper, fmt, TOTAL_ELEMENTS / elem, elem, 1,
                                     'BUILD-TIME', avg_build_time)
                        store_result(PLOT1, tx_type, oper, fmt, TOTAL_ELEMENTS / elem, elem, 1,
                                     'EXEC-TIME', avg_exec_time)
        ...
        write_results_to_file(PLOT1, 'plot1.csv', PLOT_FILTER)
        write_results_to_file(PLOT2, 'plot2.csv', PLOT_FILTER)

...
AuthStandalone library is used to handle restconf authentication.
...
try:
    ...
    import Counter  # Assumes user copies Counter.py around.
...
def str2bool(text):
    """based onhttp://stackoverflow.com/a/19227287"""
...
def iterable_msg(pccs, lsps, workers, hop):
    """Generator yielding tuple of worker number and restconf data it should send."""
    ...
    for lsp in range(1, lsps + 1):
        ...
        for pcc in range(pccs):
            ...
            # TODO: Make AuthStandalone understand *args correctly.
            post_tuple = (uri, post_kwargs)
            yield worker, post_tuple
...
def queued_send(session, queue_messages, queue_responses):
    ...
    while 1:
        try:
            uri, kwargs = queue_messages.popleft()
        except IndexError:  # nothing more to send
            return
        # The response perhaps holds some resources, so we close it.
        with AuthStandalone.Post_Using_Session(session, uri, **kwargs) as response:
            status = int(response.status_code)  # shallow copy
            content = str(response.content)  # shallow copy
...
for worker, post_tuple in iterable_msg(args.pccs, args.lsps, args.workers, args.hop):
    # print 'DEBUG: worker', repr(worker), 'message', repr(message)
    list_q_msg[worker].append(post_tuple)
queue_responses = collections.deque()  # thread safe
threads = []
for worker in range(args.workers):
    print repr(args.reuse)

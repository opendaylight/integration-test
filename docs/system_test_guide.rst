...
Replace $project with your project name and $functionality with the functionality
you want to test.
...
Wait until int/test devs approves and merges your changes.
...
* Push you new csit job to sandbox::
  ...
  jenkins-jobs --conf jenkins.ini test jjb/ $project-csit-1node-$functionality-only-$branch
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Your system test jobs can be browsed in jenkins-releng_:
